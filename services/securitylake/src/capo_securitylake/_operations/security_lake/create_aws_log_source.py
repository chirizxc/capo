"""Generated from Smithy shape ``com.amazonaws.securitylake#CreateAwsLogSource``."""
...
def handle_response(
    ...
    out: capo_securitylake.types.create_aws_log_source_response.CreateAwsLogSourceResponse = capo_securitylake.types.create_aws_log_source_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/datalake/logsources/aws"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securitylake.types.create_aws_log_source_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_aws_log_source(
