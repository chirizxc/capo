"""Generated from Smithy shape ``com.amazonaws.securitylake#DeleteDataLake``."""
...
def handle_response(
    ...
    out: capo_securitylake.types.delete_data_lake_response.DeleteDataLakeResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/datalake/delete"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securitylake.types.delete_data_lake_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_delete_data_lake(
