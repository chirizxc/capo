...
def handle_response(
    ...
    out: capo_securitylake.types.update_subscriber_notification_response.UpdateSubscriberNotificationResponse = capo_securitylake.types.update_subscriber_notification_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{subscriberId}", quote(input_["subscriber_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securitylake.types.update_subscriber_notification_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
