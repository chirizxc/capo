"""Generated from Smithy shape ``com.amazonaws.securitylake#ListDataLakes``."""
...
def handle_response(
    ...
    out: capo_securitylake.types.list_data_lakes_response.ListDataLakesResponse = (
        capo_securitylake.types.list_data_lakes_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/datalakes"
    params: list[tuple[str, str]] = []
    if "regions" in input_:
        for item in input_["regions"]:
            params.append(("regions", item))
...
async def async_list_data_lakes(
