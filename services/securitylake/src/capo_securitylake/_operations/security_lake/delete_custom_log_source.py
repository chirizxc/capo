"""Generated from Smithy shape ``com.amazonaws.securitylake#DeleteCustomLogSource``."""
...
import capo_securitylake._protocol.eventstream
...
def handle_response(
    ...
    out: capo_securitylake.types.delete_custom_log_source_response.DeleteCustomLogSourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_securitylake._auth._sigv4.build_sigv4_auth_scheme(
                "securitylake", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/datalake/logsources/custom/{sourceName}"
    url = url.replace("{sourceName}", quote(input_["source_name"], safe=""))
    params: list[tuple[str, str]] = []
    if "source_version" in input_:
        params.append(("sourceVersion", input_["source_version"]))
...
async def async_delete_custom_log_source(
