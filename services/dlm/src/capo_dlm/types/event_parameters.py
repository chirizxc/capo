...
if TYPE_CHECKING:
    import capo_dlm.types.description_regex
...
class EventParameters(TypedDict, closed=True):
    event_type: NotRequired["capo_dlm.types.event_type_values.EventTypeValues"]
    """<p>The type of event. Currently, only snapshot sharing events are supported.</p>"""
    snapshot_owner: NotRequired["capo_dlm.types.snapshot_owner_list.SnapshotOwnerList"]
    ...
    description_regex: NotRequired["capo_dlm.types.description_regex.DescriptionRegex"]
...
def serialize_json(value: EventParameters) -> dict:
    ...
    if "event_type" in value:
        ...
        out["EventType"] = capo_dlm.types.event_type_values.serialize_json(
            value["event_type"]
        )
    if "snapshot_owner" in value:
        import capo_dlm.types.snapshot_owner_list

        out["SnapshotOwner"] = capo_dlm.types.snapshot_owner_list.serialize_json(
