...
if TYPE_CHECKING:
    import capo_sagemaker.types.trial_component_simple_summary

TrialComponentSimpleSummaries: TypeAlias = list[
    "capo_sagemaker.types.trial_component_simple_summary.TrialComponentSimpleSummary"
]
...
def serialize_aws_json_1_1(value: TrialComponentSimpleSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.trial_component_simple_summary.serialize_aws_json_1_1(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> TrialComponentSimpleSummaries:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.trial_component_simple_summary.deserialize_aws_json_1_1(
                item
            )
        )
    return out
