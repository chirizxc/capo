...
if TYPE_CHECKING:
    import capo_sagemaker.types.default_gid
    import capo_sagemaker.types.default_uid
    import capo_sagemaker.types.mount_path


class FileSystemConfig(TypedDict, closed=True):
    mount_path: NotRequired["capo_sagemaker.types.mount_path.MountPath"]
    ...
    default_uid: NotRequired["capo_sagemaker.types.default_uid.DefaultUid"]
    """<p>The default POSIX user ID (UID). If not specified, defaults to <code>1000</code>.</p>"""
    default_gid: NotRequired["capo_sagemaker.types.default_gid.DefaultGid"]
