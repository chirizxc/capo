...
if TYPE_CHECKING:
    import capo_sagemaker.types.ai_recommendation_instance_detail

AIRecommendationInstanceDetailList: TypeAlias = list[
    "capo_sagemaker.types.ai_recommendation_instance_detail.AIRecommendationInstanceDetail"
]
...
def serialize_aws_json_1_1(value: AIRecommendationInstanceDetailList) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.ai_recommendation_instance_detail.serialize_aws_json_1_1(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> AIRecommendationInstanceDetailList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.ai_recommendation_instance_detail.deserialize_aws_json_1_1(
                item
            )
        )
    return out
