...
if TYPE_CHECKING:
    import capo_sagemaker.types.tracking_server_summary

TrackingServerSummaryList: TypeAlias = list[
    "capo_sagemaker.types.tracking_server_summary.TrackingServerSummary"
]
...
def serialize_aws_json_1_1(value: TrackingServerSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.tracking_server_summary.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> TrackingServerSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.tracking_server_summary.deserialize_aws_json_1_1(item)
        )
    return out
