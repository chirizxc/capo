"""Generated from Smithy shape ``com.amazonaws.sagemaker#TransformDataSource``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_sagemaker.types.transform_s3_data_source


class TransformDataSource(TypedDict, closed=True):
    s3_data_source: NotRequired[
        "capo_sagemaker.types.transform_s3_data_source.TransformS3DataSource"
    ]
    """<p>The S3 location of the data source that is associated with a channel.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: TransformDataSource) -> dict:
    out: dict = {}
    if "s3_data_source" in value:
        import capo_sagemaker.types.transform_s3_data_source

        out["S3DataSource"] = (
            capo_sagemaker.types.transform_s3_data_source.serialize_aws_json_1_1(
                value["s3_data_source"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> TransformDataSource:
    out: TransformDataSource = {}  # type: ignore[typeddict-item]
    if data.get("S3DataSource") is not None:
        import capo_sagemaker.types.transform_s3_data_source

        out["s3_data_source"] = (
            capo_sagemaker.types.transform_s3_data_source.deserialize_aws_json_1_1(
                data["S3DataSource"]
            )
        )
    return out
