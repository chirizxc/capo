...
if TYPE_CHECKING:
    import capo_sagemaker.types.async_notification_topic_types

AsyncNotificationTopicTypeList: TypeAlias = list[
    "capo_sagemaker.types.async_notification_topic_types.AsyncNotificationTopicTypes"
]
...
def serialize_aws_json_1_1(value: AsyncNotificationTopicTypeList) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.async_notification_topic_types.serialize_aws_json_1_1(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> AsyncNotificationTopicTypeList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.async_notification_topic_types.deserialize_aws_json_1_1(
                item
            )
        )
    return out
