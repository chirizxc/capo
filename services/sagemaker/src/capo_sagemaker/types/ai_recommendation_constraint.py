"""Generated from Smithy shape ``com.amazonaws.sagemaker#AIRecommendationConstraint``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_sagemaker.types.ai_recommendation_metric


class AIRecommendationConstraint(TypedDict, closed=True):
    metric: NotRequired[
        "capo_sagemaker.types.ai_recommendation_metric.AIRecommendationMetric"
    ]
    """<p>The performance metric. Valid values are <code>ttft-ms</code> (time to first token in milliseconds), <code>throughput</code>, and <code>cost</code>.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: AIRecommendationConstraint) -> dict:
    out: dict = {}
    if "metric" in value:
        import capo_sagemaker.types.ai_recommendation_metric

        out["Metric"] = (
            capo_sagemaker.types.ai_recommendation_metric.serialize_aws_json_1_1(
                value["metric"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> AIRecommendationConstraint:
    out: AIRecommendationConstraint = {}  # type: ignore[typeddict-item]
    if data.get("Metric") is not None:
        import capo_sagemaker.types.ai_recommendation_metric

        out["metric"] = (
            capo_sagemaker.types.ai_recommendation_metric.deserialize_aws_json_1_1(
                data["Metric"]
            )
        )
    return out
