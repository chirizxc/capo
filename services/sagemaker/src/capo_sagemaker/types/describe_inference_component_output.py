"""Generated from Smithy shape ``com.amazonaws.sagemaker#DescribeInferenceComponentOutput``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_sagemaker.types.endpoint_arn
    import capo_sagemaker.types.endpoint_name
    import capo_sagemaker.types.failure_reason
    import capo_sagemaker.types.inference_component_arn
    import capo_sagemaker.types.inference_component_deployment_config
    import capo_sagemaker.types.inference_component_name
    import capo_sagemaker.types.inference_component_runtime_config_summary
    import capo_sagemaker.types.inference_component_specification_summary
    import capo_sagemaker.types.inference_component_specification_summary_list
    import capo_sagemaker.types.inference_component_status
    import capo_sagemaker.types.timestamp
    import capo_sagemaker.types.variant_name


class DescribeInferenceComponentOutput(TypedDict, closed=True):
    inference_component_name: NotRequired[
        "capo_sagemaker.types.inference_component_name.InferenceComponentName"
    ]
    """<p>The name of the inference component.</p>"""
    inference_component_arn: NotRequired[
        "capo_sagemaker.types.inference_component_arn.InferenceComponentArn"
    ]
    """<p>The Amazon Resource Name (ARN) of the inference component.</p>"""
    endpoint_name: NotRequired["capo_sagemaker.types.endpoint_name.EndpointName"]
    """<p>The name of the endpoint that hosts the inference component.</p>"""
    endpoint_arn: NotRequired["capo_sagemaker.types.endpoint_arn.EndpointArn"]
    """<p>The Amazon Resource Name (ARN) of the endpoint that hosts the inference component.</p>"""
    variant_name: NotRequired["capo_sagemaker.types.variant_name.VariantName"]
    """<p>The name of the production variant that hosts the inference component.</p>"""
    failure_reason: NotRequired["capo_sagemaker.types.failure_reason.FailureReason"]
    """<p>If the inference component status is <code>Failed</code>, the reason for the failure.</p>"""
    specification: NotRequired[
        "capo_sagemaker.types.inference_component_specification_summary.InferenceComponentSpecificationSummary"
    ]
    """<p>Details about the resources that are deployed with this inference component.</p>"""
    specifications: NotRequired[
        "capo_sagemaker.types.inference_component_specification_summary_list.InferenceComponentSpecificationSummaryList"
    ]
    """<p>A list of specification summaries for the inference component, one per instance type. This parameter is populated when the inference component was created with multiple specifications. When this parameter is populated, the singular <code>Specification</code> parameter is not returned.</p>"""
    runtime_config: NotRequired[
        "capo_sagemaker.types.inference_component_runtime_config_summary.InferenceComponentRuntimeConfigSummary"
    ]
    """<p>Details about the runtime settings for the model that is deployed with the inference component.</p>"""
    creation_time: NotRequired["capo_sagemaker.types.timestamp.Timestamp"]
    """<p>The time when the inference component was created.</p>"""
    last_modified_time: NotRequired["capo_sagemaker.types.timestamp.Timestamp"]
    """<p>The time when the inference component was last updated.</p>"""
    inference_component_status: NotRequired[
        "capo_sagemaker.types.inference_component_status.InferenceComponentStatus"
    ]
    """<p>The status of the inference component.</p>"""
    last_deployment_config: NotRequired[
        "capo_sagemaker.types.inference_component_deployment_config.InferenceComponentDeploymentConfig"
    ]
    """<p>The deployment and rollback settings that you assigned to the inference component.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: DescribeInferenceComponentOutput) -> dict:
    out: dict = {}
    if "inference_component_name" in value:
        out["InferenceComponentName"] = value["inference_component_name"]
    if "inference_component_arn" in value:
        out["InferenceComponentArn"] = value["inference_component_arn"]
    if "endpoint_name" in value:
        out["EndpointName"] = value["endpoint_name"]
    if "endpoint_arn" in value:
        out["EndpointArn"] = value["endpoint_arn"]
    if "variant_name" in value:
        out["VariantName"] = value["variant_name"]
    if "failure_reason" in value:
        out["FailureReason"] = value["failure_reason"]
    if "specification" in value:
        import capo_sagemaker.types.inference_component_specification_summary

        out["Specification"] = (
            capo_sagemaker.types.inference_component_specification_summary.serialize_aws_json_1_1(
                value["specification"]
            )
        )
    if "specifications" in value:
        import capo_sagemaker.types.inference_component_specification_summary_list

        out["Specifications"] = (
            capo_sagemaker.types.inference_component_specification_summary_list.serialize_aws_json_1_1(
                value["specifications"]
            )
        )
    if "runtime_config" in value:
        import capo_sagemaker.types.inference_component_runtime_config_summary

        out["RuntimeConfig"] = (
            capo_sagemaker.types.inference_component_runtime_config_summary.serialize_aws_json_1_1(
                value["runtime_config"]
            )
        )
    if "creation_time" in value:
        import capo_sagemaker.types.timestamp

        out["CreationTime"] = capo_sagemaker.types.timestamp.serialize_aws_json_1_1(
            value["creation_time"]
        )
    if "last_modified_time" in value:
        import capo_sagemaker.types.timestamp

        out["LastModifiedTime"] = capo_sagemaker.types.timestamp.serialize_aws_json_1_1(
            value["last_modified_time"]
        )
    if "inference_component_status" in value:
        import capo_sagemaker.types.inference_component_status

        out["InferenceComponentStatus"] = (
            capo_sagemaker.types.inference_component_status.serialize_aws_json_1_1(
                value["inference_component_status"]
            )
        )
    if "last_deployment_config" in value:
        import capo_sagemaker.types.inference_component_deployment_config

        out["LastDeploymentConfig"] = (
            capo_sagemaker.types.inference_component_deployment_config.serialize_aws_json_1_1(
                value["last_deployment_config"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> DescribeInferenceComponentOutput:
    out: DescribeInferenceComponentOutput = {}  # type: ignore[typeddict-item]
    if data.get("InferenceComponentName") is not None:
        out["inference_component_name"] = data["InferenceComponentName"]
    if data.get("InferenceComponentArn") is not None:
        out["inference_component_arn"] = data["InferenceComponentArn"]
    if data.get("EndpointName") is not None:
        out["endpoint_name"] = data["EndpointName"]
    if data.get("EndpointArn") is not None:
        out["endpoint_arn"] = data["EndpointArn"]
    if data.get("VariantName") is not None:
        out["variant_name"] = data["VariantName"]
    if data.get("FailureReason") is not None:
        out["failure_reason"] = data["FailureReason"]
    if data.get("Specification") is not None:
        import capo_sagemaker.types.inference_component_specification_summary

        out["specification"] = (
            capo_sagemaker.types.inference_component_specification_summary.deserialize_aws_json_1_1(
                data["Specification"]
            )
        )
    if data.get("Specifications") is not None:
        import capo_sagemaker.types.inference_component_specification_summary_list

        out["specifications"] = (
            capo_sagemaker.types.inference_component_specification_summary_list.deserialize_aws_json_1_1(
                data["Specifications"]
            )
        )
    if data.get("RuntimeConfig") is not None:
        import capo_sagemaker.types.inference_component_runtime_config_summary

        out["runtime_config"] = (
            capo_sagemaker.types.inference_component_runtime_config_summary.deserialize_aws_json_1_1(
                data["RuntimeConfig"]
            )
        )
    if data.get("CreationTime") is not None:
        import capo_sagemaker.types.timestamp

        out["creation_time"] = capo_sagemaker.types.timestamp.deserialize_aws_json_1_1(
            data["CreationTime"]
        )
    if data.get("LastModifiedTime") is not None:
        import capo_sagemaker.types.timestamp

        out["last_modified_time"] = (
            capo_sagemaker.types.timestamp.deserialize_aws_json_1_1(
                data["LastModifiedTime"]
            )
        )
    if data.get("InferenceComponentStatus") is not None:
        import capo_sagemaker.types.inference_component_status

        out["inference_component_status"] = (
            capo_sagemaker.types.inference_component_status.deserialize_aws_json_1_1(
                data["InferenceComponentStatus"]
            )
        )
    if data.get("LastDeploymentConfig") is not None:
        import capo_sagemaker.types.inference_component_deployment_config

        out["last_deployment_config"] = (
            capo_sagemaker.types.inference_component_deployment_config.deserialize_aws_json_1_1(
                data["LastDeploymentConfig"]
            )
        )
    return out
