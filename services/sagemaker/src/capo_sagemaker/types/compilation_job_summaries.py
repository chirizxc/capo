...
if TYPE_CHECKING:
    import capo_sagemaker.types.compilation_job_summary

CompilationJobSummaries: TypeAlias = list[
    "capo_sagemaker.types.compilation_job_summary.CompilationJobSummary"
]
...
def serialize_aws_json_1_1(value: CompilationJobSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.compilation_job_summary.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> CompilationJobSummaries:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.compilation_job_summary.deserialize_aws_json_1_1(item)
        )
    return out
