...
if TYPE_CHECKING:
    import capo_sagemaker.types.batch_add_cluster_nodes_error

BatchAddClusterNodesErrorList: TypeAlias = list[
    "capo_sagemaker.types.batch_add_cluster_nodes_error.BatchAddClusterNodesError"
]
...
def serialize_aws_json_1_1(value: BatchAddClusterNodesErrorList) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.batch_add_cluster_nodes_error.serialize_aws_json_1_1(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> BatchAddClusterNodesErrorList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.batch_add_cluster_nodes_error.deserialize_aws_json_1_1(
                item
            )
        )
    return out
