...
if TYPE_CHECKING:
    ...
    import capo_sagemaker.types.resource_id
...
class DescribeDomainResponse(TypedDict, closed=True):
    ...
    """<p>The domain name.</p>"""
    home_efs_file_system_id: NotRequired["capo_sagemaker.types.resource_id.ResourceId"]
    ...
    status: NotRequired["capo_sagemaker.types.domain_status.DomainStatus"]
    """<p>The status.</p>"""
...
def serialize_aws_json_1_1(value: DescribeDomainResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_sagemaker.types.domain_status.serialize_aws_json_1_1(
            value["status"]
        )
...
def deserialize_aws_json_1_1(data: dict) -> DescribeDomainResponse:
    ...
    if data.get("HomeEfsFileSystemId") is not None:
        out["home_efs_file_system_id"] = data["HomeEfsFileSystemId"]
    if data.get("SingleSignOnManagedApplicationInstanceId") is not None:
    ...
    if data.get("SecurityGroupIdForDomainBoundary") is not None:
