...
if TYPE_CHECKING:
    import capo_sagemaker.types.device_deployment_summary

DeviceDeploymentSummaries: TypeAlias = list[
    "capo_sagemaker.types.device_deployment_summary.DeviceDeploymentSummary"
]
...
def serialize_aws_json_1_1(value: DeviceDeploymentSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.device_deployment_summary.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> DeviceDeploymentSummaries:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.device_deployment_summary.deserialize_aws_json_1_1(
                item
            )
        )
    return out
