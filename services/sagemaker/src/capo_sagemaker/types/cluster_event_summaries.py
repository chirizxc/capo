...
if TYPE_CHECKING:
    import capo_sagemaker.types.cluster_event_summary

ClusterEventSummaries: TypeAlias = list[
    "capo_sagemaker.types.cluster_event_summary.ClusterEventSummary"
]
...
def serialize_aws_json_1_1(value: ClusterEventSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.cluster_event_summary.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> ClusterEventSummaries:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.cluster_event_summary.deserialize_aws_json_1_1(item)
        )
    return out
