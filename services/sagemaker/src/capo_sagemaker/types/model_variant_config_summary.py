...
class ModelVariantConfigSummary(TypedDict, closed=True):
    ...
    status: NotRequired["capo_sagemaker.types.model_variant_status.ModelVariantStatus"]
...
def serialize_aws_json_1_1(value: ModelVariantConfigSummary) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            capo_sagemaker.types.model_variant_status.serialize_aws_json_1_1(
                value["status"]
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> ModelVariantConfigSummary:
    ...
    if data.get("Status") is not None:
        ...
        out["status"] = (
            capo_sagemaker.types.model_variant_status.deserialize_aws_json_1_1(
                data["Status"]
            )
        )
    return out
