"""Generated from Smithy shape ``com.amazonaws.sagemaker#FileSource``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_sagemaker.types.content_digest
    import capo_sagemaker.types.content_type
    import capo_sagemaker.types.s3_uri


class FileSource(TypedDict, closed=True):
    content_type: NotRequired["capo_sagemaker.types.content_type.ContentType"]
    """<p>The type of content stored in the file source.</p>"""
    content_digest: NotRequired["capo_sagemaker.types.content_digest.ContentDigest"]
    """<p>The digest of the file source.</p>"""
    s3_uri: NotRequired["capo_sagemaker.types.s3_uri.S3Uri"]
    """<p>The Amazon S3 URI for the file source.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: FileSource) -> dict:
    out: dict = {}
    if "content_type" in value:
        out["ContentType"] = value["content_type"]
    if "content_digest" in value:
        out["ContentDigest"] = value["content_digest"]
    if "s3_uri" in value:
        out["S3Uri"] = value["s3_uri"]
    return out


def deserialize_aws_json_1_1(data: dict) -> FileSource:
    out: FileSource = {}  # type: ignore[typeddict-item]
    if data.get("ContentType") is not None:
        out["content_type"] = data["ContentType"]
    if data.get("ContentDigest") is not None:
        out["content_digest"] = data["ContentDigest"]
    if data.get("S3Uri") is not None:
        out["s3_uri"] = data["S3Uri"]
    return out
