"""Generated from Smithy shape ``com.amazonaws.sagemaker#AlgorithmValidationProfiles``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_sagemaker.types.algorithm_validation_profile

AlgorithmValidationProfiles: TypeAlias = list[
    "capo_sagemaker.types.algorithm_validation_profile.AlgorithmValidationProfile"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: AlgorithmValidationProfiles) -> list:
    import capo_sagemaker.types.algorithm_validation_profile

    out: list = []
    for item in value:
        out.append(
            capo_sagemaker.types.algorithm_validation_profile.serialize_aws_json_1_1(
                item
            )
        )
    return out


def deserialize_aws_json_1_1(data: list) -> AlgorithmValidationProfiles:
    import capo_sagemaker.types.algorithm_validation_profile

    out: AlgorithmValidationProfiles = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_sagemaker.types.algorithm_validation_profile.deserialize_aws_json_1_1(
                item
            )
        )
    return out
