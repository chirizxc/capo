"""Generated from Smithy shape ``com.amazonaws.backupsearch#ListSearchJobBackups``."""
...
def handle_response(
    ...
    out: capo_backupsearch.types.list_search_job_backups_output.ListSearchJobBackupsOutput = capo_backupsearch.types.list_search_job_backups_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{SearchJobIdentifier}", quote(input_["search_job_identifier"], safe="")
    )
    params: list[tuple[str, str]] = []
    if "next_token" in input_:
        params.append(("nextToken", input_["next_token"]))
    params.append(("maxResults", str(input_.get("max_results", 1000))))
...
async def async_list_search_job_backups(
