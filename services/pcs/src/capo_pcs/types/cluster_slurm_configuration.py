...
class ClusterSlurmConfiguration(TypedDict, closed=True):
    ...
    auth_key: NotRequired["capo_pcs.types.slurm_auth_key.SlurmAuthKey"]
    """<p>The shared Slurm key for authentication, also known as the <b>cluster secret</b>.</p>"""
    jwt_auth: NotRequired["capo_pcs.types.jwt_auth.JwtAuth"]
    """<p>The JWT authentication configuration for Slurm REST API access.</p>"""
    accounting: NotRequired["capo_pcs.types.accounting.Accounting"]
    """<p>The accounting configuration includes configurable settings for Slurm accounting.</p>"""
    slurm_rest: NotRequired["capo_pcs.types.slurm_rest.SlurmRest"]
...
def serialize_aws_json_1_0(value: ClusterSlurmConfiguration) -> dict:
    ...
    if "auth_key" in value:
        ...
        out["authKey"] = capo_pcs.types.slurm_auth_key.serialize_aws_json_1_0(
            value["auth_key"]
        )
    if "jwt_auth" in value:
        import capo_pcs.types.jwt_auth

        out["jwtAuth"] = capo_pcs.types.jwt_auth.serialize_aws_json_1_0(
            value["jwt_auth"]
        )
    if "accounting" in value:
        import capo_pcs.types.accounting

        out["accounting"] = capo_pcs.types.accounting.serialize_aws_json_1_0(
            value["accounting"]
        )
    if "slurm_rest" in value:
        import capo_pcs.types.slurm_rest

        out["slurmRest"] = capo_pcs.types.slurm_rest.serialize_aws_json_1_0(
            value["slurm_rest"]
        )
    return out
