"""Generated from Smithy shape ``com.amazonaws.rtbfabric#RejectLink``."""
...
import capo_rtbfabric._protocol.eventstream
...
def handle_response(
    ...
    out: capo_rtbfabric.types.reject_link_response.RejectLinkResponse = (
        capo_rtbfabric.types.reject_link_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_rtbfabric._auth._sigv4.build_sigv4_auth_scheme(
                "rtbfabric", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/gateway/{gatewayId}/link/{linkId}/reject"
    url = url.replace("{gatewayId}", quote(input_["gateway_id"], safe=""))
    url = url.replace("{linkId}", quote(input_["link_id"], safe=""))
...
def reject_link(
    ...
    capo_rtbfabric.types.reject_link_response.RejectLinkResponse, zapros.Response
]:
