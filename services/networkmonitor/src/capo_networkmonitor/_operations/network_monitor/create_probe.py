"""Generated from Smithy shape ``com.amazonaws.networkmonitor#CreateProbe``."""
...
def handle_response(
    ...
    out: capo_networkmonitor.types.create_probe_output.CreateProbeOutput = (
        capo_networkmonitor.types.create_probe_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/monitors/{monitorName}/probes"
    url = url.replace("{monitorName}", quote(input_["monitor_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_networkmonitor.types.create_probe_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def create_probe(
    ...
    capo_networkmonitor.types.create_probe_output.CreateProbeOutput, zapros.Response
]:
