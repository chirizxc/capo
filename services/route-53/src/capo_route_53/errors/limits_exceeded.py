"""Generated from Smithy shape ``com.amazonaws.route53#LimitsExceeded``."""
...
class LimitsExceeded_(TypedDict, closed=True):
    message: NotRequired["capo_route_53.types.error_message.ErrorMessage"]
    """<p></p>"""


# --- restXml ser/de ---
def serialize_xml(value: LimitsExceeded_, parent: Element, tag: str) -> None:
    el = SubElement(parent, tag)
    if "message" in value:
        SubElement(el, "message").text = str(value["message"])


def deserialize_xml(el: Element) -> LimitsExceeded_:
    out: LimitsExceeded_ = {}  # type: ignore[typeddict-item]
...
class LimitsExceeded(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.route53#LimitsExceeded``."""
