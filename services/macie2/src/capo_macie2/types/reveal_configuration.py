...
class RevealConfiguration(TypedDict, closed=True):
    ...
    status: NotRequired["capo_macie2.types.reveal_status.RevealStatus"]
...
def serialize_json(value: RevealConfiguration) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_macie2.types.reveal_status.serialize_json(value["status"])
    return out
