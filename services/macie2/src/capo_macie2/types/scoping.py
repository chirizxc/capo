"""Generated from Smithy shape ``com.amazonaws.macie2#Scoping``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_macie2.types.job_scoping_block


class Scoping(TypedDict, closed=True):
    excludes: NotRequired["capo_macie2.types.job_scoping_block.JobScopingBlock"]
    """<p>The property- and tag-based conditions that determine which objects to exclude from the analysis.</p>"""
    includes: NotRequired["capo_macie2.types.job_scoping_block.JobScopingBlock"]
    """<p>The property- and tag-based conditions that determine which objects to include in the analysis.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: Scoping) -> dict:
    out: dict = {}
    if "excludes" in value:
        import capo_macie2.types.job_scoping_block

        out["excludes"] = capo_macie2.types.job_scoping_block.serialize_json(
            value["excludes"]
        )
    if "includes" in value:
        import capo_macie2.types.job_scoping_block

        out["includes"] = capo_macie2.types.job_scoping_block.serialize_json(
            value["includes"]
        )
    return out


def deserialize_json(data: dict) -> Scoping:
    out: Scoping = {}  # type: ignore[typeddict-item]
    if data.get("excludes") is not None:
        import capo_macie2.types.job_scoping_block

        out["excludes"] = capo_macie2.types.job_scoping_block.deserialize_json(
            data["excludes"]
        )
    if data.get("includes") is not None:
        import capo_macie2.types.job_scoping_block

        out["includes"] = capo_macie2.types.job_scoping_block.deserialize_json(
            data["includes"]
        )
    return out
