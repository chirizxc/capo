...
class ListResourceProfileArtifactsResponse(TypedDict, closed=True):
    artifacts: NotRequired[
        "capo_macie2.types.__list_of_resource_profile_artifact.__listOfResourceProfileArtifact"
    ]
...
def serialize_json(value: ListResourceProfileArtifactsResponse) -> dict:
    ...
    if "artifacts" in value:
        ...
        out["artifacts"] = (
            capo_macie2.types.__list_of_resource_profile_artifact.serialize_json(
                value["artifacts"]
            )
        )
...
def deserialize_json(data: dict) -> ListResourceProfileArtifactsResponse:
    ...
    if data.get("artifacts") is not None:
        ...
        out["artifacts"] = (
            capo_macie2.types.__list_of_resource_profile_artifact.deserialize_json(
                data["artifacts"]
            )
        )
