...
class ListOrganizationAdminAccountsResponse(TypedDict, closed=True):
    admin_accounts: NotRequired[
        "capo_macie2.types.__list_of_admin_account.__listOfAdminAccount"
    ]
...
def serialize_json(value: ListOrganizationAdminAccountsResponse) -> dict:
    ...
    if "admin_accounts" in value:
        ...
        out["adminAccounts"] = capo_macie2.types.__list_of_admin_account.serialize_json(
...
def deserialize_json(data: dict) -> ListOrganizationAdminAccountsResponse:
    ...
    if data.get("adminAccounts") is not None:
        ...
        out["admin_accounts"] = (
            capo_macie2.types.__list_of_admin_account.deserialize_json(
                data["adminAccounts"]
            )
        )
