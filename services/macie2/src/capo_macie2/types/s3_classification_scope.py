...
if TYPE_CHECKING:
    import capo_macie2.types.s3_classification_scope_exclusion


class S3ClassificationScope(TypedDict, closed=True):
    excludes: NotRequired[
        "capo_macie2.types.s3_classification_scope_exclusion.S3ClassificationScopeExclusion"
    ]
...
def serialize_json(value: S3ClassificationScope) -> dict:
    ...
    if "excludes" in value:
        ...
        out["excludes"] = (
            capo_macie2.types.s3_classification_scope_exclusion.serialize_json(
                value["excludes"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> S3ClassificationScope:
    ...
    if data.get("excludes") is not None:
        ...
        out["excludes"] = (
            capo_macie2.types.s3_classification_scope_exclusion.deserialize_json(
                data["excludes"]
            )
        )
    return out
