...
class SearchResourcesTagCriterion(TypedDict, closed=True):
    ...
    tag_values: NotRequired[
        "capo_macie2.types.__list_of_search_resources_tag_criterion_pair.__listOfSearchResourcesTagCriterionPair"
    ]
...
def serialize_json(value: SearchResourcesTagCriterion) -> dict:
    ...
    if "tag_values" in value:
        ...
        out["tagValues"] = (
            capo_macie2.types.__list_of_search_resources_tag_criterion_pair.serialize_json(
                value["tag_values"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> SearchResourcesTagCriterion:
    ...
    if data.get("tagValues") is not None:
        ...
        out["tag_values"] = (
            capo_macie2.types.__list_of_search_resources_tag_criterion_pair.deserialize_json(
                data["tagValues"]
            )
        )
    return out
