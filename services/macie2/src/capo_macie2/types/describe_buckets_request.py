...
class DescribeBucketsRequest(TypedDict, closed=True):
    criteria: NotRequired["capo_macie2.types.bucket_criteria.BucketCriteria"]
...
def serialize_json(value: DescribeBucketsRequest) -> dict:
    ...
    if "criteria" in value:
        ...
        out["criteria"] = capo_macie2.types.bucket_criteria.serialize_json(
            value["criteria"]
        )
    ...
    if "sort_criteria" in value:
        ...
        out["sortCriteria"] = capo_macie2.types.bucket_sort_criteria.serialize_json(
            value["sort_criteria"]
        )
    return out
