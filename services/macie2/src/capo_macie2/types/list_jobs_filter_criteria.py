...
class ListJobsFilterCriteria(TypedDict, closed=True):
    excludes: NotRequired[
        "capo_macie2.types.__list_of_list_jobs_filter_term.__listOfListJobsFilterTerm"
    ]
...
def serialize_json(value: ListJobsFilterCriteria) -> dict:
    ...
    if "excludes" in value:
        ...
        out["excludes"] = (
            capo_macie2.types.__list_of_list_jobs_filter_term.serialize_json(
                value["excludes"]
            )
        )
...
def deserialize_json(data: dict) -> ListJobsFilterCriteria:
    ...
    if data.get("excludes") is not None:
        ...
        out["excludes"] = (
            capo_macie2.types.__list_of_list_jobs_filter_term.deserialize_json(
                data["excludes"]
            )
        )
