...
class Occurrences(TypedDict, closed=True):
    cells: NotRequired["capo_macie2.types.cells.Cells"]
    ...
    line_ranges: NotRequired["capo_macie2.types.ranges.Ranges"]
    ...
    offset_ranges: NotRequired["capo_macie2.types.ranges.Ranges"]
    """<p>Reserved for future use.</p>"""
    pages: NotRequired["capo_macie2.types.pages.Pages"]
    ...
    records: NotRequired["capo_macie2.types.records.Records"]
...
def serialize_json(value: Occurrences) -> dict:
    ...
    if "cells" in value:
        ...
        out["cells"] = capo_macie2.types.cells.serialize_json(value["cells"])
    if "line_ranges" in value:
        import capo_macie2.types.ranges

        out["lineRanges"] = capo_macie2.types.ranges.serialize_json(
            value["line_ranges"]
        )
    if "offset_ranges" in value:
        import capo_macie2.types.ranges

        out["offsetRanges"] = capo_macie2.types.ranges.serialize_json(
            value["offset_ranges"]
        )
    if "pages" in value:
        import capo_macie2.types.pages

        out["pages"] = capo_macie2.types.pages.serialize_json(value["pages"])
    if "records" in value:
        import capo_macie2.types.records

        out["records"] = capo_macie2.types.records.serialize_json(value["records"])
    return out
