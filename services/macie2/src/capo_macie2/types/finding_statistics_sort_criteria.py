...
if TYPE_CHECKING:
    import capo_macie2.types.finding_statistics_sort_attribute_name
    import capo_macie2.types.order_by


class FindingStatisticsSortCriteria(TypedDict, closed=True):
    attribute_name: NotRequired[
        "capo_macie2.types.finding_statistics_sort_attribute_name.FindingStatisticsSortAttributeName"
    ]
...
def serialize_json(value: FindingStatisticsSortCriteria) -> dict:
    ...
    if "attribute_name" in value:
        ...
        out["attributeName"] = (
            capo_macie2.types.finding_statistics_sort_attribute_name.serialize_json(
...
def deserialize_json(data: dict) -> FindingStatisticsSortCriteria:
    ...
    if data.get("attributeName") is not None:
        ...
        out["attribute_name"] = (
            capo_macie2.types.finding_statistics_sort_attribute_name.deserialize_json(
                data["attributeName"]
            )
        )
