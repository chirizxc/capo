...
def handle_response(
    ...
    out: capo_nova_act.types.update_act_response.UpdateActResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{workflowRunId}", quote(input_["workflow_run_id"], safe=""))
    url = url.replace("{sessionId}", quote(input_["session_id"], safe=""))
    url = url.replace("{actId}", quote(input_["act_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_nova_act.types.update_act_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def update_act(
    ...
    input_: capo_nova_act.types.update_act_request.UpdateActRequest,
) -> tuple[capo_nova_act.types.update_act_response.UpdateActResponse, zapros.Response]:
