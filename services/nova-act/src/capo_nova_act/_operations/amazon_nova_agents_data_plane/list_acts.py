"""Generated from Smithy shape ``com.amazonaws.novaact#ListActs``."""
...
import capo_nova_act._protocol.eventstream
...
def handle_response(
    ...
    out: capo_nova_act.types.list_acts_response.ListActsResponse = (
        capo_nova_act.types.list_acts_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_nova_act._auth._sigv4.build_sigv4_auth_scheme(
                "nova-act", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace(
        "{workflowDefinitionName}", quote(input_["workflow_definition_name"], safe="")
    )
    params: list[tuple[str, str]] = []
    if "workflow_run_id" in input_:
        params.append(("workflowRunId", input_["workflow_run_id"]))
    ...
    body: bytes | None = json.dumps(
        capo_nova_act.types.list_acts_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def list_acts(
    ...
    input_: capo_nova_act.types.list_acts_request.ListActsRequest,
) -> tuple[capo_nova_act.types.list_acts_response.ListActsResponse, zapros.Response]:
