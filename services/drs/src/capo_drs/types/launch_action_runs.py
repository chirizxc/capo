"""Generated from Smithy shape ``com.amazonaws.drs#LaunchActionRuns``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_drs.types.launch_action_run

LaunchActionRuns: TypeAlias = list["capo_drs.types.launch_action_run.LaunchActionRun"]


# --- restJson1 ser/de ---
def serialize_json(value: LaunchActionRuns) -> list:
    import capo_drs.types.launch_action_run

    out: list = []
    for item in value:
        out.append(capo_drs.types.launch_action_run.serialize_json(item))
    return out


def deserialize_json(data: list) -> LaunchActionRuns:
    import capo_drs.types.launch_action_run

    out: LaunchActionRuns = []
    for item in data:
        if item is None:
            continue
        out.append(capo_drs.types.launch_action_run.deserialize_json(item))
    return out
