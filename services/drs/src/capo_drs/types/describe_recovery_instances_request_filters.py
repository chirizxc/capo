...
class DescribeRecoveryInstancesRequestFilters(TypedDict, closed=True):
    recovery_instance_i_ds: NotRequired[
        "capo_drs.types.recovery_instance_i_ds.RecoveryInstanceIDs"
    ]
    """<p>An array of Recovery Instance IDs that should be returned. An empty array means all Recovery Instances.</p>"""
    source_server_i_ds: NotRequired["capo_drs.types.source_server_i_ds.SourceServerIDs"]
...
def serialize_json(value: DescribeRecoveryInstancesRequestFilters) -> dict:
    ...
    if "recovery_instance_i_ds" in value:
        ...
        out["recoveryInstanceIDs"] = (
            capo_drs.types.recovery_instance_i_ds.serialize_json(
    ...
    if "source_server_i_ds" in value:
        ...
        out["sourceServerIDs"] = capo_drs.types.source_server_i_ds.serialize_json(
...
def deserialize_json(data: dict) -> DescribeRecoveryInstancesRequestFilters:
    ...
    if data.get("recoveryInstanceIDs") is not None:
        ...
        out["recovery_instance_i_ds"] = (
            capo_drs.types.recovery_instance_i_ds.deserialize_json(
