...
class ListStagingAccountsResponse(TypedDict, closed=True):
    accounts: NotRequired["capo_drs.types.accounts.Accounts"]
...
def serialize_json(value: ListStagingAccountsResponse) -> dict:
    ...
    if "accounts" in value:
        ...
        out["accounts"] = capo_drs.types.accounts.serialize_json(value["accounts"])
...
def deserialize_json(data: dict) -> ListStagingAccountsResponse:
    ...
    if data.get("accounts") is not None:
        ...
        out["accounts"] = capo_drs.types.accounts.deserialize_json(data["accounts"])
