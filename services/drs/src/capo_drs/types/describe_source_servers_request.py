"""Generated from Smithy shape ``com.amazonaws.drs#DescribeSourceServersRequest``."""
...
class DescribeSourceServersRequest(TypedDict, closed=True):
    filters: NotRequired[
...
def serialize_json(value: DescribeSourceServersRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_drs.types.describe_source_servers_request_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> DescribeSourceServersRequest:
    ...
    if data.get("filters") is not None:
        ...
        out["filters"] = (
            capo_drs.types.describe_source_servers_request_filters.deserialize_json(
                data["filters"]
            )
        )
