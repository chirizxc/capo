...
if TYPE_CHECKING:
    import capo_codeguru_reviewer.types.commit_id


class CommitDiffSourceCodeType(TypedDict, closed=True):
    source_commit: NotRequired["capo_codeguru_reviewer.types.commit_id.CommitId"]
    """<p>The SHA of the source commit used to generate a commit diff. This field is required for a pull request code review.</p>"""
    destination_commit: NotRequired["capo_codeguru_reviewer.types.commit_id.CommitId"]
    ...
    merge_base_commit: NotRequired["capo_codeguru_reviewer.types.commit_id.CommitId"]
