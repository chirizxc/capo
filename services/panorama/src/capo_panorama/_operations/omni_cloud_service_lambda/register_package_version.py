"""Generated from Smithy shape ``com.amazonaws.panorama#RegisterPackageVersion``."""
...
import capo_panorama._protocol.eventstream
...
def handle_response(
    ...
    out: capo_panorama.types.register_package_version_response.RegisterPackageVersionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_panorama._auth._sigv4.build_sigv4_auth_scheme(
                "panorama", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace("{PackageId}", quote(input_["package_id"], safe=""))
    url = url.replace("{PackageVersion}", quote(input_["package_version"], safe=""))
    url = url.replace("{PatchVersion}", quote(input_["patch_version"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_panorama.types.register_package_version_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_register_package_version(
