...
class NetworkStatus(TypedDict, closed=True):
    ethernet0_status: NotRequired["capo_panorama.types.ethernet_status.EthernetStatus"]
    """<p>The status of Ethernet port 0.</p>"""
    ethernet1_status: NotRequired["capo_panorama.types.ethernet_status.EthernetStatus"]
    """<p>The status of Ethernet port 1.</p>"""
    ntp_status: NotRequired["capo_panorama.types.ntp_status.NtpStatus"]
...
def serialize_json(value: NetworkStatus) -> dict:
    ...
    if "ethernet0_status" in value:
        ...
        out["Ethernet0Status"] = capo_panorama.types.ethernet_status.serialize_json(
            value["ethernet0_status"]
        )
    if "ethernet1_status" in value:
        import capo_panorama.types.ethernet_status

        out["Ethernet1Status"] = capo_panorama.types.ethernet_status.serialize_json(
            value["ethernet1_status"]
        )
    if "ntp_status" in value:
        import capo_panorama.types.ntp_status

        out["NtpStatus"] = capo_panorama.types.ntp_status.serialize_json(
            value["ntp_status"]
        )
