"""Generated from Smithy shape ``com.amazonaws.devopsagent#RegisterService``."""
...
def handle_response(
    ...
    out: capo_devops_agent.types.register_service_output.RegisterServiceOutput = (
        capo_devops_agent.types.register_service_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/register/{service}"
    url = url.replace(
        "{service}",
        quote(
            capo_devops_agent.types.post_register_service_supported_service.serialize_json(
                input_["service"]
            ),
            safe="",
        ),
    ...
    body: bytes | None = json.dumps(
        capo_devops_agent.types.register_service_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
