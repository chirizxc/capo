"""Generated from Smithy shape ``com.amazonaws.devopsagent#EnableOperatorApp``."""
...
import capo_devops_agent.errors.identity_center_service_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "IdentityCenterServiceException":
            raise capo_devops_agent.errors.identity_center_service_exception.IdentityCenterServiceException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_devops_agent.types.enable_operator_app_output.EnableOperatorAppOutput = (
        capo_devops_agent.types.enable_operator_app_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/agentspaces/{agentSpaceId}/operator"
    url = url.replace("{agentSpaceId}", quote(input_["agent_space_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_devops_agent.types.enable_operator_app_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_enable_operator_app(
