"""Generated from Smithy shape ``com.amazonaws.devopsagent#ListAssetFiles``."""
...
def handle_response(
    ...
    out: capo_devops_agent.types.list_asset_files_response.ListAssetFilesResponse = (
        capo_devops_agent.types.list_asset_files_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/asset/agent-space/{agentSpaceId}/assets/{assetId}/files"
    )
    ...
    if "asset_version" in input_:
        params.append(("assetVersion", str(input_["asset_version"])))
...
async def async_list_asset_files(
