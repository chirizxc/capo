"""Generated from Smithy shape ``com.amazonaws.devopsagent#ListServices``."""
...
def handle_response(
    ...
    out: capo_devops_agent.types.list_services_output.ListServicesOutput = (
        capo_devops_agent.types.list_services_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/services/list"
    params: list[tuple[str, str]] = []
    params.append(("maxResults", str(input_.get("max_results", 20))))
    if "next_token" in input_:
        params.append(("nextToken", input_["next_token"]))
    if "filter_service_type" in input_:
        params.append(
            (
                "filterServiceType",
                capo_devops_agent.types.service.serialize_json(
                    input_["filter_service_type"]
                ),
            )
        )
...
def list_services(
    ...
    capo_devops_agent.types.list_services_output.ListServicesOutput, zapros.Response
]:
