"""Generated from Smithy shape ``com.amazonaws.kinesisvideo#ListStreamsOutput``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_kinesis_video.types.next_token
    import capo_kinesis_video.types.stream_info_list


class ListStreamsOutput(TypedDict, closed=True):
    stream_info_list: NotRequired[
        "capo_kinesis_video.types.stream_info_list.StreamInfoList"
    ]
    """<p>An array of <code>StreamInfo</code> objects.</p>"""
    next_token: NotRequired["capo_kinesis_video.types.next_token.NextToken"]
    """<p>If the response is truncated, the call returns this element with a token. To get the next batch of streams, use this token in your next request. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListStreamsOutput) -> dict:
    out: dict = {}
    if "stream_info_list" in value:
        import capo_kinesis_video.types.stream_info_list

        out["StreamInfoList"] = (
            capo_kinesis_video.types.stream_info_list.serialize_json(
                value["stream_info_list"]
            )
        )
    if "next_token" in value:
        out["NextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListStreamsOutput:
    out: ListStreamsOutput = {}  # type: ignore[typeddict-item]
    if data.get("StreamInfoList") is not None:
        import capo_kinesis_video.types.stream_info_list

        out["stream_info_list"] = (
            capo_kinesis_video.types.stream_info_list.deserialize_json(
                data["StreamInfoList"]
            )
        )
    if data.get("NextToken") is not None:
        out["next_token"] = data["NextToken"]
    return out
