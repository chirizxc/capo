...
def handle_response(
    ...
    out: capo_ebs.types.list_changed_blocks_response.ListChangedBlocksResponse = (
        capo_ebs.types.list_changed_blocks_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{SecondSnapshotId}", quote(input_["second_snapshot_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    if "first_snapshot_id" in input_:
        params.append(("firstSnapshotId", input_["first_snapshot_id"]))
    if "next_token" in input_:
        params.append(("pageToken", input_["next_token"]))
    if "max_results" in input_:
        params.append(("maxResults", str(input_["max_results"])))
    if "starting_block_index" in input_:
        params.append(("startingBlockIndex", str(input_["starting_block_index"])))
