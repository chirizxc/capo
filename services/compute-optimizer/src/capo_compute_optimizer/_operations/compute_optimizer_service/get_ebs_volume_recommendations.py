"""Generated from Smithy shape ``com.amazonaws.computeoptimizer#GetEBSVolumeRecommendations``."""
...
import capo_compute_optimizer._protocol.eventstream
...
def handle_response(
    ...
    out: capo_compute_optimizer.types.get_ebs_volume_recommendations_response.GetEBSVolumeRecommendationsResponse = capo_compute_optimizer.types.get_ebs_volume_recommendations_response.deserialize_aws_json_1_0(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_compute_optimizer._auth._sigv4.build_sigv4_auth_scheme(
                "compute-optimizer", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "ComputeOptimizerService.GetEBSVolumeRecommendations"
    body: bytes | None = json.dumps(
        capo_compute_optimizer.types.get_ebs_volume_recommendations_request.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_ebs_volume_recommendations(
