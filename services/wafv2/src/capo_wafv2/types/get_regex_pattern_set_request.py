"""Generated from Smithy shape ``com.amazonaws.wafv2#GetRegexPatternSetRequest``."""
...
def deserialize_aws_json_1_1(data: dict) -> GetRegexPatternSetRequest:
    ...
    if data.get("Name") is not None:
        ...
        raise DeserializationError("GetRegexPatternSetRequest.name required")
    if data.get("Scope") is not None:
        import capo_wafv2.types.scope

        out["scope"] = capo_wafv2.types.scope.deserialize_aws_json_1_1(data["Scope"])
    else:
        raise DeserializationError("GetRegexPatternSetRequest.scope required")
    if data.get("Id") is not None:
        out["id"] = data["Id"]
    else:
        raise DeserializationError("GetRegexPatternSetRequest.id required")
    return out
