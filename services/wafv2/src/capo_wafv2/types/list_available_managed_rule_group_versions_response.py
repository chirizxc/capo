"""Generated from Smithy shape ``com.amazonaws.wafv2#ListAvailableManagedRuleGroupVersionsResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_wafv2.types.managed_rule_group_versions
    import capo_wafv2.types.next_marker
    import capo_wafv2.types.version_key_string


class ListAvailableManagedRuleGroupVersionsResponse(TypedDict, closed=True):
    next_marker: NotRequired["capo_wafv2.types.next_marker.NextMarker"]
    """<p>When you request a list of objects with a <code>Limit</code> setting, if the number of objects that are still available for retrieval exceeds the limit, WAF returns a <code>NextMarker</code> value in the response. To retrieve the next batch of objects, provide the marker from the prior call in your next request.</p>"""
    versions: NotRequired[
        "capo_wafv2.types.managed_rule_group_versions.ManagedRuleGroupVersions"
    ]
    """<p>The versions that are currently available for the specified managed rule group. If you specified a <code>Limit</code> in your request, this might not be the full list. </p>"""
    current_default_version: NotRequired[
        "capo_wafv2.types.version_key_string.VersionKeyString"
    ]
    """<p>The name of the version that's currently set as the default. </p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(
    value: ListAvailableManagedRuleGroupVersionsResponse,
) -> dict:
    out: dict = {}
    if "next_marker" in value:
        out["NextMarker"] = value["next_marker"]
    if "versions" in value:
        import capo_wafv2.types.managed_rule_group_versions

        out["Versions"] = (
            capo_wafv2.types.managed_rule_group_versions.serialize_aws_json_1_1(
                value["versions"]
            )
        )
    if "current_default_version" in value:
        out["CurrentDefaultVersion"] = value["current_default_version"]
    return out


def deserialize_aws_json_1_1(
    data: dict,
) -> ListAvailableManagedRuleGroupVersionsResponse:
    out: ListAvailableManagedRuleGroupVersionsResponse = {}  # type: ignore[typeddict-item]
    if data.get("NextMarker") is not None:
        out["next_marker"] = data["NextMarker"]
    if data.get("Versions") is not None:
        import capo_wafv2.types.managed_rule_group_versions

        out["versions"] = (
            capo_wafv2.types.managed_rule_group_versions.deserialize_aws_json_1_1(
                data["Versions"]
            )
        )
    if data.get("CurrentDefaultVersion") is not None:
        out["current_default_version"] = data["CurrentDefaultVersion"]
    return out
