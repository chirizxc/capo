"""Generated from Smithy shape ``com.amazonaws.lakeformation#ListLFTagExpressionsResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_lakeformation.types.lf_tag_expressions_list
    import capo_lakeformation.types.token


class ListLFTagExpressionsResponse(TypedDict, closed=True):
    lf_tag_expressions: NotRequired[
        "capo_lakeformation.types.lf_tag_expressions_list.LFTagExpressionsList"
    ]
    """<p>Logical expressions composed of one more LF-Tag key-value pairs.</p>"""
    next_token: NotRequired["capo_lakeformation.types.token.Token"]
    """<p>A continuation token, if this is not the first call to retrieve this list.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListLFTagExpressionsResponse) -> dict:
    out: dict = {}
    if "lf_tag_expressions" in value:
        import capo_lakeformation.types.lf_tag_expressions_list

        out["LFTagExpressions"] = (
            capo_lakeformation.types.lf_tag_expressions_list.serialize_json(
                value["lf_tag_expressions"]
            )
        )
    if "next_token" in value:
        out["NextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListLFTagExpressionsResponse:
    out: ListLFTagExpressionsResponse = {}  # type: ignore[typeddict-item]
    if data.get("LFTagExpressions") is not None:
        import capo_lakeformation.types.lf_tag_expressions_list

        out["lf_tag_expressions"] = (
            capo_lakeformation.types.lf_tag_expressions_list.deserialize_json(
                data["LFTagExpressions"]
            )
        )
    if data.get("NextToken") is not None:
        out["next_token"] = data["NextToken"]
    return out
