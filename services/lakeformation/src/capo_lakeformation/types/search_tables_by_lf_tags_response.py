...
class SearchTablesByLFTagsResponse(TypedDict, closed=True):
    ...
    table_list: NotRequired[
        "capo_lakeformation.types.table_lf_tags_list.TableLFTagsList"
    ]
...
def serialize_json(value: SearchTablesByLFTagsResponse) -> dict:
    ...
    if "table_list" in value:
        ...
        out["TableList"] = capo_lakeformation.types.table_lf_tags_list.serialize_json(
            value["table_list"]
        )
    return out
...
def deserialize_json(data: dict) -> SearchTablesByLFTagsResponse:
    ...
    if data.get("TableList") is not None:
        ...
        out["table_list"] = (
            capo_lakeformation.types.table_lf_tags_list.deserialize_json(
                data["TableList"]
            )
        )
    return out
