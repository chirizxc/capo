...
def serialize_json(value: DataLakePrincipalList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_lakeformation.types.data_lake_principal.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> DataLakePrincipalList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_lakeformation.types.data_lake_principal.deserialize_json(item))
    return out
