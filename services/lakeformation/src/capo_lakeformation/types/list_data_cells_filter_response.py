...
class ListDataCellsFilterResponse(TypedDict, closed=True):
    data_cells_filters: NotRequired[
        "capo_lakeformation.types.data_cells_filter_list.DataCellsFilterList"
    ]
...
def serialize_json(value: ListDataCellsFilterResponse) -> dict:
    ...
    if "data_cells_filters" in value:
        ...
        out["DataCellsFilters"] = (
            capo_lakeformation.types.data_cells_filter_list.serialize_json(
...
def deserialize_json(data: dict) -> ListDataCellsFilterResponse:
    ...
    if data.get("DataCellsFilters") is not None:
        import capo_lakeformation.types.data_cells_filter_list

        out["data_cells_filters"] = (
            capo_lakeformation.types.data_cells_filter_list.deserialize_json(
