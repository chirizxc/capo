...
class GetTableObjectsResponse(TypedDict, closed=True):
    objects: NotRequired[
        "capo_lakeformation.types.partitioned_table_objects_list.PartitionedTableObjectsList"
    ]
...
def serialize_json(value: GetTableObjectsResponse) -> dict:
    ...
    if "objects" in value:
        ...
        out["Objects"] = (
            capo_lakeformation.types.partitioned_table_objects_list.serialize_json(
                value["objects"]
            )
        )
...
def deserialize_json(data: dict) -> GetTableObjectsResponse:
    ...
    if data.get("Objects") is not None:
        ...
        out["objects"] = (
            capo_lakeformation.types.partitioned_table_objects_list.deserialize_json(
                data["Objects"]
            )
        )
