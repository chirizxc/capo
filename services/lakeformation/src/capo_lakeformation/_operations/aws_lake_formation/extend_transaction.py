"""Generated from Smithy shape ``com.amazonaws.lakeformation#ExtendTransaction``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.extend_transaction_response.ExtendTransactionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ExtendTransaction"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.extend_transaction_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_extend_transaction(
