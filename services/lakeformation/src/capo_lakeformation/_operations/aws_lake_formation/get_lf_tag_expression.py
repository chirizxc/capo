"""Generated from Smithy shape ``com.amazonaws.lakeformation#GetLFTagExpression``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.get_lf_tag_expression_response.GetLFTagExpressionResponse = capo_lakeformation.types.get_lf_tag_expression_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetLFTagExpression"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.get_lf_tag_expression_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_lf_tag_expression(
