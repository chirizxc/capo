"""Generated from Smithy shape ``com.amazonaws.lakeformation#DeleteDataCellsFilter``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.delete_data_cells_filter_response.DeleteDataCellsFilterResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteDataCellsFilter"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.delete_data_cells_filter_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_delete_data_cells_filter(
