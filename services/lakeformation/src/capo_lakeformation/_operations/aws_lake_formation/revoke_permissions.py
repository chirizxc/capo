"""Generated from Smithy shape ``com.amazonaws.lakeformation#RevokePermissions``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.revoke_permissions_response.RevokePermissionsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/RevokePermissions"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.revoke_permissions_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_revoke_permissions(
