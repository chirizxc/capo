"""Generated from Smithy shape ``com.amazonaws.lakeformation#ListTransactions``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.list_transactions_response.ListTransactionsResponse = capo_lakeformation.types.list_transactions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListTransactions"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.list_transactions_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
