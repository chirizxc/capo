"""Generated from Smithy shape ``com.amazonaws.lakeformation#BatchRevokePermissions``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.batch_revoke_permissions_response.BatchRevokePermissionsResponse = capo_lakeformation.types.batch_revoke_permissions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/BatchRevokePermissions"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.batch_revoke_permissions_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_revoke_permissions(
