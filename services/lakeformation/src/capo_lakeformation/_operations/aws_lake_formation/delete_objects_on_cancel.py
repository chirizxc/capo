"""Generated from Smithy shape ``com.amazonaws.lakeformation#DeleteObjectsOnCancel``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.delete_objects_on_cancel_response.DeleteObjectsOnCancelResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteObjectsOnCancel"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.delete_objects_on_cancel_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_delete_objects_on_cancel(
