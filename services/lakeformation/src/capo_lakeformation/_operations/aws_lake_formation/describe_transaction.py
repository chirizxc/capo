"""Generated from Smithy shape ``com.amazonaws.lakeformation#DescribeTransaction``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.describe_transaction_response.DescribeTransactionResponse = capo_lakeformation.types.describe_transaction_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DescribeTransaction"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.describe_transaction_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_describe_transaction(
