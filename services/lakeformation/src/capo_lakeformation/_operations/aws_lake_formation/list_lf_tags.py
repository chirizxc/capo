"""Generated from Smithy shape ``com.amazonaws.lakeformation#ListLFTags``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.list_lf_tags_response.ListLFTagsResponse = (
        capo_lakeformation.types.list_lf_tags_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListLFTags"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.list_lf_tags_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def list_lf_tags(
    ...
    capo_lakeformation.types.list_lf_tags_response.ListLFTagsResponse, zapros.Response
]:
