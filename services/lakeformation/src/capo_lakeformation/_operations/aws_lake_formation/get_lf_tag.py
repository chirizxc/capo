"""Generated from Smithy shape ``com.amazonaws.lakeformation#GetLFTag``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.get_lf_tag_response.GetLFTagResponse = (
        capo_lakeformation.types.get_lf_tag_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetLFTag"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.get_lf_tag_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def get_lf_tag(
    ...
    capo_lakeformation.types.get_lf_tag_response.GetLFTagResponse, zapros.Response
]:
