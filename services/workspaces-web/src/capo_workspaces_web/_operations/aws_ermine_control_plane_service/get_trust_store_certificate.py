"""Generated from Smithy shape ``com.amazonaws.workspacesweb#GetTrustStoreCertificate``."""
...
def handle_response(
    ...
    out: capo_workspaces_web.types.get_trust_store_certificate_response.GetTrustStoreCertificateResponse = capo_workspaces_web.types.get_trust_store_certificate_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/trustStores/{trustStoreArn+}/certificate"
    url = url.replace("{trustStoreArn+}", quote(input_["trust_store_arn"], safe="/"))
    params: list[tuple[str, str]] = []
    if "thumbprint" in input_:
        params.append(("thumbprint", input_["thumbprint"]))
...
async def async_get_trust_store_certificate(
