"""Generated from Smithy shape ``com.amazonaws.workspacesweb#ListSessions``."""
...
def handle_response(
    ...
    out: capo_workspaces_web.types.list_sessions_response.ListSessionsResponse = (
        capo_workspaces_web.types.list_sessions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/portals/{portalId}/sessions"
    url = url.replace("{portalId}", quote(input_["portal_id"], safe=""))
    ...
    if "sort_by" in input_:
        params.append(
            (
                "sortBy",
                capo_workspaces_web.types.session_sort_by.serialize_json(
                    input_["sort_by"]
                ),
            )
        )
    if "status" in input_:
        params.append(
            (
                "status",
                capo_workspaces_web.types.session_status.serialize_json(
                    input_["status"]
                ),
            )
        )
