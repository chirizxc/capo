"""Generated from Smithy shape ``com.amazonaws.workspacesweb#UpdateBrowserSettings``."""
...
def handle_response(
    ...
    out: capo_workspaces_web.types.update_browser_settings_response.UpdateBrowserSettingsResponse = capo_workspaces_web.types.update_browser_settings_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{browserSettingsArn+}", quote(input_["browser_settings_arn"], safe="/")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_workspaces_web.types.update_browser_settings_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
