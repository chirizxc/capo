"""Generated from Smithy shape ``com.amazonaws.workspacesweb#UpdateUserAccessLoggingSettings``."""
...
def handle_response(
    ...
    out: capo_workspaces_web.types.update_user_access_logging_settings_response.UpdateUserAccessLoggingSettingsResponse = capo_workspaces_web.types.update_user_access_logging_settings_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(input_["user_access_logging_settings_arn"], safe="/"),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_workspaces_web.types.update_user_access_logging_settings_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_user_access_logging_settings(
