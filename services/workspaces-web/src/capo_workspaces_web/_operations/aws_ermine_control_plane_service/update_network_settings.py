"""Generated from Smithy shape ``com.amazonaws.workspacesweb#UpdateNetworkSettings``."""
...
def handle_response(
    ...
    out: capo_workspaces_web.types.update_network_settings_response.UpdateNetworkSettingsResponse = capo_workspaces_web.types.update_network_settings_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{networkSettingsArn+}", quote(input_["network_settings_arn"], safe="/")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_workspaces_web.types.update_network_settings_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
