...
if TYPE_CHECKING:
    ...
    import capo_workspaces_web.types.max_display_resolution
    import capo_workspaces_web.types.toolbar_type
    import capo_workspaces_web.types.visual_mode


class ToolbarConfiguration(TypedDict, closed=True):
    toolbar_type: NotRequired["capo_workspaces_web.types.toolbar_type.ToolbarType"]
    """<p>The type of toolbar displayed during the session.</p>"""
    visual_mode: NotRequired["capo_workspaces_web.types.visual_mode.VisualMode"]
    """<p>The visual mode of the toolbar.</p>"""
    hidden_toolbar_items: NotRequired[
        "capo_workspaces_web.types.hidden_toolbar_item_list.HiddenToolbarItemList"
    ]
    """<p>The list of toolbar items to be hidden.</p>"""
    max_display_resolution: NotRequired[
        "capo_workspaces_web.types.max_display_resolution.MaxDisplayResolution"
    ]
...
def serialize_json(value: ToolbarConfiguration) -> dict:
    ...
    if "hidden_toolbar_items" in value:
        ...
        out["hiddenToolbarItems"] = (
            capo_workspaces_web.types.hidden_toolbar_item_list.serialize_json(
...
def deserialize_json(data: dict) -> ToolbarConfiguration:
    ...
    if data.get("hiddenToolbarItems") is not None:
        ...
        out["hidden_toolbar_items"] = (
            capo_workspaces_web.types.hidden_toolbar_item_list.deserialize_json(
