"""Generated from Smithy shape ``com.amazonaws.applicationdiscoveryservice#BatchDeleteImportDataError``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_application_discovery_service.types.batch_delete_import_data_error_code
    import capo_application_discovery_service.types.batch_delete_import_data_error_description
    import capo_application_discovery_service.types.import_task_identifier


class BatchDeleteImportDataError(TypedDict, closed=True):
    import_task_id: NotRequired[
        "capo_application_discovery_service.types.import_task_identifier.ImportTaskIdentifier"
    ]
    """<p>The unique import ID associated with the error that occurred.</p>"""
    error_code: NotRequired[
        "capo_application_discovery_service.types.batch_delete_import_data_error_code.BatchDeleteImportDataErrorCode"
    ]
    """<p>The type of error that occurred for a specific import task.</p>"""
    error_description: NotRequired[
        "capo_application_discovery_service.types.batch_delete_import_data_error_description.BatchDeleteImportDataErrorDescription"
    ]
    """<p>The description of the error that occurred for a specific import task.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: BatchDeleteImportDataError) -> dict:
    out: dict = {}
    if "import_task_id" in value:
        out["importTaskId"] = value["import_task_id"]
    if "error_code" in value:
        import capo_application_discovery_service.types.batch_delete_import_data_error_code

        out["errorCode"] = (
            capo_application_discovery_service.types.batch_delete_import_data_error_code.serialize_aws_json_1_1(
                value["error_code"]
            )
        )
    if "error_description" in value:
        out["errorDescription"] = value["error_description"]
    return out


def deserialize_aws_json_1_1(data: dict) -> BatchDeleteImportDataError:
    out: BatchDeleteImportDataError = {}  # type: ignore[typeddict-item]
    if data.get("importTaskId") is not None:
        out["import_task_id"] = data["importTaskId"]
    if data.get("errorCode") is not None:
        import capo_application_discovery_service.types.batch_delete_import_data_error_code

        out["error_code"] = (
            capo_application_discovery_service.types.batch_delete_import_data_error_code.deserialize_aws_json_1_1(
                data["errorCode"]
            )
        )
    if data.get("errorDescription") is not None:
        out["error_description"] = data["errorDescription"]
    return out
