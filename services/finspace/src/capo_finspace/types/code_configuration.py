...
if TYPE_CHECKING:
    import capo_finspace.types.s3_bucket
    import capo_finspace.types.s3_key
    import capo_finspace.types.s3_object_version


class CodeConfiguration(TypedDict, closed=True):
    s3_bucket: NotRequired["capo_finspace.types.s3_bucket.S3Bucket"]
    """<p>A unique name for the S3 bucket.</p>"""
    s3_key: NotRequired["capo_finspace.types.s3_key.S3Key"]
