...
if TYPE_CHECKING:
    import capo_finspace.types.kx_nas1_size
    import capo_finspace.types.kx_nas1_type


class KxNAS1Configuration(TypedDict, closed=True):
    type: NotRequired["capo_finspace.types.kx_nas1_type.KxNAS1Type"]
    """<p> The type of the network attached storage. </p>"""
    size: NotRequired["capo_finspace.types.kx_nas1_size.KxNAS1Size"]
...
def serialize_json(value: KxNAS1Configuration) -> dict:
    ...
    if "type" in value:
        ...
        out["type"] = capo_finspace.types.kx_nas1_type.serialize_json(value["type"])
    if "size" in value:
        out["size"] = value["size"]
    return out
