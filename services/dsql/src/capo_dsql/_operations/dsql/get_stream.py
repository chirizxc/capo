"""Generated from Smithy shape ``com.amazonaws.dsql#GetStream``."""
...
import capo_dsql._protocol.eventstream
...
def handle_response(
    ...
    out: capo_dsql.types.get_stream_output.GetStreamOutput = (
        capo_dsql.types.get_stream_output.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_dsql.types.get_stream_output.GetStreamOutput = (
        capo_dsql.types.get_stream_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_dsql._auth._sigv4.build_sigv4_auth_scheme(
                "dsql", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace(
        "{clusterIdentifier}", quote(input_["cluster_identifier"], safe="")
    )
    url = url.replace("{streamIdentifier}", quote(input_["stream_identifier"], safe=""))
...
def get_stream(
    options: OperationOptions, input_: capo_dsql.types.get_stream_input.GetStreamInput
) -> tuple[capo_dsql.types.get_stream_output.GetStreamOutput, zapros.Response]:
