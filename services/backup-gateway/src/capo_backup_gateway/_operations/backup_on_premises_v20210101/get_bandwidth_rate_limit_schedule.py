"""Generated from Smithy shape ``com.amazonaws.backupgateway#GetBandwidthRateLimitSchedule``."""
...
import capo_backup_gateway._protocol.eventstream
...
def handle_response(
    ...
    out: capo_backup_gateway.types.get_bandwidth_rate_limit_schedule_output.GetBandwidthRateLimitScheduleOutput = capo_backup_gateway.types.get_bandwidth_rate_limit_schedule_output.deserialize_aws_json_1_0(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_backup_gateway._auth._sigv4.build_sigv4_auth_scheme(
                "backup-gateway", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "BackupOnPremises_v20210101.GetBandwidthRateLimitSchedule"
    body: bytes | None = json.dumps(
        capo_backup_gateway.types.get_bandwidth_rate_limit_schedule_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_bandwidth_rate_limit_schedule(
