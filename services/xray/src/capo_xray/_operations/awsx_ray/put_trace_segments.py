"""Generated from Smithy shape ``com.amazonaws.xray#PutTraceSegments``."""
...
def handle_response(
    ...
    out: capo_xray.types.put_trace_segments_result.PutTraceSegmentsResult = (
        capo_xray.types.put_trace_segments_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/TraceSegments"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.put_trace_segments_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def put_trace_segments(
    ...
    capo_xray.types.put_trace_segments_result.PutTraceSegmentsResult, zapros.Response
]:
...
async def async_put_trace_segments(
