"""Generated from Smithy shape ``com.amazonaws.xray#GetTimeSeriesServiceStatistics``."""
...
def handle_response(
    ...
    out: capo_xray.types.get_time_series_service_statistics_result.GetTimeSeriesServiceStatisticsResult = capo_xray.types.get_time_series_service_statistics_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/TimeSeriesServiceStatistics"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.get_time_series_service_statistics_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_time_series_service_statistics(
