"""Generated from Smithy shape ``com.amazonaws.xray#GetSamplingStatisticSummaries``."""
...
def handle_response(
    ...
    out: capo_xray.types.get_sampling_statistic_summaries_result.GetSamplingStatisticSummariesResult = capo_xray.types.get_sampling_statistic_summaries_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/SamplingStatisticSummaries"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.get_sampling_statistic_summaries_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_sampling_statistic_summaries(
