"""Generated from Smithy shape ``com.amazonaws.xray#CreateSamplingRule``."""
...
import capo_xray.errors.rule_limit_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "RuleLimitExceededException":
            raise capo_xray.errors.rule_limit_exceeded_exception.RuleLimitExceededException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_xray.types.create_sampling_rule_result.CreateSamplingRuleResult = (
        capo_xray.types.create_sampling_rule_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateSamplingRule"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.create_sampling_rule_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_sampling_rule(
