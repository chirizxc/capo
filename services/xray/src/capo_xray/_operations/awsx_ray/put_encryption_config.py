"""Generated from Smithy shape ``com.amazonaws.xray#PutEncryptionConfig``."""
...
def handle_response(
    ...
    out: capo_xray.types.put_encryption_config_result.PutEncryptionConfigResult = (
        capo_xray.types.put_encryption_config_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/PutEncryptionConfig"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.put_encryption_config_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_put_encryption_config(
