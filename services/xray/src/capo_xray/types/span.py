"""Generated from Smithy shape ``com.amazonaws.xray#Span``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_xray.types.span_document
    import capo_xray.types.span_id


class Span(TypedDict, closed=True):
    id: NotRequired["capo_xray.types.span_id.SpanId"]
    """<p>The span ID.</p>"""
    document: NotRequired["capo_xray.types.span_document.SpanDocument"]
    """<p> The span document.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: Span) -> dict:
    out: dict = {}
    if "id" in value:
        out["Id"] = value["id"]
    if "document" in value:
        out["Document"] = value["document"]
    return out


def deserialize_json(data: dict) -> Span:
    out: Span = {}  # type: ignore[typeddict-item]
    if data.get("Id") is not None:
        out["id"] = data["Id"]
    if data.get("Document") is not None:
        out["document"] = data["Document"]
    return out
