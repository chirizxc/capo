...
class BatchGetTracesResult(TypedDict, closed=True):
    traces: NotRequired["capo_xray.types.trace_list.TraceList"]
    """<p>Full traces for the specified requests.</p>"""
    unprocessed_trace_ids: NotRequired[
        "capo_xray.types.unprocessed_trace_id_list.UnprocessedTraceIdList"
    ]
...
def serialize_json(value: BatchGetTracesResult) -> dict:
    ...
    if "traces" in value:
        ...
        out["Traces"] = capo_xray.types.trace_list.serialize_json(value["traces"])
    if "unprocessed_trace_ids" in value:
        import capo_xray.types.unprocessed_trace_id_list

        out["UnprocessedTraceIds"] = (
            capo_xray.types.unprocessed_trace_id_list.serialize_json(
...
def deserialize_json(data: dict) -> BatchGetTracesResult:
    ...
    if data.get("UnprocessedTraceIds") is not None:
        ...
        out["unprocessed_trace_ids"] = (
            capo_xray.types.unprocessed_trace_id_list.deserialize_json(
