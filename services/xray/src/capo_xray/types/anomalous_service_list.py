"""Generated from Smithy shape ``com.amazonaws.xray#AnomalousServiceList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_xray.types.anomalous_service

AnomalousServiceList: TypeAlias = list[
    "capo_xray.types.anomalous_service.AnomalousService"
]


# --- restJson1 ser/de ---
def serialize_json(value: AnomalousServiceList) -> list:
    import capo_xray.types.anomalous_service

    out: list = []
    for item in value:
        out.append(capo_xray.types.anomalous_service.serialize_json(item))
    return out


def deserialize_json(data: list) -> AnomalousServiceList:
    import capo_xray.types.anomalous_service

    out: AnomalousServiceList = []
    for item in data:
        if item is None:
            continue
        out.append(capo_xray.types.anomalous_service.deserialize_json(item))
    return out
