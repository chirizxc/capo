...
if TYPE_CHECKING:
    import capo_elastic_beanstalk.types.platform_branch_summary

PlatformBranchSummaryList: TypeAlias = list[
    "capo_elastic_beanstalk.types.platform_branch_summary.PlatformBranchSummary"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_beanstalk.types.platform_branch_summary.serialize_query(
...
def deserialize_query(el: Element) -> PlatformBranchSummaryList:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_beanstalk.types.platform_branch_summary.deserialize_query(
                child
            )
        )
    return out
