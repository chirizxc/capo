...
if TYPE_CHECKING:
    import capo_elastic_beanstalk.types.auto_scaling_group

AutoScalingGroupList: TypeAlias = list[
    "capo_elastic_beanstalk.types.auto_scaling_group.AutoScalingGroup"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_beanstalk.types.auto_scaling_group.serialize_query(
...
def deserialize_query(el: Element) -> AutoScalingGroupList:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_beanstalk.types.auto_scaling_group.deserialize_query(child)
        )
    return out
