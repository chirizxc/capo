...
if TYPE_CHECKING:
    import capo_elastic_beanstalk.types.managed_action

ManagedActions: TypeAlias = list[
    "capo_elastic_beanstalk.types.managed_action.ManagedAction"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_beanstalk.types.managed_action.serialize_query(
...
def deserialize_query(el: Element) -> ManagedActions:
    ...
    for child in el.findall("member"):
        out.append(capo_elastic_beanstalk.types.managed_action.deserialize_query(child))
    return out
