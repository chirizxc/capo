...
ManagedActionHistoryItems: TypeAlias = list[
    "capo_elastic_beanstalk.types.managed_action_history_item.ManagedActionHistoryItem"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_beanstalk.types.managed_action_history_item.serialize_query(
...
def deserialize_query(el: Element) -> ManagedActionHistoryItems:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_beanstalk.types.managed_action_history_item.deserialize_query(
                child
            )
        )
    return out
