...
if TYPE_CHECKING:
    import capo_elastic_beanstalk.types.load_balancer

LoadBalancerList: TypeAlias = list[
    "capo_elastic_beanstalk.types.load_balancer.LoadBalancer"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_beanstalk.types.load_balancer.serialize_query(
...
def deserialize_query(el: Element) -> LoadBalancerList:
    ...
    for child in el.findall("member"):
        out.append(capo_elastic_beanstalk.types.load_balancer.deserialize_query(child))
    return out
