"""Generated from Smithy shape ``com.amazonaws.elasticbeanstalk#CreateStorageLocation``."""
...
import capo_elastic_beanstalk.errors.s3_subscription_required_exception
...
from capo_elastic_beanstalk._protocol.errors import (
    find_error_element,
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "S3SubscriptionRequiredException":
            raise capo_elastic_beanstalk.errors.s3_subscription_required_exception.S3SubscriptionRequiredException.from_query(
                error_el, message
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateStorageLocationResult")
    out: capo_elastic_beanstalk.types.create_storage_location_result_message.CreateStorageLocationResultMessage = capo_elastic_beanstalk.types.create_storage_location_result_message.deserialize_query(
...
def build_request(options: OperationOptions | AsyncOperationOptions) -> zapros.Request:
    ...
    pairs.append(("Action", "CreateStorageLocation"))
...
async def async_create_storage_location(
