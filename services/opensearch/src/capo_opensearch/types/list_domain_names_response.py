...
class ListDomainNamesResponse(TypedDict, closed=True):
    domain_names: NotRequired["capo_opensearch.types.domain_info_list.DomainInfoList"]
...
def serialize_json(value: ListDomainNamesResponse) -> dict:
    ...
    if "domain_names" in value:
        ...
        out["DomainNames"] = capo_opensearch.types.domain_info_list.serialize_json(
            value["domain_names"]
        )
    return out
...
def deserialize_json(data: dict) -> ListDomainNamesResponse:
    ...
    if data.get("DomainNames") is not None:
        ...
        out["domain_names"] = capo_opensearch.types.domain_info_list.deserialize_json(
            data["DomainNames"]
        )
    return out
