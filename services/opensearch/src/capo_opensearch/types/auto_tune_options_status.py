"""Generated from Smithy shape ``com.amazonaws.opensearch#AutoTuneOptionsStatus``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_opensearch.types.auto_tune_options
    import capo_opensearch.types.auto_tune_status


class AutoTuneOptionsStatus(TypedDict, closed=True):
    options: NotRequired["capo_opensearch.types.auto_tune_options.AutoTuneOptions"]
    """<p>Auto-Tune settings for updating a domain.</p>"""
    status: NotRequired["capo_opensearch.types.auto_tune_status.AutoTuneStatus"]
    """<p>The current status of Auto-Tune for a domain.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: AutoTuneOptionsStatus) -> dict:
    out: dict = {}
    if "options" in value:
        import capo_opensearch.types.auto_tune_options

        out["Options"] = capo_opensearch.types.auto_tune_options.serialize_json(
            value["options"]
        )
    if "status" in value:
        import capo_opensearch.types.auto_tune_status

        out["Status"] = capo_opensearch.types.auto_tune_status.serialize_json(
            value["status"]
        )
    return out


def deserialize_json(data: dict) -> AutoTuneOptionsStatus:
    out: AutoTuneOptionsStatus = {}  # type: ignore[typeddict-item]
    if data.get("Options") is not None:
        import capo_opensearch.types.auto_tune_options

        out["options"] = capo_opensearch.types.auto_tune_options.deserialize_json(
            data["Options"]
        )
    if data.get("Status") is not None:
        import capo_opensearch.types.auto_tune_status

        out["status"] = capo_opensearch.types.auto_tune_status.deserialize_json(
            data["Status"]
        )
    return out
