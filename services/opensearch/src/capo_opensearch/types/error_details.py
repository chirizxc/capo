"""Generated from Smithy shape ``com.amazonaws.opensearch#ErrorDetails``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_opensearch.types.error_message
    import capo_opensearch.types.error_type


class ErrorDetails(TypedDict, closed=True):
    error_type: NotRequired["capo_opensearch.types.error_type.ErrorType"]
    """<p>The type of error that occurred.</p>"""
    error_message: NotRequired["capo_opensearch.types.error_message.ErrorMessage"]
    """<p>A message describing the error.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ErrorDetails) -> dict:
    out: dict = {}
    if "error_type" in value:
        out["ErrorType"] = value["error_type"]
    if "error_message" in value:
        out["ErrorMessage"] = value["error_message"]
    return out


def deserialize_json(data: dict) -> ErrorDetails:
    out: ErrorDetails = {}  # type: ignore[typeddict-item]
    if data.get("ErrorType") is not None:
        out["error_type"] = data["ErrorType"]
    if data.get("ErrorMessage") is not None:
        out["error_message"] = data["ErrorMessage"]
    return out
