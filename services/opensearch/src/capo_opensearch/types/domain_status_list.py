...
DomainStatusList: TypeAlias = list["capo_opensearch.types.domain_status.DomainStatus"]
...
def serialize_json(value: DomainStatusList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_opensearch.types.domain_status.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> DomainStatusList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_opensearch.types.domain_status.deserialize_json(item))
    return out
