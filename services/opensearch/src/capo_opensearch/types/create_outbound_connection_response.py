"""Generated from Smithy shape ``com.amazonaws.opensearch#CreateOutboundConnectionResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_opensearch.types.connection_alias
    import capo_opensearch.types.connection_id
    import capo_opensearch.types.connection_mode
    import capo_opensearch.types.connection_properties
    import capo_opensearch.types.domain_information_container
    import capo_opensearch.types.outbound_connection_status


class CreateOutboundConnectionResponse(TypedDict, closed=True):
    local_domain_info: NotRequired[
        "capo_opensearch.types.domain_information_container.DomainInformationContainer"
    ]
    """<p>Information about the source (local) domain.</p>"""
    remote_domain_info: NotRequired[
        "capo_opensearch.types.domain_information_container.DomainInformationContainer"
    ]
    """<p>Information about the destination (remote) domain.</p>"""
    connection_alias: NotRequired[
        "capo_opensearch.types.connection_alias.ConnectionAlias"
    ]
    """<p>Name of the connection.</p>"""
    connection_status: NotRequired[
        "capo_opensearch.types.outbound_connection_status.OutboundConnectionStatus"
    ]
    """<p>The status of the connection.</p>"""
    connection_id: NotRequired["capo_opensearch.types.connection_id.ConnectionId"]
    """<p>The unique identifier for the created outbound connection, which is used for subsequent operations on the connection.</p>"""
    connection_mode: NotRequired["capo_opensearch.types.connection_mode.ConnectionMode"]
    """<p>The connection mode.</p>"""
    connection_properties: NotRequired[
        "capo_opensearch.types.connection_properties.ConnectionProperties"
    ]
    """<p>The <code>ConnectionProperties</code> for the newly created connection.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: CreateOutboundConnectionResponse) -> dict:
    out: dict = {}
    if "local_domain_info" in value:
        import capo_opensearch.types.domain_information_container

        out["LocalDomainInfo"] = (
            capo_opensearch.types.domain_information_container.serialize_json(
                value["local_domain_info"]
            )
        )
    if "remote_domain_info" in value:
        import capo_opensearch.types.domain_information_container

        out["RemoteDomainInfo"] = (
            capo_opensearch.types.domain_information_container.serialize_json(
                value["remote_domain_info"]
            )
        )
    if "connection_alias" in value:
        out["ConnectionAlias"] = value["connection_alias"]
    if "connection_status" in value:
        import capo_opensearch.types.outbound_connection_status

        out["ConnectionStatus"] = (
            capo_opensearch.types.outbound_connection_status.serialize_json(
                value["connection_status"]
            )
        )
    if "connection_id" in value:
        out["ConnectionId"] = value["connection_id"]
    if "connection_mode" in value:
        import capo_opensearch.types.connection_mode

        out["ConnectionMode"] = capo_opensearch.types.connection_mode.serialize_json(
            value["connection_mode"]
        )
    if "connection_properties" in value:
        import capo_opensearch.types.connection_properties

        out["ConnectionProperties"] = (
            capo_opensearch.types.connection_properties.serialize_json(
                value["connection_properties"]
            )
        )
    return out


def deserialize_json(data: dict) -> CreateOutboundConnectionResponse:
    out: CreateOutboundConnectionResponse = {}  # type: ignore[typeddict-item]
    if data.get("LocalDomainInfo") is not None:
        import capo_opensearch.types.domain_information_container

        out["local_domain_info"] = (
            capo_opensearch.types.domain_information_container.deserialize_json(
                data["LocalDomainInfo"]
            )
        )
    if data.get("RemoteDomainInfo") is not None:
        import capo_opensearch.types.domain_information_container

        out["remote_domain_info"] = (
            capo_opensearch.types.domain_information_container.deserialize_json(
                data["RemoteDomainInfo"]
            )
        )
    if data.get("ConnectionAlias") is not None:
        out["connection_alias"] = data["ConnectionAlias"]
    if data.get("ConnectionStatus") is not None:
        import capo_opensearch.types.outbound_connection_status

        out["connection_status"] = (
            capo_opensearch.types.outbound_connection_status.deserialize_json(
                data["ConnectionStatus"]
            )
        )
    if data.get("ConnectionId") is not None:
        out["connection_id"] = data["ConnectionId"]
    if data.get("ConnectionMode") is not None:
        import capo_opensearch.types.connection_mode

        out["connection_mode"] = capo_opensearch.types.connection_mode.deserialize_json(
            data["ConnectionMode"]
        )
    if data.get("ConnectionProperties") is not None:
        import capo_opensearch.types.connection_properties

        out["connection_properties"] = (
            capo_opensearch.types.connection_properties.deserialize_json(
                data["ConnectionProperties"]
            )
        )
    return out
