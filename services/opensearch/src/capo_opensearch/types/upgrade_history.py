...
class UpgradeHistory(TypedDict, closed=True):
    ...
    start_timestamp: NotRequired["capo_opensearch.types.start_timestamp.StartTimestamp"]
    """<p>UTC timestamp at which the upgrade API call was made, in the format <code>yyyy-MM-ddTHH:mm:ssZ</code>.</p>"""
    upgrade_status: NotRequired["capo_opensearch.types.upgrade_status.UpgradeStatus"]
    ...
    steps_list: NotRequired["capo_opensearch.types.upgrade_steps_list.UpgradeStepsList"]
...
def serialize_json(value: UpgradeHistory) -> dict:
    ...
    if "start_timestamp" in value:
        ...
        out["StartTimestamp"] = capo_opensearch.types.start_timestamp.serialize_json(
            value["start_timestamp"]
        )
    if "upgrade_status" in value:
        import capo_opensearch.types.upgrade_status

        out["UpgradeStatus"] = capo_opensearch.types.upgrade_status.serialize_json(
            value["upgrade_status"]
        )
    if "steps_list" in value:
        import capo_opensearch.types.upgrade_steps_list

        out["StepsList"] = capo_opensearch.types.upgrade_steps_list.serialize_json(
            value["steps_list"]
        )
    return out
