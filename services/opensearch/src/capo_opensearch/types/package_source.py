...
if TYPE_CHECKING:
    import capo_opensearch.types.s3_bucket_name
    import capo_opensearch.types.s3_key


class PackageSource(TypedDict, closed=True):
    s3_bucket_name: NotRequired["capo_opensearch.types.s3_bucket_name.S3BucketName"]
    """<p>The name of the Amazon S3 bucket containing the package.</p>"""
    s3_key: NotRequired["capo_opensearch.types.s3_key.S3Key"]
