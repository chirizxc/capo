...
if TYPE_CHECKING:
    import capo_opensearch.types.storage_sub_type_name
    import capo_opensearch.types.storage_type_limit_list
    import capo_opensearch.types.storage_type_name


class StorageType(TypedDict, closed=True):
    storage_type_name: NotRequired[
        "capo_opensearch.types.storage_type_name.StorageTypeName"
    ]
    """<p>The name of the storage type.</p>"""
    storage_sub_type_name: NotRequired[
        "capo_opensearch.types.storage_sub_type_name.StorageSubTypeName"
    ]
    """<p>The storage sub-type, such as <code>gp3</code> or <code>io1</code>.</p>"""
    storage_type_limits: NotRequired[
        "capo_opensearch.types.storage_type_limit_list.StorageTypeLimitList"
    ]
...
def serialize_json(value: StorageType) -> dict:
    ...
    if "storage_type_limits" in value:
        ...
        out["StorageTypeLimits"] = (
            capo_opensearch.types.storage_type_limit_list.serialize_json(
...
def deserialize_json(data: dict) -> StorageType:
    ...
    if data.get("StorageTypeLimits") is not None:
        ...
        out["storage_type_limits"] = (
            capo_opensearch.types.storage_type_limit_list.deserialize_json(
