...
if TYPE_CHECKING:
    ...
    import capo_opensearch.types.outbound_connection_status_code


class OutboundConnectionStatus(TypedDict, closed=True):
    status_code: NotRequired[
        "capo_opensearch.types.outbound_connection_status_code.OutboundConnectionStatusCode"
    ]
...
def serialize_json(value: OutboundConnectionStatus) -> dict:
    ...
    if "status_code" in value:
        ...
        out["StatusCode"] = (
            capo_opensearch.types.outbound_connection_status_code.serialize_json(
                value["status_code"]
            )
        )
    if "message" in value:
...
def deserialize_json(data: dict) -> OutboundConnectionStatus:
    ...
    if data.get("StatusCode") is not None:
        ...
        out["status_code"] = (
            capo_opensearch.types.outbound_connection_status_code.deserialize_json(
                data["StatusCode"]
            )
        )
