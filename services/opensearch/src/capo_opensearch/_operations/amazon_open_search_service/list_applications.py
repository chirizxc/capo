"""Generated from Smithy shape ``com.amazonaws.opensearch#ListApplications``."""
...
import capo_opensearch.types.application_summaries
...
def handle_response(
    ...
    out: capo_opensearch.types.list_applications_response.ListApplicationsResponse = (
        capo_opensearch.types.list_applications_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/opensearch/list-applications"
    ...
    if "statuses" in input_:
        for item in input_["statuses"]:
            params.append(
                (
                    "statuses",
                    capo_opensearch.types.application_status.serialize_json(item),
                )
            )
