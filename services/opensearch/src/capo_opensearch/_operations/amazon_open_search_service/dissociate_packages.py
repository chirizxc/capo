"""Generated from Smithy shape ``com.amazonaws.opensearch#DissociatePackages``."""
...
def handle_response(
    ...
    out: capo_opensearch.types.dissociate_packages_response.DissociatePackagesResponse = capo_opensearch.types.dissociate_packages_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/packages/dissociateMultiple"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_opensearch.types.dissociate_packages_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_dissociate_packages(
