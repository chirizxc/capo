"""Generated from Smithy shape ``com.amazonaws.opensearch#UpdatePackageScope``."""
...
def handle_response(
    ...
    out: capo_opensearch.types.update_package_scope_response.UpdatePackageScopeResponse = capo_opensearch.types.update_package_scope_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/packages/updateScope"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_opensearch.types.update_package_scope_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_package_scope(
