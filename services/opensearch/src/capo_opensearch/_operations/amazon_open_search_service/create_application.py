"""Generated from Smithy shape ``com.amazonaws.opensearch#CreateApplication``."""
...
def handle_response(
    ...
    out: capo_opensearch.types.create_application_response.CreateApplicationResponse = (
        capo_opensearch.types.create_application_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/opensearch/application"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_opensearch.types.create_application_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
