"""Generated from Smithy shape ``com.amazonaws.opensearch#UpdatePackage``."""
...
def handle_response(
    ...
    out: capo_opensearch.types.update_package_response.UpdatePackageResponse = (
        capo_opensearch.types.update_package_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/packages/update"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_opensearch.types.update_package_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_package(
    ...
    capo_opensearch.types.update_package_response.UpdatePackageResponse, zapros.Response
]:
