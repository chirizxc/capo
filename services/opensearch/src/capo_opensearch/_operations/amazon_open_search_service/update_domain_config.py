"""Generated from Smithy shape ``com.amazonaws.opensearch#UpdateDomainConfig``."""
...
import capo_opensearch._protocol.eventstream
import capo_opensearch.errors.base_exception
import capo_opensearch.errors.internal_exception
import capo_opensearch.errors.invalid_type_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "BaseException":
            raise capo_opensearch.errors.base_exception.BaseException.from_json(
                data, message
            )
        ...
        case "InvalidTypeException":
            raise capo_opensearch.errors.invalid_type_exception.InvalidTypeException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_opensearch.types.update_domain_config_response.UpdateDomainConfigResponse = capo_opensearch.types.update_domain_config_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_opensearch._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_opensearch.types.update_domain_config_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_domain_config(
