...
def handle_response(
    ...
    out: capo_opensearch.types.describe_instance_type_limits_response.DescribeInstanceTypeLimitsResponse = capo_opensearch.types.describe_instance_type_limits_response.deserialize_json(
    ...
    url = url.replace(
        ...
        quote(
            capo_opensearch.types.open_search_partition_instance_type.serialize_json(
                input_["instance_type"]
            ),
            safe="",
        ),
    )
    url = url.replace("{EngineVersion}", quote(input_["engine_version"], safe=""))
    params: list[tuple[str, str]] = []
    if "domain_name" in input_:
        params.append(("domainName", input_["domain_name"]))
