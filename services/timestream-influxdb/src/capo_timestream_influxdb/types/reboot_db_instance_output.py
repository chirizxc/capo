"""Generated from Smithy shape ``com.amazonaws.timestreaminfluxdb#RebootDbInstanceOutput``."""
...
class RebootDbInstanceOutput(TypedDict, closed=True):
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: RebootDbInstanceOutput) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    out["name"] = value["name"]
    out["arn"] = value["arn"]
...
def deserialize_aws_json_1_0(data: dict) -> RebootDbInstanceOutput:
    out: RebootDbInstanceOutput = {}  # type: ignore[typeddict-item]
    if data.get("id") is not None:
        out["id"] = data["id"]
    else:
        raise DeserializationError("RebootDbInstanceOutput.id required")
    if data.get("name") is not None:
        out["name"] = data["name"]
    else:
        raise DeserializationError("RebootDbInstanceOutput.name required")
    if data.get("arn") is not None:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("RebootDbInstanceOutput.arn required")
    ...
    if data.get("vpcSubnetIds") is not None:
        ...
        raise DeserializationError("RebootDbInstanceOutput.vpc_subnet_ids required")
