...
DbParameterGroupSummaryList: TypeAlias = list[
    "capo_timestream_influxdb.types.db_parameter_group_summary.DbParameterGroupSummary"
]
...
def serialize_aws_json_1_0(value: DbParameterGroupSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_timestream_influxdb.types.db_parameter_group_summary.serialize_aws_json_1_0(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_0(data: list) -> DbParameterGroupSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_timestream_influxdb.types.db_parameter_group_summary.deserialize_aws_json_1_0(
                item
            )
        )
    return out
