...
class ResourceScanSummary(TypedDict, closed=True):
    ...
    scan_type: NotRequired["capo_cloudformation.types.scan_type.ScanType"]
...
def deserialize_query(el: Element) -> ResourceScanSummary:
    ...
    if child_scan_type is not None:
        ...
        out["scan_type"] = capo_cloudformation.types.scan_type.deserialize_query(
            child_scan_type
        )
    return out
