...
StackRefactorSummaries: TypeAlias = list[
    "capo_cloudformation.types.stack_refactor_summary.StackRefactorSummary"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_cloudformation.types.stack_refactor_summary.serialize_query(
...
def deserialize_query(el: Element) -> StackRefactorSummaries:
    ...
    for child in el.findall("member"):
        out.append(
            capo_cloudformation.types.stack_refactor_summary.deserialize_query(child)
        )
    return out
