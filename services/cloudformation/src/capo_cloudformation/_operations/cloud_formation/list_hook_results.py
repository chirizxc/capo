"""Generated from Smithy shape ``com.amazonaws.cloudformation#ListHookResults``."""
...
import capo_cloudformation.errors.hook_result_not_found_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "HookResultNotFound":
            raise capo_cloudformation.errors.hook_result_not_found_exception.HookResultNotFoundException.from_query(
                error_el, message
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListHookResultsResult")
    out: capo_cloudformation.types.list_hook_results_output.ListHookResultsOutput = (
        capo_cloudformation.types.list_hook_results_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListHookResults"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.list_hook_results_input.serialize_query(input_, pairs, "")
...
async def async_list_hook_results(
