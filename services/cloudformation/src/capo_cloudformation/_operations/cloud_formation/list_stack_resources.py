"""Generated from Smithy shape ``com.amazonaws.cloudformation#ListStackResources``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListStackResourcesResult")
    out: capo_cloudformation.types.list_stack_resources_output.ListStackResourcesOutput = capo_cloudformation.types.list_stack_resources_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListStackResources"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.list_stack_resources_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_list_stack_resources(
