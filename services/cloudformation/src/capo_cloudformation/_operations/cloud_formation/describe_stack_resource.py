"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeStackResource``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeStackResourceResult")
    out: capo_cloudformation.types.describe_stack_resource_output.DescribeStackResourceOutput = capo_cloudformation.types.describe_stack_resource_output.deserialize_query(
...
def build_request(
    ...
    capo_cloudformation.types.describe_stack_resource_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_stack_resource(
