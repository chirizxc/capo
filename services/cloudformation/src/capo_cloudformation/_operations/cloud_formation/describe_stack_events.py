"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeStackEvents``."""
...
import capo_cloudformation.types.stack_events
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeStackEventsResult")
    out: capo_cloudformation.types.describe_stack_events_output.DescribeStackEventsOutput = capo_cloudformation.types.describe_stack_events_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeStackEvents"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_stack_events_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_stack_events(
