"""Generated from Smithy shape ``com.amazonaws.cloudformation#DetectStackSetDrift``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DetectStackSetDriftResult")
    out: capo_cloudformation.types.detect_stack_set_drift_output.DetectStackSetDriftOutput = capo_cloudformation.types.detect_stack_set_drift_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DetectStackSetDrift"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.detect_stack_set_drift_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_detect_stack_set_drift(
