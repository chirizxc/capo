"""Generated from Smithy shape ``com.amazonaws.cloudformation#ExecuteChangeSet``."""
...
import capo_cloudformation.errors.invalid_change_set_status_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidChangeSetStatus":
            raise capo_cloudformation.errors.invalid_change_set_status_exception.InvalidChangeSetStatusException.from_query(
                error_el, message
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ExecuteChangeSetResult")
    out: capo_cloudformation.types.execute_change_set_output.ExecuteChangeSetOutput = (
        capo_cloudformation.types.execute_change_set_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ExecuteChangeSet"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.execute_change_set_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_execute_change_set(
