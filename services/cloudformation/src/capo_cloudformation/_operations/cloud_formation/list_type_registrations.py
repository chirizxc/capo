"""Generated from Smithy shape ``com.amazonaws.cloudformation#ListTypeRegistrations``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListTypeRegistrationsResult")
    out: capo_cloudformation.types.list_type_registrations_output.ListTypeRegistrationsOutput = capo_cloudformation.types.list_type_registrations_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListTypeRegistrations"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.list_type_registrations_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_list_type_registrations(
