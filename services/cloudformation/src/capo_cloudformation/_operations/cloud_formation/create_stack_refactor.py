"""Generated from Smithy shape ``com.amazonaws.cloudformation#CreateStackRefactor``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateStackRefactorResult")
    out: capo_cloudformation.types.create_stack_refactor_output.CreateStackRefactorOutput = capo_cloudformation.types.create_stack_refactor_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "CreateStackRefactor"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.create_stack_refactor_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_create_stack_refactor(
