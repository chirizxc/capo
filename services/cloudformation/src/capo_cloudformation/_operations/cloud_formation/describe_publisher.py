"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribePublisher``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribePublisherResult")
    out: capo_cloudformation.types.describe_publisher_output.DescribePublisherOutput = (
        capo_cloudformation.types.describe_publisher_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribePublisher"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_publisher_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_publisher(
