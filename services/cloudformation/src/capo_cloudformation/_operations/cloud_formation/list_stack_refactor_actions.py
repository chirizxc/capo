"""Generated from Smithy shape ``com.amazonaws.cloudformation#ListStackRefactorActions``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListStackRefactorActionsResult")
    out: capo_cloudformation.types.list_stack_refactor_actions_output.ListStackRefactorActionsOutput = capo_cloudformation.types.list_stack_refactor_actions_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListStackRefactorActions"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.list_stack_refactor_actions_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_list_stack_refactor_actions(
