"""Generated from Smithy shape ``com.amazonaws.cloudformation#CreateStackInstances``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateStackInstancesResult")
    out: capo_cloudformation.types.create_stack_instances_output.CreateStackInstancesOutput = capo_cloudformation.types.create_stack_instances_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "CreateStackInstances"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.create_stack_instances_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_create_stack_instances(
