"""Generated from Smithy shape ``com.amazonaws.cloudformation#ListStackRefactors``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListStackRefactorsResult")
    out: capo_cloudformation.types.list_stack_refactors_output.ListStackRefactorsOutput = capo_cloudformation.types.list_stack_refactors_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListStackRefactors"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.list_stack_refactors_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_list_stack_refactors(
