"""Generated from Smithy shape ``com.amazonaws.costoptimizationhub#Filter``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_cost_optimization_hub.types.account_id_list
    import capo_cost_optimization_hub.types.action_type_list
    import capo_cost_optimization_hub.types.implementation_effort_list
    import capo_cost_optimization_hub.types.recommendation_id_list
    import capo_cost_optimization_hub.types.region_list
    import capo_cost_optimization_hub.types.resource_arn_list
    import capo_cost_optimization_hub.types.resource_id_list
    import capo_cost_optimization_hub.types.resource_type_list
    import capo_cost_optimization_hub.types.tag_list


class Filter(TypedDict, closed=True):
    restart_needed: NotRequired["bool"]
    """<p>Whether or not implementing the recommendation requires a restart.</p>"""
    rollback_possible: NotRequired["bool"]
    """<p>Whether or not implementing the recommendation can be rolled back.</p>"""
    implementation_efforts: NotRequired[
        "capo_cost_optimization_hub.types.implementation_effort_list.ImplementationEffortList"
    ]
    """<p>The effort required to implement the recommendation.</p>"""
    account_ids: NotRequired[
        "capo_cost_optimization_hub.types.account_id_list.AccountIdList"
    ]
    """<p>The account to which the recommendation applies.</p>"""
    regions: NotRequired["capo_cost_optimization_hub.types.region_list.RegionList"]
    """<p>The Amazon Web Services Region of the resource.</p>"""
    resource_types: NotRequired[
        "capo_cost_optimization_hub.types.resource_type_list.ResourceTypeList"
    ]
    """<p>The resource type of the recommendation.</p>"""
    action_types: NotRequired[
        "capo_cost_optimization_hub.types.action_type_list.ActionTypeList"
    ]
    """<p>The type of action you can take by adopting the recommendation.</p>"""
    tags: NotRequired["capo_cost_optimization_hub.types.tag_list.TagList"]
    """<p>A list of tags assigned to the recommendation.</p>"""
    resource_ids: NotRequired[
        "capo_cost_optimization_hub.types.resource_id_list.ResourceIdList"
    ]
    """<p>The resource ID of the recommendation.</p>"""
    resource_arns: NotRequired[
        "capo_cost_optimization_hub.types.resource_arn_list.ResourceArnList"
    ]
    """<p>The Amazon Resource Name (ARN) of the recommendation.</p>"""
    recommendation_ids: NotRequired[
        "capo_cost_optimization_hub.types.recommendation_id_list.RecommendationIdList"
    ]
    """<p>The IDs for the recommendations.</p>"""


# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: Filter) -> dict:
    out: dict = {}
    if "restart_needed" in value:
        out["restartNeeded"] = value["restart_needed"]
    if "rollback_possible" in value:
        out["rollbackPossible"] = value["rollback_possible"]
    if "implementation_efforts" in value:
        import capo_cost_optimization_hub.types.implementation_effort_list

        out["implementationEfforts"] = (
            capo_cost_optimization_hub.types.implementation_effort_list.serialize_aws_json_1_0(
                value["implementation_efforts"]
            )
        )
    if "account_ids" in value:
        import capo_cost_optimization_hub.types.account_id_list

        out["accountIds"] = (
            capo_cost_optimization_hub.types.account_id_list.serialize_aws_json_1_0(
                value["account_ids"]
            )
        )
    if "regions" in value:
        import capo_cost_optimization_hub.types.region_list

        out["regions"] = (
            capo_cost_optimization_hub.types.region_list.serialize_aws_json_1_0(
                value["regions"]
            )
        )
    if "resource_types" in value:
        import capo_cost_optimization_hub.types.resource_type_list

        out["resourceTypes"] = (
            capo_cost_optimization_hub.types.resource_type_list.serialize_aws_json_1_0(
                value["resource_types"]
            )
        )
    if "action_types" in value:
        import capo_cost_optimization_hub.types.action_type_list

        out["actionTypes"] = (
            capo_cost_optimization_hub.types.action_type_list.serialize_aws_json_1_0(
                value["action_types"]
            )
        )
    if "tags" in value:
        import capo_cost_optimization_hub.types.tag_list

        out["tags"] = capo_cost_optimization_hub.types.tag_list.serialize_aws_json_1_0(
            value["tags"]
        )
    if "resource_ids" in value:
        import capo_cost_optimization_hub.types.resource_id_list

        out["resourceIds"] = (
            capo_cost_optimization_hub.types.resource_id_list.serialize_aws_json_1_0(
                value["resource_ids"]
            )
        )
    if "resource_arns" in value:
        import capo_cost_optimization_hub.types.resource_arn_list

        out["resourceArns"] = (
            capo_cost_optimization_hub.types.resource_arn_list.serialize_aws_json_1_0(
                value["resource_arns"]
            )
        )
    if "recommendation_ids" in value:
        import capo_cost_optimization_hub.types.recommendation_id_list

        out["recommendationIds"] = (
            capo_cost_optimization_hub.types.recommendation_id_list.serialize_aws_json_1_0(
                value["recommendation_ids"]
            )
        )
    return out


def deserialize_aws_json_1_0(data: dict) -> Filter:
    out: Filter = {}  # type: ignore[typeddict-item]
    if data.get("restartNeeded") is not None:
        out["restart_needed"] = data["restartNeeded"]
    if data.get("rollbackPossible") is not None:
        out["rollback_possible"] = data["rollbackPossible"]
    if data.get("implementationEfforts") is not None:
        import capo_cost_optimization_hub.types.implementation_effort_list

        out["implementation_efforts"] = (
            capo_cost_optimization_hub.types.implementation_effort_list.deserialize_aws_json_1_0(
                data["implementationEfforts"]
            )
        )
    if data.get("accountIds") is not None:
        import capo_cost_optimization_hub.types.account_id_list

        out["account_ids"] = (
            capo_cost_optimization_hub.types.account_id_list.deserialize_aws_json_1_0(
                data["accountIds"]
            )
        )
    if data.get("regions") is not None:
        import capo_cost_optimization_hub.types.region_list

        out["regions"] = (
            capo_cost_optimization_hub.types.region_list.deserialize_aws_json_1_0(
                data["regions"]
            )
        )
    if data.get("resourceTypes") is not None:
        import capo_cost_optimization_hub.types.resource_type_list

        out["resource_types"] = (
            capo_cost_optimization_hub.types.resource_type_list.deserialize_aws_json_1_0(
                data["resourceTypes"]
            )
        )
    if data.get("actionTypes") is not None:
        import capo_cost_optimization_hub.types.action_type_list

        out["action_types"] = (
            capo_cost_optimization_hub.types.action_type_list.deserialize_aws_json_1_0(
                data["actionTypes"]
            )
        )
    if data.get("tags") is not None:
        import capo_cost_optimization_hub.types.tag_list

        out["tags"] = (
            capo_cost_optimization_hub.types.tag_list.deserialize_aws_json_1_0(
                data["tags"]
            )
        )
    if data.get("resourceIds") is not None:
        import capo_cost_optimization_hub.types.resource_id_list

        out["resource_ids"] = (
            capo_cost_optimization_hub.types.resource_id_list.deserialize_aws_json_1_0(
                data["resourceIds"]
            )
        )
    if data.get("resourceArns") is not None:
        import capo_cost_optimization_hub.types.resource_arn_list

        out["resource_arns"] = (
            capo_cost_optimization_hub.types.resource_arn_list.deserialize_aws_json_1_0(
                data["resourceArns"]
            )
        )
    if data.get("recommendationIds") is not None:
        import capo_cost_optimization_hub.types.recommendation_id_list

        out["recommendation_ids"] = (
            capo_cost_optimization_hub.types.recommendation_id_list.deserialize_aws_json_1_0(
                data["recommendationIds"]
            )
        )
    return out
