...
if TYPE_CHECKING:
    import capo_cost_optimization_hub.types.rds_db_instance_storage_configuration
    import capo_cost_optimization_hub.types.resource_cost_calculation


class RdsDbInstanceStorage(TypedDict, closed=True):
    configuration: NotRequired[
        "capo_cost_optimization_hub.types.rds_db_instance_storage_configuration.RdsDbInstanceStorageConfiguration"
    ]
...
def serialize_aws_json_1_0(value: RdsDbInstanceStorage) -> dict:
    ...
    if "configuration" in value:
        ...
        out["configuration"] = (
            capo_cost_optimization_hub.types.rds_db_instance_storage_configuration.serialize_aws_json_1_0(
                value["configuration"]
            )
        )
...
def deserialize_aws_json_1_0(data: dict) -> RdsDbInstanceStorage:
    ...
    if data.get("configuration") is not None:
        ...
        out["configuration"] = (
            capo_cost_optimization_hub.types.rds_db_instance_storage_configuration.deserialize_aws_json_1_0(
                data["configuration"]
            )
        )
