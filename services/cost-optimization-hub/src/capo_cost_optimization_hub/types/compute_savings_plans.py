...
if TYPE_CHECKING:
    import capo_cost_optimization_hub.types.compute_savings_plans_configuration
...
class ComputeSavingsPlans(TypedDict, closed=True):
    configuration: NotRequired[
        "capo_cost_optimization_hub.types.compute_savings_plans_configuration.ComputeSavingsPlansConfiguration"
    ]
...
def serialize_aws_json_1_0(value: ComputeSavingsPlans) -> dict:
    ...
    if "configuration" in value:
        ...
        out["configuration"] = (
            capo_cost_optimization_hub.types.compute_savings_plans_configuration.serialize_aws_json_1_0(
                value["configuration"]
            )
        )
...
def deserialize_aws_json_1_0(data: dict) -> ComputeSavingsPlans:
    ...
    if data.get("configuration") is not None:
        ...
        out["configuration"] = (
            capo_cost_optimization_hub.types.compute_savings_plans_configuration.deserialize_aws_json_1_0(
                data["configuration"]
            )
        )
