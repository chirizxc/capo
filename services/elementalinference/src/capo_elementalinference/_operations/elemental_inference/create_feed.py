"""Generated from Smithy shape ``com.amazonaws.elementalinference#CreateFeed``."""
...
def handle_response(
    ...
    out: capo_elementalinference.types.create_feed_response.CreateFeedResponse = (
        capo_elementalinference.types.create_feed_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/feed"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elementalinference.types.create_feed_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
