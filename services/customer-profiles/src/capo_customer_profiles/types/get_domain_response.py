...
class GetDomainResponse(TypedDict, closed=True):
    ...
    stats: NotRequired["capo_customer_profiles.types.domain_stats.DomainStats"]
...
def serialize_json(value: GetDomainResponse) -> dict:
    ...
    if "stats" in value:
        ...
        out["Stats"] = capo_customer_profiles.types.domain_stats.serialize_json(
            value["stats"]
        )
...
def deserialize_json(data: dict) -> GetDomainResponse:
    ...
    if data.get("Stats") is not None:
        ...
        out["stats"] = capo_customer_profiles.types.domain_stats.deserialize_json(
            data["Stats"]
        )
