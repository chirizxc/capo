...
class BatchPutProfileObjectResponse(TypedDict, closed=True):
    successful: NotRequired[
        "capo_customer_profiles.types.batch_put_profile_object_response_list.BatchPutProfileObjectResponseList"
    ]
    """<p>A list of items that were successfully added to the domain.</p>"""
    failed: NotRequired[
        "capo_customer_profiles.types.batch_put_profile_object_error_list.BatchPutProfileObjectErrorList"
    ]
...
def serialize_json(value: BatchPutProfileObjectResponse) -> dict:
    ...
    if "successful" in value:
        ...
        out["Successful"] = (
            capo_customer_profiles.types.batch_put_profile_object_response_list.serialize_json(
                value["successful"]
            )
        )
    if "failed" in value:
        ...
        out["Failed"] = (
            capo_customer_profiles.types.batch_put_profile_object_error_list.serialize_json(
                value["failed"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> BatchPutProfileObjectResponse:
    ...
    if data.get("Successful") is not None:
        ...
        out["successful"] = (
            capo_customer_profiles.types.batch_put_profile_object_response_list.deserialize_json(
                data["Successful"]
            )
        )
    if data.get("Failed") is not None:
        ...
        out["failed"] = (
            capo_customer_profiles.types.batch_put_profile_object_error_list.deserialize_json(
                data["Failed"]
            )
        )
    return out
