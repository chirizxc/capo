...
if TYPE_CHECKING:
    ...
    import capo_customer_profiles.types.s3_key_name


class S3ExportingLocation(TypedDict, closed=True):
    s3_bucket_name: NotRequired[
        "capo_customer_profiles.types.s3_bucket_name.s3BucketName"
    ]
    """<p>The name of the S3 bucket name where Identity Resolution Jobs write result files.</p>"""
    s3_key_name: NotRequired["capo_customer_profiles.types.s3_key_name.s3KeyName"]
...
def deserialize_json(data: dict) -> S3ExportingLocation:
    ...
    if data.get("S3KeyName") is not None:
