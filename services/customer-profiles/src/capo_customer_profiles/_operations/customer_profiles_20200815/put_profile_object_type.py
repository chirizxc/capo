...
def handle_response(
    ...
    out: capo_customer_profiles.types.put_profile_object_type_response.PutProfileObjectTypeResponse = capo_customer_profiles.types.put_profile_object_type_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{ObjectTypeName}", quote(input_["object_type_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.put_profile_object_type_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
