"""Generated from Smithy shape ``com.amazonaws.customerprofiles#DeleteProfileObject``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.delete_profile_object_response.DeleteProfileObjectResponse = capo_customer_profiles.types.delete_profile_object_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/domains/{DomainName}/profiles/objects/delete"
    url = url.replace("{DomainName}", quote(input_["domain_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.delete_profile_object_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_delete_profile_object(
