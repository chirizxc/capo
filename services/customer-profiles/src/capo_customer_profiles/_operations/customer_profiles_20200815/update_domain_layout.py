...
def handle_response(
    ...
    out: capo_customer_profiles.types.update_domain_layout_response.UpdateDomainLayoutResponse = capo_customer_profiles.types.update_domain_layout_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{LayoutDefinitionName}", quote(input_["layout_definition_name"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.update_domain_layout_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
