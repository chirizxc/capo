"""Generated from Smithy shape ``com.amazonaws.customerprofiles#CreateEventTrigger``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.create_event_trigger_response.CreateEventTriggerResponse = capo_customer_profiles.types.create_event_trigger_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{EventTriggerName}", quote(input_["event_trigger_name"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.create_event_trigger_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_event_trigger(
