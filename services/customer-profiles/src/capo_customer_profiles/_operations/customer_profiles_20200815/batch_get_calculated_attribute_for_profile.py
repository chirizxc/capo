...
def handle_response(
    ...
    out: capo_customer_profiles.types.batch_get_calculated_attribute_for_profile_response.BatchGetCalculatedAttributeForProfileResponse = capo_customer_profiles.types.batch_get_calculated_attribute_for_profile_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{CalculatedAttributeName}", quote(input_["calculated_attribute_name"], safe="")
    )
    url = url.replace("{DomainName}", quote(input_["domain_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.batch_get_calculated_attribute_for_profile_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
