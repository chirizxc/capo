"""Generated from Smithy shape ``com.amazonaws.customerprofiles#BatchGetProfile``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.batch_get_profile_response.BatchGetProfileResponse = capo_customer_profiles.types.batch_get_profile_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/domains/{DomainName}/batch-get-profiles"
    url = url.replace("{DomainName}", quote(input_["domain_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.batch_get_profile_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_batch_get_profile(
