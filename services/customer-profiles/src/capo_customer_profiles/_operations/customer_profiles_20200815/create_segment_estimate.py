"""Generated from Smithy shape ``com.amazonaws.customerprofiles#CreateSegmentEstimate``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.create_segment_estimate_response.CreateSegmentEstimateResponse = capo_customer_profiles.types.create_segment_estimate_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/domains/{DomainName}/segment-estimates"
    url = url.replace("{DomainName}", quote(input_["domain_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.create_segment_estimate_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_segment_estimate(
