...
def handle_response(
    ...
    out: capo_customer_profiles.types.update_recommender_response.UpdateRecommenderResponse = capo_customer_profiles.types.update_recommender_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{RecommenderName}", quote(input_["recommender_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.update_recommender_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
