"""Generated from Smithy shape ``com.amazonaws.customerprofiles#BatchPutProfileObject``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.batch_put_profile_object_response.BatchPutProfileObjectResponse = capo_customer_profiles.types.batch_put_profile_object_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/domains/{DomainName}/profiles/objects/batch-put-profile-object"
    )
    url = url.replace("{DomainName}", quote(input_["domain_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.batch_put_profile_object_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_put_profile_object(
