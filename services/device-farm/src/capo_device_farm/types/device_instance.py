...
class DeviceInstance(TypedDict, closed=True):
    ...
    status: NotRequired["capo_device_farm.types.instance_status.InstanceStatus"]
    """<p>The status of the device instance. Valid values are listed here.</p>"""
    udid: NotRequired["capo_device_farm.types.string.String"]
...
def serialize_aws_json_1_1(value: DeviceInstance) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_device_farm.types.instance_status.serialize_aws_json_1_1(
            value["status"]
        )
    if "udid" in value:
        out["udid"] = value["udid"]
