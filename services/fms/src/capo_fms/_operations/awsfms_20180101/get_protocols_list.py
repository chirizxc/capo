"""Generated from Smithy shape ``com.amazonaws.fms#GetProtocolsList``."""
...
import capo_fms._protocol.eventstream
...
def handle_response(
    ...
    out: capo_fms.types.get_protocols_list_response.GetProtocolsListResponse = (
        capo_fms.types.get_protocols_list_response.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_fms._auth._sigv4.build_sigv4_auth_scheme(
                "fms", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "AWSFMS_20180101.GetProtocolsList"
    body: bytes | None = json.dumps(
        capo_fms.types.get_protocols_list_request.serialize_aws_json_1_1(input_),
        allow_nan=False,
    ).encode()
...
def get_protocols_list(
    ...
    capo_fms.types.get_protocols_list_response.GetProtocolsListResponse, zapros.Response
]:
...
async def async_get_protocols_list(
