...
if TYPE_CHECKING:
    import capo_cloudwatch_events.types.connection

ConnectionResponseList: TypeAlias = list[
    "capo_cloudwatch_events.types.connection.Connection"
]
...
def serialize_aws_json_1_1(value: ConnectionResponseList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_cloudwatch_events.types.connection.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> ConnectionResponseList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_cloudwatch_events.types.connection.deserialize_aws_json_1_1(item)
        )
    return out
