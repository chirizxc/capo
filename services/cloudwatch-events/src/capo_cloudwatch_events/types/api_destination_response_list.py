...
if TYPE_CHECKING:
    import capo_cloudwatch_events.types.api_destination

ApiDestinationResponseList: TypeAlias = list[
    "capo_cloudwatch_events.types.api_destination.ApiDestination"
]
...
def serialize_aws_json_1_1(value: ApiDestinationResponseList) -> list:
    ...
    for item in value:
        out.append(
            capo_cloudwatch_events.types.api_destination.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> ApiDestinationResponseList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_cloudwatch_events.types.api_destination.deserialize_aws_json_1_1(item)
        )
    return out
