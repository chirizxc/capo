"""Generated from Smithy shape ``com.amazonaws.cloudwatchevents#ListApiDestinationsResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_cloudwatch_events.types.api_destination_response_list
    import capo_cloudwatch_events.types.next_token


class ListApiDestinationsResponse(TypedDict, closed=True):
    api_destinations: NotRequired[
        "capo_cloudwatch_events.types.api_destination_response_list.ApiDestinationResponseList"
    ]
    """<p>An array of <code>ApiDestination</code> objects that include information about an API destination.</p>"""
    next_token: NotRequired["capo_cloudwatch_events.types.next_token.NextToken"]
    """<p>A token you can use in a subsequent request to retrieve the next set of results.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: ListApiDestinationsResponse) -> dict:
    out: dict = {}
    if "api_destinations" in value:
        import capo_cloudwatch_events.types.api_destination_response_list

        out["ApiDestinations"] = (
            capo_cloudwatch_events.types.api_destination_response_list.serialize_aws_json_1_1(
                value["api_destinations"]
            )
        )
    if "next_token" in value:
        out["NextToken"] = value["next_token"]
    return out


def deserialize_aws_json_1_1(data: dict) -> ListApiDestinationsResponse:
    out: ListApiDestinationsResponse = {}  # type: ignore[typeddict-item]
    if data.get("ApiDestinations") is not None:
        import capo_cloudwatch_events.types.api_destination_response_list

        out["api_destinations"] = (
            capo_cloudwatch_events.types.api_destination_response_list.deserialize_aws_json_1_1(
                data["ApiDestinations"]
            )
        )
    if data.get("NextToken") is not None:
        out["next_token"] = data["NextToken"]
    return out
