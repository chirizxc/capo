...
class RemoveTargetsRequest(TypedDict, closed=True):
    ...
    force: "capo_cloudwatch_events.types.boolean.Boolean"
...
def serialize_aws_json_1_1(value: RemoveTargetsRequest) -> dict:
    ...
    out["Ids"] = capo_cloudwatch_events.types.target_id_list.serialize_aws_json_1_1(
        value["ids"]
    )
...
def deserialize_aws_json_1_1(data: dict) -> RemoveTargetsRequest:
    ...
    if data.get("Ids") is not None:
        ...
        out["ids"] = (
            capo_cloudwatch_events.types.target_id_list.deserialize_aws_json_1_1(
                data["Ids"]
            )
        )
    else:
