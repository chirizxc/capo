"""Generated from Smithy shape ``com.amazonaws.ivs#GetStreamKey``."""
...
def handle_response(
    ...
    out: capo_ivs.types.get_stream_key_response.GetStreamKeyResponse = (
        capo_ivs.types.get_stream_key_response.deserialize_json(
...
def build_request(
    ...
    input_: capo_ivs.types.get_stream_key_request.GetStreamKeyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/GetStreamKey"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.get_stream_key_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def get_stream_key(
    ...
    capo_ivs.types.get_stream_key_response.GetStreamKeyResponse, zapros.Response
]:
...
async def async_get_stream_key(
