"""Generated from Smithy shape ``com.amazonaws.ivs#InsertAdBreak``."""
...
def handle_response(
    ...
    out: capo_ivs.types.insert_ad_break_response.InsertAdBreakResponse = (
        capo_ivs.types.insert_ad_break_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/InsertAdBreak"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.insert_ad_break_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def insert_ad_break(
    ...
    capo_ivs.types.insert_ad_break_response.InsertAdBreakResponse, zapros.Response
]:
...
async def async_insert_ad_break(
