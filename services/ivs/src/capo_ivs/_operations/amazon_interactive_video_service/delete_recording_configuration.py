"""Generated from Smithy shape ``com.amazonaws.ivs#DeleteRecordingConfiguration``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteRecordingConfiguration"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.delete_recording_configuration_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_delete_recording_configuration(
