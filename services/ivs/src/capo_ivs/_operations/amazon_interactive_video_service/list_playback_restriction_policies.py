"""Generated from Smithy shape ``com.amazonaws.ivs#ListPlaybackRestrictionPolicies``."""
...
import capo_ivs._protocol.eventstream
...
import capo_ivs.types.playback_restriction_policy_list
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_ivs.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_ivs.types.list_playback_restriction_policies_response.ListPlaybackRestrictionPoliciesResponse = capo_ivs.types.list_playback_restriction_policies_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_ivs._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListPlaybackRestrictionPolicies"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.list_playback_restriction_policies_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_playback_restriction_policies(
