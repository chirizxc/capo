"""Generated from Smithy shape ``com.amazonaws.ivs#StartViewerSessionRevocation``."""
...
def handle_response(
    ...
    out: capo_ivs.types.start_viewer_session_revocation_response.StartViewerSessionRevocationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartViewerSessionRevocation"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.start_viewer_session_revocation_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_start_viewer_session_revocation(
