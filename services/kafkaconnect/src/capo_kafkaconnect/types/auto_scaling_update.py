...
class AutoScalingUpdate(TypedDict, closed=True):
    ...
    scale_in_policy: (
        "capo_kafkaconnect.types.scale_in_policy_update.ScaleInPolicyUpdate"
    )
    """<p>The target scale-in policy for the connector.</p>"""
    scale_out_policy: (
        "capo_kafkaconnect.types.scale_out_policy_update.ScaleOutPolicyUpdate"
    )
...
def serialize_json(value: AutoScalingUpdate) -> dict:
    ...
    out["scaleInPolicy"] = (
        capo_kafkaconnect.types.scale_in_policy_update.serialize_json(
    ...
    out["scaleOutPolicy"] = (
        capo_kafkaconnect.types.scale_out_policy_update.serialize_json(
...
def deserialize_json(data: dict) -> AutoScalingUpdate:
    ...
    if data.get("scaleInPolicy") is not None:
        ...
        out["scale_in_policy"] = (
            capo_kafkaconnect.types.scale_in_policy_update.deserialize_json(
                data["scaleInPolicy"]
            )
        )
    else:
    ...
    if data.get("scaleOutPolicy") is not None:
        ...
        out["scale_out_policy"] = (
            capo_kafkaconnect.types.scale_out_policy_update.deserialize_json(
                data["scaleOutPolicy"]
            )
        )
    else:
