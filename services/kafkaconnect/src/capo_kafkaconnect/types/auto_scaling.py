...
if TYPE_CHECKING:
    ...
    import capo_kafkaconnect.types.scale_in_policy
    import capo_kafkaconnect.types.scale_out_policy
...
class AutoScaling(TypedDict, closed=True):
    ...
    scale_in_policy: NotRequired[
        "capo_kafkaconnect.types.scale_in_policy.ScaleInPolicy"
    ]
    """<p>The scale-in policy for the connector.</p>"""
    scale_out_policy: NotRequired[
        "capo_kafkaconnect.types.scale_out_policy.ScaleOutPolicy"
    ]
...
def serialize_json(value: AutoScaling) -> dict:
    ...
    if "scale_in_policy" in value:
        ...
        out["scaleInPolicy"] = capo_kafkaconnect.types.scale_in_policy.serialize_json(
            value["scale_in_policy"]
        )
    if "scale_out_policy" in value:
        import capo_kafkaconnect.types.scale_out_policy

        out["scaleOutPolicy"] = capo_kafkaconnect.types.scale_out_policy.serialize_json(
...
def deserialize_json(data: dict) -> AutoScaling:
    ...
    if data.get("scaleInPolicy") is not None:
        ...
        out["scale_in_policy"] = (
            capo_kafkaconnect.types.scale_in_policy.deserialize_json(
                data["scaleInPolicy"]
            )
        )
    if data.get("scaleOutPolicy") is not None:
        ...
        out["scale_out_policy"] = (
            capo_kafkaconnect.types.scale_out_policy.deserialize_json(
                data["scaleOutPolicy"]
            )
        )
