...
__listOfConnectorOperationSummary: TypeAlias = list[
    "capo_kafkaconnect.types.connector_operation_summary.ConnectorOperationSummary"
]
...
def serialize_json(value: __listOfConnectorOperationSummary) -> list:
    ...
    for item in value:
        out.append(
            capo_kafkaconnect.types.connector_operation_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> __listOfConnectorOperationSummary:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_kafkaconnect.types.connector_operation_summary.deserialize_json(item)
        )
    return out
