"""Generated from Smithy shape ``com.amazonaws.kafkaconnect#WorkerLogDeliveryDescription``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_kafkaconnect.types.cloud_watch_logs_log_delivery_description
    import capo_kafkaconnect.types.firehose_log_delivery_description
    import capo_kafkaconnect.types.s3_log_delivery_description


class WorkerLogDeliveryDescription(TypedDict, closed=True):
    cloud_watch_logs: NotRequired[
        "capo_kafkaconnect.types.cloud_watch_logs_log_delivery_description.CloudWatchLogsLogDeliveryDescription"
    ]
    """<p>Details about delivering logs to Amazon CloudWatch Logs.</p>"""
    firehose: NotRequired[
        "capo_kafkaconnect.types.firehose_log_delivery_description.FirehoseLogDeliveryDescription"
    ]
    """<p>Details about delivering logs to Amazon Kinesis Data Firehose.</p>"""
    s3: NotRequired[
        "capo_kafkaconnect.types.s3_log_delivery_description.S3LogDeliveryDescription"
    ]
    """<p>Details about delivering logs to Amazon S3.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: WorkerLogDeliveryDescription) -> dict:
    out: dict = {}
    if "cloud_watch_logs" in value:
        import capo_kafkaconnect.types.cloud_watch_logs_log_delivery_description

        out["cloudWatchLogs"] = (
            capo_kafkaconnect.types.cloud_watch_logs_log_delivery_description.serialize_json(
                value["cloud_watch_logs"]
            )
        )
    if "firehose" in value:
        import capo_kafkaconnect.types.firehose_log_delivery_description

        out["firehose"] = (
            capo_kafkaconnect.types.firehose_log_delivery_description.serialize_json(
                value["firehose"]
            )
        )
    if "s3" in value:
        import capo_kafkaconnect.types.s3_log_delivery_description

        out["s3"] = capo_kafkaconnect.types.s3_log_delivery_description.serialize_json(
            value["s3"]
        )
    return out


def deserialize_json(data: dict) -> WorkerLogDeliveryDescription:
    out: WorkerLogDeliveryDescription = {}  # type: ignore[typeddict-item]
    if data.get("cloudWatchLogs") is not None:
        import capo_kafkaconnect.types.cloud_watch_logs_log_delivery_description

        out["cloud_watch_logs"] = (
            capo_kafkaconnect.types.cloud_watch_logs_log_delivery_description.deserialize_json(
                data["cloudWatchLogs"]
            )
        )
    if data.get("firehose") is not None:
        import capo_kafkaconnect.types.firehose_log_delivery_description

        out["firehose"] = (
            capo_kafkaconnect.types.firehose_log_delivery_description.deserialize_json(
                data["firehose"]
            )
        )
    if data.get("s3") is not None:
        import capo_kafkaconnect.types.s3_log_delivery_description

        out["s3"] = (
            capo_kafkaconnect.types.s3_log_delivery_description.deserialize_json(
                data["s3"]
            )
        )
    return out
