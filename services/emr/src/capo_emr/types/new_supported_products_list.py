"""Generated from Smithy shape ``com.amazonaws.emr#NewSupportedProductsList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_emr.types.supported_product_config

NewSupportedProductsList: TypeAlias = list[
    "capo_emr.types.supported_product_config.SupportedProductConfig"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: NewSupportedProductsList) -> list:
    import capo_emr.types.supported_product_config

    out: list = []
    for item in value:
        out.append(capo_emr.types.supported_product_config.serialize_aws_json_1_1(item))
    return out


def deserialize_aws_json_1_1(data: list) -> NewSupportedProductsList:
    import capo_emr.types.supported_product_config

    out: NewSupportedProductsList = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_emr.types.supported_product_config.deserialize_aws_json_1_1(item)
        )
    return out
