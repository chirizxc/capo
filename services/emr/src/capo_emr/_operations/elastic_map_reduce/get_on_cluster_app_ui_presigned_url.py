"""Generated from Smithy shape ``com.amazonaws.emr#GetOnClusterAppUIPresignedURL``."""
...
import capo_emr._protocol.eventstream
...
def handle_response(
    ...
    out: capo_emr.types.get_on_cluster_app_ui_presigned_url_output.GetOnClusterAppUIPresignedURLOutput = capo_emr.types.get_on_cluster_app_ui_presigned_url_output.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_emr._auth._sigv4.build_sigv4_auth_scheme(
                "elasticmapreduce", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "ElasticMapReduce.GetOnClusterAppUIPresignedURL"
    body: bytes | None = json.dumps(
        capo_emr.types.get_on_cluster_app_ui_presigned_url_input.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_on_cluster_app_ui_presigned_url(
