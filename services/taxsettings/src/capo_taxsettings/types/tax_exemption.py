"""Generated from Smithy shape ``com.amazonaws.taxsettings#TaxExemption``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_taxsettings.errors import DeserializationError

if TYPE_CHECKING:
    import datetime

    import capo_taxsettings.types.authority
    import capo_taxsettings.types.entity_exemption_account_status
    import capo_taxsettings.types.tax_exemption_type


class TaxExemption(TypedDict, closed=True):
    authority: "capo_taxsettings.types.authority.Authority"
    """<p>The address domain associate with tax exemption. </p>"""
    tax_exemption_type: "capo_taxsettings.types.tax_exemption_type.TaxExemptionType"
    """<p>The tax exemption type. </p>"""
    effective_date: NotRequired["datetime.datetime"]
    """<p>The tax exemption effective date. </p>"""
    expiration_date: NotRequired["datetime.datetime"]
    """<p>The tax exemption expiration date. </p>"""
    system_effective_date: NotRequired["datetime.datetime"]
    """<p>The tax exemption recording time in the <code>TaxSettings</code> system. </p>"""
    status: NotRequired[
        "capo_taxsettings.types.entity_exemption_account_status.EntityExemptionAccountStatus"
    ]
    """<p>The tax exemption status. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: TaxExemption) -> dict:
    out: dict = {}
    import capo_taxsettings.types.authority

    out["authority"] = capo_taxsettings.types.authority.serialize_json(
        value["authority"]
    )
    import capo_taxsettings.types.tax_exemption_type

    out["taxExemptionType"] = capo_taxsettings.types.tax_exemption_type.serialize_json(
        value["tax_exemption_type"]
    )
    if "effective_date" in value:
        import capo_taxsettings.types._prelude.timestamp

        out["effectiveDate"] = capo_taxsettings.types._prelude.timestamp.serialize_json(
            value["effective_date"]
        )
    if "expiration_date" in value:
        import capo_taxsettings.types._prelude.timestamp

        out["expirationDate"] = (
            capo_taxsettings.types._prelude.timestamp.serialize_json(
                value["expiration_date"]
            )
        )
    if "system_effective_date" in value:
        import capo_taxsettings.types._prelude.timestamp

        out["systemEffectiveDate"] = (
            capo_taxsettings.types._prelude.timestamp.serialize_json(
                value["system_effective_date"]
            )
        )
    if "status" in value:
        import capo_taxsettings.types.entity_exemption_account_status

        out["status"] = (
            capo_taxsettings.types.entity_exemption_account_status.serialize_json(
                value["status"]
            )
        )
    return out


def deserialize_json(data: dict) -> TaxExemption:
    out: TaxExemption = {}  # type: ignore[typeddict-item]
    if data.get("authority") is not None:
        import capo_taxsettings.types.authority

        out["authority"] = capo_taxsettings.types.authority.deserialize_json(
            data["authority"]
        )
    else:
        raise DeserializationError("TaxExemption.authority required")
    if data.get("taxExemptionType") is not None:
        import capo_taxsettings.types.tax_exemption_type

        out["tax_exemption_type"] = (
            capo_taxsettings.types.tax_exemption_type.deserialize_json(
                data["taxExemptionType"]
            )
        )
    else:
        raise DeserializationError("TaxExemption.tax_exemption_type required")
    if data.get("effectiveDate") is not None:
        import capo_taxsettings.types._prelude.timestamp

        out["effective_date"] = (
            capo_taxsettings.types._prelude.timestamp.deserialize_json(
                data["effectiveDate"]
            )
        )
    if data.get("expirationDate") is not None:
        import capo_taxsettings.types._prelude.timestamp

        out["expiration_date"] = (
            capo_taxsettings.types._prelude.timestamp.deserialize_json(
                data["expirationDate"]
            )
        )
    if data.get("systemEffectiveDate") is not None:
        import capo_taxsettings.types._prelude.timestamp

        out["system_effective_date"] = (
            capo_taxsettings.types._prelude.timestamp.deserialize_json(
                data["systemEffectiveDate"]
            )
        )
    if data.get("status") is not None:
        import capo_taxsettings.types.entity_exemption_account_status

        out["status"] = (
            capo_taxsettings.types.entity_exemption_account_status.deserialize_json(
                data["status"]
            )
        )
    return out
