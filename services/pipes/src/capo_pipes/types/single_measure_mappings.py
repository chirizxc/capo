...
if TYPE_CHECKING:
    import capo_pipes.types.single_measure_mapping

SingleMeasureMappings: TypeAlias = list[
    "capo_pipes.types.single_measure_mapping.SingleMeasureMapping"
]
...
def serialize_json(value: SingleMeasureMappings) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_pipes.types.single_measure_mapping.serialize_json(item))
    return out
