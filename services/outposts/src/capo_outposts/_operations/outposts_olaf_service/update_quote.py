...
def handle_response(
    ...
    out: capo_outposts.types.update_quote_output.UpdateQuoteOutput = (
        capo_outposts.types.update_quote_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{QuoteIdentifier}", quote(input_["quote_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_outposts.types.update_quote_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def update_quote(
    ...
    input_: capo_outposts.types.update_quote_input.UpdateQuoteInput,
) -> tuple[capo_outposts.types.update_quote_output.UpdateQuoteOutput, zapros.Response]:
