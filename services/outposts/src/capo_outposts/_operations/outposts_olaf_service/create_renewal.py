"""Generated from Smithy shape ``com.amazonaws.outposts#CreateRenewal``."""
...
def handle_response(
    ...
    out: capo_outposts.types.create_renewal_output.CreateRenewalOutput = (
        capo_outposts.types.create_renewal_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/renewals"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_outposts.types.create_renewal_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def create_renewal(
    ...
    capo_outposts.types.create_renewal_output.CreateRenewalOutput, zapros.Response
]:
...
async def async_create_renewal(
