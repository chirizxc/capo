...
def build_request(
    ...
    import capo_outposts.types.supported_storage_enum
    ...
    if "item_class_filter" in input_:
        for item in input_["item_class_filter"]:
            params.append(
                (
                    "ItemClassFilter",
                    capo_outposts.types.catalog_item_class.serialize_json(item),
                )
            )
    if "supported_storage_filter" in input_:
        for item in input_["supported_storage_filter"]:
            params.append(
                (
                    "SupportedStorageFilter",
                    capo_outposts.types.supported_storage_enum.serialize_json(item),
                )
            )
    if "ec2_family_filter" in input_:
        for item in input_["ec2_family_filter"]:
            params.append(("EC2FamilyFilter", item))
