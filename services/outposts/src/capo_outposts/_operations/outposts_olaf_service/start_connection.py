"""Generated from Smithy shape ``com.amazonaws.outposts#StartConnection``."""
...
def handle_response(
    ...
    out: capo_outposts.types.start_connection_response.StartConnectionResponse = (
        capo_outposts.types.start_connection_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/connections"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_outposts.types.start_connection_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
