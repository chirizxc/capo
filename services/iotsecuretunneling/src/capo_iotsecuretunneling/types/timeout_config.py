...
if TYPE_CHECKING:
    import capo_iotsecuretunneling.types.timeout_in_min


class TimeoutConfig(TypedDict, closed=True):
    max_lifetime_timeout_minutes: NotRequired[
        "capo_iotsecuretunneling.types.timeout_in_min.TimeoutInMin"
    ]
...
def deserialize_aws_json_1_1(data: dict) -> TimeoutConfig:
    ...
    if data.get("maxLifetimeTimeoutMinutes") is not None:
