"""Generated from Smithy shape ``com.amazonaws.kendra#CreateQuerySuggestionsBlockList``."""
...
import capo_kendra._protocol.eventstream
...
def handle_response(
    ...
    out: capo_kendra.types.create_query_suggestions_block_list_response.CreateQuerySuggestionsBlockListResponse = capo_kendra.types.create_query_suggestions_block_list_response.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_kendra._auth._sigv4.build_sigv4_auth_scheme(
                "kendra", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "AWSKendraFrontendService.CreateQuerySuggestionsBlockList"
    body: bytes | None = json.dumps(
        capo_kendra.types.create_query_suggestions_block_list_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_query_suggestions_block_list(
