"""Generated from Smithy shape ``com.amazonaws.firehose#PutRecordBatchRequestEntryList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_firehose.types.record

PutRecordBatchRequestEntryList: TypeAlias = list["capo_firehose.types.record.Record"]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: PutRecordBatchRequestEntryList) -> list:
    import capo_firehose.types.record

    out: list = []
    for item in value:
        out.append(capo_firehose.types.record.serialize_aws_json_1_1(item))
    return out


def deserialize_aws_json_1_1(data: list) -> PutRecordBatchRequestEntryList:
    import capo_firehose.types.record

    out: PutRecordBatchRequestEntryList = []
    for item in data:
        if item is None:
            continue
        out.append(capo_firehose.types.record.deserialize_aws_json_1_1(item))
    return out
