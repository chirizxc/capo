"""Generated from Smithy shape ``com.amazonaws.firehose#ElasticsearchRetryOptions``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_firehose.types.elasticsearch_retry_duration_in_seconds


class ElasticsearchRetryOptions(TypedDict, closed=True):
    duration_in_seconds: NotRequired[
        "capo_firehose.types.elasticsearch_retry_duration_in_seconds.ElasticsearchRetryDurationInSeconds"
    ]
    """<p>After an initial failure to deliver to Amazon OpenSearch Service, the total amount of time during which Firehose retries delivery (including the first attempt). After this time has elapsed, the failed documents are written to Amazon S3. Default value is 300 seconds (5 minutes). A value of 0 (zero) results in no retries.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: ElasticsearchRetryOptions) -> dict:
    out: dict = {}
    if "duration_in_seconds" in value:
        out["DurationInSeconds"] = value["duration_in_seconds"]
    return out


def deserialize_aws_json_1_1(data: dict) -> ElasticsearchRetryOptions:
    out: ElasticsearchRetryOptions = {}  # type: ignore[typeddict-item]
    if data.get("DurationInSeconds") is not None:
        out["duration_in_seconds"] = data["DurationInSeconds"]
    return out
