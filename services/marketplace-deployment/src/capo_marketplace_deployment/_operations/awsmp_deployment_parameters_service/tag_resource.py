"""Generated from Smithy shape ``com.amazonaws.marketplacedeployment#TagResource``."""
...
import capo_marketplace_deployment._protocol.eventstream
...
def handle_response(
    ...
    out: capo_marketplace_deployment.types.tag_resource_response.TagResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = (
                capo_marketplace_deployment._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_marketplace_deployment.types.tag_resource_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
