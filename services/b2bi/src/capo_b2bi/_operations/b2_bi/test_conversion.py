"""Generated from Smithy shape ``com.amazonaws.b2bi#TestConversion``."""
...
def handle_response(
    ...
    out: capo_b2bi.types.test_conversion_response.TestConversionResponse = (
        capo_b2bi.types.test_conversion_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/testconversion"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "B2BI.TestConversion"
    body: bytes | None = json.dumps(
        capo_b2bi.types.test_conversion_request.serialize_aws_json_1_0(input_),
        allow_nan=False,
    ).encode()
...
def test_conversion(
    ...
    capo_b2bi.types.test_conversion_response.TestConversionResponse, zapros.Response
]:
...
async def async_test_conversion(
