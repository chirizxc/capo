"""Generated from Smithy shape ``com.amazonaws.b2bi#UpdateCapability``."""
...
def handle_response(
    ...
    out: capo_b2bi.types.update_capability_response.UpdateCapabilityResponse = (
        capo_b2bi.types.update_capability_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/capabilities/{capabilityId}"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "B2BI.UpdateCapability"
    body: bytes | None = json.dumps(
        capo_b2bi.types.update_capability_request.serialize_aws_json_1_0(input_),
        allow_nan=False,
    ).encode()
...
def update_capability(
    ...
    capo_b2bi.types.update_capability_response.UpdateCapabilityResponse, zapros.Response
]:
...
async def async_update_capability(
