"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancingv2#ModifyIpPools``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ModifyIpPoolsResult")
    out: capo_elastic_load_balancing_v2.types.modify_ip_pools_output.ModifyIpPoolsOutput = capo_elastic_load_balancing_v2.types.modify_ip_pools_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ModifyIpPools"))
    pairs.append(("Version", "2015-12-01"))
    capo_elastic_load_balancing_v2.types.modify_ip_pools_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_modify_ip_pools(
