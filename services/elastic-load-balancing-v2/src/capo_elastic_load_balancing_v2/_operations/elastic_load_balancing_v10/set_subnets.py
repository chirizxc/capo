"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancingv2#SetSubnets``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("SetSubnetsResult")
    out: capo_elastic_load_balancing_v2.types.set_subnets_output.SetSubnetsOutput = (
        capo_elastic_load_balancing_v2.types.set_subnets_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "SetSubnets"))
    pairs.append(("Version", "2015-12-01"))
    capo_elastic_load_balancing_v2.types.set_subnets_input.serialize_query(
        input_, pairs, ""
    )
