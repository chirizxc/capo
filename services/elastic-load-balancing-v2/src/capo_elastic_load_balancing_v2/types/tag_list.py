...
if TYPE_CHECKING:
    import capo_elastic_load_balancing_v2.types.tag

TagList: TypeAlias = list["capo_elastic_load_balancing_v2.types.tag.Tag"]
...
def serialize_query(value: TagList, pairs: list[tuple[str, str]], prefix: str) -> None:
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.tag.serialize_query(
...
def deserialize_query(el: Element) -> TagList:
    ...
    for child in el.findall("member"):
        out.append(capo_elastic_load_balancing_v2.types.tag.deserialize_query(child))
    return out
