...
if TYPE_CHECKING:
    import capo_elastic_load_balancing_v2.types.load_balancer_address

LoadBalancerAddresses: TypeAlias = list[
    "capo_elastic_load_balancing_v2.types.load_balancer_address.LoadBalancerAddress"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.load_balancer_address.serialize_query(
...
def deserialize_query(el: Element) -> LoadBalancerAddresses:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.load_balancer_address.deserialize_query(
                child
            )
        )
    return out
