...
if TYPE_CHECKING:
    import capo_elastic_load_balancing_v2.types.ssl_policy

SslPolicies: TypeAlias = list[
    "capo_elastic_load_balancing_v2.types.ssl_policy.SslPolicy"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.ssl_policy.serialize_query(
...
def deserialize_query(el: Element) -> SslPolicies:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.ssl_policy.deserialize_query(child)
        )
    return out
