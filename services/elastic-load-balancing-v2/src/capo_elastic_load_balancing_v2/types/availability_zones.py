...
if TYPE_CHECKING:
    import capo_elastic_load_balancing_v2.types.availability_zone

AvailabilityZones: TypeAlias = list[
    "capo_elastic_load_balancing_v2.types.availability_zone.AvailabilityZone"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.availability_zone.serialize_query(
...
def deserialize_query(el: Element) -> AvailabilityZones:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.availability_zone.deserialize_query(
                child
            )
        )
    return out
