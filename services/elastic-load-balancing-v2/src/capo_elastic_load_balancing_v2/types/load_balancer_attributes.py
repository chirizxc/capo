...
if TYPE_CHECKING:
    import capo_elastic_load_balancing_v2.types.load_balancer_attribute

LoadBalancerAttributes: TypeAlias = list[
    "capo_elastic_load_balancing_v2.types.load_balancer_attribute.LoadBalancerAttribute"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.load_balancer_attribute.serialize_query(
...
def deserialize_query(el: Element) -> LoadBalancerAttributes:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.load_balancer_attribute.deserialize_query(
                child
            )
        )
    return out
