...
if TYPE_CHECKING:
    import capo_voice_id.types.enrollment_job_fraud_detection_config
    import capo_voice_id.types.existing_enrollment_action


class EnrollmentConfig(TypedDict, closed=True):
    existing_enrollment_action: NotRequired[
        "capo_voice_id.types.existing_enrollment_action.ExistingEnrollmentAction"
    ]
    ...
    fraud_detection_config: NotRequired[
        "capo_voice_id.types.enrollment_job_fraud_detection_config.EnrollmentJobFraudDetectionConfig"
    ]
...
def serialize_aws_json_1_0(value: EnrollmentConfig) -> dict:
    ...
    if "fraud_detection_config" in value:
        ...
        out["FraudDetectionConfig"] = (
            capo_voice_id.types.enrollment_job_fraud_detection_config.serialize_aws_json_1_0(
...
def deserialize_aws_json_1_0(data: dict) -> EnrollmentConfig:
    ...
    if data.get("ExistingEnrollmentAction") is not None:
        out["existing_enrollment_action"] = data["ExistingEnrollmentAction"]
    if data.get("FraudDetectionConfig") is not None:
        import capo_voice_id.types.enrollment_job_fraud_detection_config

        out["fraud_detection_config"] = (
            capo_voice_id.types.enrollment_job_fraud_detection_config.deserialize_aws_json_1_0(
