"""Generated from Smithy shape ``com.amazonaws.aiops#PutInvestigationGroupPolicy``."""
...
import capo_aiops._protocol.eventstream
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_aiops.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_aiops.types.put_investigation_group_policy_response.PutInvestigationGroupPolicyResponse = capo_aiops.types.put_investigation_group_policy_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_aiops._auth._sigv4.build_sigv4_auth_scheme(
                "aiops", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_aiops.types.put_investigation_group_policy_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_put_investigation_group_policy(
