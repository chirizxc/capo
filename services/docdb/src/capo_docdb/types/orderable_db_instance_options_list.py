...
OrderableDBInstanceOptionsList: TypeAlias = list[
    "capo_docdb.types.orderable_db_instance_option.OrderableDBInstanceOption"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_docdb.types.orderable_db_instance_option.serialize_query(
...
def deserialize_query(el: Element) -> OrderableDBInstanceOptionsList:
    ...
    for child in el.findall("OrderableDBInstanceOption"):
        out.append(
            capo_docdb.types.orderable_db_instance_option.deserialize_query(child)
        )
    return out
