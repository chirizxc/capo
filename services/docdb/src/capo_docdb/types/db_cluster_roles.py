...
if TYPE_CHECKING:
    import capo_docdb.types.db_cluster_role

DBClusterRoles: TypeAlias = list["capo_docdb.types.db_cluster_role.DBClusterRole"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_docdb.types.db_cluster_role.serialize_query(
...
def deserialize_query(el: Element) -> DBClusterRoles:
    ...
    for child in el.findall("DBClusterRole"):
        out.append(capo_docdb.types.db_cluster_role.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    if not value:
        ...
        return
    for n, item in enumerate(value, 1):
        capo_docdb.types.db_cluster_role.serialize_query(item, pairs, f"{prefix}.{n}")
