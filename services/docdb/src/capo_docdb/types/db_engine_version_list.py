"""Generated from Smithy shape ``com.amazonaws.docdb#DBEngineVersionList``."""
...
DBEngineVersionList: TypeAlias = list[
    "capo_docdb.types.db_engine_version.DBEngineVersion"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_docdb.types.db_engine_version.serialize_query(
...
def deserialize_query(el: Element) -> DBEngineVersionList:
    ...
    for child in el.findall("DBEngineVersion"):
        out.append(capo_docdb.types.db_engine_version.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    if not value:
        ...
        return
    for n, item in enumerate(value, 1):
        capo_docdb.types.db_engine_version.serialize_query(item, pairs, f"{prefix}.{n}")
