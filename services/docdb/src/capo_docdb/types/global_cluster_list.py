...
GlobalClusterList: TypeAlias = list["capo_docdb.types.global_cluster.GlobalCluster"]
...
def deserialize_query(el: Element) -> GlobalClusterList:
    ...
    for child in el.findall("GlobalClusterMember"):
        out.append(capo_docdb.types.global_cluster.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    if not value:
        ...
        return
    for n, item in enumerate(value, 1):
        capo_docdb.types.global_cluster.serialize_query(item, pairs, f"{prefix}.{n}")
