...
if TYPE_CHECKING:
    import capo_docdb.types.pending_maintenance_action

PendingMaintenanceActionDetails: TypeAlias = list[
    "capo_docdb.types.pending_maintenance_action.PendingMaintenanceAction"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_docdb.types.pending_maintenance_action.serialize_query(
...
def deserialize_query(el: Element) -> PendingMaintenanceActionDetails:
    ...
    for child in el.findall("PendingMaintenanceAction"):
        out.append(capo_docdb.types.pending_maintenance_action.deserialize_query(child))
    return out
