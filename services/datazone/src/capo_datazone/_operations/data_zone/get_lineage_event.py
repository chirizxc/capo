...
def handle_response(
    ...
    out: capo_datazone.types.get_lineage_event_output.GetLineageEventOutput = {
        "event": b"".join(response.iter_raw())
    }  # type: ignore[typeddict-item]
    if "Domain-Id" in response.headers:
        out["domain_id"] = response.headers["Domain-Id"]
    if "Id" in response.headers:
        out["id"] = response.headers["Id"]
    if "Created-By" in response.headers:
        out["created_by"] = response.headers["Created-By"]
...
async def async_handle_response(
    ...
    out: capo_datazone.types.get_lineage_event_output.GetLineageEventOutput = {
        "event": b"".join([chunk async for chunk in response.async_iter_raw()])
    }  # type: ignore[typeddict-item]
    if "Domain-Id" in response.headers:
        out["domain_id"] = response.headers["Domain-Id"]
    if "Id" in response.headers:
...
def get_lineage_event(
    ...
    capo_datazone.types.get_lineage_event_output.GetLineageEventOutput, zapros.Response
]:
