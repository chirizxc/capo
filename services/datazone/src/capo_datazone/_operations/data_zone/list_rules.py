...
def handle_response(
    ...
    out: capo_datazone.types.list_rules_output.ListRulesOutput = (
        capo_datazone.types.list_rules_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{targetType}",
        quote(
            capo_datazone.types.rule_target_type.serialize_json(input_["target_type"]),
            safe="",
        ),
    )
    url = url.replace("{targetIdentifier}", quote(input_["target_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    if "rule_type" in input_:
        params.append(
            (
                "ruleType",
                capo_datazone.types.rule_type.serialize_json(input_["rule_type"]),
            )
        )
    if "action" in input_:
        params.append(
            (
                "ruleAction",
                capo_datazone.types.rule_action.serialize_json(input_["action"]),
            )
        )
    if "project_ids" in input_:
        for item in input_["project_ids"]:
            params.append(("projectIds", item))
    if "asset_types" in input_:
        for item in input_["asset_types"]:
            params.append(("assetTypes", item))
    if "data_product" in input_:
        params.append(("dataProduct", "true" if input_["data_product"] else "false"))
    if "include_cascaded" in input_:
        params.append(
            ("includeCascaded", "true" if input_["include_cascaded"] else "false")
        )
...
def list_rules(
    ...
    input_: capo_datazone.types.list_rules_input.ListRulesInput,
) -> tuple[capo_datazone.types.list_rules_output.ListRulesOutput, zapros.Response]:
