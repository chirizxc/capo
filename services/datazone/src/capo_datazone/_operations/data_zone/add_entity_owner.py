...
def handle_response(
    ...
    out: capo_datazone.types.add_entity_owner_output.AddEntityOwnerOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{entityType}",
        quote(
            capo_datazone.types.data_zone_entity_type.serialize_json(
                input_["entity_type"]
            ),
            safe="",
        ),
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.add_entity_owner_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def add_entity_owner(
    ...
    capo_datazone.types.add_entity_owner_output.AddEntityOwnerOutput, zapros.Response
]:
