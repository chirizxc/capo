"""Generated from Smithy shape ``com.amazonaws.datazone#GetUserProfile``."""
...
def handle_response(
    ...
    out: capo_datazone.types.get_user_profile_output.GetUserProfileOutput = (
        capo_datazone.types.get_user_profile_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{userIdentifier}", quote(input_["user_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    if "type" in input_:
        params.append(
            (
                "type",
                capo_datazone.types.user_profile_type.serialize_json(input_["type"]),
            )
        )
    if "session_name" in input_:
        params.append(("sessionName", input_["session_name"]))
...
def get_user_profile(
    ...
    capo_datazone.types.get_user_profile_output.GetUserProfileOutput, zapros.Response
]:
