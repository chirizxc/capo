"""Generated from Smithy shape ``com.amazonaws.datazone#Search``."""
...
def handle_response(
    ...
    out: capo_datazone.types.search_output.SearchOutput = (
        capo_datazone.types.search_output.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_datazone.types.search_output.SearchOutput = (
        capo_datazone.types.search_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/search"
    url = url.replace("{domainIdentifier}", quote(input_["domain_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.search_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def search(
    options: OperationOptions, input_: capo_datazone.types.search_input.SearchInput
) -> tuple[capo_datazone.types.search_output.SearchOutput, zapros.Response]:
...
async def async_search(
    options: AsyncOperationOptions, input_: capo_datazone.types.search_input.SearchInput
) -> tuple[capo_datazone.types.search_output.SearchOutput, zapros.Response]:
