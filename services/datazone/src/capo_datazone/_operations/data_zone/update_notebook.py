"""Generated from Smithy shape ``com.amazonaws.datazone#UpdateNotebook``."""
...
def handle_response(
    ...
    out: capo_datazone.types.update_notebook_output.UpdateNotebookOutput = (
        capo_datazone.types.update_notebook_output.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/v2/domains/{domainIdentifier}/notebooks/{identifier}"
    )
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.update_notebook_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_notebook(
    ...
    capo_datazone.types.update_notebook_output.UpdateNotebookOutput, zapros.Response
]:
