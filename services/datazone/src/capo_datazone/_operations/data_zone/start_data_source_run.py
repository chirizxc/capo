...
def handle_response(
    ...
    out: capo_datazone.types.start_data_source_run_output.StartDataSourceRunOutput = (
        capo_datazone.types.start_data_source_run_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{dataSourceIdentifier}", quote(input_["data_source_identifier"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.start_data_source_run_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
