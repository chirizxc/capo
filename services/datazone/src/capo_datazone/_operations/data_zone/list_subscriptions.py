...
import capo_datazone._protocol.eventstream
...
def handle_response(
    ...
    out: capo_datazone.types.list_subscriptions_output.ListSubscriptionsOutput = (
        capo_datazone.types.list_subscriptions_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_datazone._auth._sigv4.build_sigv4_auth_scheme(
                "datazone", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace("{domainIdentifier}", quote(input_["domain_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    if "subscription_request_identifier" in input_:
        params.append(
            ("subscriptionRequestIdentifier", input_["subscription_request_identifier"])
        )
    if "status" in input_:
        params.append(
            (
                "status",
                capo_datazone.types.subscription_status.serialize_json(
                    input_["status"]
                ),
            )
        )
    if "subscribed_listing_id" in input_:
        params.append(("subscribedListingId", input_["subscribed_listing_id"]))
    if "owning_project_id" in input_:
        params.append(("owningProjectId", input_["owning_project_id"]))
    if "owning_iam_principal_arn" in input_:
        params.append(("owningIamPrincipalArn", input_["owning_iam_principal_arn"]))
    if "owning_user_id" in input_:
        params.append(("owningUserId", input_["owning_user_id"]))
    if "owning_group_id" in input_:
        params.append(("owningGroupId", input_["owning_group_id"]))
    if "approver_project_id" in input_:
        params.append(("approverProjectId", input_["approver_project_id"]))
    if "sort_by" in input_:
        params.append(
            ("sortBy", capo_datazone.types.sort_key.serialize_json(input_["sort_by"]))
        )
    if "sort_order" in input_:
        params.append(
            (
                "sortOrder",
                capo_datazone.types.sort_order.serialize_json(input_["sort_order"]),
            )
        )
