"""Generated from Smithy shape ``com.amazonaws.datazone#CreateRule``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_rule_output.CreateRuleOutput = (
        capo_datazone.types.create_rule_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/rules"
    url = url.replace("{domainIdentifier}", quote(input_["domain_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.create_rule_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def create_rule(
    ...
    input_: capo_datazone.types.create_rule_input.CreateRuleInput,
) -> tuple[capo_datazone.types.create_rule_output.CreateRuleOutput, zapros.Response]:
