...
def handle_response(
    ...
    out: capo_datazone.types.associate_governed_terms_output.AssociateGovernedTermsOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{entityType}",
        quote(
            capo_datazone.types.governed_entity_type.serialize_json(
                input_["entity_type"]
            ),
            safe="",
        ),
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.associate_governed_terms_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
