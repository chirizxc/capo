...
def handle_response(
    ...
    out: capo_datazone.types.update_group_profile_output.UpdateGroupProfileOutput = (
        capo_datazone.types.update_group_profile_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{groupIdentifier}", quote(input_["group_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.update_group_profile_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_group_profile(
