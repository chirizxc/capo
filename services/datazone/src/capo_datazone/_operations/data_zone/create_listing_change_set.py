"""Generated from Smithy shape ``com.amazonaws.datazone#CreateListingChangeSet``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_listing_change_set_output.CreateListingChangeSetOutput = capo_datazone.types.create_listing_change_set_output.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/listings/change-set"
    )
    url = url.replace("{domainIdentifier}", quote(input_["domain_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.create_listing_change_set_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_listing_change_set(
