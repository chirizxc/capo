"""Generated from Smithy shape ``com.amazonaws.datazone#CreateAssetFilter``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_asset_filter_output.CreateAssetFilterOutput = (
        capo_datazone.types.create_asset_filter_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{assetIdentifier}", quote(input_["asset_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.create_asset_filter_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_asset_filter(
