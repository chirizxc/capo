"""Generated from Smithy shape ``com.amazonaws.datazone#GetAsset``."""
...
def handle_response(
    ...
    out: capo_datazone.types.get_asset_output.GetAssetOutput = (
        capo_datazone.types.get_asset_output.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/assets/{identifier}"
    )
    ...
    if "revision" in input_:
        params.append(("revision", input_["revision"]))
...
def get_asset(
    options: OperationOptions, input_: capo_datazone.types.get_asset_input.GetAssetInput
) -> tuple[capo_datazone.types.get_asset_output.GetAssetOutput, zapros.Response]:
