...
class SearchGroupProfilesOutput(TypedDict, closed=True):
    items: NotRequired[
        "capo_datazone.types.group_profile_summaries.GroupProfileSummaries"
    ]
...
def serialize_json(value: SearchGroupProfilesOutput) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_datazone.types.group_profile_summaries.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> SearchGroupProfilesOutput:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = capo_datazone.types.group_profile_summaries.deserialize_json(
            data["items"]
        )
