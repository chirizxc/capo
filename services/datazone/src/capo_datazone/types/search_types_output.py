...
class SearchTypesOutput(TypedDict, closed=True):
    items: NotRequired[
        "capo_datazone.types.search_types_result_items.SearchTypesResultItems"
    ]
...
def serialize_json(value: SearchTypesOutput) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_datazone.types.search_types_result_items.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> SearchTypesOutput:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = capo_datazone.types.search_types_result_items.deserialize_json(
            data["items"]
        )
