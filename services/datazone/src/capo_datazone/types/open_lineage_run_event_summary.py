"""Generated from Smithy shape ``com.amazonaws.datazone#OpenLineageRunEventSummary``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_datazone.types.name_identifier
    import capo_datazone.types.name_identifiers
    import capo_datazone.types.open_lineage_run_state


class OpenLineageRunEventSummary(TypedDict, closed=True):
    event_type: NotRequired[
        "capo_datazone.types.open_lineage_run_state.OpenLineageRunState"
    ]
    """<p>The event type of the open lineage run event summary.</p>"""
    run_id: NotRequired["str"]
    """<p>The runID of the open lineage run event summary.</p>"""
    job: NotRequired["capo_datazone.types.name_identifier.NameIdentifier"]
    """<p>The job of the open lineage run event summary.</p>"""
    inputs: NotRequired["capo_datazone.types.name_identifiers.NameIdentifiers"]
    """<p>The inputs of the open lineage run event summary.</p>"""
    outputs: NotRequired["capo_datazone.types.name_identifiers.NameIdentifiers"]
    """<p>The outputs of the open lineage run event summary.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: OpenLineageRunEventSummary) -> dict:
    out: dict = {}
    if "event_type" in value:
        import capo_datazone.types.open_lineage_run_state

        out["eventType"] = capo_datazone.types.open_lineage_run_state.serialize_json(
            value["event_type"]
        )
    if "run_id" in value:
        out["runId"] = value["run_id"]
    if "job" in value:
        import capo_datazone.types.name_identifier

        out["job"] = capo_datazone.types.name_identifier.serialize_json(value["job"])
    if "inputs" in value:
        import capo_datazone.types.name_identifiers

        out["inputs"] = capo_datazone.types.name_identifiers.serialize_json(
            value["inputs"]
        )
    if "outputs" in value:
        import capo_datazone.types.name_identifiers

        out["outputs"] = capo_datazone.types.name_identifiers.serialize_json(
            value["outputs"]
        )
    return out


def deserialize_json(data: dict) -> OpenLineageRunEventSummary:
    out: OpenLineageRunEventSummary = {}  # type: ignore[typeddict-item]
    if data.get("eventType") is not None:
        import capo_datazone.types.open_lineage_run_state

        out["event_type"] = capo_datazone.types.open_lineage_run_state.deserialize_json(
            data["eventType"]
        )
    if data.get("runId") is not None:
        out["run_id"] = data["runId"]
    if data.get("job") is not None:
        import capo_datazone.types.name_identifier

        out["job"] = capo_datazone.types.name_identifier.deserialize_json(data["job"])
    if data.get("inputs") is not None:
        import capo_datazone.types.name_identifiers

        out["inputs"] = capo_datazone.types.name_identifiers.deserialize_json(
            data["inputs"]
        )
    if data.get("outputs") is not None:
        import capo_datazone.types.name_identifiers

        out["outputs"] = capo_datazone.types.name_identifiers.deserialize_json(
            data["outputs"]
        )
    return out
