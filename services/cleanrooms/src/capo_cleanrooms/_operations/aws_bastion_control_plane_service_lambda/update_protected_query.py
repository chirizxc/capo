...
def handle_response(
    ...
    out: capo_cleanrooms.types.update_protected_query_output.UpdateProtectedQueryOutput = capo_cleanrooms.types.update_protected_query_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(input_["protected_query_identifier"], safe=""),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_cleanrooms.types.update_protected_query_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
