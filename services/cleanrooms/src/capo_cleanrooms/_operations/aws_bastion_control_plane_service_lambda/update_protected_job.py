"""Generated from Smithy shape ``com.amazonaws.cleanrooms#UpdateProtectedJob``."""
...
def handle_response(
    ...
    out: capo_cleanrooms.types.update_protected_job_output.UpdateProtectedJobOutput = (
        capo_cleanrooms.types.update_protected_job_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{protectedJobIdentifier}", quote(input_["protected_job_identifier"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_cleanrooms.types.update_protected_job_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_protected_job(
