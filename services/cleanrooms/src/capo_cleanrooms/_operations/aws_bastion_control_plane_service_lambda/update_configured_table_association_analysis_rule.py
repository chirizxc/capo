"""Generated from Smithy shape ``com.amazonaws.cleanrooms#UpdateConfiguredTableAssociationAnalysisRule``."""
...
import capo_cleanrooms._protocol.eventstream
...
def handle_response(
    ...
    out: capo_cleanrooms.types.update_configured_table_association_analysis_rule_output.UpdateConfiguredTableAssociationAnalysisRuleOutput = capo_cleanrooms.types.update_configured_table_association_analysis_rule_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_cleanrooms._auth._sigv4.build_sigv4_auth_scheme(
                "cleanrooms", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace(
        "{membershipIdentifier}", quote(input_["membership_identifier"], safe="")
    )
    url = url.replace(
        "{configuredTableAssociationIdentifier}",
        quote(input_["configured_table_association_identifier"], safe=""),
    )
    url = url.replace(
        "{analysisRuleType}",
        quote(
            capo_cleanrooms.types.configured_table_association_analysis_rule_type.serialize_json(
                input_["analysis_rule_type"]
            ),
            safe="",
        ),
    )
    ...
    body: bytes | None = json.dumps(
        capo_cleanrooms.types.update_configured_table_association_analysis_rule_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_configured_table_association_analysis_rule(
