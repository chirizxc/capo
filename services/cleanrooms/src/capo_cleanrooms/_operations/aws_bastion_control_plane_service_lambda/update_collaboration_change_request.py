...
def handle_response(
    ...
    out: capo_cleanrooms.types.update_collaboration_change_request_output.UpdateCollaborationChangeRequestOutput = capo_cleanrooms.types.update_collaboration_change_request_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{changeRequestIdentifier}", quote(input_["change_request_identifier"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_cleanrooms.types.update_collaboration_change_request_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
