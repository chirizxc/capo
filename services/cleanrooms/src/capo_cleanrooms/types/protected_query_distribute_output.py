...
class ProtectedQueryDistributeOutput(TypedDict, closed=True):
    s3: NotRequired[
        "capo_cleanrooms.types.protected_query_s3_output.ProtectedQueryS3Output"
    ]
    member_list: NotRequired[
        "capo_cleanrooms.types.protected_query_member_output_list.ProtectedQueryMemberOutputList"
    ]
...
def serialize_json(value: ProtectedQueryDistributeOutput) -> dict:
    ...
    if "s3" in value:
        ...
        out["s3"] = capo_cleanrooms.types.protected_query_s3_output.serialize_json(
            value["s3"]
        )
    if "member_list" in value:
        ...
        out["memberList"] = (
            capo_cleanrooms.types.protected_query_member_output_list.serialize_json(
                value["member_list"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ProtectedQueryDistributeOutput:
    ...
    if data.get("memberList") is not None:
        ...
        out["member_list"] = (
            capo_cleanrooms.types.protected_query_member_output_list.deserialize_json(
                data["memberList"]
            )
        )
    return out
