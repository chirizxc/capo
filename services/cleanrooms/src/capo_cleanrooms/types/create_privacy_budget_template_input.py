...
def serialize_json(value: CreatePrivacyBudgetTemplateInput) -> dict:
    ...
    out["parameters"] = (
        capo_cleanrooms.types.privacy_budget_template_parameters_input.serialize_json(
            value["parameters"]
        )
    )
    if "tags" in value:
...
def deserialize_json(data: dict) -> CreatePrivacyBudgetTemplateInput:
    ...
    if data.get("parameters") is not None:
        ...
        out["parameters"] = (
            capo_cleanrooms.types.privacy_budget_template_parameters_input.deserialize_json(
                data["parameters"]
            )
        )
    else:
