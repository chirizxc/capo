"""Generated from Smithy shape ``com.amazonaws.marketplacecatalog#ListEntities``."""
...
def handle_response(
    ...
    out: capo_marketplace_catalog.types.list_entities_response.ListEntitiesResponse = (
        capo_marketplace_catalog.types.list_entities_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListEntities"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_marketplace_catalog.types.list_entities_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
