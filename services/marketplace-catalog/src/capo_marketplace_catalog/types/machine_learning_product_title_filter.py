...
class MachineLearningProductTitleFilter(TypedDict, closed=True):
    value_list: NotRequired[
        "capo_marketplace_catalog.types.machine_learning_product_title_filter_value_list.MachineLearningProductTitleFilterValueList"
    ]
...
def serialize_json(value: MachineLearningProductTitleFilter) -> dict:
    ...
    if "value_list" in value:
        ...
        out["ValueList"] = (
            capo_marketplace_catalog.types.machine_learning_product_title_filter_value_list.serialize_json(
                value["value_list"]
            )
        )
...
def deserialize_json(data: dict) -> MachineLearningProductTitleFilter:
    ...
    if data.get("ValueList") is not None:
        ...
        out["value_list"] = (
            capo_marketplace_catalog.types.machine_learning_product_title_filter_value_list.deserialize_json(
                data["ValueList"]
            )
        )
