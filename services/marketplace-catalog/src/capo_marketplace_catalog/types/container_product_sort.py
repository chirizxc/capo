...
class ContainerProductSort(TypedDict, closed=True):
    sort_by: NotRequired[
        "capo_marketplace_catalog.types.container_product_sort_by.ContainerProductSortBy"
    ]
...
def serialize_json(value: ContainerProductSort) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["SortBy"] = (
            capo_marketplace_catalog.types.container_product_sort_by.serialize_json(
                value["sort_by"]
            )
        )
...
def deserialize_json(data: dict) -> ContainerProductSort:
    ...
    if data.get("SortBy") is not None:
        ...
        out["sort_by"] = (
            capo_marketplace_catalog.types.container_product_sort_by.deserialize_json(
                data["SortBy"]
            )
        )
