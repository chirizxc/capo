...
if TYPE_CHECKING:
    import capo_marketplace_catalog.types.offer_resale_authorization_id_filter_value_list


class OfferResaleAuthorizationIdFilter(TypedDict, closed=True):
    value_list: NotRequired[
        "capo_marketplace_catalog.types.offer_resale_authorization_id_filter_value_list.OfferResaleAuthorizationIdFilterValueList"
    ]
...
def serialize_json(value: OfferResaleAuthorizationIdFilter) -> dict:
    ...
    if "value_list" in value:
        ...
        out["ValueList"] = (
            capo_marketplace_catalog.types.offer_resale_authorization_id_filter_value_list.serialize_json(
                value["value_list"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> OfferResaleAuthorizationIdFilter:
    ...
    if data.get("ValueList") is not None:
        ...
        out["value_list"] = (
            capo_marketplace_catalog.types.offer_resale_authorization_id_filter_value_list.deserialize_json(
                data["ValueList"]
            )
        )
    return out
