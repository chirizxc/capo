...
if TYPE_CHECKING:
    import capo_marketplace_catalog.types.offer_set_solution_id_filter_value_list


class OfferSetSolutionIdFilter(TypedDict, closed=True):
    value_list: NotRequired[
        "capo_marketplace_catalog.types.offer_set_solution_id_filter_value_list.OfferSetSolutionIdFilterValueList"
    ]
...
def serialize_json(value: OfferSetSolutionIdFilter) -> dict:
    ...
    if "value_list" in value:
        ...
        out["ValueList"] = (
            capo_marketplace_catalog.types.offer_set_solution_id_filter_value_list.serialize_json(
                value["value_list"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> OfferSetSolutionIdFilter:
    ...
    if data.get("ValueList") is not None:
        ...
        out["value_list"] = (
            capo_marketplace_catalog.types.offer_set_solution_id_filter_value_list.deserialize_json(
                data["ValueList"]
            )
        )
    return out
