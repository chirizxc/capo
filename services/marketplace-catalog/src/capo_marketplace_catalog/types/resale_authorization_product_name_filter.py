...
if TYPE_CHECKING:
    import capo_marketplace_catalog.types.resale_authorization_product_name_filter_value_list
    import capo_marketplace_catalog.types.resale_authorization_product_name_filter_wildcard


class ResaleAuthorizationProductNameFilter(TypedDict, closed=True):
    value_list: NotRequired[
        "capo_marketplace_catalog.types.resale_authorization_product_name_filter_value_list.ResaleAuthorizationProductNameFilterValueList"
    ]
    """<p>Allows filtering on the <code>ProductName</code> of a ResaleAuthorization with list input.</p>"""
    wild_card_value: NotRequired[
        "capo_marketplace_catalog.types.resale_authorization_product_name_filter_wildcard.ResaleAuthorizationProductNameFilterWildcard"
    ]
...
def serialize_json(value: ResaleAuthorizationProductNameFilter) -> dict:
    ...
    if "value_list" in value:
        ...
        out["ValueList"] = (
            capo_marketplace_catalog.types.resale_authorization_product_name_filter_value_list.serialize_json(
                value["value_list"]
            )
        )
...
def deserialize_json(data: dict) -> ResaleAuthorizationProductNameFilter:
    ...
    if data.get("ValueList") is not None:
        ...
        out["value_list"] = (
            capo_marketplace_catalog.types.resale_authorization_product_name_filter_value_list.deserialize_json(
                data["ValueList"]
            )
        )
