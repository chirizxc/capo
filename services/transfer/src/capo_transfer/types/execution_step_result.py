...
if TYPE_CHECKING:
    ...
    import capo_transfer.types.step_result_outputs_json
    import capo_transfer.types.workflow_step_type


class ExecutionStepResult(TypedDict, closed=True):
    step_type: NotRequired["capo_transfer.types.workflow_step_type.WorkflowStepType"]
    ...
    outputs: NotRequired[
        "capo_transfer.types.step_result_outputs_json.StepResultOutputsJson"
    ]
    """<p>The values for the key/value pair applied as a tag to the file. Only applicable if the step type is <code>TAG</code>.</p>"""
    error: NotRequired["capo_transfer.types.execution_error.ExecutionError"]
...
def serialize_aws_json_1_1(value: ExecutionStepResult) -> dict:
    ...
    if "step_type" in value:
        ...
        out["StepType"] = capo_transfer.types.workflow_step_type.serialize_aws_json_1_1(
            value["step_type"]
        )
    if "outputs" in value:
    ...
    if "error" in value:
        ...
        out["Error"] = capo_transfer.types.execution_error.serialize_aws_json_1_1(
            value["error"]
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> ExecutionStepResult:
    ...
    if data.get("StepType") is not None:
        ...
        out["step_type"] = (
            capo_transfer.types.workflow_step_type.deserialize_aws_json_1_1(
                data["StepType"]
            )
        )
