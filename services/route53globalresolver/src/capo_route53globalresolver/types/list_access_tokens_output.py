...
class ListAccessTokensOutput(TypedDict, closed=True):
    ...
    access_tokens: NotRequired[
        "capo_route53globalresolver.types.access_tokens.AccessTokens"
    ]
...
def serialize_json(value: ListAccessTokensOutput) -> dict:
    ...
    if "access_tokens" in value:
        ...
        out["accessTokens"] = (
            capo_route53globalresolver.types.access_tokens.serialize_json(
                value["access_tokens"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListAccessTokensOutput:
    ...
    if data.get("accessTokens") is not None:
        import capo_route53globalresolver.types.access_tokens

        out["access_tokens"] = (
            capo_route53globalresolver.types.access_tokens.deserialize_json(
                data["accessTokens"]
            )
        )
    return out
