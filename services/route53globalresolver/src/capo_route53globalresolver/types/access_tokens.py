...
AccessTokens: TypeAlias = list[
    "capo_route53globalresolver.types.access_token_item.AccessTokenItem"
]
...
def serialize_json(value: AccessTokens) -> list:
    ...
    for item in value:
        out.append(
            capo_route53globalresolver.types.access_token_item.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AccessTokens:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_route53globalresolver.types.access_token_item.deserialize_json(item)
        )
    return out
