"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#ListTagsForResource``."""
...
def handle_response(
    ...
    out: capo_route53globalresolver.types.list_tags_for_resource_response.ListTagsForResourceResponse = capo_route53globalresolver.types.list_tags_for_resource_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/get-all-tags"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_route53globalresolver.types.list_tags_for_resource_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
