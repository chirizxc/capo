...
def handle_response(
    ...
    out: capo_route53globalresolver.types.update_hosted_zone_association_output.UpdateHostedZoneAssociationOutput = capo_route53globalresolver.types.update_hosted_zone_association_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(input_["hosted_zone_association_id"], safe=""),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_route53globalresolver.types.update_hosted_zone_association_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
