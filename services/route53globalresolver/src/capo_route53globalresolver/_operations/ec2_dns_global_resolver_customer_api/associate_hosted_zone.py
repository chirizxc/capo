"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#AssociateHostedZone``."""
...
def handle_response(
    ...
    out: capo_route53globalresolver.types.associate_hosted_zone_output.AssociateHostedZoneOutput = capo_route53globalresolver.types.associate_hosted_zone_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/hosted-zone-associations/{hostedZoneId}"
    url = url.replace("{hostedZoneId}", quote(input_["hosted_zone_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_route53globalresolver.types.associate_hosted_zone_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_associate_hosted_zone(
