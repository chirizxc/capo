"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#CreateAccessToken``."""
...
def handle_response(
    ...
    out: capo_route53globalresolver.types.create_access_token_output.CreateAccessTokenOutput = capo_route53globalresolver.types.create_access_token_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/tokens/{dnsViewId}"
    url = url.replace("{dnsViewId}", quote(input_["dns_view_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_route53globalresolver.types.create_access_token_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
