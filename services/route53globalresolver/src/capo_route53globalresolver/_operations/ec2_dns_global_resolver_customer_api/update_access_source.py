"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#UpdateAccessSource``."""
...
def handle_response(
    ...
    out: capo_route53globalresolver.types.update_access_source_output.UpdateAccessSourceOutput = capo_route53globalresolver.types.update_access_source_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{accessSourceId}", quote(input_["access_source_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_route53globalresolver.types.update_access_source_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_access_source(
