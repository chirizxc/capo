"""Generated from Smithy shape ``com.amazonaws.groundstation#UpdateAgentStatus``."""
...
def handle_response(
    ...
    out: capo_groundstation.types.update_agent_status_response.UpdateAgentStatusResponse = capo_groundstation.types.update_agent_status_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/agent/{agentId}"
    url = url.replace("{agentId}", quote(input_["agent_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_groundstation.types.update_agent_status_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
