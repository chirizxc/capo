...
import capo_groundstation._protocol.eventstream
...
def handle_response(
    ...
    out: capo_groundstation.types.list_ground_station_reservations_response.ListGroundStationReservationsResponse = capo_groundstation.types.list_ground_station_reservations_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_groundstation._auth._sigv4.build_sigv4_auth_scheme(
                "groundstation", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace("{groundStationId}", quote(input_["ground_station_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "start_time" in input_:
        params.append(
            (
                "startTime",
                capo_groundstation._protocol.serialize.fmt_epoch_seconds(
                    input_["start_time"]
                ),
            )
        )
    ...
    if "reservation_types" in input_:
        for item in input_["reservation_types"]:
            params.append(
                (
                    "reservationTypes",
                    capo_groundstation.types.reservation_type.serialize_json(item),
                )
            )
