"""Generated from Smithy shape ``com.amazonaws.groundstation#ListContacts``."""
...
def handle_response(
    ...
    out: capo_groundstation.types.list_contacts_response.ListContactsResponse = (
        capo_groundstation.types.list_contacts_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contacts"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_groundstation.types.list_contacts_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
