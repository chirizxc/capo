...
if TYPE_CHECKING:
    import capo_ses.types.cloud_watch_dimension_configuration

CloudWatchDimensionConfigurations: TypeAlias = list[
    "capo_ses.types.cloud_watch_dimension_configuration.CloudWatchDimensionConfiguration"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_ses.types.cloud_watch_dimension_configuration.serialize_query(
...
def deserialize_query(el: Element) -> CloudWatchDimensionConfigurations:
    ...
    for child in el.findall("member"):
        out.append(
            capo_ses.types.cloud_watch_dimension_configuration.deserialize_query(child)
        )
    return out
