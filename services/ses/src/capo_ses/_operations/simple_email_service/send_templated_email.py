"""Generated from Smithy shape ``com.amazonaws.ses#SendTemplatedEmail``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("SendTemplatedEmailResult")
    out: capo_ses.types.send_templated_email_response.SendTemplatedEmailResponse = (
        capo_ses.types.send_templated_email_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "SendTemplatedEmail"))
    pairs.append(("Version", "2010-12-01"))
    capo_ses.types.send_templated_email_request.serialize_query(input_, pairs, "")
...
async def async_send_templated_email(
