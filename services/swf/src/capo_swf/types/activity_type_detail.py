...
class ActivityTypeDetail(TypedDict, closed=True):
    type_info: "capo_swf.types.activity_type_info.ActivityTypeInfo"
...
def serialize_aws_json_1_0(value: ActivityTypeDetail) -> dict:
    ...
    out["typeInfo"] = capo_swf.types.activity_type_info.serialize_aws_json_1_0(
        value["type_info"]
    )
    import capo_swf.types.activity_type_configuration

    out["configuration"] = (
        capo_swf.types.activity_type_configuration.serialize_aws_json_1_0(
            value["configuration"]
        )
    )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> ActivityTypeDetail:
    ...
    if data.get("typeInfo") is not None:
        ...
        out["type_info"] = capo_swf.types.activity_type_info.deserialize_aws_json_1_0(
            data["typeInfo"]
        )
    else:
    ...
    if data.get("configuration") is not None:
        ...
        out["configuration"] = (
            capo_swf.types.activity_type_configuration.deserialize_aws_json_1_0(
                data["configuration"]
            )
        )
    else:
