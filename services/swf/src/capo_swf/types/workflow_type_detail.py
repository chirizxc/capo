...
class WorkflowTypeDetail(TypedDict, closed=True):
    type_info: "capo_swf.types.workflow_type_info.WorkflowTypeInfo"
    ...
    configuration: (
        "capo_swf.types.workflow_type_configuration.WorkflowTypeConfiguration"
    )
...
def serialize_aws_json_1_0(value: WorkflowTypeDetail) -> dict:
    ...
    out["typeInfo"] = capo_swf.types.workflow_type_info.serialize_aws_json_1_0(
        value["type_info"]
    )
    import capo_swf.types.workflow_type_configuration

    out["configuration"] = (
        capo_swf.types.workflow_type_configuration.serialize_aws_json_1_0(
            value["configuration"]
        )
    )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> WorkflowTypeDetail:
    ...
    if data.get("configuration") is not None:
        ...
        out["configuration"] = (
            capo_swf.types.workflow_type_configuration.deserialize_aws_json_1_0(
                data["configuration"]
            )
        )
    else:
