...
if TYPE_CHECKING:
    import capo_swf.types.domain_configuration
    import capo_swf.types.domain_info


class DomainDetail(TypedDict, closed=True):
    domain_info: "capo_swf.types.domain_info.DomainInfo"
    """<p>The basic information about a domain, such as its name, status, and description.</p>"""
    configuration: "capo_swf.types.domain_configuration.DomainConfiguration"
...
def serialize_aws_json_1_0(value: DomainDetail) -> dict:
    ...
    out["domainInfo"] = capo_swf.types.domain_info.serialize_aws_json_1_0(
        value["domain_info"]
    )
    import capo_swf.types.domain_configuration

    out["configuration"] = capo_swf.types.domain_configuration.serialize_aws_json_1_0(
        value["configuration"]
    )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> DomainDetail:
    ...
    if data.get("domainInfo") is not None:
        import capo_swf.types.domain_info

        out["domain_info"] = capo_swf.types.domain_info.deserialize_aws_json_1_0(
            data["domainInfo"]
        )
    else:
    ...
    if data.get("configuration") is not None:
        ...
        out["configuration"] = (
            capo_swf.types.domain_configuration.deserialize_aws_json_1_0(
                data["configuration"]
            )
        )
    else:
