"""Generated from Smithy shape ``com.amazonaws.licensemanagerusersubscriptions#DeregisterIdentityProvider``."""
...
def handle_response(
    ...
    out: capo_license_manager_user_subscriptions.types.deregister_identity_provider_response.DeregisterIdentityProviderResponse = capo_license_manager_user_subscriptions.types.deregister_identity_provider_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identity-provider/DeregisterIdentityProvider"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_license_manager_user_subscriptions.types.deregister_identity_provider_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_deregister_identity_provider(
