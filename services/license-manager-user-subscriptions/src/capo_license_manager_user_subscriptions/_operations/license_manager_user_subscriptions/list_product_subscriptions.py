"""Generated from Smithy shape ``com.amazonaws.licensemanagerusersubscriptions#ListProductSubscriptions``."""
...
def handle_response(
    ...
    out: capo_license_manager_user_subscriptions.types.list_product_subscriptions_response.ListProductSubscriptionsResponse = capo_license_manager_user_subscriptions.types.list_product_subscriptions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/user/ListProductSubscriptions"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_license_manager_user_subscriptions.types.list_product_subscriptions_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_product_subscriptions(
