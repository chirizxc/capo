"""Generated from Smithy shape ``com.amazonaws.licensemanagerusersubscriptions#ListLicenseServerEndpoints``."""
...
import capo_license_manager_user_subscriptions._protocol.eventstream
...
import capo_license_manager_user_subscriptions.types.license_server_endpoint_list
...
def handle_response(
    ...
    out: capo_license_manager_user_subscriptions.types.list_license_server_endpoints_response.ListLicenseServerEndpointsResponse = capo_license_manager_user_subscriptions.types.list_license_server_endpoints_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_license_manager_user_subscriptions._auth._sigv4.build_sigv4_auth_scheme(
                "license-manager-user-subscriptions", options.region, endpoint_scheme
            )
            if sigv4_config is not None:
                return (
                    capo_license_manager_user_subscriptions._auth._signers.SigV4Signer(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/license-server/ListLicenseServerEndpoints"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_license_manager_user_subscriptions.types.list_license_server_endpoints_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_license_server_endpoints(
