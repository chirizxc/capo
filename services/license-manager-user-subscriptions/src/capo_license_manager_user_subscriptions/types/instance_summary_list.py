...
InstanceSummaryList: TypeAlias = list[
    "capo_license_manager_user_subscriptions.types.instance_summary.InstanceSummary"
]
...
def serialize_json(value: InstanceSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_license_manager_user_subscriptions.types.instance_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> InstanceSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_license_manager_user_subscriptions.types.instance_summary.deserialize_json(
                item
            )
        )
    return out
