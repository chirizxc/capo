...
class UpdateIdentityProviderSettingsRequest(TypedDict, closed=True):
    ...
    update_settings: (
        "capo_license_manager_user_subscriptions.types.update_settings.UpdateSettings"
    )
...
def serialize_json(value: UpdateIdentityProviderSettingsRequest) -> dict:
    ...
    out["UpdateSettings"] = (
        capo_license_manager_user_subscriptions.types.update_settings.serialize_json(
...
def deserialize_json(data: dict) -> UpdateIdentityProviderSettingsRequest:
    ...
    if data.get("UpdateSettings") is not None:
        ...
        out["update_settings"] = (
            capo_license_manager_user_subscriptions.types.update_settings.deserialize_json(
                data["UpdateSettings"]
            )
        )
    else:
