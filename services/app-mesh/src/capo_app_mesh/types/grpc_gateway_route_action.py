...
class GrpcGatewayRouteAction(TypedDict, closed=True):
    ...
    rewrite: NotRequired[
        "capo_app_mesh.types.grpc_gateway_route_rewrite.GrpcGatewayRouteRewrite"
    ]
...
def serialize_json(value: GrpcGatewayRouteAction) -> dict:
    ...
    if "rewrite" in value:
        ...
        out["rewrite"] = capo_app_mesh.types.grpc_gateway_route_rewrite.serialize_json(
            value["rewrite"]
        )
    return out
...
def deserialize_json(data: dict) -> GrpcGatewayRouteAction:
    ...
    if data.get("rewrite") is not None:
        ...
        out["rewrite"] = (
            capo_app_mesh.types.grpc_gateway_route_rewrite.deserialize_json(
                data["rewrite"]
            )
        )
    return out
