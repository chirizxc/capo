...
if TYPE_CHECKING:
    ...
    import capo_app_mesh.types.method_name
...
class GrpcRouteMatch(TypedDict, closed=True):
    ...
    method_name: NotRequired["capo_app_mesh.types.method_name.MethodName"]
    """<p>The method name to match from the request. If you specify a name, you must also specify a <code>serviceName</code>.</p>"""
    metadata: NotRequired[
        "capo_app_mesh.types.grpc_route_metadata_list.GrpcRouteMetadataList"
    ]
...
def serialize_json(value: GrpcRouteMatch) -> dict:
    ...
    if "metadata" in value:
        ...
        out["metadata"] = capo_app_mesh.types.grpc_route_metadata_list.serialize_json(
            value["metadata"]
        )
    if "port" in value:
        out["port"] = value["port"]
