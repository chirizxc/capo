...
class VirtualNodeSpec(TypedDict, closed=True):
    service_discovery: NotRequired[
        "capo_app_mesh.types.service_discovery.ServiceDiscovery"
    ]
...
def serialize_json(value: VirtualNodeSpec) -> dict:
    ...
    if "service_discovery" in value:
        ...
        out["serviceDiscovery"] = capo_app_mesh.types.service_discovery.serialize_json(
            value["service_discovery"]
        )
    if "listeners" in value:
        import capo_app_mesh.types.listeners

        out["listeners"] = capo_app_mesh.types.listeners.serialize_json(
            value["listeners"]
        )
    if "backends" in value:
        import capo_app_mesh.types.backends

        out["backends"] = capo_app_mesh.types.backends.serialize_json(value["backends"])
    if "backend_defaults" in value:
        import capo_app_mesh.types.backend_defaults

        out["backendDefaults"] = capo_app_mesh.types.backend_defaults.serialize_json(
            value["backend_defaults"]
        )
    if "logging" in value:
        import capo_app_mesh.types.logging

        out["logging"] = capo_app_mesh.types.logging.serialize_json(value["logging"])
    return out
...
def deserialize_json(data: dict) -> VirtualNodeSpec:
    ...
    if data.get("serviceDiscovery") is not None:
        ...
        out["service_discovery"] = (
            capo_app_mesh.types.service_discovery.deserialize_json(
