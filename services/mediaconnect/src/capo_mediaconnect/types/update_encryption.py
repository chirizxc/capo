"""Generated from Smithy shape ``com.amazonaws.mediaconnect#UpdateEncryption``."""
...
class UpdateEncryption(TypedDict, closed=True):
    algorithm: NotRequired["capo_mediaconnect.types.algorithm.Algorithm"]
    ...
    key_type: NotRequired["capo_mediaconnect.types.key_type.KeyType"]
...
# --- restJson1 ser/de ---
def serialize_json(value: UpdateEncryption) -> dict:
    out: dict = {}
    if "algorithm" in value:
        import capo_mediaconnect.types.algorithm

        out["algorithm"] = capo_mediaconnect.types.algorithm.serialize_json(
            value["algorithm"]
        )
    ...
    if "key_type" in value:
        ...
        out["keyType"] = capo_mediaconnect.types.key_type.serialize_json(
            value["key_type"]
        )
    if "region" in value:
