...
class NdiMediaInfo(TypedDict, closed=True):
    streams: NotRequired[
        "capo_mediaconnect.types.__list_of_ndi_media_stream_info.__listOfNdiMediaStreamInfo"
    ]
...
def serialize_json(value: NdiMediaInfo) -> dict:
    ...
    if "streams" in value:
        ...
        out["streams"] = (
            capo_mediaconnect.types.__list_of_ndi_media_stream_info.serialize_json(
                value["streams"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> NdiMediaInfo:
    ...
    if data.get("streams") is not None:
        ...
        out["streams"] = (
            capo_mediaconnect.types.__list_of_ndi_media_stream_info.deserialize_json(
                data["streams"]
            )
        )
    return out
