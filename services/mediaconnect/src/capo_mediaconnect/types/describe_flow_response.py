...
class DescribeFlowResponse(TypedDict, closed=True):
    ...
    messages: NotRequired["capo_mediaconnect.types.messages.Messages"]
...
def serialize_json(value: DescribeFlowResponse) -> dict:
    ...
    if "messages" in value:
        ...
        out["messages"] = capo_mediaconnect.types.messages.serialize_json(
            value["messages"]
        )
    return out
...
def deserialize_json(data: dict) -> DescribeFlowResponse:
    ...
    if data.get("messages") is not None:
        ...
        out["messages"] = capo_mediaconnect.types.messages.deserialize_json(
            data["messages"]
        )
    return out
