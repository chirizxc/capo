"""Generated from Smithy shape ``com.amazonaws.mediaconnect#PurchaseOffering``."""
...
def handle_response(
    ...
    out: capo_mediaconnect.types.purchase_offering_response.PurchaseOfferingResponse = (
        capo_mediaconnect.types.purchase_offering_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{OfferingArn}", quote(input_["offering_arn"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediaconnect.types.purchase_offering_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
