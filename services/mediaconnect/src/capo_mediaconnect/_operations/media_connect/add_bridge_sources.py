"""Generated from Smithy shape ``com.amazonaws.mediaconnect#AddBridgeSources``."""
...
def handle_response(
    ...
    out: capo_mediaconnect.types.add_bridge_sources_response.AddBridgeSourcesResponse = capo_mediaconnect.types.add_bridge_sources_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/bridges/{BridgeArn}/sources"
    url = url.replace("{BridgeArn}", quote(input_["bridge_arn"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediaconnect.types.add_bridge_sources_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_add_bridge_sources(
