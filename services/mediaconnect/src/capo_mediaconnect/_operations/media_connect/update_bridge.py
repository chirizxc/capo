"""Generated from Smithy shape ``com.amazonaws.mediaconnect#UpdateBridge``."""
...
def handle_response(
    ...
    out: capo_mediaconnect.types.update_bridge_response.UpdateBridgeResponse = (
        capo_mediaconnect.types.update_bridge_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{BridgeArn}", quote(input_["bridge_arn"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediaconnect.types.update_bridge_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_bridge(
    ...
    capo_mediaconnect.types.update_bridge_response.UpdateBridgeResponse, zapros.Response
]:
