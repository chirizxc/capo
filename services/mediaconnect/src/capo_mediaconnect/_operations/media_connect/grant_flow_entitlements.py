"""Generated from Smithy shape ``com.amazonaws.mediaconnect#GrantFlowEntitlements``."""
...
import capo_mediaconnect.errors.grant_flow_entitlements420_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "GrantFlowEntitlements420Exception":
            raise capo_mediaconnect.errors.grant_flow_entitlements420_exception.GrantFlowEntitlements420Exception.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_mediaconnect.types.grant_flow_entitlements_response.GrantFlowEntitlementsResponse = capo_mediaconnect.types.grant_flow_entitlements_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/flows/{FlowArn}/entitlements"
    url = url.replace("{FlowArn}", quote(input_["flow_arn"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediaconnect.types.grant_flow_entitlements_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_grant_flow_entitlements(
