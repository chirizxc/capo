...
def handle_response(
    ...
    out: capo_mediaconnect.types.update_flow_media_stream_response.UpdateFlowMediaStreamResponse = capo_mediaconnect.types.update_flow_media_stream_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{MediaStreamName}", quote(input_["media_stream_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediaconnect.types.update_flow_media_stream_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
