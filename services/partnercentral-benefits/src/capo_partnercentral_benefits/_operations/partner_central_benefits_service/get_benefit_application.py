"""Generated from Smithy shape ``com.amazonaws.partnercentralbenefits#GetBenefitApplication``."""
...
def handle_response(
    ...
    out: capo_partnercentral_benefits.types.get_benefit_application_output.GetBenefitApplicationOutput = capo_partnercentral_benefits.types.get_benefit_application_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetBenefitApplication"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "PartnerCentralBenefitsService.GetBenefitApplication"
    body: bytes | None = json.dumps(
        capo_partnercentral_benefits.types.get_benefit_application_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_benefit_application(
