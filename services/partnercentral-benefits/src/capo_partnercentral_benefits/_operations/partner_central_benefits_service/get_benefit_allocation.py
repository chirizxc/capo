"""Generated from Smithy shape ``com.amazonaws.partnercentralbenefits#GetBenefitAllocation``."""
...
def handle_response(
    ...
    out: capo_partnercentral_benefits.types.get_benefit_allocation_output.GetBenefitAllocationOutput = capo_partnercentral_benefits.types.get_benefit_allocation_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetBenefitAllocation"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "PartnerCentralBenefitsService.GetBenefitAllocation"
    body: bytes | None = json.dumps(
        capo_partnercentral_benefits.types.get_benefit_allocation_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_benefit_allocation(
