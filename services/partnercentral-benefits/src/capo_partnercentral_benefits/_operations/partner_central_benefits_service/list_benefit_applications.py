"""Generated from Smithy shape ``com.amazonaws.partnercentralbenefits#ListBenefitApplications``."""
...
def handle_response(
    ...
    out: capo_partnercentral_benefits.types.list_benefit_applications_output.ListBenefitApplicationsOutput = capo_partnercentral_benefits.types.list_benefit_applications_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListBenefitApplications"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "PartnerCentralBenefitsService.ListBenefitApplications"
    body: bytes | None = json.dumps(
        capo_partnercentral_benefits.types.list_benefit_applications_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_benefit_applications(
