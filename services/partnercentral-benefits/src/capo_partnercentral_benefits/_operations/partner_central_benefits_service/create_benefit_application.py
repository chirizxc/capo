"""Generated from Smithy shape ``com.amazonaws.partnercentralbenefits#CreateBenefitApplication``."""
...
def handle_response(
    ...
    out: capo_partnercentral_benefits.types.create_benefit_application_output.CreateBenefitApplicationOutput = capo_partnercentral_benefits.types.create_benefit_application_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateBenefitApplication"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "PartnerCentralBenefitsService.CreateBenefitApplication"
    body: bytes | None = json.dumps(
        capo_partnercentral_benefits.types.create_benefit_application_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_benefit_application(
