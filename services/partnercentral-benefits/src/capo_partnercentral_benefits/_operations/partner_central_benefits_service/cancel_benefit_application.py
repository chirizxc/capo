"""Generated from Smithy shape ``com.amazonaws.partnercentralbenefits#CancelBenefitApplication``."""
...
def handle_response(
    ...
    out: capo_partnercentral_benefits.types.cancel_benefit_application_output.CancelBenefitApplicationOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CancelBenefitApplication"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "PartnerCentralBenefitsService.CancelBenefitApplication"
    body: bytes | None = json.dumps(
        capo_partnercentral_benefits.types.cancel_benefit_application_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_cancel_benefit_application(
