"""Generated from Smithy shape ``com.amazonaws.securityagent#BatchGetTargetDomains``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.batch_get_target_domains_output.BatchGetTargetDomainsOutput = capo_securityagent.types.batch_get_target_domains_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/BatchGetTargetDomains"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.batch_get_target_domains_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_batch_get_target_domains(
