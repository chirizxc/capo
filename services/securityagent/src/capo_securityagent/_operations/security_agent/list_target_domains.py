"""Generated from Smithy shape ``com.amazonaws.securityagent#ListTargetDomains``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.list_target_domains_output.ListTargetDomainsOutput = (
        capo_securityagent.types.list_target_domains_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListTargetDomains"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.list_target_domains_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_target_domains(
