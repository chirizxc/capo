"""Generated from Smithy shape ``com.amazonaws.securityagent#UpdateCodeReview``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.update_code_review_output.UpdateCodeReviewOutput = (
        capo_securityagent.types.update_code_review_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateCodeReview"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.update_code_review_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_code_review(
