"""Generated from Smithy shape ``com.amazonaws.securityagent#StartCodeRemediation``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.start_code_remediation_output.StartCodeRemediationOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartCodeRemediation"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.start_code_remediation_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_start_code_remediation(
