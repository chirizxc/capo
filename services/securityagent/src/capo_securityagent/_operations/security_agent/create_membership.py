"""Generated from Smithy shape ``com.amazonaws.securityagent#CreateMembership``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.create_membership_response.CreateMembershipResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateMembership"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.create_membership_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
