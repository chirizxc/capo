"""Generated from Smithy shape ``com.amazonaws.securityagent#StopCodeReviewJob``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.stop_code_review_job_output.StopCodeReviewJobOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StopCodeReviewJob"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.stop_code_review_job_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_stop_code_review_job(
