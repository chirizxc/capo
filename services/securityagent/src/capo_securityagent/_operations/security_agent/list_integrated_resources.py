"""Generated from Smithy shape ``com.amazonaws.securityagent#ListIntegratedResources``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.list_integrated_resources_output.ListIntegratedResourcesOutput = capo_securityagent.types.list_integrated_resources_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListIntegratedResources"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.list_integrated_resources_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_integrated_resources(
