"""Generated from Smithy shape ``com.amazonaws.securityagent#ListCodeReviewJobTasks``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.list_code_review_job_tasks_output.ListCodeReviewJobTasksOutput = capo_securityagent.types.list_code_review_job_tasks_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListCodeReviewJobTasks"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.list_code_review_job_tasks_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_code_review_job_tasks(
