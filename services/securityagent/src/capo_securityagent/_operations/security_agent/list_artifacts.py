"""Generated from Smithy shape ``com.amazonaws.securityagent#ListArtifacts``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.list_artifacts_output.ListArtifactsOutput = (
        capo_securityagent.types.list_artifacts_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListArtifacts"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.list_artifacts_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def list_artifacts(
    ...
    capo_securityagent.types.list_artifacts_output.ListArtifactsOutput, zapros.Response
]:
