"""Generated from Smithy shape ``com.amazonaws.securityagent#DeleteAgentSpace``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.delete_agent_space_output.DeleteAgentSpaceOutput = (
        capo_securityagent.types.delete_agent_space_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteAgentSpace"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.delete_agent_space_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
