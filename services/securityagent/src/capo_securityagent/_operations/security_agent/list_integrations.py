"""Generated from Smithy shape ``com.amazonaws.securityagent#ListIntegrations``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.list_integrations_output.ListIntegrationsOutput = (
        capo_securityagent.types.list_integrations_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListIntegrations"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.list_integrations_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
