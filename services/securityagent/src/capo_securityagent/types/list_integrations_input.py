...
class ListIntegrationsInput(TypedDict, closed=True):
    filter: NotRequired["capo_securityagent.types.integration_filter.IntegrationFilter"]
...
def serialize_json(value: ListIntegrationsInput) -> dict:
    ...
    if "filter" in value:
        ...
        out["filter"] = capo_securityagent.types.integration_filter.serialize_json(
            value["filter"]
        )
...
def deserialize_json(data: dict) -> ListIntegrationsInput:
    ...
    if data.get("filter") is not None:
        ...
        out["filter"] = capo_securityagent.types.integration_filter.deserialize_json(
            data["filter"]
        )
