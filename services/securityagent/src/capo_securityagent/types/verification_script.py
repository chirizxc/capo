...
class VerificationScript(TypedDict, closed=True):
    ...
    env_vars: NotRequired[
        "capo_securityagent.types.verification_script_env_var_list.VerificationScriptEnvVarList"
    ]
...
def serialize_json(value: VerificationScript) -> dict:
    ...
    if "env_vars" in value:
        ...
        out["envVars"] = (
            capo_securityagent.types.verification_script_env_var_list.serialize_json(
                value["env_vars"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> VerificationScript:
    ...
    if data.get("envVars") is not None:
        ...
        out["env_vars"] = (
            capo_securityagent.types.verification_script_env_var_list.deserialize_json(
                data["envVars"]
            )
        )
    return out
