...
class NetworkTrafficConfig(TypedDict, closed=True):
    rules: NotRequired[
        "capo_securityagent.types.network_traffic_rule_list.NetworkTrafficRuleList"
    ]
    ...
    custom_headers: NotRequired[
        "capo_securityagent.types.custom_header_list.CustomHeaderList"
    ]
...
def serialize_json(value: NetworkTrafficConfig) -> dict:
    ...
    if "rules" in value:
        ...
        out["rules"] = (
            capo_securityagent.types.network_traffic_rule_list.serialize_json(
                value["rules"]
            )
        )
    if "custom_headers" in value:
        ...
        out["customHeaders"] = (
            capo_securityagent.types.custom_header_list.serialize_json(
...
def deserialize_json(data: dict) -> NetworkTrafficConfig:
    ...
    if data.get("rules") is not None:
        ...
        out["rules"] = (
            capo_securityagent.types.network_traffic_rule_list.deserialize_json(
                data["rules"]
            )
        )
    if data.get("customHeaders") is not None:
        ...
        out["custom_headers"] = (
            capo_securityagent.types.custom_header_list.deserialize_json(
                data["customHeaders"]
            )
        )
    return out
