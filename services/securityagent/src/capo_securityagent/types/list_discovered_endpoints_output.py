...
if TYPE_CHECKING:
    import capo_securityagent.types.discovered_endpoint_list
    import capo_securityagent.types.next_token


class ListDiscoveredEndpointsOutput(TypedDict, closed=True):
    discovered_endpoints: NotRequired[
        "capo_securityagent.types.discovered_endpoint_list.DiscoveredEndpointList"
    ]
...
def serialize_json(value: ListDiscoveredEndpointsOutput) -> dict:
    ...
    if "discovered_endpoints" in value:
        ...
        out["discoveredEndpoints"] = (
            capo_securityagent.types.discovered_endpoint_list.serialize_json(
...
def deserialize_json(data: dict) -> ListDiscoveredEndpointsOutput:
    ...
    if data.get("discoveredEndpoints") is not None:
        import capo_securityagent.types.discovered_endpoint_list

        out["discovered_endpoints"] = (
            capo_securityagent.types.discovered_endpoint_list.deserialize_json(
