...
class NetworkTrafficRule(TypedDict, closed=True):
    effect: NotRequired[
        "capo_securityagent.types.network_traffic_rule_effect.NetworkTrafficRuleEffect"
    ]
    ...
    network_traffic_rule_type: NotRequired[
        "capo_securityagent.types.network_traffic_rule_type.NetworkTrafficRuleType"
    ]
...
def serialize_json(value: NetworkTrafficRule) -> dict:
    ...
    if "effect" in value:
        ...
        out["effect"] = (
            capo_securityagent.types.network_traffic_rule_effect.serialize_json(
                value["effect"]
            )
        )
    if "pattern" in value:
    ...
    if "network_traffic_rule_type" in value:
        ...
        out["networkTrafficRuleType"] = (
            capo_securityagent.types.network_traffic_rule_type.serialize_json(
...
def deserialize_json(data: dict) -> NetworkTrafficRule:
    ...
    if data.get("effect") is not None:
        ...
        out["effect"] = (
            capo_securityagent.types.network_traffic_rule_effect.deserialize_json(
                data["effect"]
            )
        )
    ...
    if data.get("networkTrafficRuleType") is not None:
        ...
        out["network_traffic_rule_type"] = (
            capo_securityagent.types.network_traffic_rule_type.deserialize_json(
