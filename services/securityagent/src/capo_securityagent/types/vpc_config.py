"""Generated from Smithy shape ``com.amazonaws.securityagent#VpcConfig``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_securityagent.types.security_group_arns
    import capo_securityagent.types.subnet_arns
    import capo_securityagent.types.vpc_arn


class VpcConfig(TypedDict, closed=True):
    vpc_arn: NotRequired["capo_securityagent.types.vpc_arn.VpcArn"]
    """<p>The Amazon Resource Name (ARN) of the VPC.</p>"""
    security_group_arns: NotRequired[
        "capo_securityagent.types.security_group_arns.SecurityGroupArns"
    ]
    """<p>The Amazon Resource Names (ARNs) of the security groups for the VPC configuration.</p>"""
    subnet_arns: NotRequired["capo_securityagent.types.subnet_arns.SubnetArns"]
    """<p>The Amazon Resource Names (ARNs) of the subnets for the VPC configuration.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: VpcConfig) -> dict:
    out: dict = {}
    if "vpc_arn" in value:
        out["vpcArn"] = value["vpc_arn"]
    if "security_group_arns" in value:
        import capo_securityagent.types.security_group_arns

        out["securityGroupArns"] = (
            capo_securityagent.types.security_group_arns.serialize_json(
                value["security_group_arns"]
            )
        )
    if "subnet_arns" in value:
        import capo_securityagent.types.subnet_arns

        out["subnetArns"] = capo_securityagent.types.subnet_arns.serialize_json(
            value["subnet_arns"]
        )
    return out


def deserialize_json(data: dict) -> VpcConfig:
    out: VpcConfig = {}  # type: ignore[typeddict-item]
    if data.get("vpcArn") is not None:
        out["vpc_arn"] = data["vpcArn"]
    if data.get("securityGroupArns") is not None:
        import capo_securityagent.types.security_group_arns

        out["security_group_arns"] = (
            capo_securityagent.types.security_group_arns.deserialize_json(
                data["securityGroupArns"]
            )
        )
    if data.get("subnetArns") is not None:
        import capo_securityagent.types.subnet_arns

        out["subnet_arns"] = capo_securityagent.types.subnet_arns.deserialize_json(
            data["subnetArns"]
        )
    return out
