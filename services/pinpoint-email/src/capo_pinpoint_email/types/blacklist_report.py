"""Generated from Smithy shape ``com.amazonaws.pinpointemail#BlacklistReport``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_pinpoint_email.types.blacklist_entries
    import capo_pinpoint_email.types.blacklist_item_name

BlacklistReport: TypeAlias = dict[
    "capo_pinpoint_email.types.blacklist_item_name.BlacklistItemName",
    "capo_pinpoint_email.types.blacklist_entries.BlacklistEntries",
]


# --- restJson1 ser/de ---
def serialize_json(input_to_serialize: BlacklistReport) -> dict:
    out: dict = {}
    for key, value in input_to_serialize.items():
        import capo_pinpoint_email.types.blacklist_entries

        out[key] = capo_pinpoint_email.types.blacklist_entries.serialize_json(value)
    return out


def deserialize_json(data: dict) -> BlacklistReport:
    out: BlacklistReport = {}
    for key, value in data.items():
        if value is None:
            continue
        import capo_pinpoint_email.types.blacklist_entries

        out[key] = capo_pinpoint_email.types.blacklist_entries.deserialize_json(value)
    return out
