"""Generated from Smithy shape ``com.amazonaws.pinpointemail#DomainDeliverabilityCampaign``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_pinpoint_email.types.campaign_id
    import capo_pinpoint_email.types.esps
    import capo_pinpoint_email.types.identity
    import capo_pinpoint_email.types.image_url
    import capo_pinpoint_email.types.ip_list
    import capo_pinpoint_email.types.percentage
    import capo_pinpoint_email.types.subject
    import capo_pinpoint_email.types.timestamp
    import capo_pinpoint_email.types.volume


class DomainDeliverabilityCampaign(TypedDict, closed=True):
    campaign_id: NotRequired["capo_pinpoint_email.types.campaign_id.CampaignId"]
    """<p>The unique identifier for the campaign. Amazon Pinpoint automatically generates and assigns this identifier to a campaign. This value is not the same as the campaign identifier that Amazon Pinpoint assigns to campaigns that you create and manage by using the Amazon Pinpoint API or the Amazon Pinpoint console.</p>"""
    image_url: NotRequired["capo_pinpoint_email.types.image_url.ImageUrl"]
    """<p>The URL of an image that contains a snapshot of the email message that was sent.</p>"""
    subject: NotRequired["capo_pinpoint_email.types.subject.Subject"]
    """<p>The subject line, or title, of the email message.</p>"""
    from_address: NotRequired["capo_pinpoint_email.types.identity.Identity"]
    """<p>The verified email address that the email message was sent from.</p>"""
    sending_ips: NotRequired["capo_pinpoint_email.types.ip_list.IpList"]
    """<p>The IP addresses that were used to send the email message.</p>"""
    first_seen_date_time: NotRequired["capo_pinpoint_email.types.timestamp.Timestamp"]
    """<p>The first time, in Unix time format, when the email message was delivered to any recipient's inbox. This value can help you determine how long it took for a campaign to deliver an email message.</p>"""
    last_seen_date_time: NotRequired["capo_pinpoint_email.types.timestamp.Timestamp"]
    """<p>The last time, in Unix time format, when the email message was delivered to any recipient's inbox. This value can help you determine how long it took for a campaign to deliver an email message.</p>"""
    inbox_count: NotRequired["capo_pinpoint_email.types.volume.Volume"]
    """<p>The number of email messages that were delivered to recipients’ inboxes.</p>"""
    spam_count: NotRequired["capo_pinpoint_email.types.volume.Volume"]
    """<p>The number of email messages that were delivered to recipients' spam or junk mail folders.</p>"""
    read_rate: NotRequired["capo_pinpoint_email.types.percentage.Percentage"]
    """<p>The percentage of email messages that were opened by recipients. Due to technical limitations, this value only includes recipients who opened the message by using an email client that supports images.</p>"""
    delete_rate: NotRequired["capo_pinpoint_email.types.percentage.Percentage"]
    """<p>The percentage of email messages that were deleted by recipients, without being opened first. Due to technical limitations, this value only includes recipients who opened the message by using an email client that supports images.</p>"""
    read_delete_rate: NotRequired["capo_pinpoint_email.types.percentage.Percentage"]
    """<p>The percentage of email messages that were opened and then deleted by recipients. Due to technical limitations, this value only includes recipients who opened the message by using an email client that supports images.</p>"""
    projected_volume: NotRequired["capo_pinpoint_email.types.volume.Volume"]
    """<p>The projected number of recipients that the email message was sent to.</p>"""
    esps: NotRequired["capo_pinpoint_email.types.esps.Esps"]
    """<p>The major email providers who handled the email message.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: DomainDeliverabilityCampaign) -> dict:
    out: dict = {}
    if "campaign_id" in value:
        out["CampaignId"] = value["campaign_id"]
    if "image_url" in value:
        out["ImageUrl"] = value["image_url"]
    if "subject" in value:
        out["Subject"] = value["subject"]
    if "from_address" in value:
        out["FromAddress"] = value["from_address"]
    if "sending_ips" in value:
        import capo_pinpoint_email.types.ip_list

        out["SendingIps"] = capo_pinpoint_email.types.ip_list.serialize_json(
            value["sending_ips"]
        )
    if "first_seen_date_time" in value:
        import capo_pinpoint_email.types.timestamp

        out["FirstSeenDateTime"] = capo_pinpoint_email.types.timestamp.serialize_json(
            value["first_seen_date_time"]
        )
    if "last_seen_date_time" in value:
        import capo_pinpoint_email.types.timestamp

        out["LastSeenDateTime"] = capo_pinpoint_email.types.timestamp.serialize_json(
            value["last_seen_date_time"]
        )
    if "inbox_count" in value:
        out["InboxCount"] = value["inbox_count"]
    if "spam_count" in value:
        out["SpamCount"] = value["spam_count"]
    if "read_rate" in value:
        out["ReadRate"] = (
            "NaN"
            if value["read_rate"] != value["read_rate"]
            else "Infinity"
            if value["read_rate"] == float("inf")
            else "-Infinity"
            if value["read_rate"] == float("-inf")
            else value["read_rate"]
        )
    if "delete_rate" in value:
        out["DeleteRate"] = (
            "NaN"
            if value["delete_rate"] != value["delete_rate"]
            else "Infinity"
            if value["delete_rate"] == float("inf")
            else "-Infinity"
            if value["delete_rate"] == float("-inf")
            else value["delete_rate"]
        )
    if "read_delete_rate" in value:
        out["ReadDeleteRate"] = (
            "NaN"
            if value["read_delete_rate"] != value["read_delete_rate"]
            else "Infinity"
            if value["read_delete_rate"] == float("inf")
            else "-Infinity"
            if value["read_delete_rate"] == float("-inf")
            else value["read_delete_rate"]
        )
    if "projected_volume" in value:
        out["ProjectedVolume"] = value["projected_volume"]
    if "esps" in value:
        import capo_pinpoint_email.types.esps

        out["Esps"] = capo_pinpoint_email.types.esps.serialize_json(value["esps"])
    return out


def deserialize_json(data: dict) -> DomainDeliverabilityCampaign:
    out: DomainDeliverabilityCampaign = {}  # type: ignore[typeddict-item]
    if data.get("CampaignId") is not None:
        out["campaign_id"] = data["CampaignId"]
    if data.get("ImageUrl") is not None:
        out["image_url"] = data["ImageUrl"]
    if data.get("Subject") is not None:
        out["subject"] = data["Subject"]
    if data.get("FromAddress") is not None:
        out["from_address"] = data["FromAddress"]
    if data.get("SendingIps") is not None:
        import capo_pinpoint_email.types.ip_list

        out["sending_ips"] = capo_pinpoint_email.types.ip_list.deserialize_json(
            data["SendingIps"]
        )
    if data.get("FirstSeenDateTime") is not None:
        import capo_pinpoint_email.types.timestamp

        out["first_seen_date_time"] = (
            capo_pinpoint_email.types.timestamp.deserialize_json(
                data["FirstSeenDateTime"]
            )
        )
    if data.get("LastSeenDateTime") is not None:
        import capo_pinpoint_email.types.timestamp

        out["last_seen_date_time"] = (
            capo_pinpoint_email.types.timestamp.deserialize_json(
                data["LastSeenDateTime"]
            )
        )
    if data.get("InboxCount") is not None:
        out["inbox_count"] = data["InboxCount"]
    if data.get("SpamCount") is not None:
        out["spam_count"] = data["SpamCount"]
    if data.get("ReadRate") is not None:
        out["read_rate"] = float(data["ReadRate"])
    if data.get("DeleteRate") is not None:
        out["delete_rate"] = float(data["DeleteRate"])
    if data.get("ReadDeleteRate") is not None:
        out["read_delete_rate"] = float(data["ReadDeleteRate"])
    if data.get("ProjectedVolume") is not None:
        out["projected_volume"] = data["ProjectedVolume"]
    if data.get("Esps") is not None:
        import capo_pinpoint_email.types.esps

        out["esps"] = capo_pinpoint_email.types.esps.deserialize_json(data["Esps"])
    return out
