...
def handle_response(
    ...
    out: capo_appsync.types.update_function_response.UpdateFunctionResponse = (
        capo_appsync.types.update_function_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{functionId}", quote(input_["function_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_appsync.types.update_function_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_function(
    ...
    capo_appsync.types.update_function_response.UpdateFunctionResponse, zapros.Response
]:
