"""Generated from Smithy shape ``com.amazonaws.appsync#CreateFunction``."""
...
def handle_response(
    ...
    out: capo_appsync.types.create_function_response.CreateFunctionResponse = (
        capo_appsync.types.create_function_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apis/{apiId}/functions"
    url = url.replace("{apiId}", quote(input_["api_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_appsync.types.create_function_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def create_function(
    ...
    capo_appsync.types.create_function_response.CreateFunctionResponse, zapros.Response
]:
