"""Generated from Smithy shape ``com.amazonaws.appsync#StartDataSourceIntrospection``."""
...
def handle_response(
    ...
    out: capo_appsync.types.start_data_source_introspection_response.StartDataSourceIntrospectionResponse = capo_appsync.types.start_data_source_introspection_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/datasources/introspections"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_appsync.types.start_data_source_introspection_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
