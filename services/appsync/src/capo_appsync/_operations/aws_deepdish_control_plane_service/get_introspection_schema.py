...
def handle_response(
    ...
    out: capo_appsync.types.get_introspection_schema_response.GetIntrospectionSchemaResponse = {
        "schema": b"".join(response.iter_raw())
...
async def async_handle_response(
    ...
    out: capo_appsync.types.get_introspection_schema_response.GetIntrospectionSchemaResponse = {
        "schema": b"".join([chunk async for chunk in response.async_iter_raw()])
    }  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    if "format" in input_:
        params.append(
            ("format", capo_appsync.types.output_type.serialize_json(input_["format"]))
        )
    if "include_directives" in input_:
        params.append(
            ("includeDirectives", "true" if input_["include_directives"] else "false")
        )
