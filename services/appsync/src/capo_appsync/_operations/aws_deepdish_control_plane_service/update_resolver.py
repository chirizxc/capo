"""Generated from Smithy shape ``com.amazonaws.appsync#UpdateResolver``."""
...
def handle_response(
    ...
    out: capo_appsync.types.update_resolver_response.UpdateResolverResponse = (
        capo_appsync.types.update_resolver_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{typeName}", quote(input_["type_name"], safe=""))
    url = url.replace("{fieldName}", quote(input_["field_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_appsync.types.update_resolver_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_resolver(
    ...
    capo_appsync.types.update_resolver_response.UpdateResolverResponse, zapros.Response
]:
...
async def async_update_resolver(
