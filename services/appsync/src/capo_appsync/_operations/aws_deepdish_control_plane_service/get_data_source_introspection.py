...
import capo_appsync.types.data_source_introspection_result
...
def handle_response(
    ...
    out: capo_appsync.types.get_data_source_introspection_response.GetDataSourceIntrospectionResponse = capo_appsync.types.get_data_source_introspection_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{introspectionId}", quote(input_["introspection_id"], safe=""))
    params: list[tuple[str, str]] = []
    params.append(
        (
            "includeModelsSDL",
            "true" if input_.get("include_models_sdl", False) else "false",
        )
    )
