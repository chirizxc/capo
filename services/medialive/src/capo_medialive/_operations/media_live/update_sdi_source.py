"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateSdiSource``."""
...
def handle_response(
    ...
    out: capo_medialive.types.update_sdi_source_response.UpdateSdiSourceResponse = (
        capo_medialive.types.update_sdi_source_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{SdiSourceId}", quote(input_["sdi_source_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.update_sdi_source_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_sdi_source(
