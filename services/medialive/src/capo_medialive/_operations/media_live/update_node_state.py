"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateNodeState``."""
...
def handle_response(
    ...
    out: capo_medialive.types.update_node_state_response.UpdateNodeStateResponse = (
        capo_medialive.types.update_node_state_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/clusters/{ClusterId}/nodes/{NodeId}/state"
    url = url.replace("{ClusterId}", quote(input_["cluster_id"], safe=""))
    url = url.replace("{NodeId}", quote(input_["node_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.update_node_state_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_node_state(
