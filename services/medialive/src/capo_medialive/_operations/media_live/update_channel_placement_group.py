"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateChannelPlacementGroup``."""
...
def handle_response(
    ...
    out: capo_medialive.types.update_channel_placement_group_response.UpdateChannelPlacementGroupResponse = capo_medialive.types.update_channel_placement_group_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(input_["channel_placement_group_id"], safe=""),
    )
    url = url.replace("{ClusterId}", quote(input_["cluster_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.update_channel_placement_group_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_channel_placement_group(
