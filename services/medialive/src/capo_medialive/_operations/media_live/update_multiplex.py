"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateMultiplex``."""
...
def handle_response(
    ...
    out: capo_medialive.types.update_multiplex_response.UpdateMultiplexResponse = (
        capo_medialive.types.update_multiplex_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{MultiplexId}", quote(input_["multiplex_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.update_multiplex_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_multiplex(
