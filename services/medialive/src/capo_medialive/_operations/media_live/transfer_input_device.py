"""Generated from Smithy shape ``com.amazonaws.medialive#TransferInputDevice``."""
...
def handle_response(
    ...
    out: capo_medialive.types.transfer_input_device_response.TransferInputDeviceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/inputDevices/{InputDeviceId}/transfer"
    url = url.replace("{InputDeviceId}", quote(input_["input_device_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.transfer_input_device_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_transfer_input_device(
