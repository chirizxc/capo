...
def handle_response(
    ...
    out: capo_medialive.types.describe_input_device_thumbnail_response.DescribeInputDeviceThumbnailResponse = {
        "body": _iter
    }  # type: ignore[reportAssignmentType]
    if "Content-Type" in response.headers:
        out["content_type"] = capo_medialive.types.content_type.deserialize_json(
...
def build_request(
    ...
    if "accept" in input_:
        headers["accept"] = capo_medialive.types.accept_header.serialize_json(
            input_["accept"]
        )
    body: bytes | None = b""
