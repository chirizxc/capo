...
class InputDeviceNetworkSettings(TypedDict, closed=True):
    ...
    ip_address: NotRequired["capo_medialive.types.__string.__string"]
    """The IP address of the input device."""
    ip_scheme: NotRequired[
        "capo_medialive.types.input_device_ip_scheme.InputDeviceIpScheme"
    ]
    """Specifies whether the input device has been configured (outside of MediaLive) to use a dynamic IP address assignment (DHCP) or a static IP address."""
    subnet_mask: NotRequired["capo_medialive.types.__string.__string"]
...
def serialize_json(value: InputDeviceNetworkSettings) -> dict:
    ...
    if "dns_addresses" in value:
        ...
        out["dnsAddresses"] = capo_medialive.types.__list_of__string.serialize_json(
            value["dns_addresses"]
        )
    if "gateway" in value:
    ...
    if "ip_scheme" in value:
        ...
        out["ipScheme"] = capo_medialive.types.input_device_ip_scheme.serialize_json(
            value["ip_scheme"]
        )
