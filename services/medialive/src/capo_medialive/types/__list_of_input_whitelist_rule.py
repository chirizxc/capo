...
if TYPE_CHECKING:
    import capo_medialive.types.input_whitelist_rule

__listOfInputWhitelistRule: TypeAlias = list[
    "capo_medialive.types.input_whitelist_rule.InputWhitelistRule"
]
...
def serialize_json(value: __listOfInputWhitelistRule) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_medialive.types.input_whitelist_rule.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfInputWhitelistRule:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_medialive.types.input_whitelist_rule.deserialize_json(item))
    return out
