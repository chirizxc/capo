...
if TYPE_CHECKING:
    import capo_medialive.types.audio_selector

__listOfAudioSelector: TypeAlias = list[
    "capo_medialive.types.audio_selector.AudioSelector"
]
...
def serialize_json(value: __listOfAudioSelector) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_medialive.types.audio_selector.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfAudioSelector:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_medialive.types.audio_selector.deserialize_json(item))
    return out
