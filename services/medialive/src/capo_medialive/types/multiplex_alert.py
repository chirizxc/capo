...
class MultiplexAlert(TypedDict, closed=True):
    ...
    state: NotRequired["capo_medialive.types.multiplex_alert_state.MultiplexAlertState"]
...
def serialize_json(value: MultiplexAlert) -> dict:
    ...
    if "state" in value:
        ...
        out["state"] = capo_medialive.types.multiplex_alert_state.serialize_json(
            value["state"]
        )
    return out
