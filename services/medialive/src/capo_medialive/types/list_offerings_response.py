"""Generated from Smithy shape ``com.amazonaws.medialive#ListOfferingsResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_medialive.types.__list_of_offering
    import capo_medialive.types.__string


class ListOfferingsResponse(TypedDict, closed=True):
    next_token: NotRequired["capo_medialive.types.__string.__string"]
    """Token to retrieve the next page of results"""
    offerings: NotRequired["capo_medialive.types.__list_of_offering.__listOfOffering"]
    """List of offerings"""


# --- restJson1 ser/de ---
def serialize_json(value: ListOfferingsResponse) -> dict:
    out: dict = {}
    if "next_token" in value:
        out["nextToken"] = value["next_token"]
    if "offerings" in value:
        import capo_medialive.types.__list_of_offering

        out["offerings"] = capo_medialive.types.__list_of_offering.serialize_json(
            value["offerings"]
        )
    return out


def deserialize_json(data: dict) -> ListOfferingsResponse:
    out: ListOfferingsResponse = {}  # type: ignore[typeddict-item]
    if data.get("nextToken") is not None:
        out["next_token"] = data["nextToken"]
    if data.get("offerings") is not None:
        import capo_medialive.types.__list_of_offering

        out["offerings"] = capo_medialive.types.__list_of_offering.deserialize_json(
            data["offerings"]
        )
    return out
