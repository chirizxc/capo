...
def serialize_json(value: UpdateNetworkRequest) -> dict:
    ...
    if "ip_pools" in value:
        ...
        out["ipPools"] = (
            capo_medialive.types.__list_of_ip_pool_update_request.serialize_json(
                value["ip_pools"]
            )
        )
    if "name" in value:
    ...
    if "routes" in value:
        ...
        out["routes"] = (
            capo_medialive.types.__list_of_route_update_request.serialize_json(
                value["routes"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> UpdateNetworkRequest:
    ...
    if data.get("ipPools") is not None:
        ...
        out["ip_pools"] = (
            capo_medialive.types.__list_of_ip_pool_update_request.deserialize_json(
                data["ipPools"]
            )
        )
    ...
    if data.get("routes") is not None:
        ...
        out["routes"] = (
            capo_medialive.types.__list_of_route_update_request.deserialize_json(
                data["routes"]
            )
        )
    return out
