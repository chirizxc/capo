...
class ClusterAlert(TypedDict, closed=True):
    ...
    node_id: NotRequired["capo_medialive.types.__string.__string"]
    """The ID of the node this alert is associated with"""
    set_timestamp: NotRequired[
        "capo_medialive.types.__timestamp_iso8601.__timestampIso8601"
    ]
    """The time when the alert was set"""
    state: NotRequired["capo_medialive.types.cluster_alert_state.ClusterAlertState"]
...
def serialize_json(value: ClusterAlert) -> dict:
    ...
    if "state" in value:
        ...
        out["state"] = capo_medialive.types.cluster_alert_state.serialize_json(
            value["state"]
        )
    return out
