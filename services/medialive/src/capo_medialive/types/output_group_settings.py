"""Generated from Smithy shape ``com.amazonaws.medialive#OutputGroupSettings``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_medialive.types.archive_group_settings
    import capo_medialive.types.cmaf_ingest_group_settings
    import capo_medialive.types.frame_capture_group_settings
    import capo_medialive.types.hls_group_settings
    import capo_medialive.types.media_connect_router_group_settings
    import capo_medialive.types.media_package_group_settings
    import capo_medialive.types.ms_smooth_group_settings
    import capo_medialive.types.multiplex_group_settings
    import capo_medialive.types.rtmp_group_settings
    import capo_medialive.types.srt_group_settings
    import capo_medialive.types.udp_group_settings


class OutputGroupSettings(TypedDict, closed=True):
    archive_group_settings: NotRequired[
        "capo_medialive.types.archive_group_settings.ArchiveGroupSettings"
    ]
    frame_capture_group_settings: NotRequired[
        "capo_medialive.types.frame_capture_group_settings.FrameCaptureGroupSettings"
    ]
    hls_group_settings: NotRequired[
        "capo_medialive.types.hls_group_settings.HlsGroupSettings"
    ]
    media_package_group_settings: NotRequired[
        "capo_medialive.types.media_package_group_settings.MediaPackageGroupSettings"
    ]
    ms_smooth_group_settings: NotRequired[
        "capo_medialive.types.ms_smooth_group_settings.MsSmoothGroupSettings"
    ]
    multiplex_group_settings: NotRequired[
        "capo_medialive.types.multiplex_group_settings.MultiplexGroupSettings"
    ]
    rtmp_group_settings: NotRequired[
        "capo_medialive.types.rtmp_group_settings.RtmpGroupSettings"
    ]
    udp_group_settings: NotRequired[
        "capo_medialive.types.udp_group_settings.UdpGroupSettings"
    ]
    cmaf_ingest_group_settings: NotRequired[
        "capo_medialive.types.cmaf_ingest_group_settings.CmafIngestGroupSettings"
    ]
    srt_group_settings: NotRequired[
        "capo_medialive.types.srt_group_settings.SrtGroupSettings"
    ]
    media_connect_router_group_settings: NotRequired[
        "capo_medialive.types.media_connect_router_group_settings.MediaConnectRouterGroupSettings"
    ]


# --- restJson1 ser/de ---
def serialize_json(value: OutputGroupSettings) -> dict:
    out: dict = {}
    if "archive_group_settings" in value:
        import capo_medialive.types.archive_group_settings

        out["archiveGroupSettings"] = (
            capo_medialive.types.archive_group_settings.serialize_json(
                value["archive_group_settings"]
            )
        )
    if "frame_capture_group_settings" in value:
        import capo_medialive.types.frame_capture_group_settings

        out["frameCaptureGroupSettings"] = (
            capo_medialive.types.frame_capture_group_settings.serialize_json(
                value["frame_capture_group_settings"]
            )
        )
    if "hls_group_settings" in value:
        import capo_medialive.types.hls_group_settings

        out["hlsGroupSettings"] = (
            capo_medialive.types.hls_group_settings.serialize_json(
                value["hls_group_settings"]
            )
        )
    if "media_package_group_settings" in value:
        import capo_medialive.types.media_package_group_settings

        out["mediaPackageGroupSettings"] = (
            capo_medialive.types.media_package_group_settings.serialize_json(
                value["media_package_group_settings"]
            )
        )
    if "ms_smooth_group_settings" in value:
        import capo_medialive.types.ms_smooth_group_settings

        out["msSmoothGroupSettings"] = (
            capo_medialive.types.ms_smooth_group_settings.serialize_json(
                value["ms_smooth_group_settings"]
            )
        )
    if "multiplex_group_settings" in value:
        import capo_medialive.types.multiplex_group_settings

        out["multiplexGroupSettings"] = (
            capo_medialive.types.multiplex_group_settings.serialize_json(
                value["multiplex_group_settings"]
            )
        )
    if "rtmp_group_settings" in value:
        import capo_medialive.types.rtmp_group_settings

        out["rtmpGroupSettings"] = (
            capo_medialive.types.rtmp_group_settings.serialize_json(
                value["rtmp_group_settings"]
            )
        )
    if "udp_group_settings" in value:
        import capo_medialive.types.udp_group_settings

        out["udpGroupSettings"] = (
            capo_medialive.types.udp_group_settings.serialize_json(
                value["udp_group_settings"]
            )
        )
    if "cmaf_ingest_group_settings" in value:
        import capo_medialive.types.cmaf_ingest_group_settings

        out["cmafIngestGroupSettings"] = (
            capo_medialive.types.cmaf_ingest_group_settings.serialize_json(
                value["cmaf_ingest_group_settings"]
            )
        )
    if "srt_group_settings" in value:
        import capo_medialive.types.srt_group_settings

        out["srtGroupSettings"] = (
            capo_medialive.types.srt_group_settings.serialize_json(
                value["srt_group_settings"]
            )
        )
    if "media_connect_router_group_settings" in value:
        import capo_medialive.types.media_connect_router_group_settings

        out["mediaConnectRouterGroupSettings"] = (
            capo_medialive.types.media_connect_router_group_settings.serialize_json(
                value["media_connect_router_group_settings"]
            )
        )
    return out


def deserialize_json(data: dict) -> OutputGroupSettings:
    out: OutputGroupSettings = {}  # type: ignore[typeddict-item]
    if data.get("archiveGroupSettings") is not None:
        import capo_medialive.types.archive_group_settings

        out["archive_group_settings"] = (
            capo_medialive.types.archive_group_settings.deserialize_json(
                data["archiveGroupSettings"]
            )
        )
    if data.get("frameCaptureGroupSettings") is not None:
        import capo_medialive.types.frame_capture_group_settings

        out["frame_capture_group_settings"] = (
            capo_medialive.types.frame_capture_group_settings.deserialize_json(
                data["frameCaptureGroupSettings"]
            )
        )
    if data.get("hlsGroupSettings") is not None:
        import capo_medialive.types.hls_group_settings

        out["hls_group_settings"] = (
            capo_medialive.types.hls_group_settings.deserialize_json(
                data["hlsGroupSettings"]
            )
        )
    if data.get("mediaPackageGroupSettings") is not None:
        import capo_medialive.types.media_package_group_settings

        out["media_package_group_settings"] = (
            capo_medialive.types.media_package_group_settings.deserialize_json(
                data["mediaPackageGroupSettings"]
            )
        )
    if data.get("msSmoothGroupSettings") is not None:
        import capo_medialive.types.ms_smooth_group_settings

        out["ms_smooth_group_settings"] = (
            capo_medialive.types.ms_smooth_group_settings.deserialize_json(
                data["msSmoothGroupSettings"]
            )
        )
    if data.get("multiplexGroupSettings") is not None:
        import capo_medialive.types.multiplex_group_settings

        out["multiplex_group_settings"] = (
            capo_medialive.types.multiplex_group_settings.deserialize_json(
                data["multiplexGroupSettings"]
            )
        )
    if data.get("rtmpGroupSettings") is not None:
        import capo_medialive.types.rtmp_group_settings

        out["rtmp_group_settings"] = (
            capo_medialive.types.rtmp_group_settings.deserialize_json(
                data["rtmpGroupSettings"]
            )
        )
    if data.get("udpGroupSettings") is not None:
        import capo_medialive.types.udp_group_settings

        out["udp_group_settings"] = (
            capo_medialive.types.udp_group_settings.deserialize_json(
                data["udpGroupSettings"]
            )
        )
    if data.get("cmafIngestGroupSettings") is not None:
        import capo_medialive.types.cmaf_ingest_group_settings

        out["cmaf_ingest_group_settings"] = (
            capo_medialive.types.cmaf_ingest_group_settings.deserialize_json(
                data["cmafIngestGroupSettings"]
            )
        )
    if data.get("srtGroupSettings") is not None:
        import capo_medialive.types.srt_group_settings

        out["srt_group_settings"] = (
            capo_medialive.types.srt_group_settings.deserialize_json(
                data["srtGroupSettings"]
            )
        )
    if data.get("mediaConnectRouterGroupSettings") is not None:
        import capo_medialive.types.media_connect_router_group_settings

        out["media_connect_router_group_settings"] = (
            capo_medialive.types.media_connect_router_group_settings.deserialize_json(
                data["mediaConnectRouterGroupSettings"]
            )
        )
    return out
