"""Generated from Smithy shape ``com.amazonaws.rekognition#ListFacesResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_rekognition.types.face_list
    import capo_rekognition.types.string


class ListFacesResponse(TypedDict, closed=True):
    faces: NotRequired["capo_rekognition.types.face_list.FaceList"]
    """<p>An array of <code>Face</code> objects. </p>"""
    next_token: NotRequired["capo_rekognition.types.string.String"]
    """<p>If the response is truncated, Amazon Rekognition returns this token that you can use in the subsequent request to retrieve the next set of faces.</p>"""
    face_model_version: NotRequired["capo_rekognition.types.string.String"]
    """<p>Version number of the face detection model associated with the input collection (<code>CollectionId</code>).</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: ListFacesResponse) -> dict:
    out: dict = {}
    if "faces" in value:
        import capo_rekognition.types.face_list

        out["Faces"] = capo_rekognition.types.face_list.serialize_aws_json_1_1(
            value["faces"]
        )
    if "next_token" in value:
        out["NextToken"] = value["next_token"]
    if "face_model_version" in value:
        out["FaceModelVersion"] = value["face_model_version"]
    return out


def deserialize_aws_json_1_1(data: dict) -> ListFacesResponse:
    out: ListFacesResponse = {}  # type: ignore[typeddict-item]
    if data.get("Faces") is not None:
        import capo_rekognition.types.face_list

        out["faces"] = capo_rekognition.types.face_list.deserialize_aws_json_1_1(
            data["Faces"]
        )
    if data.get("NextToken") is not None:
        out["next_token"] = data["NextToken"]
    if data.get("FaceModelVersion") is not None:
        out["face_model_version"] = data["FaceModelVersion"]
    return out
