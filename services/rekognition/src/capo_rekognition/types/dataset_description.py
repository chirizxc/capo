...
if TYPE_CHECKING:
    import capo_rekognition.types.dataset_stats
...
class DatasetDescription(TypedDict, closed=True):
    ...
    last_updated_timestamp: NotRequired["capo_rekognition.types.date_time.DateTime"]
    ...
    dataset_stats: NotRequired["capo_rekognition.types.dataset_stats.DatasetStats"]
...
def serialize_aws_json_1_1(value: DatasetDescription) -> dict:
    ...
    if "dataset_stats" in value:
        ...
        out["DatasetStats"] = (
            capo_rekognition.types.dataset_stats.serialize_aws_json_1_1(
                value["dataset_stats"]
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> DatasetDescription:
    ...
    if data.get("DatasetStats") is not None:
        import capo_rekognition.types.dataset_stats

        out["dataset_stats"] = (
            capo_rekognition.types.dataset_stats.deserialize_aws_json_1_1(
                data["DatasetStats"]
            )
        )
    return out
