"""Generated from Smithy shape ``com.amazonaws.rekognition#UnsuccessfulFaceDisassociationReasons``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_rekognition.types.unsuccessful_face_disassociation_reason

UnsuccessfulFaceDisassociationReasons: TypeAlias = list[
    "capo_rekognition.types.unsuccessful_face_disassociation_reason.UnsuccessfulFaceDisassociationReason"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: UnsuccessfulFaceDisassociationReasons) -> list:
    import capo_rekognition.types.unsuccessful_face_disassociation_reason

    out: list = []
    for item in value:
        out.append(
            capo_rekognition.types.unsuccessful_face_disassociation_reason.serialize_aws_json_1_1(
                item
            )
        )
    return out


def deserialize_aws_json_1_1(data: list) -> UnsuccessfulFaceDisassociationReasons:
    import capo_rekognition.types.unsuccessful_face_disassociation_reason

    out: UnsuccessfulFaceDisassociationReasons = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_rekognition.types.unsuccessful_face_disassociation_reason.deserialize_aws_json_1_1(
                item
            )
        )
    return out
