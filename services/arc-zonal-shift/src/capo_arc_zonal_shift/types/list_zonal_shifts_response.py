...
class ListZonalShiftsResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_arc_zonal_shift.types.zonal_shift_summaries.ZonalShiftSummaries"
    ]
...
def serialize_json(value: ListZonalShiftsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_arc_zonal_shift.types.zonal_shift_summaries.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListZonalShiftsResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = (
            capo_arc_zonal_shift.types.zonal_shift_summaries.deserialize_json(
                data["items"]
            )
        )
