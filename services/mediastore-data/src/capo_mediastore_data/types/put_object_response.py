...
if TYPE_CHECKING:
    ...
    import capo_mediastore_data.types.sha256_hash
    import capo_mediastore_data.types.storage_class


class PutObjectResponse(TypedDict, closed=True):
    content_sha256: NotRequired["capo_mediastore_data.types.sha256_hash.SHA256Hash"]
...
def serialize_json(value: PutObjectResponse) -> dict:
    ...
    if "storage_class" in value:
        ...
        out["StorageClass"] = capo_mediastore_data.types.storage_class.serialize_json(
            value["storage_class"]
        )
    return out
...
def deserialize_json(data: dict) -> PutObjectResponse:
    ...
    if data.get("StorageClass") is not None:
        ...
        out["storage_class"] = (
            capo_mediastore_data.types.storage_class.deserialize_json(
                data["StorageClass"]
            )
        )
    return out
