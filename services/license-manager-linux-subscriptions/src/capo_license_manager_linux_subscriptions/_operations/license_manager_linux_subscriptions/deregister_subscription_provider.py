"""Generated from Smithy shape ``com.amazonaws.licensemanagerlinuxsubscriptions#DeregisterSubscriptionProvider``."""
...
import capo_license_manager_linux_subscriptions._protocol.eventstream
...
def handle_response(
    ...
    out: capo_license_manager_linux_subscriptions.types.deregister_subscription_provider_response.DeregisterSubscriptionProviderResponse = {}  # type: ignore[typeddict-item]
    return out
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_license_manager_linux_subscriptions._auth._sigv4.build_sigv4_auth_scheme(
                "license-manager-linux-subscriptions", options.region, endpoint_scheme
            )
            if sigv4_config is not None:
                return (
                    capo_license_manager_linux_subscriptions._auth._signers.SigV4Signer(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/subscription/DeregisterSubscriptionProvider"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_license_manager_linux_subscriptions.types.deregister_subscription_provider_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_deregister_subscription_provider(
