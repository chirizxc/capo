"""Generated from Smithy shape ``com.amazonaws.wellarchitected#CreateReviewTemplate``."""
...
def handle_response(
    ...
    out: capo_wellarchitected.types.create_review_template_output.CreateReviewTemplateOutput = capo_wellarchitected.types.create_review_template_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/reviewTemplates"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wellarchitected.types.create_review_template_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_review_template(
