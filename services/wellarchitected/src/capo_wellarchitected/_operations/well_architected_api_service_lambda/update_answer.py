"""Generated from Smithy shape ``com.amazonaws.wellarchitected#UpdateAnswer``."""
...
def handle_response(
    ...
    out: capo_wellarchitected.types.update_answer_output.UpdateAnswerOutput = (
        capo_wellarchitected.types.update_answer_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{QuestionId}", quote(input_["question_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wellarchitected.types.update_answer_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_answer(
    ...
    capo_wellarchitected.types.update_answer_output.UpdateAnswerOutput, zapros.Response
]:
