"""Generated from Smithy shape ``com.amazonaws.wellarchitected#CreateLensVersion``."""
...
def handle_response(
    ...
    out: capo_wellarchitected.types.create_lens_version_output.CreateLensVersionOutput = capo_wellarchitected.types.create_lens_version_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/lenses/{LensAlias}/versions"
    url = url.replace("{LensAlias}", quote(input_["lens_alias"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wellarchitected.types.create_lens_version_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_lens_version(
