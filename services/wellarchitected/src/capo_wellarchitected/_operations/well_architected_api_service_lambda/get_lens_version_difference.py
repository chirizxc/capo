"""Generated from Smithy shape ``com.amazonaws.wellarchitected#GetLensVersionDifference``."""
...
def handle_response(
    ...
    out: capo_wellarchitected.types.get_lens_version_difference_output.GetLensVersionDifferenceOutput = capo_wellarchitected.types.get_lens_version_difference_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/lenses/{LensAlias}/versionDifference"
    url = url.replace("{LensAlias}", quote(input_["lens_alias"], safe=""))
    params: list[tuple[str, str]] = []
    if "base_lens_version" in input_:
        params.append(("BaseLensVersion", input_["base_lens_version"]))
    if "target_lens_version" in input_:
        params.append(("TargetLensVersion", input_["target_lens_version"]))
...
async def async_get_lens_version_difference(
