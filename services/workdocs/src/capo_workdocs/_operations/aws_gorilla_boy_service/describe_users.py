"""Generated from Smithy shape ``com.amazonaws.workdocs#DescribeUsers``."""
...
import capo_workdocs.errors.requested_entity_too_large_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "RequestedEntityTooLargeException":
            raise capo_workdocs.errors.requested_entity_too_large_exception.RequestedEntityTooLargeException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_workdocs.types.describe_users_response.DescribeUsersResponse = (
        capo_workdocs.types.describe_users_response.deserialize_json(
...
def build_request(
    ...
    if "organization_id" in input_:
        params.append(("organizationId", input_["organization_id"]))
    if "user_ids" in input_:
        params.append(("userIds", input_["user_ids"]))
    if "query" in input_:
        params.append(("query", input_["query"]))
    if "include" in input_:
        params.append(
            (
                "include",
                capo_workdocs.types.user_filter_type.serialize_json(input_["include"]),
            )
        )
    if "order" in input_:
        params.append(
    ...
    if "sort" in input_:
        params.append(
            ("sort", capo_workdocs.types.user_sort_type.serialize_json(input_["sort"]))
        )
    if "marker" in input_:
    ...
    if "fields" in input_:
        params.append(("fields", input_["fields"]))
...
def describe_users(
    ...
    capo_workdocs.types.describe_users_response.DescribeUsersResponse, zapros.Response
]:
