"""Generated from Smithy shape ``com.amazonaws.workdocs#DeleteCustomMetadata``."""
...
def handle_response(
    ...
    out: capo_workdocs.types.delete_custom_metadata_response.DeleteCustomMetadataResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    if "keys" in input_:
        for item in input_["keys"]:
            params.append(("keys", item))
    params.append(("deleteAll", "true" if input_.get("delete_all", False) else "false"))
...
async def async_delete_custom_metadata(
