...
def handle_response(
    ...
    out: capo_workdocs.types.describe_activities_response.DescribeActivitiesResponse = (
        capo_workdocs.types.describe_activities_response.deserialize_json(
...
def build_request(
    ...
    import capo_workdocs._protocol.serialize
    ...
    if "start_time" in input_:
        params.append(
            (
                "startTime",
                capo_workdocs._protocol.serialize.fmt_date_time(input_["start_time"]),
            )
        )
    if "end_time" in input_:
        params.append(
            (
                "endTime",
                capo_workdocs._protocol.serialize.fmt_date_time(input_["end_time"]),
            )
        )
    if "organization_id" in input_:
        params.append(("organizationId", input_["organization_id"]))
    if "activity_types" in input_:
        params.append(("activityTypes", input_["activity_types"]))
    ...
    params.append(
        (
            ...
            "true" if input_.get("include_indirect_activities", False) else "false",
        )
    )
    if "limit" in input_:
