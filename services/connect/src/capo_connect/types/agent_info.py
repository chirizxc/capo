...
if TYPE_CHECKING:
    ...
    import capo_connect.types.duration
...
class AgentInfo(TypedDict, closed=True):
    ...
    accepted_by_agent_timestamp: NotRequired["capo_connect.types.timestamp.Timestamp"]
    """<p>The timestamp when the contact was accepted by the agent.</p>"""
    preview_end_timestamp: NotRequired["capo_connect.types.timestamp.Timestamp"]
    ...
    agent_pause_duration_in_seconds: NotRequired[
        "capo_connect.types.agent_pause_duration_in_seconds.AgentPauseDurationInSeconds"
    ]
    """<p>Agent pause duration for a contact in seconds.</p>"""
    hierarchy_groups: NotRequired["capo_connect.types.hierarchy_groups.HierarchyGroups"]
    ...
    after_contact_work_duration: NotRequired["capo_connect.types.duration.Duration"]
    ...
    agent_initiated_hold_duration: NotRequired["capo_connect.types.duration.Duration"]
    """<p>The total hold duration in seconds initiated by the agent.</p>"""
    state_transitions: NotRequired[
        "capo_connect.types.state_transitions.StateTransitions"
    ]
...
def serialize_json(value: AgentInfo) -> dict:
    ...
    if "accepted_by_agent_timestamp" in value:
        ...
        out["AcceptedByAgentTimestamp"] = capo_connect.types.timestamp.serialize_json(
            value["accepted_by_agent_timestamp"]
        )
    if "preview_end_timestamp" in value:
        import capo_connect.types.timestamp

        out["PreviewEndTimestamp"] = capo_connect.types.timestamp.serialize_json(
    ...
    if "hierarchy_groups" in value:
        ...
        out["HierarchyGroups"] = capo_connect.types.hierarchy_groups.serialize_json(
    ...
    if "state_transitions" in value:
        ...
        out["StateTransitions"] = capo_connect.types.state_transitions.serialize_json(
...
def deserialize_json(data: dict) -> AgentInfo:
    ...
    if data.get("StateTransitions") is not None:
        ...
        out["state_transitions"] = (
            capo_connect.types.state_transitions.deserialize_json(
