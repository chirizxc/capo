...
class EvaluationFormSearchCriteria(TypedDict, closed=True):
    or_conditions: NotRequired[
        "capo_connect.types.evaluation_form_search_condition_list.EvaluationFormSearchConditionList"
    ]
...
def serialize_json(value: EvaluationFormSearchCriteria) -> dict:
    ...
    if "or_conditions" in value:
        ...
        out["OrConditions"] = (
            capo_connect.types.evaluation_form_search_condition_list.serialize_json(
                value["or_conditions"]
            )
        )
...
def deserialize_json(data: dict) -> EvaluationFormSearchCriteria:
    ...
    if data.get("OrConditions") is not None:
        ...
        out["or_conditions"] = (
            capo_connect.types.evaluation_form_search_condition_list.deserialize_json(
                data["OrConditions"]
            )
        )
