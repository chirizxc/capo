...
if TYPE_CHECKING:
    import capo_connect.types.security_token
    import capo_connect.types.timestamp


class Credentials(TypedDict, closed=True):
    access_token: NotRequired["capo_connect.types.security_token.SecurityToken"]
    """<p>An access token generated for a federated user to access Connect Customer.</p>"""
    access_token_expiration: NotRequired["capo_connect.types.timestamp.Timestamp"]
    """<p>A token generated with an expiration time for the session a user is logged in to Connect Customer.</p>"""
    refresh_token: NotRequired["capo_connect.types.security_token.SecurityToken"]
    """<p>Renews a token generated for a user to access the Connect Customer instance.</p>"""
    refresh_token_expiration: NotRequired["capo_connect.types.timestamp.Timestamp"]
...
def serialize_json(value: Credentials) -> dict:
    ...
    if "access_token_expiration" in value:
        ...
        out["AccessTokenExpiration"] = capo_connect.types.timestamp.serialize_json(
    ...
    if "refresh_token_expiration" in value:
        ...
        out["RefreshTokenExpiration"] = capo_connect.types.timestamp.serialize_json(
