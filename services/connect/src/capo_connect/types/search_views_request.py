...
if TYPE_CHECKING:
    ...
    import capo_connect.types.view_search_filter
...
class SearchViewsRequest(TypedDict, closed=True):
    ...
    search_filter: NotRequired["capo_connect.types.view_search_filter.ViewSearchFilter"]
...
# --- restJson1 ser/de ---
def serialize_json(value: SearchViewsRequest) -> dict:
    out: dict = {}
    ...
    if "search_filter" in value:
        ...
        out["SearchFilter"] = capo_connect.types.view_search_filter.serialize_json(
            value["search_filter"]
        )
    if "search_criteria" in value:
        import capo_connect.types.view_search_criteria

        out["SearchCriteria"] = capo_connect.types.view_search_criteria.serialize_json(
            value["search_criteria"]
        )
    return out


def deserialize_json(data: dict) -> SearchViewsRequest:
    out: SearchViewsRequest = {}  # type: ignore[typeddict-item]
    if data.get("InstanceId") is not None:
        out["instance_id"] = data["InstanceId"]
    else:
        raise DeserializationError("SearchViewsRequest.instance_id required")
    ...
    if data.get("SearchFilter") is not None:
        ...
        out["search_filter"] = capo_connect.types.view_search_filter.deserialize_json(
            data["SearchFilter"]
        )
    if data.get("SearchCriteria") is not None:
        ...
        out["search_criteria"] = (
            capo_connect.types.view_search_criteria.deserialize_json(
                data["SearchCriteria"]
            )
        )
    return out
