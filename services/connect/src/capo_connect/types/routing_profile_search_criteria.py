...
if TYPE_CHECKING:
    import capo_connect.types.routing_profile_search_condition_list
...
class RoutingProfileSearchCriteria(TypedDict, closed=True):
    or_conditions: NotRequired[
        "capo_connect.types.routing_profile_search_condition_list.RoutingProfileSearchConditionList"
    ]
...
def serialize_json(value: RoutingProfileSearchCriteria) -> dict:
    ...
    if "or_conditions" in value:
        ...
        out["OrConditions"] = (
            capo_connect.types.routing_profile_search_condition_list.serialize_json(
                value["or_conditions"]
            )
        )
...
def deserialize_json(data: dict) -> RoutingProfileSearchCriteria:
    ...
    if data.get("OrConditions") is not None:
        ...
        out["or_conditions"] = (
            capo_connect.types.routing_profile_search_condition_list.deserialize_json(
                data["OrConditions"]
            )
        )
