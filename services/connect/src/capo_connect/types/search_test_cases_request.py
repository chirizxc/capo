"""Generated from Smithy shape ``com.amazonaws.connect#SearchTestCasesRequest``."""
...
class SearchTestCasesRequest(TypedDict, closed=True):
    ...
    search_filter: NotRequired[
        "capo_connect.types.test_case_search_filter.TestCaseSearchFilter"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: SearchTestCasesRequest) -> dict:
    out: dict = {}
    ...
    if "search_filter" in value:
        ...
        out["SearchFilter"] = capo_connect.types.test_case_search_filter.serialize_json(
            value["search_filter"]
        )
    if "search_criteria" in value:
        ...
        out["SearchCriteria"] = (
            capo_connect.types.test_case_search_criteria.serialize_json(
...
def deserialize_json(data: dict) -> SearchTestCasesRequest:
    ...
    if data.get("SearchFilter") is not None:
        ...
        out["search_filter"] = (
            capo_connect.types.test_case_search_filter.deserialize_json(
                data["SearchFilter"]
            )
        )
    if data.get("SearchCriteria") is not None:
        ...
        out["search_criteria"] = (
            capo_connect.types.test_case_search_criteria.deserialize_json(
                data["SearchCriteria"]
            )
        )
    return out
