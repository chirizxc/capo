...
class GetContactMetricsResponse(TypedDict, closed=True):
    metric_results: NotRequired[
        "capo_connect.types.contact_metric_results.ContactMetricResults"
    ]
...
def serialize_json(value: GetContactMetricsResponse) -> dict:
    ...
    if "metric_results" in value:
        ...
        out["MetricResults"] = capo_connect.types.contact_metric_results.serialize_json(
...
def deserialize_json(data: dict) -> GetContactMetricsResponse:
    ...
    if data.get("MetricResults") is not None:
        ...
        out["metric_results"] = (
            capo_connect.types.contact_metric_results.deserialize_json(
                data["MetricResults"]
            )
        )
