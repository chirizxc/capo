...
if TYPE_CHECKING:
    ...
    import capo_connect.types.outbound_caller_id_name
...
class OutboundCallerConfig(TypedDict, closed=True):
    outbound_caller_id_name: NotRequired[
        "capo_connect.types.outbound_caller_id_name.OutboundCallerIdName"
    ]
    """<p>The caller ID name.</p>"""
    outbound_caller_id_number_id: NotRequired[
        "capo_connect.types.phone_number_id.PhoneNumberId"
    ]
    """<p>The caller ID number.</p>"""
    outbound_flow_id: NotRequired["capo_connect.types.contact_flow_id.ContactFlowId"]
