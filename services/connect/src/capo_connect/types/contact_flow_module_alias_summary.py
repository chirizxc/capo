...
class ContactFlowModuleAliasSummary(TypedDict, closed=True):
    arn: NotRequired["capo_connect.types.arn.ARN"]
    """<p>The Amazon Resource Name (ARN) of the flow module alias.</p>"""
    alias_id: NotRequired["capo_connect.types.resource_id.ResourceId"]
...
def deserialize_json(data: dict) -> ContactFlowModuleAliasSummary:
    ...
    if data.get("AliasDescription") is not None:
