...
if TYPE_CHECKING:
    import capo_connect.types.failed_request_list
    import capo_connect.types.successful_request_list


class BatchPutContactResponse(TypedDict, closed=True):
    successful_request_list: NotRequired[
        "capo_connect.types.successful_request_list.SuccessfulRequestList"
    ]
    """<p>List of requests for which contact was successfully created.</p>"""
    failed_request_list: NotRequired[
        "capo_connect.types.failed_request_list.FailedRequestList"
    ]
...
def serialize_json(value: BatchPutContactResponse) -> dict:
    ...
    if "successful_request_list" in value:
        ...
        out["SuccessfulRequestList"] = (
            capo_connect.types.successful_request_list.serialize_json(
    ...
    if "failed_request_list" in value:
        ...
        out["FailedRequestList"] = (
            capo_connect.types.failed_request_list.serialize_json(
...
def deserialize_json(data: dict) -> BatchPutContactResponse:
    ...
    if data.get("SuccessfulRequestList") is not None:
        import capo_connect.types.successful_request_list

        out["successful_request_list"] = (
            capo_connect.types.successful_request_list.deserialize_json(
                data["SuccessfulRequestList"]
            )
        )
    if data.get("FailedRequestList") is not None:
        import capo_connect.types.failed_request_list

        out["failed_request_list"] = (
            capo_connect.types.failed_request_list.deserialize_json(
