...
if TYPE_CHECKING:
    import capo_connect.types.searchable_agent_criteria_step


class SearchableRoutingCriteriaStep(TypedDict, closed=True):
    agent_criteria: NotRequired[
        "capo_connect.types.searchable_agent_criteria_step.SearchableAgentCriteriaStep"
    ]
...
def serialize_json(value: SearchableRoutingCriteriaStep) -> dict:
    ...
    if "agent_criteria" in value:
        ...
        out["AgentCriteria"] = (
            capo_connect.types.searchable_agent_criteria_step.serialize_json(
...
def deserialize_json(data: dict) -> SearchableRoutingCriteriaStep:
    ...
    if data.get("AgentCriteria") is not None:
        import capo_connect.types.searchable_agent_criteria_step

        out["agent_criteria"] = (
            capo_connect.types.searchable_agent_criteria_step.deserialize_json(
                data["AgentCriteria"]
            )
        )
    return out
