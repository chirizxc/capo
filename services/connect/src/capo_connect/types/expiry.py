...
if TYPE_CHECKING:
    import capo_connect.types.duration_in_seconds
    import capo_connect.types.timestamp


class Expiry(TypedDict, closed=True):
    duration_in_seconds: NotRequired[
        "capo_connect.types.duration_in_seconds.DurationInSeconds"
    ]
    """<p>The number of seconds to wait before expiring the routing step.</p>"""
    expiry_timestamp: NotRequired["capo_connect.types.timestamp.Timestamp"]
...
def serialize_json(value: Expiry) -> dict:
    ...
    if "expiry_timestamp" in value:
        ...
        out["ExpiryTimestamp"] = capo_connect.types.timestamp.serialize_json(
