"""Generated from Smithy shape ``com.amazonaws.connect#AssociateEmailAddressAlias``."""
...
import capo_connect.types.associate_email_address_alias_request
import capo_connect.types.associate_email_address_alias_response
...
def handle_response(
    ...
    out: capo_connect.types.associate_email_address_alias_response.AssociateEmailAddressAliasResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{EmailAddressId}", quote(input_["email_address_id"], safe=""))
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.associate_email_address_alias_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_associate_email_address_alias(
