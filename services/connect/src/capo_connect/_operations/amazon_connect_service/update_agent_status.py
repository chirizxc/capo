...
import capo_connect.types.update_agent_status_request
...
def build_request(
    ...
    url = url.replace("{AgentStatusId}", quote(input_["agent_status_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_agent_status_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
