"""Generated from Smithy shape ``com.amazonaws.connect#GetMetricData``."""
...
import capo_connect.types.get_metric_data_request
import capo_connect.types.get_metric_data_response
...
def handle_response(
    ...
    out: capo_connect.types.get_metric_data_response.GetMetricDataResponse = (
        capo_connect.types.get_metric_data_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/metrics/historical/{InstanceId}"
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.get_metric_data_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def get_metric_data(
    ...
    capo_connect.types.get_metric_data_response.GetMetricDataResponse, zapros.Response
]:
