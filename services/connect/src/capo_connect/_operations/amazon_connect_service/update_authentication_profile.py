...
import capo_connect.types.update_authentication_profile_request
...
def build_request(
    ...
    url = url.replace(
        "{AuthenticationProfileId}", quote(input_["authentication_profile_id"], safe="")
    )
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_authentication_profile_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
