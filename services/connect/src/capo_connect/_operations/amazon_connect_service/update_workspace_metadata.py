"""Generated from Smithy shape ``com.amazonaws.connect#UpdateWorkspaceMetadata``."""
...
import capo_connect.types.update_workspace_metadata_request
import capo_connect.types.update_workspace_metadata_response
...
def handle_response(
    ...
    out: capo_connect.types.update_workspace_metadata_response.UpdateWorkspaceMetadataResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workspaces/{InstanceId}/{WorkspaceId}/metadata"
    ...
    body: bytes | None = json.dumps(
        capo_connect.types.update_workspace_metadata_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_workspace_metadata(
