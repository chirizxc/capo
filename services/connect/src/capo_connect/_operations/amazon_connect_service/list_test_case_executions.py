"""Generated from Smithy shape ``com.amazonaws.connect#ListTestCaseExecutions``."""
...
import capo_connect.types.list_test_case_executions_request
import capo_connect.types.list_test_case_executions_response
import capo_connect.types.test_case_execution_list
...
def handle_response(
    ...
    capo_connect.types.list_test_case_executions_response.ListTestCaseExecutionsResponse
):
    out: capo_connect.types.list_test_case_executions_response.ListTestCaseExecutionsResponse = capo_connect.types.list_test_case_executions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/test-case-executions/{InstanceId}"
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "test_case_id" in input_:
        params.append(("testCaseId", input_["test_case_id"]))
    if "test_case_name" in input_:
        params.append(("testCaseName", input_["test_case_name"]))
    if "start_time" in input_:
        params.append(("startTime", str(input_["start_time"])))
    if "end_time" in input_:
        params.append(("endTime", str(input_["end_time"])))
    if "status" in input_:
        params.append(
            (
                "status",
...
async def async_list_test_case_executions(
