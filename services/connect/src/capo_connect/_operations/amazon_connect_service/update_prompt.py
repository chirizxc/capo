"""Generated from Smithy shape ``com.amazonaws.connect#UpdatePrompt``."""
...
import capo_connect.types.update_prompt_request
import capo_connect.types.update_prompt_response
...
def handle_response(
    ...
    out: capo_connect.types.update_prompt_response.UpdatePromptResponse = (
        capo_connect.types.update_prompt_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{PromptId}", quote(input_["prompt_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_prompt_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def update_prompt(
    ...
    capo_connect.types.update_prompt_response.UpdatePromptResponse, zapros.Response
]:
