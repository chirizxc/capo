"""Generated from Smithy shape ``com.amazonaws.connect#CreateContactFlowVersion``."""
...
import capo_connect.types.create_contact_flow_version_request
import capo_connect.types.create_contact_flow_version_response
...
) -> capo_connect.types.create_contact_flow_version_response.CreateContactFlowVersionResponse:
    out: capo_connect.types.create_contact_flow_version_response.CreateContactFlowVersionResponse = capo_connect.types.create_contact_flow_version_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/contact-flows/{InstanceId}/{ContactFlowId}/version"
    )
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    url = url.replace("{ContactFlowId}", quote(input_["contact_flow_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_contact_flow_version_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_contact_flow_version(
