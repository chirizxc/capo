"""Generated from Smithy shape ``com.amazonaws.connect#DisassociateRoutingProfileQueues``."""
...
import capo_connect.types.disassociate_routing_profile_queues_request
...
def build_request(
    ...
    url = url.replace(
        "{RoutingProfileId}", quote(input_["routing_profile_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.disassociate_routing_profile_queues_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_disassociate_routing_profile_queues(
