"""Generated from Smithy shape ``com.amazonaws.connect#BatchGetFlowAssociation``."""
...
import capo_connect.types.batch_get_flow_association_request
import capo_connect.types.batch_get_flow_association_response
...
def handle_response(
    ...
    out: capo_connect.types.batch_get_flow_association_response.BatchGetFlowAssociationResponse = capo_connect.types.batch_get_flow_association_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/flow-associations-batch/{InstanceId}"
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.batch_get_flow_association_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_batch_get_flow_association(
