"""Generated from Smithy shape ``com.amazonaws.connect#AssociateFlow``."""
...
import capo_connect.types.associate_flow_request
import capo_connect.types.associate_flow_response
...
def handle_response(
    ...
    out: capo_connect.types.associate_flow_response.AssociateFlowResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/flow-associations/{InstanceId}"
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.associate_flow_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def associate_flow(
    ...
    capo_connect.types.associate_flow_response.AssociateFlowResponse, zapros.Response
]:
...
async def async_associate_flow(
