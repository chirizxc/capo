"""Generated from Smithy shape ``com.amazonaws.connect#UpdateContactFlowModuleAlias``."""
...
import capo_connect.types.update_contact_flow_module_alias_request
import capo_connect.types.update_contact_flow_module_alias_response
...
def handle_response(
    ...
    out: capo_connect.types.update_contact_flow_module_alias_response.UpdateContactFlowModuleAliasResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{ContactFlowModuleId}", quote(input_["contact_flow_module_id"], safe="")
    )
    url = url.replace("{AliasId}", quote(input_["alias_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_contact_flow_module_alias_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_contact_flow_module_alias(
