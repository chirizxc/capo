"""Generated from Smithy shape ``com.amazonaws.connect#SearchNotifications``."""
...
def handle_response(
    ...
    out: capo_connect.types.search_notifications_response.SearchNotificationsResponse = capo_connect.types.search_notifications_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/search-notifications"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.search_notifications_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_search_notifications(
