"""Generated from Smithy shape ``com.amazonaws.connect#AssociateSecurityProfiles``."""
...
import capo_connect.types.associate_security_profiles_request
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/associate-security-profiles/{InstanceId}"
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.associate_security_profiles_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_associate_security_profiles(
