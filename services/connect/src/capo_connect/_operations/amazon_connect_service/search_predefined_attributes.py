"""Generated from Smithy shape ``com.amazonaws.connect#SearchPredefinedAttributes``."""
...
import capo_connect.types.search_predefined_attributes_response
...
def handle_response(
    ...
    out: capo_connect.types.search_predefined_attributes_response.SearchPredefinedAttributesResponse = capo_connect.types.search_predefined_attributes_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/search-predefined-attributes"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.search_predefined_attributes_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_search_predefined_attributes(
