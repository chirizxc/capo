...
import capo_connect.types.create_view_version_request
import capo_connect.types.create_view_version_response
...
) -> capo_connect.types.create_view_version_response.CreateViewVersionResponse:
    out: capo_connect.types.create_view_version_response.CreateViewVersionResponse = (
        capo_connect.types.create_view_version_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{ViewId}", quote(input_["view_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_view_version_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
