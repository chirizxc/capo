"""Generated from Smithy shape ``com.amazonaws.connect#UpdatePhoneNumberMetadata``."""
...
import capo_connect.types.update_phone_number_metadata_request
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/phone-number/{PhoneNumberId}/metadata"
    url = url.replace("{PhoneNumberId}", quote(input_["phone_number_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_phone_number_metadata_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_phone_number_metadata(
