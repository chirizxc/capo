"""Generated from Smithy shape ``com.amazonaws.connect#StopContactRecording``."""
...
import capo_connect.types.stop_contact_recording_request
import capo_connect.types.stop_contact_recording_response
...
def handle_response(
    ...
    out: capo_connect.types.stop_contact_recording_response.StopContactRecordingResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/stop-recording"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.stop_contact_recording_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_stop_contact_recording(
