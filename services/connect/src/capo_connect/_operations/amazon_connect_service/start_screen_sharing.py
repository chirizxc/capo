"""Generated from Smithy shape ``com.amazonaws.connect#StartScreenSharing``."""
...
import capo_connect.types.start_screen_sharing_request
import capo_connect.types.start_screen_sharing_response
...
def handle_response(
    ...
    out: capo_connect.types.start_screen_sharing_response.StartScreenSharingResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/screen-sharing"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.start_screen_sharing_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_start_screen_sharing(
