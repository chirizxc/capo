"""Generated from Smithy shape ``com.amazonaws.connect#UpdateEvaluationForm``."""
...
def handle_response(
    ...
    out: capo_connect.types.update_evaluation_form_response.UpdateEvaluationFormResponse = capo_connect.types.update_evaluation_form_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{EvaluationFormId}", quote(input_["evaluation_form_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_evaluation_form_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_evaluation_form(
