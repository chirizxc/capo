"""Generated from Smithy shape ``com.amazonaws.connect#UpdateParticipantAuthentication``."""
...
import capo_connect.types.update_participant_authentication_response
...
def handle_response(
    ...
    out: capo_connect.types.update_participant_authentication_response.UpdateParticipantAuthenticationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/update-participant-authentication"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_participant_authentication_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_participant_authentication(
