"""Generated from Smithy shape ``com.amazonaws.connect#UpdateWorkspacePage``."""
...
import capo_connect.types.update_workspace_page_request
import capo_connect.types.update_workspace_page_response
...
def handle_response(
    ...
    out: capo_connect.types.update_workspace_page_response.UpdateWorkspacePageResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{Page}", quote(input_["page"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_workspace_page_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_workspace_page(
