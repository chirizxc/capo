"""Generated from Smithy shape ``com.amazonaws.connect#UpdateContactFlowContent``."""
...
import capo_connect.types.update_contact_flow_content_request
import capo_connect.types.update_contact_flow_content_response
...
def handle_response(
    ...
    out: capo_connect.types.update_contact_flow_content_response.UpdateContactFlowContentResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/contact-flows/{InstanceId}/{ContactFlowId}/content"
    )
    ...
    body: bytes | None = json.dumps(
        capo_connect.types.update_contact_flow_content_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_contact_flow_content(
