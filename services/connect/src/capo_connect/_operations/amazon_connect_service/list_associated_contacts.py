"""Generated from Smithy shape ``com.amazonaws.connect#ListAssociatedContacts``."""
...
import capo_connect.types.list_associated_contacts_request
import capo_connect.types.list_associated_contacts_response
...
def handle_response(
    ...
    out: capo_connect.types.list_associated_contacts_response.ListAssociatedContactsResponse = capo_connect.types.list_associated_contacts_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/associated/{InstanceId}"
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "contact_id" in input_:
        params.append(("contactId", input_["contact_id"]))
...
async def async_list_associated_contacts(
