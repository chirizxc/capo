"""Generated from Smithy shape ``com.amazonaws.connect#SearchResourceTags``."""
...
import capo_connect.errors.maximum_result_returned_exception
...
import capo_connect.types.search_resource_tags_request
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "MaximumResultReturnedException":
            raise capo_connect.errors.maximum_result_returned_exception.MaximumResultReturnedException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_connect.types.search_resource_tags_response.SearchResourceTagsResponse = (
        capo_connect.types.search_resource_tags_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/search-resource-tags"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.search_resource_tags_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_search_resource_tags(
