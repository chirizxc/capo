"""Generated from Smithy shape ``com.amazonaws.connect#SuspendContactRecording``."""
...
import capo_connect.types.suspend_contact_recording_request
import capo_connect.types.suspend_contact_recording_response
...
def handle_response(
    ...
    out: capo_connect.types.suspend_contact_recording_response.SuspendContactRecordingResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/suspend-recording"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.suspend_contact_recording_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_suspend_contact_recording(
