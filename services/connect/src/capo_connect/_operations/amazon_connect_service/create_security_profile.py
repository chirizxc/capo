"""Generated from Smithy shape ``com.amazonaws.connect#CreateSecurityProfile``."""
...
import capo_connect.types.create_security_profile_response
...
def handle_response(
    ...
    out: capo_connect.types.create_security_profile_response.CreateSecurityProfileResponse = capo_connect.types.create_security_profile_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/security-profiles/{InstanceId}"
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_security_profile_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
