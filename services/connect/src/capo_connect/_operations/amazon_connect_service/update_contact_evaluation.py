"""Generated from Smithy shape ``com.amazonaws.connect#UpdateContactEvaluation``."""
...
import capo_connect.types.update_contact_evaluation_request
import capo_connect.types.update_contact_evaluation_response
...
def handle_response(
    ...
    out: capo_connect.types.update_contact_evaluation_response.UpdateContactEvaluationResponse = capo_connect.types.update_contact_evaluation_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{EvaluationId}", quote(input_["evaluation_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_contact_evaluation_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_contact_evaluation(
