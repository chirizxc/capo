"""Generated from Smithy shape ``com.amazonaws.connect#ListPhoneNumbers``."""
...
import capo_connect.types.list_phone_numbers_response
...
def handle_response(
    ...
    out: capo_connect.types.list_phone_numbers_response.ListPhoneNumbersResponse = (
        capo_connect.types.list_phone_numbers_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/phone-numbers-summary/{InstanceId}"
    url = url.replace("{InstanceId}", quote(input_["instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "phone_number_types" in input_:
        for item in input_["phone_number_types"]:
            params.append(
                (
                    "phoneNumberTypes",
                    capo_connect.types.phone_number_type.serialize_json(item),
                )
            )
    if "phone_number_country_codes" in input_:
        for item in input_["phone_number_country_codes"]:
            params.append(
                (
                    "phoneNumberCountryCodes",
                    capo_connect.types.phone_number_country_code.serialize_json(item),
                )
            )
