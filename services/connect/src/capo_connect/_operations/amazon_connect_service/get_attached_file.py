"""Generated from Smithy shape ``com.amazonaws.connect#GetAttachedFile``."""
...
import capo_connect.types.get_attached_file_request
import capo_connect.types.get_attached_file_response
...
def handle_response(
    ...
    out: capo_connect.types.get_attached_file_response.GetAttachedFileResponse = (
        capo_connect.types.get_attached_file_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{FileId}", quote(input_["file_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "url_expiry_in_seconds" in input_:
        params.append(("urlExpiryInSeconds", str(input_["url_expiry_in_seconds"])))
    if "associated_resource_arn" in input_:
        params.append(("associatedResourceArn", input_["associated_resource_arn"]))
