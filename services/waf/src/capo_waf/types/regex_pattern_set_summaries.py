...
if TYPE_CHECKING:
    import capo_waf.types.regex_pattern_set_summary

RegexPatternSetSummaries: TypeAlias = list[
    "capo_waf.types.regex_pattern_set_summary.RegexPatternSetSummary"
]
...
def serialize_aws_json_1_1(value: RegexPatternSetSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_waf.types.regex_pattern_set_summary.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> RegexPatternSetSummaries:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_waf.types.regex_pattern_set_summary.deserialize_aws_json_1_1(item)
        )
    return out
