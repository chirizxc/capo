"""Generated from Smithy shape ``com.amazonaws.waf#LoggingConfigurations``."""
...
def serialize_aws_json_1_1(value: LoggingConfigurations) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_waf.types.logging_configuration.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> LoggingConfigurations:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_waf.types.logging_configuration.deserialize_aws_json_1_1(item))
    return out
