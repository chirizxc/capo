...
class DescribeGlobalSettingsOutput(TypedDict, closed=True):
    global_settings: NotRequired["capo_backup.types.global_settings.GlobalSettings"]
    ...
    last_update_time: NotRequired["capo_backup.types.timestamp.timestamp"]
...
def serialize_json(value: DescribeGlobalSettingsOutput) -> dict:
    ...
    if "global_settings" in value:
        ...
        out["GlobalSettings"] = capo_backup.types.global_settings.serialize_json(
            value["global_settings"]
        )
    if "last_update_time" in value:
        import capo_backup.types.timestamp

        out["LastUpdateTime"] = capo_backup.types.timestamp.serialize_json(
