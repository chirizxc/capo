...
class ListRestoreJobSummariesOutput(TypedDict, closed=True):
    restore_job_summaries: NotRequired[
        "capo_backup.types.restore_job_summary_list.RestoreJobSummaryList"
    ]
    ...
    aggregation_period: NotRequired["capo_backup.types.string.string"]
...
def serialize_json(value: ListRestoreJobSummariesOutput) -> dict:
    ...
    if "restore_job_summaries" in value:
        ...
        out["RestoreJobSummaries"] = (
            capo_backup.types.restore_job_summary_list.serialize_json(
...
def deserialize_json(data: dict) -> ListRestoreJobSummariesOutput:
    ...
    if data.get("RestoreJobSummaries") is not None:
        import capo_backup.types.restore_job_summary_list

        out["restore_job_summaries"] = (
            capo_backup.types.restore_job_summary_list.deserialize_json(
