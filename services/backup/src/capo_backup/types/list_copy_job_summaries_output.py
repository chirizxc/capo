...
class ListCopyJobSummariesOutput(TypedDict, closed=True):
    copy_job_summaries: NotRequired[
        "capo_backup.types.copy_job_summary_list.CopyJobSummaryList"
    ]
...
def serialize_json(value: ListCopyJobSummariesOutput) -> dict:
    ...
    if "copy_job_summaries" in value:
        ...
        out["CopyJobSummaries"] = (
            capo_backup.types.copy_job_summary_list.serialize_json(
...
def deserialize_json(data: dict) -> ListCopyJobSummariesOutput:
    ...
    if data.get("CopyJobSummaries") is not None:
        import capo_backup.types.copy_job_summary_list

        out["copy_job_summaries"] = (
            capo_backup.types.copy_job_summary_list.deserialize_json(
