...
class ListReportJobsOutput(TypedDict, closed=True):
    report_jobs: NotRequired["capo_backup.types.report_job_list.ReportJobList"]
...
def serialize_json(value: ListReportJobsOutput) -> dict:
    ...
    if "report_jobs" in value:
        ...
        out["ReportJobs"] = capo_backup.types.report_job_list.serialize_json(
            value["report_jobs"]
        )
...
def deserialize_json(data: dict) -> ListReportJobsOutput:
    ...
    if data.get("ReportJobs") is not None:
        import capo_backup.types.report_job_list

        out["report_jobs"] = capo_backup.types.report_job_list.deserialize_json(
            data["ReportJobs"]
        )
