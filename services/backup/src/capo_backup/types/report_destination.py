...
class ReportDestination(TypedDict, closed=True):
    s3_bucket_name: NotRequired["capo_backup.types.string.string"]
    """<p>The unique name of the Amazon S3 bucket that receives your reports.</p>"""
    s3_keys: NotRequired["capo_backup.types.string_list.stringList"]
...
def serialize_json(value: ReportDestination) -> dict:
    ...
    if "s3_keys" in value:
        ...
        out["S3Keys"] = capo_backup.types.string_list.serialize_json(value["s3_keys"])
    return out
