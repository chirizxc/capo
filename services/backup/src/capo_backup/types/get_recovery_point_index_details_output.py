...
class GetRecoveryPointIndexDetailsOutput(TypedDict, closed=True):
    ...
    index_deletion_date: NotRequired["capo_backup.types.timestamp.timestamp"]
    ...
    index_completion_date: NotRequired["capo_backup.types.timestamp.timestamp"]
    ...
    total_items_indexed: NotRequired["capo_backup.types.long.Long"]
...
def serialize_json(value: GetRecoveryPointIndexDetailsOutput) -> dict:
    ...
    if "index_deletion_date" in value:
        ...
        out["IndexDeletionDate"] = capo_backup.types.timestamp.serialize_json(
            value["index_deletion_date"]
        )
    if "index_completion_date" in value:
        import capo_backup.types.timestamp

        out["IndexCompletionDate"] = capo_backup.types.timestamp.serialize_json(
