...
class ScanJobSummary(TypedDict, closed=True):
...
def serialize_json(value: ScanJobSummary) -> dict:
    ...
    if "state" in value:
        ...
        out["State"] = capo_backup.types.scan_job_status.serialize_json(value["state"])
    ...
    if "scan_result_status" in value:
        ...
        out["ScanResultStatus"] = capo_backup.types.scan_result_status.serialize_json(
...
def deserialize_json(data: dict) -> ScanJobSummary:
    ...
    if data.get("ScanResultStatus") is not None:
        ...
        out["scan_result_status"] = (
            capo_backup.types.scan_result_status.deserialize_json(
