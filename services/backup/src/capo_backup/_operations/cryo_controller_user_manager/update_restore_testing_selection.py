"""Generated from Smithy shape ``com.amazonaws.backup#UpdateRestoreTestingSelection``."""
...
def handle_response(
    ...
    out: capo_backup.types.update_restore_testing_selection_output.UpdateRestoreTestingSelectionOutput = capo_backup.types.update_restore_testing_selection_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{RestoreTestingPlanName}", quote(input_["restore_testing_plan_name"], safe="")
    )
    url = url.replace(
        "{RestoreTestingSelectionName}",
        quote(input_["restore_testing_selection_name"], safe=""),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_backup.types.update_restore_testing_selection_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_restore_testing_selection(
