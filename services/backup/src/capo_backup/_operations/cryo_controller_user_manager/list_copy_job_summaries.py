...
def build_request(
    ...
    if "state" in input_:
        params.append(
            ("State", capo_backup.types.copy_job_status.serialize_json(input_["state"]))
        )
    if "resource_type" in input_:
        params.append(("ResourceType", input_["resource_type"]))
    if "message_category" in input_:
        params.append(("MessageCategory", input_["message_category"]))
    if "aggregation_period" in input_:
        params.append(
            (
                "AggregationPeriod",
                capo_backup.types.aggregation_period.serialize_json(
                    input_["aggregation_period"]
                ),
            )
        )
