"""Generated from Smithy shape ``com.amazonaws.backup#UpdateBackupPlan``."""
...
def handle_response(
    ...
    out: capo_backup.types.update_backup_plan_output.UpdateBackupPlanOutput = (
        capo_backup.types.update_backup_plan_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{BackupPlanId}", quote(input_["backup_plan_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_backup.types.update_backup_plan_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_backup_plan(
    ...
    capo_backup.types.update_backup_plan_output.UpdateBackupPlanOutput, zapros.Response
]:
