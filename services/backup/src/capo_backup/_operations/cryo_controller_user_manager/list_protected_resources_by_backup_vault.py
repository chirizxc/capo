"""Generated from Smithy shape ``com.amazonaws.backup#ListProtectedResourcesByBackupVault``."""
...
def handle_response(
    ...
    out: capo_backup.types.list_protected_resources_by_backup_vault_output.ListProtectedResourcesByBackupVaultOutput = capo_backup.types.list_protected_resources_by_backup_vault_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/backup-vaults/{BackupVaultName}/resources"
    url = url.replace("{BackupVaultName}", quote(input_["backup_vault_name"], safe=""))
    params: list[tuple[str, str]] = []
    if "backup_vault_account_id" in input_:
        params.append(("backupVaultAccountId", input_["backup_vault_account_id"]))
...
async def async_list_protected_resources_by_backup_vault(
