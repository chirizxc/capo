...
def handle_response(
    ...
    out: capo_backup.types.update_framework_output.UpdateFrameworkOutput = (
        capo_backup.types.update_framework_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{FrameworkName}", quote(input_["framework_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_backup.types.update_framework_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def update_framework(
    ...
    capo_backup.types.update_framework_output.UpdateFrameworkOutput, zapros.Response
]:
