"""Generated from Smithy shape ``com.amazonaws.bcmdashboards#TagResource``."""
...
import capo_bcm_dashboards._protocol.eventstream
...
def handle_response(
    ...
    out: capo_bcm_dashboards.types.tag_resource_response.TagResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_bcm_dashboards._auth._sigv4.build_sigv4_auth_scheme(
                "bcm-dashboards", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "AWSBCMDashboardsService.TagResource"
    body: bytes | None = json.dumps(
        capo_bcm_dashboards.types.tag_resource_request.serialize_aws_json_1_0(input_),
        allow_nan=False,
    ).encode()
...
def tag_resource(
    ...
    capo_bcm_dashboards.types.tag_resource_response.TagResourceResponse, zapros.Response
]:
