"""Generated from Smithy shape ``com.amazonaws.polly#GetSpeechSynthesisTask``."""
...
import capo_polly.errors.invalid_task_id_exception
import capo_polly.errors.service_failure_exception
import capo_polly.errors.synthesis_task_not_found_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "InvalidTaskIdException":
            raise capo_polly.errors.invalid_task_id_exception.InvalidTaskIdException.from_json(
                data, message
            )
        ...
        case "SynthesisTaskNotFoundException":
            raise capo_polly.errors.synthesis_task_not_found_exception.SynthesisTaskNotFoundException.from_json(
                data, message
            )
        case _:
...
def handle_response(
    ...
    out: capo_polly.types.get_speech_synthesis_task_output.GetSpeechSynthesisTaskOutput = capo_polly.types.get_speech_synthesis_task_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/synthesisTasks/{TaskId}"
    url = url.replace("{TaskId}", quote(input_["task_id"], safe=""))
...
async def async_get_speech_synthesis_task(
