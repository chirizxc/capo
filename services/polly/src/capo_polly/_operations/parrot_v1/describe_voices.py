...
def build_request(
    ...
    if "engine" in input_:
        params.append(
            ("Engine", capo_polly.types.engine.serialize_json(input_["engine"]))
        )
    if "language_code" in input_:
        params.append(
            (
                "LanguageCode",
                capo_polly.types.language_code.serialize_json(input_["language_code"]),
            )
        )
    params.append(
        (
            ...
            "true"
            if input_.get("include_additional_language_codes", False)
            else "false",
        )
    )
