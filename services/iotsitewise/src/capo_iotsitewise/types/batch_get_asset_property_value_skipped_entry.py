...
class BatchGetAssetPropertyValueSkippedEntry(TypedDict, closed=True):
    ...
    error_info: NotRequired[
        "capo_iotsitewise.types.batch_get_asset_property_value_error_info.BatchGetAssetPropertyValueErrorInfo"
    ]
...
def serialize_json(value: BatchGetAssetPropertyValueSkippedEntry) -> dict:
    ...
    if "error_info" in value:
        ...
        out["errorInfo"] = (
            capo_iotsitewise.types.batch_get_asset_property_value_error_info.serialize_json(
                value["error_info"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> BatchGetAssetPropertyValueSkippedEntry:
    ...
    if data.get("errorInfo") is not None:
        ...
        out["error_info"] = (
            capo_iotsitewise.types.batch_get_asset_property_value_error_info.deserialize_json(
                data["errorInfo"]
            )
        )
    return out
