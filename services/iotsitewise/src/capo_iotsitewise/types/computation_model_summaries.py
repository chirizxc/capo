...
ComputationModelSummaries: TypeAlias = list[
    "capo_iotsitewise.types.computation_model_summary.ComputationModelSummary"
]
...
def serialize_json(value: ComputationModelSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_iotsitewise.types.computation_model_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ComputationModelSummaries:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_iotsitewise.types.computation_model_summary.deserialize_json(item)
        )
    return out
