...
if TYPE_CHECKING:
    import capo_iotsitewise.types.asset_hierarchy

AssetHierarchies: TypeAlias = list[
    "capo_iotsitewise.types.asset_hierarchy.AssetHierarchy"
]
...
def serialize_json(value: AssetHierarchies) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_iotsitewise.types.asset_hierarchy.serialize_json(item))
    return out
