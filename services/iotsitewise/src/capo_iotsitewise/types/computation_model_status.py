...
if TYPE_CHECKING:
    import capo_iotsitewise.types.computation_model_state
    import capo_iotsitewise.types.error_details


class ComputationModelStatus(TypedDict, closed=True):
    state: "capo_iotsitewise.types.computation_model_state.ComputationModelState"
...
def serialize_json(value: ComputationModelStatus) -> dict:
    ...
    out["state"] = capo_iotsitewise.types.computation_model_state.serialize_json(
        value["state"]
    )
    if "error" in value:
        import capo_iotsitewise.types.error_details
...
def deserialize_json(data: dict) -> ComputationModelStatus:
    ...
    if data.get("state") is not None:
        ...
        out["state"] = capo_iotsitewise.types.computation_model_state.deserialize_json(
            data["state"]
        )
    else:
