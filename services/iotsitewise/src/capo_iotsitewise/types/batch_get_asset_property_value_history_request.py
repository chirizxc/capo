...
def serialize_json(value: BatchGetAssetPropertyValueHistoryRequest) -> dict:
    ...
    out["entries"] = (
        capo_iotsitewise.types.batch_get_asset_property_value_history_entries.serialize_json(
            value["entries"]
        )
    )
...
def deserialize_json(data: dict) -> BatchGetAssetPropertyValueHistoryRequest:
    ...
    if data.get("entries") is not None:
        ...
        out["entries"] = (
            capo_iotsitewise.types.batch_get_asset_property_value_history_entries.deserialize_json(
                data["entries"]
            )
        )
    else:
