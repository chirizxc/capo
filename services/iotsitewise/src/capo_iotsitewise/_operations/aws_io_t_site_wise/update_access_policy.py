...
def handle_response(
    ...
    out: capo_iotsitewise.types.update_access_policy_response.UpdateAccessPolicyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{accessPolicyId}", quote(input_["access_policy_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iotsitewise.types.update_access_policy_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
