"""Generated from Smithy shape ``com.amazonaws.iotsitewise#ListAssociatedAssets``."""
...
def handle_response(
    ...
    out: capo_iotsitewise.types.list_associated_assets_response.ListAssociatedAssetsResponse = capo_iotsitewise.types.list_associated_assets_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/assets/{assetId}/hierarchies"
    url = url.replace("{assetId}", quote(input_["asset_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "hierarchy_id" in input_:
        params.append(("hierarchyId", input_["hierarchy_id"]))
    if "traversal_direction" in input_:
        params.append(
            (
                "traversalDirection",
                capo_iotsitewise.types.traversal_direction.serialize_json(
                    input_["traversal_direction"]
                ),
            )
        )
...
async def async_list_associated_assets(
