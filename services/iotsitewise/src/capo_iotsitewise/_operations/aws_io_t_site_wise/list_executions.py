...
def build_request(
    ...
    if "target_resource_type" in input_:
        params.append(
            (
                "targetResourceType",
                capo_iotsitewise.types.target_resource_type.serialize_json(
                    input_["target_resource_type"]
                ),
            )
        )
    if "target_resource_id" in input_:
        params.append(("targetResourceId", input_["target_resource_id"]))
    if "resolve_to_resource_type" in input_:
        params.append(
            (
                "resolveToResourceType",
                capo_iotsitewise.types.resolve_to_resource_type.serialize_json(
                    input_["resolve_to_resource_type"]
                ),
            )
        )
    if "resolve_to_resource_id" in input_:
        params.append(("resolveToResourceId", input_["resolve_to_resource_id"]))
    if "next_token" in input_:
        params.append(("nextToken", input_["next_token"]))
    if "max_results" in input_:
        params.append(("maxResults", str(input_["max_results"])))
    if "action_type" in input_:
        params.append(("actionType", input_["action_type"]))
