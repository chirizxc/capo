...
from capo_iotsitewise._protocol.eventstream import (
    MessageDecoder,
...
def handle_response(
    ...
    _union_deser = capo_iotsitewise.types.response_stream.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: capo_iotsitewise.types.invoke_assistant_response.InvokeAssistantResponse = {
        "body": cast(Any, raw_stream_to_events(_iter, _message_decoder, _union_deser))
    }  # type: ignore[reportAssignmentType]
    out["conversation_id"] = response.headers[
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/assistant/invocation"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iotsitewise.types.invoke_assistant_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
