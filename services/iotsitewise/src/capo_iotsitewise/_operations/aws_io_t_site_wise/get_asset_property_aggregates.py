"""Generated from Smithy shape ``com.amazonaws.iotsitewise#GetAssetPropertyAggregates``."""
...
def handle_response(
    ...
    out: capo_iotsitewise.types.get_asset_property_aggregates_response.GetAssetPropertyAggregatesResponse = capo_iotsitewise.types.get_asset_property_aggregates_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/properties/aggregates"
    ...
    for item in input_["aggregate_types"]:
        params.append(
            (
                "aggregateTypes",
                capo_iotsitewise.types.aggregate_type.serialize_json(item),
            )
        )
    if "resolution" in input_:
        params.append(("resolution", input_["resolution"]))
    if "qualities" in input_:
        for item in input_["qualities"]:
            params.append(
                ("qualities", capo_iotsitewise.types.quality.serialize_json(item))
            )
    ...
    if "time_ordering" in input_:
        params.append(
            (
                "timeOrdering",
                capo_iotsitewise.types.time_ordering.serialize_json(
                    input_["time_ordering"]
                ),
            )
        )
...
async def async_get_asset_property_aggregates(
