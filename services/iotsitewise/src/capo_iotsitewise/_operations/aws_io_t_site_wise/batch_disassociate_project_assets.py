...
def handle_response(
    ...
    out: capo_iotsitewise.types.batch_disassociate_project_assets_response.BatchDisassociateProjectAssetsResponse = capo_iotsitewise.types.batch_disassociate_project_assets_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{projectId}", quote(input_["project_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iotsitewise.types.batch_disassociate_project_assets_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
