...
class Body(TypedDict, closed=True):
    text: NotRequired["capo_sesv2.types.content.Content"]
    ...
    html: NotRequired["capo_sesv2.types.content.Content"]
...
def serialize_json(value: Body) -> dict:
    ...
    if "text" in value:
        ...
        out["Text"] = capo_sesv2.types.content.serialize_json(value["text"])
    if "html" in value:
        import capo_sesv2.types.content

        out["Html"] = capo_sesv2.types.content.serialize_json(value["html"])
    return out
...
def deserialize_json(data: dict) -> Body:
    ...
    if data.get("Text") is not None:
        ...
        out["text"] = capo_sesv2.types.content.deserialize_json(data["Text"])
    if data.get("Html") is not None:
        import capo_sesv2.types.content

        out["html"] = capo_sesv2.types.content.deserialize_json(data["Html"])
    return out
