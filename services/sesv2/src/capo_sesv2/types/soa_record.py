...
if TYPE_CHECKING:
    import capo_sesv2.types.admin_email
    import capo_sesv2.types.primary_name_server
    import capo_sesv2.types.serial_number


class SOARecord(TypedDict, closed=True):
    primary_name_server: NotRequired[
        "capo_sesv2.types.primary_name_server.PrimaryNameServer"
    ]
    """<p>Primary name server specified in the SOA record.</p>"""
    admin_email: NotRequired["capo_sesv2.types.admin_email.AdminEmail"]
    """<p>Administrative contact email from the SOA record.</p>"""
    serial_number: "capo_sesv2.types.serial_number.SerialNumber"
