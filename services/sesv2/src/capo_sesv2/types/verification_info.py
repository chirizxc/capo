...
class VerificationInfo(TypedDict, closed=True):
    last_checked_timestamp: NotRequired["capo_sesv2.types.timestamp.Timestamp"]
    """<p>The last time a verification attempt was made for this identity.</p>"""
    last_success_timestamp: NotRequired["capo_sesv2.types.timestamp.Timestamp"]
    """<p>The last time a successful verification was made for this identity.</p>"""
    error_type: NotRequired["capo_sesv2.types.verification_error.VerificationError"]
    ...
    soa_record: NotRequired["capo_sesv2.types.soa_record.SOARecord"]
...
def serialize_json(value: VerificationInfo) -> dict:
    ...
    if "last_checked_timestamp" in value:
        ...
        out["LastCheckedTimestamp"] = capo_sesv2.types.timestamp.serialize_json(
            value["last_checked_timestamp"]
        )
    if "last_success_timestamp" in value:
        import capo_sesv2.types.timestamp

        out["LastSuccessTimestamp"] = capo_sesv2.types.timestamp.serialize_json(
            value["last_success_timestamp"]
        )
    if "error_type" in value:
        import capo_sesv2.types.verification_error

        out["ErrorType"] = capo_sesv2.types.verification_error.serialize_json(
            value["error_type"]
        )
    if "soa_record" in value:
        import capo_sesv2.types.soa_record

        out["SOARecord"] = capo_sesv2.types.soa_record.serialize_json(
            value["soa_record"]
        )
    return out
