"""Generated from Smithy shape ``com.amazonaws.sesv2#ListResourceTenants``."""
...
import capo_sesv2.types.resource_tenant_metadata_list
...
def handle_response(
    ...
    out: capo_sesv2.types.list_resource_tenants_response.ListResourceTenantsResponse = (
        capo_sesv2.types.list_resource_tenants_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/resources/tenants/list"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.list_resource_tenants_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_resource_tenants(
