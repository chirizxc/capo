"""Generated from Smithy shape ``com.amazonaws.sesv2#CreateExportJob``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.create_export_job_response.CreateExportJobResponse = (
        capo_sesv2.types.create_export_job_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/export-jobs"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.create_export_job_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def create_export_job(
    ...
    capo_sesv2.types.create_export_job_response.CreateExportJobResponse, zapros.Response
]:
