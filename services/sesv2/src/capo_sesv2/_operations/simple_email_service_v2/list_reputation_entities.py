"""Generated from Smithy shape ``com.amazonaws.sesv2#ListReputationEntities``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.list_reputation_entities_response.ListReputationEntitiesResponse = capo_sesv2.types.list_reputation_entities_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/reputation/entities"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.list_reputation_entities_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_reputation_entities(
