"""Generated from Smithy shape ``com.amazonaws.sesv2#ListContacts``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.list_contacts_response.ListContactsResponse = (
        capo_sesv2.types.list_contacts_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{ContactListName}", quote(input_["contact_list_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.list_contacts_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def list_contacts(
    ...
    capo_sesv2.types.list_contacts_response.ListContactsResponse, zapros.Response
]:
