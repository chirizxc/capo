"""Generated from Smithy shape ``com.amazonaws.sesv2#BatchGetMetricData``."""
...
import capo_sesv2.errors.internal_service_error_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InternalServiceErrorException":
            raise capo_sesv2.errors.internal_service_error_exception.InternalServiceErrorException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_sesv2.types.batch_get_metric_data_response.BatchGetMetricDataResponse = (
        capo_sesv2.types.batch_get_metric_data_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/metrics/batch"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.batch_get_metric_data_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_batch_get_metric_data(
