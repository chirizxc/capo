"""Generated from Smithy shape ``com.amazonaws.sesv2#PutAccountVdmAttributes``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.put_account_vdm_attributes_response.PutAccountVdmAttributesResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/account/vdm"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.put_account_vdm_attributes_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_put_account_vdm_attributes(
