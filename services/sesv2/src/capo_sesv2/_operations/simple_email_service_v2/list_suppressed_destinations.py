"""Generated from Smithy shape ``com.amazonaws.sesv2#ListSuppressedDestinations``."""
...
import capo_sesv2.errors.invalid_next_token_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidNextTokenException":
            raise capo_sesv2.errors.invalid_next_token_exception.InvalidNextTokenException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_sesv2.types.list_suppressed_destinations_response.ListSuppressedDestinationsResponse = capo_sesv2.types.list_suppressed_destinations_response.deserialize_json(
...
def build_request(
    ...
    import capo_sesv2._protocol.serialize
    ...
    if "tenant_name" in input_:
        params.append(("TenantName", input_["tenant_name"]))
    if "reasons" in input_:
        for item in input_["reasons"]:
            params.append(
                (
                    "Reason",
                    capo_sesv2.types.suppression_list_reason.serialize_json(item),
                )
            )
    if "start_date" in input_:
        params.append(
            (
                "StartDate",
                capo_sesv2._protocol.serialize.fmt_date_time(input_["start_date"]),
            )
        )
    if "end_date" in input_:
        params.append(
            (
                "EndDate",
                capo_sesv2._protocol.serialize.fmt_date_time(input_["end_date"]),
            )
        )
...
async def async_list_suppressed_destinations(
