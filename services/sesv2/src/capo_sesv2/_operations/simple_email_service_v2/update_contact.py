"""Generated from Smithy shape ``com.amazonaws.sesv2#UpdateContact``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.update_contact_response.UpdateContactResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{EmailAddress}", quote(input_["email_address"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.update_contact_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def update_contact(
    ...
    capo_sesv2.types.update_contact_response.UpdateContactResponse, zapros.Response
]:
