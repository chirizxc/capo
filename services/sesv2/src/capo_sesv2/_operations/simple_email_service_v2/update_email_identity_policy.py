"""Generated from Smithy shape ``com.amazonaws.sesv2#UpdateEmailIdentityPolicy``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.update_email_identity_policy_response.UpdateEmailIdentityPolicyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{PolicyName}", quote(input_["policy_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.update_email_identity_policy_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_email_identity_policy(
