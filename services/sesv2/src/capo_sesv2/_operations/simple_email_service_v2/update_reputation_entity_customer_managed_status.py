...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_sesv2.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_sesv2.types.update_reputation_entity_customer_managed_status_response.UpdateReputationEntityCustomerManagedStatusResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(input_["reputation_entity_reference"], safe=""),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.update_reputation_entity_customer_managed_status_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
