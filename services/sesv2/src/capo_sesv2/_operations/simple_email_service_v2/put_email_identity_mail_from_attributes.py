"""Generated from Smithy shape ``com.amazonaws.sesv2#PutEmailIdentityMailFromAttributes``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.put_email_identity_mail_from_attributes_response.PutEmailIdentityMailFromAttributesResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/identities/{EmailIdentity}/mail-from"
    url = url.replace("{EmailIdentity}", quote(input_["email_identity"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.put_email_identity_mail_from_attributes_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
