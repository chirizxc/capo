"""Generated from Smithy shape ``com.amazonaws.sesv2#PutDedicatedIpWarmupAttributes``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.put_dedicated_ip_warmup_attributes_response.PutDedicatedIpWarmupAttributesResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/dedicated-ips/{Ip}/warmup"
    url = url.replace("{Ip}", quote(input_["ip"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.put_dedicated_ip_warmup_attributes_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
