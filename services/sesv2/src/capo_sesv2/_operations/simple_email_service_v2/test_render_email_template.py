"""Generated from Smithy shape ``com.amazonaws.sesv2#TestRenderEmailTemplate``."""
...
def handle_response(
    ...
    out: capo_sesv2.types.test_render_email_template_response.TestRenderEmailTemplateResponse = capo_sesv2.types.test_render_email_template_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/templates/{TemplateName}/render"
    url = url.replace("{TemplateName}", quote(input_["template_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.test_render_email_template_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_test_render_email_template(
