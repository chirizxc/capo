"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#TlsConfigInput``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_apigatewayv2.types.string_with_length_between1_and512


class TlsConfigInput(TypedDict, closed=True):
    server_name_to_verify: NotRequired[
        "capo_apigatewayv2.types.string_with_length_between1_and512.StringWithLengthBetween1And512"
    ]
    """<p>If you specify a server name, API Gateway uses it to verify the hostname on the integration's certificate. The server name is also included in the TLS handshake to support Server Name Indication (SNI) or virtual hosting.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: TlsConfigInput) -> dict:
    out: dict = {}
    if "server_name_to_verify" in value:
        out["serverNameToVerify"] = value["server_name_to_verify"]
    return out


def deserialize_json(data: dict) -> TlsConfigInput:
    out: TlsConfigInput = {}  # type: ignore[typeddict-item]
    if data.get("serverNameToVerify") is not None:
        out["server_name_to_verify"] = data["serverNameToVerify"]
    return out
