...
if TYPE_CHECKING:
    import capo_apigatewayv2.types.__list_of_integration_response
    import capo_apigatewayv2.types.next_token


class GetIntegrationResponsesResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_apigatewayv2.types.__list_of_integration_response.__listOfIntegrationResponse"
    ]
...
def serialize_json(value: GetIntegrationResponsesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            capo_apigatewayv2.types.__list_of_integration_response.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> GetIntegrationResponsesResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = (
            capo_apigatewayv2.types.__list_of_integration_response.deserialize_json(
                data["items"]
            )
        )
