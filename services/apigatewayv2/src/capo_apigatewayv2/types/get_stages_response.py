...
class GetStagesResponse(TypedDict, closed=True):
    items: NotRequired["capo_apigatewayv2.types.__list_of_stage.__listOfStage"]
...
def serialize_json(value: GetStagesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_apigatewayv2.types.__list_of_stage.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> GetStagesResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = capo_apigatewayv2.types.__list_of_stage.deserialize_json(
            data["items"]
        )
