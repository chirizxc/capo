"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#UpdatePortalProductResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_apigatewayv2.types.__string_min0_max1024
    import capo_apigatewayv2.types.__string_min1_max255
    import capo_apigatewayv2.types.__string_min10_max30_pattern_az09
    import capo_apigatewayv2.types.__string_min20_max2048
    import capo_apigatewayv2.types.__timestamp_iso8601
    import capo_apigatewayv2.types.display_order
    import capo_apigatewayv2.types.tags


class UpdatePortalProductResponse(TypedDict, closed=True):
    description: NotRequired[
        "capo_apigatewayv2.types.__string_min0_max1024.__stringMin0Max1024"
    ]
    """<p>The description of the portal product.</p>"""
    display_name: NotRequired[
        "capo_apigatewayv2.types.__string_min1_max255.__stringMin1Max255"
    ]
    """<p>The display name of a portal product.</p>"""
    display_order: NotRequired["capo_apigatewayv2.types.display_order.DisplayOrder"]
    """<p>The display order that the portal products will appear in a portal.</p>"""
    last_modified: NotRequired[
        "capo_apigatewayv2.types.__timestamp_iso8601.__timestampIso8601"
    ]
    """<p>The timestamp when the portal product was last modified.</p>"""
    portal_product_arn: NotRequired[
        "capo_apigatewayv2.types.__string_min20_max2048.__stringMin20Max2048"
    ]
    """<p>The ARN of the portal product.</p>"""
    portal_product_id: NotRequired[
        "capo_apigatewayv2.types.__string_min10_max30_pattern_az09.__stringMin10Max30PatternAZ09"
    ]
    """<p>The portal product identifier.</p>"""
    tags: NotRequired["capo_apigatewayv2.types.tags.Tags"]
    """<p>The collection of tags. Each tag element is associated with a given resource.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: UpdatePortalProductResponse) -> dict:
    out: dict = {}
    if "description" in value:
        out["description"] = value["description"]
    if "display_name" in value:
        out["displayName"] = value["display_name"]
    if "display_order" in value:
        import capo_apigatewayv2.types.display_order

        out["displayOrder"] = capo_apigatewayv2.types.display_order.serialize_json(
            value["display_order"]
        )
    if "last_modified" in value:
        import capo_apigatewayv2.types.__timestamp_iso8601

        out["lastModified"] = (
            capo_apigatewayv2.types.__timestamp_iso8601.serialize_json(
                value["last_modified"]
            )
        )
    if "portal_product_arn" in value:
        out["portalProductArn"] = value["portal_product_arn"]
    if "portal_product_id" in value:
        out["portalProductId"] = value["portal_product_id"]
    if "tags" in value:
        import capo_apigatewayv2.types.tags

        out["tags"] = capo_apigatewayv2.types.tags.serialize_json(value["tags"])
    return out


def deserialize_json(data: dict) -> UpdatePortalProductResponse:
    out: UpdatePortalProductResponse = {}  # type: ignore[typeddict-item]
    if data.get("description") is not None:
        out["description"] = data["description"]
    if data.get("displayName") is not None:
        out["display_name"] = data["displayName"]
    if data.get("displayOrder") is not None:
        import capo_apigatewayv2.types.display_order

        out["display_order"] = capo_apigatewayv2.types.display_order.deserialize_json(
            data["displayOrder"]
        )
    if data.get("lastModified") is not None:
        import capo_apigatewayv2.types.__timestamp_iso8601

        out["last_modified"] = (
            capo_apigatewayv2.types.__timestamp_iso8601.deserialize_json(
                data["lastModified"]
            )
        )
    if data.get("portalProductArn") is not None:
        out["portal_product_arn"] = data["portalProductArn"]
    if data.get("portalProductId") is not None:
        out["portal_product_id"] = data["portalProductId"]
    if data.get("tags") is not None:
        import capo_apigatewayv2.types.tags

        out["tags"] = capo_apigatewayv2.types.tags.deserialize_json(data["tags"])
    return out
