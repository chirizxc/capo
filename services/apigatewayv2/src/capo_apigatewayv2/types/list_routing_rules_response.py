...
class ListRoutingRulesResponse(TypedDict, closed=True):
    ...
    routing_rules: NotRequired[
        "capo_apigatewayv2.types.__list_of_routing_rule.__listOfRoutingRule"
    ]
...
def serialize_json(value: ListRoutingRulesResponse) -> dict:
    ...
    if "routing_rules" in value:
        ...
        out["routingRules"] = (
            capo_apigatewayv2.types.__list_of_routing_rule.serialize_json(
                value["routing_rules"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListRoutingRulesResponse:
    ...
    if data.get("routingRules") is not None:
        ...
        out["routing_rules"] = (
            capo_apigatewayv2.types.__list_of_routing_rule.deserialize_json(
                data["routingRules"]
            )
        )
    return out
