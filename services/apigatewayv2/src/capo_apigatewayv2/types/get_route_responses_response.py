...
if TYPE_CHECKING:
    import capo_apigatewayv2.types.__list_of_route_response
    import capo_apigatewayv2.types.next_token


class GetRouteResponsesResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_apigatewayv2.types.__list_of_route_response.__listOfRouteResponse"
    ]
...
def serialize_json(value: GetRouteResponsesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_apigatewayv2.types.__list_of_route_response.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> GetRouteResponsesResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = (
            capo_apigatewayv2.types.__list_of_route_response.deserialize_json(
                data["items"]
            )
        )
