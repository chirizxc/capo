"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#RouteModels``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_apigatewayv2.types.__string
    import capo_apigatewayv2.types.string_with_length_between1_and128

RouteModels: TypeAlias = dict[
    "capo_apigatewayv2.types.__string.__string",
    "capo_apigatewayv2.types.string_with_length_between1_and128.StringWithLengthBetween1And128",
]


# --- restJson1 ser/de ---
def serialize_json(input_to_serialize: RouteModels) -> dict:
    out: dict = {}
    for key, value in input_to_serialize.items():
        out[key] = value
    return out


def deserialize_json(data: dict) -> RouteModels:
    out: RouteModels = {}
    for key, value in data.items():
        if value is None:
            continue
        out[key] = value
    return out
