"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#DisplayContentOverrides``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_apigatewayv2.types.__string_min1_max255
    import capo_apigatewayv2.types.__string_min1_max1024
    import capo_apigatewayv2.types.__string_min1_max32768


class DisplayContentOverrides(TypedDict, closed=True):
    body: NotRequired[
        "capo_apigatewayv2.types.__string_min1_max32768.__stringMin1Max32768"
    ]
    """<p>By default, this is the documentation of your REST API from API Gateway. You can provide custom documentation to override this value.</p>"""
    endpoint: NotRequired[
        "capo_apigatewayv2.types.__string_min1_max1024.__stringMin1Max1024"
    ]
    """<p>The URL for your REST API. By default, API Gateway uses the default execute API endpoint. You can provide a custom domain to override this value.</p>"""
    operation_name: NotRequired[
        "capo_apigatewayv2.types.__string_min1_max255.__stringMin1Max255"
    ]
    """<p>The operation name of the product REST endpoint.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: DisplayContentOverrides) -> dict:
    out: dict = {}
    if "body" in value:
        out["body"] = value["body"]
    if "endpoint" in value:
        out["endpoint"] = value["endpoint"]
    if "operation_name" in value:
        out["operationName"] = value["operation_name"]
    return out


def deserialize_json(data: dict) -> DisplayContentOverrides:
    out: DisplayContentOverrides = {}  # type: ignore[typeddict-item]
    if data.get("body") is not None:
        out["body"] = data["body"]
    if data.get("endpoint") is not None:
        out["endpoint"] = data["endpoint"]
    if data.get("operationName") is not None:
        out["operation_name"] = data["operationName"]
    return out
