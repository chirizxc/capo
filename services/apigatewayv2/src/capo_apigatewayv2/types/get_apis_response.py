"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#GetApisResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_apigatewayv2.types.__list_of_api
    import capo_apigatewayv2.types.next_token


class GetApisResponse(TypedDict, closed=True):
    items: NotRequired["capo_apigatewayv2.types.__list_of_api.__listOfApi"]
    """<p>The elements from this collection.</p>"""
    next_token: NotRequired["capo_apigatewayv2.types.next_token.NextToken"]
    """<p>The next page of elements from this collection. Not valid for the last element of the collection.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: GetApisResponse) -> dict:
    out: dict = {}
    if "items" in value:
        import capo_apigatewayv2.types.__list_of_api

        out["items"] = capo_apigatewayv2.types.__list_of_api.serialize_json(
            value["items"]
        )
    if "next_token" in value:
        out["nextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> GetApisResponse:
    out: GetApisResponse = {}  # type: ignore[typeddict-item]
    if data.get("items") is not None:
        import capo_apigatewayv2.types.__list_of_api

        out["items"] = capo_apigatewayv2.types.__list_of_api.deserialize_json(
            data["items"]
        )
    if data.get("nextToken") is not None:
        out["next_token"] = data["nextToken"]
    return out
