...
if TYPE_CHECKING:
    import capo_apigatewayv2.types.__list_of_domain_name
    import capo_apigatewayv2.types.next_token


class GetDomainNamesResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_apigatewayv2.types.__list_of_domain_name.__listOfDomainName"
    ]
...
def serialize_json(value: GetDomainNamesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_apigatewayv2.types.__list_of_domain_name.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> GetDomainNamesResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = capo_apigatewayv2.types.__list_of_domain_name.deserialize_json(
            data["items"]
        )
