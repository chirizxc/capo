"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#UpdateIntegrationResponse``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.update_integration_response_response.UpdateIntegrationResponseResponse = capo_apigatewayv2.types.update_integration_response_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{IntegrationResponseId}", quote(input_["integration_response_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.update_integration_response_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
