"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#UpdateProductPage``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.update_product_page_response.UpdateProductPageResponse = capo_apigatewayv2.types.update_product_page_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{ProductPageId}", quote(input_["product_page_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.update_product_page_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_product_page(
