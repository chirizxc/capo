"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#CreateRouteResponse``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.create_route_response_response.CreateRouteResponseResponse = capo_apigatewayv2.types.create_route_response_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{RouteId}", quote(input_["route_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.create_route_response_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_route_response(
