...
def build_request(
    ...
    url = url.replace("{Specification}", quote(input_["specification"], safe=""))
    params: list[tuple[str, str]] = []
    if "export_version" in input_:
        params.append(("exportVersion", input_["export_version"]))
    if "include_extensions" in input_:
        params.append(
            ("includeExtensions", "true" if input_["include_extensions"] else "false")
        )
    if "output_type" in input_:
        params.append(("outputType", input_["output_type"]))
    if "stage_name" in input_:
        params.append(("stageName", input_["stage_name"]))
