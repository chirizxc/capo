...
def handle_response(
    ...
    out: capo_apigatewayv2.types.get_product_rest_endpoint_page_response.GetProductRestEndpointPageResponse = capo_apigatewayv2.types.get_product_rest_endpoint_page_response.deserialize_json(
...
def build_request(
    ...
    if "include_raw_display_content" in input_:
        params.append(
            ("includeRawDisplayContent", input_["include_raw_display_content"])
        )
    if "resource_owner_account_id" in input_:
        params.append(("resourceOwnerAccountId", input_["resource_owner_account_id"]))
