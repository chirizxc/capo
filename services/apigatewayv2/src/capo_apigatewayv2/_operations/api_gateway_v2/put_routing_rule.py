...
def handle_response(
    ...
    out: capo_apigatewayv2.types.put_routing_rule_response.PutRoutingRuleResponse = (
        capo_apigatewayv2.types.put_routing_rule_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{RoutingRuleId}", quote(input_["routing_rule_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "domain_name_id" in input_:
        params.append(("domainNameId", input_["domain_name_id"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.put_routing_rule_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
