"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#ReimportApi``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.reimport_api_response.ReimportApiResponse = (
        capo_apigatewayv2.types.reimport_api_response.deserialize_json(
...
def build_request(
    ...
    if "basepath" in input_:
        params.append(("basepath", input_["basepath"]))
    if "fail_on_warnings" in input_:
        params.append(
            ("failOnWarnings", "true" if input_["fail_on_warnings"] else "false")
        )
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.reimport_api_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def reimport_api(
    ...
    capo_apigatewayv2.types.reimport_api_response.ReimportApiResponse, zapros.Response
]:
