"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#UpdateProductRestEndpointPage``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.update_product_rest_endpoint_page_response.UpdateProductRestEndpointPageResponse = capo_apigatewayv2.types.update_product_rest_endpoint_page_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{PortalProductId}", quote(input_["portal_product_id"], safe=""))
    url = url.replace(
        "{ProductRestEndpointPageId}",
        quote(input_["product_rest_endpoint_page_id"], safe=""),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.update_product_rest_endpoint_page_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_product_rest_endpoint_page(
