...
import capo_emr_containers._protocol.eventstream
import capo_emr_containers.errors.internal_server_exception
import capo_emr_containers.errors.request_throttled_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "RequestThrottledException":
            raise capo_emr_containers.errors.request_throttled_exception.RequestThrottledException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_emr_containers.types.get_managed_endpoint_session_credentials_response.GetManagedEndpointSessionCredentialsResponse = capo_emr_containers.types.get_managed_endpoint_session_credentials_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_emr_containers._auth._sigv4.build_sigv4_auth_scheme(
                "emr-containers", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace(
        "{endpointIdentifier}", quote(input_["endpoint_identifier"], safe="")
    )
    url = url.replace(
        "{virtualClusterIdentifier}",
        quote(input_["virtual_cluster_identifier"], safe=""),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_emr_containers.types.get_managed_endpoint_session_credentials_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
