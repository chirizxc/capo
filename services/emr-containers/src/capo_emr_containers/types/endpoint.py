"""Generated from Smithy shape ``com.amazonaws.emrcontainers#Endpoint``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_emr_containers.types.acm_cert_arn
    import capo_emr_containers.types.certificate
    import capo_emr_containers.types.configuration_overrides
    import capo_emr_containers.types.date
    import capo_emr_containers.types.endpoint_arn
    import capo_emr_containers.types.endpoint_state
    import capo_emr_containers.types.endpoint_type
    import capo_emr_containers.types.failure_reason
    import capo_emr_containers.types.iam_role_arn
    import capo_emr_containers.types.release_label
    import capo_emr_containers.types.resource_id_string
    import capo_emr_containers.types.resource_name_string
    import capo_emr_containers.types.string256
    import capo_emr_containers.types.subnet_ids
    import capo_emr_containers.types.tag_map
    import capo_emr_containers.types.uri_string


class Endpoint(TypedDict, closed=True):
    id: NotRequired["capo_emr_containers.types.resource_id_string.ResourceIdString"]
    """<p>The ID of the endpoint.</p>"""
    name: NotRequired[
        "capo_emr_containers.types.resource_name_string.ResourceNameString"
    ]
    """<p>The name of the endpoint.</p>"""
    arn: NotRequired["capo_emr_containers.types.endpoint_arn.EndpointArn"]
    """<p>The ARN of the endpoint.</p>"""
    virtual_cluster_id: NotRequired[
        "capo_emr_containers.types.resource_id_string.ResourceIdString"
    ]
    """<p>The ID of the endpoint's virtual cluster.</p>"""
    type: NotRequired["capo_emr_containers.types.endpoint_type.EndpointType"]
    """<p>The type of the endpoint.</p>"""
    state: NotRequired["capo_emr_containers.types.endpoint_state.EndpointState"]
    """<p>The state of the endpoint.</p>"""
    release_label: NotRequired["capo_emr_containers.types.release_label.ReleaseLabel"]
    """<p>The EMR release version to be used for the endpoint.</p>"""
    execution_role_arn: NotRequired["capo_emr_containers.types.iam_role_arn.IAMRoleArn"]
    """<p>The execution role ARN of the endpoint.</p>"""
    certificate_arn: NotRequired["capo_emr_containers.types.acm_cert_arn.ACMCertArn"]
    """<p>The certificate ARN of the endpoint. This field is under deprecation and will be removed in future.</p>"""
    certificate_authority: NotRequired[
        "capo_emr_containers.types.certificate.Certificate"
    ]
    """<p>The certificate generated by emr control plane on customer behalf to secure the managed endpoint.</p>"""
    configuration_overrides: NotRequired[
        "capo_emr_containers.types.configuration_overrides.ConfigurationOverrides"
    ]
    """<p>The configuration settings that are used to override existing configurations for endpoints.</p>"""
    server_url: NotRequired["capo_emr_containers.types.uri_string.UriString"]
    """<p>The server URL of the endpoint.</p>"""
    created_at: NotRequired["capo_emr_containers.types.date.Date"]
    """<p>The date and time when the endpoint was created.</p>"""
    security_group: NotRequired["capo_emr_containers.types.string256.String256"]
    """<p>The security group configuration of the endpoint. </p>"""
    subnet_ids: NotRequired["capo_emr_containers.types.subnet_ids.SubnetIds"]
    """<p>The subnet IDs of the endpoint. </p>"""
    state_details: NotRequired["capo_emr_containers.types.string256.String256"]
    """<p> Additional details of the endpoint state. </p>"""
    failure_reason: NotRequired[
        "capo_emr_containers.types.failure_reason.FailureReason"
    ]
    """<p> The reasons why the endpoint has failed. </p>"""
    tags: NotRequired["capo_emr_containers.types.tag_map.TagMap"]
    """<p>The tags of the endpoint. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: Endpoint) -> dict:
    out: dict = {}
    if "id" in value:
        out["id"] = value["id"]
    if "name" in value:
        out["name"] = value["name"]
    if "arn" in value:
        out["arn"] = value["arn"]
    if "virtual_cluster_id" in value:
        out["virtualClusterId"] = value["virtual_cluster_id"]
    if "type" in value:
        out["type"] = value["type"]
    if "state" in value:
        import capo_emr_containers.types.endpoint_state

        out["state"] = capo_emr_containers.types.endpoint_state.serialize_json(
            value["state"]
        )
    if "release_label" in value:
        out["releaseLabel"] = value["release_label"]
    if "execution_role_arn" in value:
        out["executionRoleArn"] = value["execution_role_arn"]
    if "certificate_arn" in value:
        out["certificateArn"] = value["certificate_arn"]
    if "certificate_authority" in value:
        import capo_emr_containers.types.certificate

        out["certificateAuthority"] = (
            capo_emr_containers.types.certificate.serialize_json(
                value["certificate_authority"]
            )
        )
    if "configuration_overrides" in value:
        import capo_emr_containers.types.configuration_overrides

        out["configurationOverrides"] = (
            capo_emr_containers.types.configuration_overrides.serialize_json(
                value["configuration_overrides"]
            )
        )
    if "server_url" in value:
        out["serverUrl"] = value["server_url"]
    if "created_at" in value:
        import capo_emr_containers.types.date

        out["createdAt"] = capo_emr_containers.types.date.serialize_json(
            value["created_at"]
        )
    if "security_group" in value:
        out["securityGroup"] = value["security_group"]
    if "subnet_ids" in value:
        import capo_emr_containers.types.subnet_ids

        out["subnetIds"] = capo_emr_containers.types.subnet_ids.serialize_json(
            value["subnet_ids"]
        )
    if "state_details" in value:
        out["stateDetails"] = value["state_details"]
    if "failure_reason" in value:
        import capo_emr_containers.types.failure_reason

        out["failureReason"] = capo_emr_containers.types.failure_reason.serialize_json(
            value["failure_reason"]
        )
    if "tags" in value:
        import capo_emr_containers.types.tag_map

        out["tags"] = capo_emr_containers.types.tag_map.serialize_json(value["tags"])
    return out


def deserialize_json(data: dict) -> Endpoint:
    out: Endpoint = {}  # type: ignore[typeddict-item]
    if data.get("id") is not None:
        out["id"] = data["id"]
    if data.get("name") is not None:
        out["name"] = data["name"]
    if data.get("arn") is not None:
        out["arn"] = data["arn"]
    if data.get("virtualClusterId") is not None:
        out["virtual_cluster_id"] = data["virtualClusterId"]
    if data.get("type") is not None:
        out["type"] = data["type"]
    if data.get("state") is not None:
        import capo_emr_containers.types.endpoint_state

        out["state"] = capo_emr_containers.types.endpoint_state.deserialize_json(
            data["state"]
        )
    if data.get("releaseLabel") is not None:
        out["release_label"] = data["releaseLabel"]
    if data.get("executionRoleArn") is not None:
        out["execution_role_arn"] = data["executionRoleArn"]
    if data.get("certificateArn") is not None:
        out["certificate_arn"] = data["certificateArn"]
    if data.get("certificateAuthority") is not None:
        import capo_emr_containers.types.certificate

        out["certificate_authority"] = (
            capo_emr_containers.types.certificate.deserialize_json(
                data["certificateAuthority"]
            )
        )
    if data.get("configurationOverrides") is not None:
        import capo_emr_containers.types.configuration_overrides

        out["configuration_overrides"] = (
            capo_emr_containers.types.configuration_overrides.deserialize_json(
                data["configurationOverrides"]
            )
        )
    if data.get("serverUrl") is not None:
        out["server_url"] = data["serverUrl"]
    if data.get("createdAt") is not None:
        import capo_emr_containers.types.date

        out["created_at"] = capo_emr_containers.types.date.deserialize_json(
            data["createdAt"]
        )
    if data.get("securityGroup") is not None:
        out["security_group"] = data["securityGroup"]
    if data.get("subnetIds") is not None:
        import capo_emr_containers.types.subnet_ids

        out["subnet_ids"] = capo_emr_containers.types.subnet_ids.deserialize_json(
            data["subnetIds"]
        )
    if data.get("stateDetails") is not None:
        out["state_details"] = data["stateDetails"]
    if data.get("failureReason") is not None:
        import capo_emr_containers.types.failure_reason

        out["failure_reason"] = (
            capo_emr_containers.types.failure_reason.deserialize_json(
                data["failureReason"]
            )
        )
    if data.get("tags") is not None:
        import capo_emr_containers.types.tag_map

        out["tags"] = capo_emr_containers.types.tag_map.deserialize_json(data["tags"])
    return out
