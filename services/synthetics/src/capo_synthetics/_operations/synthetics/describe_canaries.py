"""Generated from Smithy shape ``com.amazonaws.synthetics#DescribeCanaries``."""
...
def handle_response(
    ...
    out: capo_synthetics.types.describe_canaries_response.DescribeCanariesResponse = (
        capo_synthetics.types.describe_canaries_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/canaries"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_synthetics.types.describe_canaries_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_describe_canaries(
