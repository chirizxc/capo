"""Generated from Smithy shape ``com.amazonaws.synthetics#ListAssociatedGroups``."""
...
import capo_synthetics._protocol.eventstream
...
def handle_response(
    ...
    out: capo_synthetics.types.list_associated_groups_response.ListAssociatedGroupsResponse = capo_synthetics.types.list_associated_groups_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_synthetics._auth._sigv4.build_sigv4_auth_scheme(
                "synthetics", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/resource/{ResourceArn}/groups"
    url = url.replace("{ResourceArn}", quote(input_["resource_arn"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_synthetics.types.list_associated_groups_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_associated_groups(
