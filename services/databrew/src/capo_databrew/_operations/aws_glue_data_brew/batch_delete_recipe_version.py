"""Generated from Smithy shape ``com.amazonaws.databrew#BatchDeleteRecipeVersion``."""
...
def handle_response(
    ...
    out: capo_databrew.types.batch_delete_recipe_version_response.BatchDeleteRecipeVersionResponse = capo_databrew.types.batch_delete_recipe_version_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/recipes/{Name}/batchDeleteRecipeVersion"
    url = url.replace("{Name}", quote(input_["name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_databrew.types.batch_delete_recipe_version_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_batch_delete_recipe_version(
