...
class ColumnStatisticsConfiguration(TypedDict, closed=True):
    ...
    statistics: "capo_databrew.types.statistics_configuration.StatisticsConfiguration"
...
def serialize_json(value: ColumnStatisticsConfiguration) -> dict:
    ...
    if "selectors" in value:
        ...
        out["Selectors"] = capo_databrew.types.column_selector_list.serialize_json(
            value["selectors"]
        )
    import capo_databrew.types.statistics_configuration

    out["Statistics"] = capo_databrew.types.statistics_configuration.serialize_json(
        value["statistics"]
    )
    return out
