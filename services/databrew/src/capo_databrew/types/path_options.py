...
class PathOptions(TypedDict, closed=True):
    last_modified_date_condition: NotRequired[
        "capo_databrew.types.filter_expression.FilterExpression"
    ]
    ...
    files_limit: NotRequired["capo_databrew.types.files_limit.FilesLimit"]
    """<p>If provided, this structure imposes a limit on a number of files that should be selected.</p>"""
    parameters: NotRequired["capo_databrew.types.path_parameters_map.PathParametersMap"]
...
def serialize_json(value: PathOptions) -> dict:
    ...
    if "last_modified_date_condition" in value:
        ...
        out["LastModifiedDateCondition"] = (
            capo_databrew.types.filter_expression.serialize_json(
    ...
    if "files_limit" in value:
        ...
        out["FilesLimit"] = capo_databrew.types.files_limit.serialize_json(
            value["files_limit"]
        )
    if "parameters" in value:
        import capo_databrew.types.path_parameters_map

        out["Parameters"] = capo_databrew.types.path_parameters_map.serialize_json(
            value["parameters"]
        )
    return out
...
def deserialize_json(data: dict) -> PathOptions:
    ...
    if data.get("LastModifiedDateCondition") is not None:
        ...
        out["last_modified_date_condition"] = (
            capo_databrew.types.filter_expression.deserialize_json(
