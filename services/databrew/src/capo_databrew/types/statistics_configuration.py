...
class StatisticsConfiguration(TypedDict, closed=True):
    included_statistics: NotRequired["capo_databrew.types.statistic_list.StatisticList"]
    ...
    overrides: NotRequired[
        "capo_databrew.types.statistic_override_list.StatisticOverrideList"
    ]
...
def serialize_json(value: StatisticsConfiguration) -> dict:
    ...
    if "included_statistics" in value:
        ...
        out["IncludedStatistics"] = capo_databrew.types.statistic_list.serialize_json(
            value["included_statistics"]
        )
    if "overrides" in value:
        import capo_databrew.types.statistic_override_list

        out["Overrides"] = capo_databrew.types.statistic_override_list.serialize_json(
            value["overrides"]
        )
    return out
...
def deserialize_json(data: dict) -> StatisticsConfiguration:
    ...
    if data.get("IncludedStatistics") is not None:
        ...
        out["included_statistics"] = (
            capo_databrew.types.statistic_list.deserialize_json(
