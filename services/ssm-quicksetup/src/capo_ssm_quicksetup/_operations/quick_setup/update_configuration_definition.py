"""Generated from Smithy shape ``com.amazonaws.ssmquicksetup#UpdateConfigurationDefinition``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/configurationDefinition/{ManagerArn}/{Id}"
    url = url.replace("{ManagerArn}", quote(input_["manager_arn"], safe=""))
    url = url.replace("{Id}", quote(input_["id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_quicksetup.types.update_configuration_definition_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_configuration_definition(
