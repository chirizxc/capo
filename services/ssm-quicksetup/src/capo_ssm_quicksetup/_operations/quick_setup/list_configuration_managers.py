"""Generated from Smithy shape ``com.amazonaws.ssmquicksetup#ListConfigurationManagers``."""
...
import capo_ssm_quicksetup._protocol.eventstream
...
import capo_ssm_quicksetup.types.configuration_manager_list
...
def handle_response(
    ...
    out: capo_ssm_quicksetup.types.list_configuration_managers_output.ListConfigurationManagersOutput = capo_ssm_quicksetup.types.list_configuration_managers_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_ssm_quicksetup._auth._sigv4.build_sigv4_auth_scheme(
                "ssm-quicksetup", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listConfigurationManagers"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_quicksetup.types.list_configuration_managers_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_configuration_managers(
