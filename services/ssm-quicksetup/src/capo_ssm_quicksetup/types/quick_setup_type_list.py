...
if TYPE_CHECKING:
    import capo_ssm_quicksetup.types.quick_setup_type_output

QuickSetupTypeList: TypeAlias = list[
    "capo_ssm_quicksetup.types.quick_setup_type_output.QuickSetupTypeOutput"
]
...
def serialize_json(value: QuickSetupTypeList) -> list:
    ...
    for item in value:
        out.append(
            capo_ssm_quicksetup.types.quick_setup_type_output.serialize_json(item)
        )
    return out
