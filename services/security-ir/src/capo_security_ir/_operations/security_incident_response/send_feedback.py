...
def handle_response(
    ...
    out: capo_security_ir.types.send_feedback_response.SendFeedbackResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{resultId}", quote(input_["result_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_security_ir.types.send_feedback_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def send_feedback(
    ...
    capo_security_ir.types.send_feedback_response.SendFeedbackResponse, zapros.Response
]:
