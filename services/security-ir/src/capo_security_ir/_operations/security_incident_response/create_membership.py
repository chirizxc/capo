"""Generated from Smithy shape ``com.amazonaws.securityir#CreateMembership``."""
...
def handle_response(
    ...
    out: capo_security_ir.types.create_membership_response.CreateMembershipResponse = (
        capo_security_ir.types.create_membership_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/membership"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_security_ir.types.create_membership_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
