"""Generated from Smithy shape ``com.amazonaws.securityir#UpdateMembership``."""
...
def handle_response(
    ...
    out: capo_security_ir.types.update_membership_response.UpdateMembershipResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/membership/{membershipId}/update-membership"
    url = url.replace("{membershipId}", quote(input_["membership_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_security_ir.types.update_membership_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
