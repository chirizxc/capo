"""Generated from Smithy shape ``com.amazonaws.securityir#UpdateCaseStatus``."""
...
def handle_response(
    ...
    out: capo_security_ir.types.update_case_status_response.UpdateCaseStatusResponse = (
        capo_security_ir.types.update_case_status_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/cases/{caseId}/update-case-status"
    url = url.replace("{caseId}", quote(input_["case_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_security_ir.types.update_case_status_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_case_status(
