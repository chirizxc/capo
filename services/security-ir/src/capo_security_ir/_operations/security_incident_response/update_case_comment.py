"""Generated from Smithy shape ``com.amazonaws.securityir#UpdateCaseComment``."""
...
def handle_response(
    ...
    out: capo_security_ir.types.update_case_comment_response.UpdateCaseCommentResponse = capo_security_ir.types.update_case_comment_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v1/cases/{caseId}/update-case-comment/{commentId}"
    )
    url = url.replace("{caseId}", quote(input_["case_id"], safe=""))
    url = url.replace("{commentId}", quote(input_["comment_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_security_ir.types.update_case_comment_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_case_comment(
