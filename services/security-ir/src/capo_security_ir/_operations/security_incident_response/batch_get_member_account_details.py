"""Generated from Smithy shape ``com.amazonaws.securityir#BatchGetMemberAccountDetails``."""
...
def handle_response(
    ...
    out: capo_security_ir.types.batch_get_member_account_details_response.BatchGetMemberAccountDetailsResponse = capo_security_ir.types.batch_get_member_account_details_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v1/membership/{membershipId}/batch-member-details"
    )
    url = url.replace("{membershipId}", quote(input_["membership_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_security_ir.types.batch_get_member_account_details_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_get_member_account_details(
