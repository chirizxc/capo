...
if TYPE_CHECKING:
    import capo_odb.types.cross_region_s3_restore_sources_access

CrossRegionS3RestoreSourcesAccessList: TypeAlias = list[
    "capo_odb.types.cross_region_s3_restore_sources_access.CrossRegionS3RestoreSourcesAccess"
]
...
def serialize_aws_json_1_0(value: CrossRegionS3RestoreSourcesAccessList) -> list:
    ...
    for item in value:
        out.append(
            capo_odb.types.cross_region_s3_restore_sources_access.serialize_aws_json_1_0(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_0(data: list) -> CrossRegionS3RestoreSourcesAccessList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_odb.types.cross_region_s3_restore_sources_access.deserialize_aws_json_1_0(
                item
            )
        )
    return out
