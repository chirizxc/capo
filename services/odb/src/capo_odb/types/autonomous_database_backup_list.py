...
AutonomousDatabaseBackupList: TypeAlias = list[
    "capo_odb.types.autonomous_database_backup_summary.AutonomousDatabaseBackupSummary"
]
...
def serialize_aws_json_1_0(value: AutonomousDatabaseBackupList) -> list:
    ...
    for item in value:
        out.append(
            capo_odb.types.autonomous_database_backup_summary.serialize_aws_json_1_0(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_0(data: list) -> AutonomousDatabaseBackupList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_odb.types.autonomous_database_backup_summary.deserialize_aws_json_1_0(
                item
            )
        )
    return out
