...
class MaintenanceWindow(TypedDict, closed=True):
    ...
    days_of_week: NotRequired["capo_odb.types.days_of_week.DaysOfWeek"]
    """<p>The days of the week when maintenance can be performed.</p>"""
    hours_of_day: NotRequired["capo_odb.types.hours_of_day.HoursOfDay"]
    ...
    months: NotRequired["capo_odb.types.months.Months"]
    """<p>The months when maintenance can be performed.</p>"""
    patching_mode: NotRequired["capo_odb.types.patching_mode_type.PatchingModeType"]
    """<p>The patching mode for the maintenance window.</p>"""
    preference: NotRequired["capo_odb.types.preference_type.PreferenceType"]
    ...
    weeks_of_month: NotRequired["capo_odb.types.weeks_of_month.WeeksOfMonth"]
...
def serialize_aws_json_1_0(value: MaintenanceWindow) -> dict:
    ...
    if "days_of_week" in value:
        ...
        out["daysOfWeek"] = capo_odb.types.days_of_week.serialize_aws_json_1_0(
            value["days_of_week"]
        )
    if "hours_of_day" in value:
        import capo_odb.types.hours_of_day

        out["hoursOfDay"] = capo_odb.types.hours_of_day.serialize_aws_json_1_0(
            value["hours_of_day"]
        )
    ...
    if "months" in value:
        ...
        out["months"] = capo_odb.types.months.serialize_aws_json_1_0(value["months"])
    if "patching_mode" in value:
        import capo_odb.types.patching_mode_type

        out["patchingMode"] = capo_odb.types.patching_mode_type.serialize_aws_json_1_0(
            value["patching_mode"]
        )
    if "preference" in value:
        import capo_odb.types.preference_type

        out["preference"] = capo_odb.types.preference_type.serialize_aws_json_1_0(
            value["preference"]
        )
    if "skip_ru" in value:
    ...
    if "weeks_of_month" in value:
        ...
        out["weeksOfMonth"] = capo_odb.types.weeks_of_month.serialize_aws_json_1_0(
...
def deserialize_aws_json_1_0(data: dict) -> MaintenanceWindow:
    ...
    if data.get("patchingMode") is not None:
        ...
        out["patching_mode"] = (
            capo_odb.types.patching_mode_type.deserialize_aws_json_1_0(
                data["patchingMode"]
            )
        )
