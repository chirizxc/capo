"""Generated from Smithy shape ``com.amazonaws.odb#ListAutonomousDatabaseCharacterSets``."""
...
import capo_odb._protocol.eventstream
...
def handle_response(
    ...
    out: capo_odb.types.list_autonomous_database_character_sets_output.ListAutonomousDatabaseCharacterSetsOutput = capo_odb.types.list_autonomous_database_character_sets_output.deserialize_aws_json_1_0(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_odb._auth._sigv4.build_sigv4_auth_scheme(
                "odb", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "Odb.ListAutonomousDatabaseCharacterSets"
    body: bytes | None = json.dumps(
        capo_odb.types.list_autonomous_database_character_sets_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_autonomous_database_character_sets(
