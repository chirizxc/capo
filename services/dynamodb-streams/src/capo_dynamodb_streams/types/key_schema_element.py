...
if TYPE_CHECKING:
    import capo_dynamodb_streams.types.key_schema_attribute_name
    import capo_dynamodb_streams.types.key_type


class KeySchemaElement(TypedDict, closed=True):
    attribute_name: (
        "capo_dynamodb_streams.types.key_schema_attribute_name.KeySchemaAttributeName"
    )
    """<p>The name of a key attribute.</p>"""
    key_type: "capo_dynamodb_streams.types.key_type.KeyType"
...
def serialize_aws_json_1_0(value: KeySchemaElement) -> dict:
    ...
    out["KeyType"] = capo_dynamodb_streams.types.key_type.serialize_aws_json_1_0(
        value["key_type"]
    )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> KeySchemaElement:
    ...
    if data.get("KeyType") is not None:
        ...
        out["key_type"] = capo_dynamodb_streams.types.key_type.deserialize_aws_json_1_0(
            data["KeyType"]
        )
    else:
