...
class GetRecordsOutput(TypedDict, closed=True):
    records: NotRequired["capo_dynamodb_streams.types.record_list.RecordList"]
...
def serialize_aws_json_1_0(value: GetRecordsOutput) -> dict:
    ...
    if "records" in value:
        ...
        out["Records"] = capo_dynamodb_streams.types.record_list.serialize_aws_json_1_0(
            value["records"]
        )
...
def deserialize_aws_json_1_0(data: dict) -> GetRecordsOutput:
    ...
    if data.get("Records") is not None:
        ...
        out["records"] = (
            capo_dynamodb_streams.types.record_list.deserialize_aws_json_1_0(
                data["Records"]
            )
        )
