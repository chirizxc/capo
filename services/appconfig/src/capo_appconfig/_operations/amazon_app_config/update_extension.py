"""Generated from Smithy shape ``com.amazonaws.appconfig#UpdateExtension``."""
...
def build_request(
    ...
    url = url.replace(
        "{ExtensionIdentifier}", quote(input_["extension_identifier"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_appconfig.types.update_extension_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_extension(
