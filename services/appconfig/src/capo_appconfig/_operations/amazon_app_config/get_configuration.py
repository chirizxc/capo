...
def handle_response(
    ...
    out: capo_appconfig.types.configuration.Configuration = {
        "content": b"".join(response.iter_raw())
    }  # type: ignore[typeddict-item]
    if "Configuration-Version" in response.headers:
        out["configuration_version"] = response.headers["Configuration-Version"]
...
def build_request(
    ...
    url = url.replace("{Environment}", quote(input_["environment"], safe=""))
    url = url.replace("{Configuration}", quote(input_["configuration"], safe=""))
    params: list[tuple[str, str]] = []
    if "client_id" in input_:
        params.append(("client_id", input_["client_id"]))
    if "client_configuration_version" in input_:
        params.append(
            ("client_configuration_version", input_["client_configuration_version"])
        )
...
def get_configuration(
    ...
    input_: capo_appconfig.types.get_configuration_request.GetConfigurationRequest,
) -> tuple[capo_appconfig.types.configuration.Configuration, zapros.Response]:
