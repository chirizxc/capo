...
if TYPE_CHECKING:
    import capo_appconfig.types.configuration_profile_summary_list
    import capo_appconfig.types.next_token


class ConfigurationProfiles(TypedDict, closed=True):
    items: NotRequired[
        "capo_appconfig.types.configuration_profile_summary_list.ConfigurationProfileSummaryList"
    ]
...
def serialize_json(value: ConfigurationProfiles) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = (
            capo_appconfig.types.configuration_profile_summary_list.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ConfigurationProfiles:
    ...
    if data.get("Items") is not None:
        ...
        out["items"] = (
            capo_appconfig.types.configuration_profile_summary_list.deserialize_json(
                data["Items"]
            )
        )
