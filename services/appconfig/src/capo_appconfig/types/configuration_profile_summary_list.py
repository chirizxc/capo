...
ConfigurationProfileSummaryList: TypeAlias = list[
    "capo_appconfig.types.configuration_profile_summary.ConfigurationProfileSummary"
]
...
def serialize_json(value: ConfigurationProfileSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_appconfig.types.configuration_profile_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ConfigurationProfileSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_appconfig.types.configuration_profile_summary.deserialize_json(item)
        )
    return out
