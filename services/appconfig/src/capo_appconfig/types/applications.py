...
class Applications(TypedDict, closed=True):
    items: NotRequired["capo_appconfig.types.application_list.ApplicationList"]
...
def serialize_json(value: Applications) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = capo_appconfig.types.application_list.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> Applications:
    ...
    if data.get("Items") is not None:
        ...
        out["items"] = capo_appconfig.types.application_list.deserialize_json(
            data["Items"]
        )
