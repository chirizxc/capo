...
if TYPE_CHECKING:
    ...
    import capo_appflow.types.data_transfer_api_type_name


class DataTransferApi(TypedDict, closed=True):
    name: NotRequired[
        "capo_appflow.types.data_transfer_api_type_name.DataTransferApiTypeName"
    ]
    """<p>The name of the connector application API.</p>"""
    type: NotRequired["capo_appflow.types.data_transfer_api_type.DataTransferApiType"]
...
def serialize_json(value: DataTransferApi) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = capo_appflow.types.data_transfer_api_type.serialize_json(
            value["type"]
        )
    return out
