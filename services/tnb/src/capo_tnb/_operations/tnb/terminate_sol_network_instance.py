"""Generated from Smithy shape ``com.amazonaws.tnb#TerminateSolNetworkInstance``."""
...
def handle_response(
    ...
    out: capo_tnb.types.terminate_sol_network_instance_output.TerminateSolNetworkInstanceOutput = capo_tnb.types.terminate_sol_network_instance_output.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/sol/nslcm/v1/ns_instances/{nsInstanceId}/terminate"
    )
    url = url.replace("{nsInstanceId}", quote(input_["ns_instance_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_tnb.types.terminate_sol_network_instance_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_terminate_sol_network_instance(
