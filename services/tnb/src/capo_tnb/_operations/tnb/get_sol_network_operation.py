"""Generated from Smithy shape ``com.amazonaws.tnb#GetSolNetworkOperation``."""
...
def handle_response(
    ...
    out: capo_tnb.types.get_sol_network_operation_output.GetSolNetworkOperationOutput = capo_tnb.types.get_sol_network_operation_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sol/nslcm/v1/ns_lcm_op_occs/{nsLcmOpOccId}"
    url = url.replace("{nsLcmOpOccId}", quote(input_["ns_lcm_op_occ_id"], safe=""))
...
async def async_get_sol_network_operation(
