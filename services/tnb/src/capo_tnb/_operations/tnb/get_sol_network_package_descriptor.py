"""Generated from Smithy shape ``com.amazonaws.tnb#GetSolNetworkPackageDescriptor``."""
...
def handle_response(
    ...
    out: capo_tnb.types.get_sol_network_package_descriptor_output.GetSolNetworkPackageDescriptorOutput = {
        "nsd": b"".join(response.iter_raw())
    }  # type: ignore[typeddict-item]
    if "Content-Type" in response.headers:
        out["content_type"] = capo_tnb.types.descriptor_content_type.deserialize_json(
...
async def async_handle_response(
    ...
    out: capo_tnb.types.get_sol_network_package_descriptor_output.GetSolNetworkPackageDescriptorOutput = {
        "nsd": b"".join([chunk async for chunk in response.async_iter_raw()])
    }  # type: ignore[typeddict-item]
    if "Content-Type" in response.headers:
        out["content_type"] = capo_tnb.types.descriptor_content_type.deserialize_json(
            response.headers["Content-Type"]
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sol/nsd/v1/ns_descriptors/{nsdInfoId}/nsd"
    url = url.replace("{nsdInfoId}", quote(input_["nsd_info_id"], safe=""))
...
async def async_get_sol_network_package_descriptor(
