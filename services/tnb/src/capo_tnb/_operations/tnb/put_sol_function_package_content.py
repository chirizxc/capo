"""Generated from Smithy shape ``com.amazonaws.tnb#PutSolFunctionPackageContent``."""
...
def handle_response(
    ...
    out: capo_tnb.types.put_sol_function_package_content_output.PutSolFunctionPackageContentOutput = capo_tnb.types.put_sol_function_package_content_output.deserialize_json(
...
def build_request(
    ...
    if "content_type" in input_:
        headers["Content-Type"] = capo_tnb.types.package_content_type.serialize_json(
            input_["content_type"]
        )
    body: bytes | None = input_["file"]
...
async def async_put_sol_function_package_content(
