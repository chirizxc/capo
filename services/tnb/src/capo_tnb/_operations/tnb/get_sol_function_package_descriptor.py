...
def handle_response(
    ...
    out: capo_tnb.types.get_sol_function_package_descriptor_output.GetSolFunctionPackageDescriptorOutput = {
        "vnfd": b"".join(response.iter_raw())
...
async def async_handle_response(
    ...
    out: capo_tnb.types.get_sol_function_package_descriptor_output.GetSolFunctionPackageDescriptorOutput = {
        "vnfd": b"".join([chunk async for chunk in response.async_iter_raw()])
    }  # type: ignore[typeddict-item]
    if "Content-Type" in response.headers:
        out["content_type"] = capo_tnb.types.descriptor_content_type.deserialize_json(
            response.headers["Content-Type"]
...
def build_request(
    ...
    if "accept" in input_:
        headers["Accept"] = capo_tnb.types.descriptor_content_type.serialize_json(
            input_["accept"]
        )
    body: bytes | None = b""
