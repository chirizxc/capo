...
if TYPE_CHECKING:
    import capo_tnb.types.get_sol_network_operation_task_details

GetSolNetworkOperationTasksList: TypeAlias = list[
    "capo_tnb.types.get_sol_network_operation_task_details.GetSolNetworkOperationTaskDetails"
]
...
def serialize_json(value: GetSolNetworkOperationTasksList) -> list:
    ...
    for item in value:
        out.append(
            capo_tnb.types.get_sol_network_operation_task_details.serialize_json(item)
        )
    return out
