"""Generated from Smithy shape ``com.amazonaws.codeartifact#UpdatePackageGroupOriginConfiguration``."""
...
def handle_response(
    ...
    out: capo_codeartifact.types.update_package_group_origin_configuration_result.UpdatePackageGroupOriginConfigurationResult = capo_codeartifact.types.update_package_group_origin_configuration_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/package-group-origin-configuration"
    ...
    if "package_group" in input_:
        params.append(("package-group", input_["package_group"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_codeartifact.types.update_package_group_origin_configuration_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_package_group_origin_configuration(
