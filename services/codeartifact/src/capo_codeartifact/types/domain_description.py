"""Generated from Smithy shape ``com.amazonaws.codeartifact#DomainDescription``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_codeartifact.types.account_id
    import capo_codeartifact.types.arn
    import capo_codeartifact.types.domain_name
    import capo_codeartifact.types.domain_status
    import capo_codeartifact.types.integer
    import capo_codeartifact.types.long
    import capo_codeartifact.types.timestamp


class DomainDescription(TypedDict, closed=True):
    name: NotRequired["capo_codeartifact.types.domain_name.DomainName"]
    """<p> The name of the domain. </p>"""
    owner: NotRequired["capo_codeartifact.types.account_id.AccountId"]
    """<p> The Amazon Web Services account ID that owns the domain. </p>"""
    arn: NotRequired["capo_codeartifact.types.arn.Arn"]
    """<p> The Amazon Resource Name (ARN) of the domain. </p>"""
    status: NotRequired["capo_codeartifact.types.domain_status.DomainStatus"]
    """<p> The current status of a domain. </p>"""
    created_time: NotRequired["capo_codeartifact.types.timestamp.Timestamp"]
    """<p> A timestamp that represents the date and time the domain was created. </p>"""
    encryption_key: NotRequired["capo_codeartifact.types.arn.Arn"]
    """<p> The ARN of an Key Management Service (KMS) key associated with a domain. </p>"""
    repository_count: "capo_codeartifact.types.integer.Integer"
    """<p> The number of repositories in the domain. </p>"""
    asset_size_bytes: "capo_codeartifact.types.long.Long"
    """<p> The total size of all assets in the domain. </p>"""
    s3_bucket_arn: NotRequired["capo_codeartifact.types.arn.Arn"]
    """<p>The Amazon Resource Name (ARN) of the Amazon S3 bucket that is used to store package assets in the domain.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: DomainDescription) -> dict:
    out: dict = {}
    if "name" in value:
        out["name"] = value["name"]
    if "owner" in value:
        out["owner"] = value["owner"]
    if "arn" in value:
        out["arn"] = value["arn"]
    if "status" in value:
        import capo_codeartifact.types.domain_status

        out["status"] = capo_codeartifact.types.domain_status.serialize_json(
            value["status"]
        )
    if "created_time" in value:
        import capo_codeartifact.types.timestamp

        out["createdTime"] = capo_codeartifact.types.timestamp.serialize_json(
            value["created_time"]
        )
    if "encryption_key" in value:
        out["encryptionKey"] = value["encryption_key"]
    out["repositoryCount"] = value.get("repository_count", 0)
    out["assetSizeBytes"] = value.get("asset_size_bytes", 0)
    if "s3_bucket_arn" in value:
        out["s3BucketArn"] = value["s3_bucket_arn"]
    return out


def deserialize_json(data: dict) -> DomainDescription:
    out: DomainDescription = {}  # type: ignore[typeddict-item]
    if data.get("name") is not None:
        out["name"] = data["name"]
    if data.get("owner") is not None:
        out["owner"] = data["owner"]
    if data.get("arn") is not None:
        out["arn"] = data["arn"]
    if data.get("status") is not None:
        import capo_codeartifact.types.domain_status

        out["status"] = capo_codeartifact.types.domain_status.deserialize_json(
            data["status"]
        )
    if data.get("createdTime") is not None:
        import capo_codeartifact.types.timestamp

        out["created_time"] = capo_codeartifact.types.timestamp.deserialize_json(
            data["createdTime"]
        )
    if data.get("encryptionKey") is not None:
        out["encryption_key"] = data["encryptionKey"]
    if data.get("repositoryCount") is not None:
        out["repository_count"] = data["repositoryCount"]
    else:
        out["repository_count"] = 0
    if data.get("assetSizeBytes") is not None:
        out["asset_size_bytes"] = data["assetSizeBytes"]
    else:
        out["asset_size_bytes"] = 0
    if data.get("s3BucketArn") is not None:
        out["s3_bucket_arn"] = data["s3BucketArn"]
    return out
