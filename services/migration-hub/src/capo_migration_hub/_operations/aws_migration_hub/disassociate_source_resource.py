"""Generated from Smithy shape ``com.amazonaws.migrationhub#DisassociateSourceResource``."""
...
def handle_response(
    ...
    out: capo_migration_hub.types.disassociate_source_resource_result.DisassociateSourceResourceResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    headers["X-Amz-Target"] = "AWSMigrationHub.DisassociateSourceResource"
    body: bytes | None = json.dumps(
        capo_migration_hub.types.disassociate_source_resource_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_disassociate_source_resource(
