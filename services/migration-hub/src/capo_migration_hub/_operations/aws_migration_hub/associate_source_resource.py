"""Generated from Smithy shape ``com.amazonaws.migrationhub#AssociateSourceResource``."""
...
def handle_response(
    ...
    out: capo_migration_hub.types.associate_source_resource_result.AssociateSourceResourceResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    headers["X-Amz-Target"] = "AWSMigrationHub.AssociateSourceResource"
    body: bytes | None = json.dumps(
        capo_migration_hub.types.associate_source_resource_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_associate_source_resource(
