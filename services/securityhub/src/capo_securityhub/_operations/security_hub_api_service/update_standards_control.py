...
def handle_response(
    ...
    out: capo_securityhub.types.update_standards_control_response.UpdateStandardsControlResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{StandardsControlArn+}", quote(input_["standards_control_arn"], safe="/")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.update_standards_control_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
