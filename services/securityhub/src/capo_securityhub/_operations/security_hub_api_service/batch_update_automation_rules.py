"""Generated from Smithy shape ``com.amazonaws.securityhub#BatchUpdateAutomationRules``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.batch_update_automation_rules_response.BatchUpdateAutomationRulesResponse = capo_securityhub.types.batch_update_automation_rules_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/automationrules/update"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.batch_update_automation_rules_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_update_automation_rules(
