"""Generated from Smithy shape ``com.amazonaws.securityhub#StartConfigurationPolicyDisassociation``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.start_configuration_policy_disassociation_response.StartConfigurationPolicyDisassociationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/configurationPolicyAssociation/disassociate"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.start_configuration_policy_disassociation_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_start_configuration_policy_disassociation(
