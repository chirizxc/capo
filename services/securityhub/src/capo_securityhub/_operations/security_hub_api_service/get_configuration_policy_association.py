"""Generated from Smithy shape ``com.amazonaws.securityhub#GetConfigurationPolicyAssociation``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.get_configuration_policy_association_response.GetConfigurationPolicyAssociationResponse = capo_securityhub.types.get_configuration_policy_association_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/configurationPolicyAssociation/get"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.get_configuration_policy_association_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_configuration_policy_association(
