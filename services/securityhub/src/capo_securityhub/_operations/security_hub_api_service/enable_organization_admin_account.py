"""Generated from Smithy shape ``com.amazonaws.securityhub#EnableOrganizationAdminAccount``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.enable_organization_admin_account_response.EnableOrganizationAdminAccountResponse = capo_securityhub.types.enable_organization_admin_account_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/organization/admin/enable"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.enable_organization_admin_account_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
