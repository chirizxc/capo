"""Generated from Smithy shape ``com.amazonaws.securityhub#UpdateFindingAggregator``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.update_finding_aggregator_response.UpdateFindingAggregatorResponse = capo_securityhub.types.update_finding_aggregator_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findingAggregator/update"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.update_finding_aggregator_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_finding_aggregator(
