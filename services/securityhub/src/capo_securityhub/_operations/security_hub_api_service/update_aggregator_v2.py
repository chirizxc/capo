"""Generated from Smithy shape ``com.amazonaws.securityhub#UpdateAggregatorV2``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.update_aggregator_v2_response.UpdateAggregatorV2Response = capo_securityhub.types.update_aggregator_v2_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/aggregatorv2/update/{AggregatorV2Arn+}"
    url = url.replace(
        "{AggregatorV2Arn+}", quote(input_["aggregator_v2_arn"], safe="/")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.update_aggregator_v2_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_aggregator_v2(
