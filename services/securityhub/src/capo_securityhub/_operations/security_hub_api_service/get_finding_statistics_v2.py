"""Generated from Smithy shape ``com.amazonaws.securityhub#GetFindingStatisticsV2``."""
...
import capo_securityhub.errors.organization_not_found_exception
import capo_securityhub.errors.organizational_unit_not_found_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "OrganizationalUnitNotFoundException":
            raise capo_securityhub.errors.organizational_unit_not_found_exception.OrganizationalUnitNotFoundException.from_json(
                data, message
            )
        case "OrganizationNotFoundException":
            raise capo_securityhub.errors.organization_not_found_exception.OrganizationNotFoundException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_securityhub.types.get_finding_statistics_v2_response.GetFindingStatisticsV2Response = capo_securityhub.types.get_finding_statistics_v2_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findingsv2/statistics"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.get_finding_statistics_v2_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_finding_statistics_v2(
