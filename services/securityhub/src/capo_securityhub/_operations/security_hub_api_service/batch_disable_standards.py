"""Generated from Smithy shape ``com.amazonaws.securityhub#BatchDisableStandards``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.batch_disable_standards_response.BatchDisableStandardsResponse = capo_securityhub.types.batch_disable_standards_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/standards/deregister"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.batch_disable_standards_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_batch_disable_standards(
