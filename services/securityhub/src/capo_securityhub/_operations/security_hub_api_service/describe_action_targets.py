"""Generated from Smithy shape ``com.amazonaws.securityhub#DescribeActionTargets``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.describe_action_targets_response.DescribeActionTargetsResponse = capo_securityhub.types.describe_action_targets_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/actionTargets/get"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.describe_action_targets_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_describe_action_targets(
