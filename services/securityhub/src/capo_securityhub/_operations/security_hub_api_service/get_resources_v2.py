"""Generated from Smithy shape ``com.amazonaws.securityhub#GetResourcesV2``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.get_resources_v2_response.GetResourcesV2Response = (
        capo_securityhub.types.get_resources_v2_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/resourcesv2"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.get_resources_v2_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_resources_v2(
