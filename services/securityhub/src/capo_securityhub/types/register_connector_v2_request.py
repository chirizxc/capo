...
class RegisterConnectorV2Request(TypedDict, closed=True):
    auth_code: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    """<p>The authCode retrieved from authUrl to complete the OAuth 2.0 authorization code flow.</p>"""
    auth_state: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
...
def deserialize_json(data: dict) -> RegisterConnectorV2Request:
    ...
    if data.get("AuthState") is not None:
