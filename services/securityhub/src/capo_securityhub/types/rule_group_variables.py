"""Generated from Smithy shape ``com.amazonaws.securityhub#RuleGroupVariables``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_securityhub.types.rule_group_variables_ip_sets_details
    import capo_securityhub.types.rule_group_variables_port_sets_details


class RuleGroupVariables(TypedDict, closed=True):
    ip_sets: NotRequired[
        "capo_securityhub.types.rule_group_variables_ip_sets_details.RuleGroupVariablesIpSetsDetails"
    ]
    """<p>A list of IP addresses and address ranges, in CIDR notation.</p>"""
    port_sets: NotRequired[
        "capo_securityhub.types.rule_group_variables_port_sets_details.RuleGroupVariablesPortSetsDetails"
    ]
    """<p>A list of port ranges.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: RuleGroupVariables) -> dict:
    out: dict = {}
    if "ip_sets" in value:
        import capo_securityhub.types.rule_group_variables_ip_sets_details

        out["IpSets"] = (
            capo_securityhub.types.rule_group_variables_ip_sets_details.serialize_json(
                value["ip_sets"]
            )
        )
    if "port_sets" in value:
        import capo_securityhub.types.rule_group_variables_port_sets_details

        out["PortSets"] = (
            capo_securityhub.types.rule_group_variables_port_sets_details.serialize_json(
                value["port_sets"]
            )
        )
    return out


def deserialize_json(data: dict) -> RuleGroupVariables:
    out: RuleGroupVariables = {}  # type: ignore[typeddict-item]
    if data.get("IpSets") is not None:
        import capo_securityhub.types.rule_group_variables_ip_sets_details

        out["ip_sets"] = (
            capo_securityhub.types.rule_group_variables_ip_sets_details.deserialize_json(
                data["IpSets"]
            )
        )
    if data.get("PortSets") is not None:
        import capo_securityhub.types.rule_group_variables_port_sets_details

        out["port_sets"] = (
            capo_securityhub.types.rule_group_variables_port_sets_details.deserialize_json(
                data["PortSets"]
            )
        )
    return out
