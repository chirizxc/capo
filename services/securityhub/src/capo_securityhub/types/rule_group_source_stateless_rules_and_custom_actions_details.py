"""Generated from Smithy shape ``com.amazonaws.securityhub#RuleGroupSourceStatelessRulesAndCustomActionsDetails``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_securityhub.types.rule_group_source_custom_actions_list
    import capo_securityhub.types.rule_group_source_stateless_rules_list


class RuleGroupSourceStatelessRulesAndCustomActionsDetails(TypedDict, closed=True):
    custom_actions: NotRequired[
        "capo_securityhub.types.rule_group_source_custom_actions_list.RuleGroupSourceCustomActionsList"
    ]
    """<p>Custom actions for the rule group.</p>"""
    stateless_rules: NotRequired[
        "capo_securityhub.types.rule_group_source_stateless_rules_list.RuleGroupSourceStatelessRulesList"
    ]
    """<p>Stateless rules for the rule group.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: RuleGroupSourceStatelessRulesAndCustomActionsDetails) -> dict:
    out: dict = {}
    if "custom_actions" in value:
        import capo_securityhub.types.rule_group_source_custom_actions_list

        out["CustomActions"] = (
            capo_securityhub.types.rule_group_source_custom_actions_list.serialize_json(
                value["custom_actions"]
            )
        )
    if "stateless_rules" in value:
        import capo_securityhub.types.rule_group_source_stateless_rules_list

        out["StatelessRules"] = (
            capo_securityhub.types.rule_group_source_stateless_rules_list.serialize_json(
                value["stateless_rules"]
            )
        )
    return out


def deserialize_json(
    data: dict,
) -> RuleGroupSourceStatelessRulesAndCustomActionsDetails:
    out: RuleGroupSourceStatelessRulesAndCustomActionsDetails = {}  # type: ignore[typeddict-item]
    if data.get("CustomActions") is not None:
        import capo_securityhub.types.rule_group_source_custom_actions_list

        out["custom_actions"] = (
            capo_securityhub.types.rule_group_source_custom_actions_list.deserialize_json(
                data["CustomActions"]
            )
        )
    if data.get("StatelessRules") is not None:
        import capo_securityhub.types.rule_group_source_stateless_rules_list

        out["stateless_rules"] = (
            capo_securityhub.types.rule_group_source_stateless_rules_list.deserialize_json(
                data["StatelessRules"]
            )
        )
    return out
