...
class Threat(TypedDict, closed=True):
    ...
    severity: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    ...
    file_paths: NotRequired["capo_securityhub.types.file_path_list.FilePathList"]
...
def serialize_json(value: Threat) -> dict:
    ...
    if "file_paths" in value:
        ...
        out["FilePaths"] = capo_securityhub.types.file_path_list.serialize_json(
            value["file_paths"]
        )
    return out
