...
class AwsEcsTaskVolumeDetails(TypedDict, closed=True):
    ...
    host: NotRequired[
        "capo_securityhub.types.aws_ecs_task_volume_host_details.AwsEcsTaskVolumeHostDetails"
    ]
...
def serialize_json(value: AwsEcsTaskVolumeDetails) -> dict:
    ...
    if "host" in value:
        ...
        out["Host"] = (
            capo_securityhub.types.aws_ecs_task_volume_host_details.serialize_json(
                value["host"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AwsEcsTaskVolumeDetails:
    ...
    if data.get("Host") is not None:
        ...
        out["host"] = (
            capo_securityhub.types.aws_ecs_task_volume_host_details.deserialize_json(
                data["Host"]
            )
        )
    return out
