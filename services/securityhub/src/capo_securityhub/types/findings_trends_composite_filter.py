...
class FindingsTrendsCompositeFilter(TypedDict, closed=True):
    string_filters: NotRequired[
        "capo_securityhub.types.findings_trends_string_filter_list.FindingsTrendsStringFilterList"
    ]
...
def serialize_json(value: FindingsTrendsCompositeFilter) -> dict:
    ...
    if "string_filters" in value:
        ...
        out["StringFilters"] = (
            capo_securityhub.types.findings_trends_string_filter_list.serialize_json(
...
def deserialize_json(data: dict) -> FindingsTrendsCompositeFilter:
    ...
    if data.get("StringFilters") is not None:
        ...
        out["string_filters"] = (
            capo_securityhub.types.findings_trends_string_filter_list.deserialize_json(
                data["StringFilters"]
            )
        )
