...
class TrendsValues(TypedDict, closed=True):
    severity_trends: NotRequired[
        "capo_securityhub.types.severity_trends_count.SeverityTrendsCount"
    ]
...
def serialize_json(value: TrendsValues) -> dict:
    ...
    if "severity_trends" in value:
        ...
        out["SeverityTrends"] = (
            capo_securityhub.types.severity_trends_count.serialize_json(
...
def deserialize_json(data: dict) -> TrendsValues:
    ...
    if data.get("SeverityTrends") is not None:
        ...
        out["severity_trends"] = (
            capo_securityhub.types.severity_trends_count.deserialize_json(
                data["SeverityTrends"]
            )
        )
    return out
