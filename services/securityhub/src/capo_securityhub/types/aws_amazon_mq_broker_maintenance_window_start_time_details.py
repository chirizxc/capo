...
class AwsAmazonMqBrokerMaintenanceWindowStartTimeDetails(TypedDict, closed=True):
    day_of_week: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    """<p> The day of the week on which the maintenance window falls. </p>"""
    time_of_day: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    """<p> The time, in 24-hour format, on which the maintenance window falls. </p>"""
    time_zone: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
