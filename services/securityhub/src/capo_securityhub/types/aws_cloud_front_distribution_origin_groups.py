"""Generated from Smithy shape ``com.amazonaws.securityhub#AwsCloudFrontDistributionOriginGroups``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_securityhub.types.aws_cloud_front_distribution_origin_groups_item_list


class AwsCloudFrontDistributionOriginGroups(TypedDict, closed=True):
    items: NotRequired[
        "capo_securityhub.types.aws_cloud_front_distribution_origin_groups_item_list.AwsCloudFrontDistributionOriginGroupsItemList"
    ]
    """<p>The list of origin groups.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: AwsCloudFrontDistributionOriginGroups) -> dict:
    out: dict = {}
    if "items" in value:
        import capo_securityhub.types.aws_cloud_front_distribution_origin_groups_item_list

        out["Items"] = (
            capo_securityhub.types.aws_cloud_front_distribution_origin_groups_item_list.serialize_json(
                value["items"]
            )
        )
    return out


def deserialize_json(data: dict) -> AwsCloudFrontDistributionOriginGroups:
    out: AwsCloudFrontDistributionOriginGroups = {}  # type: ignore[typeddict-item]
    if data.get("Items") is not None:
        import capo_securityhub.types.aws_cloud_front_distribution_origin_groups_item_list

        out["items"] = (
            capo_securityhub.types.aws_cloud_front_distribution_origin_groups_item_list.deserialize_json(
                data["Items"]
            )
        )
    return out
