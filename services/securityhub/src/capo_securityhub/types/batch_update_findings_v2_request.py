...
def serialize_json(value: BatchUpdateFindingsV2Request) -> dict:
    ...
    if "metadata_uids" in value:
        ...
        out["MetadataUids"] = capo_securityhub.types.metadata_uid_list.serialize_json(
            value["metadata_uids"]
        )
    if "finding_identifiers" in value:
        ...
        out["FindingIdentifiers"] = (
            capo_securityhub.types.ocsf_finding_identifier_list.serialize_json(
...
def deserialize_json(data: dict) -> BatchUpdateFindingsV2Request:
    ...
    if data.get("MetadataUids") is not None:
        import capo_securityhub.types.metadata_uid_list

        out["metadata_uids"] = (
            capo_securityhub.types.metadata_uid_list.deserialize_json(
                data["MetadataUids"]
            )
        )
    if data.get("FindingIdentifiers") is not None:
        ...
        out["finding_identifiers"] = (
            capo_securityhub.types.ocsf_finding_identifier_list.deserialize_json(
