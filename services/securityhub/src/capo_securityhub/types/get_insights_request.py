...
class GetInsightsRequest(TypedDict, closed=True):
    insight_arns: NotRequired["capo_securityhub.types.arn_list.ArnList"]
...
def serialize_json(value: GetInsightsRequest) -> dict:
    ...
    if "insight_arns" in value:
        ...
        out["InsightArns"] = capo_securityhub.types.arn_list.serialize_json(
            value["insight_arns"]
        )
...
def deserialize_json(data: dict) -> GetInsightsRequest:
    ...
    if data.get("InsightArns") is not None:
        import capo_securityhub.types.arn_list

        out["insight_arns"] = capo_securityhub.types.arn_list.deserialize_json(
            data["InsightArns"]
        )
