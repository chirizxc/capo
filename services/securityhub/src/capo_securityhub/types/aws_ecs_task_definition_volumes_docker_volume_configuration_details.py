...
class AwsEcsTaskDefinitionVolumesDockerVolumeConfigurationDetails(
    ...
    autoprovision: NotRequired["capo_securityhub.types.boolean.Boolean"]
    """<p>Whether to create the Docker volume automatically if it does not already exist.</p>"""
    driver: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    """<p>The Docker volume driver to use.</p>"""
    driver_opts: NotRequired["capo_securityhub.types.field_map.FieldMap"]
    """<p>A map of Docker driver-specific options that are passed through.</p>"""
    labels: NotRequired["capo_securityhub.types.field_map.FieldMap"]
    """<p>Custom metadata to add to the Docker volume.</p>"""
    scope: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
...
def serialize_json(
    ...
    if "driver_opts" in value:
        ...
        out["DriverOpts"] = capo_securityhub.types.field_map.serialize_json(
            value["driver_opts"]
        )
    if "labels" in value:
        import capo_securityhub.types.field_map

        out["Labels"] = capo_securityhub.types.field_map.serialize_json(value["labels"])
    if "scope" in value:
