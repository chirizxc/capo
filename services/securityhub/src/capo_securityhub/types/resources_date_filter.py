...
class ResourcesDateFilter(TypedDict, closed=True):
    field_name: NotRequired[
        "capo_securityhub.types.resources_date_field.ResourcesDateField"
    ]
...
def serialize_json(value: ResourcesDateFilter) -> dict:
    ...
    if "field_name" in value:
        ...
        out["FieldName"] = capo_securityhub.types.resources_date_field.serialize_json(
            value["field_name"]
        )
    if "filter" in value:
        import capo_securityhub.types.date_filter
...
def deserialize_json(data: dict) -> ResourcesDateFilter:
    ...
    if data.get("FieldName") is not None:
        ...
        out["field_name"] = (
            capo_securityhub.types.resources_date_field.deserialize_json(
                data["FieldName"]
            )
        )
