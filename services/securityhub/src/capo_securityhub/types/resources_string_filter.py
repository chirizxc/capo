...
class ResourcesStringFilter(TypedDict, closed=True):
    field_name: NotRequired[
        "capo_securityhub.types.resources_string_field.ResourcesStringField"
    ]
...
def serialize_json(value: ResourcesStringFilter) -> dict:
    ...
    if "field_name" in value:
        ...
        out["FieldName"] = capo_securityhub.types.resources_string_field.serialize_json(
            value["field_name"]
        )
    if "filter" in value:
        import capo_securityhub.types.string_filter
...
def deserialize_json(data: dict) -> ResourcesStringFilter:
    ...
    if data.get("FieldName") is not None:
        ...
        out["field_name"] = (
            capo_securityhub.types.resources_string_field.deserialize_json(
                data["FieldName"]
            )
        )
