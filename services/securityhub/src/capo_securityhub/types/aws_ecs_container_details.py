...
class AwsEcsContainerDetails(TypedDict, closed=True):
    ...
    mount_points: NotRequired[
        "capo_securityhub.types.aws_mount_point_list.AwsMountPointList"
    ]
...
def serialize_json(value: AwsEcsContainerDetails) -> dict:
    ...
    if "mount_points" in value:
        ...
        out["MountPoints"] = capo_securityhub.types.aws_mount_point_list.serialize_json(
            value["mount_points"]
        )
...
def deserialize_json(data: dict) -> AwsEcsContainerDetails:
    ...
    if data.get("MountPoints") is not None:
        ...
        out["mount_points"] = (
            capo_securityhub.types.aws_mount_point_list.deserialize_json(
                data["MountPoints"]
            )
        )
