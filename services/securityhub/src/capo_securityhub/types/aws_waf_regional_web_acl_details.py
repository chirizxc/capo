...
if TYPE_CHECKING:
    import capo_securityhub.types.aws_waf_regional_web_acl_rules_list
    import capo_securityhub.types.non_empty_string


class AwsWafRegionalWebAclDetails(TypedDict, closed=True):
    ...
    rules_list: NotRequired[
        "capo_securityhub.types.aws_waf_regional_web_acl_rules_list.AwsWafRegionalWebAclRulesList"
    ]
...
def serialize_json(value: AwsWafRegionalWebAclDetails) -> dict:
    ...
    if "rules_list" in value:
        ...
        out["RulesList"] = (
            capo_securityhub.types.aws_waf_regional_web_acl_rules_list.serialize_json(
                value["rules_list"]
            )
        )
...
def deserialize_json(data: dict) -> AwsWafRegionalWebAclDetails:
    ...
    if data.get("Name") is not None:
        out["name"] = data["Name"]
    if data.get("RulesList") is not None:
        import capo_securityhub.types.aws_waf_regional_web_acl_rules_list

        out["rules_list"] = (
            capo_securityhub.types.aws_waf_regional_web_acl_rules_list.deserialize_json(
                data["RulesList"]
            )
        )
