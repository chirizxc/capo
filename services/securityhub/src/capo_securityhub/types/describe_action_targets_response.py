...
class DescribeActionTargetsResponse(TypedDict, closed=True):
    action_targets: NotRequired[
        "capo_securityhub.types.action_target_list.ActionTargetList"
    ]
...
def serialize_json(value: DescribeActionTargetsResponse) -> dict:
    ...
    if "action_targets" in value:
        ...
        out["ActionTargets"] = capo_securityhub.types.action_target_list.serialize_json(
...
def deserialize_json(data: dict) -> DescribeActionTargetsResponse:
    ...
    if data.get("ActionTargets") is not None:
        ...
        out["action_targets"] = (
            capo_securityhub.types.action_target_list.deserialize_json(
                data["ActionTargets"]
            )
        )
