...
class AwsEc2NetworkAclDetails(TypedDict, closed=True):
    is_default: NotRequired["capo_securityhub.types.boolean.Boolean"]
    ...
    associations: NotRequired[
        "capo_securityhub.types.aws_ec2_network_acl_association_list.AwsEc2NetworkAclAssociationList"
    ]
    """<p>Associations between the network ACL and subnets.</p>"""
    entries: NotRequired[
        "capo_securityhub.types.aws_ec2_network_acl_entry_list.AwsEc2NetworkAclEntryList"
    ]
...
def serialize_json(value: AwsEc2NetworkAclDetails) -> dict:
    ...
    if "associations" in value:
        ...
        out["Associations"] = (
            capo_securityhub.types.aws_ec2_network_acl_association_list.serialize_json(
                value["associations"]
            )
        )
    if "entries" in value:
        ...
        out["Entries"] = (
            capo_securityhub.types.aws_ec2_network_acl_entry_list.serialize_json(
                value["entries"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AwsEc2NetworkAclDetails:
    ...
    if data.get("Associations") is not None:
        ...
        out["associations"] = (
            capo_securityhub.types.aws_ec2_network_acl_association_list.deserialize_json(
                data["Associations"]
            )
        )
    if data.get("Entries") is not None:
        ...
        out["entries"] = (
            capo_securityhub.types.aws_ec2_network_acl_entry_list.deserialize_json(
                data["Entries"]
            )
        )
    return out
