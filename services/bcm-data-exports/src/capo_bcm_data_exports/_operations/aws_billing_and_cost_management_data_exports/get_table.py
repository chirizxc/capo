"""Generated from Smithy shape ``com.amazonaws.bcmdataexports#GetTable``."""
...
import capo_bcm_data_exports._protocol.eventstream
...
def handle_response(
    ...
    out: capo_bcm_data_exports.types.get_table_response.GetTableResponse = (
        capo_bcm_data_exports.types.get_table_response.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_bcm_data_exports._auth._sigv4.build_sigv4_auth_scheme(
                "bcm-data-exports", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "AWSBillingAndCostManagementDataExports.GetTable"
    body: bytes | None = json.dumps(
        capo_bcm_data_exports.types.get_table_request.serialize_aws_json_1_1(input_),
        allow_nan=False,
    ).encode()
...
def get_table(
    ...
    capo_bcm_data_exports.types.get_table_response.GetTableResponse, zapros.Response
]:
