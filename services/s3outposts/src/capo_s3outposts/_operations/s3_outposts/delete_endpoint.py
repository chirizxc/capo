...
import capo_s3outposts._protocol.eventstream
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_s3outposts._auth._sigv4.build_sigv4_auth_scheme(
                "s3-outposts", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    if "endpoint_id" in input_:
        params.append(("endpointId", input_["endpoint_id"]))
    if "outpost_id" in input_:
        params.append(("outpostId", input_["outpost_id"]))
