...
def handle_response(
    ...
    out: capo_entityresolution.types.batch_delete_unique_id_output.BatchDeleteUniqueIdOutput = capo_entityresolution.types.batch_delete_unique_id_output.deserialize_json(
...
def build_request(
    ...
    if "input_source" in input_:
        headers["inputSource"] = input_["input_source"]
    if "unique_ids" in input_:
        headers["uniqueIds"] = ", ".join(item for item in input_["unique_ids"])
    body: bytes | None = b""
