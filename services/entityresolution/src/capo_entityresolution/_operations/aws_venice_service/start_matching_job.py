"""Generated from Smithy shape ``com.amazonaws.entityresolution#StartMatchingJob``."""
...
def handle_response(
    ...
    out: capo_entityresolution.types.start_matching_job_output.StartMatchingJobOutput = capo_entityresolution.types.start_matching_job_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_entityresolution._auth._sigv4.build_sigv4_auth_scheme(
                "entityresolution", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/matchingworkflows/{workflowName}/jobs"
    url = url.replace("{workflowName}", quote(input_["workflow_name"], safe=""))
...
async def async_start_matching_job(
