"""Generated from Smithy shape ``com.amazonaws.codecommit#PullRequestTargetList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_codecommit.types.pull_request_target

PullRequestTargetList: TypeAlias = list[
    "capo_codecommit.types.pull_request_target.PullRequestTarget"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: PullRequestTargetList) -> list:
    import capo_codecommit.types.pull_request_target

    out: list = []
    for item in value:
        out.append(
            capo_codecommit.types.pull_request_target.serialize_aws_json_1_1(item)
        )
    return out


def deserialize_aws_json_1_1(data: list) -> PullRequestTargetList:
    import capo_codecommit.types.pull_request_target

    out: PullRequestTargetList = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_codecommit.types.pull_request_target.deserialize_aws_json_1_1(item)
        )
    return out
