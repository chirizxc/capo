...
__listOfChannel: TypeAlias = list["capo_mediatailor.types.channel.Channel"]
...
def serialize_json(value: __listOfChannel) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_mediatailor.types.channel.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfChannel:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_mediatailor.types.channel.deserialize_json(item))
    return out
