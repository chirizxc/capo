...
class AvailSuppression(TypedDict, closed=True):
    mode: NotRequired["capo_mediatailor.types.mode.Mode"]
    ...
    value: NotRequired["capo_mediatailor.types.__string.__string"]
    ...
    fill_policy: NotRequired["capo_mediatailor.types.fill_policy.FillPolicy"]
...
def serialize_json(value: AvailSuppression) -> dict:
    ...
    if "mode" in value:
        ...
        out["Mode"] = capo_mediatailor.types.mode.serialize_json(value["mode"])
    if "value" in value:
        out["Value"] = value["value"]
    if "fill_policy" in value:
        import capo_mediatailor.types.fill_policy

        out["FillPolicy"] = capo_mediatailor.types.fill_policy.serialize_json(
            value["fill_policy"]
        )
    return out
