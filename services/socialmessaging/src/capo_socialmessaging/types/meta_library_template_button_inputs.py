...
if TYPE_CHECKING:
    import capo_socialmessaging.types.library_template_button_input

MetaLibraryTemplateButtonInputs: TypeAlias = list[
    "capo_socialmessaging.types.library_template_button_input.LibraryTemplateButtonInput"
]
...
def serialize_json(value: MetaLibraryTemplateButtonInputs) -> list:
    ...
    for item in value:
        out.append(
            capo_socialmessaging.types.library_template_button_input.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> MetaLibraryTemplateButtonInputs:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_socialmessaging.types.library_template_button_input.deserialize_json(
                item
            )
        )
    return out
