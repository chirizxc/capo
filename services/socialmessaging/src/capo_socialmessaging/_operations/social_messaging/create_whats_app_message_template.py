"""Generated from Smithy shape ``com.amazonaws.socialmessaging#CreateWhatsAppMessageTemplate``."""
...
import capo_socialmessaging._protocol.eventstream
...
def handle_response(
    ...
    out: capo_socialmessaging.types.create_whats_app_message_template_output.CreateWhatsAppMessageTemplateOutput = capo_socialmessaging.types.create_whats_app_message_template_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_socialmessaging._auth._sigv4.build_sigv4_auth_scheme(
                "social-messaging", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/template/put"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_socialmessaging.types.create_whats_app_message_template_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_whats_app_message_template(
