...
def handle_response(
    ...
    out: capo_socialmessaging.types.delete_whats_app_message_template_output.DeleteWhatsAppMessageTemplateOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    if "meta_template_id" in input_:
        params.append(("metaTemplateId", input_["meta_template_id"]))
    if "delete_all_languages" in input_:
        params.append(
            (
                "deleteAllTemplates",
                "true" if input_["delete_all_languages"] else "false",
            )
        )
    if "id" in input_:
