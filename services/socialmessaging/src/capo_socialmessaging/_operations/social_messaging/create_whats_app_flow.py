"""Generated from Smithy shape ``com.amazonaws.socialmessaging#CreateWhatsAppFlow``."""
...
def handle_response(
    ...
    out: capo_socialmessaging.types.create_whats_app_flow_output.CreateWhatsAppFlowOutput = capo_socialmessaging.types.create_whats_app_flow_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/flow/create"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_socialmessaging.types.create_whats_app_flow_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_whats_app_flow(
