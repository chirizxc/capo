"""Generated from Smithy shape ``com.amazonaws.socialmessaging#GetWhatsAppMessageMedia``."""
...
def handle_response(
    ...
    out: capo_socialmessaging.types.get_whats_app_message_media_output.GetWhatsAppMessageMediaOutput = capo_socialmessaging.types.get_whats_app_message_media_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/media/get"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_socialmessaging.types.get_whats_app_message_media_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_whats_app_message_media(
