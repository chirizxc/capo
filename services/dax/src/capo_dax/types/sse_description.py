...
if TYPE_CHECKING:
    import capo_dax.types.sse_status


class SSEDescription(TypedDict, closed=True):
    status: NotRequired["capo_dax.types.sse_status.SSEStatus"]
...
def serialize_aws_json_1_1(value: SSEDescription) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_dax.types.sse_status.serialize_aws_json_1_1(
            value["status"]
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> SSEDescription:
    ...
    if data.get("Status") is not None:
        ...
        out["status"] = capo_dax.types.sse_status.deserialize_aws_json_1_1(
            data["Status"]
        )
    return out
