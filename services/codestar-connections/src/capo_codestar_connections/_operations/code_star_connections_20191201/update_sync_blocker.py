"""Generated from Smithy shape ``com.amazonaws.codestarconnections#UpdateSyncBlocker``."""
...
import capo_codestar_connections._protocol.eventstream
...
import capo_codestar_connections.errors.retry_latest_commit_failed_exception
import capo_codestar_connections.errors.sync_blocker_does_not_exist_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "RetryLatestCommitFailedException":
            raise capo_codestar_connections.errors.retry_latest_commit_failed_exception.RetryLatestCommitFailedException.from_aws_json_1_0(
                data, message
            )
        case "SyncBlockerDoesNotExistException":
            raise capo_codestar_connections.errors.sync_blocker_does_not_exist_exception.SyncBlockerDoesNotExistException.from_aws_json_1_0(
                data, message
            )
...
def handle_response(
    ...
    out: capo_codestar_connections.types.update_sync_blocker_output.UpdateSyncBlockerOutput = capo_codestar_connections.types.update_sync_blocker_output.deserialize_aws_json_1_0(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = (
                capo_codestar_connections._auth._sigv4.build_sigv4_auth_scheme(
                    "codestar-connections", options.region, endpoint_scheme
                )
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "CodeStar_connections_20191201.UpdateSyncBlocker"
    body: bytes | None = json.dumps(
        capo_codestar_connections.types.update_sync_blocker_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
