...
class ListHostsOutput(TypedDict, closed=True):
    hosts: NotRequired["capo_codestar_connections.types.host_list.HostList"]
...
def serialize_aws_json_1_0(value: ListHostsOutput) -> dict:
    ...
    if "hosts" in value:
        ...
        out["Hosts"] = capo_codestar_connections.types.host_list.serialize_aws_json_1_0(
            value["hosts"]
        )
...
def deserialize_aws_json_1_0(data: dict) -> ListHostsOutput:
    ...
    if data.get("Hosts") is not None:
        ...
        out["hosts"] = (
            capo_codestar_connections.types.host_list.deserialize_aws_json_1_0(
                data["Hosts"]
            )
        )
