...
if TYPE_CHECKING:
    import capo_transcribe_streaming.types.transcript


class TranscriptEvent(TypedDict, closed=True):
    transcript: NotRequired["capo_transcribe_streaming.types.transcript.Transcript"]
...
def serialize_json(value: TranscriptEvent) -> dict:
    ...
    if "transcript" in value:
        ...
        out["Transcript"] = capo_transcribe_streaming.types.transcript.serialize_json(
            value["transcript"]
        )
    return out
...
def deserialize_json(data: dict) -> TranscriptEvent:
    ...
    if data.get("Transcript") is not None:
        ...
        out["transcript"] = capo_transcribe_streaming.types.transcript.deserialize_json(
            data["Transcript"]
        )
    return out
