"""Generated from Smithy shape ``com.amazonaws.transcribestreaming#MedicalScribeTranscriptSegment``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_transcribe_streaming.types.boolean
    import capo_transcribe_streaming.types.double
    import capo_transcribe_streaming.types.medical_scribe_transcript_item_list
    import capo_transcribe_streaming.types.string


class MedicalScribeTranscriptSegment(TypedDict, closed=True):
    segment_id: NotRequired["capo_transcribe_streaming.types.string.String"]
    """<p>The identifier of the segment.</p>"""
    begin_audio_time: "capo_transcribe_streaming.types.double.Double"
    """<p>The start time, in milliseconds, of the segment.</p>"""
    end_audio_time: "capo_transcribe_streaming.types.double.Double"
    """<p>The end time, in milliseconds, of the segment.</p>"""
    content: NotRequired["capo_transcribe_streaming.types.string.String"]
    """<p>Contains transcribed text of the segment.</p>"""
    items: NotRequired[
        "capo_transcribe_streaming.types.medical_scribe_transcript_item_list.MedicalScribeTranscriptItemList"
    ]
    """<p>Contains words, phrases, or punctuation marks in your segment.</p>"""
    is_partial: "capo_transcribe_streaming.types.boolean.Boolean"
    """<p>Indicates if the segment is complete.</p> <p>If <code>IsPartial</code> is <code>true</code>, the segment is not complete. If <code>IsPartial</code> is <code>false</code>, the segment is complete. </p>"""
    channel_id: NotRequired["capo_transcribe_streaming.types.string.String"]
    """<p>Indicates which audio channel is associated with the <code>MedicalScribeTranscriptSegment</code>. </p> <p>If <code>MedicalScribeChannelDefinition</code> is not provided in the <code>MedicalScribeConfigurationEvent</code>, then this field will not be included. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: MedicalScribeTranscriptSegment) -> dict:
    out: dict = {}
    if "segment_id" in value:
        out["SegmentId"] = value["segment_id"]
    out["BeginAudioTime"] = (
        "NaN"
        if value.get("begin_audio_time", 0) != value.get("begin_audio_time", 0)
        else "Infinity"
        if value.get("begin_audio_time", 0) == float("inf")
        else "-Infinity"
        if value.get("begin_audio_time", 0) == float("-inf")
        else value.get("begin_audio_time", 0)
    )
    out["EndAudioTime"] = (
        "NaN"
        if value.get("end_audio_time", 0) != value.get("end_audio_time", 0)
        else "Infinity"
        if value.get("end_audio_time", 0) == float("inf")
        else "-Infinity"
        if value.get("end_audio_time", 0) == float("-inf")
        else value.get("end_audio_time", 0)
    )
    if "content" in value:
        out["Content"] = value["content"]
    if "items" in value:
        import capo_transcribe_streaming.types.medical_scribe_transcript_item_list

        out["Items"] = (
            capo_transcribe_streaming.types.medical_scribe_transcript_item_list.serialize_json(
                value["items"]
            )
        )
    out["IsPartial"] = value.get("is_partial", False)
    if "channel_id" in value:
        out["ChannelId"] = value["channel_id"]
    return out


def deserialize_json(data: dict) -> MedicalScribeTranscriptSegment:
    out: MedicalScribeTranscriptSegment = {}  # type: ignore[typeddict-item]
    if data.get("SegmentId") is not None:
        out["segment_id"] = data["SegmentId"]
    if data.get("BeginAudioTime") is not None:
        out["begin_audio_time"] = float(data["BeginAudioTime"])
    else:
        out["begin_audio_time"] = 0
    if data.get("EndAudioTime") is not None:
        out["end_audio_time"] = float(data["EndAudioTime"])
    else:
        out["end_audio_time"] = 0
    if data.get("Content") is not None:
        out["content"] = data["Content"]
    if data.get("Items") is not None:
        import capo_transcribe_streaming.types.medical_scribe_transcript_item_list

        out["items"] = (
            capo_transcribe_streaming.types.medical_scribe_transcript_item_list.deserialize_json(
                data["Items"]
            )
        )
    if data.get("IsPartial") is not None:
        out["is_partial"] = data["IsPartial"]
    else:
        out["is_partial"] = False
    if data.get("ChannelId") is not None:
        out["channel_id"] = data["ChannelId"]
    return out
