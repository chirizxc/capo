...
def handle_response(
    ...
    _union_deser = (
        capo_transcribe_streaming.types.transcript_result_stream.deserialize_event_json
    )
    _iter = cast(Any, response.iter_bytes())
    out: capo_transcribe_streaming.types.start_stream_transcription_response.StartStreamTranscriptionResponse = {
    ...
    out["identify_multiple_languages"] = (
        response.headers["x-amzn-transcribe-identify-multiple-languages"].lower()
        == "true"
    )
