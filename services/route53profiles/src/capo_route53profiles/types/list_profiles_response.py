...
class ListProfilesResponse(TypedDict, closed=True):
    profile_summaries: NotRequired[
        "capo_route53profiles.types.profile_summary_list.ProfileSummaryList"
    ]
...
def serialize_json(value: ListProfilesResponse) -> dict:
    ...
    if "profile_summaries" in value:
        ...
        out["ProfileSummaries"] = (
            capo_route53profiles.types.profile_summary_list.serialize_json(
...
def deserialize_json(data: dict) -> ListProfilesResponse:
    ...
    if data.get("ProfileSummaries") is not None:
        ...
        out["profile_summaries"] = (
            capo_route53profiles.types.profile_summary_list.deserialize_json(
