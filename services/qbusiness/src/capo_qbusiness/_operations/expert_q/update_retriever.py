"""Generated from Smithy shape ``com.amazonaws.qbusiness#UpdateRetriever``."""
...
def handle_response(
    ...
    out: capo_qbusiness.types.update_retriever_response.UpdateRetrieverResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{retrieverId}", quote(input_["retriever_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qbusiness.types.update_retriever_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_retriever(
