"""Generated from Smithy shape ``com.amazonaws.qbusiness#CreateAnonymousWebExperienceUrl``."""
...
def handle_response(
    ...
    out: capo_qbusiness.types.create_anonymous_web_experience_url_response.CreateAnonymousWebExperienceUrlResponse = capo_qbusiness.types.create_anonymous_web_experience_url_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{webExperienceId}", quote(input_["web_experience_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qbusiness.types.create_anonymous_web_experience_url_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_anonymous_web_experience_url(
