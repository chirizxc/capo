...
def handle_response(
    ...
    out: capo_qbusiness.types.update_plugin_response.UpdatePluginResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{pluginId}", quote(input_["plugin_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qbusiness.types.update_plugin_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_plugin(
    ...
    capo_qbusiness.types.update_plugin_response.UpdatePluginResponse, zapros.Response
]:
