...
class ListRetrieversResponse(TypedDict, closed=True):
    retrievers: NotRequired["capo_qbusiness.types.retrievers.Retrievers"]
...
def serialize_json(value: ListRetrieversResponse) -> dict:
    ...
    if "retrievers" in value:
        ...
        out["retrievers"] = capo_qbusiness.types.retrievers.serialize_json(
            value["retrievers"]
        )
...
def deserialize_json(data: dict) -> ListRetrieversResponse:
    ...
    if data.get("retrievers") is not None:
        ...
        out["retrievers"] = capo_qbusiness.types.retrievers.deserialize_json(
            data["retrievers"]
        )
