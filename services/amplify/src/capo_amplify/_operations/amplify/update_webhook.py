...
def handle_response(
    ...
    out: capo_amplify.types.update_webhook_result.UpdateWebhookResult = (
        capo_amplify.types.update_webhook_result.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{webhookId}", quote(input_["webhook_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_amplify.types.update_webhook_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_webhook(
    ...
    capo_amplify.types.update_webhook_result.UpdateWebhookResult, zapros.Response
]:
