...
__listOfReadinessCheckSummary: TypeAlias = list[
    "capo_route53_recovery_readiness.types.readiness_check_summary.ReadinessCheckSummary"
]
...
def serialize_json(value: __listOfReadinessCheckSummary) -> list:
    ...
    for item in value:
        out.append(
            capo_route53_recovery_readiness.types.readiness_check_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> __listOfReadinessCheckSummary:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_route53_recovery_readiness.types.readiness_check_summary.deserialize_json(
                item
            )
        )
    return out
