...
if TYPE_CHECKING:
    import capo_route53_recovery_readiness.types.message

__listOfMessage: TypeAlias = list[
    "capo_route53_recovery_readiness.types.message.Message"
]
...
def serialize_json(value: __listOfMessage) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_route53_recovery_readiness.types.message.serialize_json(item))
    return out
