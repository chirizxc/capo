...
class DescribeEndpointsRequest(TypedDict, closed=True):
    max_results: NotRequired["capo_mediaconvert.types.__integer.__integer"]
...
def serialize_json(value: DescribeEndpointsRequest) -> dict:
    ...
    if "mode" in value:
        ...
        out["mode"] = capo_mediaconvert.types.describe_endpoints_mode.serialize_json(
            value["mode"]
        )
...
def deserialize_json(data: dict) -> DescribeEndpointsRequest:
    ...
    if data.get("mode") is not None:
        ...
        out["mode"] = capo_mediaconvert.types.describe_endpoints_mode.deserialize_json(
            data["mode"]
        )
