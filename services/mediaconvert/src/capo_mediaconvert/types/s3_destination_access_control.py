...
class S3DestinationAccessControl(TypedDict, closed=True):
    canned_acl: NotRequired[
        "capo_mediaconvert.types.s3_object_canned_acl.S3ObjectCannedAcl"
    ]
...
def serialize_json(value: S3DestinationAccessControl) -> dict:
    ...
    if "canned_acl" in value:
        ...
        out["cannedAcl"] = capo_mediaconvert.types.s3_object_canned_acl.serialize_json(
            value["canned_acl"]
        )
    return out
...
def deserialize_json(data: dict) -> S3DestinationAccessControl:
    ...
    if data.get("cannedAcl") is not None:
        ...
        out["canned_acl"] = (
            capo_mediaconvert.types.s3_object_canned_acl.deserialize_json(
                data["cannedAcl"]
            )
        )
    return out
