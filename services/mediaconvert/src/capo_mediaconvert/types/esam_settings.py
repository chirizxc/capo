...
if TYPE_CHECKING:
    import capo_mediaconvert.types.__integer_min0_max30000
...
class EsamSettings(TypedDict, closed=True):
    manifest_confirm_condition_notification: NotRequired[
        "capo_mediaconvert.types.esam_manifest_confirm_condition_notification.EsamManifestConfirmConditionNotification"
    ]
    ...
    response_signal_preroll: NotRequired[
        "capo_mediaconvert.types.__integer_min0_max30000.__integerMin0Max30000"
    ]
    """Specifies the stream distance, in milliseconds, between the SCTE 35 messages that the transcoder places and the splice points that they refer to. If the time between the start of the asset and the SCTE-35 message is less than this value, then the transcoder places the SCTE-35 marker at the beginning of the stream."""
    signal_processing_notification: NotRequired[
        "capo_mediaconvert.types.esam_signal_processing_notification.EsamSignalProcessingNotification"
    ]
...
def serialize_json(value: EsamSettings) -> dict:
    ...
    if "manifest_confirm_condition_notification" in value:
        ...
        out["manifestConfirmConditionNotification"] = (
            capo_mediaconvert.types.esam_manifest_confirm_condition_notification.serialize_json(
    ...
    if "signal_processing_notification" in value:
        ...
        out["signalProcessingNotification"] = (
            capo_mediaconvert.types.esam_signal_processing_notification.serialize_json(
...
def deserialize_json(data: dict) -> EsamSettings:
    ...
    if data.get("manifestConfirmConditionNotification") is not None:
        ...
        out["manifest_confirm_condition_notification"] = (
            capo_mediaconvert.types.esam_manifest_confirm_condition_notification.deserialize_json(
    ...
    if data.get("signalProcessingNotification") is not None:
        ...
        out["signal_processing_notification"] = (
            capo_mediaconvert.types.esam_signal_processing_notification.deserialize_json(
