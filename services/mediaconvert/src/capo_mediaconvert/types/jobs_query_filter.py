...
class JobsQueryFilter(TypedDict, closed=True):
    key: NotRequired["capo_mediaconvert.types.jobs_query_filter_key.JobsQueryFilterKey"]
    ...
    values: NotRequired[
        "capo_mediaconvert.types.__list_of__string_max100.__listOf__stringMax100"
    ]
...
def serialize_json(value: JobsQueryFilter) -> dict:
    ...
    if "key" in value:
        ...
        out["key"] = capo_mediaconvert.types.jobs_query_filter_key.serialize_json(
            value["key"]
        )
    if "values" in value:
        import capo_mediaconvert.types.__list_of__string_max100

        out["values"] = capo_mediaconvert.types.__list_of__string_max100.serialize_json(
            value["values"]
        )
    return out
...
def deserialize_json(data: dict) -> JobsQueryFilter:
    ...
    if data.get("values") is not None:
        ...
        out["values"] = (
            capo_mediaconvert.types.__list_of__string_max100.deserialize_json(
                data["values"]
            )
        )
    return out
