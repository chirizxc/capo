...
class QueueTransition(TypedDict, closed=True):
    destination_queue: NotRequired["capo_mediaconvert.types.__string.__string"]
    """The queue that the job was on after the transition."""
    source_queue: NotRequired["capo_mediaconvert.types.__string.__string"]
    """The queue that the job was on before the transition."""
    timestamp: NotRequired["capo_mediaconvert.types.__timestamp_unix.__timestampUnix"]
...
def serialize_json(value: QueueTransition) -> dict:
    ...
    if "timestamp" in value:
        ...
        out["timestamp"] = capo_mediaconvert.types.__timestamp_unix.serialize_json(
            value["timestamp"]
        )
    return out
