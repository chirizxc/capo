...
class Xavc4kIntraCbgProfileSettings(TypedDict, closed=True):
    xavc_class: NotRequired[
        "capo_mediaconvert.types.xavc4k_intra_cbg_profile_class.Xavc4kIntraCbgProfileClass"
    ]
...
def serialize_json(value: Xavc4kIntraCbgProfileSettings) -> dict:
    ...
    if "xavc_class" in value:
        ...
        out["xavcClass"] = (
            capo_mediaconvert.types.xavc4k_intra_cbg_profile_class.serialize_json(
                value["xavc_class"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> Xavc4kIntraCbgProfileSettings:
    ...
    if data.get("xavcClass") is not None:
        ...
        out["xavc_class"] = (
            capo_mediaconvert.types.xavc4k_intra_cbg_profile_class.deserialize_json(
                data["xavcClass"]
            )
        )
    return out
