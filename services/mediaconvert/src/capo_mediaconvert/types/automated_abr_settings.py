...
if TYPE_CHECKING:
    import capo_mediaconvert.types.__double_min1_max10
    import capo_mediaconvert.types.__integer_min3_max15
    import capo_mediaconvert.types.__integer_min100000_max100000000
...
class AutomatedAbrSettings(TypedDict, closed=True):
    max_abr_bitrate: NotRequired[
        "capo_mediaconvert.types.__integer_min100000_max100000000.__integerMin100000Max100000000"
    ]
    ...
    max_quality_level: NotRequired[
        "capo_mediaconvert.types.__double_min1_max10.__doubleMin1Max10"
    ]
    ...
    max_renditions: NotRequired[
        "capo_mediaconvert.types.__integer_min3_max15.__integerMin3Max15"
    ]
    ...
    rules: NotRequired[
        "capo_mediaconvert.types.__list_of_automated_abr_rule.__listOfAutomatedAbrRule"
    ]
...
def serialize_json(value: AutomatedAbrSettings) -> dict:
    ...
    if "rules" in value:
        ...
        out["rules"] = (
            capo_mediaconvert.types.__list_of_automated_abr_rule.serialize_json(
                value["rules"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AutomatedAbrSettings:
    ...
    if data.get("rules") is not None:
        ...
        out["rules"] = (
            capo_mediaconvert.types.__list_of_automated_abr_rule.deserialize_json(
                data["rules"]
            )
        )
    return out
