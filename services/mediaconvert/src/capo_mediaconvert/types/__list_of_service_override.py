...
if TYPE_CHECKING:
    import capo_mediaconvert.types.service_override

__listOfServiceOverride: TypeAlias = list[
    "capo_mediaconvert.types.service_override.ServiceOverride"
]
...
def serialize_json(value: __listOfServiceOverride) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_mediaconvert.types.service_override.serialize_json(item))
    return out
