...
class F4vSettings(TypedDict, closed=True):
    moov_placement: NotRequired[
        "capo_mediaconvert.types.f4v_moov_placement.F4vMoovPlacement"
    ]
...
def serialize_json(value: F4vSettings) -> dict:
    ...
    if "moov_placement" in value:
        ...
        out["moovPlacement"] = (
            capo_mediaconvert.types.f4v_moov_placement.serialize_json(
...
def deserialize_json(data: dict) -> F4vSettings:
    ...
    if data.get("moovPlacement") is not None:
        ...
        out["moov_placement"] = (
            capo_mediaconvert.types.f4v_moov_placement.deserialize_json(
                data["moovPlacement"]
            )
        )
    return out
