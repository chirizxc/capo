"""Generated from Smithy shape ``com.amazonaws.elastictranscoder#TestRole``."""
...
def handle_response(
    ...
    out: capo_elastic_transcoder.types.test_role_response.TestRoleResponse = (
        capo_elastic_transcoder.types.test_role_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2012-09-25/roleTests"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elastic_transcoder.types.test_role_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def test_role(
    ...
    capo_elastic_transcoder.types.test_role_response.TestRoleResponse, zapros.Response
]:
