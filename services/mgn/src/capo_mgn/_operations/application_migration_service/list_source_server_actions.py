"""Generated from Smithy shape ``com.amazonaws.mgn#ListSourceServerActions``."""
...
def handle_response(
    ...
    out: capo_mgn.types.list_source_server_actions_response.ListSourceServerActionsResponse = capo_mgn.types.list_source_server_actions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListSourceServerActions"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.list_source_server_actions_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_source_server_actions(
