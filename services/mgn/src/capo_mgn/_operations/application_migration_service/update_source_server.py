"""Generated from Smithy shape ``com.amazonaws.mgn#UpdateSourceServer``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateSourceServer"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.update_source_server_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_source_server(
