"""Generated from Smithy shape ``com.amazonaws.mgn#StartImport``."""
...
def handle_response(
    ...
    out: capo_mgn.types.start_import_response.StartImportResponse = (
        capo_mgn.types.start_import_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartImport"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.start_import_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def start_import(
    ...
    input_: capo_mgn.types.start_import_request.StartImportRequest,
) -> tuple[capo_mgn.types.start_import_response.StartImportResponse, zapros.Response]:
