"""Generated from Smithy shape ``com.amazonaws.mgn#StartCutover``."""
...
def handle_response(
    ...
    out: capo_mgn.types.start_cutover_response.StartCutoverResponse = (
        capo_mgn.types.start_cutover_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartCutover"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.start_cutover_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def start_cutover(
    ...
    input_: capo_mgn.types.start_cutover_request.StartCutoverRequest,
) -> tuple[capo_mgn.types.start_cutover_response.StartCutoverResponse, zapros.Response]:
