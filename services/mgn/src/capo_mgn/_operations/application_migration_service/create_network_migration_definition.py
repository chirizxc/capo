"""Generated from Smithy shape ``com.amazonaws.mgn#CreateNetworkMigrationDefinition``."""
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/network-migration/CreateNetworkMigrationDefinition"
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.create_network_migration_definition_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_network_migration_definition(
