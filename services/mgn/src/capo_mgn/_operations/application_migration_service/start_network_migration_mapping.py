"""Generated from Smithy shape ``com.amazonaws.mgn#StartNetworkMigrationMapping``."""
...
def handle_response(
    ...
    out: capo_mgn.types.start_network_migration_mapping_response.StartNetworkMigrationMappingResponse = capo_mgn.types.start_network_migration_mapping_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/network-migration/StartNetworkMigrationMapping"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.start_network_migration_mapping_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_start_network_migration_mapping(
