"""Generated from Smithy shape ``com.amazonaws.mgn#CreateConnector``."""
...
def handle_response(response: zapros.Response) -> capo_mgn.types.connector.Connector:
    out: capo_mgn.types.connector.Connector = capo_mgn.types.connector.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateConnector"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.create_connector_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def create_connector(
    ...
    input_: capo_mgn.types.create_connector_request.CreateConnectorRequest,
) -> tuple[capo_mgn.types.connector.Connector, zapros.Response]:
