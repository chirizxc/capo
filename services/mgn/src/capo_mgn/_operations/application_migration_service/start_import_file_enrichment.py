"""Generated from Smithy shape ``com.amazonaws.mgn#StartImportFileEnrichment``."""
...
def handle_response(
    ...
    out: capo_mgn.types.start_import_file_enrichment_response.StartImportFileEnrichmentResponse = capo_mgn.types.start_import_file_enrichment_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/network-migration/StartImportFileEnrichment"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.start_import_file_enrichment_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_start_import_file_enrichment(
