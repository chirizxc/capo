"""Generated from Smithy shape ``com.amazonaws.mgn#StartExport``."""
...
def handle_response(
    ...
    out: capo_mgn.types.start_export_response.StartExportResponse = (
        capo_mgn.types.start_export_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartExport"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.start_export_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def start_export(
    ...
    input_: capo_mgn.types.start_export_request.StartExportRequest,
) -> tuple[capo_mgn.types.start_export_response.StartExportResponse, zapros.Response]:
