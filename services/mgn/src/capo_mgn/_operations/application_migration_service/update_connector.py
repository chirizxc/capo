"""Generated from Smithy shape ``com.amazonaws.mgn#UpdateConnector``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateConnector"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.update_connector_request.serialize_json(input_), allow_nan=False
    ).encode()
