...
if TYPE_CHECKING:
    import capo_mgn.types.encryption_algorithm
    import capo_mgn.types.hash


class Checksum(TypedDict, closed=True):
    encryption_algorithm: NotRequired[
        "capo_mgn.types.encryption_algorithm.EncryptionAlgorithm"
    ]
    """<p>The encryption algorithm used to generate the checksum.</p>"""
    hash: NotRequired["capo_mgn.types.hash.Hash"]
