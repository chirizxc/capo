...
class ListNetworkMigrationDeployedStacksResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_mgn.types.network_migration_deployed_stacks_list.NetworkMigrationDeployedStacksList"
    ]
...
def serialize_json(value: ListNetworkMigrationDeployedStacksResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            capo_mgn.types.network_migration_deployed_stacks_list.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationDeployedStacksResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = (
            capo_mgn.types.network_migration_deployed_stacks_list.deserialize_json(
                data["items"]
            )
        )
