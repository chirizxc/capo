...
CodeGenerationOutputFormatStatusDetailsMap: TypeAlias = dict[
    ...
    "capo_mgn.types.code_generation_output_format_status_details.CodeGenerationOutputFormatStatusDetails",
]
...
def serialize_json(
    ...
    for key, value in input_to_serialize.items():
        ...
        out[key] = (
            capo_mgn.types.code_generation_output_format_status_details.serialize_json(
                value
            )
        )
    return out
...
def deserialize_json(data: dict) -> CodeGenerationOutputFormatStatusDetailsMap:
    ...
    for key, value in data.items():
        ...
        out[key] = (
            capo_mgn.types.code_generation_output_format_status_details.deserialize_json(
                value
            )
        )
    return out
