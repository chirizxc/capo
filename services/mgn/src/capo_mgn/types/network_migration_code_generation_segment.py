...
if TYPE_CHECKING:
    ...
    import capo_mgn.types.network_migration_code_generation_segment_type
...
class NetworkMigrationCodeGenerationSegment(TypedDict, closed=True):
    ...
    segment_type: NotRequired[
        "capo_mgn.types.network_migration_code_generation_segment_type.NetworkMigrationCodeGenerationSegmentType"
    ]
    ...
    mapper_segment_id: NotRequired["capo_mgn.types.segment_id.SegmentID"]
    """<p>The ID of the mapper segment that this code generation segment was created from.</p>"""
    artifacts: NotRequired[
        "capo_mgn.types.network_migration_code_generation_artifacts.NetworkMigrationCodeGenerationArtifacts"
    ]
...
def serialize_json(value: NetworkMigrationCodeGenerationSegment) -> dict:
    ...
    if "artifacts" in value:
        ...
        out["artifacts"] = (
            capo_mgn.types.network_migration_code_generation_artifacts.serialize_json(
                value["artifacts"]
            )
        )
...
def deserialize_json(data: dict) -> NetworkMigrationCodeGenerationSegment:
    ...
    if data.get("artifacts") is not None:
        ...
        out["artifacts"] = (
            capo_mgn.types.network_migration_code_generation_artifacts.deserialize_json(
                data["artifacts"]
            )
        )
