...
if TYPE_CHECKING:
    import capo_mgn.types.network_migration_mappings_list
    import capo_mgn.types.pagination_token


class ListNetworkMigrationMappingsResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_mgn.types.network_migration_mappings_list.NetworkMigrationMappingsList"
    ]
...
def serialize_json(value: ListNetworkMigrationMappingsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_mgn.types.network_migration_mappings_list.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationMappingsResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = capo_mgn.types.network_migration_mappings_list.deserialize_json(
            data["items"]
        )
