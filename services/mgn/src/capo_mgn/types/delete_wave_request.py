"""Generated from Smithy shape ``com.amazonaws.mgn#DeleteWaveRequest``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_mgn.errors import DeserializationError

if TYPE_CHECKING:
    import capo_mgn.types.account_id
    import capo_mgn.types.wave_id


class DeleteWaveRequest(TypedDict, closed=True):
    wave_id: "capo_mgn.types.wave_id.WaveID"
    """<p>Wave ID.</p>"""
    account_id: NotRequired["capo_mgn.types.account_id.AccountID"]
    """<p>Account ID.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: DeleteWaveRequest) -> dict:
    out: dict = {}
    out["waveID"] = value["wave_id"]
    if "account_id" in value:
        out["accountID"] = value["account_id"]
    return out


def deserialize_json(data: dict) -> DeleteWaveRequest:
    out: DeleteWaveRequest = {}  # type: ignore[typeddict-item]
    if data.get("waveID") is not None:
        out["wave_id"] = data["waveID"]
    else:
        raise DeserializationError("DeleteWaveRequest.wave_id required")
    if data.get("accountID") is not None:
        out["account_id"] = data["accountID"]
    return out
