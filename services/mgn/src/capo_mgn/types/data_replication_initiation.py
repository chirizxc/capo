...
if TYPE_CHECKING:
    import capo_mgn.types.data_replication_initiation_steps
...
class DataReplicationInitiation(TypedDict, closed=True):
    ...
    steps: NotRequired[
        "capo_mgn.types.data_replication_initiation_steps.DataReplicationInitiationSteps"
    ]
...
def serialize_json(value: DataReplicationInitiation) -> dict:
    ...
    if "steps" in value:
        ...
        out["steps"] = capo_mgn.types.data_replication_initiation_steps.serialize_json(
            value["steps"]
        )
    return out
...
def deserialize_json(data: dict) -> DataReplicationInitiation:
    ...
    if data.get("steps") is not None:
        ...
        out["steps"] = (
            capo_mgn.types.data_replication_initiation_steps.deserialize_json(
                data["steps"]
            )
        )
    return out
