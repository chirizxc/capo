"""Generated from Smithy shape ``com.amazonaws.mgn#ListNetworkMigrationMappingUpdatesRequest``."""
...
class ListNetworkMigrationMappingUpdatesRequest(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: ListNetworkMigrationMappingUpdatesRequest) -> dict:
    out: dict = {}
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_mgn.types.list_network_migration_mapping_updates_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationMappingUpdatesRequest:
    out: ListNetworkMigrationMappingUpdatesRequest = {}  # type: ignore[typeddict-item]
    if data.get("networkMigrationExecutionID") is not None:
        ...
        raise DeserializationError(
            "ListNetworkMigrationMappingUpdatesRequest.network_migration_execution_id required"
        )
    if data.get("networkMigrationDefinitionID") is not None:
        ...
        raise DeserializationError(
            "ListNetworkMigrationMappingUpdatesRequest.network_migration_definition_id required"
        )
    if data.get("filters") is not None:
        import capo_mgn.types.list_network_migration_mapping_updates_filters

        out["filters"] = (
            capo_mgn.types.list_network_migration_mapping_updates_filters.deserialize_json(
                data["filters"]
            )
        )
