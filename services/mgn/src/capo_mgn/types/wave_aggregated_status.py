...
if TYPE_CHECKING:
    ...
    import capo_mgn.types.wave_health_status
    import capo_mgn.types.wave_progress_status


class WaveAggregatedStatus(TypedDict, closed=True):
    ...
    health_status: NotRequired["capo_mgn.types.wave_health_status.WaveHealthStatus"]
    """<p>Wave aggregated status health status.</p>"""
    progress_status: NotRequired[
        "capo_mgn.types.wave_progress_status.WaveProgressStatus"
    ]
    """<p>Wave aggregated status progress status.</p>"""
    total_applications: "capo_mgn.types.positive_integer.PositiveInteger"
