...
class ListNetworkMigrationAnalysesResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_mgn.types.network_migration_analyses_list.NetworkMigrationAnalysesList"
    ]
...
def serialize_json(value: ListNetworkMigrationAnalysesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_mgn.types.network_migration_analyses_list.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationAnalysesResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = capo_mgn.types.network_migration_analyses_list.deserialize_json(
            data["items"]
        )
