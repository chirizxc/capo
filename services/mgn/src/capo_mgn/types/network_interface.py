...
class NetworkInterface(TypedDict, closed=True):
    mac_address: NotRequired["capo_mgn.types.bounded_string.BoundedString"]
    """<p>Network interface Mac address.</p>"""
    ips: NotRequired["capo_mgn.types.i_ps_list.IPsList"]
...
def serialize_json(value: NetworkInterface) -> dict:
    ...
    if "ips" in value:
        ...
        out["ips"] = capo_mgn.types.i_ps_list.serialize_json(value["ips"])
