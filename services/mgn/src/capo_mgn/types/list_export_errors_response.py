"""Generated from Smithy shape ``com.amazonaws.mgn#ListExportErrorsResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_mgn.types.export_errors
    import capo_mgn.types.pagination_token


class ListExportErrorsResponse(TypedDict, closed=True):
    items: NotRequired["capo_mgn.types.export_errors.ExportErrors"]
    """<p>List export errors response items.</p>"""
    next_token: NotRequired["capo_mgn.types.pagination_token.PaginationToken"]
    """<p>List export errors response next token.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListExportErrorsResponse) -> dict:
    out: dict = {}
    if "items" in value:
        import capo_mgn.types.export_errors

        out["items"] = capo_mgn.types.export_errors.serialize_json(value["items"])
    if "next_token" in value:
        out["nextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListExportErrorsResponse:
    out: ListExportErrorsResponse = {}  # type: ignore[typeddict-item]
    if data.get("items") is not None:
        import capo_mgn.types.export_errors

        out["items"] = capo_mgn.types.export_errors.deserialize_json(data["items"])
    if data.get("nextToken") is not None:
        out["next_token"] = data["nextToken"]
    return out
