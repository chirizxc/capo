...
class LifeCycle(TypedDict, closed=True):
    ...
    last_test: NotRequired["capo_mgn.types.life_cycle_last_test.LifeCycleLastTest"]
    """<p>Lifecycle last Test.</p>"""
    last_cutover: NotRequired[
        "capo_mgn.types.life_cycle_last_cutover.LifeCycleLastCutover"
    ]
    """<p>Lifecycle last Cutover.</p>"""
    state: NotRequired["capo_mgn.types.life_cycle_state.LifeCycleState"]
    """<p>Lifecycle state.</p>"""
...
def serialize_json(value: LifeCycle) -> dict:
    ...
    if "last_test" in value:
        ...
        out["lastTest"] = capo_mgn.types.life_cycle_last_test.serialize_json(
            value["last_test"]
        )
    if "last_cutover" in value:
        import capo_mgn.types.life_cycle_last_cutover

        out["lastCutover"] = capo_mgn.types.life_cycle_last_cutover.serialize_json(
            value["last_cutover"]
        )
    if "state" in value:
