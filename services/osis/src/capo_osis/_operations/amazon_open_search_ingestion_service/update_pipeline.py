"""Generated from Smithy shape ``com.amazonaws.osis#UpdatePipeline``."""
...
import capo_osis._protocol.eventstream
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_osis.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
        ...
        case "InternalException":
            raise capo_osis.errors.internal_exception.InternalException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_osis.types.update_pipeline_response.UpdatePipelineResponse = (
        capo_osis.types.update_pipeline_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_osis._auth._sigv4.build_sigv4_auth_scheme(
                "osis", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2022-01-01/osis/updatePipeline/{PipelineName}"
    url = url.replace("{PipelineName}", quote(input_["pipeline_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_osis.types.update_pipeline_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def update_pipeline(
    ...
    capo_osis.types.update_pipeline_response.UpdatePipelineResponse, zapros.Response
]:
