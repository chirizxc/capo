"""Generated from Smithy shape ``com.amazonaws.osis#UntagResource``."""
...
def handle_response(
    ...
    out: capo_osis.types.untag_resource_response.UntagResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2022-01-01/osis/untagResource"
    params: list[tuple[str, str]] = []
    if "arn" in input_:
        params.append(("arn", input_["arn"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_osis.types.untag_resource_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def untag_resource(
    ...
    capo_osis.types.untag_resource_response.UntagResourceResponse, zapros.Response
]:
