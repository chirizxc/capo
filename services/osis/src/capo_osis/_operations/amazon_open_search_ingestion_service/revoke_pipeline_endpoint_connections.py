"""Generated from Smithy shape ``com.amazonaws.osis#RevokePipelineEndpointConnections``."""
...
def handle_response(
    ...
    out: capo_osis.types.revoke_pipeline_endpoint_connections_response.RevokePipelineEndpointConnectionsResponse = capo_osis.types.revoke_pipeline_endpoint_connections_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/2022-01-01/osis/revokePipelineEndpointConnections"
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_osis.types.revoke_pipeline_endpoint_connections_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_revoke_pipeline_endpoint_connections(
