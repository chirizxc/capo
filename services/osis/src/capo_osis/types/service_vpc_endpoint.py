...
class ServiceVpcEndpoint(TypedDict, closed=True):
    service_name: NotRequired[
        "capo_osis.types.vpc_endpoint_service_name.VpcEndpointServiceName"
    ]
...
def serialize_json(value: ServiceVpcEndpoint) -> dict:
    ...
    if "service_name" in value:
        ...
        out["ServiceName"] = capo_osis.types.vpc_endpoint_service_name.serialize_json(
            value["service_name"]
        )
...
def deserialize_json(data: dict) -> ServiceVpcEndpoint:
    ...
    if data.get("ServiceName") is not None:
        ...
        out["service_name"] = (
            capo_osis.types.vpc_endpoint_service_name.deserialize_json(
                data["ServiceName"]
            )
        )
