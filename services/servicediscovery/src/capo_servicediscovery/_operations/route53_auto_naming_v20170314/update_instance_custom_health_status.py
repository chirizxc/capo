...
import capo_servicediscovery.errors.custom_health_not_found
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "CustomHealthNotFound":
            raise capo_servicediscovery.errors.custom_health_not_found.CustomHealthNotFound.from_aws_json_1_1(
                data, message
            )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_servicediscovery.types.update_instance_custom_health_status_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
