...
if TYPE_CHECKING:
    ...
    import capo_lex_model_building_service.types.log_settings_response_list


class ConversationLogsResponse(TypedDict, closed=True):
    log_settings: NotRequired[
        "capo_lex_model_building_service.types.log_settings_response_list.LogSettingsResponseList"
    ]
...
def serialize_json(value: ConversationLogsResponse) -> dict:
    ...
    if "log_settings" in value:
        ...
        out["logSettings"] = (
            capo_lex_model_building_service.types.log_settings_response_list.serialize_json(
                value["log_settings"]
            )
        )
...
def deserialize_json(data: dict) -> ConversationLogsResponse:
    ...
    if data.get("logSettings") is not None:
        ...
        out["log_settings"] = (
            capo_lex_model_building_service.types.log_settings_response_list.deserialize_json(
                data["logSettings"]
            )
        )
