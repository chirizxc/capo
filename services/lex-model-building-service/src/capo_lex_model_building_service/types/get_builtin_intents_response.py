...
class GetBuiltinIntentsResponse(TypedDict, closed=True):
    intents: NotRequired[
        "capo_lex_model_building_service.types.builtin_intent_metadata_list.BuiltinIntentMetadataList"
    ]
...
def serialize_json(value: GetBuiltinIntentsResponse) -> dict:
    ...
    if "intents" in value:
        ...
        out["intents"] = (
            capo_lex_model_building_service.types.builtin_intent_metadata_list.serialize_json(
                value["intents"]
            )
        )
...
def deserialize_json(data: dict) -> GetBuiltinIntentsResponse:
    ...
    if data.get("intents") is not None:
        ...
        out["intents"] = (
            capo_lex_model_building_service.types.builtin_intent_metadata_list.deserialize_json(
                data["intents"]
            )
        )
