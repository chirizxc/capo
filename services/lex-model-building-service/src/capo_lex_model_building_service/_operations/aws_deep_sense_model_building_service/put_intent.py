"""Generated from Smithy shape ``com.amazonaws.lexmodelbuildingservice#PutIntent``."""
...
def handle_response(
    ...
    out: capo_lex_model_building_service.types.put_intent_response.PutIntentResponse = (
        capo_lex_model_building_service.types.put_intent_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/intents/{name}/versions/$LATEST"
    url = url.replace("{name}", quote(input_["name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_model_building_service.types.put_intent_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
