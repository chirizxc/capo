"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#ListMetadataTransferJobs``."""
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.list_metadata_transfer_jobs_response.ListMetadataTransferJobsResponse = capo_iottwinmaker.types.list_metadata_transfer_jobs_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/metadata-transfer-jobs-list"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.list_metadata_transfer_jobs_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_metadata_transfer_jobs(
