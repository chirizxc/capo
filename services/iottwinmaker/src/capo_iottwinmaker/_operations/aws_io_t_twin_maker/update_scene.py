...
def handle_response(
    ...
    out: capo_iottwinmaker.types.update_scene_response.UpdateSceneResponse = (
        capo_iottwinmaker.types.update_scene_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{sceneId}", quote(input_["scene_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.update_scene_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_scene(
    ...
    capo_iottwinmaker.types.update_scene_response.UpdateSceneResponse, zapros.Response
]:
