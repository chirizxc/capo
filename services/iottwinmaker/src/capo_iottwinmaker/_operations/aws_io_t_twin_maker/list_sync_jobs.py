"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#ListSyncJobs``."""
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.list_sync_jobs_response.ListSyncJobsResponse = (
        capo_iottwinmaker.types.list_sync_jobs_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workspaces/{workspaceId}/sync-jobs-list"
    url = url.replace("{workspaceId}", quote(input_["workspace_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.list_sync_jobs_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_sync_jobs(
