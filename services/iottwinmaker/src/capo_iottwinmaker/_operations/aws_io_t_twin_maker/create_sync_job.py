"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#CreateSyncJob``."""
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.create_sync_job_response.CreateSyncJobResponse = (
        capo_iottwinmaker.types.create_sync_job_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{syncSource}", quote(input_["sync_source"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.create_sync_job_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
