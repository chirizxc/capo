"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#GetPropertyValueHistory``."""
...
import capo_iottwinmaker._protocol.eventstream
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.get_property_value_history_response.GetPropertyValueHistoryResponse = capo_iottwinmaker.types.get_property_value_history_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_iottwinmaker._auth._sigv4.build_sigv4_auth_scheme(
                "iottwinmaker", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/workspaces/{workspaceId}/entity-properties/history"
    )
    url = url.replace("{workspaceId}", quote(input_["workspace_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.get_property_value_history_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_property_value_history(
