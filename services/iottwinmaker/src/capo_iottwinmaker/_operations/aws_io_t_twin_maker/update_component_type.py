...
def handle_response(
    ...
    out: capo_iottwinmaker.types.update_component_type_response.UpdateComponentTypeResponse = capo_iottwinmaker.types.update_component_type_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{componentTypeId}", quote(input_["component_type_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.update_component_type_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
