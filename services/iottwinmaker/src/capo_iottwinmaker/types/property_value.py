...
class PropertyValue(TypedDict, closed=True):
    timestamp: NotRequired["capo_iottwinmaker.types.timestamp.Timestamp"]
    """<p>The timestamp of a value for a time series property.</p>"""
    value: "capo_iottwinmaker.types.data_value.DataValue"
    """<p>An object that specifies a value for a time series property.</p>"""
    time: NotRequired["capo_iottwinmaker.types.time.Time"]
...
def serialize_json(value: PropertyValue) -> dict:
    ...
    if "timestamp" in value:
        ...
        out["timestamp"] = capo_iottwinmaker.types.timestamp.serialize_json(
            value["timestamp"]
        )
    import capo_iottwinmaker.types.data_value
