"""Generated from Smithy shape ``com.amazonaws.apigateway#UpdateAuthorizer``."""
...
def handle_response(
    ...
    out: capo_api_gateway.types.authorizer.Authorizer = (
        capo_api_gateway.types.authorizer.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_api_gateway.types.authorizer.Authorizer = (
        capo_api_gateway.types.authorizer.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{authorizerId}", quote(input_["authorizer_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_api_gateway.types.update_authorizer_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_authorizer(
    ...
    input_: capo_api_gateway.types.update_authorizer_request.UpdateAuthorizerRequest,
) -> tuple[capo_api_gateway.types.authorizer.Authorizer, zapros.Response]:
