...
def handle_response(
    ...
    out: capo_api_gateway.types.vpc_link.VpcLink = (
        capo_api_gateway.types.vpc_link.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_api_gateway.types.vpc_link.VpcLink = (
        capo_api_gateway.types.vpc_link.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{vpcLinkId}", quote(input_["vpc_link_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_api_gateway.types.update_vpc_link_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_vpc_link(
    ...
    input_: capo_api_gateway.types.update_vpc_link_request.UpdateVpcLinkRequest,
) -> tuple[capo_api_gateway.types.vpc_link.VpcLink, zapros.Response]:
