"""Generated from Smithy shape ``com.amazonaws.apigateway#UpdateBasePathMapping``."""
...
def build_request(
    ...
    url = url.replace("{basePath}", quote(input_["base_path"], safe=""))
    params: list[tuple[str, str]] = []
    if "domain_name_id" in input_:
        params.append(("domainNameId", input_["domain_name_id"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_api_gateway.types.update_base_path_mapping_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_base_path_mapping(
