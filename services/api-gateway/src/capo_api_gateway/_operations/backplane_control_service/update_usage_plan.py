"""Generated from Smithy shape ``com.amazonaws.apigateway#UpdateUsagePlan``."""
...
def build_request(
    ...
    url = url.replace("{usagePlanId}", quote(input_["usage_plan_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_api_gateway.types.update_usage_plan_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_usage_plan(
