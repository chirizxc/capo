"""Generated from Smithy shape ``com.amazonaws.apigateway#GetApiKeys``."""
...
def handle_response(
    ...
    out: capo_api_gateway.types.api_keys.ApiKeys = (
        capo_api_gateway.types.api_keys.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_api_gateway.types.api_keys.ApiKeys = (
        capo_api_gateway.types.api_keys.deserialize_json(
...
def build_request(
    ...
    if "position" in input_:
        params.append(("position", input_["position"]))
    if "limit" in input_:
        params.append(("limit", str(input_["limit"])))
    if "name_query" in input_:
        params.append(("name", input_["name_query"]))
    if "customer_id" in input_:
        params.append(("customerId", input_["customer_id"]))
    if "include_values" in input_:
        params.append(
            ("includeValues", "true" if input_["include_values"] else "false")
        )
...
def get_api_keys(
    ...
    input_: capo_api_gateway.types.get_api_keys_request.GetApiKeysRequest,
) -> tuple[capo_api_gateway.types.api_keys.ApiKeys, zapros.Response]:
