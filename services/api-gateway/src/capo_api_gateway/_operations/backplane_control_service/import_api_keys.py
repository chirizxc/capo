...
def handle_response(
    ...
    out: capo_api_gateway.types.api_key_ids.ApiKeyIds = (
        capo_api_gateway.types.api_key_ids.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_api_gateway.types.api_key_ids.ApiKeyIds = (
        capo_api_gateway.types.api_key_ids.deserialize_json(
...
def build_request(
    ...
    if "format" in input_:
        params.append(
            (
                "format",
                capo_api_gateway.types.api_keys_format.serialize_json(input_["format"]),
            )
        )
    params.append(
        ("failonwarnings", "true" if input_.get("fail_on_warnings", False) else "false")
    )
...
def import_api_keys(
    ...
    input_: capo_api_gateway.types.import_api_keys_request.ImportApiKeysRequest,
) -> tuple[capo_api_gateway.types.api_key_ids.ApiKeyIds, zapros.Response]:
