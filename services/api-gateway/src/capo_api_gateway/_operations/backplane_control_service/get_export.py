"""Generated from Smithy shape ``com.amazonaws.apigateway#GetExport``."""
...
def handle_response(
    ...
    out: capo_api_gateway.types.export_response.ExportResponse = {
...
def build_request(
    ...
    url = url.replace("{stageName}", quote(input_["stage_name"], safe=""))
    url = url.replace("{exportType}", quote(input_["export_type"], safe=""))
    params: list[tuple[str, str]] = []
    if "parameters" in input_:
        for k, v in input_["parameters"].items():
...
def get_export(
    ...
    input_: capo_api_gateway.types.get_export_request.GetExportRequest,
) -> tuple[capo_api_gateway.types.export_response.ExportResponse, zapros.Response]:
