...
class ApiKey(TypedDict, closed=True):
    ...
    stage_keys: NotRequired["capo_api_gateway.types.list_of_string.ListOfString"]
...
def serialize_json(value: ApiKey) -> dict:
    ...
    if "stage_keys" in value:
        ...
        out["stageKeys"] = capo_api_gateway.types.list_of_string.serialize_json(
            value["stage_keys"]
        )
    if "tags" in value:
        import capo_api_gateway.types.map_of_string_to_string
...
def deserialize_json(data: dict) -> ApiKey:
    ...
    if data.get("stageKeys") is not None:
        ...
        out["stage_keys"] = capo_api_gateway.types.list_of_string.deserialize_json(
            data["stageKeys"]
        )
