...
def serialize_json(value: DomainNameAccessAssociations) -> dict:
    ...
    if "items" in value:
        ...
        out["item"] = (
            capo_api_gateway.types.list_of_domain_name_access_association.serialize_json(
                value["items"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> DomainNameAccessAssociations:
    ...
    if data.get("item") is not None:
        ...
        out["items"] = (
            capo_api_gateway.types.list_of_domain_name_access_association.deserialize_json(
                data["item"]
            )
        )
    return out
