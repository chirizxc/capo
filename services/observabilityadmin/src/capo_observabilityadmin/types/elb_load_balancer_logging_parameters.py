...
class ELBLoadBalancerLoggingParameters(TypedDict, closed=True):
    output_format: NotRequired[
        "capo_observabilityadmin.types.output_format.OutputFormat"
    ]
...
def serialize_json(value: ELBLoadBalancerLoggingParameters) -> dict:
    ...
    if "output_format" in value:
        ...
        out["OutputFormat"] = (
            capo_observabilityadmin.types.output_format.serialize_json(
                value["output_format"]
            )
        )
...
def deserialize_json(data: dict) -> ELBLoadBalancerLoggingParameters:
    ...
    if data.get("OutputFormat") is not None:
        ...
        out["output_format"] = (
            capo_observabilityadmin.types.output_format.deserialize_json(
                data["OutputFormat"]
            )
        )
