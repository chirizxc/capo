"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#GetTelemetryPipeline``."""
...
def handle_response(
    ...
    out: capo_observabilityadmin.types.get_telemetry_pipeline_output.GetTelemetryPipelineOutput = capo_observabilityadmin.types.get_telemetry_pipeline_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetTelemetryPipeline"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.get_telemetry_pipeline_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_telemetry_pipeline(
