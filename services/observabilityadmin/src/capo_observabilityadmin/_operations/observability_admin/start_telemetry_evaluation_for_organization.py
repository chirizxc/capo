"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#StartTelemetryEvaluationForOrganization``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartTelemetryEvaluationForOrganization"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.start_telemetry_evaluation_for_organization_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_start_telemetry_evaluation_for_organization(
