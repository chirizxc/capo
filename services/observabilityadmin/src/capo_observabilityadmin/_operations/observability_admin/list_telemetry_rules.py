"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#ListTelemetryRules``."""
...
def handle_response(
    ...
    out: capo_observabilityadmin.types.list_telemetry_rules_output.ListTelemetryRulesOutput = capo_observabilityadmin.types.list_telemetry_rules_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListTelemetryRules"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.list_telemetry_rules_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_telemetry_rules(
