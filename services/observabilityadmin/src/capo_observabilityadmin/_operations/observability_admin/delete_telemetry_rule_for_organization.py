"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#DeleteTelemetryRuleForOrganization``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteTelemetryRuleForOrganization"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.delete_telemetry_rule_for_organization_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_delete_telemetry_rule_for_organization(
