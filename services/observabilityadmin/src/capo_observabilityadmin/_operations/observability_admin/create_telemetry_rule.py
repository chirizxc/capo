"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#CreateTelemetryRule``."""
...
def handle_response(
    ...
    out: capo_observabilityadmin.types.create_telemetry_rule_output.CreateTelemetryRuleOutput = capo_observabilityadmin.types.create_telemetry_rule_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateTelemetryRule"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.create_telemetry_rule_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_telemetry_rule(
