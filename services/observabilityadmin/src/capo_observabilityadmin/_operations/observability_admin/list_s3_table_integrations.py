"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#ListS3TableIntegrations``."""
...
def handle_response(
    ...
    out: capo_observabilityadmin.types.list_s3_table_integrations_output.ListS3TableIntegrationsOutput = capo_observabilityadmin.types.list_s3_table_integrations_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListS3TableIntegrations"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.list_s3_table_integrations_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_s3_table_integrations(
