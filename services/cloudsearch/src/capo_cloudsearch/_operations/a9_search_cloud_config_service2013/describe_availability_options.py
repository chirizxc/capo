"""Generated from Smithy shape ``com.amazonaws.cloudsearch#DescribeAvailabilityOptions``."""
...
import capo_cloudsearch._protocol.eventstream
...
from capo_cloudsearch._protocol.errors import find_error_element, parse_error_metadata
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "BaseException":
            raise capo_cloudsearch.errors.base_exception.BaseException.from_query(
                error_el, message
            )
        case "DisabledAction":
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeAvailabilityOptionsResult")
    out: capo_cloudsearch.types.describe_availability_options_response.DescribeAvailabilityOptionsResponse = capo_cloudsearch.types.describe_availability_options_response.deserialize_query(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_cloudsearch._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeAvailabilityOptions"))
    pairs.append(("Version", "2013-01-01"))
    capo_cloudsearch.types.describe_availability_options_request.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_availability_options(
