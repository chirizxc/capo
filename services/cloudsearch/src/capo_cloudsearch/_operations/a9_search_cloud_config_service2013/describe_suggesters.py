"""Generated from Smithy shape ``com.amazonaws.cloudsearch#DescribeSuggesters``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeSuggestersResult")
    out: capo_cloudsearch.types.describe_suggesters_response.DescribeSuggestersResponse = capo_cloudsearch.types.describe_suggesters_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeSuggesters"))
    pairs.append(("Version", "2013-01-01"))
    capo_cloudsearch.types.describe_suggesters_request.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_suggesters(
