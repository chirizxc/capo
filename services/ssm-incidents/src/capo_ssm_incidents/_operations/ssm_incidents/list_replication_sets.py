"""Generated from Smithy shape ``com.amazonaws.ssmincidents#ListReplicationSets``."""
...
import capo_ssm_incidents._protocol.eventstream
...
def handle_response(
    ...
    out: capo_ssm_incidents.types.list_replication_sets_output.ListReplicationSetsOutput = capo_ssm_incidents.types.list_replication_sets_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_ssm_incidents._auth._sigv4.build_sigv4_auth_scheme(
                "ssm-incidents", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listReplicationSets"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_incidents.types.list_replication_sets_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_replication_sets(
