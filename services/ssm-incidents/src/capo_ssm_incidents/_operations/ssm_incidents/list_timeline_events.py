"""Generated from Smithy shape ``com.amazonaws.ssmincidents#ListTimelineEvents``."""
...
def handle_response(
    ...
    out: capo_ssm_incidents.types.list_timeline_events_output.ListTimelineEventsOutput = capo_ssm_incidents.types.list_timeline_events_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listTimelineEvents"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_incidents.types.list_timeline_events_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_timeline_events(
