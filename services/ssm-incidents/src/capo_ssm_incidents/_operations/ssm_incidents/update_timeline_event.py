"""Generated from Smithy shape ``com.amazonaws.ssmincidents#UpdateTimelineEvent``."""
...
def handle_response(
    ...
    out: capo_ssm_incidents.types.update_timeline_event_output.UpdateTimelineEventOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/updateTimelineEvent"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_incidents.types.update_timeline_event_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_timeline_event(
