"""Generated from Smithy shape ``com.amazonaws.ssmincidents#UpdateIncidentRecord``."""
...
def handle_response(
    ...
    out: capo_ssm_incidents.types.update_incident_record_output.UpdateIncidentRecordOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/updateIncidentRecord"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_incidents.types.update_incident_record_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_incident_record(
