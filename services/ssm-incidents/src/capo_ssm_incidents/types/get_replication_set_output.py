...
class GetReplicationSetOutput(TypedDict, closed=True):
    replication_set: "capo_ssm_incidents.types.replication_set.ReplicationSet"
...
def serialize_json(value: GetReplicationSetOutput) -> dict:
    ...
    out["replicationSet"] = capo_ssm_incidents.types.replication_set.serialize_json(
...
def deserialize_json(data: dict) -> GetReplicationSetOutput:
    ...
    if data.get("replicationSet") is not None:
        ...
        out["replication_set"] = (
            capo_ssm_incidents.types.replication_set.deserialize_json(
                data["replicationSet"]
            )
        )
    else:
