"""Generated from Smithy shape ``com.amazonaws.fis#ExperimentSummaryList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_fis.types.experiment_summary

ExperimentSummaryList: TypeAlias = list[
    "capo_fis.types.experiment_summary.ExperimentSummary"
]


# --- restJson1 ser/de ---
def serialize_json(value: ExperimentSummaryList) -> list:
    import capo_fis.types.experiment_summary

    out: list = []
    for item in value:
        out.append(capo_fis.types.experiment_summary.serialize_json(item))
    return out


def deserialize_json(data: list) -> ExperimentSummaryList:
    import capo_fis.types.experiment_summary

    out: ExperimentSummaryList = []
    for item in data:
        if item is None:
            continue
        out.append(capo_fis.types.experiment_summary.deserialize_json(item))
    return out
