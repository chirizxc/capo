...
if TYPE_CHECKING:
    import capo_fis.types.report_configuration_s3_output


class ExperimentTemplateReportConfigurationOutputs(TypedDict, closed=True):
    s3_configuration: NotRequired[
        "capo_fis.types.report_configuration_s3_output.ReportConfigurationS3Output"
    ]
...
def serialize_json(value: ExperimentTemplateReportConfigurationOutputs) -> dict:
    ...
    if "s3_configuration" in value:
        ...
        out["s3Configuration"] = (
            capo_fis.types.report_configuration_s3_output.serialize_json(
...
def deserialize_json(data: dict) -> ExperimentTemplateReportConfigurationOutputs:
    ...
    if data.get("s3Configuration") is not None:
        ...
        out["s3_configuration"] = (
            capo_fis.types.report_configuration_s3_output.deserialize_json(
