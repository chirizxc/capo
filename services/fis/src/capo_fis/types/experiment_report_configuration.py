...
if TYPE_CHECKING:
    import capo_fis.types.experiment_report_configuration_data_sources
    import capo_fis.types.experiment_report_configuration_outputs
    import capo_fis.types.report_configuration_duration


class ExperimentReportConfiguration(TypedDict, closed=True):
    outputs: NotRequired[
        "capo_fis.types.experiment_report_configuration_outputs.ExperimentReportConfigurationOutputs"
    ]
    """<p>The output destinations of the experiment report.</p>"""
    data_sources: NotRequired[
        "capo_fis.types.experiment_report_configuration_data_sources.ExperimentReportConfigurationDataSources"
    ]
...
def serialize_json(value: ExperimentReportConfiguration) -> dict:
    ...
    if "outputs" in value:
        ...
        out["outputs"] = (
            capo_fis.types.experiment_report_configuration_outputs.serialize_json(
                value["outputs"]
            )
        )
    if "data_sources" in value:
        ...
        out["dataSources"] = (
            capo_fis.types.experiment_report_configuration_data_sources.serialize_json(
                value["data_sources"]
            )
        )
...
def deserialize_json(data: dict) -> ExperimentReportConfiguration:
    ...
    if data.get("outputs") is not None:
        ...
        out["outputs"] = (
            capo_fis.types.experiment_report_configuration_outputs.deserialize_json(
                data["outputs"]
            )
        )
    if data.get("dataSources") is not None:
        ...
        out["data_sources"] = (
            capo_fis.types.experiment_report_configuration_data_sources.deserialize_json(
                data["dataSources"]
            )
        )
