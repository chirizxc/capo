...
if TYPE_CHECKING:
    import capo_eks.types.oidc_identity_provider_config


class IdentityProviderConfigResponse(TypedDict, closed=True):
    oidc: NotRequired[
        "capo_eks.types.oidc_identity_provider_config.OidcIdentityProviderConfig"
    ]
...
def serialize_json(value: IdentityProviderConfigResponse) -> dict:
    ...
    if "oidc" in value:
        ...
        out["oidc"] = capo_eks.types.oidc_identity_provider_config.serialize_json(
            value["oidc"]
        )
    return out
...
def deserialize_json(data: dict) -> IdentityProviderConfigResponse:
    ...
    if data.get("oidc") is not None:
        ...
        out["oidc"] = capo_eks.types.oidc_identity_provider_config.deserialize_json(
            data["oidc"]
        )
    return out
