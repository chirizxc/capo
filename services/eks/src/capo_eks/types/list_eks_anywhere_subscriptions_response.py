...
class ListEksAnywhereSubscriptionsResponse(TypedDict, closed=True):
    subscriptions: NotRequired[
        "capo_eks.types.eks_anywhere_subscription_list.EksAnywhereSubscriptionList"
    ]
...
def serialize_json(value: ListEksAnywhereSubscriptionsResponse) -> dict:
    ...
    if "subscriptions" in value:
        ...
        out["subscriptions"] = (
            capo_eks.types.eks_anywhere_subscription_list.serialize_json(
                value["subscriptions"]
            )
        )
...
def deserialize_json(data: dict) -> ListEksAnywhereSubscriptionsResponse:
    ...
    if data.get("subscriptions") is not None:
        ...
        out["subscriptions"] = (
            capo_eks.types.eks_anywhere_subscription_list.deserialize_json(
                data["subscriptions"]
            )
        )
