...
class ListAccessEntriesResponse(TypedDict, closed=True):
    access_entries: NotRequired["capo_eks.types.string_list.StringList"]
...
def serialize_json(value: ListAccessEntriesResponse) -> dict:
    ...
    if "access_entries" in value:
        ...
        out["accessEntries"] = capo_eks.types.string_list.serialize_json(
...
def deserialize_json(data: dict) -> ListAccessEntriesResponse:
    ...
    if data.get("accessEntries") is not None:
        import capo_eks.types.string_list

        out["access_entries"] = capo_eks.types.string_list.deserialize_json(
            data["accessEntries"]
        )
