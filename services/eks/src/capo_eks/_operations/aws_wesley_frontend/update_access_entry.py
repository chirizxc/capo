...
def handle_response(
    ...
    out: capo_eks.types.update_access_entry_response.UpdateAccessEntryResponse = (
        capo_eks.types.update_access_entry_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{principalArn}", quote(input_["principal_arn"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_eks.types.update_access_entry_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
