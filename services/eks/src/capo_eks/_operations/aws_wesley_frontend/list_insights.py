"""Generated from Smithy shape ``com.amazonaws.eks#ListInsights``."""
...
def handle_response(
    ...
    out: capo_eks.types.list_insights_response.ListInsightsResponse = (
        capo_eks.types.list_insights_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/clusters/{clusterName}/insights"
    url = url.replace("{clusterName}", quote(input_["cluster_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_eks.types.list_insights_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def list_insights(
    ...
    input_: capo_eks.types.list_insights_request.ListInsightsRequest,
) -> tuple[capo_eks.types.list_insights_response.ListInsightsResponse, zapros.Response]:
