"""Generated from Smithy shape ``com.amazonaws.redshift#RestoreTableFromClusterSnapshot``."""
...
import capo_redshift.errors.in_progress_table_restore_quota_exceeded_fault
import capo_redshift.errors.invalid_cluster_snapshot_state_fault
import capo_redshift.errors.invalid_cluster_state_fault
import capo_redshift.errors.invalid_table_restore_argument_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InProgressTableRestoreQuotaExceededFault":
            raise capo_redshift.errors.in_progress_table_restore_quota_exceeded_fault.InProgressTableRestoreQuotaExceededFault.from_query(
                error_el, message
            )
        ...
        case "InvalidTableRestoreArgument":
            raise capo_redshift.errors.invalid_table_restore_argument_fault.InvalidTableRestoreArgumentFault.from_query(
                error_el, message
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("RestoreTableFromClusterSnapshotResult")
    out: capo_redshift.types.restore_table_from_cluster_snapshot_result.RestoreTableFromClusterSnapshotResult = capo_redshift.types.restore_table_from_cluster_snapshot_result.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "RestoreTableFromClusterSnapshot"))
    pairs.append(("Version", "2012-12-01"))
    capo_redshift.types.restore_table_from_cluster_snapshot_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_restore_table_from_cluster_snapshot(
