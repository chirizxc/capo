"""Generated from Smithy shape ``com.amazonaws.redshift#DescribeResize``."""
...
import capo_redshift.errors.resize_not_found_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResizeNotFound":
            raise capo_redshift.errors.resize_not_found_fault.ResizeNotFoundFault.from_query(
                error_el, message
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeResizeResult")
    out: capo_redshift.types.resize_progress_message.ResizeProgressMessage = (
        capo_redshift.types.resize_progress_message.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeResize"))
    pairs.append(("Version", "2012-12-01"))
    capo_redshift.types.describe_resize_message.serialize_query(input_, pairs, "")
...
def describe_resize(
    ...
    capo_redshift.types.resize_progress_message.ResizeProgressMessage, zapros.Response
]:
...
async def async_describe_resize(
