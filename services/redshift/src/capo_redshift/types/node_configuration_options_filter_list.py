...
if TYPE_CHECKING:
    import capo_redshift.types.node_configuration_options_filter

NodeConfigurationOptionsFilterList: TypeAlias = list[
    "capo_redshift.types.node_configuration_options_filter.NodeConfigurationOptionsFilter"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_redshift.types.node_configuration_options_filter.serialize_query(
...
def deserialize_query(el: Element) -> NodeConfigurationOptionsFilterList:
    ...
    for child in el.findall("NodeConfigurationOptionsFilter"):
        out.append(
            capo_redshift.types.node_configuration_options_filter.deserialize_query(
                child
            )
        )
    return out
