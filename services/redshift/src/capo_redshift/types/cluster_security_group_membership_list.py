...
if TYPE_CHECKING:
    import capo_redshift.types.cluster_security_group_membership

ClusterSecurityGroupMembershipList: TypeAlias = list[
    "capo_redshift.types.cluster_security_group_membership.ClusterSecurityGroupMembership"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_redshift.types.cluster_security_group_membership.serialize_query(
...
def deserialize_query(el: Element) -> ClusterSecurityGroupMembershipList:
    ...
    for child in el.findall("ClusterSecurityGroup"):
        out.append(
            capo_redshift.types.cluster_security_group_membership.deserialize_query(
                child
            )
        )
    return out
