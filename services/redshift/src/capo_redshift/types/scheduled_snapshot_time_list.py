...
ScheduledSnapshotTimeList: TypeAlias = list["capo_redshift.types.t_stamp.TStamp"]
...
def deserialize_query(el: Element) -> ScheduledSnapshotTimeList:
    ...
    for child in el.findall("SnapshotTime"):
        out.append(capo_redshift.types.t_stamp.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    if not value:
        ...
        return
    for n, item in enumerate(value, 1):
        capo_redshift.types.t_stamp.serialize_query(item, pairs, f"{prefix}.{n}")
