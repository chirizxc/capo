"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#DescribeInboundCrossClusterSearchConnections``."""
...
import capo_elasticsearch_service.errors.invalid_pagination_token_exception
...
import capo_elasticsearch_service.types.inbound_cross_cluster_search_connections
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidPaginationTokenException":
            raise capo_elasticsearch_service.errors.invalid_pagination_token_exception.InvalidPaginationTokenException.from_json(
                data, message
            )
        case _:
...
def handle_response(
    ...
    out: capo_elasticsearch_service.types.describe_inbound_cross_cluster_search_connections_response.DescribeInboundCrossClusterSearchConnectionsResponse = capo_elasticsearch_service.types.describe_inbound_cross_cluster_search_connections_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2015-01-01/es/ccs/inboundConnection/search"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elasticsearch_service.types.describe_inbound_cross_cluster_search_connections_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_describe_inbound_cross_cluster_search_connections(
