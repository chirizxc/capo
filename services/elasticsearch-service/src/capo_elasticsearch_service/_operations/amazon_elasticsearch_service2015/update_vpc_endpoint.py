"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#UpdateVpcEndpoint``."""
...
def handle_response(
    ...
    out: capo_elasticsearch_service.types.update_vpc_endpoint_response.UpdateVpcEndpointResponse = capo_elasticsearch_service.types.update_vpc_endpoint_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2015-01-01/es/vpcEndpoints/update"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elasticsearch_service.types.update_vpc_endpoint_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
