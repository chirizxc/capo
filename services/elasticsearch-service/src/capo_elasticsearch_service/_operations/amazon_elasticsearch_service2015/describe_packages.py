"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#DescribePackages``."""
...
def handle_response(
    ...
    out: capo_elasticsearch_service.types.describe_packages_response.DescribePackagesResponse = capo_elasticsearch_service.types.describe_packages_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2015-01-01/packages/describe"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elasticsearch_service.types.describe_packages_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
