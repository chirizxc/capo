...
if TYPE_CHECKING:
    ...
    import capo_elasticsearch_service.types.outbound_cross_cluster_search_connection_status_code


class OutboundCrossClusterSearchConnectionStatus(TypedDict, closed=True):
    status_code: NotRequired[
        "capo_elasticsearch_service.types.outbound_cross_cluster_search_connection_status_code.OutboundCrossClusterSearchConnectionStatusCode"
    ]
...
def serialize_json(value: OutboundCrossClusterSearchConnectionStatus) -> dict:
    ...
    if "status_code" in value:
        ...
        out["StatusCode"] = (
            capo_elasticsearch_service.types.outbound_cross_cluster_search_connection_status_code.serialize_json(
                value["status_code"]
            )
        )
    if "message" in value:
...
def deserialize_json(data: dict) -> OutboundCrossClusterSearchConnectionStatus:
    ...
    if data.get("StatusCode") is not None:
        ...
        out["status_code"] = (
            capo_elasticsearch_service.types.outbound_cross_cluster_search_connection_status_code.deserialize_json(
                data["StatusCode"]
            )
        )
