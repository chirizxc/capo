"""Generated from Smithy shape ``com.amazonaws.notifications#AssociateManagedNotificationAccountContact``."""
...
def handle_response(
    ...
    out: capo_notifications.types.associate_managed_notification_account_contact_response.AssociateManagedNotificationAccountContactResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{contactIdentifier}", quote(input_["contact_identifier"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_notifications.types.associate_managed_notification_account_contact_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_associate_managed_notification_account_contact(
