"""Generated from Smithy shape ``com.amazonaws.codegurusecurity#Remediation``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_codeguru_security.types.recommendation
    import capo_codeguru_security.types.suggested_fixes


class Remediation(TypedDict, closed=True):
    recommendation: NotRequired[
        "capo_codeguru_security.types.recommendation.Recommendation"
    ]
    """<p>An object that contains information about the recommended course of action to remediate a finding.</p>"""
    suggested_fixes: NotRequired[
        "capo_codeguru_security.types.suggested_fixes.SuggestedFixes"
    ]
    """<p>A list of <code>SuggestedFix</code> objects. Each object contains information about a suggested code fix to remediate the finding.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: Remediation) -> dict:
    out: dict = {}
    if "recommendation" in value:
        import capo_codeguru_security.types.recommendation

        out["recommendation"] = (
            capo_codeguru_security.types.recommendation.serialize_json(
                value["recommendation"]
            )
        )
    if "suggested_fixes" in value:
        import capo_codeguru_security.types.suggested_fixes

        out["suggestedFixes"] = (
            capo_codeguru_security.types.suggested_fixes.serialize_json(
                value["suggested_fixes"]
            )
        )
    return out


def deserialize_json(data: dict) -> Remediation:
    out: Remediation = {}  # type: ignore[typeddict-item]
    if data.get("recommendation") is not None:
        import capo_codeguru_security.types.recommendation

        out["recommendation"] = (
            capo_codeguru_security.types.recommendation.deserialize_json(
                data["recommendation"]
            )
        )
    if data.get("suggestedFixes") is not None:
        import capo_codeguru_security.types.suggested_fixes

        out["suggested_fixes"] = (
            capo_codeguru_security.types.suggested_fixes.deserialize_json(
                data["suggestedFixes"]
            )
        )
    return out
