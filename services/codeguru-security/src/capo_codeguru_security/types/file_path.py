...
class FilePath(TypedDict, closed=True):
    ...
    code_snippet: NotRequired["capo_codeguru_security.types.code_snippet.CodeSnippet"]
...
def serialize_json(value: FilePath) -> dict:
    ...
    if "code_snippet" in value:
        ...
        out["codeSnippet"] = capo_codeguru_security.types.code_snippet.serialize_json(
            value["code_snippet"]
        )
    return out
...
def deserialize_json(data: dict) -> FilePath:
    ...
    if data.get("codeSnippet") is not None:
        ...
        out["code_snippet"] = (
            capo_codeguru_security.types.code_snippet.deserialize_json(
                data["codeSnippet"]
            )
        )
    return out
