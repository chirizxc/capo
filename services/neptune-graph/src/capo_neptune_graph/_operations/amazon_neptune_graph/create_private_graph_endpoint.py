"""Generated from Smithy shape ``com.amazonaws.neptunegraph#CreatePrivateGraphEndpoint``."""
...
def handle_response(
    ...
    out: capo_neptune_graph.types.create_private_graph_endpoint_output.CreatePrivateGraphEndpointOutput = capo_neptune_graph.types.create_private_graph_endpoint_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{graphIdentifier}", quote(input_["graph_identifier"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_neptune_graph.types.create_private_graph_endpoint_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_private_graph_endpoint(
