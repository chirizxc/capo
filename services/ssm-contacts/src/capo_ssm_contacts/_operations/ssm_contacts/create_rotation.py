"""Generated from Smithy shape ``com.amazonaws.ssmcontacts#CreateRotation``."""
...
import capo_ssm_contacts._protocol.eventstream
...
def handle_response(
    ...
    out: capo_ssm_contacts.types.create_rotation_result.CreateRotationResult = (
        capo_ssm_contacts.types.create_rotation_result.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_ssm_contacts._auth._sigv4.build_sigv4_auth_scheme(
                "ssm-contacts", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "SSMContacts.CreateRotation"
    body: bytes | None = json.dumps(
        capo_ssm_contacts.types.create_rotation_request.serialize_aws_json_1_1(input_),
        allow_nan=False,
    ).encode()
...
def create_rotation(
    ...
    capo_ssm_contacts.types.create_rotation_result.CreateRotationResult, zapros.Response
]:
...
async def async_create_rotation(
