"""Generated from Smithy shape ``com.amazonaws.comprehend#StartTopicsDetectionJobRequest``."""
...
class StartTopicsDetectionJobRequest(TypedDict, closed=True):
    ...
    r"""<p>Configuration parameters for an optional private Virtual Private Cloud (VPC) containing the resources you are using for your topic detection job. For more information, see <a href=\"https://docs.aws.amazon.com/vpc/latest/userguide/what-is-amazon-vpc.html\">Amazon VPC</a>. </p>"""
    tags: NotRequired["capo_comprehend.types.tag_list.TagList"]
    r"""<p>Tags to associate with the topics detection job. A tag is a key-value pair that adds metadata to a resource used by Amazon Comprehend. For example, a tag with \"Sales\" as the key might be added to a resource to indicate its use by the sales department.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: StartTopicsDetectionJobRequest) -> dict:
    out: dict = {}
    import capo_comprehend.types.input_data_config
...
def deserialize_aws_json_1_1(data: dict) -> StartTopicsDetectionJobRequest:
    out: StartTopicsDetectionJobRequest = {}  # type: ignore[typeddict-item]
    if data.get("InputDataConfig") is not None:
        ...
        raise DeserializationError(
            "StartTopicsDetectionJobRequest.input_data_config required"
        )
    if data.get("OutputDataConfig") is not None:
        ...
        raise DeserializationError(
            "StartTopicsDetectionJobRequest.output_data_config required"
        )
    if data.get("DataAccessRoleArn") is not None:
        out["data_access_role_arn"] = data["DataAccessRoleArn"]
    else:
        raise DeserializationError(
            "StartTopicsDetectionJobRequest.data_access_role_arn required"
        )
    if data.get("JobName") is not None:
        out["job_name"] = data["JobName"]
    if data.get("NumberOfTopics") is not None:
