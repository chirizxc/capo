"""Generated from Smithy shape ``com.amazonaws.omics#CreateSequenceStore``."""
...
def handle_response(
    ...
    out: capo_omics.types.create_sequence_store_response.CreateSequenceStoreResponse = (
        capo_omics.types.create_sequence_store_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sequencestore"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_omics.types.create_sequence_store_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_sequence_store(
