"""Generated from Smithy shape ``com.amazonaws.omics#CreateMultipartReadSetUpload``."""
...
def handle_response(
    ...
    out: capo_omics.types.create_multipart_read_set_upload_response.CreateMultipartReadSetUploadResponse = capo_omics.types.create_multipart_read_set_upload_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sequencestore/{sequenceStoreId}/upload"
    url = url.replace("{sequenceStoreId}", quote(input_["sequence_store_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_omics.types.create_multipart_read_set_upload_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_multipart_read_set_upload(
