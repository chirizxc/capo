"""Generated from Smithy shape ``com.amazonaws.omics#CancelRunBatch``."""
...
def handle_response(
    ...
    out: capo_omics.types.cancel_run_batch_response.CancelRunBatchResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/runBatch/cancel"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_omics.types.cancel_run_batch_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def cancel_run_batch(
    ...
    capo_omics.types.cancel_run_batch_response.CancelRunBatchResponse, zapros.Response
]:
...
async def async_cancel_run_batch(
