"""Generated from Smithy shape ``com.amazonaws.omics#PutS3AccessPolicy``."""
...
def handle_response(
    ...
    out: capo_omics.types.put_s3_access_policy_response.PutS3AccessPolicyResponse = (
        capo_omics.types.put_s3_access_policy_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{s3AccessPointArn}", quote(input_["s3_access_point_arn"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_omics.types.put_s3_access_policy_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_put_s3_access_policy(
