...
class ListRunCachesResponse(TypedDict, closed=True):
    items: NotRequired["capo_omics.types.run_cache_list.RunCacheList"]
...
def serialize_json(value: ListRunCachesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_omics.types.run_cache_list.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> ListRunCachesResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = capo_omics.types.run_cache_list.deserialize_json(data["items"])
