...
class ListMultipartReadSetUploadsResponse(TypedDict, closed=True):
    ...
    uploads: NotRequired[
        "capo_omics.types.multipart_read_set_upload_list.MultipartReadSetUploadList"
    ]
...
def serialize_json(value: ListMultipartReadSetUploadsResponse) -> dict:
    ...
    if "uploads" in value:
        ...
        out["uploads"] = capo_omics.types.multipart_read_set_upload_list.serialize_json(
            value["uploads"]
        )
    return out
...
def deserialize_json(data: dict) -> ListMultipartReadSetUploadsResponse:
    ...
    if data.get("uploads") is not None:
        ...
        out["uploads"] = (
            capo_omics.types.multipart_read_set_upload_list.deserialize_json(
                data["uploads"]
            )
        )
    return out
