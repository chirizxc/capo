"""Generated from Smithy shape ``com.amazonaws.applicationsignals#ListServiceLevelObjectives``."""
...
def handle_response(
    ...
    out: capo_application_signals.types.list_service_level_objectives_output.ListServiceLevelObjectivesOutput = capo_application_signals.types.list_service_level_objectives_output.deserialize_json(
...
def build_request(
    ...
    if "operation_name" in input_:
        params.append(("OperationName", input_["operation_name"]))
    ...
    if "slo_owner_aws_account_id" in input_:
        params.append(("SloOwnerAwsAccountId", input_["slo_owner_aws_account_id"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_application_signals.types.list_service_level_objectives_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
