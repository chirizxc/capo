"""Generated from Smithy shape ``com.amazonaws.applicationsignals#BatchUpdateExclusionWindows``."""
...
def handle_response(
    ...
    out: capo_application_signals.types.batch_update_exclusion_windows_output.BatchUpdateExclusionWindowsOutput = capo_application_signals.types.batch_update_exclusion_windows_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/exclusion-windows"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_application_signals.types.batch_update_exclusion_windows_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_update_exclusion_windows(
