...
class Edge(TypedDict, closed=True):
    ...
    connection_type: NotRequired[
        "capo_application_signals.types.connection_type.ConnectionType"
    ]
...
def serialize_json(value: Edge) -> dict:
    ...
    if "connection_type" in value:
        ...
        out["ConnectionType"] = (
            capo_application_signals.types.connection_type.serialize_json(
...
def deserialize_json(data: dict) -> Edge:
    ...
    if data.get("ConnectionType") is not None:
        ...
        out["connection_type"] = (
            capo_application_signals.types.connection_type.deserialize_json(
                data["ConnectionType"]
            )
        )
    return out
