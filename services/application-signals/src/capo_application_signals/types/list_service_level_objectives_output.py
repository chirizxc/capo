...
class ListServiceLevelObjectivesOutput(TypedDict, closed=True):
    slo_summaries: NotRequired[
        "capo_application_signals.types.service_level_objective_summaries.ServiceLevelObjectiveSummaries"
    ]
...
def serialize_json(value: ListServiceLevelObjectivesOutput) -> dict:
    ...
    if "slo_summaries" in value:
        ...
        out["SloSummaries"] = (
            capo_application_signals.types.service_level_objective_summaries.serialize_json(
                value["slo_summaries"]
            )
        )
...
def deserialize_json(data: dict) -> ListServiceLevelObjectivesOutput:
    ...
    if data.get("SloSummaries") is not None:
        ...
        out["slo_summaries"] = (
            capo_application_signals.types.service_level_objective_summaries.deserialize_json(
                data["SloSummaries"]
            )
        )
