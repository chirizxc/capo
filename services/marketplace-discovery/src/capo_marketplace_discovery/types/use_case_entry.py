...
class UseCaseEntry(TypedDict, closed=True):
    use_case: "capo_marketplace_discovery.types.use_case.UseCase"
...
def serialize_json(value: UseCaseEntry) -> dict:
    ...
    out["useCase"] = capo_marketplace_discovery.types.use_case.serialize_json(
        value["use_case"]
    )
    return out
