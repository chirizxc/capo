"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListBotAliasReplicas``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.list_bot_alias_replicas_response.ListBotAliasReplicasResponse = capo_lex_models_v2.types.list_bot_alias_replicas_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/replicas/{replicaRegion}/botaliases"
    url = url.replace("{botId}", quote(input_["bot_id"], safe=""))
    url = url.replace("{replicaRegion}", quote(input_["replica_region"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.list_bot_alias_replicas_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_bot_alias_replicas(
