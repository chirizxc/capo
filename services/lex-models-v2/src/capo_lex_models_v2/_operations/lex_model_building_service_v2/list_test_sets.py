"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListTestSets``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.list_test_sets_response.ListTestSetsResponse = (
        capo_lex_models_v2.types.list_test_sets_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/testsets"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.list_test_sets_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_test_sets(
