"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListAggregatedUtterances``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.list_aggregated_utterances_response.ListAggregatedUtterancesResponse = capo_lex_models_v2.types.list_aggregated_utterances_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/aggregatedutterances"
    url = url.replace("{botId}", quote(input_["bot_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.list_aggregated_utterances_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_aggregated_utterances(
