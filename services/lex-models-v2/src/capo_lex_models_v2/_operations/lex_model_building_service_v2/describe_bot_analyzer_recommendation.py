"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#DescribeBotAnalyzerRecommendation``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.describe_bot_analyzer_recommendation_response.DescribeBotAnalyzerRecommendationResponse = capo_lex_models_v2.types.describe_bot_analyzer_recommendation_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{botAnalyzerRequestId}", quote(input_["bot_analyzer_request_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.describe_bot_analyzer_recommendation_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_describe_bot_analyzer_recommendation(
