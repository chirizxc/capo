"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#StartTestSetGeneration``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.start_test_set_generation_response.StartTestSetGenerationResponse = capo_lex_models_v2.types.start_test_set_generation_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/testsetgenerations"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.start_test_set_generation_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_start_test_set_generation(
