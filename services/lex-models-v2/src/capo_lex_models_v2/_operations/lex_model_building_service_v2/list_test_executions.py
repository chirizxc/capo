"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListTestExecutions``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.list_test_executions_response.ListTestExecutionsResponse = capo_lex_models_v2.types.list_test_executions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/testexecutions"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.list_test_executions_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_test_executions(
