...
def handle_response(
    ...
    out: capo_lex_models_v2.types.search_associated_transcripts_response.SearchAssociatedTranscriptsResponse = capo_lex_models_v2.types.search_associated_transcripts_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{botRecommendationId}", quote(input_["bot_recommendation_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.search_associated_transcripts_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
