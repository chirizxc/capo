"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListBuiltInSlotTypes``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.list_built_in_slot_types_response.ListBuiltInSlotTypesResponse = capo_lex_models_v2.types.list_built_in_slot_types_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/builtins/locales/{localeId}/slottypes"
    url = url.replace("{localeId}", quote(input_["locale_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.list_built_in_slot_types_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_built_in_slot_types(
