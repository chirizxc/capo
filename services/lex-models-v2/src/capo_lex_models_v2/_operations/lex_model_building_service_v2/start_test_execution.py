"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#StartTestExecution``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.start_test_execution_response.StartTestExecutionResponse = capo_lex_models_v2.types.start_test_execution_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/testsets/{testSetId}/testexecutions"
    url = url.replace("{testSetId}", quote(input_["test_set_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.start_test_execution_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_start_test_execution(
