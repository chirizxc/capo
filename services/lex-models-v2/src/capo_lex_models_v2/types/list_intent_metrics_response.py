...
class ListIntentMetricsResponse(TypedDict, closed=True):
    bot_id: NotRequired["capo_lex_models_v2.types.id.Id"]
    """<p>The identifier for the bot for which you retrieved intent metrics.</p>"""
    results: NotRequired[
        "capo_lex_models_v2.types.analytics_intent_results.AnalyticsIntentResults"
    ]
...
def serialize_json(value: ListIntentMetricsResponse) -> dict:
    ...
    if "results" in value:
        ...
        out["results"] = (
            capo_lex_models_v2.types.analytics_intent_results.serialize_json(
                value["results"]
            )
        )
...
def deserialize_json(data: dict) -> ListIntentMetricsResponse:
    ...
    if data.get("results") is not None:
        ...
        out["results"] = (
            capo_lex_models_v2.types.analytics_intent_results.deserialize_json(
                data["results"]
            )
        )
