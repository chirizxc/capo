...
class TextLogDestination(TypedDict, closed=True):
    cloud_watch: "capo_lex_models_v2.types.cloud_watch_log_group_log_destination.CloudWatchLogGroupLogDestination"
...
def serialize_json(value: TextLogDestination) -> dict:
    ...
    out["cloudWatch"] = (
        capo_lex_models_v2.types.cloud_watch_log_group_log_destination.serialize_json(
            value["cloud_watch"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> TextLogDestination:
    ...
    if data.get("cloudWatch") is not None:
        ...
        out["cloud_watch"] = (
            capo_lex_models_v2.types.cloud_watch_log_group_log_destination.deserialize_json(
                data["cloudWatch"]
            )
        )
    else:
