...
if TYPE_CHECKING:
    import capo_lex_models_v2.types.analytics_node_count
    import capo_lex_models_v2.types.analytics_node_level
...
class AnalyticsIntentNodeSummary(TypedDict, closed=True):
    ...
    intent_path: NotRequired["capo_lex_models_v2.types.analytics_path.AnalyticsPath"]
    """<p>The path.</p>"""
    intent_count: NotRequired[
        "capo_lex_models_v2.types.analytics_node_count.AnalyticsNodeCount"
    ]
    """<p>The total number of sessions that follow the given path to the given intent.</p>"""
    intent_level: NotRequired[
        "capo_lex_models_v2.types.analytics_node_level.AnalyticsNodeLevel"
    ]
    """<p>The number of intents up to and including the requested path.</p>"""
    node_type: NotRequired[
        "capo_lex_models_v2.types.analytics_node_type.AnalyticsNodeType"
    ]
...
def serialize_json(value: AnalyticsIntentNodeSummary) -> dict:
    ...
    if "node_type" in value:
        ...
        out["nodeType"] = capo_lex_models_v2.types.analytics_node_type.serialize_json(
            value["node_type"]
        )
    return out
...
def deserialize_json(data: dict) -> AnalyticsIntentNodeSummary:
    ...
    if data.get("nodeType") is not None:
        ...
        out["node_type"] = (
            capo_lex_models_v2.types.analytics_node_type.deserialize_json(
                data["nodeType"]
            )
        )
    return out
