...
class AnalyticsIntentResult(TypedDict, closed=True):
    ...
    group_by_keys: NotRequired[
        "capo_lex_models_v2.types.analytics_intent_group_by_keys.AnalyticsIntentGroupByKeys"
    ]
    ...
    metrics_results: NotRequired[
        "capo_lex_models_v2.types.analytics_intent_metric_results.AnalyticsIntentMetricResults"
    ]
...
def serialize_json(value: AnalyticsIntentResult) -> dict:
    ...
    if "group_by_keys" in value:
        ...
        out["groupByKeys"] = (
            capo_lex_models_v2.types.analytics_intent_group_by_keys.serialize_json(
                value["group_by_keys"]
            )
        )
    if "metrics_results" in value:
        ...
        out["metricsResults"] = (
            capo_lex_models_v2.types.analytics_intent_metric_results.serialize_json(
...
def deserialize_json(data: dict) -> AnalyticsIntentResult:
    ...
    if data.get("groupByKeys") is not None:
        ...
        out["group_by_keys"] = (
            capo_lex_models_v2.types.analytics_intent_group_by_keys.deserialize_json(
                data["groupByKeys"]
            )
        )
    if data.get("metricsResults") is not None:
        ...
        out["metrics_results"] = (
            capo_lex_models_v2.types.analytics_intent_metric_results.deserialize_json(
                data["metricsResults"]
            )
        )
    return out
