...
class SpeechModelConfig(TypedDict, closed=True):
    deepgram_config: NotRequired[
        "capo_lex_models_v2.types.deepgram_speech_model_config.DeepgramSpeechModelConfig"
    ]
...
def serialize_json(value: SpeechModelConfig) -> dict:
    ...
    if "deepgram_config" in value:
        ...
        out["deepgramConfig"] = (
            capo_lex_models_v2.types.deepgram_speech_model_config.serialize_json(
...
def deserialize_json(data: dict) -> SpeechModelConfig:
    ...
    if data.get("deepgramConfig") is not None:
        ...
        out["deepgram_config"] = (
            capo_lex_models_v2.types.deepgram_speech_model_config.deserialize_json(
                data["deepgramConfig"]
            )
        )
    return out
