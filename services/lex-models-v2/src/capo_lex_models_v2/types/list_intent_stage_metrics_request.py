...
def serialize_json(value: ListIntentStageMetricsRequest) -> dict:
    ...
    out["metrics"] = (
        capo_lex_models_v2.types.analytics_intent_stage_metrics.serialize_json(
            value["metrics"]
        )
    )
    if "bin_by" in value:
    ...
    if "group_by" in value:
        ...
        out["groupBy"] = (
            capo_lex_models_v2.types.analytics_intent_stage_group_by_list.serialize_json(
                value["group_by"]
            )
        )
    if "filters" in value:
        ...
        out["filters"] = (
            capo_lex_models_v2.types.analytics_intent_stage_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListIntentStageMetricsRequest:
    ...
    if data.get("metrics") is not None:
        ...
        out["metrics"] = (
            capo_lex_models_v2.types.analytics_intent_stage_metrics.deserialize_json(
                data["metrics"]
            )
        )
    else:
    ...
    if data.get("groupBy") is not None:
        ...
        out["group_by"] = (
            capo_lex_models_v2.types.analytics_intent_stage_group_by_list.deserialize_json(
                data["groupBy"]
            )
        )
    if data.get("filters") is not None:
        ...
        out["filters"] = (
            capo_lex_models_v2.types.analytics_intent_stage_filters.deserialize_json(
                data["filters"]
            )
        )
