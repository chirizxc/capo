...
if TYPE_CHECKING:
    import capo_lex_models_v2.types.analytics_common_filter_name
...
class AnalyticsPathFilter(TypedDict, closed=True):
    name: "capo_lex_models_v2.types.analytics_common_filter_name.AnalyticsCommonFilterName"
...
def serialize_json(value: AnalyticsPathFilter) -> dict:
    ...
    out["name"] = capo_lex_models_v2.types.analytics_common_filter_name.serialize_json(
        value["name"]
    )
    import capo_lex_models_v2.types.analytics_filter_operator
...
def deserialize_json(data: dict) -> AnalyticsPathFilter:
    ...
    if data.get("name") is not None:
        ...
        out["name"] = (
            capo_lex_models_v2.types.analytics_common_filter_name.deserialize_json(
                data["name"]
            )
        )
    else:
