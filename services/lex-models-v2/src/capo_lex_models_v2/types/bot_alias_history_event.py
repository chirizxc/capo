"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#BotAliasHistoryEvent``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_lex_models_v2.types.bot_version
    import capo_lex_models_v2.types.timestamp


class BotAliasHistoryEvent(TypedDict, closed=True):
    bot_version: NotRequired["capo_lex_models_v2.types.bot_version.BotVersion"]
    """<p>The version of the bot that was used in the event. </p>"""
    start_date: NotRequired["capo_lex_models_v2.types.timestamp.Timestamp"]
    """<p>The date and time that the event started.</p>"""
    end_date: NotRequired["capo_lex_models_v2.types.timestamp.Timestamp"]
    """<p>The date and time that the event ended.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: BotAliasHistoryEvent) -> dict:
    out: dict = {}
    if "bot_version" in value:
        out["botVersion"] = value["bot_version"]
    if "start_date" in value:
        import capo_lex_models_v2.types.timestamp

        out["startDate"] = capo_lex_models_v2.types.timestamp.serialize_json(
            value["start_date"]
        )
    if "end_date" in value:
        import capo_lex_models_v2.types.timestamp

        out["endDate"] = capo_lex_models_v2.types.timestamp.serialize_json(
            value["end_date"]
        )
    return out


def deserialize_json(data: dict) -> BotAliasHistoryEvent:
    out: BotAliasHistoryEvent = {}  # type: ignore[typeddict-item]
    if data.get("botVersion") is not None:
        out["bot_version"] = data["botVersion"]
    if data.get("startDate") is not None:
        import capo_lex_models_v2.types.timestamp

        out["start_date"] = capo_lex_models_v2.types.timestamp.deserialize_json(
            data["startDate"]
        )
    if data.get("endDate") is not None:
        import capo_lex_models_v2.types.timestamp

        out["end_date"] = capo_lex_models_v2.types.timestamp.deserialize_json(
            data["endDate"]
        )
    return out
