...
class ListUtteranceAnalyticsDataResponse(TypedDict, closed=True):
    ...
    utterances: NotRequired[
        "capo_lex_models_v2.types.utterance_specifications.UtteranceSpecifications"
    ]
...
def serialize_json(value: ListUtteranceAnalyticsDataResponse) -> dict:
    ...
    if "utterances" in value:
        ...
        out["utterances"] = (
            capo_lex_models_v2.types.utterance_specifications.serialize_json(
                value["utterances"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListUtteranceAnalyticsDataResponse:
    ...
    if data.get("utterances") is not None:
        ...
        out["utterances"] = (
            capo_lex_models_v2.types.utterance_specifications.deserialize_json(
                data["utterances"]
            )
        )
    return out
