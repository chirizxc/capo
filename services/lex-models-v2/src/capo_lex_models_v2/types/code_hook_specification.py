...
class CodeHookSpecification(TypedDict, closed=True):
    lambda_code_hook: "capo_lex_models_v2.types.lambda_code_hook.LambdaCodeHook"
...
def serialize_json(value: CodeHookSpecification) -> dict:
    ...
    out["lambdaCodeHook"] = capo_lex_models_v2.types.lambda_code_hook.serialize_json(
...
def deserialize_json(data: dict) -> CodeHookSpecification:
    ...
    if data.get("lambdaCodeHook") is not None:
        ...
        out["lambda_code_hook"] = (
            capo_lex_models_v2.types.lambda_code_hook.deserialize_json(
                data["lambdaCodeHook"]
            )
        )
    else:
