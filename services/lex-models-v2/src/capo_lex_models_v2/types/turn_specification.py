...
class TurnSpecification(TypedDict, closed=True):
    agent_turn: NotRequired[
        "capo_lex_models_v2.types.agent_turn_specification.AgentTurnSpecification"
    ]
    """<p>Contains information about the agent messages in the turn.</p>"""
    user_turn: NotRequired[
        "capo_lex_models_v2.types.user_turn_specification.UserTurnSpecification"
    ]
...
def serialize_json(value: TurnSpecification) -> dict:
    ...
    if "agent_turn" in value:
        ...
        out["agentTurn"] = (
            capo_lex_models_v2.types.agent_turn_specification.serialize_json(
                value["agent_turn"]
            )
        )
    if "user_turn" in value:
        ...
        out["userTurn"] = (
            capo_lex_models_v2.types.user_turn_specification.serialize_json(
                value["user_turn"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> TurnSpecification:
    ...
    if data.get("agentTurn") is not None:
        ...
        out["agent_turn"] = (
            capo_lex_models_v2.types.agent_turn_specification.deserialize_json(
                data["agentTurn"]
            )
        )
    if data.get("userTurn") is not None:
        ...
        out["user_turn"] = (
            capo_lex_models_v2.types.user_turn_specification.deserialize_json(
                data["userTurn"]
            )
        )
    return out
