"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ConditionKeyValueMap``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_lex_models_v2.types.condition_key
    import capo_lex_models_v2.types.condition_value

ConditionKeyValueMap: TypeAlias = dict[
    "capo_lex_models_v2.types.condition_key.ConditionKey",
    "capo_lex_models_v2.types.condition_value.ConditionValue",
]


# --- restJson1 ser/de ---
def serialize_json(input_to_serialize: ConditionKeyValueMap) -> dict:
    out: dict = {}
    for key, value in input_to_serialize.items():
        out[key] = value
    return out


def deserialize_json(data: dict) -> ConditionKeyValueMap:
    out: ConditionKeyValueMap = {}
    for key, value in data.items():
        if value is None:
            continue
        out[key] = value
    return out
