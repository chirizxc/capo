"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#Principal``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_lex_models_v2.types.principal_arn
    import capo_lex_models_v2.types.service_principal


class Principal(TypedDict, closed=True):
    service: NotRequired["capo_lex_models_v2.types.service_principal.ServicePrincipal"]
    """<p>The name of the Amazon Web Services service that should allowed or denied access to an Amazon Lex action.</p>"""
    arn: NotRequired["capo_lex_models_v2.types.principal_arn.PrincipalArn"]
    """<p>The Amazon Resource Name (ARN) of the principal.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: Principal) -> dict:
    out: dict = {}
    if "service" in value:
        out["service"] = value["service"]
    if "arn" in value:
        out["arn"] = value["arn"]
    return out


def deserialize_json(data: dict) -> Principal:
    out: Principal = {}  # type: ignore[typeddict-item]
    if data.get("service") is not None:
        out["service"] = data["service"]
    if data.get("arn") is not None:
        out["arn"] = data["arn"]
    return out
