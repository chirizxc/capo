...
class BotLocaleFilter(TypedDict, closed=True):
    name: "capo_lex_models_v2.types.bot_locale_filter_name.BotLocaleFilterName"
    ...
    operator: (
        "capo_lex_models_v2.types.bot_locale_filter_operator.BotLocaleFilterOperator"
    )
...
def serialize_json(value: BotLocaleFilter) -> dict:
    ...
    out["name"] = capo_lex_models_v2.types.bot_locale_filter_name.serialize_json(
        value["name"]
    )
    import capo_lex_models_v2.types.filter_values
    ...
    out["operator"] = (
        capo_lex_models_v2.types.bot_locale_filter_operator.serialize_json(
            value["operator"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> BotLocaleFilter:
    ...
    if data.get("operator") is not None:
        ...
        out["operator"] = (
            capo_lex_models_v2.types.bot_locale_filter_operator.deserialize_json(
                data["operator"]
            )
        )
    else:
