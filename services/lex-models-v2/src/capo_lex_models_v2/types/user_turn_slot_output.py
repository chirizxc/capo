"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#UserTurnSlotOutput``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_lex_models_v2.types.non_empty_string
    import capo_lex_models_v2.types.user_turn_slot_output_list
    import capo_lex_models_v2.types.user_turn_slot_output_map


class UserTurnSlotOutput(TypedDict, closed=True):
    value: NotRequired["capo_lex_models_v2.types.non_empty_string.NonEmptyString"]
    """<p>The value output by the slot recognition.</p>"""
    values: NotRequired[
        "capo_lex_models_v2.types.user_turn_slot_output_list.UserTurnSlotOutputList"
    ]
    """<p>Values that are output by the slot recognition.</p>"""
    sub_slots: NotRequired[
        "capo_lex_models_v2.types.user_turn_slot_output_map.UserTurnSlotOutputMap"
    ]
    """<p>A list of items mapping the name of the subslots to information about those subslots.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: UserTurnSlotOutput) -> dict:
    out: dict = {}
    if "value" in value:
        out["value"] = value["value"]
    if "values" in value:
        import capo_lex_models_v2.types.user_turn_slot_output_list

        out["values"] = (
            capo_lex_models_v2.types.user_turn_slot_output_list.serialize_json(
                value["values"]
            )
        )
    if "sub_slots" in value:
        import capo_lex_models_v2.types.user_turn_slot_output_map

        out["subSlots"] = (
            capo_lex_models_v2.types.user_turn_slot_output_map.serialize_json(
                value["sub_slots"]
            )
        )
    return out


def deserialize_json(data: dict) -> UserTurnSlotOutput:
    out: UserTurnSlotOutput = {}  # type: ignore[typeddict-item]
    if data.get("value") is not None:
        out["value"] = data["value"]
    if data.get("values") is not None:
        import capo_lex_models_v2.types.user_turn_slot_output_list

        out["values"] = (
            capo_lex_models_v2.types.user_turn_slot_output_list.deserialize_json(
                data["values"]
            )
        )
    if data.get("subSlots") is not None:
        import capo_lex_models_v2.types.user_turn_slot_output_map

        out["sub_slots"] = (
            capo_lex_models_v2.types.user_turn_slot_output_map.deserialize_json(
                data["subSlots"]
            )
        )
    return out
