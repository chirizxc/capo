...
class ListServiceViewsOutput(TypedDict, closed=True):
    ...
    service_views: NotRequired[
        "capo_resource_explorer_2.types.service_view_arn_list.ServiceViewArnList"
    ]
...
def serialize_json(value: ListServiceViewsOutput) -> dict:
    ...
    if "service_views" in value:
        ...
        out["ServiceViews"] = (
            capo_resource_explorer_2.types.service_view_arn_list.serialize_json(
                value["service_views"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListServiceViewsOutput:
    ...
    if data.get("ServiceViews") is not None:
        import capo_resource_explorer_2.types.service_view_arn_list

        out["service_views"] = (
            capo_resource_explorer_2.types.service_view_arn_list.deserialize_json(
                data["ServiceViews"]
            )
        )
    return out
