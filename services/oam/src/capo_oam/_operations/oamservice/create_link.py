"""Generated from Smithy shape ``com.amazonaws.oam#CreateLink``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ConflictException":
            raise capo_oam.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_oam.types.create_link_output.CreateLinkOutput = (
        capo_oam.types.create_link_output.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_oam.types.create_link_output.CreateLinkOutput = (
        capo_oam.types.create_link_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateLink"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_oam.types.create_link_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def create_link(
    options: OperationOptions, input_: capo_oam.types.create_link_input.CreateLinkInput
) -> tuple[capo_oam.types.create_link_output.CreateLinkOutput, zapros.Response]:
