"""Generated from Smithy shape ``com.amazonaws.oam#DeleteSink``."""
...
def handle_response(
    ...
    out: capo_oam.types.delete_sink_output.DeleteSinkOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteSink"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_oam.types.delete_sink_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def delete_sink(
    options: OperationOptions, input_: capo_oam.types.delete_sink_input.DeleteSinkInput
) -> tuple[capo_oam.types.delete_sink_output.DeleteSinkOutput, zapros.Response]:
