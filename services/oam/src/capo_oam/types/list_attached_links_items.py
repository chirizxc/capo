...
ListAttachedLinksItems: TypeAlias = list[
    "capo_oam.types.list_attached_links_item.ListAttachedLinksItem"
]
...
def serialize_json(value: ListAttachedLinksItems) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_oam.types.list_attached_links_item.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ListAttachedLinksItems:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_oam.types.list_attached_links_item.deserialize_json(item))
    return out
