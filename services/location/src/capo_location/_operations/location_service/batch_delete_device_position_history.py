"""Generated from Smithy shape ``com.amazonaws.location#BatchDeleteDevicePositionHistory``."""
...
def handle_response(
    ...
    out: capo_location.types.batch_delete_device_position_history_response.BatchDeleteDevicePositionHistoryResponse = capo_location.types.batch_delete_device_position_history_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{TrackerName}", quote(input_["tracker_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.batch_delete_device_position_history_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_delete_device_position_history(
