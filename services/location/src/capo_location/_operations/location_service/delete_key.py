...
def handle_response(
    ...
    out: capo_location.types.delete_key_response.DeleteKeyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{KeyName}", quote(input_["key_name"], safe=""))
    params: list[tuple[str, str]] = []
    if "force_delete" in input_:
        params.append(("forceDelete", "true" if input_["force_delete"] else "false"))
...
def delete_key(
    ...
    input_: capo_location.types.delete_key_request.DeleteKeyRequest,
) -> tuple[capo_location.types.delete_key_response.DeleteKeyResponse, zapros.Response]:
