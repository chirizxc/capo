"""Generated from Smithy shape ``com.amazonaws.location#CreateRouteCalculator``."""
...
def handle_response(
    ...
    out: capo_location.types.create_route_calculator_response.CreateRouteCalculatorResponse = capo_location.types.create_route_calculator_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/routes/v0/calculators"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.create_route_calculator_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_route_calculator(
