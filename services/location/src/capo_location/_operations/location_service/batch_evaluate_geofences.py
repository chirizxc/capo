"""Generated from Smithy shape ``com.amazonaws.location#BatchEvaluateGeofences``."""
...
def handle_response(
    ...
    out: capo_location.types.batch_evaluate_geofences_response.BatchEvaluateGeofencesResponse = capo_location.types.batch_evaluate_geofences_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{CollectionName}", quote(input_["collection_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.batch_evaluate_geofences_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_batch_evaluate_geofences(
