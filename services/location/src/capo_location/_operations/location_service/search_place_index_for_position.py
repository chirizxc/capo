"""Generated from Smithy shape ``com.amazonaws.location#SearchPlaceIndexForPosition``."""
...
def handle_response(
    ...
    out: capo_location.types.search_place_index_for_position_response.SearchPlaceIndexForPositionResponse = capo_location.types.search_place_index_for_position_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/places/v0/indexes/{IndexName}/search/position"
    ...
    body: bytes | None = json.dumps(
        capo_location.types.search_place_index_for_position_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_search_place_index_for_position(
