"""Generated from Smithy shape ``com.amazonaws.location#ListJobs``."""
...
import capo_location.types.list_jobs_response_entry_list
...
def handle_response(
    ...
    out: capo_location.types.list_jobs_response.ListJobsResponse = (
        capo_location.types.list_jobs_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/metadata/v0/jobs/list-jobs"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.list_jobs_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def list_jobs(
    ...
    input_: capo_location.types.list_jobs_request.ListJobsRequest,
) -> tuple[capo_location.types.list_jobs_response.ListJobsResponse, zapros.Response]:
