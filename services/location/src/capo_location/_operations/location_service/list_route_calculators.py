"""Generated from Smithy shape ``com.amazonaws.location#ListRouteCalculators``."""
...
def handle_response(
    ...
    out: capo_location.types.list_route_calculators_response.ListRouteCalculatorsResponse = capo_location.types.list_route_calculators_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/routes/v0/list-calculators"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.list_route_calculators_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_route_calculators(
