"""Generated from Smithy shape ``com.amazonaws.location#CalculateRoute``."""
...
def handle_response(
    ...
    out: capo_location.types.calculate_route_response.CalculateRouteResponse = (
        capo_location.types.calculate_route_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{CalculatorName}", quote(input_["calculator_name"], safe=""))
    params: list[tuple[str, str]] = []
    if "key" in input_:
        params.append(("key", input_["key"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.calculate_route_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def calculate_route(
    ...
    capo_location.types.calculate_route_response.CalculateRouteResponse, zapros.Response
]:
