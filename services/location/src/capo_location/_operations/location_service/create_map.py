"""Generated from Smithy shape ``com.amazonaws.location#CreateMap``."""
...
def handle_response(
    ...
    out: capo_location.types.create_map_response.CreateMapResponse = (
        capo_location.types.create_map_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/maps/v0/maps"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.create_map_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def create_map(
    ...
    input_: capo_location.types.create_map_request.CreateMapRequest,
) -> tuple[capo_location.types.create_map_response.CreateMapResponse, zapros.Response]:
