"""Generated from Smithy shape ``com.amazonaws.location#SearchPlaceIndexForSuggestions``."""
...
import capo_location.types.search_for_suggestions_result_list
...
import capo_location.types.search_place_index_for_suggestions_summary
...
def handle_response(
    ...
    out: capo_location.types.search_place_index_for_suggestions_response.SearchPlaceIndexForSuggestionsResponse = capo_location.types.search_place_index_for_suggestions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/places/v0/indexes/{IndexName}/search/suggestions"
    url = url.replace("{IndexName}", quote(input_["index_name"], safe=""))
    params: list[tuple[str, str]] = []
    if "key" in input_:
        params.append(("key", input_["key"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.search_place_index_for_suggestions_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_search_place_index_for_suggestions(
