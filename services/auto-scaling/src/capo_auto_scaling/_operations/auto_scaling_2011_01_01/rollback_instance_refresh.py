"""Generated from Smithy shape ``com.amazonaws.autoscaling#RollbackInstanceRefresh``."""
...
import capo_auto_scaling.errors.irreversible_instance_refresh_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "IrreversibleInstanceRefresh":
            raise capo_auto_scaling.errors.irreversible_instance_refresh_fault.IrreversibleInstanceRefreshFault.from_query(
                error_el, message
            )
        case "LimitExceeded":
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("RollbackInstanceRefreshResult")
    out: capo_auto_scaling.types.rollback_instance_refresh_answer.RollbackInstanceRefreshAnswer = capo_auto_scaling.types.rollback_instance_refresh_answer.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "RollbackInstanceRefresh"))
    pairs.append(("Version", "2011-01-01"))
    capo_auto_scaling.types.rollback_instance_refresh_type.serialize_query(
        input_, pairs, ""
    )
...
async def async_rollback_instance_refresh(
