"""Generated from Smithy shape ``com.amazonaws.autoscaling#DescribeScalingProcessTypes``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeScalingProcessTypesResult")
    out: capo_auto_scaling.types.processes_type.ProcessesType = (
        capo_auto_scaling.types.processes_type.deserialize_query(
...
def build_request(options: OperationOptions | AsyncOperationOptions) -> zapros.Request:
    ...
    pairs.append(("Action", "DescribeScalingProcessTypes"))
...
) -> tuple[capo_auto_scaling.types.processes_type.ProcessesType, zapros.Response]:
...
async def async_describe_scaling_process_types(
