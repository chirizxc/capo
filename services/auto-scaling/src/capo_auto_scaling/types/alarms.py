...
if TYPE_CHECKING:
    import capo_auto_scaling.types.alarm

Alarms: TypeAlias = list["capo_auto_scaling.types.alarm.Alarm"]
...
def serialize_query(value: Alarms, pairs: list[tuple[str, str]], prefix: str) -> None:
    ...
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.alarm.serialize_query(
...
def deserialize_query(el: Element) -> Alarms:
    ...
    for child in el.findall("member"):
        out.append(capo_auto_scaling.types.alarm.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    if not value:
        ...
        return
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.alarm.serialize_query(item, pairs, f"{prefix}.{n}")
