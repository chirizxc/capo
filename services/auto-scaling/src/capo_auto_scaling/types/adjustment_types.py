...
if TYPE_CHECKING:
    import capo_auto_scaling.types.adjustment_type

AdjustmentTypes: TypeAlias = list[
    "capo_auto_scaling.types.adjustment_type.AdjustmentType"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.adjustment_type.serialize_query(
...
def deserialize_query(el: Element) -> AdjustmentTypes:
    ...
    for child in el.findall("member"):
        out.append(capo_auto_scaling.types.adjustment_type.deserialize_query(child))
    return out
