"""Generated from Smithy shape ``com.amazonaws.autoscaling#CpuManufacturers``."""

from typing import TYPE_CHECKING, TypeAlias

from capo_auto_scaling._protocol.xml import Element

if TYPE_CHECKING:
    import capo_auto_scaling.types.cpu_manufacturer

CpuManufacturers: TypeAlias = list[
    "capo_auto_scaling.types.cpu_manufacturer.CpuManufacturer"
]


# --- awsQuery ser/de ---
def serialize_query(
    value: CpuManufacturers, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import capo_auto_scaling.types.cpu_manufacturer

    if not value:
        pairs.append((prefix, ""))
        return
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.cpu_manufacturer.serialize_query(
            item, pairs, f"{prefix}.member.{n}"
        )


def deserialize_query(el: Element) -> CpuManufacturers:
    import capo_auto_scaling.types.cpu_manufacturer

    out: CpuManufacturers = []
    for child in el.findall("member"):
        out.append(capo_auto_scaling.types.cpu_manufacturer.deserialize_query(child))
    return out


def serialize_query_flat(
    value: CpuManufacturers, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import capo_auto_scaling.types.cpu_manufacturer

    if not value:
        pairs.append((prefix, ""))
        return
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.cpu_manufacturer.serialize_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_query_flat(parent: Element, tag: str) -> CpuManufacturers:
    import capo_auto_scaling.types.cpu_manufacturer

    out: CpuManufacturers = []
    for child in parent.findall(tag):
        out.append(capo_auto_scaling.types.cpu_manufacturer.deserialize_query(child))
    return out
