...
if TYPE_CHECKING:
    import capo_auto_scaling.types.local_storage_type

LocalStorageTypes: TypeAlias = list[
    "capo_auto_scaling.types.local_storage_type.LocalStorageType"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.local_storage_type.serialize_query(
...
def deserialize_query(el: Element) -> LocalStorageTypes:
    ...
    for child in el.findall("member"):
        out.append(capo_auto_scaling.types.local_storage_type.deserialize_query(child))
    return out
