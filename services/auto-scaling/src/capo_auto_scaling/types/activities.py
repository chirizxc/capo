...
Activities: TypeAlias = list["capo_auto_scaling.types.activity.Activity"]
...
def deserialize_query(el: Element) -> Activities:
    ...
    for child in el.findall("member"):
        out.append(capo_auto_scaling.types.activity.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    if not value:
        ...
        return
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.activity.serialize_query(item, pairs, f"{prefix}.{n}")
