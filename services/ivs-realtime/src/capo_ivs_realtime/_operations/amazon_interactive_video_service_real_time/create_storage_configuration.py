"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#CreateStorageConfiguration``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.create_storage_configuration_response.CreateStorageConfigurationResponse = capo_ivs_realtime.types.create_storage_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateStorageConfiguration"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.create_storage_configuration_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_storage_configuration(
