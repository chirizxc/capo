"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#GetPublicKey``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.get_public_key_response.GetPublicKeyResponse = (
        capo_ivs_realtime.types.get_public_key_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetPublicKey"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.get_public_key_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
