"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#DeleteEncoderConfiguration``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.delete_encoder_configuration_response.DeleteEncoderConfigurationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteEncoderConfiguration"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.delete_encoder_configuration_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_delete_encoder_configuration(
