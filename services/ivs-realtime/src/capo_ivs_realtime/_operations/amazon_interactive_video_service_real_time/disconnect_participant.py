"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#DisconnectParticipant``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.disconnect_participant_response.DisconnectParticipantResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DisconnectParticipant"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.disconnect_participant_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
