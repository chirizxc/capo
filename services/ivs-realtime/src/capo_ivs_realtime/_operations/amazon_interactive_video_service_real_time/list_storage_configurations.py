"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#ListStorageConfigurations``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.list_storage_configurations_response.ListStorageConfigurationsResponse = capo_ivs_realtime.types.list_storage_configurations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListStorageConfigurations"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.list_storage_configurations_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_storage_configurations(
