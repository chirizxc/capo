"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#UpdateStage``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.update_stage_response.UpdateStageResponse = (
        capo_ivs_realtime.types.update_stage_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateStage"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.update_stage_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_stage(
    ...
    capo_ivs_realtime.types.update_stage_response.UpdateStageResponse, zapros.Response
]:
