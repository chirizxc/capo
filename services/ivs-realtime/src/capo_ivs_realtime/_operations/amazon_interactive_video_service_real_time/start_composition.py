"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#StartComposition``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.start_composition_response.StartCompositionResponse = (
        capo_ivs_realtime.types.start_composition_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartComposition"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.start_composition_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_start_composition(
