...
if TYPE_CHECKING:
    ...
    import capo_ivs_realtime.types.destination_configuration_name
    import capo_ivs_realtime.types.s3_destination_configuration


class DestinationConfiguration(TypedDict, closed=True):
    name: NotRequired[
        "capo_ivs_realtime.types.destination_configuration_name.DestinationConfigurationName"
    ]
    """<p>Name that can be specified to help identify the destination.</p>"""
    channel: NotRequired[
        "capo_ivs_realtime.types.channel_destination_configuration.ChannelDestinationConfiguration"
    ]
    ...
    s3: NotRequired[
        "capo_ivs_realtime.types.s3_destination_configuration.S3DestinationConfiguration"
    ]
...
def serialize_json(value: DestinationConfiguration) -> dict:
    ...
    if "channel" in value:
        ...
        out["channel"] = (
            capo_ivs_realtime.types.channel_destination_configuration.serialize_json(
                value["channel"]
            )
        )
    if "s3" in value:
        ...
        out["s3"] = capo_ivs_realtime.types.s3_destination_configuration.serialize_json(
            value["s3"]
        )
    return out
...
def deserialize_json(data: dict) -> DestinationConfiguration:
    ...
    if data.get("channel") is not None:
        ...
        out["channel"] = (
            capo_ivs_realtime.types.channel_destination_configuration.deserialize_json(
                data["channel"]
            )
        )
    if data.get("s3") is not None:
        ...
        out["s3"] = (
            capo_ivs_realtime.types.s3_destination_configuration.deserialize_json(
                data["s3"]
            )
        )
    return out
