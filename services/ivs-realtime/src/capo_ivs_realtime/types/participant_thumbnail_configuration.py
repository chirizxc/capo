...
class ParticipantThumbnailConfiguration(TypedDict, closed=True):
    ...
    recording_mode: NotRequired[
        "capo_ivs_realtime.types.thumbnail_recording_mode.ThumbnailRecordingMode"
    ]
...
def serialize_json(value: ParticipantThumbnailConfiguration) -> dict:
    ...
    if "recording_mode" in value:
        ...
        out["recordingMode"] = (
            capo_ivs_realtime.types.thumbnail_recording_mode.serialize_json(
...
def deserialize_json(data: dict) -> ParticipantThumbnailConfiguration:
    ...
    if data.get("recordingMode") is not None:
        ...
        out["recording_mode"] = (
            capo_ivs_realtime.types.thumbnail_recording_mode.deserialize_json(
                data["recordingMode"]
            )
        )
    return out
