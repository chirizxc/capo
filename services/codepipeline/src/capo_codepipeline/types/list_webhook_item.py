"""Generated from Smithy shape ``com.amazonaws.codepipeline#ListWebhookItem``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_codepipeline.errors import DeserializationError

if TYPE_CHECKING:
    import capo_codepipeline.types.tag_list
    import capo_codepipeline.types.webhook_arn
    import capo_codepipeline.types.webhook_definition
    import capo_codepipeline.types.webhook_error_code
    import capo_codepipeline.types.webhook_error_message
    import capo_codepipeline.types.webhook_last_triggered
    import capo_codepipeline.types.webhook_url


class ListWebhookItem(TypedDict, closed=True):
    definition: "capo_codepipeline.types.webhook_definition.WebhookDefinition"
    """<p>The detail returned for each webhook, such as the webhook authentication type and filter rules.</p>"""
    url: "capo_codepipeline.types.webhook_url.WebhookUrl"
    """<p>A unique URL generated by CodePipeline. When a POST request is made to this URL, the defined pipeline is started as long as the body of the post request satisfies the defined authentication and filtering conditions. Deleting and re-creating a webhook makes the old URL invalid and generates a new one.</p>"""
    error_message: NotRequired[
        "capo_codepipeline.types.webhook_error_message.WebhookErrorMessage"
    ]
    """<p>The text of the error message about the webhook.</p>"""
    error_code: NotRequired[
        "capo_codepipeline.types.webhook_error_code.WebhookErrorCode"
    ]
    """<p>The number code of the error.</p>"""
    last_triggered: NotRequired[
        "capo_codepipeline.types.webhook_last_triggered.WebhookLastTriggered"
    ]
    """<p>The date and time a webhook was last successfully triggered, in timestamp format.</p>"""
    arn: NotRequired["capo_codepipeline.types.webhook_arn.WebhookArn"]
    """<p>The Amazon Resource Name (ARN) of the webhook.</p>"""
    tags: NotRequired["capo_codepipeline.types.tag_list.TagList"]
    """<p>Specifies the tags applied to the webhook.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: ListWebhookItem) -> dict:
    out: dict = {}
    import capo_codepipeline.types.webhook_definition

    out["definition"] = (
        capo_codepipeline.types.webhook_definition.serialize_aws_json_1_1(
            value["definition"]
        )
    )
    out["url"] = value["url"]
    if "error_message" in value:
        out["errorMessage"] = value["error_message"]
    if "error_code" in value:
        out["errorCode"] = value["error_code"]
    if "last_triggered" in value:
        import capo_codepipeline.types.webhook_last_triggered

        out["lastTriggered"] = (
            capo_codepipeline.types.webhook_last_triggered.serialize_aws_json_1_1(
                value["last_triggered"]
            )
        )
    if "arn" in value:
        out["arn"] = value["arn"]
    if "tags" in value:
        import capo_codepipeline.types.tag_list

        out["tags"] = capo_codepipeline.types.tag_list.serialize_aws_json_1_1(
            value["tags"]
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> ListWebhookItem:
    out: ListWebhookItem = {}  # type: ignore[typeddict-item]
    if data.get("definition") is not None:
        import capo_codepipeline.types.webhook_definition

        out["definition"] = (
            capo_codepipeline.types.webhook_definition.deserialize_aws_json_1_1(
                data["definition"]
            )
        )
    else:
        raise DeserializationError("ListWebhookItem.definition required")
    if data.get("url") is not None:
        out["url"] = data["url"]
    else:
        raise DeserializationError("ListWebhookItem.url required")
    if data.get("errorMessage") is not None:
        out["error_message"] = data["errorMessage"]
    if data.get("errorCode") is not None:
        out["error_code"] = data["errorCode"]
    if data.get("lastTriggered") is not None:
        import capo_codepipeline.types.webhook_last_triggered

        out["last_triggered"] = (
            capo_codepipeline.types.webhook_last_triggered.deserialize_aws_json_1_1(
                data["lastTriggered"]
            )
        )
    if data.get("arn") is not None:
        out["arn"] = data["arn"]
    if data.get("tags") is not None:
        import capo_codepipeline.types.tag_list

        out["tags"] = capo_codepipeline.types.tag_list.deserialize_aws_json_1_1(
            data["tags"]
        )
    return out
