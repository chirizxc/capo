...
import capo_iot.types.create_thing_type_request
import capo_iot.types.create_thing_type_response
...
def handle_response(
    ...
    out: capo_iot.types.create_thing_type_response.CreateThingTypeResponse = (
        capo_iot.types.create_thing_type_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{thingTypeName}", quote(input_["thing_type_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.create_thing_type_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def create_thing_type(
    ...
    capo_iot.types.create_thing_type_response.CreateThingTypeResponse, zapros.Response
]:
