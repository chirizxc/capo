...
import capo_iot.types.list_jobs_request
import capo_iot.types.list_jobs_response
...
def handle_response(
    ...
    out: capo_iot.types.list_jobs_response.ListJobsResponse = (
        capo_iot.types.list_jobs_response.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_iot.types.list_jobs_response.ListJobsResponse = (
        capo_iot.types.list_jobs_response.deserialize_json(
...
def build_request(
    ...
    if "status" in input_:
        params.append(
            ("status", capo_iot.types.job_status.serialize_json(input_["status"]))
        )
    if "target_selection" in input_:
        params.append(
            (
                "targetSelection",
                capo_iot.types.target_selection.serialize_json(
                    input_["target_selection"]
                ),
            )
        )
    ...
    if "thing_group_name" in input_:
        params.append(("thingGroupName", input_["thing_group_name"]))
    if "thing_group_id" in input_:
        params.append(("thingGroupId", input_["thing_group_id"]))
    if "namespace_id" in input_:
        params.append(("namespaceId", input_["namespace_id"]))
...
def list_jobs(
    options: OperationOptions, input_: capo_iot.types.list_jobs_request.ListJobsRequest
) -> tuple[capo_iot.types.list_jobs_response.ListJobsResponse, zapros.Response]:
