...
import capo_iot.errors.registration_code_validation_exception
...
import capo_iot.types.register_ca_certificate_request
import capo_iot.types.register_ca_certificate_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "RegistrationCodeValidationException":
            raise capo_iot.errors.registration_code_validation_exception.RegistrationCodeValidationException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_iot.types.register_ca_certificate_response.RegisterCACertificateResponse = capo_iot.types.register_ca_certificate_response.deserialize_json(
...
def build_request(
    ...
    params.append(
        ("setAsActive", "true" if input_.get("set_as_active", False) else "false")
    )
    params.append(
        (
            "allowAutoRegistration",
            "true" if input_.get("allow_auto_registration", False) else "false",
        )
    )
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.register_ca_certificate_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
