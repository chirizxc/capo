...
import capo_iot.types.create_topic_rule_request
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "SqlParseException":
            raise capo_iot.errors.sql_parse_exception.SqlParseException.from_json(
                data, message
            )
...
def build_request(
    ...
    if "tags" in input_:
        headers["x-amz-tagging"] = input_["tags"]
    body: bytes | None = json.dumps(
        capo_iot.types.topic_rule_payload.serialize_json(input_["topic_rule_payload"]),
        allow_nan=False,
    ).encode()
