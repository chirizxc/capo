"""Generated from Smithy shape ``com.amazonaws.iot#ListPrincipalThingsV2``."""
...
import capo_iot.types.list_principal_things_v2_request
import capo_iot.types.list_principal_things_v2_response
import capo_iot.types.principal_thing_objects
...
def handle_response(
    ...
    out: capo_iot.types.list_principal_things_v2_response.ListPrincipalThingsV2Response = capo_iot.types.list_principal_things_v2_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/principals/things-v2"
    ...
    if "thing_principal_type" in input_:
        params.append(
            (
                "thingPrincipalType",
                capo_iot.types.thing_principal_type.serialize_json(
                    input_["thing_principal_type"]
                ),
            )
        )
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "principal" in input_:
        headers["x-amzn-principal"] = input_["principal"]
    body: bytes | None = b""
...
async def async_list_principal_things_v2(
