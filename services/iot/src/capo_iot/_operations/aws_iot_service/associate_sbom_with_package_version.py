"""Generated from Smithy shape ``com.amazonaws.iot#AssociateSbomWithPackageVersion``."""
...
import capo_iot.types.associate_sbom_with_package_version_request
import capo_iot.types.associate_sbom_with_package_version_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ConflictException":
            raise capo_iot.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_iot.types.associate_sbom_with_package_version_response.AssociateSbomWithPackageVersionResponse = capo_iot.types.associate_sbom_with_package_version_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{packageName}", quote(input_["package_name"], safe=""))
    url = url.replace("{versionName}", quote(input_["version_name"], safe=""))
    params: list[tuple[str, str]] = []
    if "client_token" in input_:
        params.append(("clientToken", input_["client_token"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.associate_sbom_with_package_version_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_associate_sbom_with_package_version(
