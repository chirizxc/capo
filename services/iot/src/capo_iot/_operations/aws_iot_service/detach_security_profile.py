"""Generated from Smithy shape ``com.amazonaws.iot#DetachSecurityProfile``."""
...
import capo_iot.types.detach_security_profile_request
import capo_iot.types.detach_security_profile_response
...
def handle_response(
    ...
    out: capo_iot.types.detach_security_profile_response.DetachSecurityProfileResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{securityProfileName}", quote(input_["security_profile_name"], safe="")
    )
    params: list[tuple[str, str]] = []
    if "security_profile_target_arn" in input_:
        params.append(
            ("securityProfileTargetArn", input_["security_profile_target_arn"])
        )
...
async def async_detach_security_profile(
