...
import capo_iot.errors.invalid_response_exception
...
import capo_iot.types.test_invoke_authorizer_request
import capo_iot.types.test_invoke_authorizer_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidResponseException":
            raise capo_iot.errors.invalid_response_exception.InvalidResponseException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_iot.types.test_invoke_authorizer_response.TestInvokeAuthorizerResponse = (
        capo_iot.types.test_invoke_authorizer_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{authorizerName}", quote(input_["authorizer_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.test_invoke_authorizer_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
