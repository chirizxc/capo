"""Generated from Smithy shape ``com.amazonaws.iot#PutVerificationStateOnViolation``."""
...
import capo_iot.types.put_verification_state_on_violation_request
import capo_iot.types.put_verification_state_on_violation_response
...
def handle_response(
    ...
    out: capo_iot.types.put_verification_state_on_violation_response.PutVerificationStateOnViolationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/violations/verification-state/{violationId}"
    url = url.replace("{violationId}", quote(input_["violation_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.put_verification_state_on_violation_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_put_verification_state_on_violation(
