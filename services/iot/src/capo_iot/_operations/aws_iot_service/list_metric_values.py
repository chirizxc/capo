...
import capo_iot.types.list_metric_values_request
import capo_iot.types.list_metric_values_response
...
) -> capo_iot.types.list_metric_values_response.ListMetricValuesResponse:
    out: capo_iot.types.list_metric_values_response.ListMetricValuesResponse = (
        capo_iot.types.list_metric_values_response.deserialize_json(
...
def build_request(
    ...
    import capo_iot._protocol.serialize
    import capo_iot.types.dimension_value_operator

    url = endpoint.url.rstrip("/") + "/metric-values"
    params: list[tuple[str, str]] = []
    if "thing_name" in input_:
        params.append(("thingName", input_["thing_name"]))
    if "metric_name" in input_:
        params.append(("metricName", input_["metric_name"]))
    if "dimension_name" in input_:
        params.append(("dimensionName", input_["dimension_name"]))
    if "dimension_value_operator" in input_:
        params.append(
            (
                "dimensionValueOperator",
                capo_iot.types.dimension_value_operator.serialize_json(
                    input_["dimension_value_operator"]
                ),
            )
        )
    if "start_time" in input_:
        params.append(
            (
                "startTime",
                capo_iot._protocol.serialize.fmt_date_time(input_["start_time"]),
            )
        )
    if "end_time" in input_:
        params.append(
            ("endTime", capo_iot._protocol.serialize.fmt_date_time(input_["end_time"]))
        )
...
def list_metric_values(
    ...
    capo_iot.types.list_metric_values_response.ListMetricValuesResponse, zapros.Response
]:
