"""Generated from Smithy shape ``com.amazonaws.iot#UpdateCommand``."""
...
import capo_iot.types.update_command_request
import capo_iot.types.update_command_response
...
) -> capo_iot.types.update_command_response.UpdateCommandResponse:
    out: capo_iot.types.update_command_response.UpdateCommandResponse = (
        capo_iot.types.update_command_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{commandId}", quote(input_["command_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_command_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def update_command(
    ...
    capo_iot.types.update_command_response.UpdateCommandResponse, zapros.Response
]:
