"""Generated from Smithy shape ``com.amazonaws.iot#CreateJobTemplate``."""
...
import capo_iot.types.create_job_template_request
import capo_iot.types.create_job_template_response
...
def handle_response(
    ...
    out: capo_iot.types.create_job_template_response.CreateJobTemplateResponse = (
        capo_iot.types.create_job_template_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{jobTemplateId}", quote(input_["job_template_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.create_job_template_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
