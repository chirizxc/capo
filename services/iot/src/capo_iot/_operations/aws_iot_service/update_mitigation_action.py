"""Generated from Smithy shape ``com.amazonaws.iot#UpdateMitigationAction``."""
...
import capo_iot.types.update_mitigation_action_request
import capo_iot.types.update_mitigation_action_response
...
def handle_response(
    ...
    out: capo_iot.types.update_mitigation_action_response.UpdateMitigationActionResponse = capo_iot.types.update_mitigation_action_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{actionName}", quote(input_["action_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_mitigation_action_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_mitigation_action(
