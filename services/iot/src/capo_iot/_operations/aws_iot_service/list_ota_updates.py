"""Generated from Smithy shape ``com.amazonaws.iot#ListOTAUpdates``."""
...
import capo_iot.types.list_ota_updates_request
import capo_iot.types.list_ota_updates_response
...
def handle_response(
    ...
    out: capo_iot.types.list_ota_updates_response.ListOTAUpdatesResponse = (
        capo_iot.types.list_ota_updates_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/otaUpdates"
    params: list[tuple[str, str]] = []
    if "max_results" in input_:
        params.append(("maxResults", str(input_["max_results"])))
    if "next_token" in input_:
        params.append(("nextToken", input_["next_token"]))
    if "ota_update_status" in input_:
        params.append(
            (
                "otaUpdateStatus",
                capo_iot.types.ota_update_status.serialize_json(
                    input_["ota_update_status"]
                ),
            )
        )
...
def list_ota_updates(
    ...
    capo_iot.types.list_ota_updates_response.ListOTAUpdatesResponse, zapros.Response
]:
...
async def async_list_ota_updates(
