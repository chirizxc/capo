"""Generated from Smithy shape ``com.amazonaws.iot#RejectCertificateTransfer``."""
...
import capo_iot.types.reject_certificate_transfer_request
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/reject-certificate-transfer/{certificateId}"
    url = url.replace("{certificateId}", quote(input_["certificate_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.reject_certificate_transfer_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_reject_certificate_transfer(
