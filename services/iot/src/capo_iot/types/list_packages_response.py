...
class ListPackagesResponse(TypedDict, closed=True):
    package_summaries: NotRequired[
        "capo_iot.types.package_summary_list.PackageSummaryList"
    ]
...
def serialize_json(value: ListPackagesResponse) -> dict:
    ...
    if "package_summaries" in value:
        ...
        out["packageSummaries"] = capo_iot.types.package_summary_list.serialize_json(
...
def deserialize_json(data: dict) -> ListPackagesResponse:
    ...
    if data.get("packageSummaries") is not None:
        import capo_iot.types.package_summary_list

        out["package_summaries"] = capo_iot.types.package_summary_list.deserialize_json(
