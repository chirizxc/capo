...
if TYPE_CHECKING:
    ...
    import capo_iot.types.connectivity_timestamp
    import capo_iot.types.disconnect_reason
    import capo_iot.types.keep_alive_duration
    import capo_iot.types.session_expiry


class ThingConnectivity(TypedDict, closed=True):
    ...
    timestamp: NotRequired[
        "capo_iot.types.connectivity_timestamp.ConnectivityTimestamp"
    ]
    """<p>The epoch time (in milliseconds) when the thing last connected or disconnected.</p>"""
    disconnect_reason: NotRequired["capo_iot.types.disconnect_reason.DisconnectReason"]
