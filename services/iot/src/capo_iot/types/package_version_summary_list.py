...
if TYPE_CHECKING:
    import capo_iot.types.package_version_summary

PackageVersionSummaryList: TypeAlias = list[
    "capo_iot.types.package_version_summary.PackageVersionSummary"
]
...
def serialize_json(value: PackageVersionSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_iot.types.package_version_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> PackageVersionSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_iot.types.package_version_summary.deserialize_json(item))
    return out
