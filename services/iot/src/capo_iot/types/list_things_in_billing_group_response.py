"""Generated from Smithy shape ``com.amazonaws.iot#ListThingsInBillingGroupResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_iot.types.next_token
    import capo_iot.types.thing_name_list


class ListThingsInBillingGroupResponse(TypedDict, closed=True):
    things: NotRequired["capo_iot.types.thing_name_list.ThingNameList"]
    """<p>A list of things in the billing group.</p>"""
    next_token: NotRequired["capo_iot.types.next_token.NextToken"]
    """<p>The token to use to get the next set of results. Will not be returned if operation has returned all results.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListThingsInBillingGroupResponse) -> dict:
    out: dict = {}
    if "things" in value:
        import capo_iot.types.thing_name_list

        out["things"] = capo_iot.types.thing_name_list.serialize_json(value["things"])
    if "next_token" in value:
        out["nextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListThingsInBillingGroupResponse:
    out: ListThingsInBillingGroupResponse = {}  # type: ignore[typeddict-item]
    if data.get("things") is not None:
        import capo_iot.types.thing_name_list

        out["things"] = capo_iot.types.thing_name_list.deserialize_json(data["things"])
    if data.get("nextToken") is not None:
        out["next_token"] = data["nextToken"]
    return out
