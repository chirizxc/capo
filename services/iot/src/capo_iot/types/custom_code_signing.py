...
if TYPE_CHECKING:
    ...
    import capo_iot.types.hash_algorithm
    import capo_iot.types.signature_algorithm


class CustomCodeSigning(TypedDict, closed=True):
    signature: NotRequired["capo_iot.types.code_signing_signature.CodeSigningSignature"]
    """<p>The signature for the file.</p>"""
    certificate_chain: NotRequired[
        "capo_iot.types.code_signing_certificate_chain.CodeSigningCertificateChain"
    ]
    """<p>The certificate chain.</p>"""
    hash_algorithm: NotRequired["capo_iot.types.hash_algorithm.HashAlgorithm"]
    ...
    signature_algorithm: NotRequired[
        "capo_iot.types.signature_algorithm.SignatureAlgorithm"
    ]
...
def serialize_json(value: CustomCodeSigning) -> dict:
    ...
    if "signature" in value:
        ...
        out["signature"] = capo_iot.types.code_signing_signature.serialize_json(
            value["signature"]
        )
    if "certificate_chain" in value:
        ...
        out["certificateChain"] = (
            capo_iot.types.code_signing_certificate_chain.serialize_json(
...
def deserialize_json(data: dict) -> CustomCodeSigning:
    ...
    if data.get("certificateChain") is not None:
        ...
        out["certificate_chain"] = (
            capo_iot.types.code_signing_certificate_chain.deserialize_json(
