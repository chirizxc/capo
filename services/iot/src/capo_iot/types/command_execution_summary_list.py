...
if TYPE_CHECKING:
    import capo_iot.types.command_execution_summary

CommandExecutionSummaryList: TypeAlias = list[
    "capo_iot.types.command_execution_summary.CommandExecutionSummary"
]
...
def serialize_json(value: CommandExecutionSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_iot.types.command_execution_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> CommandExecutionSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_iot.types.command_execution_summary.deserialize_json(item))
    return out
