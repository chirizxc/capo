...
class ListIndicesResponse(TypedDict, closed=True):
    index_names: NotRequired["capo_iot.types.index_names_list.IndexNamesList"]
    """<p>The index names.</p>"""
...
def serialize_json(value: ListIndicesResponse) -> dict:
    ...
    if "index_names" in value:
        ...
        out["indexNames"] = capo_iot.types.index_names_list.serialize_json(
            value["index_names"]
        )
...
def deserialize_json(data: dict) -> ListIndicesResponse:
    ...
    if data.get("indexNames") is not None:
        import capo_iot.types.index_names_list

        out["index_names"] = capo_iot.types.index_names_list.deserialize_json(
            data["indexNames"]
        )
