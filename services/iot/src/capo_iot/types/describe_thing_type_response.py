...
if TYPE_CHECKING:
    ...
    import capo_iot.types.thing_type_metadata
    import capo_iot.types.thing_type_name
    import capo_iot.types.thing_type_properties


class DescribeThingTypeResponse(TypedDict, closed=True):
    ...
    thing_type_metadata: NotRequired[
        "capo_iot.types.thing_type_metadata.ThingTypeMetadata"
    ]
...
def serialize_json(value: DescribeThingTypeResponse) -> dict:
    ...
    if "thing_type_metadata" in value:
        ...
        out["thingTypeMetadata"] = capo_iot.types.thing_type_metadata.serialize_json(
...
def deserialize_json(data: dict) -> DescribeThingTypeResponse:
    ...
    if data.get("thingTypeId") is not None:
        out["thing_type_id"] = data["thingTypeId"]
    if data.get("thingTypeArn") is not None:
    ...
    if data.get("thingTypeMetadata") is not None:
        import capo_iot.types.thing_type_metadata

        out["thing_type_metadata"] = (
            capo_iot.types.thing_type_metadata.deserialize_json(
