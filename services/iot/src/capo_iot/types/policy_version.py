...
class PolicyVersion(TypedDict, closed=True):
    version_id: NotRequired["capo_iot.types.policy_version_id.PolicyVersionId"]
    """<p>The policy version ID.</p>"""
    is_default_version: "capo_iot.types.is_default_version.IsDefaultVersion"
    """<p>Specifies whether the policy version is the default.</p>"""
    create_date: NotRequired["capo_iot.types.date_type.DateType"]
...
def serialize_json(value: PolicyVersion) -> dict:
    ...
    if "create_date" in value:
        ...
        out["createDate"] = capo_iot.types.date_type.serialize_json(
            value["create_date"]
        )
    return out
