...
if TYPE_CHECKING:
    import capo_iot.types.config_name
    import capo_iot.types.config_value

CheckCustomConfiguration: TypeAlias = dict[
    "capo_iot.types.config_name.ConfigName", "capo_iot.types.config_value.ConfigValue"
]
...
def serialize_json(input_to_serialize: CheckCustomConfiguration) -> dict:
    ...
    for key, value in input_to_serialize.items():
        ...
        out[capo_iot.types.config_name.serialize_json(key)] = value
    return out
...
def deserialize_json(data: dict) -> CheckCustomConfiguration:
    ...
    for key, value in data.items():
        ...
        if value is None:
            continue
        out[capo_iot.types.config_name.deserialize_json(key)] = value
    return out
