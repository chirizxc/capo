"""Generated from Smithy shape ``com.amazonaws.iot#ListAuditMitigationActionsTasksResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_iot.types.audit_mitigation_actions_task_metadata_list
    import capo_iot.types.next_token


class ListAuditMitigationActionsTasksResponse(TypedDict, closed=True):
    tasks: NotRequired[
        "capo_iot.types.audit_mitigation_actions_task_metadata_list.AuditMitigationActionsTaskMetadataList"
    ]
    """<p>The collection of audit mitigation tasks that matched the filter criteria.</p>"""
    next_token: NotRequired["capo_iot.types.next_token.NextToken"]
    """<p>The token for the next set of results.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListAuditMitigationActionsTasksResponse) -> dict:
    out: dict = {}
    if "tasks" in value:
        import capo_iot.types.audit_mitigation_actions_task_metadata_list

        out["tasks"] = (
            capo_iot.types.audit_mitigation_actions_task_metadata_list.serialize_json(
                value["tasks"]
            )
        )
    if "next_token" in value:
        out["nextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListAuditMitigationActionsTasksResponse:
    out: ListAuditMitigationActionsTasksResponse = {}  # type: ignore[typeddict-item]
    if data.get("tasks") is not None:
        import capo_iot.types.audit_mitigation_actions_task_metadata_list

        out["tasks"] = (
            capo_iot.types.audit_mitigation_actions_task_metadata_list.deserialize_json(
                data["tasks"]
            )
        )
    if data.get("nextToken") is not None:
        out["next_token"] = data["nextToken"]
    return out
