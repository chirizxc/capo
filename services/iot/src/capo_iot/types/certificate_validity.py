...
class CertificateValidity(TypedDict, closed=True):
    not_before: NotRequired["capo_iot.types.date_type.DateType"]
    """<p>The certificate is not valid before this date.</p>"""
    not_after: NotRequired["capo_iot.types.date_type.DateType"]
...
def serialize_json(value: CertificateValidity) -> dict:
    ...
    if "not_before" in value:
        ...
        out["notBefore"] = capo_iot.types.date_type.serialize_json(value["not_before"])
    if "not_after" in value:
        import capo_iot.types.date_type

        out["notAfter"] = capo_iot.types.date_type.serialize_json(value["not_after"])
    return out
