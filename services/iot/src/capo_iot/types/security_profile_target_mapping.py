...
class SecurityProfileTargetMapping(TypedDict, closed=True):
    ...
    target: NotRequired["capo_iot.types.security_profile_target.SecurityProfileTarget"]
...
def serialize_json(value: SecurityProfileTargetMapping) -> dict:
    ...
    if "security_profile_identifier" in value:
        ...
        out["securityProfileIdentifier"] = (
            capo_iot.types.security_profile_identifier.serialize_json(
    ...
    if "target" in value:
        ...
        out["target"] = capo_iot.types.security_profile_target.serialize_json(
            value["target"]
        )
    return out
...
def deserialize_json(data: dict) -> SecurityProfileTargetMapping:
    ...
    if data.get("securityProfileIdentifier") is not None:
        import capo_iot.types.security_profile_identifier

        out["security_profile_identifier"] = (
            capo_iot.types.security_profile_identifier.deserialize_json(
    ...
    if data.get("target") is not None:
        ...
        out["target"] = capo_iot.types.security_profile_target.deserialize_json(
            data["target"]
        )
    return out
