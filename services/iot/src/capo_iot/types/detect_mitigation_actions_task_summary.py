"""Generated from Smithy shape ``com.amazonaws.iot#DetectMitigationActionsTaskSummary``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_iot.types.detect_mitigation_actions_task_statistics
    import capo_iot.types.detect_mitigation_actions_task_status
    import capo_iot.types.detect_mitigation_actions_task_target
    import capo_iot.types.mitigation_action_list
    import capo_iot.types.mitigation_actions_task_id
    import capo_iot.types.primitive_boolean
    import capo_iot.types.timestamp
    import capo_iot.types.violation_event_occurrence_range


class DetectMitigationActionsTaskSummary(TypedDict, closed=True):
    task_id: NotRequired[
        "capo_iot.types.mitigation_actions_task_id.MitigationActionsTaskId"
    ]
    """<p> The unique identifier of the task. </p>"""
    task_status: NotRequired[
        "capo_iot.types.detect_mitigation_actions_task_status.DetectMitigationActionsTaskStatus"
    ]
    """<p> The status of the task. </p>"""
    task_start_time: NotRequired["capo_iot.types.timestamp.Timestamp"]
    """<p> The date the task started. </p>"""
    task_end_time: NotRequired["capo_iot.types.timestamp.Timestamp"]
    """<p> The date the task ended. </p>"""
    target: NotRequired[
        "capo_iot.types.detect_mitigation_actions_task_target.DetectMitigationActionsTaskTarget"
    ]
    """<p> Specifies the ML Detect findings to which the mitigation actions are applied. </p>"""
    violation_event_occurrence_range: NotRequired[
        "capo_iot.types.violation_event_occurrence_range.ViolationEventOccurrenceRange"
    ]
    """<p> Specifies the time period of which violation events occurred between. </p>"""
    only_active_violations_included: "capo_iot.types.primitive_boolean.PrimitiveBoolean"
    """<p> Includes only active violations. </p>"""
    suppressed_alerts_included: "capo_iot.types.primitive_boolean.PrimitiveBoolean"
    """<p> Includes suppressed alerts. </p>"""
    actions_definition: NotRequired[
        "capo_iot.types.mitigation_action_list.MitigationActionList"
    ]
    """<p> The definition of the actions. </p>"""
    task_statistics: NotRequired[
        "capo_iot.types.detect_mitigation_actions_task_statistics.DetectMitigationActionsTaskStatistics"
    ]
    """<p> The statistics of a mitigation action task. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: DetectMitigationActionsTaskSummary) -> dict:
    out: dict = {}
    if "task_id" in value:
        out["taskId"] = value["task_id"]
    if "task_status" in value:
        import capo_iot.types.detect_mitigation_actions_task_status

        out["taskStatus"] = (
            capo_iot.types.detect_mitigation_actions_task_status.serialize_json(
                value["task_status"]
            )
        )
    if "task_start_time" in value:
        import capo_iot.types.timestamp

        out["taskStartTime"] = capo_iot.types.timestamp.serialize_json(
            value["task_start_time"]
        )
    if "task_end_time" in value:
        import capo_iot.types.timestamp

        out["taskEndTime"] = capo_iot.types.timestamp.serialize_json(
            value["task_end_time"]
        )
    if "target" in value:
        import capo_iot.types.detect_mitigation_actions_task_target

        out["target"] = (
            capo_iot.types.detect_mitigation_actions_task_target.serialize_json(
                value["target"]
            )
        )
    if "violation_event_occurrence_range" in value:
        import capo_iot.types.violation_event_occurrence_range

        out["violationEventOccurrenceRange"] = (
            capo_iot.types.violation_event_occurrence_range.serialize_json(
                value["violation_event_occurrence_range"]
            )
        )
    out["onlyActiveViolationsIncluded"] = value.get(
        "only_active_violations_included", False
    )
    out["suppressedAlertsIncluded"] = value.get("suppressed_alerts_included", False)
    if "actions_definition" in value:
        import capo_iot.types.mitigation_action_list

        out["actionsDefinition"] = capo_iot.types.mitigation_action_list.serialize_json(
            value["actions_definition"]
        )
    if "task_statistics" in value:
        import capo_iot.types.detect_mitigation_actions_task_statistics

        out["taskStatistics"] = (
            capo_iot.types.detect_mitigation_actions_task_statistics.serialize_json(
                value["task_statistics"]
            )
        )
    return out


def deserialize_json(data: dict) -> DetectMitigationActionsTaskSummary:
    out: DetectMitigationActionsTaskSummary = {}  # type: ignore[typeddict-item]
    if data.get("taskId") is not None:
        out["task_id"] = data["taskId"]
    if data.get("taskStatus") is not None:
        import capo_iot.types.detect_mitigation_actions_task_status

        out["task_status"] = (
            capo_iot.types.detect_mitigation_actions_task_status.deserialize_json(
                data["taskStatus"]
            )
        )
    if data.get("taskStartTime") is not None:
        import capo_iot.types.timestamp

        out["task_start_time"] = capo_iot.types.timestamp.deserialize_json(
            data["taskStartTime"]
        )
    if data.get("taskEndTime") is not None:
        import capo_iot.types.timestamp

        out["task_end_time"] = capo_iot.types.timestamp.deserialize_json(
            data["taskEndTime"]
        )
    if data.get("target") is not None:
        import capo_iot.types.detect_mitigation_actions_task_target

        out["target"] = (
            capo_iot.types.detect_mitigation_actions_task_target.deserialize_json(
                data["target"]
            )
        )
    if data.get("violationEventOccurrenceRange") is not None:
        import capo_iot.types.violation_event_occurrence_range

        out["violation_event_occurrence_range"] = (
            capo_iot.types.violation_event_occurrence_range.deserialize_json(
                data["violationEventOccurrenceRange"]
            )
        )
    if data.get("onlyActiveViolationsIncluded") is not None:
        out["only_active_violations_included"] = data["onlyActiveViolationsIncluded"]
    else:
        out["only_active_violations_included"] = False
    if data.get("suppressedAlertsIncluded") is not None:
        out["suppressed_alerts_included"] = data["suppressedAlertsIncluded"]
    else:
        out["suppressed_alerts_included"] = False
    if data.get("actionsDefinition") is not None:
        import capo_iot.types.mitigation_action_list

        out["actions_definition"] = (
            capo_iot.types.mitigation_action_list.deserialize_json(
                data["actionsDefinition"]
            )
        )
    if data.get("taskStatistics") is not None:
        import capo_iot.types.detect_mitigation_actions_task_statistics

        out["task_statistics"] = (
            capo_iot.types.detect_mitigation_actions_task_statistics.deserialize_json(
                data["taskStatistics"]
            )
        )
    return out
