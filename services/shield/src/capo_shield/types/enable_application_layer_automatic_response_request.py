"""Generated from Smithy shape ``com.amazonaws.shield#EnableApplicationLayerAutomaticResponseRequest``."""
...
class EnableApplicationLayerAutomaticResponseRequest(TypedDict, closed=True):
...
def serialize_aws_json_1_1(
    value: EnableApplicationLayerAutomaticResponseRequest,
) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_1(
    ...
    out: EnableApplicationLayerAutomaticResponseRequest = {}  # type: ignore[typeddict-item]
    if data.get("ResourceArn") is not None:
        out["resource_arn"] = data["ResourceArn"]
    else:
        raise DeserializationError(
            "EnableApplicationLayerAutomaticResponseRequest.resource_arn required"
        )
    if data.get("Action") is not None:
        ...
        raise DeserializationError(
            "EnableApplicationLayerAutomaticResponseRequest.action required"
        )
    return out
