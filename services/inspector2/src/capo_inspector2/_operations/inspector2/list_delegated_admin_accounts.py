"""Generated from Smithy shape ``com.amazonaws.inspector2#ListDelegatedAdminAccounts``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.list_delegated_admin_accounts_response.ListDelegatedAdminAccountsResponse = capo_inspector2.types.list_delegated_admin_accounts_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delegatedadminaccounts/list"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.list_delegated_admin_accounts_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_delegated_admin_accounts(
