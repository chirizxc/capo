"""Generated from Smithy shape ``com.amazonaws.inspector2#GetClustersForImage``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.get_clusters_for_image_response.GetClustersForImageResponse = capo_inspector2.types.get_clusters_for_image_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/cluster/get"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.get_clusters_for_image_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_clusters_for_image(
