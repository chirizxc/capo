"""Generated from Smithy shape ``com.amazonaws.inspector2#CancelSbomExport``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.cancel_sbom_export_response.CancelSbomExportResponse = (
        capo_inspector2.types.cancel_sbom_export_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sbomexport/cancel"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.cancel_sbom_export_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_cancel_sbom_export(
