"""Generated from Smithy shape ``com.amazonaws.inspector2#CreateFilter``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.create_filter_response.CreateFilterResponse = (
        capo_inspector2.types.create_filter_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/filters/create"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.create_filter_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def create_filter(
    ...
    capo_inspector2.types.create_filter_response.CreateFilterResponse, zapros.Response
]:
