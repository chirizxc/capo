"""Generated from Smithy shape ``com.amazonaws.inspector2#EnableDelegatedAdminAccount``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.enable_delegated_admin_account_response.EnableDelegatedAdminAccountResponse = capo_inspector2.types.enable_delegated_admin_account_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delegatedadminaccounts/enable"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.enable_delegated_admin_account_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_enable_delegated_admin_account(
