"""Generated from Smithy shape ``com.amazonaws.inspector2#CreateFindingsReport``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.create_findings_report_response.CreateFindingsReportResponse = capo_inspector2.types.create_findings_report_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/reporting/create"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.create_findings_report_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_findings_report(
