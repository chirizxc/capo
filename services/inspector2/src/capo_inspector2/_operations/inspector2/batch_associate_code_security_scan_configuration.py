"""Generated from Smithy shape ``com.amazonaws.inspector2#BatchAssociateCodeSecurityScanConfiguration``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.batch_associate_code_security_scan_configuration_response.BatchAssociateCodeSecurityScanConfigurationResponse = capo_inspector2.types.batch_associate_code_security_scan_configuration_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_inspector2._auth._sigv4.build_sigv4_auth_scheme(
                "inspector2", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/codesecurity/scan-configuration/batch/associate"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.batch_associate_code_security_scan_configuration_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_associate_code_security_scan_configuration(
