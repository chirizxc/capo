"""Generated from Smithy shape ``com.amazonaws.inspector2#StopCisSession``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.stop_cis_session_response.StopCisSessionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/cissession/stop"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.stop_cis_session_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_stop_cis_session(
