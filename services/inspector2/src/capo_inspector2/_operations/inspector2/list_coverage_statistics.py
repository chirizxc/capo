"""Generated from Smithy shape ``com.amazonaws.inspector2#ListCoverageStatistics``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.list_coverage_statistics_response.ListCoverageStatisticsResponse = capo_inspector2.types.list_coverage_statistics_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/coverage/statistics/list"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.list_coverage_statistics_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_coverage_statistics(
