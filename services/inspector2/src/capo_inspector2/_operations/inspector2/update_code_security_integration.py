"""Generated from Smithy shape ``com.amazonaws.inspector2#UpdateCodeSecurityIntegration``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.update_code_security_integration_response.UpdateCodeSecurityIntegrationResponse = capo_inspector2.types.update_code_security_integration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/codesecurity/integration/update"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.update_code_security_integration_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_code_security_integration(
