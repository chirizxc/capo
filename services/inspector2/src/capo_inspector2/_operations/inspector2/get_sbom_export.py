"""Generated from Smithy shape ``com.amazonaws.inspector2#GetSbomExport``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.get_sbom_export_response.GetSbomExportResponse = (
        capo_inspector2.types.get_sbom_export_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sbomexport/get"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.get_sbom_export_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_sbom_export(
