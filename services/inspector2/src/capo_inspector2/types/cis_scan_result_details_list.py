...
CisScanResultDetailsList: TypeAlias = list[
    "capo_inspector2.types.cis_scan_result_details.CisScanResultDetails"
]
...
def serialize_json(value: CisScanResultDetailsList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_inspector2.types.cis_scan_result_details.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> CisScanResultDetailsList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_inspector2.types.cis_scan_result_details.deserialize_json(item))
    return out
