...
class ListUsageTotalsResponse(TypedDict, closed=True):
    ...
    totals: NotRequired["capo_inspector2.types.usage_total_list.UsageTotalList"]
...
def serialize_json(value: ListUsageTotalsResponse) -> dict:
    ...
    if "totals" in value:
        ...
        out["totals"] = capo_inspector2.types.usage_total_list.serialize_json(
            value["totals"]
        )
    return out
...
def deserialize_json(data: dict) -> ListUsageTotalsResponse:
    ...
    if data.get("totals") is not None:
        ...
        out["totals"] = capo_inspector2.types.usage_total_list.deserialize_json(
            data["totals"]
        )
    return out
