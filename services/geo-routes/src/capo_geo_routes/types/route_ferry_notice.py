...
if TYPE_CHECKING:
    import capo_geo_routes.types.route_ferry_notice_code
    import capo_geo_routes.types.route_notice_impact


class RouteFerryNotice(TypedDict, closed=True):
    code: "capo_geo_routes.types.route_ferry_notice_code.RouteFerryNoticeCode"
...
def serialize_json(value: RouteFerryNotice) -> dict:
    ...
    out["Code"] = capo_geo_routes.types.route_ferry_notice_code.serialize_json(
        value["code"]
    )
    if "impact" in value:
        import capo_geo_routes.types.route_notice_impact
...
def deserialize_json(data: dict) -> RouteFerryNotice:
    ...
    if data.get("Code") is not None:
        ...
        out["code"] = capo_geo_routes.types.route_ferry_notice_code.deserialize_json(
            data["Code"]
        )
    else:
