"""Generated from Smithy shape ``com.amazonaws.georoutes#RouteRoundaboutPassStepDetails``."""
...
class RouteRoundaboutPassStepDetails(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: RouteRoundaboutPassStepDetails) -> dict:
    out: dict = {}
    import capo_geo_routes.types.localized_string_list
...
def deserialize_json(data: dict) -> RouteRoundaboutPassStepDetails:
    out: RouteRoundaboutPassStepDetails = {}  # type: ignore[typeddict-item]
    if data.get("Intersection") is not None:
        ...
        raise DeserializationError(
            "RouteRoundaboutPassStepDetails.intersection required"
        )
