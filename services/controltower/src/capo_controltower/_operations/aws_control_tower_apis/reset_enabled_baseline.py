"""Generated from Smithy shape ``com.amazonaws.controltower#ResetEnabledBaseline``."""
...
def handle_response(
    ...
    out: capo_controltower.types.reset_enabled_baseline_output.ResetEnabledBaselineOutput = capo_controltower.types.reset_enabled_baseline_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/reset-enabled-baseline"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.reset_enabled_baseline_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_reset_enabled_baseline(
