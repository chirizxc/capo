"""Generated from Smithy shape ``com.amazonaws.controltower#UpdateLandingZone``."""
...
def handle_response(
    ...
    out: capo_controltower.types.update_landing_zone_output.UpdateLandingZoneOutput = (
        capo_controltower.types.update_landing_zone_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/update-landingzone"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.update_landing_zone_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_landing_zone(
