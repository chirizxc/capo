"""Generated from Smithy shape ``com.amazonaws.controltower#ListBaselines``."""
...
def handle_response(
    ...
    out: capo_controltower.types.list_baselines_output.ListBaselinesOutput = (
        capo_controltower.types.list_baselines_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-baselines"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.list_baselines_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def list_baselines(
    ...
    capo_controltower.types.list_baselines_output.ListBaselinesOutput, zapros.Response
]:
...
async def async_list_baselines(
