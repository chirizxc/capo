"""Generated from Smithy shape ``com.amazonaws.controltower#ListEnabledControls``."""
...
import capo_controltower.types.enabled_controls
...
def handle_response(
    ...
    out: capo_controltower.types.list_enabled_controls_output.ListEnabledControlsOutput = capo_controltower.types.list_enabled_controls_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-enabled-controls"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.list_enabled_controls_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_enabled_controls(
