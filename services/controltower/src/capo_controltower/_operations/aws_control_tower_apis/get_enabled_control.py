"""Generated from Smithy shape ``com.amazonaws.controltower#GetEnabledControl``."""
...
def handle_response(
    ...
    out: capo_controltower.types.get_enabled_control_output.GetEnabledControlOutput = (
        capo_controltower.types.get_enabled_control_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/get-enabled-control"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.get_enabled_control_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_enabled_control(
