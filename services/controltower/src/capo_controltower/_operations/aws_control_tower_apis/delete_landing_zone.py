"""Generated from Smithy shape ``com.amazonaws.controltower#DeleteLandingZone``."""
...
def handle_response(
    ...
    out: capo_controltower.types.delete_landing_zone_output.DeleteLandingZoneOutput = (
        capo_controltower.types.delete_landing_zone_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-landingzone"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.delete_landing_zone_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_delete_landing_zone(
