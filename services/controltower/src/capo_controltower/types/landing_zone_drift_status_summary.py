...
class LandingZoneDriftStatusSummary(TypedDict, closed=True):
    status: NotRequired[
        "capo_controltower.types.landing_zone_drift_status.LandingZoneDriftStatus"
    ]
...
def serialize_json(value: LandingZoneDriftStatusSummary) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = (
            capo_controltower.types.landing_zone_drift_status.serialize_json(
                value["status"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> LandingZoneDriftStatusSummary:
    ...
    if data.get("status") is not None:
        ...
        out["status"] = (
            capo_controltower.types.landing_zone_drift_status.deserialize_json(
                data["status"]
            )
        )
    return out
