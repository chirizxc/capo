...
if TYPE_CHECKING:
    import capo_managedblockchain.types.network_fabric_configuration


class NetworkFrameworkConfiguration(TypedDict, closed=True):
    fabric: NotRequired[
        "capo_managedblockchain.types.network_fabric_configuration.NetworkFabricConfiguration"
    ]
...
def serialize_json(value: NetworkFrameworkConfiguration) -> dict:
    ...
    if "fabric" in value:
        ...
        out["Fabric"] = (
            capo_managedblockchain.types.network_fabric_configuration.serialize_json(
                value["fabric"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> NetworkFrameworkConfiguration:
    ...
    if data.get("Fabric") is not None:
        ...
        out["fabric"] = (
            capo_managedblockchain.types.network_fabric_configuration.deserialize_json(
                data["Fabric"]
            )
        )
    return out
