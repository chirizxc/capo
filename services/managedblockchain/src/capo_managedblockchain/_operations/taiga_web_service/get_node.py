"""Generated from Smithy shape ``com.amazonaws.managedblockchain#GetNode``."""
...
import capo_managedblockchain._protocol.eventstream
...
def handle_response(
    ...
    out: capo_managedblockchain.types.get_node_output.GetNodeOutput = (
        capo_managedblockchain.types.get_node_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_managedblockchain._auth._sigv4.build_sigv4_auth_scheme(
                "managedblockchain", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace("{NodeId}", quote(input_["node_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "member_id" in input_:
        params.append(("memberId", input_["member_id"]))
...
def get_node(
    ...
    input_: capo_managedblockchain.types.get_node_input.GetNodeInput,
) -> tuple[capo_managedblockchain.types.get_node_output.GetNodeOutput, zapros.Response]:
