...
def handle_response(
    ...
    out: capo_managedblockchain.types.list_networks_output.ListNetworksOutput = (
        capo_managedblockchain.types.list_networks_output.deserialize_json(
...
def build_request(
    ...
    if "framework" in input_:
        params.append(
            (
                "framework",
                capo_managedblockchain.types.framework.serialize_json(
                    input_["framework"]
                ),
            )
        )
    if "status" in input_:
        params.append(
            (
                "status",
                capo_managedblockchain.types.network_status.serialize_json(
                    input_["status"]
                ),
            )
        )
