"""Generated from Smithy shape ``com.amazonaws.managedblockchain#VoteOnProposal``."""
...
def handle_response(
    ...
    out: capo_managedblockchain.types.vote_on_proposal_output.VoteOnProposalOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{ProposalId}", quote(input_["proposal_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_managedblockchain.types.vote_on_proposal_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_vote_on_proposal(
