"""Generated from Smithy shape ``com.amazonaws.sagemakergeospatial#SearchRasterDataCollection``."""
...
def handle_response(
    ...
    out: capo_sagemaker_geospatial.types.search_raster_data_collection_output.SearchRasterDataCollectionOutput = capo_sagemaker_geospatial.types.search_raster_data_collection_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/search-raster-data-collection"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sagemaker_geospatial.types.search_raster_data_collection_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_search_raster_data_collection(
