"""Generated from Smithy shape ``com.amazonaws.sagemakergeospatial#StopEarthObservationJob``."""
...
def handle_response(
    ...
    out: capo_sagemaker_geospatial.types.stop_earth_observation_job_output.StopEarthObservationJobOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/earth-observation-jobs/stop"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sagemaker_geospatial.types.stop_earth_observation_job_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_stop_earth_observation_job(
