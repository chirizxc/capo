...
class OutputConfigInput(TypedDict, closed=True):
    s3_data: "capo_sagemaker_geospatial.types.export_s3_data_input.ExportS3DataInput"
...
def serialize_json(value: OutputConfigInput) -> dict:
    ...
    out["S3Data"] = capo_sagemaker_geospatial.types.export_s3_data_input.serialize_json(
        value["s3_data"]
    )
    return out
...
def deserialize_json(data: dict) -> OutputConfigInput:
    ...
    if data.get("S3Data") is not None:
        ...
        out["s3_data"] = (
            capo_sagemaker_geospatial.types.export_s3_data_input.deserialize_json(
                data["S3Data"]
            )
        )
    else:
