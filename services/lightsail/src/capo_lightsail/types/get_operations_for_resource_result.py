...
class GetOperationsForResourceResult(TypedDict, closed=True):
    operations: NotRequired["capo_lightsail.types.operation_list.OperationList"]
    """<p>An array of objects that describe the result of the action, such as the status of the request, the timestamp of the request, and the resources affected by the request.</p>"""
    next_page_count: NotRequired["capo_lightsail.types.string.string"]
...
def deserialize_aws_json_1_1(data: dict) -> GetOperationsForResourceResult:
    ...
    if data.get("nextPageCount") is not None:
