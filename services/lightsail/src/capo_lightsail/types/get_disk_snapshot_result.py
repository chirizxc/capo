...
class GetDiskSnapshotResult(TypedDict, closed=True):
    disk_snapshot: NotRequired["capo_lightsail.types.disk_snapshot.DiskSnapshot"]
...
def serialize_aws_json_1_1(value: GetDiskSnapshotResult) -> dict:
    ...
    if "disk_snapshot" in value:
        ...
        out["diskSnapshot"] = capo_lightsail.types.disk_snapshot.serialize_aws_json_1_1(
            value["disk_snapshot"]
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> GetDiskSnapshotResult:
    ...
    if data.get("diskSnapshot") is not None:
        import capo_lightsail.types.disk_snapshot

        out["disk_snapshot"] = (
            capo_lightsail.types.disk_snapshot.deserialize_aws_json_1_1(
                data["diskSnapshot"]
            )
        )
    return out
