"""Generated from Smithy shape ``com.amazonaws.lightsail#InstanceNetworking``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_lightsail.types.instance_port_info_list
    import capo_lightsail.types.monthly_transfer


class InstanceNetworking(TypedDict, closed=True):
    monthly_transfer: NotRequired[
        "capo_lightsail.types.monthly_transfer.MonthlyTransfer"
    ]
    """<p>The amount of data in GB allocated for monthly data transfers.</p>"""
    ports: NotRequired[
        "capo_lightsail.types.instance_port_info_list.InstancePortInfoList"
    ]
    """<p>An array of key-value pairs containing information about the ports on the instance.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: InstanceNetworking) -> dict:
    out: dict = {}
    if "monthly_transfer" in value:
        import capo_lightsail.types.monthly_transfer

        out["monthlyTransfer"] = (
            capo_lightsail.types.monthly_transfer.serialize_aws_json_1_1(
                value["monthly_transfer"]
            )
        )
    if "ports" in value:
        import capo_lightsail.types.instance_port_info_list

        out["ports"] = (
            capo_lightsail.types.instance_port_info_list.serialize_aws_json_1_1(
                value["ports"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> InstanceNetworking:
    out: InstanceNetworking = {}  # type: ignore[typeddict-item]
    if data.get("monthlyTransfer") is not None:
        import capo_lightsail.types.monthly_transfer

        out["monthly_transfer"] = (
            capo_lightsail.types.monthly_transfer.deserialize_aws_json_1_1(
                data["monthlyTransfer"]
            )
        )
    if data.get("ports") is not None:
        import capo_lightsail.types.instance_port_info_list

        out["ports"] = (
            capo_lightsail.types.instance_port_info_list.deserialize_aws_json_1_1(
                data["ports"]
            )
        )
    return out
