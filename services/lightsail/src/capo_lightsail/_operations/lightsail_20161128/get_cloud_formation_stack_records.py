"""Generated from Smithy shape ``com.amazonaws.lightsail#GetCloudFormationStackRecords``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.get_cloud_formation_stack_records_result.GetCloudFormationStackRecordsResult = capo_lightsail.types.get_cloud_formation_stack_records_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/GetCloudFormationStackRecords"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.GetCloudFormationStackRecords"
    body: bytes | None = json.dumps(
        capo_lightsail.types.get_cloud_formation_stack_records_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_cloud_formation_stack_records(
