"""Generated from Smithy shape ``com.amazonaws.lightsail#GetRelationalDatabaseSnapshot``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.get_relational_database_snapshot_result.GetRelationalDatabaseSnapshotResult = capo_lightsail.types.get_relational_database_snapshot_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/GetRelationalDatabaseSnapshot"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.GetRelationalDatabaseSnapshot"
    body: bytes | None = json.dumps(
        capo_lightsail.types.get_relational_database_snapshot_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_relational_database_snapshot(
