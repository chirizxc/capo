"""Generated from Smithy shape ``com.amazonaws.lightsail#UpdateDistributionBundle``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.update_distribution_bundle_result.UpdateDistributionBundleResult = capo_lightsail.types.update_distribution_bundle_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/UpdateDistributionBundle"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.UpdateDistributionBundle"
    body: bytes | None = json.dumps(
        capo_lightsail.types.update_distribution_bundle_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_distribution_bundle(
