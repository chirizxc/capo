"""Generated from Smithy shape ``com.amazonaws.lightsail#UpdateBucketBundle``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.update_bucket_bundle_result.UpdateBucketBundleResult = (
        capo_lightsail.types.update_bucket_bundle_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/UpdateBucketBundle"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.UpdateBucketBundle"
    body: bytes | None = json.dumps(
        capo_lightsail.types.update_bucket_bundle_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_bucket_bundle(
