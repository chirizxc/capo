"""Generated from Smithy shape ``com.amazonaws.lightsail#GetBucketBundles``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.get_bucket_bundles_result.GetBucketBundlesResult = (
        capo_lightsail.types.get_bucket_bundles_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/GetBucketBundles"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.GetBucketBundles"
    body: bytes | None = json.dumps(
        capo_lightsail.types.get_bucket_bundles_request.serialize_aws_json_1_1(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_bucket_bundles(
