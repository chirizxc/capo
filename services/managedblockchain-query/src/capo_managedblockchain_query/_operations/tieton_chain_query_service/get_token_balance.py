"""Generated from Smithy shape ``com.amazonaws.managedblockchainquery#GetTokenBalance``."""
...
def handle_response(
    ...
    out: capo_managedblockchain_query.types.get_token_balance_output.GetTokenBalanceOutput = capo_managedblockchain_query.types.get_token_balance_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/get-token-balance"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_managedblockchain_query.types.get_token_balance_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
