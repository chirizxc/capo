"""Generated from Smithy shape ``com.amazonaws.managedblockchainquery#BatchGetTokenBalance``."""
...
def handle_response(
    ...
    out: capo_managedblockchain_query.types.batch_get_token_balance_output.BatchGetTokenBalanceOutput = capo_managedblockchain_query.types.batch_get_token_balance_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/batch-get-token-balance"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_managedblockchain_query.types.batch_get_token_balance_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_get_token_balance(
