"""Generated from Smithy shape ``com.amazonaws.managedblockchainquery#ListAssetContracts``."""
...
def handle_response(
    ...
    out: capo_managedblockchain_query.types.list_asset_contracts_output.ListAssetContractsOutput = capo_managedblockchain_query.types.list_asset_contracts_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-asset-contracts"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_managedblockchain_query.types.list_asset_contracts_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_asset_contracts(
