...
if TYPE_CHECKING:
    import capo_networkmanager.types.network_route

NetworkRouteList: TypeAlias = list[
    "capo_networkmanager.types.network_route.NetworkRoute"
]
...
def serialize_json(value: NetworkRouteList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_networkmanager.types.network_route.serialize_json(item))
    return out
