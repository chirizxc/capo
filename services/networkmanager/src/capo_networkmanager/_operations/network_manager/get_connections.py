"""Generated from Smithy shape ``com.amazonaws.networkmanager#GetConnections``."""
...
def handle_response(
    ...
    out: capo_networkmanager.types.get_connections_response.GetConnectionsResponse = (
        capo_networkmanager.types.get_connections_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/global-networks/{GlobalNetworkId}/connections"
    url = url.replace("{GlobalNetworkId}", quote(input_["global_network_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "connection_ids" in input_:
        for item in input_["connection_ids"]:
            params.append(("connectionIds", item))
    if "device_id" in input_:
        params.append(("deviceId", input_["device_id"]))
