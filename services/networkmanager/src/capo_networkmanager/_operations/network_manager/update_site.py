...
def handle_response(
    ...
    out: capo_networkmanager.types.update_site_response.UpdateSiteResponse = (
        capo_networkmanager.types.update_site_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{SiteId}", quote(input_["site_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_networkmanager.types.update_site_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_site(
    ...
    capo_networkmanager.types.update_site_response.UpdateSiteResponse, zapros.Response
]:
