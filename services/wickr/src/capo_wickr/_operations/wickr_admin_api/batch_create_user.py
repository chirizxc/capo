"""Generated from Smithy shape ``com.amazonaws.wickr#BatchCreateUser``."""
...
def handle_response(
    ...
    out: capo_wickr.types.batch_create_user_response.BatchCreateUserResponse = (
        capo_wickr.types.batch_create_user_response.deserialize_json(
...
def build_request(
    ...
    if "client_token" in input_:
        headers["X-Client-Token"] = input_["client_token"]
    body: bytes | None = json.dumps(
        capo_wickr.types.batch_create_user_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def batch_create_user(
    ...
    capo_wickr.types.batch_create_user_response.BatchCreateUserResponse, zapros.Response
]:
...
async def async_batch_create_user(
