"""Generated from Smithy shape ``com.amazonaws.wickr#GetUser``."""
...
def handle_response(
    ...
    out: capo_wickr.types.get_user_response.GetUserResponse = (
        capo_wickr.types.get_user_response.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_wickr.types.get_user_response.GetUserResponse = (
        capo_wickr.types.get_user_response.deserialize_json(
...
def build_request(
    ...
    import capo_wickr._protocol.serialize

    url = endpoint.url.rstrip("/") + "/networks/{networkId}/users/{userId}"
    ...
    if "start_time" in input_:
        params.append(
            (
                "startTime",
                capo_wickr._protocol.serialize.fmt_epoch_seconds(input_["start_time"]),
            )
        )
    if "end_time" in input_:
        params.append(
            (
                "endTime",
                capo_wickr._protocol.serialize.fmt_epoch_seconds(input_["end_time"]),
            )
        )
...
def get_user(
    options: OperationOptions, input_: capo_wickr.types.get_user_request.GetUserRequest
) -> tuple[capo_wickr.types.get_user_response.GetUserResponse, zapros.Response]:
