"""Generated from Smithy shape ``com.amazonaws.wickr#ListBots``."""
...
def handle_response(
    ...
    out: capo_wickr.types.list_bots_response.ListBotsResponse = (
        capo_wickr.types.list_bots_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/networks/{networkId}/bots"
    ...
    if "sort_fields" in input_:
        params.append(("sortFields", input_["sort_fields"]))
    if "sort_direction" in input_:
        params.append(
            (
                "sortDirection",
                capo_wickr.types.sort_direction.serialize_json(
                    input_["sort_direction"]
                ),
            )
        )
    if "display_name" in input_:
        params.append(("displayName", input_["display_name"]))
    if "username" in input_:
        params.append(("username", input_["username"]))
    if "status" in input_:
        params.append(
            (
                "status",
                str(capo_wickr.types.bot_status.serialize_json(input_["status"])),
            )
        )
    if "group_id" in input_:
        params.append(("groupId", input_["group_id"]))
...
def list_bots(
    ...
    input_: capo_wickr.types.list_bots_request.ListBotsRequest,
) -> tuple[capo_wickr.types.list_bots_response.ListBotsResponse, zapros.Response]:
