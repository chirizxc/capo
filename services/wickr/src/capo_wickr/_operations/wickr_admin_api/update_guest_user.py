"""Generated from Smithy shape ``com.amazonaws.wickr#UpdateGuestUser``."""
...
def handle_response(
    ...
    out: capo_wickr.types.update_guest_user_response.UpdateGuestUserResponse = (
        capo_wickr.types.update_guest_user_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/networks/{networkId}/guest-users/{usernameHash}"
    url = url.replace("{networkId}", quote(input_["network_id"], safe=""))
    url = url.replace("{usernameHash}", quote(input_["username_hash"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wickr.types.update_guest_user_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_guest_user(
    ...
    capo_wickr.types.update_guest_user_response.UpdateGuestUserResponse, zapros.Response
]:
...
async def async_update_guest_user(
