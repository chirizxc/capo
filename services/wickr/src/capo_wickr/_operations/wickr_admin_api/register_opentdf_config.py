...
def handle_response(
    ...
    out: capo_wickr.types.register_opentdf_config_response.RegisterOpentdfConfigResponse = capo_wickr.types.register_opentdf_config_response.deserialize_json(
...
def build_request(
    ...
    if "dry_run" in input_:
        params.append(("dryRun", "true" if input_["dry_run"] else "false"))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wickr.types.register_opentdf_config_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
