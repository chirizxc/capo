...
class BatchLookupUserUnameResponse(TypedDict, closed=True):
    ...
    successful: NotRequired[
        "capo_wickr.types.batch_uname_success_response_items.BatchUnameSuccessResponseItems"
    ]
    """<p>A list of successfully resolved username hashes with their corresponding email addresses.</p>"""
    failed: NotRequired[
        "capo_wickr.types.batch_uname_error_response_items.BatchUnameErrorResponseItems"
    ]
...
def serialize_json(value: BatchLookupUserUnameResponse) -> dict:
    ...
    if "successful" in value:
        ...
        out["successful"] = (
            capo_wickr.types.batch_uname_success_response_items.serialize_json(
                value["successful"]
            )
        )
    if "failed" in value:
        ...
        out["failed"] = (
            capo_wickr.types.batch_uname_error_response_items.serialize_json(
                value["failed"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> BatchLookupUserUnameResponse:
    ...
    if data.get("successful") is not None:
        ...
        out["successful"] = (
            capo_wickr.types.batch_uname_success_response_items.deserialize_json(
                data["successful"]
            )
        )
    if data.get("failed") is not None:
        ...
        out["failed"] = (
            capo_wickr.types.batch_uname_error_response_items.deserialize_json(
                data["failed"]
            )
        )
    return out
