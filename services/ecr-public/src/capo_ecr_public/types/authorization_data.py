...
if TYPE_CHECKING:
    import capo_ecr_public.types.base64
    import capo_ecr_public.types.expiration_timestamp


class AuthorizationData(TypedDict, closed=True):
    authorization_token: NotRequired["capo_ecr_public.types.base64.Base64"]
    ...
    expires_at: NotRequired[
        "capo_ecr_public.types.expiration_timestamp.ExpirationTimestamp"
    ]
...
def serialize_aws_json_1_1(value: AuthorizationData) -> dict:
    ...
    if "expires_at" in value:
        ...
        out["expiresAt"] = (
            capo_ecr_public.types.expiration_timestamp.serialize_aws_json_1_1(
                value["expires_at"]
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> AuthorizationData:
    ...
    if data.get("expiresAt") is not None:
        ...
        out["expires_at"] = (
            capo_ecr_public.types.expiration_timestamp.deserialize_aws_json_1_1(
                data["expiresAt"]
            )
        )
    return out
