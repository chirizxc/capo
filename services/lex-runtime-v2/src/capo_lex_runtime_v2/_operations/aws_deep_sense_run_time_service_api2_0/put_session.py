"""Generated from Smithy shape ``com.amazonaws.lexruntimev2#PutSession``."""
...
def handle_response(
    ...
    out: capo_lex_runtime_v2.types.put_session_response.PutSessionResponse = {
        "audio_stream": _iter
...
def build_request(
    ...
    if "response_content_type" in input_:
        headers["ResponseContentType"] = input_["response_content_type"]
    body: bytes | None = json.dumps(
        capo_lex_runtime_v2.types.put_session_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def put_session(
    ...
    capo_lex_runtime_v2.types.put_session_response.PutSessionResponse, zapros.Response
]:
