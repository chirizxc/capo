"""Generated from Smithy shape ``com.amazonaws.ssmguiconnect#UpdateConnectionRecordingPreferences``."""
...
def handle_response(
    ...
    out: capo_ssm_guiconnect.types.update_connection_recording_preferences_response.UpdateConnectionRecordingPreferencesResponse = capo_ssm_guiconnect.types.update_connection_recording_preferences_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateConnectionRecordingPreferences"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_guiconnect.types.update_connection_recording_preferences_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_connection_recording_preferences(
