...
if TYPE_CHECKING:
    import capo_pinpoint.types.campaign_status


class CampaignState(TypedDict, closed=True):
    campaign_status: NotRequired["capo_pinpoint.types.campaign_status.CampaignStatus"]
...
def serialize_json(value: CampaignState) -> dict:
    ...
    if "campaign_status" in value:
        ...
        out["CampaignStatus"] = capo_pinpoint.types.campaign_status.serialize_json(
...
def deserialize_json(data: dict) -> CampaignState:
    ...
    if data.get("CampaignStatus") is not None:
        import capo_pinpoint.types.campaign_status

        out["campaign_status"] = capo_pinpoint.types.campaign_status.deserialize_json(
            data["CampaignStatus"]
        )
    return out
