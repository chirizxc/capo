...
if TYPE_CHECKING:
    import capo_pinpoint.types.write_treatment_resource

ListOfWriteTreatmentResource: TypeAlias = list[
    "capo_pinpoint.types.write_treatment_resource.WriteTreatmentResource"
]
...
def serialize_json(value: ListOfWriteTreatmentResource) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_pinpoint.types.write_treatment_resource.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ListOfWriteTreatmentResource:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_pinpoint.types.write_treatment_resource.deserialize_json(item))
    return out
