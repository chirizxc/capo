...
class Condition(TypedDict, closed=True):
    conditions: NotRequired[
        "capo_pinpoint.types.list_of_simple_condition.ListOfSimpleCondition"
    ]
    """<p>The conditions to evaluate for the activity.</p>"""
    operator: NotRequired["capo_pinpoint.types.operator.Operator"]
...
def serialize_json(value: Condition) -> dict:
    ...
    if "conditions" in value:
        ...
        out["Conditions"] = capo_pinpoint.types.list_of_simple_condition.serialize_json(
            value["conditions"]
        )
    if "operator" in value:
        import capo_pinpoint.types.operator

        out["Operator"] = capo_pinpoint.types.operator.serialize_json(value["operator"])
    return out
...
def deserialize_json(data: dict) -> Condition:
    ...
    if data.get("Conditions") is not None:
        ...
        out["conditions"] = (
            capo_pinpoint.types.list_of_simple_condition.deserialize_json(
                data["Conditions"]
            )
        )
