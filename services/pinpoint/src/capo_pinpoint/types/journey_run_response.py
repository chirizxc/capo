"""Generated from Smithy shape ``com.amazonaws.pinpoint#JourneyRunResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_pinpoint.types.__string
    import capo_pinpoint.types.journey_run_status


class JourneyRunResponse(TypedDict, closed=True):
    creation_time: NotRequired["capo_pinpoint.types.__string.__string"]
    """<p>The time when the journey run was created or scheduled, in ISO 8601 format.</p>"""
    last_update_time: NotRequired["capo_pinpoint.types.__string.__string"]
    """<p>The last time the journey run was updated, in ISO 8601 format..</p>"""
    run_id: NotRequired["capo_pinpoint.types.__string.__string"]
    """<p>The unique identifier for the run.</p>"""
    status: NotRequired["capo_pinpoint.types.journey_run_status.JourneyRunStatus"]
    """<p>The current status of the journey run.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: JourneyRunResponse) -> dict:
    out: dict = {}
    if "creation_time" in value:
        out["CreationTime"] = value["creation_time"]
    if "last_update_time" in value:
        out["LastUpdateTime"] = value["last_update_time"]
    if "run_id" in value:
        out["RunId"] = value["run_id"]
    if "status" in value:
        import capo_pinpoint.types.journey_run_status

        out["Status"] = capo_pinpoint.types.journey_run_status.serialize_json(
            value["status"]
        )
    return out


def deserialize_json(data: dict) -> JourneyRunResponse:
    out: JourneyRunResponse = {}  # type: ignore[typeddict-item]
    if data.get("CreationTime") is not None:
        out["creation_time"] = data["CreationTime"]
    if data.get("LastUpdateTime") is not None:
        out["last_update_time"] = data["LastUpdateTime"]
    if data.get("RunId") is not None:
        out["run_id"] = data["RunId"]
    if data.get("Status") is not None:
        import capo_pinpoint.types.journey_run_status

        out["status"] = capo_pinpoint.types.journey_run_status.deserialize_json(
            data["Status"]
        )
    return out
