...
if TYPE_CHECKING:
    ...
    import capo_pinpoint.types.journey_sms_message


class SMSMessageActivity(TypedDict, closed=True):
    message_config: NotRequired[
        "capo_pinpoint.types.journey_sms_message.JourneySMSMessage"
    ]
...
def serialize_json(value: SMSMessageActivity) -> dict:
    ...
    if "message_config" in value:
        ...
        out["MessageConfig"] = capo_pinpoint.types.journey_sms_message.serialize_json(
...
def deserialize_json(data: dict) -> SMSMessageActivity:
    ...
    if data.get("MessageConfig") is not None:
        ...
        out["message_config"] = (
            capo_pinpoint.types.journey_sms_message.deserialize_json(
                data["MessageConfig"]
            )
        )
