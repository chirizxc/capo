...
class TemplateVersionsResponse(TypedDict, closed=True):
    item: NotRequired[
        "capo_pinpoint.types.list_of_template_version_response.ListOfTemplateVersionResponse"
    ]
...
def serialize_json(value: TemplateVersionsResponse) -> dict:
    ...
    if "item" in value:
        ...
        out["Item"] = (
            capo_pinpoint.types.list_of_template_version_response.serialize_json(
                value["item"]
            )
        )
    if "message" in value:
...
def deserialize_json(data: dict) -> TemplateVersionsResponse:
    ...
    if data.get("Item") is not None:
        ...
        out["item"] = (
            capo_pinpoint.types.list_of_template_version_response.deserialize_json(
                data["Item"]
            )
        )
