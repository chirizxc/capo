"""Generated from Smithy shape ``com.amazonaws.pinpoint#ADMMessage``."""
...
class ADMMessage(TypedDict, closed=True):
    ...
    consolidation_key: NotRequired["capo_pinpoint.types.__string.__string"]
    ...
    expires_after: NotRequired["capo_pinpoint.types.__string.__string"]
    ...
    md5: NotRequired["capo_pinpoint.types.__string.__string"]
...
# --- restJson1 ser/de ---
def serialize_json(value: ADMMessage) -> dict:
    out: dict = {}
    if "action" in value:
        import capo_pinpoint.types.action
    ...
    if "body" in value:
        out["Body"] = value["body"]
    if "consolidation_key" in value:
        out["ConsolidationKey"] = value["consolidation_key"]
    if "data" in value:
        import capo_pinpoint.types.map_of__string
...
def deserialize_json(data: dict) -> ADMMessage:
    out: ADMMessage = {}  # type: ignore[typeddict-item]
    ...
    if data.get("Body") is not None:
        out["body"] = data["Body"]
    if data.get("ConsolidationKey") is not None:
        out["consolidation_key"] = data["ConsolidationKey"]
