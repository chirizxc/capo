"""Generated from Smithy shape ``com.amazonaws.pinpoint#GetCampaignsResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_pinpoint.types.campaigns_response


class GetCampaignsResponse(TypedDict, closed=True):
    campaigns_response: NotRequired[
        "capo_pinpoint.types.campaigns_response.CampaignsResponse"
    ]


# --- restJson1 ser/de ---
def serialize_json(value: GetCampaignsResponse) -> dict:
    out: dict = {}
    if "campaigns_response" in value:
        import capo_pinpoint.types.campaigns_response

        out["CampaignsResponse"] = (
            capo_pinpoint.types.campaigns_response.serialize_json(
                value["campaigns_response"]
            )
        )
    return out


def deserialize_json(data: dict) -> GetCampaignsResponse:
    out: GetCampaignsResponse = {}  # type: ignore[typeddict-item]
    if data.get("CampaignsResponse") is not None:
        import capo_pinpoint.types.campaigns_response

        out["campaigns_response"] = (
            capo_pinpoint.types.campaigns_response.deserialize_json(
                data["CampaignsResponse"]
            )
        )
    return out
