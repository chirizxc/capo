...
if TYPE_CHECKING:
    import capo_pinpoint.types.list_of_endpoint_response


class EndpointsResponse(TypedDict, closed=True):
    item: NotRequired[
        "capo_pinpoint.types.list_of_endpoint_response.ListOfEndpointResponse"
    ]
...
def serialize_json(value: EndpointsResponse) -> dict:
    ...
    if "item" in value:
        ...
        out["Item"] = capo_pinpoint.types.list_of_endpoint_response.serialize_json(
            value["item"]
        )
    return out
...
def deserialize_json(data: dict) -> EndpointsResponse:
    ...
    if data.get("Item") is not None:
        ...
        out["item"] = capo_pinpoint.types.list_of_endpoint_response.deserialize_json(
            data["Item"]
        )
    return out
