...
class EmailMessageActivity(TypedDict, closed=True):
    message_config: NotRequired[
        "capo_pinpoint.types.journey_email_message.JourneyEmailMessage"
    ]
...
def serialize_json(value: EmailMessageActivity) -> dict:
    ...
    if "message_config" in value:
        ...
        out["MessageConfig"] = capo_pinpoint.types.journey_email_message.serialize_json(
...
def deserialize_json(data: dict) -> EmailMessageActivity:
    ...
    if data.get("MessageConfig") is not None:
        ...
        out["message_config"] = (
            capo_pinpoint.types.journey_email_message.deserialize_json(
                data["MessageConfig"]
            )
        )
