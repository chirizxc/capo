"""Generated from Smithy shape ``com.amazonaws.pinpoint#GetJourneyRunExecutionActivityMetrics``."""
...
import capo_pinpoint._protocol.eventstream
...
def handle_response(
    ...
    out: capo_pinpoint.types.get_journey_run_execution_activity_metrics_response.GetJourneyRunExecutionActivityMetricsResponse = {
        "journey_run_execution_activity_metrics_response": capo_pinpoint.types.journey_run_execution_activity_metrics_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_pinpoint._auth._sigv4.build_sigv4_auth_scheme(
                "mobiletargeting", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace(
        "{JourneyActivityId}", quote(input_["journey_activity_id"], safe="")
    )
    url = url.replace("{JourneyId}", quote(input_["journey_id"], safe=""))
    url = url.replace("{RunId}", quote(input_["run_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "next_token" in input_:
        params.append(("next-token", input_["next_token"]))
    if "page_size" in input_:
        params.append(("page-size", input_["page_size"]))
...
async def async_get_journey_run_execution_activity_metrics(
