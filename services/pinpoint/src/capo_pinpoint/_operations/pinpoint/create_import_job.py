"""Generated from Smithy shape ``com.amazonaws.pinpoint#CreateImportJob``."""
...
def handle_response(
    ...
    out: capo_pinpoint.types.create_import_job_response.CreateImportJobResponse = {
        "import_job_response": capo_pinpoint.types.import_job_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/jobs/import"
    url = url.replace("{ApplicationId}", quote(input_["application_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "import_job_request" in input_:
        body: bytes | None = json.dumps(
            capo_pinpoint.types.import_job_request.serialize_json(
                input_["import_job_request"]
            ),
            allow_nan=False,
        ).encode()
