...
class ListBrokersResponse(TypedDict, closed=True):
    broker_summaries: NotRequired[
        "capo_mq.types.__list_of_broker_summary.__listOfBrokerSummary"
    ]
...
def serialize_json(value: ListBrokersResponse) -> dict:
    ...
    if "broker_summaries" in value:
        ...
        out["brokerSummaries"] = capo_mq.types.__list_of_broker_summary.serialize_json(
...
def deserialize_json(data: dict) -> ListBrokersResponse:
    ...
    if data.get("brokerSummaries") is not None:
        ...
        out["broker_summaries"] = (
            capo_mq.types.__list_of_broker_summary.deserialize_json(
