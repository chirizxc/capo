...
ComponentOverrides: TypeAlias = dict[
    "str",
    "capo_amplifyuibuilder.types.component_overrides_value.ComponentOverridesValue",
]
...
def serialize_json(input_to_serialize: ComponentOverrides) -> dict:
    ...
    for key, value in input_to_serialize.items():
        ...
        out[key] = capo_amplifyuibuilder.types.component_overrides_value.serialize_json(
            value
        )
    return out
...
def deserialize_json(data: dict) -> ComponentOverrides:
    ...
    for key, value in data.items():
        ...
        out[key] = (
            capo_amplifyuibuilder.types.component_overrides_value.deserialize_json(
                value
            )
        )
    return out
