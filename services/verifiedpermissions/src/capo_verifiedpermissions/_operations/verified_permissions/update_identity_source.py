"""Generated from Smithy shape ``com.amazonaws.verifiedpermissions#UpdateIdentitySource``."""
...
import capo_verifiedpermissions._protocol.eventstream
...
def handle_response(
    ...
    out: capo_verifiedpermissions.types.update_identity_source_output.UpdateIdentitySourceOutput = capo_verifiedpermissions.types.update_identity_source_output.deserialize_aws_json_1_0(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = (
                capo_verifiedpermissions._auth._sigv4.build_sigv4_auth_scheme(
                    "verifiedpermissions", options.region, endpoint_scheme
                )
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "VerifiedPermissions.UpdateIdentitySource"
    body: bytes | None = json.dumps(
        capo_verifiedpermissions.types.update_identity_source_input.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_identity_source(
