...
if TYPE_CHECKING:
    import capo_verifiedpermissions.types.batch_is_authorized_with_token_input_item
...
class BatchIsAuthorizedWithTokenOutputItem(TypedDict, closed=True):
    request: "capo_verifiedpermissions.types.batch_is_authorized_with_token_input_item.BatchIsAuthorizedWithTokenInputItem"
...
def serialize_aws_json_1_0(value: BatchIsAuthorizedWithTokenOutputItem) -> dict:
    ...
    out["request"] = (
        capo_verifiedpermissions.types.batch_is_authorized_with_token_input_item.serialize_aws_json_1_0(
            value["request"]
        )
    )
    import capo_verifiedpermissions.types.decision
...
def deserialize_aws_json_1_0(data: dict) -> BatchIsAuthorizedWithTokenOutputItem:
    ...
    if data.get("request") is not None:
        ...
        out["request"] = (
            capo_verifiedpermissions.types.batch_is_authorized_with_token_input_item.deserialize_aws_json_1_0(
                data["request"]
            )
        )
    else:
