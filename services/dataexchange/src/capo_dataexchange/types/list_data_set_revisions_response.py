...
class ListDataSetRevisionsResponse(TypedDict, closed=True):
    ...
    revisions: NotRequired[
        "capo_dataexchange.types.list_of_revision_entry.ListOfRevisionEntry"
    ]
...
def serialize_json(value: ListDataSetRevisionsResponse) -> dict:
    ...
    if "revisions" in value:
        ...
        out["Revisions"] = (
            capo_dataexchange.types.list_of_revision_entry.serialize_json(
                value["revisions"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListDataSetRevisionsResponse:
    ...
    if data.get("Revisions") is not None:
        ...
        out["revisions"] = (
            capo_dataexchange.types.list_of_revision_entry.deserialize_json(
                data["Revisions"]
            )
        )
    return out
