"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#CreateUser``."""
...
def handle_response(
    ...
    out: capo_directory_service_data.types.create_user_result.CreateUserResult = (
        capo_directory_service_data.types.create_user_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/Users/CreateUser"
    params: list[tuple[str, str]] = []
    if "directory_id" in input_:
        params.append(("DirectoryId", input_["directory_id"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.create_user_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
