"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#DeleteGroup``."""
...
def handle_response(
    ...
    out: capo_directory_service_data.types.delete_group_result.DeleteGroupResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/Groups/DeleteGroup"
    params: list[tuple[str, str]] = []
    if "directory_id" in input_:
        params.append(("DirectoryId", input_["directory_id"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.delete_group_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
