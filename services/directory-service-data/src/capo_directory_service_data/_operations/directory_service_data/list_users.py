"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#ListUsers``."""
...
import capo_directory_service_data._protocol.eventstream
...
def handle_response(
    ...
    out: capo_directory_service_data.types.list_users_result.ListUsersResult = (
        capo_directory_service_data.types.list_users_result.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = (
                capo_directory_service_data._auth._sigv4.build_sigv4_auth_scheme(
                    "ds-data", options.region, endpoint_scheme
                )
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/Users/ListUsers"
    params: list[tuple[str, str]] = []
    if "directory_id" in input_:
        params.append(("DirectoryId", input_["directory_id"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.list_users_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def list_users(
    ...
    capo_directory_service_data.types.list_users_result.ListUsersResult, zapros.Response
]:
