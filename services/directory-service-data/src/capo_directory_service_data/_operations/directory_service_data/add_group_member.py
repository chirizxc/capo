"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#AddGroupMember``."""
...
def handle_response(
    ...
    out: capo_directory_service_data.types.add_group_member_result.AddGroupMemberResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GroupMemberships/AddGroupMember"
    params: list[tuple[str, str]] = []
    if "directory_id" in input_:
        params.append(("DirectoryId", input_["directory_id"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.add_group_member_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
