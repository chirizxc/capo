...
if TYPE_CHECKING:
    ...
    import capo_resource_groups.types.group_lifecycle_events_status_message
...
class AccountSettings(TypedDict, closed=True):
    ...
    group_lifecycle_events_status: NotRequired[
        "capo_resource_groups.types.group_lifecycle_events_status.GroupLifecycleEventsStatus"
    ]
    """<p>The current status of the group lifecycle events feature.</p>"""
    group_lifecycle_events_status_message: NotRequired[
        "capo_resource_groups.types.group_lifecycle_events_status_message.GroupLifecycleEventsStatusMessage"
    ]
...
def serialize_json(value: AccountSettings) -> dict:
    ...
    if "group_lifecycle_events_status" in value:
        ...
        out["GroupLifecycleEventsStatus"] = (
            capo_resource_groups.types.group_lifecycle_events_status.serialize_json(
...
def deserialize_json(data: dict) -> AccountSettings:
    ...
    if data.get("GroupLifecycleEventsStatus") is not None:
        ...
        out["group_lifecycle_events_status"] = (
            capo_resource_groups.types.group_lifecycle_events_status.deserialize_json(
