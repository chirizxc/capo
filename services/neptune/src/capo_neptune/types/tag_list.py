...
if TYPE_CHECKING:
    import capo_neptune.types.tag

TagList: TypeAlias = list["capo_neptune.types.tag.Tag"]
...
def serialize_query(value: TagList, pairs: list[tuple[str, str]], prefix: str) -> None:
    ...
    for n, item in enumerate(value, 1):
        capo_neptune.types.tag.serialize_query(item, pairs, f"{prefix}.Tag.{n}")
...
def deserialize_query(el: Element) -> TagList:
    ...
    for child in el.findall("Tag"):
        out.append(capo_neptune.types.tag.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    if not value:
        ...
        return
    for n, item in enumerate(value, 1):
        capo_neptune.types.tag.serialize_query(item, pairs, f"{prefix}.{n}")
