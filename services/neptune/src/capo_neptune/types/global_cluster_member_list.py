...
if TYPE_CHECKING:
    import capo_neptune.types.global_cluster_member

GlobalClusterMemberList: TypeAlias = list[
    "capo_neptune.types.global_cluster_member.GlobalClusterMember"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_neptune.types.global_cluster_member.serialize_query(
...
def deserialize_query(el: Element) -> GlobalClusterMemberList:
    ...
    for child in el.findall("GlobalClusterMember"):
        out.append(capo_neptune.types.global_cluster_member.deserialize_query(child))
    return out
