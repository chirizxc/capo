...
if TYPE_CHECKING:
    import capo_neptune.types.vpc_security_group_membership

VpcSecurityGroupMembershipList: TypeAlias = list[
    "capo_neptune.types.vpc_security_group_membership.VpcSecurityGroupMembership"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_neptune.types.vpc_security_group_membership.serialize_query(
...
def deserialize_query(el: Element) -> VpcSecurityGroupMembershipList:
    ...
    for child in el.findall("VpcSecurityGroupMembership"):
        out.append(
            capo_neptune.types.vpc_security_group_membership.deserialize_query(child)
        )
    return out
