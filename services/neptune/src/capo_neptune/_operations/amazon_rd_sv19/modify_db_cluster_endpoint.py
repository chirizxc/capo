"""Generated from Smithy shape ``com.amazonaws.neptune#ModifyDBClusterEndpoint``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ModifyDBClusterEndpointResult")
    out: capo_neptune.types.modify_db_cluster_endpoint_output.ModifyDBClusterEndpointOutput = capo_neptune.types.modify_db_cluster_endpoint_output.deserialize_query(
...
def build_request(
    ...
    capo_neptune.types.modify_db_cluster_endpoint_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_modify_db_cluster_endpoint(
