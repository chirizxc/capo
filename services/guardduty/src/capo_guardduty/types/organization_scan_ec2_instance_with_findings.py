...
class OrganizationScanEc2InstanceWithFindings(TypedDict, closed=True):
    ebs_volumes: NotRequired[
        "capo_guardduty.types.organization_ebs_volumes.OrganizationEbsVolumes"
    ]
...
def serialize_json(value: OrganizationScanEc2InstanceWithFindings) -> dict:
    ...
    if "ebs_volumes" in value:
        ...
        out["ebsVolumes"] = (
            capo_guardduty.types.organization_ebs_volumes.serialize_json(
                value["ebs_volumes"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> OrganizationScanEc2InstanceWithFindings:
    ...
    if data.get("ebsVolumes") is not None:
        ...
        out["ebs_volumes"] = (
            capo_guardduty.types.organization_ebs_volumes.deserialize_json(
                data["ebsVolumes"]
            )
        )
    return out
