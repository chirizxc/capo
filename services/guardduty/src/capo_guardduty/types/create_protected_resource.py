...
if TYPE_CHECKING:
    import capo_guardduty.types.create_s3_bucket_resource


class CreateProtectedResource(TypedDict, closed=True):
    s3_bucket: NotRequired[
        "capo_guardduty.types.create_s3_bucket_resource.CreateS3BucketResource"
    ]
...
def serialize_json(value: CreateProtectedResource) -> dict:
    ...
    if "s3_bucket" in value:
        ...
        out["s3Bucket"] = capo_guardduty.types.create_s3_bucket_resource.serialize_json(
            value["s3_bucket"]
        )
    return out
...
def deserialize_json(data: dict) -> CreateProtectedResource:
    ...
    if data.get("s3Bucket") is not None:
        ...
        out["s3_bucket"] = (
            capo_guardduty.types.create_s3_bucket_resource.deserialize_json(
                data["s3Bucket"]
            )
        )
    return out
