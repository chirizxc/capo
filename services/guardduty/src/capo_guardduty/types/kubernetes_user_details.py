...
class KubernetesUserDetails(TypedDict, closed=True):
    ...
    session_name: NotRequired["capo_guardduty.types.session_name_list.SessionNameList"]
    ...
    impersonated_user: NotRequired[
        "capo_guardduty.types.impersonated_user.ImpersonatedUser"
    ]
...
def serialize_json(value: KubernetesUserDetails) -> dict:
    ...
    if "session_name" in value:
        ...
        out["sessionName"] = capo_guardduty.types.session_name_list.serialize_json(
            value["session_name"]
        )
    if "impersonated_user" in value:
        import capo_guardduty.types.impersonated_user

        out["impersonatedUser"] = capo_guardduty.types.impersonated_user.serialize_json(
...
def deserialize_json(data: dict) -> KubernetesUserDetails:
    ...
    if data.get("impersonatedUser") is not None:
        ...
        out["impersonated_user"] = (
            capo_guardduty.types.impersonated_user.deserialize_json(
