...
if TYPE_CHECKING:
    import capo_guardduty.types.kubernetes_audit_logs_configuration_result


class KubernetesConfigurationResult(TypedDict, closed=True):
    audit_logs: NotRequired[
        "capo_guardduty.types.kubernetes_audit_logs_configuration_result.KubernetesAuditLogsConfigurationResult"
    ]
...
def serialize_json(value: KubernetesConfigurationResult) -> dict:
    ...
    if "audit_logs" in value:
        ...
        out["auditLogs"] = (
            capo_guardduty.types.kubernetes_audit_logs_configuration_result.serialize_json(
                value["audit_logs"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> KubernetesConfigurationResult:
    ...
    if data.get("auditLogs") is not None:
        ...
        out["audit_logs"] = (
            capo_guardduty.types.kubernetes_audit_logs_configuration_result.deserialize_json(
                data["auditLogs"]
            )
        )
    return out
