...
if TYPE_CHECKING:
    import capo_guardduty.types.coverage_filter_condition
    import capo_guardduty.types.coverage_filter_criterion_key


class CoverageFilterCriterion(TypedDict, closed=True):
    criterion_key: NotRequired[
        "capo_guardduty.types.coverage_filter_criterion_key.CoverageFilterCriterionKey"
    ]
    ...
    filter_condition: NotRequired[
        "capo_guardduty.types.coverage_filter_condition.CoverageFilterCondition"
    ]
...
def serialize_json(value: CoverageFilterCriterion) -> dict:
    ...
    if "criterion_key" in value:
        ...
        out["criterionKey"] = (
            capo_guardduty.types.coverage_filter_criterion_key.serialize_json(
                value["criterion_key"]
            )
        )
    if "filter_condition" in value:
        ...
        out["filterCondition"] = (
            capo_guardduty.types.coverage_filter_condition.serialize_json(
...
def deserialize_json(data: dict) -> CoverageFilterCriterion:
    ...
    if data.get("criterionKey") is not None:
        import capo_guardduty.types.coverage_filter_criterion_key

        out["criterion_key"] = (
            capo_guardduty.types.coverage_filter_criterion_key.deserialize_json(
                data["criterionKey"]
            )
        )
    if data.get("filterCondition") is not None:
        import capo_guardduty.types.coverage_filter_condition

        out["filter_condition"] = (
            capo_guardduty.types.coverage_filter_condition.deserialize_json(
