...
class GetDetectorResponse(TypedDict, closed=True):
    ...
    service_role: NotRequired["capo_guardduty.types.string.String"]
    """<p>The GuardDuty service role.</p>"""
    status: NotRequired["capo_guardduty.types.detector_status.DetectorStatus"]
    ...
    features: NotRequired[
        "capo_guardduty.types.detector_feature_configurations_results.DetectorFeatureConfigurationsResults"
    ]
...
def serialize_json(value: GetDetectorResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_guardduty.types.detector_status.serialize_json(
            value["status"]
        )
    ...
    if "features" in value:
        ...
        out["features"] = (
            capo_guardduty.types.detector_feature_configurations_results.serialize_json(
                value["features"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> GetDetectorResponse:
    ...
    if data.get("features") is not None:
        ...
        out["features"] = (
            capo_guardduty.types.detector_feature_configurations_results.deserialize_json(
                data["features"]
            )
        )
    return out
