...
class FilterCriteria(TypedDict, closed=True):
    filter_criterion: NotRequired[
        "capo_guardduty.types.filter_criterion_list.FilterCriterionList"
    ]
...
def serialize_json(value: FilterCriteria) -> dict:
    ...
    if "filter_criterion" in value:
        ...
        out["filterCriterion"] = (
            capo_guardduty.types.filter_criterion_list.serialize_json(
...
def deserialize_json(data: dict) -> FilterCriteria:
    ...
    if data.get("filterCriterion") is not None:
        ...
        out["filter_criterion"] = (
            capo_guardduty.types.filter_criterion_list.deserialize_json(
