"""Generated from Smithy shape ``com.amazonaws.guardduty#Threat``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_guardduty.types.item_details_list
    import capo_guardduty.types.item_paths
    import capo_guardduty.types.long
    import capo_guardduty.types.string


class Threat(TypedDict, closed=True):
    name: NotRequired["capo_guardduty.types.string.String"]
    """<p>Name of the detected threat that caused GuardDuty to generate this finding.</p>"""
    source: NotRequired["capo_guardduty.types.string.String"]
    """<p>Source of the threat that generated this finding.</p>"""
    item_paths: NotRequired["capo_guardduty.types.item_paths.ItemPaths"]
    """<p>Information about the nested item path and hash of the protected resource.</p>"""
    count: NotRequired["capo_guardduty.types.long.Long"]
    """<p>The number of occurrences of this specific threat detected during the scan.</p>"""
    hash: NotRequired["capo_guardduty.types.string.String"]
    """<p>The hash identifier of the detected malware threat.</p>"""
    item_details: NotRequired["capo_guardduty.types.item_details_list.ItemDetailsList"]
    """<p>Detailed information about the detected malware threat.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: Threat) -> dict:
    out: dict = {}
    if "name" in value:
        out["name"] = value["name"]
    if "source" in value:
        out["source"] = value["source"]
    if "item_paths" in value:
        import capo_guardduty.types.item_paths

        out["itemPaths"] = capo_guardduty.types.item_paths.serialize_json(
            value["item_paths"]
        )
    if "count" in value:
        out["count"] = value["count"]
    if "hash" in value:
        out["hash"] = value["hash"]
    if "item_details" in value:
        import capo_guardduty.types.item_details_list

        out["itemDetails"] = capo_guardduty.types.item_details_list.serialize_json(
            value["item_details"]
        )
    return out


def deserialize_json(data: dict) -> Threat:
    out: Threat = {}  # type: ignore[typeddict-item]
    if data.get("name") is not None:
        out["name"] = data["name"]
    if data.get("source") is not None:
        out["source"] = data["source"]
    if data.get("itemPaths") is not None:
        import capo_guardduty.types.item_paths

        out["item_paths"] = capo_guardduty.types.item_paths.deserialize_json(
            data["itemPaths"]
        )
    if data.get("count") is not None:
        out["count"] = data["count"]
    if data.get("hash") is not None:
        out["hash"] = data["hash"]
    if data.get("itemDetails") is not None:
        import capo_guardduty.types.item_details_list

        out["item_details"] = capo_guardduty.types.item_details_list.deserialize_json(
            data["itemDetails"]
        )
    return out
