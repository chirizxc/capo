"""Generated from Smithy shape ``com.amazonaws.guardduty#UpdateOrganizationConfiguration``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.update_organization_configuration_response.UpdateOrganizationConfigurationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/admin"
    url = url.replace("{DetectorId}", quote(input_["detector_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.update_organization_configuration_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
