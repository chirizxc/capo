"""Generated from Smithy shape ``com.amazonaws.guardduty#UpdateFindingsFeedback``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.update_findings_feedback_response.UpdateFindingsFeedbackResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/findings/feedback"
    url = url.replace("{DetectorId}", quote(input_["detector_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.update_findings_feedback_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_findings_feedback(
