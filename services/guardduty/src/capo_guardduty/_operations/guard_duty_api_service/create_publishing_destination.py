"""Generated from Smithy shape ``com.amazonaws.guardduty#CreatePublishingDestination``."""
...
import capo_guardduty._protocol.eventstream
...
def handle_response(
    ...
    out: capo_guardduty.types.create_publishing_destination_response.CreatePublishingDestinationResponse = capo_guardduty.types.create_publishing_destination_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_guardduty._auth._sigv4.build_sigv4_auth_scheme(
                "guardduty", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/publishingDestination"
    url = url.replace("{DetectorId}", quote(input_["detector_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.create_publishing_destination_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_publishing_destination(
