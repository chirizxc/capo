"""Generated from Smithy shape ``com.amazonaws.guardduty#GetUsageStatistics``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.get_usage_statistics_response.GetUsageStatisticsResponse = capo_guardduty.types.get_usage_statistics_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/usage/statistics"
    url = url.replace("{DetectorId}", quote(input_["detector_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.get_usage_statistics_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
