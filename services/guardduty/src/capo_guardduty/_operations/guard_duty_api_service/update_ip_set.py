"""Generated from Smithy shape ``com.amazonaws.guardduty#UpdateIPSet``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.update_ip_set_response.UpdateIPSetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{IpSetId}", quote(input_["ip_set_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.update_ip_set_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_ip_set(
    ...
    capo_guardduty.types.update_ip_set_response.UpdateIPSetResponse, zapros.Response
]:
