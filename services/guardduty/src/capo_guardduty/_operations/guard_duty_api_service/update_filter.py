...
def handle_response(
    ...
    out: capo_guardduty.types.update_filter_response.UpdateFilterResponse = (
        capo_guardduty.types.update_filter_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{FilterName}", quote(input_["filter_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.update_filter_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_filter(
    ...
    capo_guardduty.types.update_filter_response.UpdateFilterResponse, zapros.Response
]:
