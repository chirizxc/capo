"""Generated from Smithy shape ``com.amazonaws.guardduty#ListFindings``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.list_findings_response.ListFindingsResponse = (
        capo_guardduty.types.list_findings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/findings"
    url = url.replace("{DetectorId}", quote(input_["detector_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.list_findings_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def list_findings(
    ...
    capo_guardduty.types.list_findings_response.ListFindingsResponse, zapros.Response
]:
