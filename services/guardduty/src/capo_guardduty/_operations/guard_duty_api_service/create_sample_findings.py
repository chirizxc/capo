"""Generated from Smithy shape ``com.amazonaws.guardduty#CreateSampleFindings``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.create_sample_findings_response.CreateSampleFindingsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/findings/create"
    url = url.replace("{DetectorId}", quote(input_["detector_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.create_sample_findings_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
