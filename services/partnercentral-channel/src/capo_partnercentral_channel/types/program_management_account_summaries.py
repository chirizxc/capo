...
ProgramManagementAccountSummaries: TypeAlias = list[
    "capo_partnercentral_channel.types.program_management_account_summary.ProgramManagementAccountSummary"
]
...
def serialize_aws_json_1_0(value: ProgramManagementAccountSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_partnercentral_channel.types.program_management_account_summary.serialize_aws_json_1_0(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_0(data: list) -> ProgramManagementAccountSummaries:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_partnercentral_channel.types.program_management_account_summary.deserialize_aws_json_1_0(
                item
            )
        )
    return out
