"""Generated from Smithy shape ``com.amazonaws.mpa#GetIdentitySource``."""
...
import capo_mpa.types.identity_source_parameters_for_get
...
def handle_response(
    ...
    out: capo_mpa.types.get_identity_source_response.GetIdentitySourceResponse = (
        capo_mpa.types.get_identity_source_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identity-sources/{IdentitySourceArn}"
    url = url.replace(
        "{IdentitySourceArn}", quote(input_["identity_source_arn"], safe="")
    )
