"""Generated from Smithy shape ``com.amazonaws.mpa#StartActiveApprovalTeamDeletion``."""
...
import capo_mpa._protocol.eventstream
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_mpa.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_mpa.types.start_active_approval_team_deletion_response.StartActiveApprovalTeamDeletionResponse = capo_mpa.types.start_active_approval_team_deletion_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_mpa._auth._sigv4.build_sigv4_auth_scheme(
                "mpa", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_mpa.types.start_active_approval_team_deletion_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
