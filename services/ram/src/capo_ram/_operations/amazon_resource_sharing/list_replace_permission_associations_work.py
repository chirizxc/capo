"""Generated from Smithy shape ``com.amazonaws.ram#ListReplacePermissionAssociationsWork``."""
...
def handle_response(
    ...
    out: capo_ram.types.list_replace_permission_associations_work_response.ListReplacePermissionAssociationsWorkResponse = capo_ram.types.list_replace_permission_associations_work_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listreplacepermissionassociationswork"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ram.types.list_replace_permission_associations_work_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_replace_permission_associations_work(
