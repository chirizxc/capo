"""Generated from Smithy shape ``com.amazonaws.ram#GetResourceShareAssociations``."""
...
def handle_response(
    ...
    out: capo_ram.types.get_resource_share_associations_response.GetResourceShareAssociationsResponse = capo_ram.types.get_resource_share_associations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/getresourceshareassociations"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ram.types.get_resource_share_associations_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_resource_share_associations(
