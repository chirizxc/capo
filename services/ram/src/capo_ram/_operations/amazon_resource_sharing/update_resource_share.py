"""Generated from Smithy shape ``com.amazonaws.ram#UpdateResourceShare``."""
...
def handle_response(
    ...
    out: capo_ram.types.update_resource_share_response.UpdateResourceShareResponse = (
        capo_ram.types.update_resource_share_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/updateresourceshare"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ram.types.update_resource_share_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_resource_share(
