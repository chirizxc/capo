...
class ResourceSharePermissionDetail(TypedDict, closed=True):
    ...
    permission: NotRequired["capo_ram.types.string.String"]
    ...
    status: NotRequired["capo_ram.types.permission_status.PermissionStatus"]
...
def serialize_json(value: ResourceSharePermissionDetail) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_ram.types.permission_status.serialize_json(value["status"])
    if "tags" in value:
        import capo_ram.types.tag_list
...
def deserialize_json(data: dict) -> ResourceSharePermissionDetail:
    ...
    if data.get("status") is not None:
        ...
        out["status"] = capo_ram.types.permission_status.deserialize_json(
            data["status"]
        )
