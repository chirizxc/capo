"""Generated from Smithy shape ``com.amazonaws.ram#AssociatedPermissionList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_ram.types.associated_permission

AssociatedPermissionList: TypeAlias = list[
    "capo_ram.types.associated_permission.AssociatedPermission"
]


# --- restJson1 ser/de ---
def serialize_json(value: AssociatedPermissionList) -> list:
    import capo_ram.types.associated_permission

    out: list = []
    for item in value:
        out.append(capo_ram.types.associated_permission.serialize_json(item))
    return out


def deserialize_json(data: list) -> AssociatedPermissionList:
    import capo_ram.types.associated_permission

    out: AssociatedPermissionList = []
    for item in data:
        if item is None:
            continue
        out.append(capo_ram.types.associated_permission.deserialize_json(item))
    return out
