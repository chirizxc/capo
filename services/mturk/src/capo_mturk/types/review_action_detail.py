"""Generated from Smithy shape ``com.amazonaws.mturk#ReviewActionDetail``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_mturk.types.entity_id
    import capo_mturk.types.review_action_status
    import capo_mturk.types.string
    import capo_mturk.types.timestamp


class ReviewActionDetail(TypedDict, closed=True):
    action_id: NotRequired["capo_mturk.types.entity_id.EntityId"]
    """<p>The unique identifier for the action.</p>"""
    action_name: NotRequired["capo_mturk.types.string.String"]
    """<p> The nature of the action itself. The Review Policy is responsible for examining the HIT and Assignments, emitting results, and deciding which other actions will be necessary. </p>"""
    target_id: NotRequired["capo_mturk.types.entity_id.EntityId"]
    """<p> The specific HITId or AssignmentID targeted by the action.</p>"""
    target_type: NotRequired["capo_mturk.types.string.String"]
    """<p> The type of object in TargetId.</p>"""
    status: NotRequired["capo_mturk.types.review_action_status.ReviewActionStatus"]
    """<p> The current disposition of the action: INTENDED, SUCCEEDED, FAILED, or CANCELLED. </p>"""
    complete_time: NotRequired["capo_mturk.types.timestamp.Timestamp"]
    """<p> The date when the action was completed.</p>"""
    result: NotRequired["capo_mturk.types.string.String"]
    """<p> A description of the outcome of the review.</p>"""
    error_code: NotRequired["capo_mturk.types.string.String"]
    """<p> Present only when the Results have a FAILED Status.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: ReviewActionDetail) -> dict:
    out: dict = {}
    if "action_id" in value:
        out["ActionId"] = value["action_id"]
    if "action_name" in value:
        out["ActionName"] = value["action_name"]
    if "target_id" in value:
        out["TargetId"] = value["target_id"]
    if "target_type" in value:
        out["TargetType"] = value["target_type"]
    if "status" in value:
        import capo_mturk.types.review_action_status

        out["Status"] = capo_mturk.types.review_action_status.serialize_aws_json_1_1(
            value["status"]
        )
    if "complete_time" in value:
        import capo_mturk.types.timestamp

        out["CompleteTime"] = capo_mturk.types.timestamp.serialize_aws_json_1_1(
            value["complete_time"]
        )
    if "result" in value:
        out["Result"] = value["result"]
    if "error_code" in value:
        out["ErrorCode"] = value["error_code"]
    return out


def deserialize_aws_json_1_1(data: dict) -> ReviewActionDetail:
    out: ReviewActionDetail = {}  # type: ignore[typeddict-item]
    if data.get("ActionId") is not None:
        out["action_id"] = data["ActionId"]
    if data.get("ActionName") is not None:
        out["action_name"] = data["ActionName"]
    if data.get("TargetId") is not None:
        out["target_id"] = data["TargetId"]
    if data.get("TargetType") is not None:
        out["target_type"] = data["TargetType"]
    if data.get("Status") is not None:
        import capo_mturk.types.review_action_status

        out["status"] = capo_mturk.types.review_action_status.deserialize_aws_json_1_1(
            data["Status"]
        )
    if data.get("CompleteTime") is not None:
        import capo_mturk.types.timestamp

        out["complete_time"] = capo_mturk.types.timestamp.deserialize_aws_json_1_1(
            data["CompleteTime"]
        )
    if data.get("Result") is not None:
        out["result"] = data["Result"]
    if data.get("ErrorCode") is not None:
        out["error_code"] = data["ErrorCode"]
    return out
