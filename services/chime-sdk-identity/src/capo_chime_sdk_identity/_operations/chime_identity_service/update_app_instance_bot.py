"""Generated from Smithy shape ``com.amazonaws.chimesdkidentity#UpdateAppInstanceBot``."""
...
def handle_response(
    ...
    out: capo_chime_sdk_identity.types.update_app_instance_bot_response.UpdateAppInstanceBotResponse = capo_chime_sdk_identity.types.update_app_instance_bot_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{AppInstanceBotArn}", quote(input_["app_instance_bot_arn"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime_sdk_identity.types.update_app_instance_bot_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_app_instance_bot(
