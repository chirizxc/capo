"""Generated from Smithy shape ``com.amazonaws.chimesdkidentity#UpdateAppInstanceUserEndpoint``."""
...
import capo_chime_sdk_identity._protocol.eventstream
...
def handle_response(
    ...
    out: capo_chime_sdk_identity.types.update_app_instance_user_endpoint_response.UpdateAppInstanceUserEndpointResponse = capo_chime_sdk_identity.types.update_app_instance_user_endpoint_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_chime_sdk_identity._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = url.replace(
        "{AppInstanceUserArn}", quote(input_["app_instance_user_arn"], safe="")
    )
    url = url.replace("{EndpointId}", quote(input_["endpoint_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime_sdk_identity.types.update_app_instance_user_endpoint_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_app_instance_user_endpoint(
