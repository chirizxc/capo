...
def handle_response(
    ...
    out: capo_s3tables.types.list_table_buckets_response.ListTableBucketsResponse = (
        capo_s3tables.types.list_table_buckets_response.deserialize_json(
...
def build_request(
    ...
    if "continuation_token" in input_:
        params.append(("continuationToken", input_["continuation_token"]))
    if "max_buckets" in input_:
        params.append(("maxBuckets", str(input_["max_buckets"])))
    if "type" in input_:
        params.append(
            (
                "type",
                capo_s3tables.types.table_bucket_type.serialize_json(input_["type"]),
            )
        )
