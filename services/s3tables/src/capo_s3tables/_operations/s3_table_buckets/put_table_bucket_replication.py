"""Generated from Smithy shape ``com.amazonaws.s3tables#PutTableBucketReplication``."""
...
def handle_response(
    ...
    out: capo_s3tables.types.put_table_bucket_replication_response.PutTableBucketReplicationResponse = capo_s3tables.types.put_table_bucket_replication_response.deserialize_json(
...
def build_request(
    ...
    if "table_bucket_arn" in input_:
        params.append(("tableBucketARN", input_["table_bucket_arn"]))
    if "version_token" in input_:
        params.append(("versionToken", input_["version_token"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_s3tables.types.put_table_bucket_replication_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_put_table_bucket_replication(
