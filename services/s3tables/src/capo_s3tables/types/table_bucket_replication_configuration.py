...
if TYPE_CHECKING:
    ...
    import capo_s3tables.types.table_bucket_replication_rules
...
class TableBucketReplicationConfiguration(TypedDict, closed=True):
    ...
    rules: (
        "capo_s3tables.types.table_bucket_replication_rules.TableBucketReplicationRules"
    )
...
def serialize_json(value: TableBucketReplicationConfiguration) -> dict:
    ...
    out["rules"] = capo_s3tables.types.table_bucket_replication_rules.serialize_json(
        value["rules"]
    )
    return out
...
def deserialize_json(data: dict) -> TableBucketReplicationConfiguration:
    ...
    if data.get("rules") is not None:
        ...
        out["rules"] = (
            capo_s3tables.types.table_bucket_replication_rules.deserialize_json(
                data["rules"]
            )
        )
    else:
