"""Generated from Smithy shape ``com.amazonaws.s3tables#GetTableStorageClassResponse``."""
...
class GetTableStorageClassResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetTableStorageClassResponse) -> dict:
    out: dict = {}
    import capo_s3tables.types.storage_class_configuration
...
def deserialize_json(data: dict) -> GetTableStorageClassResponse:
    out: GetTableStorageClassResponse = {}  # type: ignore[typeddict-item]
    if data.get("storageClassConfiguration") is not None:
        ...
        raise DeserializationError(
            "GetTableStorageClassResponse.storage_class_configuration required"
        )
    return out
