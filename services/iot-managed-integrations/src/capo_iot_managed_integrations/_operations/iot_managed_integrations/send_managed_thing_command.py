"""Generated from Smithy shape ``com.amazonaws.iotmanagedintegrations#SendManagedThingCommand``."""
...
def handle_response(
    ...
    out: capo_iot_managed_integrations.types.send_managed_thing_command_response.SendManagedThingCommandResponse = capo_iot_managed_integrations.types.send_managed_thing_command_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/managed-things-command/{ManagedThingId}"
    url = url.replace("{ManagedThingId}", quote(input_["managed_thing_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_managed_integrations.types.send_managed_thing_command_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_send_managed_thing_command(
