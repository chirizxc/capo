"""Generated from Smithy shape ``com.amazonaws.iotmanagedintegrations#SendConnectorEvent``."""
...
def handle_response(
    ...
    out: capo_iot_managed_integrations.types.send_connector_event_response.SendConnectorEventResponse = capo_iot_managed_integrations.types.send_connector_event_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/connector-event/{ConnectorId}"
    url = url.replace("{ConnectorId}", quote(input_["connector_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_managed_integrations.types.send_connector_event_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_send_connector_event(
