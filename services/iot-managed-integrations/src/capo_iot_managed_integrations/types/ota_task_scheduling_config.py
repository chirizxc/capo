...
if TYPE_CHECKING:
    import capo_iot_managed_integrations.types.end_time
    import capo_iot_managed_integrations.types.schedule_maintenance_window_list
    import capo_iot_managed_integrations.types.schedule_start_time
...
class OtaTaskSchedulingConfig(TypedDict, closed=True):
    end_behavior: NotRequired[
        "capo_iot_managed_integrations.types.scheduling_config_end_behavior.SchedulingConfigEndBehavior"
    ]
    ...
    end_time: NotRequired["capo_iot_managed_integrations.types.end_time.EndTime"]
    """<p>The time an over-the-air (OTA) task will stop.</p>"""
    maintenance_windows: NotRequired[
        "capo_iot_managed_integrations.types.schedule_maintenance_window_list.ScheduleMaintenanceWindowList"
    ]
    """<p>Maintenance window list for over-the-air (OTA) task scheduling config.</p>"""
    start_time: NotRequired[
        "capo_iot_managed_integrations.types.schedule_start_time.ScheduleStartTime"
    ]
...
def serialize_json(value: OtaTaskSchedulingConfig) -> dict:
    ...
    if "end_behavior" in value:
        ...
        out["EndBehavior"] = (
            capo_iot_managed_integrations.types.scheduling_config_end_behavior.serialize_json(
                value["end_behavior"]
            )
        )
    ...
    if "maintenance_windows" in value:
        ...
        out["MaintenanceWindows"] = (
            capo_iot_managed_integrations.types.schedule_maintenance_window_list.serialize_json(
...
def deserialize_json(data: dict) -> OtaTaskSchedulingConfig:
    ...
    if data.get("EndBehavior") is not None:
        ...
        out["end_behavior"] = (
            capo_iot_managed_integrations.types.scheduling_config_end_behavior.deserialize_json(
                data["EndBehavior"]
            )
        )
    ...
    if data.get("MaintenanceWindows") is not None:
        ...
        out["maintenance_windows"] = (
            capo_iot_managed_integrations.types.schedule_maintenance_window_list.deserialize_json(
