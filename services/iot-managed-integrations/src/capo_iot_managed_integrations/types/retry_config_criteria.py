...
if TYPE_CHECKING:
    import capo_iot_managed_integrations.types.min_number_of_retries
...
class RetryConfigCriteria(TypedDict, closed=True):
    failure_type: NotRequired[
        "capo_iot_managed_integrations.types.retry_criteria_failure_type.RetryCriteriaFailureType"
    ]
    """<p>Over-the-air (OTA) retry criteria failure type.</p>"""
    min_number_of_retries: NotRequired[
        "capo_iot_managed_integrations.types.min_number_of_retries.MinNumberOfRetries"
    ]
...
def serialize_json(value: RetryConfigCriteria) -> dict:
    ...
    if "failure_type" in value:
        ...
        out["FailureType"] = (
            capo_iot_managed_integrations.types.retry_criteria_failure_type.serialize_json(
                value["failure_type"]
            )
        )
...
def deserialize_json(data: dict) -> RetryConfigCriteria:
    ...
    if data.get("FailureType") is not None:
        ...
        out["failure_type"] = (
            capo_iot_managed_integrations.types.retry_criteria_failure_type.deserialize_json(
                data["FailureType"]
            )
        )
