...
class ListDeviceDiscoveriesResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_iot_managed_integrations.types.device_discovery_list_definition.DeviceDiscoveryListDefinition"
    ]
...
def serialize_json(value: ListDeviceDiscoveriesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = (
            capo_iot_managed_integrations.types.device_discovery_list_definition.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListDeviceDiscoveriesResponse:
    ...
    if data.get("Items") is not None:
        ...
        out["items"] = (
            capo_iot_managed_integrations.types.device_discovery_list_definition.deserialize_json(
                data["Items"]
            )
        )
