...
if TYPE_CHECKING:
    import capo_iot_managed_integrations.types.matter_capability_report_endpoint

MatterCapabilityReportEndpoints: TypeAlias = list[
    "capo_iot_managed_integrations.types.matter_capability_report_endpoint.MatterCapabilityReportEndpoint"
]
...
def serialize_json(value: MatterCapabilityReportEndpoints) -> list:
    ...
    for item in value:
        out.append(
            capo_iot_managed_integrations.types.matter_capability_report_endpoint.serialize_json(
                item
            )
        )
    return out
