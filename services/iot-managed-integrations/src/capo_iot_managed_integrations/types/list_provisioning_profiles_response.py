...
class ListProvisioningProfilesResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_iot_managed_integrations.types.provisioning_profile_list_definition.ProvisioningProfileListDefinition"
    ]
...
def serialize_json(value: ListProvisioningProfilesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = (
            capo_iot_managed_integrations.types.provisioning_profile_list_definition.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListProvisioningProfilesResponse:
    ...
    if data.get("Items") is not None:
        ...
        out["items"] = (
            capo_iot_managed_integrations.types.provisioning_profile_list_definition.deserialize_json(
                data["Items"]
            )
        )
