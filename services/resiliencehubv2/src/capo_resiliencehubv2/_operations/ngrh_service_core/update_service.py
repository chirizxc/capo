"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#UpdateService``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.update_service_response.UpdateServiceResponse = (
        capo_resiliencehubv2.types.update_service_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/update-service"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.update_service_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
