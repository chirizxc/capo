"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#ListInputSources``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.list_input_sources_response.ListInputSourcesResponse = capo_resiliencehubv2.types.list_input_sources_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/list-input-sources"
    ...
    if "type" in input_:
        params.append(
            (
                "type",
                capo_resiliencehubv2.types.input_source_type.serialize_json(
                    input_["type"]
                ),
            )
        )
...
async def async_list_input_sources(
