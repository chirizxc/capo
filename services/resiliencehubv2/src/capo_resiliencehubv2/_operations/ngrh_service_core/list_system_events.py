"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#ListSystemEvents``."""
...
import capo_resiliencehubv2._protocol.eventstream
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.list_system_events_response.ListSystemEventsResponse = capo_resiliencehubv2.types.list_system_events_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_resiliencehubv2._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/list-system-events"
    params: list[tuple[str, str]] = []
    if "system_arn" in input_:
        params.append(("systemArn", input_["system_arn"]))
    if "event_types" in input_:
        for item in input_["event_types"]:
            params.append(
                (
                    "eventTypes",
                    capo_resiliencehubv2.types.system_event_type.serialize_json(item),
                )
            )
...
async def async_list_system_events(
