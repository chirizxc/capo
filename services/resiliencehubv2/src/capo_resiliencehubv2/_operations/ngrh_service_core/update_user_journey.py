"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#UpdateUserJourney``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.update_user_journey_response.UpdateUserJourneyResponse = capo_resiliencehubv2.types.update_user_journey_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/update-user-journey"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.update_user_journey_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_user_journey(
