"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#CreateInputSource``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.create_input_source_response.CreateInputSourceResponse = capo_resiliencehubv2.types.create_input_source_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/create-input-source"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.create_input_source_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_input_source(
