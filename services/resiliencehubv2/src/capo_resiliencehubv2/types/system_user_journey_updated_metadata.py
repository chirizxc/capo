...
class SystemUserJourneyUpdatedMetadata(TypedDict, closed=True):
    ...
    changes: NotRequired[
        "capo_resiliencehubv2.types.user_journey_changes.UserJourneyChanges"
    ]
...
def serialize_json(value: SystemUserJourneyUpdatedMetadata) -> dict:
    ...
    if "changes" in value:
        ...
        out["changes"] = capo_resiliencehubv2.types.user_journey_changes.serialize_json(
            value["changes"]
        )
    return out
...
def deserialize_json(data: dict) -> SystemUserJourneyUpdatedMetadata:
    ...
    if data.get("changes") is not None:
        ...
        out["changes"] = (
            capo_resiliencehubv2.types.user_journey_changes.deserialize_json(
                data["changes"]
            )
        )
    return out
