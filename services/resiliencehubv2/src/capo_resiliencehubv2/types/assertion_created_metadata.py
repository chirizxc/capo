"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#AssertionCreatedMetadata``."""

from typing_extensions import NotRequired, TypedDict


class AssertionCreatedMetadata(TypedDict, closed=True):
    assertion_id: NotRequired["str"]
    """<p>The unique identifier of the created assertion.</p>"""
    assertion_name: NotRequired["str"]
    """<p>The name of the created assertion.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: AssertionCreatedMetadata) -> dict:
    out: dict = {}
    if "assertion_id" in value:
        out["assertionId"] = value["assertion_id"]
    if "assertion_name" in value:
        out["assertionName"] = value["assertion_name"]
    return out


def deserialize_json(data: dict) -> AssertionCreatedMetadata:
    out: AssertionCreatedMetadata = {}  # type: ignore[typeddict-item]
    if data.get("assertionId") is not None:
        out["assertion_id"] = data["assertionId"]
    if data.get("assertionName") is not None:
        out["assertion_name"] = data["assertionName"]
    return out
