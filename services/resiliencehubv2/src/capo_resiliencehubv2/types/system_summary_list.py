...
SystemSummaryList: TypeAlias = list[
    "capo_resiliencehubv2.types.system_summary.SystemSummary"
]
...
def serialize_json(value: SystemSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_resiliencehubv2.types.system_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> SystemSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_resiliencehubv2.types.system_summary.deserialize_json(item))
    return out
