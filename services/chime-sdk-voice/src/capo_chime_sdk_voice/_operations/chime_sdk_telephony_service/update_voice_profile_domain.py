...
def handle_response(
    ...
    out: capo_chime_sdk_voice.types.update_voice_profile_domain_response.UpdateVoiceProfileDomainResponse = capo_chime_sdk_voice.types.update_voice_profile_domain_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{VoiceProfileDomainId}", quote(input_["voice_profile_domain_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime_sdk_voice.types.update_voice_profile_domain_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
