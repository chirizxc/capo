...
def handle_response(
    ...
    out: capo_chime_sdk_voice.types.associate_phone_numbers_with_voice_connector_group_response.AssociatePhoneNumbersWithVoiceConnectorGroupResponse = capo_chime_sdk_voice.types.associate_phone_numbers_with_voice_connector_group_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{VoiceConnectorGroupId}", quote(input_["voice_connector_group_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime_sdk_voice.types.associate_phone_numbers_with_voice_connector_group_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
