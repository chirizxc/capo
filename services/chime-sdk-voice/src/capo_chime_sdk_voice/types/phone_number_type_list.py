...
def serialize_json(value: PhoneNumberTypeList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_chime_sdk_voice.types.phone_number_type.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> PhoneNumberTypeList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_chime_sdk_voice.types.phone_number_type.deserialize_json(item))
    return out
