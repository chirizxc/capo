...
class BatchDeletePhoneNumberRequest(TypedDict, closed=True):
    phone_number_ids: (
        "capo_chime_sdk_voice.types.non_empty_string_list.NonEmptyStringList"
    )
...
def serialize_json(value: BatchDeletePhoneNumberRequest) -> dict:
    ...
    out["PhoneNumberIds"] = (
        capo_chime_sdk_voice.types.non_empty_string_list.serialize_json(
...
def deserialize_json(data: dict) -> BatchDeletePhoneNumberRequest:
    ...
    if data.get("PhoneNumberIds") is not None:
        ...
        out["phone_number_ids"] = (
            capo_chime_sdk_voice.types.non_empty_string_list.deserialize_json(
                data["PhoneNumberIds"]
            )
        )
    else:
