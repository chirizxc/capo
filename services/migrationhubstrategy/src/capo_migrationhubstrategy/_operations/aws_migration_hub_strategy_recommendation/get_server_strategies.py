"""Generated from Smithy shape ``com.amazonaws.migrationhubstrategy#GetServerStrategies``."""
...
def handle_response(
    ...
    out: capo_migrationhubstrategy.types.get_server_strategies_response.GetServerStrategiesResponse = capo_migrationhubstrategy.types.get_server_strategies_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/get-server-strategies/{serverId}"
    url = url.replace("{serverId}", quote(input_["server_id"], safe=""))
...
async def async_get_server_strategies(
