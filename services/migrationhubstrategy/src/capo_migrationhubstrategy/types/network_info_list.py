...
if TYPE_CHECKING:
    import capo_migrationhubstrategy.types.network_info

NetworkInfoList: TypeAlias = list[
    "capo_migrationhubstrategy.types.network_info.NetworkInfo"
]
...
def serialize_json(value: NetworkInfoList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_migrationhubstrategy.types.network_info.serialize_json(item))
    return out
