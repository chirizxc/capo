"""Generated from Smithy shape ``com.amazonaws.migrationhubstrategy#ListImportFileTaskResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_migrationhubstrategy.types.list_import_file_task_information
    import capo_migrationhubstrategy.types.string


class ListImportFileTaskResponse(TypedDict, closed=True):
    task_infos: NotRequired[
        "capo_migrationhubstrategy.types.list_import_file_task_information.ListImportFileTaskInformation"
    ]
    """<p> Lists information about the files you import.</p>"""
    next_token: NotRequired["capo_migrationhubstrategy.types.string.String"]
    """<p> The token you use to retrieve the next set of results, or null if there are no more results. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListImportFileTaskResponse) -> dict:
    out: dict = {}
    if "task_infos" in value:
        import capo_migrationhubstrategy.types.list_import_file_task_information

        out["taskInfos"] = (
            capo_migrationhubstrategy.types.list_import_file_task_information.serialize_json(
                value["task_infos"]
            )
        )
    if "next_token" in value:
        out["nextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListImportFileTaskResponse:
    out: ListImportFileTaskResponse = {}  # type: ignore[typeddict-item]
    if data.get("taskInfos") is not None:
        import capo_migrationhubstrategy.types.list_import_file_task_information

        out["task_infos"] = (
            capo_migrationhubstrategy.types.list_import_file_task_information.deserialize_json(
                data["taskInfos"]
            )
        )
    if data.get("nextToken") is not None:
        out["next_token"] = data["nextToken"]
    return out
