...
class SystemInfo(TypedDict, closed=True):
    os_info: NotRequired["capo_migrationhubstrategy.types.os_info.OSInfo"]
    """<p> Operating system corresponding to a server. </p>"""
    file_system_type: NotRequired["capo_migrationhubstrategy.types.string.String"]
    """<p> File system type for the server. </p>"""
    network_info_list: NotRequired[
        "capo_migrationhubstrategy.types.network_info_list.NetworkInfoList"
    ]
    """<p> Networking information related to a server. </p>"""
    cpu_architecture: NotRequired["capo_migrationhubstrategy.types.string.String"]
...
def serialize_json(value: SystemInfo) -> dict:
    ...
    if "os_info" in value:
        ...
        out["osInfo"] = capo_migrationhubstrategy.types.os_info.serialize_json(
            value["os_info"]
        )
    ...
    if "network_info_list" in value:
        ...
        out["networkInfoList"] = (
            capo_migrationhubstrategy.types.network_info_list.serialize_json(
...
def deserialize_json(data: dict) -> SystemInfo:
    ...
    if data.get("networkInfoList") is not None:
        ...
        out["network_info_list"] = (
            capo_migrationhubstrategy.types.network_info_list.deserialize_json(
