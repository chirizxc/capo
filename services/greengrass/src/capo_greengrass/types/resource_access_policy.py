...
class ResourceAccessPolicy(TypedDict, closed=True):
    permission: NotRequired["capo_greengrass.types.permission.Permission"]
    """The permissions that the Lambda function has to the resource. Can be one of ''rw'' (read/write) or ''ro'' (read-only)."""
    resource_id: NotRequired["capo_greengrass.types.__string.__string"]
...
def serialize_json(value: ResourceAccessPolicy) -> dict:
    ...
    if "permission" in value:
        ...
        out["Permission"] = capo_greengrass.types.permission.serialize_json(
            value["permission"]
        )
