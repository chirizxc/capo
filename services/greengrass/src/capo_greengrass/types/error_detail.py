...
class ErrorDetail(TypedDict, closed=True):
    detailed_error_code: NotRequired["capo_greengrass.types.__string.__string"]
    """A detailed error code."""
    detailed_error_message: NotRequired["capo_greengrass.types.__string.__string"]
...
def deserialize_json(data: dict) -> ErrorDetail:
    ...
    if data.get("DetailedErrorCode") is not None:
        out["detailed_error_code"] = data["DetailedErrorCode"]
    if data.get("DetailedErrorMessage") is not None:
