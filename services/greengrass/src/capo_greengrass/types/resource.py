"""Generated from Smithy shape ``com.amazonaws.greengrass#Resource``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_greengrass.types.__string
    import capo_greengrass.types.resource_data_container


class Resource(TypedDict, closed=True):
    id: NotRequired["capo_greengrass.types.__string.__string"]
    """The resource ID, used to refer to a resource in the Lambda function configuration. Max length is 128 characters with pattern ''[a-zA-Z0-9:_-]+''. This must be unique within a Greengrass group."""
    name: NotRequired["capo_greengrass.types.__string.__string"]
    """The descriptive resource name, which is displayed on the AWS IoT Greengrass console. Max length 128 characters with pattern ''[a-zA-Z0-9:_-]+''. This must be unique within a Greengrass group."""
    resource_data_container: NotRequired[
        "capo_greengrass.types.resource_data_container.ResourceDataContainer"
    ]
    """A container of data for all resource types."""


# --- restJson1 ser/de ---
def serialize_json(value: Resource) -> dict:
    out: dict = {}
    if "id" in value:
        out["Id"] = value["id"]
    if "name" in value:
        out["Name"] = value["name"]
    if "resource_data_container" in value:
        import capo_greengrass.types.resource_data_container

        out["ResourceDataContainer"] = (
            capo_greengrass.types.resource_data_container.serialize_json(
                value["resource_data_container"]
            )
        )
    return out


def deserialize_json(data: dict) -> Resource:
    out: Resource = {}  # type: ignore[typeddict-item]
    if data.get("Id") is not None:
        out["id"] = data["Id"]
    if data.get("Name") is not None:
        out["name"] = data["Name"]
    if data.get("ResourceDataContainer") is not None:
        import capo_greengrass.types.resource_data_container

        out["resource_data_container"] = (
            capo_greengrass.types.resource_data_container.deserialize_json(
                data["ResourceDataContainer"]
            )
        )
    return out
