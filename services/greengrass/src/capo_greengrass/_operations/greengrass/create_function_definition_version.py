"""Generated from Smithy shape ``com.amazonaws.greengrass#CreateFunctionDefinitionVersion``."""
...
import capo_greengrass._protocol.eventstream
...
def handle_response(
    ...
    out: capo_greengrass.types.create_function_definition_version_response.CreateFunctionDefinitionVersionResponse = capo_greengrass.types.create_function_definition_version_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_greengrass._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = url.replace(
        "{FunctionDefinitionId}", quote(input_["function_definition_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "amzn_client_token" in input_:
        headers["X-Amzn-Client-Token"] = input_["amzn_client_token"]
    body: bytes | None = json.dumps(
        capo_greengrass.types.create_function_definition_version_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_function_definition_version(
