"""Generated from Smithy shape ``com.amazonaws.greengrass#CreateDeviceDefinition``."""
...
def handle_response(
    ...
    out: capo_greengrass.types.create_device_definition_response.CreateDeviceDefinitionResponse = capo_greengrass.types.create_device_definition_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/greengrass/definition/devices"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "amzn_client_token" in input_:
        headers["X-Amzn-Client-Token"] = input_["amzn_client_token"]
    body: bytes | None = json.dumps(
        capo_greengrass.types.create_device_definition_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_device_definition(
