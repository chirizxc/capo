"""Generated from Smithy shape ``com.amazonaws.greengrass#UpdateLoggerDefinition``."""
...
def handle_response(
    ...
    out: capo_greengrass.types.update_logger_definition_response.UpdateLoggerDefinitionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{LoggerDefinitionId}", quote(input_["logger_definition_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_greengrass.types.update_logger_definition_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_logger_definition(
