"""Generated from Smithy shape ``com.amazonaws.greengrass#UpdateDeviceDefinition``."""
...
def handle_response(
    ...
    out: capo_greengrass.types.update_device_definition_response.UpdateDeviceDefinitionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{DeviceDefinitionId}", quote(input_["device_definition_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_greengrass.types.update_device_definition_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_device_definition(
