...
class GetDataQualityModelResponse(TypedDict, closed=True):
    status: NotRequired[
        "capo_glue.types.data_quality_model_status.DataQualityModelStatus"
    ]
    ...
    failure_reason: NotRequired["capo_glue.types.hash_string.HashString"]
...
def serialize_aws_json_1_1(value: GetDataQualityModelResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            capo_glue.types.data_quality_model_status.serialize_aws_json_1_1(
                value["status"]
            )
        )
...
def deserialize_aws_json_1_1(data: dict) -> GetDataQualityModelResponse:
    ...
    if data.get("Status") is not None:
        ...
        out["status"] = (
            capo_glue.types.data_quality_model_status.deserialize_aws_json_1_1(
                data["Status"]
            )
        )
