...
if TYPE_CHECKING:
    import capo_glue.types.batch_update_partition_request_entry

BatchUpdatePartitionRequestEntryList: TypeAlias = list[
    "capo_glue.types.batch_update_partition_request_entry.BatchUpdatePartitionRequestEntry"
]
...
def serialize_aws_json_1_1(value: BatchUpdatePartitionRequestEntryList) -> list:
    ...
    for item in value:
        out.append(
            capo_glue.types.batch_update_partition_request_entry.serialize_aws_json_1_1(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> BatchUpdatePartitionRequestEntryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_glue.types.batch_update_partition_request_entry.deserialize_aws_json_1_1(
                item
            )
        )
    return out
