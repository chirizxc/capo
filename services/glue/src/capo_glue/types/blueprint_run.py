...
class BlueprintRun(TypedDict, closed=True):
    ...
    state: NotRequired["capo_glue.types.blueprint_run_state.BlueprintRunState"]
    ...
    error_message: NotRequired["capo_glue.types.message_string.MessageString"]
    """<p>Indicates any errors that are seen while running the blueprint.</p>"""
    rollback_error_message: NotRequired["capo_glue.types.message_string.MessageString"]
    ...
    role_arn: NotRequired[
        "capo_glue.types.orchestration_iam_role_arn.OrchestrationIAMRoleArn"
    ]
...
def serialize_aws_json_1_1(value: BlueprintRun) -> dict:
    ...
    if "state" in value:
        ...
        out["State"] = capo_glue.types.blueprint_run_state.serialize_aws_json_1_1(
            value["state"]
        )
