"""Generated from Smithy shape ``com.amazonaws.glue#CompactionMetrics``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_glue.types.iceberg_compaction_metrics


class CompactionMetrics(TypedDict, closed=True):
    iceberg_metrics: NotRequired[
        "capo_glue.types.iceberg_compaction_metrics.IcebergCompactionMetrics"
    ]
    """<p>A structure containing the Iceberg compaction metrics for the optimizer run.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: CompactionMetrics) -> dict:
    out: dict = {}
    if "iceberg_metrics" in value:
        import capo_glue.types.iceberg_compaction_metrics

        out["IcebergMetrics"] = (
            capo_glue.types.iceberg_compaction_metrics.serialize_aws_json_1_1(
                value["iceberg_metrics"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> CompactionMetrics:
    out: CompactionMetrics = {}  # type: ignore[typeddict-item]
    if data.get("IcebergMetrics") is not None:
        import capo_glue.types.iceberg_compaction_metrics

        out["iceberg_metrics"] = (
            capo_glue.types.iceberg_compaction_metrics.deserialize_aws_json_1_1(
                data["IcebergMetrics"]
            )
        )
    return out
