"""Generated from Smithy shape ``com.amazonaws.glue#QuerySessionContext``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_glue.types.additional_context_map
    import capo_glue.types.hash_string
    import capo_glue.types.nullable_string
    import capo_glue.types.timestamp


class QuerySessionContext(TypedDict, closed=True):
    query_id: NotRequired["capo_glue.types.hash_string.HashString"]
    """<p>A unique identifier generated by the query engine for the query.</p>"""
    query_start_time: NotRequired["capo_glue.types.timestamp.Timestamp"]
    """<p>A timestamp provided by the query engine for when the query started.</p>"""
    cluster_id: NotRequired["capo_glue.types.nullable_string.NullableString"]
    """<p>An identifier string for the consumer cluster.</p>"""
    query_authorization_id: NotRequired["capo_glue.types.hash_string.HashString"]
    """<p>A cryptographically generated query identifier generated by Glue or Lake Formation.</p>"""
    additional_context: NotRequired[
        "capo_glue.types.additional_context_map.AdditionalContextMap"
    ]
    """<p>An opaque string-string map passed by the query engine.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: QuerySessionContext) -> dict:
    out: dict = {}
    if "query_id" in value:
        out["QueryId"] = value["query_id"]
    if "query_start_time" in value:
        import capo_glue.types.timestamp

        out["QueryStartTime"] = capo_glue.types.timestamp.serialize_aws_json_1_1(
            value["query_start_time"]
        )
    if "cluster_id" in value:
        out["ClusterId"] = value["cluster_id"]
    if "query_authorization_id" in value:
        out["QueryAuthorizationId"] = value["query_authorization_id"]
    if "additional_context" in value:
        import capo_glue.types.additional_context_map

        out["AdditionalContext"] = (
            capo_glue.types.additional_context_map.serialize_aws_json_1_1(
                value["additional_context"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> QuerySessionContext:
    out: QuerySessionContext = {}  # type: ignore[typeddict-item]
    if data.get("QueryId") is not None:
        out["query_id"] = data["QueryId"]
    if data.get("QueryStartTime") is not None:
        import capo_glue.types.timestamp

        out["query_start_time"] = capo_glue.types.timestamp.deserialize_aws_json_1_1(
            data["QueryStartTime"]
        )
    if data.get("ClusterId") is not None:
        out["cluster_id"] = data["ClusterId"]
    if data.get("QueryAuthorizationId") is not None:
        out["query_authorization_id"] = data["QueryAuthorizationId"]
    if data.get("AdditionalContext") is not None:
        import capo_glue.types.additional_context_map

        out["additional_context"] = (
            capo_glue.types.additional_context_map.deserialize_aws_json_1_1(
                data["AdditionalContext"]
            )
        )
    return out
