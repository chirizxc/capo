"""Generated from Smithy shape ``com.amazonaws.glue#LimitedPathList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_glue.types.limited_string_list

LimitedPathList: TypeAlias = list[
    "capo_glue.types.limited_string_list.LimitedStringList"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: LimitedPathList) -> list:
    import capo_glue.types.limited_string_list

    out: list = []
    for item in value:
        out.append(capo_glue.types.limited_string_list.serialize_aws_json_1_1(item))
    return out


def deserialize_aws_json_1_1(data: list) -> LimitedPathList:
    import capo_glue.types.limited_string_list

    out: LimitedPathList = []
    for item in data:
        if item is None:
            continue
        out.append(capo_glue.types.limited_string_list.deserialize_aws_json_1_1(item))
    return out
