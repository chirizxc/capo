"""Generated from Smithy shape ``com.amazonaws.glue#IntegrationPartitionSpecList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_glue.types.integration_partition

IntegrationPartitionSpecList: TypeAlias = list[
    "capo_glue.types.integration_partition.IntegrationPartition"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: IntegrationPartitionSpecList) -> list:
    import capo_glue.types.integration_partition

    out: list = []
    for item in value:
        out.append(capo_glue.types.integration_partition.serialize_aws_json_1_1(item))
    return out


def deserialize_aws_json_1_1(data: list) -> IntegrationPartitionSpecList:
    import capo_glue.types.integration_partition

    out: IntegrationPartitionSpecList = []
    for item in data:
        if item is None:
            continue
        out.append(capo_glue.types.integration_partition.deserialize_aws_json_1_1(item))
    return out
