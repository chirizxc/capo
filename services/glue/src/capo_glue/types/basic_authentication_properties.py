...
class BasicAuthenticationProperties(TypedDict, closed=True):
    username: NotRequired["capo_glue.types.connector_property.ConnectorProperty"]
    """<p>The username property name to use for Basic authentication credentials.</p>"""
    password: NotRequired["capo_glue.types.connector_property.ConnectorProperty"]
...
def serialize_aws_json_1_1(value: BasicAuthenticationProperties) -> dict:
    ...
    if "username" in value:
        ...
        out["Username"] = capo_glue.types.connector_property.serialize_aws_json_1_1(
            value["username"]
        )
    if "password" in value:
        import capo_glue.types.connector_property

        out["Password"] = capo_glue.types.connector_property.serialize_aws_json_1_1(
            value["password"]
        )
    return out
