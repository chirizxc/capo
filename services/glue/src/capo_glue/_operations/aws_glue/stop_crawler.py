"""Generated from Smithy shape ``com.amazonaws.glue#StopCrawler``."""
...
import capo_glue.errors.crawler_not_running_exception
import capo_glue.errors.crawler_stopping_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "CrawlerNotRunningException":
            raise capo_glue.errors.crawler_not_running_exception.CrawlerNotRunningException.from_aws_json_1_1(
                data, message
            )
        case "CrawlerStoppingException":
            raise capo_glue.errors.crawler_stopping_exception.CrawlerStoppingException.from_aws_json_1_1(
                data, message
            )
...
def handle_response(
    ...
    out: capo_glue.types.stop_crawler_response.StopCrawlerResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: capo_glue.types.stop_crawler_request.StopCrawlerRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        capo_glue.types.stop_crawler_request.serialize_aws_json_1_1(input_),
        allow_nan=False,
    ).encode()
...
def stop_crawler(
    ...
    input_: capo_glue.types.stop_crawler_request.StopCrawlerRequest,
) -> tuple[capo_glue.types.stop_crawler_response.StopCrawlerResponse, zapros.Response]:
