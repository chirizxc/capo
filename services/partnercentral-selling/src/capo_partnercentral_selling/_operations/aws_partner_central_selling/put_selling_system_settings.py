"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#PutSellingSystemSettings``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.put_selling_system_settings_response.PutSellingSystemSettingsResponse = capo_partnercentral_selling.types.put_selling_system_settings_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/PutSellingSystemSettings"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.PutSellingSystemSettings"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.put_selling_system_settings_request.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_put_selling_system_settings(
