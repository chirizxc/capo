"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#UpdateEngagementContext``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.update_engagement_context_response.UpdateEngagementContextResponse = capo_partnercentral_selling.types.update_engagement_context_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateEngagementContext"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.UpdateEngagementContext"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.update_engagement_context_request.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_engagement_context(
