"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#GetResourceSnapshotJob``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.get_resource_snapshot_job_response.GetResourceSnapshotJobResponse = capo_partnercentral_selling.types.get_resource_snapshot_job_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetResourceSnapshotJob"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.GetResourceSnapshotJob"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.get_resource_snapshot_job_request.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_resource_snapshot_job(
