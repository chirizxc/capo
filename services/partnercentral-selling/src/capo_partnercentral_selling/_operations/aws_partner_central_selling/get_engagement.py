"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#GetEngagement``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.get_engagement_response.GetEngagementResponse = capo_partnercentral_selling.types.get_engagement_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetEngagement"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.GetEngagement"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.get_engagement_request.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_engagement(
