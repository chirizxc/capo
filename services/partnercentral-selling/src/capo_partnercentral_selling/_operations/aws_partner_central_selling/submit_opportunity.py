"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#SubmitOpportunity``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/SubmitOpportunity"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.SubmitOpportunity"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.submit_opportunity_request.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_submit_opportunity(
