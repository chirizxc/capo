"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#StartResourceSnapshotJob``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartResourceSnapshotJob"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.StartResourceSnapshotJob"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.start_resource_snapshot_job_request.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_start_resource_snapshot_job(
