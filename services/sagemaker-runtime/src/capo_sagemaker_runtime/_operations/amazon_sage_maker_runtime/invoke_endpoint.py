...
def handle_response(
    ...
    if "X-Amzn-SageMaker-New-Session-Id" in response.headers:
        out["new_session_id"] = response.headers["X-Amzn-SageMaker-New-Session-Id"]
    if "X-Amzn-SageMaker-Closed-Session-Id" in response.headers:
        out["closed_session_id"] = response.headers[
...
def build_request(
    ...
    if "target_model" in input_:
        headers["X-Amzn-SageMaker-Target-Model"] = input_["target_model"]
    ...
    if "enable_explanations" in input_:
        headers["X-Amzn-SageMaker-Enable-Explanations"] = input_["enable_explanations"]
