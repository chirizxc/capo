...
def handle_response(
    ...
    out: capo_greengrassv2.types.list_core_devices_response.ListCoreDevicesResponse = (
        capo_greengrassv2.types.list_core_devices_response.deserialize_json(
...
def build_request(
    ...
    if "thing_group_arn" in input_:
        params.append(("thingGroupArn", input_["thing_group_arn"]))
    if "status" in input_:
        params.append(
            (
                "status",
                capo_greengrassv2.types.core_device_status.serialize_json(
                    input_["status"]
                ),
            )
        )
    ...
    if "runtime" in input_:
        params.append(("runtime", input_["runtime"]))
