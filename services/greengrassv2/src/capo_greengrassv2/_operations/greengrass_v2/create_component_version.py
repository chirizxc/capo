"""Generated from Smithy shape ``com.amazonaws.greengrassv2#CreateComponentVersion``."""
...
import capo_greengrassv2._protocol.eventstream
...
import capo_greengrassv2.errors.request_already_in_progress_exception
import capo_greengrassv2.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "RequestAlreadyInProgressException":
            raise capo_greengrassv2.errors.request_already_in_progress_exception.RequestAlreadyInProgressException.from_json(
                data, message
            )
        case "ServiceQuotaExceededException":
            raise capo_greengrassv2.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_greengrassv2.types.create_component_version_response.CreateComponentVersionResponse = capo_greengrassv2.types.create_component_version_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_greengrassv2._auth._sigv4.build_sigv4_auth_scheme(
                "greengrass", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/greengrass/v2/createComponentVersion"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_greengrassv2.types.create_component_version_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_component_version(
