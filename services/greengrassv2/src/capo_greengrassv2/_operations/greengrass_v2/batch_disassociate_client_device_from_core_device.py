"""Generated from Smithy shape ``com.amazonaws.greengrassv2#BatchDisassociateClientDeviceFromCoreDevice``."""
...
def handle_response(
    ...
    out: capo_greengrassv2.types.batch_disassociate_client_device_from_core_device_response.BatchDisassociateClientDeviceFromCoreDeviceResponse = capo_greengrassv2.types.batch_disassociate_client_device_from_core_device_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{coreDeviceThingName}", quote(input_["core_device_thing_name"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_greengrassv2.types.batch_disassociate_client_device_from_core_device_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_disassociate_client_device_from_core_device(
