"""Generated from Smithy shape ``com.amazonaws.greengrassv2#ResolveComponentCandidates``."""
...
def handle_response(
    ...
    out: capo_greengrassv2.types.resolve_component_candidates_response.ResolveComponentCandidatesResponse = capo_greengrassv2.types.resolve_component_candidates_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/greengrass/v2/resolveComponentCandidates"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_greengrassv2.types.resolve_component_candidates_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_resolve_component_candidates(
