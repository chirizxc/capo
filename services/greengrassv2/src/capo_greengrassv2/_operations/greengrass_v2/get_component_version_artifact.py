...
def build_request(
    ...
    url = url.replace("{artifactName+}", quote(input_["artifact_name"], safe="/"))
    params: list[tuple[str, str]] = []
    if "s3_endpoint_type" in input_:
        params.append(
            (
                "s3EndpointType",
                capo_greengrassv2.types.s3_endpoint_type.serialize_json(
                    input_["s3_endpoint_type"]
                ),
            )
        )
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "iot_endpoint_type" in input_:
        headers["x-amz-iot-endpoint-type"] = (
            capo_greengrassv2.types.iot_endpoint_type.serialize_json(
                input_["iot_endpoint_type"]
            )
        )
    body: bytes | None = b""
