...
def build_request(
    ...
    if "target_arn" in input_:
        params.append(("targetArn", input_["target_arn"]))
    if "history_filter" in input_:
        params.append(
            (
                "historyFilter",
                capo_greengrassv2.types.deployment_history_filter.serialize_json(
                    input_["history_filter"]
                ),
            )
        )
    if "parent_target_arn" in input_:
        params.append(("parentTargetArn", input_["parent_target_arn"]))
