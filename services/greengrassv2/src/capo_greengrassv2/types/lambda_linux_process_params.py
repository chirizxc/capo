...
class LambdaLinuxProcessParams(TypedDict, closed=True):
    isolation_mode: NotRequired[
        "capo_greengrassv2.types.lambda_isolation_mode.LambdaIsolationMode"
    ]
    ...
    container_params: NotRequired[
        "capo_greengrassv2.types.lambda_container_params.LambdaContainerParams"
    ]
...
def serialize_json(value: LambdaLinuxProcessParams) -> dict:
    ...
    if "isolation_mode" in value:
        ...
        out["isolationMode"] = (
            capo_greengrassv2.types.lambda_isolation_mode.serialize_json(
    ...
    if "container_params" in value:
        ...
        out["containerParams"] = (
            capo_greengrassv2.types.lambda_container_params.serialize_json(
...
def deserialize_json(data: dict) -> LambdaLinuxProcessParams:
    ...
    if data.get("isolationMode") is not None:
        ...
        out["isolation_mode"] = (
            capo_greengrassv2.types.lambda_isolation_mode.deserialize_json(
                data["isolationMode"]
            )
        )
    if data.get("containerParams") is not None:
        ...
        out["container_params"] = (
            capo_greengrassv2.types.lambda_container_params.deserialize_json(
