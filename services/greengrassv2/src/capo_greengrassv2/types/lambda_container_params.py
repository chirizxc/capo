...
class LambdaContainerParams(TypedDict, closed=True):
    ...
    volumes: NotRequired["capo_greengrassv2.types.lambda_volume_list.LambdaVolumeList"]
    """<p>The list of volumes that the container can access.</p>"""
    devices: NotRequired["capo_greengrassv2.types.lambda_device_list.LambdaDeviceList"]
...
def serialize_json(value: LambdaContainerParams) -> dict:
    ...
    if "volumes" in value:
        ...
        out["volumes"] = capo_greengrassv2.types.lambda_volume_list.serialize_json(
            value["volumes"]
        )
    if "devices" in value:
        import capo_greengrassv2.types.lambda_device_list

        out["devices"] = capo_greengrassv2.types.lambda_device_list.serialize_json(
            value["devices"]
        )
    return out
