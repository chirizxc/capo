"""Generated from Smithy shape ``com.amazonaws.glacier#InitiateVaultLock``."""
...
def handle_response(
    ...
    out: capo_glacier.types.initiate_vault_lock_output.InitiateVaultLockOutput = {}  # type: ignore[typeddict-item]
    if "x-amz-lock-id" in response.headers:
        out["lock_id"] = response.headers["x-amz-lock-id"]
    return out
...
def build_request(
    ...
    if "policy" in input_:
        body: bytes | None = json.dumps(
            capo_glacier.types.vault_lock_policy.serialize_json(input_["policy"]),
            allow_nan=False,
        ).encode()
...
async def async_initiate_vault_lock(
