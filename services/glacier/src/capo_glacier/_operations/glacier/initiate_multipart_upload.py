...
def handle_response(
    ...
    out: capo_glacier.types.initiate_multipart_upload_output.InitiateMultipartUploadOutput = {}  # type: ignore[typeddict-item]
    if "Location" in response.headers:
        out["location"] = response.headers["Location"]
    if "x-amz-multipart-upload-id" in response.headers:
        out["upload_id"] = response.headers["x-amz-multipart-upload-id"]
    return out
...
def build_request(
    ...
    if "part_size" in input_:
        headers["x-amz-part-size"] = input_["part_size"]
    body: bytes | None = b""
