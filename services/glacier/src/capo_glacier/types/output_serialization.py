"""Generated from Smithy shape ``com.amazonaws.glacier#OutputSerialization``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_glacier.types.csv_output


class OutputSerialization(TypedDict, closed=True):
    csv: NotRequired["capo_glacier.types.csv_output.CSVOutput"]
    """<p>Describes the serialization of CSV-encoded query results.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: OutputSerialization) -> dict:
    out: dict = {}
    if "csv" in value:
        import capo_glacier.types.csv_output

        out["csv"] = capo_glacier.types.csv_output.serialize_json(value["csv"])
    return out


def deserialize_json(data: dict) -> OutputSerialization:
    out: OutputSerialization = {}  # type: ignore[typeddict-item]
    if data.get("csv") is not None:
        import capo_glacier.types.csv_output

        out["csv"] = capo_glacier.types.csv_output.deserialize_json(data["csv"])
    return out
