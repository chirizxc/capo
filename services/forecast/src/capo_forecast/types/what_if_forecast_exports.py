"""Generated from Smithy shape ``com.amazonaws.forecast#WhatIfForecastExports``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_forecast.types.what_if_forecast_export_summary

WhatIfForecastExports: TypeAlias = list[
    "capo_forecast.types.what_if_forecast_export_summary.WhatIfForecastExportSummary"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: WhatIfForecastExports) -> list:
    import capo_forecast.types.what_if_forecast_export_summary

    out: list = []
    for item in value:
        out.append(
            capo_forecast.types.what_if_forecast_export_summary.serialize_aws_json_1_1(
                item
            )
        )
    return out


def deserialize_aws_json_1_1(data: list) -> WhatIfForecastExports:
    import capo_forecast.types.what_if_forecast_export_summary

    out: WhatIfForecastExports = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_forecast.types.what_if_forecast_export_summary.deserialize_aws_json_1_1(
                item
            )
        )
    return out
