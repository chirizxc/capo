...
class DescribeMonitorResponse(TypedDict, closed=True):
    monitor_name: NotRequired["capo_forecast.types.name.Name"]
    ...
    last_evaluation_time: NotRequired["capo_forecast.types.timestamp.Timestamp"]
    ...
    baseline: NotRequired["capo_forecast.types.baseline.Baseline"]
...
def serialize_aws_json_1_1(value: DescribeMonitorResponse) -> dict:
    ...
    if "baseline" in value:
        ...
        out["Baseline"] = capo_forecast.types.baseline.serialize_aws_json_1_1(
            value["baseline"]
        )
    if "message" in value:
