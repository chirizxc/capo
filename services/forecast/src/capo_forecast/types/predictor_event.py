...
if TYPE_CHECKING:
    import capo_forecast.types.detail
    import capo_forecast.types.timestamp


class PredictorEvent(TypedDict, closed=True):
    detail: NotRequired["capo_forecast.types.detail.Detail"]
    ...
    datetime: NotRequired["capo_forecast.types.timestamp.Timestamp"]
...
def serialize_aws_json_1_1(value: PredictorEvent) -> dict:
    ...
    if "datetime" in value:
        ...
        out["Datetime"] = capo_forecast.types.timestamp.serialize_aws_json_1_1(
            value["datetime"]
        )
    return out
