"""Generated from Smithy shape ``com.amazonaws.forecast#DatasetGroups``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_forecast.types.dataset_group_summary

DatasetGroups: TypeAlias = list[
    "capo_forecast.types.dataset_group_summary.DatasetGroupSummary"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: DatasetGroups) -> list:
    import capo_forecast.types.dataset_group_summary

    out: list = []
    for item in value:
        out.append(
            capo_forecast.types.dataset_group_summary.serialize_aws_json_1_1(item)
        )
    return out


def deserialize_aws_json_1_1(data: list) -> DatasetGroups:
    import capo_forecast.types.dataset_group_summary

    out: DatasetGroups = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_forecast.types.dataset_group_summary.deserialize_aws_json_1_1(item)
        )
    return out
