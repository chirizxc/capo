"""Generated from Smithy shape ``com.amazonaws.storagegateway#UpdateBandwidthRateLimitSchedule``."""
...
import capo_storage_gateway._protocol.eventstream
...
def handle_response(
    ...
    out: capo_storage_gateway.types.update_bandwidth_rate_limit_schedule_output.UpdateBandwidthRateLimitScheduleOutput = capo_storage_gateway.types.update_bandwidth_rate_limit_schedule_output.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_storage_gateway._auth._sigv4.build_sigv4_auth_scheme(
                "storagegateway", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "StorageGateway_20130630.UpdateBandwidthRateLimitSchedule"
    body: bytes | None = json.dumps(
        capo_storage_gateway.types.update_bandwidth_rate_limit_schedule_input.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_bandwidth_rate_limit_schedule(
