"""Generated from Smithy shape ``com.amazonaws.storagegateway#CachediSCSIVolumes``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_storage_gateway.types.cachedi_scsi_volume

CachediSCSIVolumes: TypeAlias = list[
    "capo_storage_gateway.types.cachedi_scsi_volume.CachediSCSIVolume"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: CachediSCSIVolumes) -> list:
    import capo_storage_gateway.types.cachedi_scsi_volume

    out: list = []
    for item in value:
        out.append(
            capo_storage_gateway.types.cachedi_scsi_volume.serialize_aws_json_1_1(item)
        )
    return out


def deserialize_aws_json_1_1(data: list) -> CachediSCSIVolumes:
    import capo_storage_gateway.types.cachedi_scsi_volume

    out: CachediSCSIVolumes = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_storage_gateway.types.cachedi_scsi_volume.deserialize_aws_json_1_1(
                item
            )
        )
    return out
