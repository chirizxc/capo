"""Generated from Smithy shape ``com.amazonaws.quicksight#ListControlDisplayOptions``."""
...
class ListControlDisplayOptions(TypedDict, closed=True):
    search_options: NotRequired[
        "capo_quicksight.types.list_control_search_options.ListControlSearchOptions"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: ListControlDisplayOptions) -> dict:
    out: dict = {}
    if "search_options" in value:
        import capo_quicksight.types.list_control_search_options

        out["SearchOptions"] = (
            capo_quicksight.types.list_control_search_options.serialize_json(
...
def deserialize_json(data: dict) -> ListControlDisplayOptions:
    ...
    if data.get("SearchOptions") is not None:
        ...
        out["search_options"] = (
            capo_quicksight.types.list_control_search_options.deserialize_json(
                data["SearchOptions"]
            )
        )
