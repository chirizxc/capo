...
if TYPE_CHECKING:
    import capo_quicksight.types.cascading_control_source

CascadingControlSourceList: TypeAlias = list[
    "capo_quicksight.types.cascading_control_source.CascadingControlSource"
]
...
def serialize_json(value: CascadingControlSourceList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_quicksight.types.cascading_control_source.serialize_json(item))
    return out
