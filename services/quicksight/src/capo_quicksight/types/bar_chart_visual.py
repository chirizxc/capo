"""Generated from Smithy shape ``com.amazonaws.quicksight#BarChartVisual``."""
...
if TYPE_CHECKING:
    import capo_quicksight.types.bar_chart_configuration
...
class BarChartVisual(TypedDict, closed=True):
    visual_id: (
    ...
    chart_configuration: NotRequired[
        "capo_quicksight.types.bar_chart_configuration.BarChartConfiguration"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: BarChartVisual) -> dict:
    out: dict = {}
    ...
    if "chart_configuration" in value:
        ...
        out["ChartConfiguration"] = (
            capo_quicksight.types.bar_chart_configuration.serialize_json(
...
def deserialize_json(data: dict) -> BarChartVisual:
    out: BarChartVisual = {}  # type: ignore[typeddict-item]
    if data.get("VisualId") is not None:
        out["visual_id"] = data["VisualId"]
    else:
        raise DeserializationError("BarChartVisual.visual_id required")
    ...
    if data.get("ChartConfiguration") is not None:
        ...
        out["chart_configuration"] = (
            capo_quicksight.types.bar_chart_configuration.deserialize_json(
