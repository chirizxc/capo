...
class SheetImageScalingConfiguration(TypedDict, closed=True):
    scaling_type: NotRequired[
        "capo_quicksight.types.sheet_image_scaling_type.SheetImageScalingType"
    ]
...
def serialize_json(value: SheetImageScalingConfiguration) -> dict:
    ...
    if "scaling_type" in value:
        ...
        out["ScalingType"] = (
            capo_quicksight.types.sheet_image_scaling_type.serialize_json(
                value["scaling_type"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> SheetImageScalingConfiguration:
    ...
    if data.get("ScalingType") is not None:
        ...
        out["scaling_type"] = (
            capo_quicksight.types.sheet_image_scaling_type.deserialize_json(
                data["ScalingType"]
            )
        )
    return out
