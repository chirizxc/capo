...
class ActionConnectorError(TypedDict, closed=True):
    ...
    type: NotRequired[
        "capo_quicksight.types.action_connector_error_type.ActionConnectorErrorType"
    ]
...
def serialize_json(value: ActionConnectorError) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = capo_quicksight.types.action_connector_error_type.serialize_json(
            value["type"]
        )
    return out
...
def deserialize_json(data: dict) -> ActionConnectorError:
    ...
    if data.get("Type") is not None:
        ...
        out["type"] = (
            capo_quicksight.types.action_connector_error_type.deserialize_json(
                data["Type"]
            )
        )
    return out
