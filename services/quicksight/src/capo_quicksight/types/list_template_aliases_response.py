"""Generated from Smithy shape ``com.amazonaws.quicksight#ListTemplateAliasesResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_quicksight.types.status_code
    import capo_quicksight.types.string
    import capo_quicksight.types.template_alias_list


class ListTemplateAliasesResponse(TypedDict, closed=True):
    template_alias_list: NotRequired[
        "capo_quicksight.types.template_alias_list.TemplateAliasList"
    ]
    """<p>A structure containing the list of the template's aliases.</p>"""
    status: "capo_quicksight.types.status_code.StatusCode"
    """<p>The HTTP status of the request.</p>"""
    request_id: NotRequired["capo_quicksight.types.string.String"]
    """<p>The Amazon Web Services request ID for this operation.</p>"""
    next_token: NotRequired["capo_quicksight.types.string.String"]
    """<p>The token for the next set of results, or null if there are no more results.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListTemplateAliasesResponse) -> dict:
    out: dict = {}
    if "template_alias_list" in value:
        import capo_quicksight.types.template_alias_list

        out["TemplateAliasList"] = (
            capo_quicksight.types.template_alias_list.serialize_json(
                value["template_alias_list"]
            )
        )
    if "request_id" in value:
        out["RequestId"] = value["request_id"]
    if "next_token" in value:
        out["NextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListTemplateAliasesResponse:
    out: ListTemplateAliasesResponse = {}  # type: ignore[typeddict-item]
    if data.get("TemplateAliasList") is not None:
        import capo_quicksight.types.template_alias_list

        out["template_alias_list"] = (
            capo_quicksight.types.template_alias_list.deserialize_json(
                data["TemplateAliasList"]
            )
        )
    if data.get("RequestId") is not None:
        out["request_id"] = data["RequestId"]
    if data.get("NextToken") is not None:
        out["next_token"] = data["NextToken"]
    return out
