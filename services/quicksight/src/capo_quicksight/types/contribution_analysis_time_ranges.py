...
def serialize_json(value: ContributionAnalysisTimeRanges) -> dict:
    ...
    if "start_range" in value:
        ...
        out["StartRange"] = capo_quicksight.types.topic_ir_filter_option.serialize_json(
            value["start_range"]
        )
    if "end_range" in value:
        import capo_quicksight.types.topic_ir_filter_option

        out["EndRange"] = capo_quicksight.types.topic_ir_filter_option.serialize_json(
            value["end_range"]
        )
    return out
...
def deserialize_json(data: dict) -> ContributionAnalysisTimeRanges:
    ...
    if data.get("StartRange") is not None:
        ...
        out["start_range"] = (
            capo_quicksight.types.topic_ir_filter_option.deserialize_json(
                data["StartRange"]
            )
        )
