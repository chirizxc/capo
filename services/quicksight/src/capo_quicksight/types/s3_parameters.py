...
class S3Parameters(TypedDict, closed=True):
    manifest_file_location: (
        "capo_quicksight.types.manifest_file_location.ManifestFileLocation"
    )
...
def serialize_json(value: S3Parameters) -> dict:
    ...
    out["ManifestFileLocation"] = (
        capo_quicksight.types.manifest_file_location.serialize_json(
...
def deserialize_json(data: dict) -> S3Parameters:
    ...
    if data.get("ManifestFileLocation") is not None:
        ...
        out["manifest_file_location"] = (
            capo_quicksight.types.manifest_file_location.deserialize_json(
