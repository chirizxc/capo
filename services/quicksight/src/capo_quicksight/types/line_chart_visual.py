"""Generated from Smithy shape ``com.amazonaws.quicksight#LineChartVisual``."""
...
if TYPE_CHECKING:
    ...
    import capo_quicksight.types.line_chart_configuration
...
class LineChartVisual(TypedDict, closed=True):
    visual_id: (
    ...
    chart_configuration: NotRequired[
        "capo_quicksight.types.line_chart_configuration.LineChartConfiguration"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: LineChartVisual) -> dict:
    out: dict = {}
    ...
    if "chart_configuration" in value:
        ...
        out["ChartConfiguration"] = (
            capo_quicksight.types.line_chart_configuration.serialize_json(
...
def deserialize_json(data: dict) -> LineChartVisual:
    out: LineChartVisual = {}  # type: ignore[typeddict-item]
    if data.get("VisualId") is not None:
        out["visual_id"] = data["VisualId"]
    else:
        raise DeserializationError("LineChartVisual.visual_id required")
    ...
    if data.get("ChartConfiguration") is not None:
        ...
        out["chart_configuration"] = (
            capo_quicksight.types.line_chart_configuration.deserialize_json(
