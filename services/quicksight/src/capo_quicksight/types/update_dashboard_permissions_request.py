...
class UpdateDashboardPermissionsRequest(TypedDict, closed=True):
    ...
    grant_link_permissions: NotRequired[
        "capo_quicksight.types.update_link_permission_list.UpdateLinkPermissionList"
    ]
...
def serialize_json(value: UpdateDashboardPermissionsRequest) -> dict:
    ...
    if "grant_link_permissions" in value:
        ...
        out["GrantLinkPermissions"] = (
            capo_quicksight.types.update_link_permission_list.serialize_json(
...
def deserialize_json(data: dict) -> UpdateDashboardPermissionsRequest:
    ...
    if data.get("GrantLinkPermissions") is not None:
        import capo_quicksight.types.update_link_permission_list

        out["grant_link_permissions"] = (
            capo_quicksight.types.update_link_permission_list.deserialize_json(
                data["GrantLinkPermissions"]
            )
        )
    if data.get("RevokeLinkPermissions") is not None:
        import capo_quicksight.types.update_link_permission_list
