"""Generated from Smithy shape ``com.amazonaws.quicksight#ComparativeOrder``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_quicksight.types.column_ordering_type
    import capo_quicksight.types.string_list
    import capo_quicksight.types.undefined_specified_value_type


class ComparativeOrder(TypedDict, closed=True):
    use_ordering: NotRequired[
        "capo_quicksight.types.column_ordering_type.ColumnOrderingType"
    ]
    """<p>The ordering type for a column. Valid values for this structure are <code>GREATER_IS_BETTER</code>, <code>LESSER_IS_BETTER</code> and <code>SPECIFIED</code>.</p>"""
    specifed_order: NotRequired["capo_quicksight.types.string_list.StringList"]
    """<p>The list of columns to be used in the ordering.</p>"""
    treat_undefined_specified_values: NotRequired[
        "capo_quicksight.types.undefined_specified_value_type.UndefinedSpecifiedValueType"
    ]
    """<p>The treat of undefined specified values. Valid values for this structure are <code>LEAST</code> and <code>MOST</code>.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ComparativeOrder) -> dict:
    out: dict = {}
    if "use_ordering" in value:
        import capo_quicksight.types.column_ordering_type

        out["UseOrdering"] = capo_quicksight.types.column_ordering_type.serialize_json(
            value["use_ordering"]
        )
    if "specifed_order" in value:
        import capo_quicksight.types.string_list

        out["SpecifedOrder"] = capo_quicksight.types.string_list.serialize_json(
            value["specifed_order"]
        )
    if "treat_undefined_specified_values" in value:
        import capo_quicksight.types.undefined_specified_value_type

        out["TreatUndefinedSpecifiedValues"] = (
            capo_quicksight.types.undefined_specified_value_type.serialize_json(
                value["treat_undefined_specified_values"]
            )
        )
    return out


def deserialize_json(data: dict) -> ComparativeOrder:
    out: ComparativeOrder = {}  # type: ignore[typeddict-item]
    if data.get("UseOrdering") is not None:
        import capo_quicksight.types.column_ordering_type

        out["use_ordering"] = (
            capo_quicksight.types.column_ordering_type.deserialize_json(
                data["UseOrdering"]
            )
        )
    if data.get("SpecifedOrder") is not None:
        import capo_quicksight.types.string_list

        out["specifed_order"] = capo_quicksight.types.string_list.deserialize_json(
            data["SpecifedOrder"]
        )
    if data.get("TreatUndefinedSpecifiedValues") is not None:
        import capo_quicksight.types.undefined_specified_value_type

        out["treat_undefined_specified_values"] = (
            capo_quicksight.types.undefined_specified_value_type.deserialize_json(
                data["TreatUndefinedSpecifiedValues"]
            )
        )
    return out
