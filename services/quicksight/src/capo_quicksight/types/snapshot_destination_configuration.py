...
if TYPE_CHECKING:
    import capo_quicksight.types.snapshot_s3_destination_configuration_list


class SnapshotDestinationConfiguration(TypedDict, closed=True):
    s3_destinations: NotRequired[
        "capo_quicksight.types.snapshot_s3_destination_configuration_list.SnapshotS3DestinationConfigurationList"
    ]
...
def serialize_json(value: SnapshotDestinationConfiguration) -> dict:
    ...
    if "s3_destinations" in value:
        ...
        out["S3Destinations"] = (
            capo_quicksight.types.snapshot_s3_destination_configuration_list.serialize_json(
...
def deserialize_json(data: dict) -> SnapshotDestinationConfiguration:
    ...
    if data.get("S3Destinations") is not None:
        import capo_quicksight.types.snapshot_s3_destination_configuration_list

        out["s3_destinations"] = (
            capo_quicksight.types.snapshot_s3_destination_configuration_list.deserialize_json(
                data["S3Destinations"]
            )
        )
    return out
