...
class SectionAfterPageBreak(TypedDict, closed=True):
    status: NotRequired[
        "capo_quicksight.types.section_page_break_status.SectionPageBreakStatus"
    ]
...
def serialize_json(value: SectionAfterPageBreak) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_quicksight.types.section_page_break_status.serialize_json(
            value["status"]
        )
    return out
...
def deserialize_json(data: dict) -> SectionAfterPageBreak:
    ...
    if data.get("Status") is not None:
        ...
        out["status"] = (
            capo_quicksight.types.section_page_break_status.deserialize_json(
                data["Status"]
            )
        )
    return out
