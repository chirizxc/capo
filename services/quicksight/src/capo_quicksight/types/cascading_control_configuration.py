...
class CascadingControlConfiguration(TypedDict, closed=True):
    source_controls: NotRequired[
        "capo_quicksight.types.cascading_control_source_list.CascadingControlSourceList"
    ]
...
def serialize_json(value: CascadingControlConfiguration) -> dict:
    ...
    if "source_controls" in value:
        ...
        out["SourceControls"] = (
            capo_quicksight.types.cascading_control_source_list.serialize_json(
...
def deserialize_json(data: dict) -> CascadingControlConfiguration:
    ...
    if data.get("SourceControls") is not None:
        ...
        out["source_controls"] = (
            capo_quicksight.types.cascading_control_source_list.deserialize_json(
                data["SourceControls"]
            )
        )
    return out
