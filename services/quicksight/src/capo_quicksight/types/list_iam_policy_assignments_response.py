...
class ListIAMPolicyAssignmentsResponse(TypedDict, closed=True):
    iam_policy_assignments: NotRequired[
        "capo_quicksight.types.iam_policy_assignment_summary_list.IAMPolicyAssignmentSummaryList"
    ]
...
def serialize_json(value: ListIAMPolicyAssignmentsResponse) -> dict:
    ...
    if "iam_policy_assignments" in value:
        ...
        out["IAMPolicyAssignments"] = (
            capo_quicksight.types.iam_policy_assignment_summary_list.serialize_json(
...
def deserialize_json(data: dict) -> ListIAMPolicyAssignmentsResponse:
    ...
    if data.get("IAMPolicyAssignments") is not None:
        import capo_quicksight.types.iam_policy_assignment_summary_list

        out["iam_policy_assignments"] = (
            capo_quicksight.types.iam_policy_assignment_summary_list.deserialize_json(
