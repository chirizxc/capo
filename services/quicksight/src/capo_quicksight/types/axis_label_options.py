"""Generated from Smithy shape ``com.amazonaws.quicksight#AxisLabelOptions``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_quicksight.types.axis_label_reference_options
    import capo_quicksight.types.font_configuration
    import capo_quicksight.types.string


class AxisLabelOptions(TypedDict, closed=True):
    font_configuration: NotRequired[
        "capo_quicksight.types.font_configuration.FontConfiguration"
    ]
    """<p>The font configuration of the axis label.</p>"""
    custom_label: NotRequired["capo_quicksight.types.string.String"]
    """<p>The text for the axis label.</p>"""
    apply_to: NotRequired[
        "capo_quicksight.types.axis_label_reference_options.AxisLabelReferenceOptions"
    ]
    """<p>The options that indicate which field the label belongs to.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: AxisLabelOptions) -> dict:
    out: dict = {}
    if "font_configuration" in value:
        import capo_quicksight.types.font_configuration

        out["FontConfiguration"] = (
            capo_quicksight.types.font_configuration.serialize_json(
                value["font_configuration"]
            )
        )
    if "custom_label" in value:
        out["CustomLabel"] = value["custom_label"]
    if "apply_to" in value:
        import capo_quicksight.types.axis_label_reference_options

        out["ApplyTo"] = (
            capo_quicksight.types.axis_label_reference_options.serialize_json(
                value["apply_to"]
            )
        )
    return out


def deserialize_json(data: dict) -> AxisLabelOptions:
    out: AxisLabelOptions = {}  # type: ignore[typeddict-item]
    if data.get("FontConfiguration") is not None:
        import capo_quicksight.types.font_configuration

        out["font_configuration"] = (
            capo_quicksight.types.font_configuration.deserialize_json(
                data["FontConfiguration"]
            )
        )
    if data.get("CustomLabel") is not None:
        out["custom_label"] = data["CustomLabel"]
    if data.get("ApplyTo") is not None:
        import capo_quicksight.types.axis_label_reference_options

        out["apply_to"] = (
            capo_quicksight.types.axis_label_reference_options.deserialize_json(
                data["ApplyTo"]
            )
        )
    return out
