"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdateRefreshSchedule``."""
...
import capo_quicksight.types.update_refresh_schedule_request
import capo_quicksight.types.update_refresh_schedule_response
...
def handle_response(
    ...
    capo_quicksight.types.update_refresh_schedule_response.UpdateRefreshScheduleResponse
):
    out: capo_quicksight.types.update_refresh_schedule_response.UpdateRefreshScheduleResponse = capo_quicksight.types.update_refresh_schedule_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{DataSetId}", quote(input_["data_set_id"], safe=""))
    url = url.replace("{AwsAccountId}", quote(input_["aws_account_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_refresh_schedule_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_refresh_schedule(
