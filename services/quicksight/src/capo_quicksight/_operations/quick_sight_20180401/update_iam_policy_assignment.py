...
import capo_quicksight.types.update_iam_policy_assignment_request
import capo_quicksight.types.update_iam_policy_assignment_response
...
) -> capo_quicksight.types.update_iam_policy_assignment_response.UpdateIAMPolicyAssignmentResponse:
    out: capo_quicksight.types.update_iam_policy_assignment_response.UpdateIAMPolicyAssignmentResponse = capo_quicksight.types.update_iam_policy_assignment_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{AssignmentName}", quote(input_["assignment_name"], safe=""))
    url = url.replace("{Namespace}", quote(input_["namespace"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_iam_policy_assignment_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
