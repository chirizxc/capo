"""Generated from Smithy shape ``com.amazonaws.quicksight#BatchCreateTopicReviewedAnswer``."""
...
import capo_quicksight.types.batch_create_topic_reviewed_answer_request
import capo_quicksight.types.batch_create_topic_reviewed_answer_response
...
) -> capo_quicksight.types.batch_create_topic_reviewed_answer_response.BatchCreateTopicReviewedAnswerResponse:
    out: capo_quicksight.types.batch_create_topic_reviewed_answer_response.BatchCreateTopicReviewedAnswerResponse = capo_quicksight.types.batch_create_topic_reviewed_answer_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{TopicId}", quote(input_["topic_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.batch_create_topic_reviewed_answer_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_create_topic_reviewed_answer(
