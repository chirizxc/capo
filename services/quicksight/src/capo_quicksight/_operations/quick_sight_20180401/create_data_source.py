"""Generated from Smithy shape ``com.amazonaws.quicksight#CreateDataSource``."""
...
) -> capo_quicksight.types.create_data_source_response.CreateDataSourceResponse:
    out: capo_quicksight.types.create_data_source_response.CreateDataSourceResponse = (
        capo_quicksight.types.create_data_source_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/data-sources"
    url = url.replace("{AwsAccountId}", quote(input_["aws_account_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.create_data_source_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
