"""Generated from Smithy shape ``com.amazonaws.quicksight#CreateIngestion``."""
...
) -> capo_quicksight.types.create_ingestion_response.CreateIngestionResponse:
    out: capo_quicksight.types.create_ingestion_response.CreateIngestionResponse = (
        capo_quicksight.types.create_ingestion_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{IngestionId}", quote(input_["ingestion_id"], safe=""))
    url = url.replace("{AwsAccountId}", quote(input_["aws_account_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.create_ingestion_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
