...
import capo_quicksight.types.update_action_connector_request
import capo_quicksight.types.update_action_connector_response
...
def handle_response(
    ...
    capo_quicksight.types.update_action_connector_response.UpdateActionConnectorResponse
):
    out: capo_quicksight.types.update_action_connector_response.UpdateActionConnectorResponse = capo_quicksight.types.update_action_connector_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{ActionConnectorId}", quote(input_["action_connector_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_action_connector_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
