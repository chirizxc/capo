...
import capo_quicksight.types.update_vpc_connection_request
import capo_quicksight.types.update_vpc_connection_response
...
) -> capo_quicksight.types.update_vpc_connection_response.UpdateVPCConnectionResponse:
    out: capo_quicksight.types.update_vpc_connection_response.UpdateVPCConnectionResponse = capo_quicksight.types.update_vpc_connection_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{VPCConnectionId}", quote(input_["vpc_connection_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_vpc_connection_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
