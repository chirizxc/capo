...
) -> capo_quicksight.types.delete_analysis_response.DeleteAnalysisResponse:
    out: capo_quicksight.types.delete_analysis_response.DeleteAnalysisResponse = (
        capo_quicksight.types.delete_analysis_response.deserialize_json(
...
def build_request(
    ...
    if "recovery_window_in_days" in input_:
        params.append(
            ("recovery-window-in-days", str(input_["recovery_window_in_days"]))
        )
    params.append(
        (
            "force-delete-without-recovery",
            "true" if input_.get("force_delete_without_recovery", False) else "false",
        )
    )
