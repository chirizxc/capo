"""Generated from Smithy shape ``com.amazonaws.chimesdkmediapipelines#CreateMediaStreamPipeline``."""
...
def handle_response(
    ...
    out: capo_chime_sdk_media_pipelines.types.create_media_stream_pipeline_response.CreateMediaStreamPipelineResponse = capo_chime_sdk_media_pipelines.types.create_media_stream_pipeline_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sdk-media-stream-pipelines"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime_sdk_media_pipelines.types.create_media_stream_pipeline_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_media_stream_pipeline(
