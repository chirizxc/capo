...
def serialize_json(value: RealTimeAlertConfiguration) -> dict:
    ...
    if "rules" in value:
        ...
        out["Rules"] = (
            capo_chime_sdk_media_pipelines.types.real_time_alert_rule_list.serialize_json(
                value["rules"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> RealTimeAlertConfiguration:
    ...
    if data.get("Rules") is not None:
        ...
        out["rules"] = (
            capo_chime_sdk_media_pipelines.types.real_time_alert_rule_list.deserialize_json(
                data["Rules"]
            )
        )
    return out
