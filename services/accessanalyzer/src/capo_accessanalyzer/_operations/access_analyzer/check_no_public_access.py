"""Generated from Smithy shape ``com.amazonaws.accessanalyzer#CheckNoPublicAccess``."""
...
def handle_response(
    ...
    out: capo_accessanalyzer.types.check_no_public_access_response.CheckNoPublicAccessResponse = capo_accessanalyzer.types.check_no_public_access_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/policy/check-no-public-access"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_accessanalyzer.types.check_no_public_access_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_check_no_public_access(
