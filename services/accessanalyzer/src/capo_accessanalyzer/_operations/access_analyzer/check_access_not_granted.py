"""Generated from Smithy shape ``com.amazonaws.accessanalyzer#CheckAccessNotGranted``."""
...
import capo_accessanalyzer._protocol.eventstream
...
def handle_response(
    ...
    out: capo_accessanalyzer.types.check_access_not_granted_response.CheckAccessNotGrantedResponse = capo_accessanalyzer.types.check_access_not_granted_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_accessanalyzer._auth._sigv4.build_sigv4_auth_scheme(
                "access-analyzer", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/policy/check-access-not-granted"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_accessanalyzer.types.check_access_not_granted_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_check_access_not_granted(
