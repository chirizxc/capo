"""Generated from Smithy shape ``com.amazonaws.accessanalyzer#ApplyArchiveRule``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/archive-rule"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_accessanalyzer.types.apply_archive_rule_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_apply_archive_rule(
