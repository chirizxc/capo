...
if TYPE_CHECKING:
    import capo_accessanalyzer.types.finding_aggregation_account_details

AccountAggregations: TypeAlias = list[
    "capo_accessanalyzer.types.finding_aggregation_account_details.FindingAggregationAccountDetails"
]
...
def serialize_json(value: AccountAggregations) -> list:
    ...
    for item in value:
        out.append(
            capo_accessanalyzer.types.finding_aggregation_account_details.serialize_json(
                item
            )
        )
    return out
