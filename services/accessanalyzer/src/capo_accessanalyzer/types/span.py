...
class Span(TypedDict, closed=True):
    start: "capo_accessanalyzer.types.position.Position"
    """<p>The start position of the span (inclusive).</p>"""
    end: "capo_accessanalyzer.types.position.Position"
...
def serialize_json(value: Span) -> dict:
    ...
    out["start"] = capo_accessanalyzer.types.position.serialize_json(value["start"])
    import capo_accessanalyzer.types.position

    out["end"] = capo_accessanalyzer.types.position.serialize_json(value["end"])
    return out
