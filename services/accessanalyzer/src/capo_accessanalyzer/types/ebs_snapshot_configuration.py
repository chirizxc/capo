...
if TYPE_CHECKING:
    ...
    import capo_accessanalyzer.types.ebs_snapshot_data_encryption_key_id
    import capo_accessanalyzer.types.ebs_user_id_list


class EbsSnapshotConfiguration(TypedDict, closed=True):
    user_ids: NotRequired["capo_accessanalyzer.types.ebs_user_id_list.EbsUserIdList"]
    ...
    groups: NotRequired["capo_accessanalyzer.types.ebs_group_list.EbsGroupList"]
    ...
    kms_key_id: NotRequired[
        "capo_accessanalyzer.types.ebs_snapshot_data_encryption_key_id.EbsSnapshotDataEncryptionKeyId"
    ]
...
def serialize_json(value: EbsSnapshotConfiguration) -> dict:
    ...
    if "user_ids" in value:
        ...
        out["userIds"] = capo_accessanalyzer.types.ebs_user_id_list.serialize_json(
            value["user_ids"]
        )
    if "groups" in value:
        import capo_accessanalyzer.types.ebs_group_list

        out["groups"] = capo_accessanalyzer.types.ebs_group_list.serialize_json(
            value["groups"]
        )
