...
class FindingAggregationAccountDetails(TypedDict, closed=True):
    ...
    details: NotRequired[
        "capo_accessanalyzer.types.finding_aggregation_account_details_map.FindingAggregationAccountDetailsMap"
    ]
...
def serialize_json(value: FindingAggregationAccountDetails) -> dict:
    ...
    if "details" in value:
        ...
        out["details"] = (
            capo_accessanalyzer.types.finding_aggregation_account_details_map.serialize_json(
                value["details"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> FindingAggregationAccountDetails:
    ...
    if data.get("details") is not None:
        ...
        out["details"] = (
            capo_accessanalyzer.types.finding_aggregation_account_details_map.deserialize_json(
                data["details"]
            )
        )
    return out
