...
if TYPE_CHECKING:
    import capo_accessanalyzer.types.secrets_manager_secret_kms_id
    import capo_accessanalyzer.types.secrets_manager_secret_policy


class SecretsManagerSecretConfiguration(TypedDict, closed=True):
    kms_key_id: NotRequired[
        "capo_accessanalyzer.types.secrets_manager_secret_kms_id.SecretsManagerSecretKmsId"
    ]
    """<p>The proposed ARN, key ID, or alias of the KMS key.</p>"""
    secret_policy: NotRequired[
        "capo_accessanalyzer.types.secrets_manager_secret_policy.SecretsManagerSecretPolicy"
    ]
