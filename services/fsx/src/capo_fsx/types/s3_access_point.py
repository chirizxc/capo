"""Generated from Smithy shape ``com.amazonaws.fsx#S3AccessPoint``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_fsx.types.general_arn
    import capo_fsx.types.s3_access_point_alias
    import capo_fsx.types.s3_access_point_vpc_configuration


class S3AccessPoint(TypedDict, closed=True):
    resource_arn: NotRequired["capo_fsx.types.general_arn.GeneralARN"]
    """<p>he S3 access point's ARN.</p>"""
    alias: NotRequired["capo_fsx.types.s3_access_point_alias.S3AccessPointAlias"]
    """<p>The S3 access point's alias.</p>"""
    vpc_configuration: NotRequired[
        "capo_fsx.types.s3_access_point_vpc_configuration.S3AccessPointVpcConfiguration"
    ]
    """<p>The S3 access point's virtual private cloud (VPC) configuration.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: S3AccessPoint) -> dict:
    out: dict = {}
    if "resource_arn" in value:
        out["ResourceARN"] = value["resource_arn"]
    if "alias" in value:
        out["Alias"] = value["alias"]
    if "vpc_configuration" in value:
        import capo_fsx.types.s3_access_point_vpc_configuration

        out["VpcConfiguration"] = (
            capo_fsx.types.s3_access_point_vpc_configuration.serialize_aws_json_1_1(
                value["vpc_configuration"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> S3AccessPoint:
    out: S3AccessPoint = {}  # type: ignore[typeddict-item]
    if data.get("ResourceARN") is not None:
        out["resource_arn"] = data["ResourceARN"]
    if data.get("Alias") is not None:
        out["alias"] = data["Alias"]
    if data.get("VpcConfiguration") is not None:
        import capo_fsx.types.s3_access_point_vpc_configuration

        out["vpc_configuration"] = (
            capo_fsx.types.s3_access_point_vpc_configuration.deserialize_aws_json_1_1(
                data["VpcConfiguration"]
            )
        )
    return out
