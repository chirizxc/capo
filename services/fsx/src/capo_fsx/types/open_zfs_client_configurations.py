"""Generated from Smithy shape ``com.amazonaws.fsx#OpenZFSClientConfigurations``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_fsx.types.open_zfs_client_configuration

OpenZFSClientConfigurations: TypeAlias = list[
    "capo_fsx.types.open_zfs_client_configuration.OpenZFSClientConfiguration"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: OpenZFSClientConfigurations) -> list:
    import capo_fsx.types.open_zfs_client_configuration

    out: list = []
    for item in value:
        out.append(
            capo_fsx.types.open_zfs_client_configuration.serialize_aws_json_1_1(item)
        )
    return out


def deserialize_aws_json_1_1(data: list) -> OpenZFSClientConfigurations:
    import capo_fsx.types.open_zfs_client_configuration

    out: OpenZFSClientConfigurations = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_fsx.types.open_zfs_client_configuration.deserialize_aws_json_1_1(item)
        )
    return out
