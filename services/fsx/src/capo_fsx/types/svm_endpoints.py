"""Generated from Smithy shape ``com.amazonaws.fsx#SvmEndpoints``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_fsx.types.svm_endpoint


class SvmEndpoints(TypedDict, closed=True):
    iscsi: NotRequired["capo_fsx.types.svm_endpoint.SvmEndpoint"]
    """<p>An endpoint for connecting using the Internet Small Computer Systems Interface (iSCSI) protocol.</p>"""
    management: NotRequired["capo_fsx.types.svm_endpoint.SvmEndpoint"]
    """<p>An endpoint for managing SVMs using the NetApp ONTAP CLI, NetApp ONTAP API, or NetApp CloudManager.</p>"""
    nfs: NotRequired["capo_fsx.types.svm_endpoint.SvmEndpoint"]
    """<p>An endpoint for connecting using the Network File System (NFS) protocol.</p>"""
    smb: NotRequired["capo_fsx.types.svm_endpoint.SvmEndpoint"]
    """<p>An endpoint for connecting using the Server Message Block (SMB) protocol.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: SvmEndpoints) -> dict:
    out: dict = {}
    if "iscsi" in value:
        import capo_fsx.types.svm_endpoint

        out["Iscsi"] = capo_fsx.types.svm_endpoint.serialize_aws_json_1_1(
            value["iscsi"]
        )
    if "management" in value:
        import capo_fsx.types.svm_endpoint

        out["Management"] = capo_fsx.types.svm_endpoint.serialize_aws_json_1_1(
            value["management"]
        )
    if "nfs" in value:
        import capo_fsx.types.svm_endpoint

        out["Nfs"] = capo_fsx.types.svm_endpoint.serialize_aws_json_1_1(value["nfs"])
    if "smb" in value:
        import capo_fsx.types.svm_endpoint

        out["Smb"] = capo_fsx.types.svm_endpoint.serialize_aws_json_1_1(value["smb"])
    return out


def deserialize_aws_json_1_1(data: dict) -> SvmEndpoints:
    out: SvmEndpoints = {}  # type: ignore[typeddict-item]
    if data.get("Iscsi") is not None:
        import capo_fsx.types.svm_endpoint

        out["iscsi"] = capo_fsx.types.svm_endpoint.deserialize_aws_json_1_1(
            data["Iscsi"]
        )
    if data.get("Management") is not None:
        import capo_fsx.types.svm_endpoint

        out["management"] = capo_fsx.types.svm_endpoint.deserialize_aws_json_1_1(
            data["Management"]
        )
    if data.get("Nfs") is not None:
        import capo_fsx.types.svm_endpoint

        out["nfs"] = capo_fsx.types.svm_endpoint.deserialize_aws_json_1_1(data["Nfs"])
    if data.get("Smb") is not None:
        import capo_fsx.types.svm_endpoint

        out["smb"] = capo_fsx.types.svm_endpoint.deserialize_aws_json_1_1(data["Smb"])
    return out
