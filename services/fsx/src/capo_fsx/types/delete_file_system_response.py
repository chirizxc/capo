"""Generated from Smithy shape ``com.amazonaws.fsx#DeleteFileSystemResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_fsx.types.delete_file_system_lustre_response
    import capo_fsx.types.delete_file_system_open_zfs_response
    import capo_fsx.types.delete_file_system_windows_response
    import capo_fsx.types.file_system_id
    import capo_fsx.types.file_system_lifecycle


class DeleteFileSystemResponse(TypedDict, closed=True):
    file_system_id: NotRequired["capo_fsx.types.file_system_id.FileSystemId"]
    """<p>The ID of the file system that's being deleted.</p>"""
    lifecycle: NotRequired["capo_fsx.types.file_system_lifecycle.FileSystemLifecycle"]
    """<p>The file system lifecycle for the deletion request. If the <code>DeleteFileSystem</code> operation is successful, this status is <code>DELETING</code>.</p>"""
    windows_response: NotRequired[
        "capo_fsx.types.delete_file_system_windows_response.DeleteFileSystemWindowsResponse"
    ]
    lustre_response: NotRequired[
        "capo_fsx.types.delete_file_system_lustre_response.DeleteFileSystemLustreResponse"
    ]
    open_zfs_response: NotRequired[
        "capo_fsx.types.delete_file_system_open_zfs_response.DeleteFileSystemOpenZFSResponse"
    ]
    """<p>The response object for the OpenZFS file system that's being deleted in the <code>DeleteFileSystem</code> operation.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: DeleteFileSystemResponse) -> dict:
    out: dict = {}
    if "file_system_id" in value:
        out["FileSystemId"] = value["file_system_id"]
    if "lifecycle" in value:
        import capo_fsx.types.file_system_lifecycle

        out["Lifecycle"] = capo_fsx.types.file_system_lifecycle.serialize_aws_json_1_1(
            value["lifecycle"]
        )
    if "windows_response" in value:
        import capo_fsx.types.delete_file_system_windows_response

        out["WindowsResponse"] = (
            capo_fsx.types.delete_file_system_windows_response.serialize_aws_json_1_1(
                value["windows_response"]
            )
        )
    if "lustre_response" in value:
        import capo_fsx.types.delete_file_system_lustre_response

        out["LustreResponse"] = (
            capo_fsx.types.delete_file_system_lustre_response.serialize_aws_json_1_1(
                value["lustre_response"]
            )
        )
    if "open_zfs_response" in value:
        import capo_fsx.types.delete_file_system_open_zfs_response

        out["OpenZFSResponse"] = (
            capo_fsx.types.delete_file_system_open_zfs_response.serialize_aws_json_1_1(
                value["open_zfs_response"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> DeleteFileSystemResponse:
    out: DeleteFileSystemResponse = {}  # type: ignore[typeddict-item]
    if data.get("FileSystemId") is not None:
        out["file_system_id"] = data["FileSystemId"]
    if data.get("Lifecycle") is not None:
        import capo_fsx.types.file_system_lifecycle

        out["lifecycle"] = (
            capo_fsx.types.file_system_lifecycle.deserialize_aws_json_1_1(
                data["Lifecycle"]
            )
        )
    if data.get("WindowsResponse") is not None:
        import capo_fsx.types.delete_file_system_windows_response

        out["windows_response"] = (
            capo_fsx.types.delete_file_system_windows_response.deserialize_aws_json_1_1(
                data["WindowsResponse"]
            )
        )
    if data.get("LustreResponse") is not None:
        import capo_fsx.types.delete_file_system_lustre_response

        out["lustre_response"] = (
            capo_fsx.types.delete_file_system_lustre_response.deserialize_aws_json_1_1(
                data["LustreResponse"]
            )
        )
    if data.get("OpenZFSResponse") is not None:
        import capo_fsx.types.delete_file_system_open_zfs_response

        out["open_zfs_response"] = (
            capo_fsx.types.delete_file_system_open_zfs_response.deserialize_aws_json_1_1(
                data["OpenZFSResponse"]
            )
        )
    return out
