"""Generated from Smithy shape ``com.amazonaws.qapps#DisassociateLibraryItemReview``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/catalog.disassociateItemRating"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "instance_id" in input_:
        headers["instance-id"] = input_["instance_id"]
    body: bytes | None = json.dumps(
        capo_qapps.types.disassociate_library_item_review_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_disassociate_library_item_review(
