"""Generated from Smithy shape ``com.amazonaws.qapps#UpdateQAppPermissions``."""
...
def handle_response(
    ...
    out: capo_qapps.types.update_q_app_permissions_output.UpdateQAppPermissionsOutput = capo_qapps.types.update_q_app_permissions_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/apps.updateQAppPermissions"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "instance_id" in input_:
        headers["instance-id"] = input_["instance_id"]
    body: bytes | None = json.dumps(
        capo_qapps.types.update_q_app_permissions_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_q_app_permissions(
