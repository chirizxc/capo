"""Generated from Smithy shape ``com.amazonaws.qapps#CreateQApp``."""
...
import capo_qapps._protocol.eventstream
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_qapps.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_qapps.types.create_q_app_output.CreateQAppOutput = (
        capo_qapps.types.create_q_app_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_qapps._auth._sigv4.build_sigv4_auth_scheme(
                "qapps", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/apps.create"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "instance_id" in input_:
        headers["instance-id"] = input_["instance_id"]
    body: bytes | None = json.dumps(
        capo_qapps.types.create_q_app_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def create_q_app(
    ...
    input_: capo_qapps.types.create_q_app_input.CreateQAppInput,
) -> tuple[capo_qapps.types.create_q_app_output.CreateQAppOutput, zapros.Response]:
