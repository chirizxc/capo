...
class SendVoiceMessageRequest(TypedDict, closed=True):
    caller_id: NotRequired["capo_pinpoint_sms_voice.types.string.String"]
...
def serialize_json(value: SendVoiceMessageRequest) -> dict:
    ...
    if "content" in value:
        ...
        out["Content"] = (
            capo_pinpoint_sms_voice.types.voice_message_content.serialize_json(
                value["content"]
            )
        )
...
def deserialize_json(data: dict) -> SendVoiceMessageRequest:
    ...
    if data.get("Content") is not None:
        ...
        out["content"] = (
            capo_pinpoint_sms_voice.types.voice_message_content.deserialize_json(
                data["Content"]
            )
        )
