...
class DescribeTrustedAdvisorCheckResultResponse(TypedDict, closed=True):
    result: NotRequired[
        "capo_support.types.trusted_advisor_check_result.TrustedAdvisorCheckResult"
    ]
...
def serialize_aws_json_1_1(value: DescribeTrustedAdvisorCheckResultResponse) -> dict:
    ...
    if "result" in value:
        ...
        out["result"] = (
            capo_support.types.trusted_advisor_check_result.serialize_aws_json_1_1(
                value["result"]
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> DescribeTrustedAdvisorCheckResultResponse:
    ...
    if data.get("result") is not None:
        ...
        out["result"] = (
            capo_support.types.trusted_advisor_check_result.deserialize_aws_json_1_1(
                data["result"]
            )
        )
    return out
