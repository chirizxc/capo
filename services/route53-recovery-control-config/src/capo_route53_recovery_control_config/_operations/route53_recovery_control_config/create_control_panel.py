"""Generated from Smithy shape ``com.amazonaws.route53recoverycontrolconfig#CreateControlPanel``."""
...
def handle_response(
    ...
    out: capo_route53_recovery_control_config.types.create_control_panel_response.CreateControlPanelResponse = capo_route53_recovery_control_config.types.create_control_panel_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/controlpanel"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_route53_recovery_control_config.types.create_control_panel_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_control_panel(
