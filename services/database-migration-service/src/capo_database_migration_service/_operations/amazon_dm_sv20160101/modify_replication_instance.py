...
import capo_database_migration_service._protocol.eventstream
import capo_database_migration_service.errors.access_denied_fault
import capo_database_migration_service.errors.insufficient_resource_capacity_fault
...
import capo_database_migration_service.errors.storage_quota_exceeded_fault
import capo_database_migration_service.errors.upgrade_dependency_failure_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InsufficientResourceCapacityFault":
            raise capo_database_migration_service.errors.insufficient_resource_capacity_fault.InsufficientResourceCapacityFault.from_aws_json_1_1(
                data, message
            )
        ...
        case "StorageQuotaExceededFault":
            raise capo_database_migration_service.errors.storage_quota_exceeded_fault.StorageQuotaExceededFault.from_aws_json_1_1(
                data, message
            )
        case "UpgradeDependencyFailureFault":
            raise capo_database_migration_service.errors.upgrade_dependency_failure_fault.UpgradeDependencyFailureFault.from_aws_json_1_1(
                data, message
            )
        case _:
...
def handle_response(
    ...
    out: capo_database_migration_service.types.modify_replication_instance_response.ModifyReplicationInstanceResponse = capo_database_migration_service.types.modify_replication_instance_response.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = (
                capo_database_migration_service._auth._sigv4.build_sigv4_auth_scheme(
                    "dms", options.region, endpoint_scheme
                )
            )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_database_migration_service.types.modify_replication_instance_message.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
