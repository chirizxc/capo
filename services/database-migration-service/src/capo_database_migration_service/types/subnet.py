...
if TYPE_CHECKING:
    import capo_database_migration_service.types.availability_zone
...
class Subnet(TypedDict, closed=True):
    ...
    subnet_availability_zone: NotRequired[
        "capo_database_migration_service.types.availability_zone.AvailabilityZone"
    ]
    """<p>The Availability Zone of the subnet.</p>"""
    subnet_status: NotRequired["capo_database_migration_service.types.string.String"]
...
def serialize_aws_json_1_1(value: Subnet) -> dict:
    ...
    if "subnet_availability_zone" in value:
        ...
        out["SubnetAvailabilityZone"] = (
            capo_database_migration_service.types.availability_zone.serialize_aws_json_1_1(
...
def deserialize_aws_json_1_1(data: dict) -> Subnet:
    ...
    if data.get("SubnetIdentifier") is not None:
        out["subnet_identifier"] = data["SubnetIdentifier"]
    if data.get("SubnetAvailabilityZone") is not None:
        import capo_database_migration_service.types.availability_zone

        out["subnet_availability_zone"] = (
            capo_database_migration_service.types.availability_zone.deserialize_aws_json_1_1(
                data["SubnetAvailabilityZone"]
            )
        )
    if data.get("SubnetStatus") is not None:
