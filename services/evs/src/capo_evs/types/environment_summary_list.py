"""Generated from Smithy shape ``com.amazonaws.evs#EnvironmentSummaryList``."""
...
def serialize_aws_json_1_0(value: EnvironmentSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_evs.types.environment_summary.serialize_aws_json_1_0(item))
    return out
...
def deserialize_aws_json_1_0(data: list) -> EnvironmentSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_evs.types.environment_summary.deserialize_aws_json_1_0(item))
    return out
