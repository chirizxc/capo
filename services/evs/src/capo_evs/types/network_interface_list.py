"""Generated from Smithy shape ``com.amazonaws.evs#NetworkInterfaceList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_evs.types.network_interface

NetworkInterfaceList: TypeAlias = list[
    "capo_evs.types.network_interface.NetworkInterface"
]


# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: NetworkInterfaceList) -> list:
    import capo_evs.types.network_interface

    out: list = []
    for item in value:
        out.append(capo_evs.types.network_interface.serialize_aws_json_1_0(item))
    return out


def deserialize_aws_json_1_0(data: list) -> NetworkInterfaceList:
    import capo_evs.types.network_interface

    out: NetworkInterfaceList = []
    for item in data:
        if item is None:
            continue
        out.append(capo_evs.types.network_interface.deserialize_aws_json_1_0(item))
    return out
