...
if TYPE_CHECKING:
    import capo_snow_device_management.types.device_summary_list
    import capo_snow_device_management.types.next_token


class ListDevicesOutput(TypedDict, closed=True):
    devices: NotRequired[
        "capo_snow_device_management.types.device_summary_list.DeviceSummaryList"
    ]
...
def serialize_json(value: ListDevicesOutput) -> dict:
    ...
    if "devices" in value:
        ...
        out["devices"] = (
            capo_snow_device_management.types.device_summary_list.serialize_json(
                value["devices"]
            )
        )
...
def deserialize_json(data: dict) -> ListDevicesOutput:
    ...
    if data.get("devices") is not None:
        ...
        out["devices"] = (
            capo_snow_device_management.types.device_summary_list.deserialize_json(
                data["devices"]
            )
        )
