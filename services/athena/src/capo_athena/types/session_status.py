...
class SessionStatus(TypedDict, closed=True):
    start_date_time: NotRequired["capo_athena.types.date.Date"]
    """<p>The date and time that the session started.</p>"""
    last_modified_date_time: NotRequired["capo_athena.types.date.Date"]
    """<p>The most recent date and time that the session was modified.</p>"""
    end_date_time: NotRequired["capo_athena.types.date.Date"]
    """<p>The date and time that the session ended.</p>"""
    idle_since_date_time: NotRequired["capo_athena.types.date.Date"]
...
def serialize_aws_json_1_1(value: SessionStatus) -> dict:
    ...
    if "start_date_time" in value:
        ...
        out["StartDateTime"] = capo_athena.types.date.serialize_aws_json_1_1(
            value["start_date_time"]
        )
    if "last_modified_date_time" in value:
        import capo_athena.types.date

        out["LastModifiedDateTime"] = capo_athena.types.date.serialize_aws_json_1_1(
            value["last_modified_date_time"]
        )
    if "end_date_time" in value:
        import capo_athena.types.date

        out["EndDateTime"] = capo_athena.types.date.serialize_aws_json_1_1(
            value["end_date_time"]
        )
    if "idle_since_date_time" in value:
        import capo_athena.types.date

        out["IdleSinceDateTime"] = capo_athena.types.date.serialize_aws_json_1_1(
...
def deserialize_aws_json_1_1(data: dict) -> SessionStatus:
    ...
    if data.get("LastModifiedDateTime") is not None:
        ...
        out["last_modified_date_time"] = (
            capo_athena.types.date.deserialize_aws_json_1_1(
