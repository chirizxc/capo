...
if TYPE_CHECKING:
    import capo_inspector.types.telemetry_metadata

TelemetryMetadataList: TypeAlias = list[
    "capo_inspector.types.telemetry_metadata.TelemetryMetadata"
]
...
def serialize_aws_json_1_1(value: TelemetryMetadataList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_inspector.types.telemetry_metadata.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> TelemetryMetadataList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_inspector.types.telemetry_metadata.deserialize_aws_json_1_1(item)
        )
    return out
