...
AgentHealthCodeList: TypeAlias = list[
    "capo_inspector.types.agent_health_code.AgentHealthCode"
]
...
def serialize_aws_json_1_1(value: AgentHealthCodeList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_inspector.types.agent_health_code.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> AgentHealthCodeList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_inspector.types.agent_health_code.deserialize_aws_json_1_1(item)
        )
    return out
