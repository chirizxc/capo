"""Generated from Smithy shape ``com.amazonaws.pi#ListPerformanceAnalysisReports``."""
...
import capo_pi._protocol.eventstream
...
import capo_pi.types.analysis_report_summary_list
...
def handle_response(
    ...
    out: capo_pi.types.list_performance_analysis_reports_response.ListPerformanceAnalysisReportsResponse = capo_pi.types.list_performance_analysis_reports_response.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_pi._auth._sigv4.build_sigv4_auth_scheme(
                "pi", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_pi.types.list_performance_analysis_reports_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_performance_analysis_reports(
