...
class ListAppMonitorsResponse(TypedDict, closed=True):
    next_token: NotRequired["str"]
    """<p>A token that you can use in a subsequent operation to retrieve the next set of results.</p>"""
    app_monitor_summaries: NotRequired[
        "capo_rum.types.app_monitor_summary_list.AppMonitorSummaryList"
    ]
...
def serialize_json(value: ListAppMonitorsResponse) -> dict:
    ...
    if "app_monitor_summaries" in value:
        ...
        out["AppMonitorSummaries"] = (
            capo_rum.types.app_monitor_summary_list.serialize_json(
                value["app_monitor_summaries"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListAppMonitorsResponse:
    ...
    if data.get("AppMonitorSummaries") is not None:
        ...
        out["app_monitor_summaries"] = (
            capo_rum.types.app_monitor_summary_list.deserialize_json(
