"""Generated from Smithy shape ``com.amazonaws.rum#GetAppMonitorData``."""
...
def handle_response(
    ...
    out: capo_rum.types.get_app_monitor_data_response.GetAppMonitorDataResponse = (
        capo_rum.types.get_app_monitor_data_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/appmonitor/{Name}/data"
    url = url.replace("{Name}", quote(input_["name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_rum.types.get_app_monitor_data_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_app_monitor_data(
