"""Generated from Smithy shape ``com.amazonaws.rum#PutRumEvents``."""
...
def handle_response(
    ...
    out: capo_rum.types.put_rum_events_response.PutRumEventsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/appmonitors/{Id}/"
    url = url.replace("{Id}", quote(input_["id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_rum.types.put_rum_events_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def put_rum_events(
    ...
    capo_rum.types.put_rum_events_response.PutRumEventsResponse, zapros.Response
]:
...
async def async_put_rum_events(
