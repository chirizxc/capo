...
if TYPE_CHECKING:
    ...
    import capo_iotdeviceadvisor.types.log_url
    import capo_iotdeviceadvisor.types.status
    import capo_iotdeviceadvisor.types.test_case_definition_name
    ...
    import capo_iotdeviceadvisor.types.warnings


class TestCaseRun(TypedDict, closed=True):
    test_case_run_id: NotRequired["capo_iotdeviceadvisor.types.uuid.UUID"]
    """<p>Provides the test case run ID.</p>"""
    test_case_definition_id: NotRequired["capo_iotdeviceadvisor.types.uuid.UUID"]
    """<p>Provides the test case run definition ID.</p>"""
    test_case_definition_name: NotRequired[
        "capo_iotdeviceadvisor.types.test_case_definition_name.TestCaseDefinitionName"
    ]
    """<p>Provides the test case run definition name.</p>"""
    status: NotRequired["capo_iotdeviceadvisor.types.status.Status"]
    ...
    log_url: NotRequired["capo_iotdeviceadvisor.types.log_url.LogUrl"]
    """<p>Provides test case run log URL.</p>"""
    warnings: NotRequired["capo_iotdeviceadvisor.types.warnings.Warnings"]
    ...
    test_scenarios: NotRequired[
        "capo_iotdeviceadvisor.types.test_case_scenarios_list.TestCaseScenariosList"
    ]
...
def serialize_json(value: TestCaseRun) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_iotdeviceadvisor.types.status.serialize_json(
            value["status"]
        )
    ...
    if "test_scenarios" in value:
        ...
        out["testScenarios"] = (
            capo_iotdeviceadvisor.types.test_case_scenarios_list.serialize_json(
...
def deserialize_json(data: dict) -> TestCaseRun:
    ...
    if data.get("testScenarios") is not None:
        ...
        out["test_scenarios"] = (
            capo_iotdeviceadvisor.types.test_case_scenarios_list.deserialize_json(
                data["testScenarios"]
            )
        )
    return out
