"""Generated from Smithy shape ``com.amazonaws.codeguruprofiler#ListProfileTimes``."""
...
def handle_response(
    ...
    out: capo_codeguruprofiler.types.list_profile_times_response.ListProfileTimesResponse = capo_codeguruprofiler.types.list_profile_times_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/profilingGroups/{profilingGroupName}/profileTimes"
    )
    url = url.replace(
    ...
    if "order_by" in input_:
        params.append(("orderBy", input_["order_by"]))
...
async def async_list_profile_times(
