"""Generated from Smithy shape ``com.amazonaws.codeguruprofiler#SubmitFeedback``."""
...
def handle_response(
    ...
    out: capo_codeguruprofiler.types.submit_feedback_response.SubmitFeedbackResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{anomalyInstanceId}", quote(input_["anomaly_instance_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_codeguruprofiler.types.submit_feedback_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
