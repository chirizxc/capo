"""Generated from Smithy shape ``com.amazonaws.codeguruprofiler#PutPermission``."""
...
def handle_response(
    ...
    out: capo_codeguruprofiler.types.put_permission_response.PutPermissionResponse = (
        capo_codeguruprofiler.types.put_permission_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{actionGroup}", quote(input_["action_group"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_codeguruprofiler.types.put_permission_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
