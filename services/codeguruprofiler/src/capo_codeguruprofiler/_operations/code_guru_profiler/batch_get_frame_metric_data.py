"""Generated from Smithy shape ``com.amazonaws.codeguruprofiler#BatchGetFrameMetricData``."""
...
def handle_response(
    ...
    out: capo_codeguruprofiler.types.batch_get_frame_metric_data_response.BatchGetFrameMetricDataResponse = capo_codeguruprofiler.types.batch_get_frame_metric_data_response.deserialize_json(
...
def build_request(
    ...
    import capo_codeguruprofiler._protocol.serialize
    ...
    url = url.replace(
        "{profilingGroupName}", quote(input_["profiling_group_name"], safe="")
    )
    ...
    if "start_time" in input_:
        params.append(
            (
                "startTime",
                capo_codeguruprofiler._protocol.serialize.fmt_date_time(
                    input_["start_time"]
                ),
            )
        )
    ...
    if "period" in input_:
        params.append(("period", input_["period"]))
    if "target_resolution" in input_:
        params.append(("targetResolution", input_["target_resolution"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_codeguruprofiler.types.batch_get_frame_metric_data_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_get_frame_metric_data(
