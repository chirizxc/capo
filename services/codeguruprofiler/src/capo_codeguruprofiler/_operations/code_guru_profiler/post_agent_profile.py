"""Generated from Smithy shape ``com.amazonaws.codeguruprofiler#PostAgentProfile``."""
...
def handle_response(
    ...
    out: capo_codeguruprofiler.types.post_agent_profile_response.PostAgentProfileResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/profilingGroups/{profilingGroupName}/agentProfile"
    )
    url = url.replace(
        "{profilingGroupName}", quote(input_["profiling_group_name"], safe="")
    )
    params: list[tuple[str, str]] = []
    if "profile_token" in input_:
        params.append(("profileToken", input_["profile_token"]))
...
async def async_post_agent_profile(
