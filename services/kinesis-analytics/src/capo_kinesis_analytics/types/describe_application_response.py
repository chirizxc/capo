"""Generated from Smithy shape ``com.amazonaws.kinesisanalytics#DescribeApplicationResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import TypedDict

from capo_kinesis_analytics.errors import DeserializationError

if TYPE_CHECKING:
    import capo_kinesis_analytics.types.application_detail


class DescribeApplicationResponse(TypedDict, closed=True):
    application_detail: (
        "capo_kinesis_analytics.types.application_detail.ApplicationDetail"
    )
    """<p>Provides a description of the application, such as the application Amazon Resource Name (ARN), status, latest version, and input and output configuration details.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: DescribeApplicationResponse) -> dict:
    out: dict = {}
    import capo_kinesis_analytics.types.application_detail

    out["ApplicationDetail"] = (
        capo_kinesis_analytics.types.application_detail.serialize_aws_json_1_1(
            value["application_detail"]
        )
    )
    return out


def deserialize_aws_json_1_1(data: dict) -> DescribeApplicationResponse:
    out: DescribeApplicationResponse = {}  # type: ignore[typeddict-item]
    if data.get("ApplicationDetail") is not None:
        import capo_kinesis_analytics.types.application_detail

        out["application_detail"] = (
            capo_kinesis_analytics.types.application_detail.deserialize_aws_json_1_1(
                data["ApplicationDetail"]
            )
        )
    else:
        raise DeserializationError(
            "DescribeApplicationResponse.application_detail required"
        )
    return out
