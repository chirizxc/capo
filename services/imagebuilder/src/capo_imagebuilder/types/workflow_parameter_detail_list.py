...
if TYPE_CHECKING:
    import capo_imagebuilder.types.workflow_parameter_detail

WorkflowParameterDetailList: TypeAlias = list[
    "capo_imagebuilder.types.workflow_parameter_detail.WorkflowParameterDetail"
]
...
def serialize_json(value: WorkflowParameterDetailList) -> list:
    ...
    for item in value:
        out.append(
            capo_imagebuilder.types.workflow_parameter_detail.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> WorkflowParameterDetailList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_imagebuilder.types.workflow_parameter_detail.deserialize_json(item)
        )
    return out
