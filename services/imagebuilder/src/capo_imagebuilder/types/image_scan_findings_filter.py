...
if TYPE_CHECKING:
    ...
    import capo_imagebuilder.types.image_scan_findings_filter_values
...
def serialize_json(value: ImageScanFindingsFilter) -> dict:
    ...
    if "values" in value:
        ...
        out["values"] = (
            capo_imagebuilder.types.image_scan_findings_filter_values.serialize_json(
                value["values"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ImageScanFindingsFilter:
    ...
    if data.get("values") is not None:
        ...
        out["values"] = (
            capo_imagebuilder.types.image_scan_findings_filter_values.deserialize_json(
                data["values"]
            )
        )
    return out
