...
ImagePackageList: TypeAlias = list["capo_imagebuilder.types.image_package.ImagePackage"]
...
def serialize_json(value: ImagePackageList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_imagebuilder.types.image_package.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ImagePackageList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_imagebuilder.types.image_package.deserialize_json(item))
    return out
