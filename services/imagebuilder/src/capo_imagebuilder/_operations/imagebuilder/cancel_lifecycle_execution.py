"""Generated from Smithy shape ``com.amazonaws.imagebuilder#CancelLifecycleExecution``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.cancel_lifecycle_execution_response.CancelLifecycleExecutionResponse = capo_imagebuilder.types.cancel_lifecycle_execution_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CancelLifecycleExecution"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.cancel_lifecycle_execution_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_cancel_lifecycle_execution(
