"""Generated from Smithy shape ``com.amazonaws.imagebuilder#ListImagePipelines``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.list_image_pipelines_response.ListImagePipelinesResponse = capo_imagebuilder.types.list_image_pipelines_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListImagePipelines"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.list_image_pipelines_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_image_pipelines(
