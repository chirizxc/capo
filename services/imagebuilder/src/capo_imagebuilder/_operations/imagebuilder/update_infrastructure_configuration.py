"""Generated from Smithy shape ``com.amazonaws.imagebuilder#UpdateInfrastructureConfiguration``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.update_infrastructure_configuration_response.UpdateInfrastructureConfigurationResponse = capo_imagebuilder.types.update_infrastructure_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateInfrastructureConfiguration"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.update_infrastructure_configuration_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_infrastructure_configuration(
