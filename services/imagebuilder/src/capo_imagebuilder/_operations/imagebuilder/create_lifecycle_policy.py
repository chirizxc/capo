"""Generated from Smithy shape ``com.amazonaws.imagebuilder#CreateLifecyclePolicy``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.create_lifecycle_policy_response.CreateLifecyclePolicyResponse = capo_imagebuilder.types.create_lifecycle_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateLifecyclePolicy"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.create_lifecycle_policy_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
