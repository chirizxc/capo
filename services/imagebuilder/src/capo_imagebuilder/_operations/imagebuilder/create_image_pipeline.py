"""Generated from Smithy shape ``com.amazonaws.imagebuilder#CreateImagePipeline``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.create_image_pipeline_response.CreateImagePipelineResponse = capo_imagebuilder.types.create_image_pipeline_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateImagePipeline"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.create_image_pipeline_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_image_pipeline(
