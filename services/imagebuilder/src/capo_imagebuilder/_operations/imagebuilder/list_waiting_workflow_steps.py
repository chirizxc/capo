"""Generated from Smithy shape ``com.amazonaws.imagebuilder#ListWaitingWorkflowSteps``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.list_waiting_workflow_steps_response.ListWaitingWorkflowStepsResponse = capo_imagebuilder.types.list_waiting_workflow_steps_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListWaitingWorkflowSteps"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.list_waiting_workflow_steps_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_waiting_workflow_steps(
