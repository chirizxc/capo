"""Generated from Smithy shape ``com.amazonaws.imagebuilder#CreateImage``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.create_image_response.CreateImageResponse = (
        capo_imagebuilder.types.create_image_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateImage"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.create_image_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def create_image(
    ...
    capo_imagebuilder.types.create_image_response.CreateImageResponse, zapros.Response
]:
