"""Generated from Smithy shape ``com.amazonaws.imagebuilder#StartResourceStateUpdate``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.start_resource_state_update_response.StartResourceStateUpdateResponse = capo_imagebuilder.types.start_resource_state_update_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartResourceStateUpdate"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.start_resource_state_update_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_start_resource_state_update(
