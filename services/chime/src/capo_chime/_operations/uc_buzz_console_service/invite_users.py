"""Generated from Smithy shape ``com.amazonaws.chime#InviteUsers``."""
...
def handle_response(
    ...
    out: capo_chime.types.invite_users_response.InviteUsersResponse = (
        capo_chime.types.invite_users_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AccountId}/users?operation=add"
    url = url.replace("{AccountId}", quote(input_["account_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime.types.invite_users_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def invite_users(
    ...
    input_: capo_chime.types.invite_users_request.InviteUsersRequest,
) -> tuple[capo_chime.types.invite_users_response.InviteUsersResponse, zapros.Response]:
