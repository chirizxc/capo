...
if TYPE_CHECKING:
    import capo_chime.types.phone_number_error

PhoneNumberErrorList: TypeAlias = list[
    "capo_chime.types.phone_number_error.PhoneNumberError"
]
...
def serialize_json(value: PhoneNumberErrorList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_chime.types.phone_number_error.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> PhoneNumberErrorList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_chime.types.phone_number_error.deserialize_json(item))
    return out
