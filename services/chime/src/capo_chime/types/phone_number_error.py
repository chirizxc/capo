...
class PhoneNumberError(TypedDict, closed=True):
    phone_number_id: NotRequired["capo_chime.types.non_empty_string.NonEmptyString"]
    """<p>The phone number ID for which the action failed.</p>"""
    error_code: NotRequired["capo_chime.types.error_code.ErrorCode"]
    """<p>The error code.</p>"""
    error_message: NotRequired["capo_chime.types.string.String"]
...
def serialize_json(value: PhoneNumberError) -> dict:
    ...
    if "error_code" in value:
        ...
        out["ErrorCode"] = capo_chime.types.error_code.serialize_json(
            value["error_code"]
        )
