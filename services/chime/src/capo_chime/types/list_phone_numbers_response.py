...
class ListPhoneNumbersResponse(TypedDict, closed=True):
    phone_numbers: NotRequired["capo_chime.types.phone_number_list.PhoneNumberList"]
...
def serialize_json(value: ListPhoneNumbersResponse) -> dict:
    ...
    if "phone_numbers" in value:
        ...
        out["PhoneNumbers"] = capo_chime.types.phone_number_list.serialize_json(
            value["phone_numbers"]
        )
...
def deserialize_json(data: dict) -> ListPhoneNumbersResponse:
    ...
    if data.get("PhoneNumbers") is not None:
        ...
        out["phone_numbers"] = capo_chime.types.phone_number_list.deserialize_json(
            data["PhoneNumbers"]
        )
