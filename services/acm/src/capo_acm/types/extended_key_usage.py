...
class ExtendedKeyUsage(TypedDict, closed=True):
    name: NotRequired["capo_acm.types.extended_key_usage_name.ExtendedKeyUsageName"]
    """<p>The name of an Extended Key Usage value.</p>"""
    oid: NotRequired["capo_acm.types.string.String"]
...
def serialize_aws_json_1_1(value: ExtendedKeyUsage) -> dict:
    ...
    if "name" in value:
        ...
        out["Name"] = capo_acm.types.extended_key_usage_name.serialize_aws_json_1_1(
            value["name"]
        )
    if "oid" in value:
        out["OID"] = value["oid"]
