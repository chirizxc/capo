"""Generated from Smithy shape ``com.amazonaws.iotevents#ListDetectorModelVersions``."""
...
import capo_iot_events._protocol.eventstream
...
import capo_iot_events.types.detector_model_version_summaries
...
def handle_response(
    ...
    out: capo_iot_events.types.list_detector_model_versions_response.ListDetectorModelVersionsResponse = capo_iot_events.types.list_detector_model_versions_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_iot_events._auth._sigv4.build_sigv4_auth_scheme(
                "iotevents", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector-models/{detectorModelName}/versions"
    url = url.replace(
        "{detectorModelName}", quote(input_["detector_model_name"], safe="")
    )
...
async def async_list_detector_model_versions(
