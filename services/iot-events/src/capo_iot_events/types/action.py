"""Generated from Smithy shape ``com.amazonaws.iotevents#Action``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_iot_events.types.clear_timer_action
    import capo_iot_events.types.dynamo_d_bv2_action
    import capo_iot_events.types.dynamo_db_action
    import capo_iot_events.types.firehose_action
    import capo_iot_events.types.iot_events_action
    import capo_iot_events.types.iot_site_wise_action
    import capo_iot_events.types.iot_topic_publish_action
    import capo_iot_events.types.lambda_action
    import capo_iot_events.types.reset_timer_action
    import capo_iot_events.types.set_timer_action
    import capo_iot_events.types.set_variable_action
    import capo_iot_events.types.sns_topic_publish_action
    import capo_iot_events.types.sqs_action

Action = TypedDict(
    "Action",
    {
        "set_variable": NotRequired[
            "capo_iot_events.types.set_variable_action.SetVariableAction"
        ],
        "sns": NotRequired[
            "capo_iot_events.types.sns_topic_publish_action.SNSTopicPublishAction"
        ],
        "iot_topic_publish": NotRequired[
            "capo_iot_events.types.iot_topic_publish_action.IotTopicPublishAction"
        ],
        "set_timer": NotRequired[
            "capo_iot_events.types.set_timer_action.SetTimerAction"
        ],
        "clear_timer": NotRequired[
            "capo_iot_events.types.clear_timer_action.ClearTimerAction"
        ],
        "reset_timer": NotRequired[
            "capo_iot_events.types.reset_timer_action.ResetTimerAction"
        ],
        "lambda": NotRequired["capo_iot_events.types.lambda_action.LambdaAction"],
        "iot_events": NotRequired[
            "capo_iot_events.types.iot_events_action.IotEventsAction"
        ],
        "sqs": NotRequired["capo_iot_events.types.sqs_action.SqsAction"],
        "firehose": NotRequired["capo_iot_events.types.firehose_action.FirehoseAction"],
        "dynamo_db": NotRequired[
            "capo_iot_events.types.dynamo_db_action.DynamoDBAction"
        ],
        "dynamo_d_bv2": NotRequired[
            "capo_iot_events.types.dynamo_d_bv2_action.DynamoDBv2Action"
        ],
        "iot_site_wise": NotRequired[
            "capo_iot_events.types.iot_site_wise_action.IotSiteWiseAction"
        ],
    },
    closed=True,
)


# --- restJson1 ser/de ---
def serialize_json(value: Action) -> dict:
    out: dict = {}
    if "set_variable" in value:
        import capo_iot_events.types.set_variable_action

        out["setVariable"] = capo_iot_events.types.set_variable_action.serialize_json(
            value["set_variable"]
        )
    if "sns" in value:
        import capo_iot_events.types.sns_topic_publish_action

        out["sns"] = capo_iot_events.types.sns_topic_publish_action.serialize_json(
            value["sns"]
        )
    if "iot_topic_publish" in value:
        import capo_iot_events.types.iot_topic_publish_action

        out["iotTopicPublish"] = (
            capo_iot_events.types.iot_topic_publish_action.serialize_json(
                value["iot_topic_publish"]
            )
        )
    if "set_timer" in value:
        import capo_iot_events.types.set_timer_action

        out["setTimer"] = capo_iot_events.types.set_timer_action.serialize_json(
            value["set_timer"]
        )
    if "clear_timer" in value:
        import capo_iot_events.types.clear_timer_action

        out["clearTimer"] = capo_iot_events.types.clear_timer_action.serialize_json(
            value["clear_timer"]
        )
    if "reset_timer" in value:
        import capo_iot_events.types.reset_timer_action

        out["resetTimer"] = capo_iot_events.types.reset_timer_action.serialize_json(
            value["reset_timer"]
        )
    if "lambda" in value:
        import capo_iot_events.types.lambda_action

        out["lambda"] = capo_iot_events.types.lambda_action.serialize_json(
            value["lambda"]
        )
    if "iot_events" in value:
        import capo_iot_events.types.iot_events_action

        out["iotEvents"] = capo_iot_events.types.iot_events_action.serialize_json(
            value["iot_events"]
        )
    if "sqs" in value:
        import capo_iot_events.types.sqs_action

        out["sqs"] = capo_iot_events.types.sqs_action.serialize_json(value["sqs"])
    if "firehose" in value:
        import capo_iot_events.types.firehose_action

        out["firehose"] = capo_iot_events.types.firehose_action.serialize_json(
            value["firehose"]
        )
    if "dynamo_db" in value:
        import capo_iot_events.types.dynamo_db_action

        out["dynamoDB"] = capo_iot_events.types.dynamo_db_action.serialize_json(
            value["dynamo_db"]
        )
    if "dynamo_d_bv2" in value:
        import capo_iot_events.types.dynamo_d_bv2_action

        out["dynamoDBv2"] = capo_iot_events.types.dynamo_d_bv2_action.serialize_json(
            value["dynamo_d_bv2"]
        )
    if "iot_site_wise" in value:
        import capo_iot_events.types.iot_site_wise_action

        out["iotSiteWise"] = capo_iot_events.types.iot_site_wise_action.serialize_json(
            value["iot_site_wise"]
        )
    return out


def deserialize_json(data: dict) -> Action:
    out: Action = {}  # type: ignore[typeddict-item]
    if data.get("setVariable") is not None:
        import capo_iot_events.types.set_variable_action

        out["set_variable"] = (
            capo_iot_events.types.set_variable_action.deserialize_json(
                data["setVariable"]
            )
        )
    if data.get("sns") is not None:
        import capo_iot_events.types.sns_topic_publish_action

        out["sns"] = capo_iot_events.types.sns_topic_publish_action.deserialize_json(
            data["sns"]
        )
    if data.get("iotTopicPublish") is not None:
        import capo_iot_events.types.iot_topic_publish_action

        out["iot_topic_publish"] = (
            capo_iot_events.types.iot_topic_publish_action.deserialize_json(
                data["iotTopicPublish"]
            )
        )
    if data.get("setTimer") is not None:
        import capo_iot_events.types.set_timer_action

        out["set_timer"] = capo_iot_events.types.set_timer_action.deserialize_json(
            data["setTimer"]
        )
    if data.get("clearTimer") is not None:
        import capo_iot_events.types.clear_timer_action

        out["clear_timer"] = capo_iot_events.types.clear_timer_action.deserialize_json(
            data["clearTimer"]
        )
    if data.get("resetTimer") is not None:
        import capo_iot_events.types.reset_timer_action

        out["reset_timer"] = capo_iot_events.types.reset_timer_action.deserialize_json(
            data["resetTimer"]
        )
    if data.get("lambda") is not None:
        import capo_iot_events.types.lambda_action

        out["lambda"] = capo_iot_events.types.lambda_action.deserialize_json(
            data["lambda"]
        )
    if data.get("iotEvents") is not None:
        import capo_iot_events.types.iot_events_action

        out["iot_events"] = capo_iot_events.types.iot_events_action.deserialize_json(
            data["iotEvents"]
        )
    if data.get("sqs") is not None:
        import capo_iot_events.types.sqs_action

        out["sqs"] = capo_iot_events.types.sqs_action.deserialize_json(data["sqs"])
    if data.get("firehose") is not None:
        import capo_iot_events.types.firehose_action

        out["firehose"] = capo_iot_events.types.firehose_action.deserialize_json(
            data["firehose"]
        )
    if data.get("dynamoDB") is not None:
        import capo_iot_events.types.dynamo_db_action

        out["dynamo_db"] = capo_iot_events.types.dynamo_db_action.deserialize_json(
            data["dynamoDB"]
        )
    if data.get("dynamoDBv2") is not None:
        import capo_iot_events.types.dynamo_d_bv2_action

        out["dynamo_d_bv2"] = (
            capo_iot_events.types.dynamo_d_bv2_action.deserialize_json(
                data["dynamoDBv2"]
            )
        )
    if data.get("iotSiteWise") is not None:
        import capo_iot_events.types.iot_site_wise_action

        out["iot_site_wise"] = (
            capo_iot_events.types.iot_site_wise_action.deserialize_json(
                data["iotSiteWise"]
            )
        )
    return out
