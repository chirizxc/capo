...
class SubCheckResult(TypedDict, closed=True):
    id: NotRequired["capo_ssm_sap.types.sub_check_result_id.SubCheckResultId"]
    ...
    references: NotRequired[
        "capo_ssm_sap.types.sub_check_references_list.SubCheckReferencesList"
    ]
...
def serialize_json(value: SubCheckResult) -> dict:
    ...
    if "references" in value:
        ...
        out["References"] = capo_ssm_sap.types.sub_check_references_list.serialize_json(
            value["references"]
        )
    return out
...
def deserialize_json(data: dict) -> SubCheckResult:
    ...
    if data.get("References") is not None:
        ...
        out["references"] = (
            capo_ssm_sap.types.sub_check_references_list.deserialize_json(
                data["References"]
            )
        )
    return out
