...
ApplicationTypeList: TypeAlias = list[
    "capo_ssm_sap.types.application_type.ApplicationType"
]
...
def serialize_json(value: ApplicationTypeList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_ssm_sap.types.application_type.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ApplicationTypeList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_ssm_sap.types.application_type.deserialize_json(item))
    return out
