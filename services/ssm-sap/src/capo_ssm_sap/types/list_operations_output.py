...
class ListOperationsOutput(TypedDict, closed=True):
    operations: NotRequired["capo_ssm_sap.types.operation_list.OperationList"]
...
def serialize_json(value: ListOperationsOutput) -> dict:
    ...
    if "operations" in value:
        ...
        out["Operations"] = capo_ssm_sap.types.operation_list.serialize_json(
            value["operations"]
        )
...
def deserialize_json(data: dict) -> ListOperationsOutput:
    ...
    if data.get("Operations") is not None:
        ...
        out["operations"] = capo_ssm_sap.types.operation_list.deserialize_json(
            data["Operations"]
        )
