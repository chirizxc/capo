...
class ListApplicationsOutput(TypedDict, closed=True):
    applications: NotRequired[
        "capo_ssm_sap.types.application_summary_list.ApplicationSummaryList"
    ]
...
def serialize_json(value: ListApplicationsOutput) -> dict:
    ...
    if "applications" in value:
        ...
        out["Applications"] = (
            capo_ssm_sap.types.application_summary_list.serialize_json(
                value["applications"]
            )
        )
...
def deserialize_json(data: dict) -> ListApplicationsOutput:
    ...
    if data.get("Applications") is not None:
        ...
        out["applications"] = (
            capo_ssm_sap.types.application_summary_list.deserialize_json(
                data["Applications"]
            )
        )
