...
class GetInvestigationResponse(TypedDict, closed=True):
    ...
    investigation_id: NotRequired[
        "capo_detective.types.investigation_id.InvestigationId"
    ]
    """<p>The investigation ID of the investigation report.</p>"""
    entity_arn: NotRequired["capo_detective.types.entity_arn.EntityArn"]
    """<p>The unique Amazon Resource Name (ARN). Detective supports IAM user ARNs and IAM role ARNs.</p>"""
    entity_type: NotRequired["capo_detective.types.entity_type.EntityType"]
    ...
    scope_start_time: NotRequired["capo_detective.types.timestamp.Timestamp"]
    ...
    scope_end_time: NotRequired["capo_detective.types.timestamp.Timestamp"]
    ...
    status: NotRequired["capo_detective.types.status.Status"]
    """<p>The status based on the completion status of the investigation.</p>"""
    severity: NotRequired["capo_detective.types.severity.Severity"]
    ...
    state: NotRequired["capo_detective.types.state.State"]
...
def serialize_json(value: GetInvestigationResponse) -> dict:
    ...
    if "entity_type" in value:
        ...
        out["EntityType"] = capo_detective.types.entity_type.serialize_json(
            value["entity_type"]
        )
    ...
    if "status" in value:
        ...
        out["Status"] = capo_detective.types.status.serialize_json(value["status"])
    if "severity" in value:
        import capo_detective.types.severity

        out["Severity"] = capo_detective.types.severity.serialize_json(
            value["severity"]
        )
    if "state" in value:
        import capo_detective.types.state
