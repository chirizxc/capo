"""Generated from Smithy shape ``com.amazonaws.detective#DisassociateMembership``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/membership/removal"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_detective.types.disassociate_membership_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_disassociate_membership(
