"""Generated from Smithy shape ``com.amazonaws.detective#ListIndicators``."""
...
def handle_response(
    ...
    out: capo_detective.types.list_indicators_response.ListIndicatorsResponse = (
        capo_detective.types.list_indicators_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/investigations/listIndicators"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_detective.types.list_indicators_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_indicators(
