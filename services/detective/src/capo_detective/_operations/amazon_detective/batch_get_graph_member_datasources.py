"""Generated from Smithy shape ``com.amazonaws.detective#BatchGetGraphMemberDatasources``."""
...
def handle_response(
    ...
    out: capo_detective.types.batch_get_graph_member_datasources_response.BatchGetGraphMemberDatasourcesResponse = capo_detective.types.batch_get_graph_member_datasources_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/graph/datasources/get"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_detective.types.batch_get_graph_member_datasources_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_batch_get_graph_member_datasources(
