"""Generated from Smithy shape ``com.amazonaws.detective#UpdateOrganizationConfiguration``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/orgs/updateOrganizationConfiguration"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_detective.types.update_organization_configuration_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
