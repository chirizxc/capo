"""Generated from Smithy shape ``com.amazonaws.detective#StartInvestigation``."""
...
def handle_response(
    ...
    out: capo_detective.types.start_investigation_response.StartInvestigationResponse = capo_detective.types.start_investigation_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/investigations/startInvestigation"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_detective.types.start_investigation_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
