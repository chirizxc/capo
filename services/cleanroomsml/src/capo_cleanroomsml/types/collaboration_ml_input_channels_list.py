...
CollaborationMLInputChannelsList: TypeAlias = list[
    "capo_cleanroomsml.types.collaboration_ml_input_channel_summary.CollaborationMLInputChannelSummary"
]
...
def serialize_json(value: CollaborationMLInputChannelsList) -> list:
    ...
    for item in value:
        out.append(
            capo_cleanroomsml.types.collaboration_ml_input_channel_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> CollaborationMLInputChannelsList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_cleanroomsml.types.collaboration_ml_input_channel_summary.deserialize_json(
                item
            )
        )
    return out
