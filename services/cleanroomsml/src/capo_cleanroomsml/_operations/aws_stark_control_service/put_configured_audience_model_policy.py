...
def handle_response(
    ...
    out: capo_cleanroomsml.types.put_configured_audience_model_policy_response.PutConfiguredAudienceModelPolicyResponse = capo_cleanroomsml.types.put_configured_audience_model_policy_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(input_["configured_audience_model_arn"], safe=""),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_cleanroomsml.types.put_configured_audience_model_policy_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
