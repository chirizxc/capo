"""Generated from Smithy shape ``com.amazonaws.cleanroomsml#ListTrainedModelVersions``."""
...
def handle_response(
    ...
    out: capo_cleanroomsml.types.list_trained_model_versions_response.ListTrainedModelVersionsResponse = capo_cleanroomsml.types.list_trained_model_versions_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{trainedModelArn}", quote(input_["trained_model_arn"], safe=""))
    ...
    if "status" in input_:
        params.append(
            (
                "status",
                capo_cleanroomsml.types.trained_model_status.serialize_json(
                    input_["status"]
                ),
            )
        )
...
async def async_list_trained_model_versions(
