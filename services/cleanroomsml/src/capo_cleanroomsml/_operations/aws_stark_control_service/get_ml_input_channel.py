...
def handle_response(
    ...
    out: capo_cleanroomsml.types.get_ml_input_channel_response.GetMLInputChannelResponse = capo_cleanroomsml.types.get_ml_input_channel_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{mlInputChannelArn}", quote(input_["ml_input_channel_arn"], safe="")
    )
    url = url.replace(
