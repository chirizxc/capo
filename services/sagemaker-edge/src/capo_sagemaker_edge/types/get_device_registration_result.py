...
if TYPE_CHECKING:
    import capo_sagemaker_edge.types.cache_ttl_seconds
    import capo_sagemaker_edge.types.device_registration


class GetDeviceRegistrationResult(TypedDict, closed=True):
    device_registration: NotRequired[
        "capo_sagemaker_edge.types.device_registration.DeviceRegistration"
    ]
    """<p>Describes if the device is currently registered with SageMaker Edge Manager.</p>"""
    cache_ttl: NotRequired[
        "capo_sagemaker_edge.types.cache_ttl_seconds.CacheTTLSeconds"
    ]
