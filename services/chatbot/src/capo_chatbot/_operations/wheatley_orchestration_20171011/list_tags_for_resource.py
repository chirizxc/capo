"""Generated from Smithy shape ``com.amazonaws.chatbot#ListTagsForResource``."""
...
def handle_response(
    ...
    out: capo_chatbot.types.list_tags_for_resource_response.ListTagsForResourceResponse = capo_chatbot.types.list_tags_for_resource_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-tags-for-resource"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chatbot.types.list_tags_for_resource_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
