"""Generated from Smithy shape ``com.amazonaws.chatbot#DeleteCustomAction``."""
...
def handle_response(
    ...
    out: capo_chatbot.types.delete_custom_action_result.DeleteCustomActionResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-custom-action"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chatbot.types.delete_custom_action_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_delete_custom_action(
