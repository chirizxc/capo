...
class InvalidKeyUsageException_(TypedDict, closed=True):
    message: NotRequired["capo_kms.types.error_message_type.ErrorMessageType"]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: InvalidKeyUsageException_) -> dict:
    out: dict = {}
    if "message" in value:
...
def deserialize_aws_json_1_1(data: dict) -> InvalidKeyUsageException_:
    out: InvalidKeyUsageException_ = {}  # type: ignore[typeddict-item]
...
class InvalidKeyUsageException(ServiceError):
    ...
    def __init__(self, data: InvalidKeyUsageException_, message: str | None = None):
        super().__init__(
            "client",
