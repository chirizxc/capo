"""Generated from Smithy shape ``com.amazonaws.codestarnotifications#EventTypeBatch``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_codestar_notifications.types.event_type_summary

EventTypeBatch: TypeAlias = list[
    "capo_codestar_notifications.types.event_type_summary.EventTypeSummary"
]


# --- restJson1 ser/de ---
def serialize_json(value: EventTypeBatch) -> list:
    import capo_codestar_notifications.types.event_type_summary

    out: list = []
    for item in value:
        out.append(
            capo_codestar_notifications.types.event_type_summary.serialize_json(item)
        )
    return out


def deserialize_json(data: list) -> EventTypeBatch:
    import capo_codestar_notifications.types.event_type_summary

    out: EventTypeBatch = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_codestar_notifications.types.event_type_summary.deserialize_json(item)
        )
    return out
