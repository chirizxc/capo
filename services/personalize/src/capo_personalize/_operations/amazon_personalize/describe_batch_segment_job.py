"""Generated from Smithy shape ``com.amazonaws.personalize#DescribeBatchSegmentJob``."""
...
import capo_personalize._protocol.eventstream
...
def handle_response(
    ...
    out: capo_personalize.types.describe_batch_segment_job_response.DescribeBatchSegmentJobResponse = capo_personalize.types.describe_batch_segment_job_response.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_personalize._auth._sigv4.build_sigv4_auth_scheme(
                "personalize", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "AmazonPersonalize.DescribeBatchSegmentJob"
    body: bytes | None = json.dumps(
        capo_personalize.types.describe_batch_segment_job_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_describe_batch_segment_job(
