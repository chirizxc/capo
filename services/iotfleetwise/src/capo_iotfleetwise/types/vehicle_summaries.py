"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#vehicleSummaries``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_iotfleetwise.types.vehicle_summary

vehicleSummaries: TypeAlias = list[
    "capo_iotfleetwise.types.vehicle_summary.VehicleSummary"
]


# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: vehicleSummaries) -> list:
    import capo_iotfleetwise.types.vehicle_summary

    out: list = []
    for item in value:
        out.append(capo_iotfleetwise.types.vehicle_summary.serialize_aws_json_1_0(item))
    return out


def deserialize_aws_json_1_0(data: list) -> vehicleSummaries:
    import capo_iotfleetwise.types.vehicle_summary

    out: vehicleSummaries = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_iotfleetwise.types.vehicle_summary.deserialize_aws_json_1_0(item)
        )
    return out
