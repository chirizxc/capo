"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#ListSignalCatalogNodes``."""
...
def handle_response(
    ...
    out: capo_iotfleetwise.types.list_signal_catalog_nodes_response.ListSignalCatalogNodesResponse = capo_iotfleetwise.types.list_signal_catalog_nodes_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/signal-catalogs/{name}/nodes"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "IoTAutobahnControlPlane.ListSignalCatalogNodes"
    body: bytes | None = json.dumps(
        capo_iotfleetwise.types.list_signal_catalog_nodes_request.serialize_aws_json_1_0(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_signal_catalog_nodes(
