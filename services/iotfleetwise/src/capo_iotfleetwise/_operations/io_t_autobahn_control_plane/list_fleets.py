"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#ListFleets``."""
...
def handle_response(
    ...
    out: capo_iotfleetwise.types.list_fleets_response.ListFleetsResponse = (
        capo_iotfleetwise.types.list_fleets_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/fleets"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "IoTAutobahnControlPlane.ListFleets"
    body: bytes | None = json.dumps(
        capo_iotfleetwise.types.list_fleets_request.serialize_aws_json_1_0(input_),
        allow_nan=False,
    ).encode()
...
def list_fleets(
    ...
    capo_iotfleetwise.types.list_fleets_response.ListFleetsResponse, zapros.Response
]:
