"""Generated from Smithy shape ``com.amazonaws.qconnect#UpdateSessionData``."""
...
def handle_response(
    ...
    out: capo_qconnect.types.update_session_data_response.UpdateSessionDataResponse = (
        capo_qconnect.types.update_session_data_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/assistants/{assistantId}/sessions/{sessionId}/data"
    )
    ...
    body: bytes | None = json.dumps(
        capo_qconnect.types.update_session_data_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_session_data(
