"""Generated from Smithy shape ``com.amazonaws.qconnect#UpdateMessageTemplateMetadata``."""
...
def handle_response(
    ...
    out: capo_qconnect.types.update_message_template_metadata_response.UpdateMessageTemplateMetadataResponse = capo_qconnect.types.update_message_template_metadata_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{messageTemplateId}", quote(input_["message_template_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qconnect.types.update_message_template_metadata_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_message_template_metadata(
