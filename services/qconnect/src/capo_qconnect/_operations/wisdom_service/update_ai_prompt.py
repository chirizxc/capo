"""Generated from Smithy shape ``com.amazonaws.qconnect#UpdateAIPrompt``."""
...
def handle_response(
    ...
    out: capo_qconnect.types.update_ai_prompt_response.UpdateAIPromptResponse = (
        capo_qconnect.types.update_ai_prompt_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{aiPromptId}", quote(input_["ai_prompt_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qconnect.types.update_ai_prompt_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_ai_prompt(
