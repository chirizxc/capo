...
if TYPE_CHECKING:
    import capo_qconnect.types.grouping_criteria
    import capo_qconnect.types.grouping_values


class GroupingConfiguration(TypedDict, closed=True):
    criteria: NotRequired["capo_qconnect.types.grouping_criteria.GroupingCriteria"]
    ...
    values: NotRequired["capo_qconnect.types.grouping_values.GroupingValues"]
...
def serialize_json(value: GroupingConfiguration) -> dict:
    ...
    if "values" in value:
        ...
        out["values"] = capo_qconnect.types.grouping_values.serialize_json(
            value["values"]
        )
    return out
...
def deserialize_json(data: dict) -> GroupingConfiguration:
    ...
    if data.get("values") is not None:
        ...
        out["values"] = capo_qconnect.types.grouping_values.deserialize_json(
            data["values"]
        )
    return out
