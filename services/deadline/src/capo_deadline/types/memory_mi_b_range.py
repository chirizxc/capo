"""Generated from Smithy shape ``com.amazonaws.deadline#MemoryMiBRange``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_deadline.errors import DeserializationError

if TYPE_CHECKING:
    import capo_deadline.types.memory_amount_mi_b


class MemoryMiBRange(TypedDict, closed=True):
    min: "capo_deadline.types.memory_amount_mi_b.MemoryAmountMiB"
    """<p>The minimum amount of memory (in MiB).</p>"""
    max: NotRequired["capo_deadline.types.memory_amount_mi_b.MemoryAmountMiB"]
    """<p>The maximum amount of memory (in MiB).</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: MemoryMiBRange) -> dict:
    out: dict = {}
    out["min"] = value["min"]
    if "max" in value:
        out["max"] = value["max"]
    return out


def deserialize_json(data: dict) -> MemoryMiBRange:
    out: MemoryMiBRange = {}  # type: ignore[typeddict-item]
    if data.get("min") is not None:
        out["min"] = data["min"]
    else:
        raise DeserializationError("MemoryMiBRange.min required")
    if data.get("max") is not None:
        out["max"] = data["max"]
    return out
