"""Generated from Smithy shape ``com.amazonaws.deadline#StorageProfileSummaries``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_deadline.types.storage_profile_summary

StorageProfileSummaries: TypeAlias = list[
    "capo_deadline.types.storage_profile_summary.StorageProfileSummary"
]


# --- restJson1 ser/de ---
def serialize_json(value: StorageProfileSummaries) -> list:
    import capo_deadline.types.storage_profile_summary

    out: list = []
    for item in value:
        out.append(capo_deadline.types.storage_profile_summary.serialize_json(item))
    return out


def deserialize_json(data: list) -> StorageProfileSummaries:
    import capo_deadline.types.storage_profile_summary

    out: StorageProfileSummaries = []
    for item in data:
        if item is None:
            continue
        out.append(capo_deadline.types.storage_profile_summary.deserialize_json(item))
    return out
