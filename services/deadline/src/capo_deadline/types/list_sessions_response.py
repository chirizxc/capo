"""Generated from Smithy shape ``com.amazonaws.deadline#ListSessionsResponse``."""
...
class ListSessionsResponse(TypedDict, closed=True):
    sessions: "capo_deadline.types.session_summaries.SessionSummaries"
...
def serialize_json(value: ListSessionsResponse) -> dict:
    ...
    out["sessions"] = capo_deadline.types.session_summaries.serialize_json(
        value["sessions"]
    )
...
def deserialize_json(data: dict) -> ListSessionsResponse:
    ...
    if data.get("sessions") is not None:
        ...
        out["sessions"] = capo_deadline.types.session_summaries.deserialize_json(
            data["sessions"]
        )
    else:
