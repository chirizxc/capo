...
def handle_response(
    ...
    out: capo_deadline.types.update_budget_response.UpdateBudgetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{budgetId}", quote(input_["budget_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "client_token" in input_:
        headers["X-Amz-Client-Token"] = input_["client_token"]
    body: bytes | None = json.dumps(
        capo_deadline.types.update_budget_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def update_budget(
    ...
    capo_deadline.types.update_budget_response.UpdateBudgetResponse, zapros.Response
]:
