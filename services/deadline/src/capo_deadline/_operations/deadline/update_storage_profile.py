"""Generated from Smithy shape ``com.amazonaws.deadline#UpdateStorageProfile``."""
...
def handle_response(
    ...
    out: capo_deadline.types.update_storage_profile_response.UpdateStorageProfileResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{storageProfileId}", quote(input_["storage_profile_id"], safe="")
    )
    ...
    body: bytes | None = json.dumps(
        capo_deadline.types.update_storage_profile_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_storage_profile(
