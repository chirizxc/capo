"""Generated from Smithy shape ``com.amazonaws.deadline#PutMeteredProduct``."""
...
def handle_response(
    ...
    out: capo_deadline.types.put_metered_product_response.PutMeteredProductResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{licenseEndpointId}", quote(input_["license_endpoint_id"], safe="")
    )
    url = url.replace("{productId}", quote(input_["product_id"], safe=""))
...
async def async_put_metered_product(
