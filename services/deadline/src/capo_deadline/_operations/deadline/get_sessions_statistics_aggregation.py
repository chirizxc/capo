"""Generated from Smithy shape ``com.amazonaws.deadline#GetSessionsStatisticsAggregation``."""
...
import capo_deadline._protocol.eventstream
...
def handle_response(
    ...
    out: capo_deadline.types.get_sessions_statistics_aggregation_response.GetSessionsStatisticsAggregationResponse = capo_deadline.types.get_sessions_statistics_aggregation_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_deadline._auth._sigv4.build_sigv4_auth_scheme(
                "deadline", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace("{farmId}", quote(input_["farm_id"], safe=""))
    params: list[tuple[str, str]] = []
    if "next_token" in input_:
        params.append(("nextToken", input_["next_token"]))
    params.append(("maxResults", str(input_.get("max_results", 100))))
    if "aggregation_id" in input_:
        params.append(("aggregationId", input_["aggregation_id"]))
...
async def async_get_sessions_statistics_aggregation(
