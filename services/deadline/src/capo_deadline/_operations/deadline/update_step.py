"""Generated from Smithy shape ``com.amazonaws.deadline#UpdateStep``."""
...
def handle_response(
    ...
    out: capo_deadline.types.update_step_response.UpdateStepResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{queueId}", quote(input_["queue_id"], safe=""))
    url = url.replace("{jobId}", quote(input_["job_id"], safe=""))
    url = url.replace("{stepId}", quote(input_["step_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "client_token" in input_:
        headers["X-Amz-Client-Token"] = input_["client_token"]
    body: bytes | None = json.dumps(
        capo_deadline.types.update_step_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def update_step(
    ...
    capo_deadline.types.update_step_response.UpdateStepResponse, zapros.Response
]:
