"""Generated from Smithy shape ``com.amazonaws.billingconductor#AssociatePricingRules``."""
...
import capo_billingconductor._protocol.eventstream
...
def handle_response(
    ...
    out: capo_billingconductor.types.associate_pricing_rules_output.AssociatePricingRulesOutput = capo_billingconductor.types.associate_pricing_rules_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_billingconductor._auth._sigv4.build_sigv4_auth_scheme(
                "billingconductor", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/associate-pricing-rules"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_billingconductor.types.associate_pricing_rules_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_associate_pricing_rules(
