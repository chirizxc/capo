"""Generated from Smithy shape ``com.amazonaws.billingconductor#DisassociateAccounts``."""
...
def handle_response(
    ...
    out: capo_billingconductor.types.disassociate_accounts_output.DisassociateAccountsOutput = capo_billingconductor.types.disassociate_accounts_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/disassociate-accounts"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_billingconductor.types.disassociate_accounts_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
