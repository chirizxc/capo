"""Generated from Smithy shape ``com.amazonaws.billingconductor#DeleteCustomLineItem``."""
...
def handle_response(
    ...
    out: capo_billingconductor.types.delete_custom_line_item_output.DeleteCustomLineItemOutput = capo_billingconductor.types.delete_custom_line_item_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-custom-line-item"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_billingconductor.types.delete_custom_line_item_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_delete_custom_line_item(
