"""Generated from Smithy shape ``com.amazonaws.elasticache#CompleteMigration``."""
...
import capo_elasticache.errors.replication_group_not_under_migration_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ReplicationGroupNotUnderMigrationFault":
            raise capo_elasticache.errors.replication_group_not_under_migration_fault.ReplicationGroupNotUnderMigrationFault.from_query(
                error_el, message
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CompleteMigrationResult")
    out: capo_elasticache.types.complete_migration_response.CompleteMigrationResponse = capo_elasticache.types.complete_migration_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "CompleteMigration"))
    pairs.append(("Version", "2015-02-02"))
    capo_elasticache.types.complete_migration_message.serialize_query(input_, pairs, "")
...
async def async_complete_migration(
