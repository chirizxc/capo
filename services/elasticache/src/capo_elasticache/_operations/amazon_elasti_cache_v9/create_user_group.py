...
import capo_elasticache.errors.default_user_required
import capo_elasticache.errors.duplicate_user_name_fault
...
import capo_elasticache.errors.user_group_already_exists_fault
import capo_elasticache.errors.user_group_quota_exceeded_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "DefaultUserRequired":
            raise capo_elasticache.errors.default_user_required.DefaultUserRequired.from_query(
                error_el, message
            )
        case "DuplicateUserName":
            raise capo_elasticache.errors.duplicate_user_name_fault.DuplicateUserNameFault.from_query(
                error_el, message
            )
        ...
        case "UserGroupAlreadyExists":
            raise capo_elasticache.errors.user_group_already_exists_fault.UserGroupAlreadyExistsFault.from_query(
                error_el, message
            )
        case "UserGroupQuotaExceeded":
            raise capo_elasticache.errors.user_group_quota_exceeded_fault.UserGroupQuotaExceededFault.from_query(
                error_el, message
            )
        case "UserNotFound":
...
def handle_response(
    ...
    out: capo_elasticache.types.user_group.UserGroup = (
        capo_elasticache.types.user_group.deserialize_query(
...
def build_request(
    ...
    capo_elasticache.types.create_user_group_message.serialize_query(input_, pairs, "")
...
def create_user_group(
    ...
    input_: capo_elasticache.types.create_user_group_message.CreateUserGroupMessage,
) -> tuple[capo_elasticache.types.user_group.UserGroup, zapros.Response]:
