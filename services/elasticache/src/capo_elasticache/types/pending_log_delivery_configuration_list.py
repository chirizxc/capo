...
if TYPE_CHECKING:
    import capo_elasticache.types.pending_log_delivery_configuration

PendingLogDeliveryConfigurationList: TypeAlias = list[
    "capo_elasticache.types.pending_log_delivery_configuration.PendingLogDeliveryConfiguration"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elasticache.types.pending_log_delivery_configuration.serialize_query(
...
def deserialize_query(el: Element) -> PendingLogDeliveryConfigurationList:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elasticache.types.pending_log_delivery_configuration.deserialize_query(
                child
            )
        )
    return out
