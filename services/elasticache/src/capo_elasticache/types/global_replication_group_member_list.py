...
if TYPE_CHECKING:
    import capo_elasticache.types.global_replication_group_member

GlobalReplicationGroupMemberList: TypeAlias = list[
    "capo_elasticache.types.global_replication_group_member.GlobalReplicationGroupMember"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elasticache.types.global_replication_group_member.serialize_query(
...
def deserialize_query(el: Element) -> GlobalReplicationGroupMemberList:
    ...
    for child in el.findall("GlobalReplicationGroupMember"):
        out.append(
            capo_elasticache.types.global_replication_group_member.deserialize_query(
                child
            )
        )
    return out
