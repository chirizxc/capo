...
# --- awsQuery ser/de ---
def serialize_query(
    value: CacheSubnetGroupNotFoundFault_, pairs: list[tuple[str, str]], prefix: str
) -> None:
...
def deserialize_query(el: Element) -> CacheSubnetGroupNotFoundFault_:
    out: CacheSubnetGroupNotFoundFault_ = {}  # type: ignore[typeddict-item]
...
class CacheSubnetGroupNotFoundFault(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.elasticache#CacheSubnetGroupNotFoundFault``."""

    code: str | None = "CacheSubnetGroupNotFoundFault"

    def __init__(
        self, data: CacheSubnetGroupNotFoundFault_, message: str | None = None
    ):
        super().__init__(
            "client",
