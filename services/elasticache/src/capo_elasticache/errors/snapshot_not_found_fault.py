"""Generated from Smithy shape ``com.amazonaws.elasticache#SnapshotNotFoundFault``."""
...
# --- awsQuery ser/de ---
def serialize_query(
    value: SnapshotNotFoundFault_, pairs: list[tuple[str, str]], prefix: str
) -> None:
...
def deserialize_query(el: Element) -> SnapshotNotFoundFault_:
...
class SnapshotNotFoundFault(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.elasticache#SnapshotNotFoundFault``."""

    code: str | None = "SnapshotNotFoundFault"

    def __init__(self, data: SnapshotNotFoundFault_, message: str | None = None):
        super().__init__(
            "client",
