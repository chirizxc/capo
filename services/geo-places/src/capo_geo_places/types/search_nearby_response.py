...
class SearchNearbyResponse(TypedDict, closed=True):
    ...
    result_items: NotRequired[
        "capo_geo_places.types.search_nearby_result_item_list.SearchNearbyResultItemList"
    ]
...
def serialize_json(value: SearchNearbyResponse) -> dict:
    ...
    if "result_items" in value:
        ...
        out["ResultItems"] = (
            capo_geo_places.types.search_nearby_result_item_list.serialize_json(
                value["result_items"]
            )
        )
...
def deserialize_json(data: dict) -> SearchNearbyResponse:
    ...
    if data.get("ResultItems") is not None:
        ...
        out["result_items"] = (
            capo_geo_places.types.search_nearby_result_item_list.deserialize_json(
                data["ResultItems"]
            )
        )
