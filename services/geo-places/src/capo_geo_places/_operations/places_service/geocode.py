"""Generated from Smithy shape ``com.amazonaws.geoplaces#Geocode``."""
...
def handle_response(
    ...
    out: capo_geo_places.types.geocode_response.GeocodeResponse = (
        capo_geo_places.types.geocode_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/geocode"
    params: list[tuple[str, str]] = []
    if "key" in input_:
        params.append(("key", input_["key"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_geo_places.types.geocode_request.serialize_json(input_), allow_nan=False
    ).encode()
...
def geocode(
    ...
    input_: capo_geo_places.types.geocode_request.GeocodeRequest,
) -> tuple[capo_geo_places.types.geocode_response.GeocodeResponse, zapros.Response]:
