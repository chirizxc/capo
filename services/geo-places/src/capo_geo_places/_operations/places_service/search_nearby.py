"""Generated from Smithy shape ``com.amazonaws.geoplaces#SearchNearby``."""
...
def handle_response(
    ...
    out: capo_geo_places.types.search_nearby_response.SearchNearbyResponse = (
        capo_geo_places.types.search_nearby_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/search-nearby"
    params: list[tuple[str, str]] = []
    if "key" in input_:
        params.append(("key", input_["key"]))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_geo_places.types.search_nearby_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def search_nearby(
    ...
    capo_geo_places.types.search_nearby_response.SearchNearbyResponse, zapros.Response
]:
