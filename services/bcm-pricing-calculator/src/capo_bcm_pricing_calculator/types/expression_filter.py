...
class ExpressionFilter(TypedDict, closed=True):
    ...
    match_options: NotRequired[
        "capo_bcm_pricing_calculator.types.string_list.StringList"
    ]
    """<p> The match options for the filter (e.g., equals, contains). </p>"""
    values: NotRequired["capo_bcm_pricing_calculator.types.string_list.StringList"]
...
def serialize_aws_json_1_0(value: ExpressionFilter) -> dict:
    ...
    if "match_options" in value:
        ...
        out["matchOptions"] = (
            capo_bcm_pricing_calculator.types.string_list.serialize_aws_json_1_0(
                value["match_options"]
            )
        )
    if "values" in value:
...
def deserialize_aws_json_1_0(data: dict) -> ExpressionFilter:
    ...
    if data.get("matchOptions") is not None:
        ...
        out["match_options"] = (
            capo_bcm_pricing_calculator.types.string_list.deserialize_aws_json_1_0(
                data["matchOptions"]
            )
        )
