...
ExpressionList: TypeAlias = list[
    "capo_bcm_pricing_calculator.types.expression.Expression"
]
...
def serialize_aws_json_1_0(value: ExpressionList) -> list:
    ...
    for item in value:
        out.append(
            capo_bcm_pricing_calculator.types.expression.serialize_aws_json_1_0(item)
        )
    return out
...
def deserialize_aws_json_1_0(data: list) -> ExpressionList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_bcm_pricing_calculator.types.expression.deserialize_aws_json_1_0(item)
        )
    return out
