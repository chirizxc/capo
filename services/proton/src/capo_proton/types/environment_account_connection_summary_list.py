...
EnvironmentAccountConnectionSummaryList: TypeAlias = list[
    "capo_proton.types.environment_account_connection_summary.EnvironmentAccountConnectionSummary"
]
...
def serialize_aws_json_1_0(value: EnvironmentAccountConnectionSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_proton.types.environment_account_connection_summary.serialize_aws_json_1_0(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_0(data: list) -> EnvironmentAccountConnectionSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_proton.types.environment_account_connection_summary.deserialize_aws_json_1_0(
                item
            )
        )
    return out
