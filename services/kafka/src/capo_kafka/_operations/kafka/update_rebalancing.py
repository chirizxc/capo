"""Generated from Smithy shape ``com.amazonaws.kafka#UpdateRebalancing``."""
...
def handle_response(
    ...
    out: capo_kafka.types.update_rebalancing_response.UpdateRebalancingResponse = (
        capo_kafka.types.update_rebalancing_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/clusters/{ClusterArn}/rebalancing"
    url = url.replace("{ClusterArn}", quote(input_["cluster_arn"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_kafka.types.update_rebalancing_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_rebalancing(
