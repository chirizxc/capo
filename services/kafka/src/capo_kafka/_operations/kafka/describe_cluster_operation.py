...
def handle_response(
    ...
    out: capo_kafka.types.describe_cluster_operation_response.DescribeClusterOperationResponse = capo_kafka.types.describe_cluster_operation_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{ClusterOperationArn}", quote(input_["cluster_operation_arn"], safe="")
    )
