"""Generated from Smithy shape ``com.amazonaws.kafka#DescribeConfigurationRevision``."""
...
def handle_response(
    ...
    out: capo_kafka.types.describe_configuration_revision_response.DescribeConfigurationRevisionResponse = capo_kafka.types.describe_configuration_revision_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/configurations/{Arn}/revisions/{Revision}"
    url = url.replace("{Arn}", quote(input_["arn"], safe=""))
    url = url.replace("{Revision}", quote(str(input_["revision"]), safe=""))
