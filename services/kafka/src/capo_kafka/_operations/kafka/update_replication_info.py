...
def handle_response(
    ...
    out: capo_kafka.types.update_replication_info_response.UpdateReplicationInfoResponse = capo_kafka.types.update_replication_info_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{ReplicatorArn}", quote(input_["replicator_arn"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_kafka.types.update_replication_info_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
