"""Generated from Smithy shape ``com.amazonaws.kafka#NodeInfo``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_kafka.types.__string
    import capo_kafka.types.broker_node_info
    import capo_kafka.types.controller_node_info
    import capo_kafka.types.node_type
    import capo_kafka.types.zookeeper_node_info


class NodeInfo(TypedDict, closed=True):
    added_to_cluster_time: NotRequired["capo_kafka.types.__string.__string"]
    """<p>The start time.</p>"""
    broker_node_info: NotRequired["capo_kafka.types.broker_node_info.BrokerNodeInfo"]
    """<p>The broker node info.</p>"""
    controller_node_info: NotRequired[
        "capo_kafka.types.controller_node_info.ControllerNodeInfo"
    ]
    """<p>The ControllerNodeInfo.</p>"""
    instance_type: NotRequired["capo_kafka.types.__string.__string"]
    """<p>The instance type.</p>"""
    node_arn: NotRequired["capo_kafka.types.__string.__string"]
    """<p>The Amazon Resource Name (ARN) of the node.</p>"""
    node_type: NotRequired["capo_kafka.types.node_type.NodeType"]
    """<p>The node type.</p>"""
    zookeeper_node_info: NotRequired[
        "capo_kafka.types.zookeeper_node_info.ZookeeperNodeInfo"
    ]
    """<p>The ZookeeperNodeInfo.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: NodeInfo) -> dict:
    out: dict = {}
    if "added_to_cluster_time" in value:
        out["addedToClusterTime"] = value["added_to_cluster_time"]
    if "broker_node_info" in value:
        import capo_kafka.types.broker_node_info

        out["brokerNodeInfo"] = capo_kafka.types.broker_node_info.serialize_json(
            value["broker_node_info"]
        )
    if "controller_node_info" in value:
        import capo_kafka.types.controller_node_info

        out["controllerNodeInfo"] = (
            capo_kafka.types.controller_node_info.serialize_json(
                value["controller_node_info"]
            )
        )
    if "instance_type" in value:
        out["instanceType"] = value["instance_type"]
    if "node_arn" in value:
        out["nodeARN"] = value["node_arn"]
    if "node_type" in value:
        import capo_kafka.types.node_type

        out["nodeType"] = capo_kafka.types.node_type.serialize_json(value["node_type"])
    if "zookeeper_node_info" in value:
        import capo_kafka.types.zookeeper_node_info

        out["zookeeperNodeInfo"] = capo_kafka.types.zookeeper_node_info.serialize_json(
            value["zookeeper_node_info"]
        )
    return out


def deserialize_json(data: dict) -> NodeInfo:
    out: NodeInfo = {}  # type: ignore[typeddict-item]
    if data.get("addedToClusterTime") is not None:
        out["added_to_cluster_time"] = data["addedToClusterTime"]
    if data.get("brokerNodeInfo") is not None:
        import capo_kafka.types.broker_node_info

        out["broker_node_info"] = capo_kafka.types.broker_node_info.deserialize_json(
            data["brokerNodeInfo"]
        )
    if data.get("controllerNodeInfo") is not None:
        import capo_kafka.types.controller_node_info

        out["controller_node_info"] = (
            capo_kafka.types.controller_node_info.deserialize_json(
                data["controllerNodeInfo"]
            )
        )
    if data.get("instanceType") is not None:
        out["instance_type"] = data["instanceType"]
    if data.get("nodeARN") is not None:
        out["node_arn"] = data["nodeARN"]
    if data.get("nodeType") is not None:
        import capo_kafka.types.node_type

        out["node_type"] = capo_kafka.types.node_type.deserialize_json(data["nodeType"])
    if data.get("zookeeperNodeInfo") is not None:
        import capo_kafka.types.zookeeper_node_info

        out["zookeeper_node_info"] = (
            capo_kafka.types.zookeeper_node_info.deserialize_json(
                data["zookeeperNodeInfo"]
            )
        )
    return out
