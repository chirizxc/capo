...
class ListConfigurationsResponse(TypedDict, closed=True):
    configurations: NotRequired[
        "capo_kafka.types.__list_of_configuration.__listOfConfiguration"
    ]
...
def serialize_json(value: ListConfigurationsResponse) -> dict:
    ...
    if "configurations" in value:
        ...
        out["configurations"] = capo_kafka.types.__list_of_configuration.serialize_json(
...
def deserialize_json(data: dict) -> ListConfigurationsResponse:
    ...
    if data.get("configurations") is not None:
        ...
        out["configurations"] = (
            capo_kafka.types.__list_of_configuration.deserialize_json(
                data["configurations"]
            )
        )
