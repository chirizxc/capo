"""Generated from Smithy shape ``com.amazonaws.kafka#TopicPartitionInfo``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_kafka.types.__integer
    import capo_kafka.types.__list_of__integer


class TopicPartitionInfo(TypedDict, closed=True):
    partition: NotRequired["capo_kafka.types.__integer.__integer"]
    """<p>The partition ID.</p>"""
    leader: NotRequired["capo_kafka.types.__integer.__integer"]
    """<p>The leader broker ID for the partition.</p>"""
    replicas: NotRequired["capo_kafka.types.__list_of__integer.__listOf__integer"]
    """<p>The list of replica broker IDs for the partition.</p>"""
    isr: NotRequired["capo_kafka.types.__list_of__integer.__listOf__integer"]
    """<p>The list of in-sync replica broker IDs for the partition.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: TopicPartitionInfo) -> dict:
    out: dict = {}
    if "partition" in value:
        out["partition"] = value["partition"]
    if "leader" in value:
        out["leader"] = value["leader"]
    if "replicas" in value:
        import capo_kafka.types.__list_of__integer

        out["replicas"] = capo_kafka.types.__list_of__integer.serialize_json(
            value["replicas"]
        )
    if "isr" in value:
        import capo_kafka.types.__list_of__integer

        out["isr"] = capo_kafka.types.__list_of__integer.serialize_json(value["isr"])
    return out


def deserialize_json(data: dict) -> TopicPartitionInfo:
    out: TopicPartitionInfo = {}  # type: ignore[typeddict-item]
    if data.get("partition") is not None:
        out["partition"] = data["partition"]
    if data.get("leader") is not None:
        out["leader"] = data["leader"]
    if data.get("replicas") is not None:
        import capo_kafka.types.__list_of__integer

        out["replicas"] = capo_kafka.types.__list_of__integer.deserialize_json(
            data["replicas"]
        )
    if data.get("isr") is not None:
        import capo_kafka.types.__list_of__integer

        out["isr"] = capo_kafka.types.__list_of__integer.deserialize_json(data["isr"])
    return out
