...
class ListClustersV2Response(TypedDict, closed=True):
    cluster_info_list: NotRequired["capo_kafka.types.__list_of_cluster.__listOfCluster"]
...
def serialize_json(value: ListClustersV2Response) -> dict:
    ...
    if "cluster_info_list" in value:
        ...
        out["clusterInfoList"] = capo_kafka.types.__list_of_cluster.serialize_json(
...
def deserialize_json(data: dict) -> ListClustersV2Response:
    ...
    if data.get("clusterInfoList") is not None:
        import capo_kafka.types.__list_of_cluster

        out["cluster_info_list"] = capo_kafka.types.__list_of_cluster.deserialize_json(
