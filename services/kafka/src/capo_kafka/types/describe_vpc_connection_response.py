...
class DescribeVpcConnectionResponse(TypedDict, closed=True):
    ...
    subnets: NotRequired["capo_kafka.types.__list_of__string.__listOf__string"]
...
def serialize_json(value: DescribeVpcConnectionResponse) -> dict:
    ...
    if "subnets" in value:
        ...
        out["subnets"] = capo_kafka.types.__list_of__string.serialize_json(
            value["subnets"]
        )
...
def deserialize_json(data: dict) -> DescribeVpcConnectionResponse:
    ...
    if data.get("subnets") is not None:
        ...
        out["subnets"] = capo_kafka.types.__list_of__string.deserialize_json(
            data["subnets"]
        )
