...
class ListClustersResponse(TypedDict, closed=True):
    cluster_info_list: NotRequired[
        "capo_kafka.types.__list_of_cluster_info.__listOfClusterInfo"
    ]
...
def serialize_json(value: ListClustersResponse) -> dict:
    ...
    if "cluster_info_list" in value:
        ...
        out["clusterInfoList"] = capo_kafka.types.__list_of_cluster_info.serialize_json(
...
def deserialize_json(data: dict) -> ListClustersResponse:
    ...
    if data.get("clusterInfoList") is not None:
        ...
        out["cluster_info_list"] = (
            capo_kafka.types.__list_of_cluster_info.deserialize_json(
