...
if TYPE_CHECKING:
    import capo_efs.types.file_system_nullable_size_value
    import capo_efs.types.file_system_size_value
    import capo_efs.types.timestamp


class FileSystemSize(TypedDict, closed=True):
    value: "capo_efs.types.file_system_size_value.FileSystemSizeValue"
    """<p>The latest known metered size (in bytes) of data stored in the file system.</p>"""
    timestamp: NotRequired["capo_efs.types.timestamp.Timestamp"]
    ...
    value_in_ia: NotRequired[
        "capo_efs.types.file_system_nullable_size_value.FileSystemNullableSizeValue"
    ]
...
def serialize_json(value: FileSystemSize) -> dict:
    ...
    if "timestamp" in value:
        ...
        out["Timestamp"] = capo_efs.types.timestamp.serialize_json(value["timestamp"])
