...
if TYPE_CHECKING:
    import capo_efs.types.access_point_arn
...
class AccessPointDescription(TypedDict, closed=True):
    client_token: NotRequired["capo_efs.types.client_token.ClientToken"]
    """<p>The opaque string specified in the request to ensure idempotent creation.</p>"""
    name: NotRequired["capo_efs.types.name.Name"]
    ...
    access_point_arn: NotRequired["capo_efs.types.access_point_arn.AccessPointArn"]
    ...
    life_cycle_state: NotRequired["capo_efs.types.life_cycle_state.LifeCycleState"]
