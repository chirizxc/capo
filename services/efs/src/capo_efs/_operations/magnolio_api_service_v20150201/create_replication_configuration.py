"""Generated from Smithy shape ``com.amazonaws.efs#CreateReplicationConfiguration``."""
...
import capo_efs.errors.conflict_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_efs.errors.conflict_exception.ConflictException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_efs.types.replication_configuration_description.ReplicationConfigurationDescription = capo_efs.types.replication_configuration_description.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{SourceFileSystemId}", quote(input_["source_file_system_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_efs.types.create_replication_configuration_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_replication_configuration(
