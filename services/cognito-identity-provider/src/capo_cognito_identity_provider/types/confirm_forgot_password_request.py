...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: ConfirmForgotPasswordRequest) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_1(data: dict) -> ConfirmForgotPasswordRequest:
    out: ConfirmForgotPasswordRequest = {}  # type: ignore[typeddict-item]
    if data.get("ClientId") is not None:
        out["client_id"] = data["ClientId"]
    else:
        raise DeserializationError("ConfirmForgotPasswordRequest.client_id required")
    if data.get("SecretHash") is not None:
        out["secret_hash"] = data["SecretHash"]
    if data.get("Username") is not None:
        out["username"] = data["Username"]
    else:
        raise DeserializationError("ConfirmForgotPasswordRequest.username required")
    if data.get("ConfirmationCode") is not None:
        out["confirmation_code"] = data["ConfirmationCode"]
    else:
        raise DeserializationError(
            "ConfirmForgotPasswordRequest.confirmation_code required"
        )
    if data.get("Password") is not None:
        out["password"] = data["Password"]
    else:
        raise DeserializationError("ConfirmForgotPasswordRequest.password required")
