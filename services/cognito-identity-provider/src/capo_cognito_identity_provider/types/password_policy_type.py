...
if TYPE_CHECKING:
    ...
    import capo_cognito_identity_provider.types.password_history_size_type
    import capo_cognito_identity_provider.types.password_policy_min_length_type
    import capo_cognito_identity_provider.types.temporary_password_validity_days_type


class PasswordPolicyType(TypedDict, closed=True):
    minimum_length: NotRequired[
        "capo_cognito_identity_provider.types.password_policy_min_length_type.PasswordPolicyMinLengthType"
    ]
    """<p>The minimum length of the password in the policy that you have set. This value can't be less than 6.</p>"""
    require_uppercase: "capo_cognito_identity_provider.types.boolean_type.BooleanType"
    """<p>The requirement in a password policy that users must include at least one uppercase letter in their password.</p>"""
    require_lowercase: "capo_cognito_identity_provider.types.boolean_type.BooleanType"
    """<p>The requirement in a password policy that users must include at least one lowercase letter in their password.</p>"""
    require_numbers: "capo_cognito_identity_provider.types.boolean_type.BooleanType"
    """<p>The requirement in a password policy that users must include at least one number in their password.</p>"""
    require_symbols: "capo_cognito_identity_provider.types.boolean_type.BooleanType"
    ...
    password_history_size: NotRequired[
        "capo_cognito_identity_provider.types.password_history_size_type.PasswordHistorySizeType"
    ]
    ...
    temporary_password_validity_days: "capo_cognito_identity_provider.types.temporary_password_validity_days_type.TemporaryPasswordValidityDaysType"
