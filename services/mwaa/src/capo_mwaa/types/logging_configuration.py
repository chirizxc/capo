...
class LoggingConfiguration(TypedDict, closed=True):
    dag_processing_logs: NotRequired[
        "capo_mwaa.types.module_logging_configuration.ModuleLoggingConfiguration"
    ]
...
def serialize_json(value: LoggingConfiguration) -> dict:
    ...
    if "dag_processing_logs" in value:
        ...
        out["DagProcessingLogs"] = (
            capo_mwaa.types.module_logging_configuration.serialize_json(
    ...
    if "worker_logs" in value:
        ...
        out["WorkerLogs"] = capo_mwaa.types.module_logging_configuration.serialize_json(
            value["worker_logs"]
        )
    if "task_logs" in value:
        import capo_mwaa.types.module_logging_configuration

        out["TaskLogs"] = capo_mwaa.types.module_logging_configuration.serialize_json(
            value["task_logs"]
        )
    return out
...
def deserialize_json(data: dict) -> LoggingConfiguration:
    ...
    if data.get("DagProcessingLogs") is not None:
        ...
        out["dag_processing_logs"] = (
            capo_mwaa.types.module_logging_configuration.deserialize_json(
