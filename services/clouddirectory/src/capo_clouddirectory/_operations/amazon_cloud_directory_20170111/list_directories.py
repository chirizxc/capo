"""Generated from Smithy shape ``com.amazonaws.clouddirectory#ListDirectories``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.list_directories_response.ListDirectoriesResponse = (
        capo_clouddirectory.types.list_directories_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/directory/list"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.list_directories_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_directories(
