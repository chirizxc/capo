"""Generated from Smithy shape ``com.amazonaws.clouddirectory#CreateIndex``."""
...
import capo_clouddirectory.errors.unsupported_index_type_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "UnsupportedIndexTypeException":
            raise capo_clouddirectory.errors.unsupported_index_type_exception.UnsupportedIndexTypeException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_clouddirectory.types.create_index_response.CreateIndexResponse = (
        capo_clouddirectory.types.create_index_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/index"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = input_["directory_arn"]
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.create_index_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def create_index(
    ...
    capo_clouddirectory.types.create_index_response.CreateIndexResponse, zapros.Response
]:
