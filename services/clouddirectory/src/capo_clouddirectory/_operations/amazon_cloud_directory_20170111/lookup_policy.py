"""Generated from Smithy shape ``com.amazonaws.clouddirectory#LookupPolicy``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.lookup_policy_response.LookupPolicyResponse = (
        capo_clouddirectory.types.lookup_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/policy/lookup"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = input_["directory_arn"]
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.lookup_policy_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
