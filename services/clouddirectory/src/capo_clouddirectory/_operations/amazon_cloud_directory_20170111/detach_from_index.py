"""Generated from Smithy shape ``com.amazonaws.clouddirectory#DetachFromIndex``."""
...
import capo_clouddirectory.errors.object_already_detached_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ObjectAlreadyDetachedException":
            raise capo_clouddirectory.errors.object_already_detached_exception.ObjectAlreadyDetachedException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_clouddirectory.types.detach_from_index_response.DetachFromIndexResponse = capo_clouddirectory.types.detach_from_index_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/index/detach"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = input_["directory_arn"]
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.detach_from_index_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_detach_from_index(
