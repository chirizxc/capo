"""Generated from Smithy shape ``com.amazonaws.clouddirectory#CreateTypedLinkFacet``."""
...
import capo_clouddirectory.errors.facet_already_exists_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "FacetAlreadyExistsException":
            raise capo_clouddirectory.errors.facet_already_exists_exception.FacetAlreadyExistsException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_clouddirectory.types.create_typed_link_facet_response.CreateTypedLinkFacetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    if "schema_arn" in input_:
        headers["x-amz-data-partition"] = input_["schema_arn"]
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.create_typed_link_facet_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_typed_link_facet(
