"""Generated from Smithy shape ``com.amazonaws.clouddirectory#AttachObject``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.attach_object_response.AttachObjectResponse = (
        capo_clouddirectory.types.attach_object_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/object/attach"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = input_["directory_arn"]
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.attach_object_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
