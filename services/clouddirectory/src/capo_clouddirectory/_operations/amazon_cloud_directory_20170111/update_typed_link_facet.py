"""Generated from Smithy shape ``com.amazonaws.clouddirectory#UpdateTypedLinkFacet``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.update_typed_link_facet_response.UpdateTypedLinkFacetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/typedlink/facet"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "schema_arn" in input_:
        headers["x-amz-data-partition"] = input_["schema_arn"]
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.update_typed_link_facet_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_typed_link_facet(
