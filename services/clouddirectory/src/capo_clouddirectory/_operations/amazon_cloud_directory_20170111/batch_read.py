"""Generated from Smithy shape ``com.amazonaws.clouddirectory#BatchRead``."""
...
import capo_clouddirectory.types.batch_read_operation_response_list
...
def handle_response(
    ...
    out: capo_clouddirectory.types.batch_read_response.BatchReadResponse = (
        capo_clouddirectory.types.batch_read_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/batchread"
    ...
    if "consistency_level" in input_:
        headers["x-amz-consistency-level"] = (
            capo_clouddirectory.types.consistency_level.serialize_json(
                input_["consistency_level"]
            )
        )
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.batch_read_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def batch_read(
    ...
    capo_clouddirectory.types.batch_read_response.BatchReadResponse, zapros.Response
]:
