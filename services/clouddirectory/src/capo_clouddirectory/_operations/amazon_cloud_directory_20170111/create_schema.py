"""Generated from Smithy shape ``com.amazonaws.clouddirectory#CreateSchema``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.create_schema_response.CreateSchemaResponse = (
        capo_clouddirectory.types.create_schema_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/schema/create"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.create_schema_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
