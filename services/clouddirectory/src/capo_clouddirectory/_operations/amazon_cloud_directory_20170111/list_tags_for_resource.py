"""Generated from Smithy shape ``com.amazonaws.clouddirectory#ListTagsForResource``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.list_tags_for_resource_response.ListTagsForResourceResponse = capo_clouddirectory.types.list_tags_for_resource_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/tags"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.list_tags_for_resource_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
