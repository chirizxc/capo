...
class BatchReadResponse(TypedDict, closed=True):
    responses: NotRequired[
        "capo_clouddirectory.types.batch_read_operation_response_list.BatchReadOperationResponseList"
    ]
...
def serialize_json(value: BatchReadResponse) -> dict:
    ...
    if "responses" in value:
        ...
        out["Responses"] = (
            capo_clouddirectory.types.batch_read_operation_response_list.serialize_json(
                value["responses"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> BatchReadResponse:
    ...
    if data.get("Responses") is not None:
        ...
        out["responses"] = (
            capo_clouddirectory.types.batch_read_operation_response_list.deserialize_json(
                data["Responses"]
            )
        )
    return out
