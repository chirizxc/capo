...
class DescribeJobQueuesResponse(TypedDict, closed=True):
    job_queues: NotRequired["capo_batch.types.job_queue_detail_list.JobQueueDetailList"]
...
def serialize_json(value: DescribeJobQueuesResponse) -> dict:
    ...
    if "job_queues" in value:
        ...
        out["jobQueues"] = capo_batch.types.job_queue_detail_list.serialize_json(
            value["job_queues"]
        )
...
def deserialize_json(data: dict) -> DescribeJobQueuesResponse:
    ...
    if data.get("jobQueues") is not None:
        import capo_batch.types.job_queue_detail_list

        out["job_queues"] = capo_batch.types.job_queue_detail_list.deserialize_json(
            data["jobQueues"]
        )
