...
class JobStateTimeLimitAction(TypedDict, closed=True):
    ...
    state: NotRequired[
        "capo_batch.types.job_state_time_limit_actions_state.JobStateTimeLimitActionsState"
    ]
    """<p>The state of the job needed to trigger the action. The only supported value is <code>RUNNABLE</code>.</p>"""
    max_time_seconds: NotRequired["capo_batch.types.integer.Integer"]
    ...
    action: NotRequired[
        "capo_batch.types.job_state_time_limit_actions_action.JobStateTimeLimitActionsAction"
    ]
...
def serialize_json(value: JobStateTimeLimitAction) -> dict:
    ...
    if "state" in value:
        ...
        out["state"] = (
            capo_batch.types.job_state_time_limit_actions_state.serialize_json(
                value["state"]
            )
        )
    ...
    if "action" in value:
        ...
        out["action"] = (
            capo_batch.types.job_state_time_limit_actions_action.serialize_json(
                value["action"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> JobStateTimeLimitAction:
    ...
    if data.get("state") is not None:
        ...
        out["state"] = (
            capo_batch.types.job_state_time_limit_actions_state.deserialize_json(
                data["state"]
            )
        )
    ...
    if data.get("action") is not None:
        ...
        out["action"] = (
            capo_batch.types.job_state_time_limit_actions_action.deserialize_json(
                data["action"]
            )
        )
    return out
