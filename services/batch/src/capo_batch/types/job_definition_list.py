...
JobDefinitionList: TypeAlias = list["capo_batch.types.job_definition.JobDefinition"]
...
def serialize_json(value: JobDefinitionList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_batch.types.job_definition.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> JobDefinitionList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_batch.types.job_definition.deserialize_json(item))
    return out
