...
class AttemptDetail(TypedDict, closed=True):
    container: NotRequired[
        "capo_batch.types.attempt_container_detail.AttemptContainerDetail"
    ]
    ...
    task_properties: NotRequired[
        "capo_batch.types.list_attempt_ecs_task_details.ListAttemptEcsTaskDetails"
    ]
...
def serialize_json(value: AttemptDetail) -> dict:
    ...
    if "container" in value:
        ...
        out["container"] = capo_batch.types.attempt_container_detail.serialize_json(
            value["container"]
        )
    ...
    if "task_properties" in value:
        ...
        out["taskProperties"] = (
            capo_batch.types.list_attempt_ecs_task_details.serialize_json(
...
def deserialize_json(data: dict) -> AttemptDetail:
    ...
    if data.get("taskProperties") is not None:
        ...
        out["task_properties"] = (
            capo_batch.types.list_attempt_ecs_task_details.deserialize_json(
                data["taskProperties"]
            )
        )
    return out
