...
class ListConsumableResourcesRequest(TypedDict, closed=True):
    filters: NotRequired[
...
def serialize_json(value: ListConsumableResourcesRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_batch.types.list_consumable_resources_filter_list.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListConsumableResourcesRequest:
    ...
    if data.get("filters") is not None:
        ...
        out["filters"] = (
            capo_batch.types.list_consumable_resources_filter_list.deserialize_json(
                data["filters"]
            )
        )
