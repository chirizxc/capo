"""Generated from Smithy shape ``com.amazonaws.batch#KeyValuesPair``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_batch.types.string
    import capo_batch.types.string_list


class KeyValuesPair(TypedDict, closed=True):
    name: NotRequired["capo_batch.types.string.String"]
    """<p>The name of the filter. Filter names are case sensitive.</p>"""
    values: NotRequired["capo_batch.types.string_list.StringList"]
    """<p>The filter values.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: KeyValuesPair) -> dict:
    out: dict = {}
    if "name" in value:
        out["name"] = value["name"]
    if "values" in value:
        import capo_batch.types.string_list

        out["values"] = capo_batch.types.string_list.serialize_json(value["values"])
    return out


def deserialize_json(data: dict) -> KeyValuesPair:
    out: KeyValuesPair = {}  # type: ignore[typeddict-item]
    if data.get("name") is not None:
        out["name"] = data["name"]
    if data.get("values") is not None:
        import capo_batch.types.string_list

        out["values"] = capo_batch.types.string_list.deserialize_json(data["values"])
    return out
