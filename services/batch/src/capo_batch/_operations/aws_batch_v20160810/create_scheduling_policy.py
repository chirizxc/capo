"""Generated from Smithy shape ``com.amazonaws.batch#CreateSchedulingPolicy``."""
...
def handle_response(
    ...
    out: capo_batch.types.create_scheduling_policy_response.CreateSchedulingPolicyResponse = capo_batch.types.create_scheduling_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/createschedulingpolicy"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.create_scheduling_policy_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_create_scheduling_policy(
