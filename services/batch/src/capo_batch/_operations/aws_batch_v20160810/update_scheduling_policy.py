"""Generated from Smithy shape ``com.amazonaws.batch#UpdateSchedulingPolicy``."""
...
def handle_response(
    ...
    out: capo_batch.types.update_scheduling_policy_response.UpdateSchedulingPolicyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/updateschedulingpolicy"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.update_scheduling_policy_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_update_scheduling_policy(
