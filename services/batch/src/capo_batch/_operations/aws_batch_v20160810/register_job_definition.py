"""Generated from Smithy shape ``com.amazonaws.batch#RegisterJobDefinition``."""
...
def handle_response(
    ...
    out: capo_batch.types.register_job_definition_response.RegisterJobDefinitionResponse = capo_batch.types.register_job_definition_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/registerjobdefinition"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.register_job_definition_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_register_job_definition(
