"""Generated from Smithy shape ``com.amazonaws.batch#DescribeServiceEnvironments``."""
...
import capo_batch._protocol.eventstream
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ClientException":
            raise capo_batch.errors.client_exception.ClientException.from_json(
                data, message
            )
        case "ServerException":
            raise capo_batch.errors.server_exception.ServerException.from_json(
                data, message
            )
        case _:
...
def handle_response(
    ...
    out: capo_batch.types.describe_service_environments_response.DescribeServiceEnvironmentsResponse = capo_batch.types.describe_service_environments_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_batch._auth._sigv4.build_sigv4_auth_scheme(
                "batch", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/describeserviceenvironments"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.describe_service_environments_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_describe_service_environments(
