"""Generated from Smithy shape ``com.amazonaws.simspaceweaver#StopClock``."""
...
import capo_simspaceweaver._protocol.eventstream
...
def handle_response(
    ...
    out: capo_simspaceweaver.types.stop_clock_output.StopClockOutput = {}  # type: ignore[typeddict-item]
    return out
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_simspaceweaver._auth._sigv4.build_sigv4_auth_scheme(
                "simspaceweaver", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/stopclock"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_simspaceweaver.types.stop_clock_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def stop_clock(
    ...
    capo_simspaceweaver.types.stop_clock_output.StopClockOutput, zapros.Response
]:
