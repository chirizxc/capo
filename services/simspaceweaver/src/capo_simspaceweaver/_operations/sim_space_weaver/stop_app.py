"""Generated from Smithy shape ``com.amazonaws.simspaceweaver#StopApp``."""
...
def handle_response(
    ...
    out: capo_simspaceweaver.types.stop_app_output.StopAppOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/stopapp"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_simspaceweaver.types.stop_app_input.serialize_json(input_), allow_nan=False
    ).encode()
...
def stop_app(
    ...
    input_: capo_simspaceweaver.types.stop_app_input.StopAppInput,
) -> tuple[capo_simspaceweaver.types.stop_app_output.StopAppOutput, zapros.Response]:
