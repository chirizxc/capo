"""Generated from Smithy shape ``com.amazonaws.datapipeline#PollForTask``."""
...
import capo_data_pipeline._protocol.eventstream
...
def handle_response(
    ...
    out: capo_data_pipeline.types.poll_for_task_output.PollForTaskOutput = (
        capo_data_pipeline.types.poll_for_task_output.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_data_pipeline._auth._sigv4.build_sigv4_auth_scheme(
                "datapipeline", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_data_pipeline.types.poll_for_task_input.serialize_aws_json_1_1(input_),
        allow_nan=False,
    ).encode()
...
def poll_for_task(
    ...
    capo_data_pipeline.types.poll_for_task_output.PollForTaskOutput, zapros.Response
]:
