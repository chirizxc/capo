...
if TYPE_CHECKING:
    import capo_data_pipeline.types.pipeline_description

PipelineDescriptionList: TypeAlias = list[
    "capo_data_pipeline.types.pipeline_description.PipelineDescription"
]
...
def serialize_aws_json_1_1(value: PipelineDescriptionList) -> list:
    ...
    for item in value:
        out.append(
            capo_data_pipeline.types.pipeline_description.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> PipelineDescriptionList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_data_pipeline.types.pipeline_description.deserialize_aws_json_1_1(item)
        )
    return out
