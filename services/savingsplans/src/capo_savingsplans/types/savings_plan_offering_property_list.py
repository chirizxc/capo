...
if TYPE_CHECKING:
    import capo_savingsplans.types.savings_plan_offering_property

SavingsPlanOfferingPropertyList: TypeAlias = list[
    "capo_savingsplans.types.savings_plan_offering_property.SavingsPlanOfferingProperty"
]
...
def serialize_json(value: SavingsPlanOfferingPropertyList) -> list:
    ...
    for item in value:
        out.append(
            capo_savingsplans.types.savings_plan_offering_property.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> SavingsPlanOfferingPropertyList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_savingsplans.types.savings_plan_offering_property.deserialize_json(
                item
            )
        )
    return out
