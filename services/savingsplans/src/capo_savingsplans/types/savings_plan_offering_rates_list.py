"""Generated from Smithy shape ``com.amazonaws.savingsplans#SavingsPlanOfferingRatesList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import capo_savingsplans.types.savings_plan_offering_rate

SavingsPlanOfferingRatesList: TypeAlias = list[
    "capo_savingsplans.types.savings_plan_offering_rate.SavingsPlanOfferingRate"
]


# --- restJson1 ser/de ---
def serialize_json(value: SavingsPlanOfferingRatesList) -> list:
    import capo_savingsplans.types.savings_plan_offering_rate

    out: list = []
    for item in value:
        out.append(
            capo_savingsplans.types.savings_plan_offering_rate.serialize_json(item)
        )
    return out


def deserialize_json(data: list) -> SavingsPlanOfferingRatesList:
    import capo_savingsplans.types.savings_plan_offering_rate

    out: SavingsPlanOfferingRatesList = []
    for item in data:
        if item is None:
            continue
        out.append(
            capo_savingsplans.types.savings_plan_offering_rate.deserialize_json(item)
        )
    return out
