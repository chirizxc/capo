...
class SigningConfiguration(TypedDict, closed=True):
    encryption_algorithm_options: (
        "capo_signer.types.encryption_algorithm_options.EncryptionAlgorithmOptions"
    )
    """<p>The encryption algorithm options that are available for a code-signing job.</p>"""
    hash_algorithm_options: (
        "capo_signer.types.hash_algorithm_options.HashAlgorithmOptions"
    )
...
def serialize_json(value: SigningConfiguration) -> dict:
    ...
    out["encryptionAlgorithmOptions"] = (
        capo_signer.types.encryption_algorithm_options.serialize_json(
    ...
    out["hashAlgorithmOptions"] = (
        capo_signer.types.hash_algorithm_options.serialize_json(
...
def deserialize_json(data: dict) -> SigningConfiguration:
    ...
    if data.get("encryptionAlgorithmOptions") is not None:
        ...
        out["encryption_algorithm_options"] = (
            capo_signer.types.encryption_algorithm_options.deserialize_json(
    ...
    if data.get("hashAlgorithmOptions") is not None:
        ...
        out["hash_algorithm_options"] = (
            capo_signer.types.hash_algorithm_options.deserialize_json(
