...
def handle_response(
    ...
    out: capo_signer.types.put_signing_profile_response.PutSigningProfileResponse = (
        capo_signer.types.put_signing_profile_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{profileName}", quote(input_["profile_name"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_signer.types.put_signing_profile_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
