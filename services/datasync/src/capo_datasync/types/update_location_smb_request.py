"""Generated from Smithy shape ``com.amazonaws.datasync#UpdateLocationSmbRequest``."""
...
class UpdateLocationSmbRequest(TypedDict, closed=True):
    ...
    """<p>Specifies a Kerberos prinicpal, which is an identity in your Kerberos realm that has permission to access the files, folders, and file metadata in your SMB file server.</p> <p>A Kerberos principal might look like <code>HOST/kerberosuser@MYDOMAIN.ORG</code>.</p> <p>Principal names are case sensitive. Your DataSync task execution will fail if the principal that you specify for this parameter doesn’t exactly match the principal that you use to create the keytab file.</p>"""
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: UpdateLocationSmbRequest) -> dict:
    out: dict = {}
