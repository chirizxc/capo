...
class ListResourceConfigurationsResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_vpc_lattice.types.resource_configuration_summary_list.ResourceConfigurationSummaryList"
    ]
...
def serialize_json(value: ListResourceConfigurationsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            capo_vpc_lattice.types.resource_configuration_summary_list.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListResourceConfigurationsResponse:
    ...
    if data.get("items") is not None:
        ...
        out["items"] = (
            capo_vpc_lattice.types.resource_configuration_summary_list.deserialize_json(
                data["items"]
            )
        )
