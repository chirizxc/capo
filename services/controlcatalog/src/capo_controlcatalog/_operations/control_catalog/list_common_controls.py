"""Generated from Smithy shape ``com.amazonaws.controlcatalog#ListCommonControls``."""
...
import capo_controlcatalog._protocol.eventstream
...
def handle_response(
    ...
    out: capo_controlcatalog.types.list_common_controls_response.ListCommonControlsResponse = capo_controlcatalog.types.list_common_controls_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_controlcatalog._auth._sigv4.build_sigv4_auth_scheme(
                "controlcatalog", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/common-controls"
    ...
    body: bytes | None = json.dumps(
        capo_controlcatalog.types.list_common_controls_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_common_controls(
