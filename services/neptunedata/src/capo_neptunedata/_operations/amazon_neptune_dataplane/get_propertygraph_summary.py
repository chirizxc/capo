...
def handle_response(
    ...
    out: capo_neptunedata.types.get_propertygraph_summary_output.GetPropertygraphSummaryOutput = capo_neptunedata.types.get_propertygraph_summary_output.deserialize_json(
...
def build_request(
    ...
    params.append(
        (
            "mode",
            capo_neptunedata.types.graph_summary_type.serialize_json(
                input_.get("mode", "basic")
            ),
        )
    )
