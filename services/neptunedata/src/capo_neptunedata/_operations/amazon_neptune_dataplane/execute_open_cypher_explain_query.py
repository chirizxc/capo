"""Generated from Smithy shape ``com.amazonaws.neptunedata#ExecuteOpenCypherExplainQuery``."""
...
def handle_response(
    ...
    out: capo_neptunedata.types.execute_open_cypher_explain_query_output.ExecuteOpenCypherExplainQueryOutput = {
        "results": b"".join(response.iter_raw())
...
async def async_handle_response(
    ...
    out: capo_neptunedata.types.execute_open_cypher_explain_query_output.ExecuteOpenCypherExplainQueryOutput = {
        "results": b"".join([chunk async for chunk in response.async_iter_raw()])
    }  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/opencypher/explain"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_neptunedata.types.execute_open_cypher_explain_query_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_execute_open_cypher_explain_query(
