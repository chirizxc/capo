...
import capo_neptunedata.errors.method_not_allowed_exception
...
import capo_neptunedata.errors.server_shutdown_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "MethodNotAllowedException":
            raise capo_neptunedata.errors.method_not_allowed_exception.MethodNotAllowedException.from_json(
                data, message
            )
        ...
        case "ServerShutdownException":
            raise capo_neptunedata.errors.server_shutdown_exception.ServerShutdownException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_neptunedata.types.execute_fast_reset_output.ExecuteFastResetOutput = (
        capo_neptunedata.types.execute_fast_reset_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/system"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_neptunedata.types.execute_fast_reset_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
