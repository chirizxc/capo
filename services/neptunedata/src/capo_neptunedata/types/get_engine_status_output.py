...
class GetEngineStatusOutput(TypedDict, closed=True):
    ...
    lab_mode: NotRequired["capo_neptunedata.types.string_valued_map.StringValuedMap"]
    ...
    settings: NotRequired["capo_neptunedata.types.string_valued_map.StringValuedMap"]
...
def serialize_json(value: GetEngineStatusOutput) -> dict:
    ...
    if "gremlin" in value:
        ...
        out["gremlin"] = capo_neptunedata.types.query_language_version.serialize_json(
            value["gremlin"]
        )
    if "sparql" in value:
        import capo_neptunedata.types.query_language_version

        out["sparql"] = capo_neptunedata.types.query_language_version.serialize_json(
            value["sparql"]
        )
    if "opencypher" in value:
        ...
        out["opencypher"] = (
            capo_neptunedata.types.query_language_version.serialize_json(
                value["opencypher"]
            )
        )
    if "lab_mode" in value:
        ...
        out["labMode"] = capo_neptunedata.types.string_valued_map.serialize_json(
            value["lab_mode"]
        )
    ...
    if "features" in value:
        ...
        out["features"] = capo_neptunedata.types.document_valued_map.serialize_json(
            value["features"]
        )
    if "settings" in value:
        import capo_neptunedata.types.string_valued_map

        out["settings"] = capo_neptunedata.types.string_valued_map.serialize_json(
            value["settings"]
        )
    return out
...
def deserialize_json(data: dict) -> GetEngineStatusOutput:
    ...
    if data.get("opencypher") is not None:
        ...
        out["opencypher"] = (
            capo_neptunedata.types.query_language_version.deserialize_json(
                data["opencypher"]
            )
        )
