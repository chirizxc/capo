...
if TYPE_CHECKING:
    import capo_pinpoint_sms_voice_v2.types.phone_number_filter
...
def serialize_aws_json_1_0(value: PhoneNumberFilterList) -> list:
    ...
    for item in value:
        out.append(
            capo_pinpoint_sms_voice_v2.types.phone_number_filter.serialize_aws_json_1_0(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_0(data: list) -> PhoneNumberFilterList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_pinpoint_sms_voice_v2.types.phone_number_filter.deserialize_aws_json_1_0(
                item
            )
        )
    return out
