"""Generated from Smithy shape ``com.amazonaws.pinpointsmsvoicev2#UpdateRcsAgentResult``."""
...
class UpdateRcsAgentResult(TypedDict, closed=True):
    rcs_agent_arn: "str"
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: UpdateRcsAgentResult) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_0(data: dict) -> UpdateRcsAgentResult:
    out: UpdateRcsAgentResult = {}  # type: ignore[typeddict-item]
    if data.get("RcsAgentArn") is not None:
        out["rcs_agent_arn"] = data["RcsAgentArn"]
    else:
        raise DeserializationError("UpdateRcsAgentResult.rcs_agent_arn required")
    if data.get("RcsAgentId") is not None:
        out["rcs_agent_id"] = data["RcsAgentId"]
    else:
        raise DeserializationError("UpdateRcsAgentResult.rcs_agent_id required")
    if data.get("Status") is not None:
        out["status"] = data["Status"]
    else:
        raise DeserializationError("UpdateRcsAgentResult.status required")
    if data.get("CreatedTimestamp") is not None:
        ...
        raise DeserializationError("UpdateRcsAgentResult.created_timestamp required")
