...
if TYPE_CHECKING:
    import capo_pinpoint_sms_voice_v2.types.spend_limit
...
def serialize_aws_json_1_0(value: SpendLimitList) -> list:
    ...
    for item in value:
        out.append(
            capo_pinpoint_sms_voice_v2.types.spend_limit.serialize_aws_json_1_0(item)
        )
    return out
...
def deserialize_aws_json_1_0(data: list) -> SpendLimitList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_pinpoint_sms_voice_v2.types.spend_limit.deserialize_aws_json_1_0(item)
        )
    return out
