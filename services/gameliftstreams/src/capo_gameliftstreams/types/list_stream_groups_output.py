...
class ListStreamGroupsOutput(TypedDict, closed=True):
    items: NotRequired[
        "capo_gameliftstreams.types.stream_group_summary_list.StreamGroupSummaryList"
    ]
...
def serialize_json(value: ListStreamGroupsOutput) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = (
            capo_gameliftstreams.types.stream_group_summary_list.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListStreamGroupsOutput:
    ...
    if data.get("Items") is not None:
        ...
        out["items"] = (
            capo_gameliftstreams.types.stream_group_summary_list.deserialize_json(
                data["Items"]
            )
        )
