...
if TYPE_CHECKING:
    ...
    import capo_gameliftstreams.types.vpc_id


class VpcTransitConfigurationResponse(TypedDict, closed=True):
    vpc_id: NotRequired["capo_gameliftstreams.types.vpc_id.VpcId"]
    """<p>The ID of the Amazon VPC that is connected to the stream group.</p>"""
    ipv4_cidr_blocks: NotRequired[
        "capo_gameliftstreams.types.ipv4_cidr_block_list.Ipv4CidrBlockList"
    ]
...
def serialize_json(value: VpcTransitConfigurationResponse) -> dict:
    ...
    if "ipv4_cidr_blocks" in value:
        ...
        out["Ipv4CidrBlocks"] = (
            capo_gameliftstreams.types.ipv4_cidr_block_list.serialize_json(
...
def deserialize_json(data: dict) -> VpcTransitConfigurationResponse:
    ...
    if data.get("Ipv4CidrBlocks") is not None:
        ...
        out["ipv4_cidr_blocks"] = (
            capo_gameliftstreams.types.ipv4_cidr_block_list.deserialize_json(
                data["Ipv4CidrBlocks"]
            )
        )
    if data.get("TransitGatewayId") is not None:
        out["transit_gateway_id"] = data["TransitGatewayId"]
    if data.get("TransitGatewayResourceShareArn") is not None:
