"""Generated from Smithy shape ``com.amazonaws.gameliftstreams#ListStreamSessionsByAccount``."""
...
import capo_gameliftstreams._protocol.eventstream
...
def handle_response(
    ...
    out: capo_gameliftstreams.types.list_stream_sessions_by_account_output.ListStreamSessionsByAccountOutput = capo_gameliftstreams.types.list_stream_sessions_by_account_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_gameliftstreams._auth._sigv4.build_sigv4_auth_scheme(
                "gameliftstreams", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/streamsessions"
    params: list[tuple[str, str]] = []
    if "status" in input_:
        params.append(
            (
                "Status",
                capo_gameliftstreams.types.stream_session_status.serialize_json(
                    input_["status"]
                ),
            )
        )
    if "export_files_status" in input_:
        params.append(
            (
                "ExportFilesStatus",
                capo_gameliftstreams.types.export_files_status.serialize_json(
                    input_["export_files_status"]
                ),
            )
        )
...
async def async_list_stream_sessions_by_account(
