...
def serialize_aws_json_1_0(value: CreateProxyRulesRequest) -> dict:
    ...
    out["Rules"] = (
        capo_network_firewall.types.create_proxy_rules_by_request_phase.serialize_aws_json_1_0(
            value["rules"]
        )
    )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> CreateProxyRulesRequest:
    ...
    if data.get("Rules") is not None:
        ...
        out["rules"] = (
            capo_network_firewall.types.create_proxy_rules_by_request_phase.deserialize_aws_json_1_0(
                data["Rules"]
            )
        )
    else:
