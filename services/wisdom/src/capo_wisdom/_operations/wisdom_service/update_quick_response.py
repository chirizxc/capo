...
def handle_response(
    ...
    out: capo_wisdom.types.update_quick_response_response.UpdateQuickResponseResponse = capo_wisdom.types.update_quick_response_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{quickResponseId}", quote(input_["quick_response_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wisdom.types.update_quick_response_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
