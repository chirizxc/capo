"""Generated from Smithy shape ``com.amazonaws.wisdom#RecommendationTrigger``."""

from typing import TYPE_CHECKING

from typing_extensions import TypedDict

from capo_wisdom.errors import DeserializationError

if TYPE_CHECKING:
    import capo_wisdom.types.recommendation_id_list
    import capo_wisdom.types.recommendation_source_type
    import capo_wisdom.types.recommendation_trigger_data
    import capo_wisdom.types.recommendation_trigger_type
    import capo_wisdom.types.uuid


class RecommendationTrigger(TypedDict, closed=True):
    id: "capo_wisdom.types.uuid.Uuid"
    """<p>The identifier of the recommendation trigger.</p>"""
    type: "capo_wisdom.types.recommendation_trigger_type.RecommendationTriggerType"
    """<p>The type of recommendation trigger.</p>"""
    source: "capo_wisdom.types.recommendation_source_type.RecommendationSourceType"
    """<p>The source of the recommendation trigger.</p> <ul> <li> <p>ISSUE_DETECTION: The corresponding recommendations were triggered by a Contact Lens issue.</p> </li> <li> <p>RULE_EVALUATION: The corresponding recommendations were triggered by a Contact Lens rule.</p> </li> </ul>"""
    data: "capo_wisdom.types.recommendation_trigger_data.RecommendationTriggerData"
    """<p>A union type containing information related to the trigger.</p>"""
    recommendation_ids: "capo_wisdom.types.recommendation_id_list.RecommendationIdList"
    """<p>The identifiers of the recommendations.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: RecommendationTrigger) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    out["type"] = value["type"]
    out["source"] = value["source"]
    import capo_wisdom.types.recommendation_trigger_data

    out["data"] = capo_wisdom.types.recommendation_trigger_data.serialize_json(
        value["data"]
    )
    import capo_wisdom.types.recommendation_id_list

    out["recommendationIds"] = capo_wisdom.types.recommendation_id_list.serialize_json(
        value["recommendation_ids"]
    )
    return out


def deserialize_json(data: dict) -> RecommendationTrigger:
    out: RecommendationTrigger = {}  # type: ignore[typeddict-item]
    if data.get("id") is not None:
        out["id"] = data["id"]
    else:
        raise DeserializationError("RecommendationTrigger.id required")
    if data.get("type") is not None:
        out["type"] = data["type"]
    else:
        raise DeserializationError("RecommendationTrigger.type required")
    if data.get("source") is not None:
        out["source"] = data["source"]
    else:
        raise DeserializationError("RecommendationTrigger.source required")
    if data.get("data") is not None:
        import capo_wisdom.types.recommendation_trigger_data

        out["data"] = capo_wisdom.types.recommendation_trigger_data.deserialize_json(
            data["data"]
        )
    else:
        raise DeserializationError("RecommendationTrigger.data required")
    if data.get("recommendationIds") is not None:
        import capo_wisdom.types.recommendation_id_list

        out["recommendation_ids"] = (
            capo_wisdom.types.recommendation_id_list.deserialize_json(
                data["recommendationIds"]
            )
        )
    else:
        raise DeserializationError("RecommendationTrigger.recommendation_ids required")
    return out
