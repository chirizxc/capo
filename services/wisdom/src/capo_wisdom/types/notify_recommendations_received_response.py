...
class NotifyRecommendationsReceivedResponse(TypedDict, closed=True):
    recommendation_ids: NotRequired[
        "capo_wisdom.types.recommendation_id_list.RecommendationIdList"
    ]
    """<p>The identifiers of the recommendations.</p>"""
    errors: NotRequired[
        "capo_wisdom.types.notify_recommendations_received_error_list.NotifyRecommendationsReceivedErrorList"
    ]
...
def serialize_json(value: NotifyRecommendationsReceivedResponse) -> dict:
    ...
    if "recommendation_ids" in value:
        ...
        out["recommendationIds"] = (
            capo_wisdom.types.recommendation_id_list.serialize_json(
    ...
    if "errors" in value:
        ...
        out["errors"] = (
            capo_wisdom.types.notify_recommendations_received_error_list.serialize_json(
                value["errors"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> NotifyRecommendationsReceivedResponse:
    ...
    if data.get("errors") is not None:
        ...
        out["errors"] = (
            capo_wisdom.types.notify_recommendations_received_error_list.deserialize_json(
                data["errors"]
            )
        )
    return out
