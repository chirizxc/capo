"""Generated from Smithy shape ``com.amazonaws.migrationhubrefactorspaces#ListEnvironmentVpcs``."""
...
import capo_migration_hub_refactor_spaces._protocol.eventstream
...
def handle_response(
    ...
    out: capo_migration_hub_refactor_spaces.types.list_environment_vpcs_response.ListEnvironmentVpcsResponse = capo_migration_hub_refactor_spaces.types.list_environment_vpcs_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = (
                capo_migration_hub_refactor_spaces._auth._sigv4.build_sigv4_auth_scheme(
                    "refactor-spaces", options.region, endpoint_scheme
                )
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/environments/{EnvironmentIdentifier}/vpcs"
    url = url.replace(
...
async def async_list_environment_vpcs(
