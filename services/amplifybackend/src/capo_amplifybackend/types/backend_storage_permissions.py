"""Generated from Smithy shape ``com.amazonaws.amplifybackend#BackendStoragePermissions``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_amplifybackend.types.list_of_authenticated_element
    import capo_amplifybackend.types.list_of_un_authenticated_element


class BackendStoragePermissions(TypedDict, closed=True):
    authenticated: NotRequired[
        "capo_amplifybackend.types.list_of_authenticated_element.ListOfAuthenticatedElement"
    ]
    """<p>Lists all authenticated user read, write, and delete permissions for your S3 bucket.</p>"""
    un_authenticated: NotRequired[
        "capo_amplifybackend.types.list_of_un_authenticated_element.ListOfUnAuthenticatedElement"
    ]
    """<p>Lists all unauthenticated user read, write, and delete permissions for your S3 bucket.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: BackendStoragePermissions) -> dict:
    out: dict = {}
    if "authenticated" in value:
        import capo_amplifybackend.types.list_of_authenticated_element

        out["authenticated"] = (
            capo_amplifybackend.types.list_of_authenticated_element.serialize_json(
                value["authenticated"]
            )
        )
    if "un_authenticated" in value:
        import capo_amplifybackend.types.list_of_un_authenticated_element

        out["unAuthenticated"] = (
            capo_amplifybackend.types.list_of_un_authenticated_element.serialize_json(
                value["un_authenticated"]
            )
        )
    return out


def deserialize_json(data: dict) -> BackendStoragePermissions:
    out: BackendStoragePermissions = {}  # type: ignore[typeddict-item]
    if data.get("authenticated") is not None:
        import capo_amplifybackend.types.list_of_authenticated_element

        out["authenticated"] = (
            capo_amplifybackend.types.list_of_authenticated_element.deserialize_json(
                data["authenticated"]
            )
        )
    if data.get("unAuthenticated") is not None:
        import capo_amplifybackend.types.list_of_un_authenticated_element

        out["un_authenticated"] = (
            capo_amplifybackend.types.list_of_un_authenticated_element.deserialize_json(
                data["unAuthenticated"]
            )
        )
    return out
