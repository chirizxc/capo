...
if TYPE_CHECKING:
    import capo_amplifybackend.types.authenticated_element

ListOfAuthenticatedElement: TypeAlias = list[
    "capo_amplifybackend.types.authenticated_element.AuthenticatedElement"
]
...
def serialize_json(value: ListOfAuthenticatedElement) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_amplifybackend.types.authenticated_element.serialize_json(item))
    return out
