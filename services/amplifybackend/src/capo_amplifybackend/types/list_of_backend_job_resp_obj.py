...
if TYPE_CHECKING:
    import capo_amplifybackend.types.backend_job_resp_obj

ListOfBackendJobRespObj: TypeAlias = list[
    "capo_amplifybackend.types.backend_job_resp_obj.BackendJobRespObj"
]
...
def serialize_json(value: ListOfBackendJobRespObj) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_amplifybackend.types.backend_job_resp_obj.serialize_json(item))
    return out
