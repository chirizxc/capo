...
CostAndUsageComparisons: TypeAlias = list[
    "capo_cost_explorer.types.cost_and_usage_comparison.CostAndUsageComparison"
]
...
def serialize_aws_json_1_1(value: CostAndUsageComparisons) -> list:
    ...
    for item in value:
        out.append(
            capo_cost_explorer.types.cost_and_usage_comparison.serialize_aws_json_1_1(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> CostAndUsageComparisons:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_cost_explorer.types.cost_and_usage_comparison.deserialize_aws_json_1_1(
                item
            )
        )
    return out
