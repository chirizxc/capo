"""Generated from Smithy shape ``com.amazonaws.s3control#DescribeMultiRegionAccessPointOperation``."""
...
import capo_s3_control.types.async_operation
...
def handle_response(
    ...
    out: capo_s3_control.types.describe_multi_region_access_point_operation_result.DescribeMultiRegionAccessPointOperationResult = capo_s3_control.types.describe_multi_region_access_point_operation_result.deserialize_xml(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v20180820/async-requests/mrap/{RequestTokenARN+}"
    url = url.replace(
        "{RequestTokenARN+}", quote(input_["request_token_arn"], safe="/")
    )
...
async def async_describe_multi_region_access_point_operation(
