...
def build_request(
    ...
    if "confirmation_required" in input_:
        SubElement(root, "ConfirmationRequired").text = (
            "true" if input_["confirmation_required"] else "false"
        )
    ...
    if "client_request_token" in input_:
        SubElement(root, "ClientRequestToken").text = input_["client_request_token"]
    ...
    if "description" in input_:
        SubElement(root, "Description").text = input_["description"]
    if "priority" in input_:
        SubElement(root, "Priority").text = str(input_["priority"])
    if "role_arn" in input_:
        SubElement(root, "RoleArn").text = input_["role_arn"]
    if "tags" in input_:
