"""Generated from Smithy shape ``com.amazonaws.s3control#GetDataAccess``."""
...
def handle_response(
    ...
    out: capo_s3_control.types.get_data_access_result.GetDataAccessResult = (
        capo_s3_control.types.get_data_access_result.deserialize_xml(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v20180820/accessgrantsinstance/dataaccess"
    ...
    if "permission" in input_:
        params.append(
            (
                "permission",
                capo_s3_control.types.permission.to_xml_text(input_["permission"]),
            )
        )
    if "duration_seconds" in input_:
        params.append(("durationSeconds", str(input_["duration_seconds"])))
    if "privilege" in input_:
        params.append(
            (
                "privilege",
                capo_s3_control.types.privilege.to_xml_text(input_["privilege"]),
            )
        )
    if "target_type" in input_:
        params.append(
            (
                "targetType",
                capo_s3_control.types.s3_prefix_type.to_xml_text(input_["target_type"]),
            )
        )
    if "audit_context" in input_:
        params.append(("auditContext", input_["audit_context"]))
...
def get_data_access(
    ...
    capo_s3_control.types.get_data_access_result.GetDataAccessResult, zapros.Response
]:
...
async def async_get_data_access(
