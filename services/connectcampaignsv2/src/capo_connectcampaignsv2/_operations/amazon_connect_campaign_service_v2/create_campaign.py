"""Generated from Smithy shape ``com.amazonaws.connectcampaignsv2#CreateCampaign``."""
...
import capo_connectcampaignsv2._protocol.eventstream
...
import capo_connectcampaignsv2.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ServiceQuotaExceededException":
            raise capo_connectcampaignsv2.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_connectcampaignsv2.types.create_campaign_response.CreateCampaignResponse = capo_connectcampaignsv2.types.create_campaign_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_connectcampaignsv2._auth._sigv4.build_sigv4_auth_scheme(
                "connect-campaigns", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/campaigns"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connectcampaignsv2.types.create_campaign_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
