"""Generated from Smithy shape ``com.amazonaws.connectcampaignsv2#PutProfileOutboundRequestBatch``."""
...
def handle_response(
    ...
    out: capo_connectcampaignsv2.types.put_profile_outbound_request_batch_response.PutProfileOutboundRequestBatchResponse = capo_connectcampaignsv2.types.put_profile_outbound_request_batch_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/campaigns/{id}/profile-outbound-requests"
    url = url.replace("{id}", quote(input_["id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connectcampaignsv2.types.put_profile_outbound_request_batch_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_put_profile_outbound_request_batch(
