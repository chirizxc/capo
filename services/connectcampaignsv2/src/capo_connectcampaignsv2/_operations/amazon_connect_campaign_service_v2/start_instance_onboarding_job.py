"""Generated from Smithy shape ``com.amazonaws.connectcampaignsv2#StartInstanceOnboardingJob``."""
...
def handle_response(
    ...
    out: capo_connectcampaignsv2.types.start_instance_onboarding_job_response.StartInstanceOnboardingJobResponse = capo_connectcampaignsv2.types.start_instance_onboarding_job_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v2/connect-instance/{connectInstanceId}/onboarding"
    )
    url = url.replace(
        "{connectInstanceId}", quote(input_["connect_instance_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connectcampaignsv2.types.start_instance_onboarding_job_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
