...
if TYPE_CHECKING:
    import capo_connectcampaignsv2.types.profile_outbound_request

ProfileOutboundRequestList: TypeAlias = list[
    "capo_connectcampaignsv2.types.profile_outbound_request.ProfileOutboundRequest"
]
...
def serialize_json(value: ProfileOutboundRequestList) -> list:
    ...
    for item in value:
        out.append(
            capo_connectcampaignsv2.types.profile_outbound_request.serialize_json(item)
        )
    return out
