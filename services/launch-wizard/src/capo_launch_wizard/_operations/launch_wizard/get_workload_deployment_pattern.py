"""Generated from Smithy shape ``com.amazonaws.launchwizard#GetWorkloadDeploymentPattern``."""
...
def handle_response(
    ...
    out: capo_launch_wizard.types.get_workload_deployment_pattern_output.GetWorkloadDeploymentPatternOutput = capo_launch_wizard.types.get_workload_deployment_pattern_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/getWorkloadDeploymentPattern"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_launch_wizard.types.get_workload_deployment_pattern_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_workload_deployment_pattern(
