"""Generated from Smithy shape ``com.amazonaws.launchwizard#ListWorkloadDeploymentPatterns``."""
...
def handle_response(
    ...
    out: capo_launch_wizard.types.list_workload_deployment_patterns_output.ListWorkloadDeploymentPatternsOutput = capo_launch_wizard.types.list_workload_deployment_patterns_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listWorkloadDeploymentPatterns"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_launch_wizard.types.list_workload_deployment_patterns_input.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_workload_deployment_patterns(
