...
class ListWorkloadsOutput(TypedDict, closed=True):
    workloads: NotRequired[
        "capo_launch_wizard.types.workload_data_summary_list.WorkloadDataSummaryList"
    ]
...
def serialize_json(value: ListWorkloadsOutput) -> dict:
    ...
    if "workloads" in value:
        ...
        out["workloads"] = (
            capo_launch_wizard.types.workload_data_summary_list.serialize_json(
                value["workloads"]
            )
        )
...
def deserialize_json(data: dict) -> ListWorkloadsOutput:
    ...
    if data.get("workloads") is not None:
        import capo_launch_wizard.types.workload_data_summary_list

        out["workloads"] = (
            capo_launch_wizard.types.workload_data_summary_list.deserialize_json(
                data["workloads"]
            )
        )
