...
class ListDeploymentsOutput(TypedDict, closed=True):
    deployments: NotRequired[
        "capo_launch_wizard.types.deployment_data_summary_list.DeploymentDataSummaryList"
    ]
...
def serialize_json(value: ListDeploymentsOutput) -> dict:
    ...
    if "deployments" in value:
        ...
        out["deployments"] = (
            capo_launch_wizard.types.deployment_data_summary_list.serialize_json(
                value["deployments"]
            )
        )
...
def deserialize_json(data: dict) -> ListDeploymentsOutput:
    ...
    if data.get("deployments") is not None:
        ...
        out["deployments"] = (
            capo_launch_wizard.types.deployment_data_summary_list.deserialize_json(
                data["deployments"]
            )
        )
