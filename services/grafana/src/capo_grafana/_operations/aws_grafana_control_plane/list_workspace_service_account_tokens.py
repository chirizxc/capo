"""Generated from Smithy shape ``com.amazonaws.grafana#ListWorkspaceServiceAccountTokens``."""
...
import capo_grafana._protocol.eventstream
...
def handle_response(
    ...
    out: capo_grafana.types.list_workspace_service_account_tokens_response.ListWorkspaceServiceAccountTokensResponse = capo_grafana.types.list_workspace_service_account_tokens_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_grafana._auth._sigv4.build_sigv4_auth_scheme(
                "grafana", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    url = url.replace(
        "{serviceAccountId}", quote(input_["service_account_id"], safe="")
    )
...
async def async_list_workspace_service_account_tokens(
