...
PermissionEntryList: TypeAlias = list[
    "capo_grafana.types.permission_entry.PermissionEntry"
]
...
def serialize_json(value: PermissionEntryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_grafana.types.permission_entry.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> PermissionEntryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_grafana.types.permission_entry.deserialize_json(item))
    return out
