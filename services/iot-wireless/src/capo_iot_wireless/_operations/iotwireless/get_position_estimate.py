"""Generated from Smithy shape ``com.amazonaws.iotwireless#GetPositionEstimate``."""
...
def handle_response(
    ...
    out: capo_iot_wireless.types.get_position_estimate_response.GetPositionEstimateResponse = {
        "geo_json_payload": b"".join(response.iter_raw())
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/position-estimate"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_wireless.types.get_position_estimate_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_get_position_estimate(
