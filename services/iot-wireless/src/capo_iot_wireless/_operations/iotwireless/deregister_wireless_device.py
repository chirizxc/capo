"""Generated from Smithy shape ``com.amazonaws.iotwireless#DeregisterWirelessDevice``."""
...
def handle_response(
    ...
    out: capo_iot_wireless.types.deregister_wireless_device_response.DeregisterWirelessDeviceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/wireless-devices/{Identifier}/deregister"
    ...
    if "wireless_device_type" in input_:
        params.append(
            (
                "WirelessDeviceType",
                capo_iot_wireless.types.wireless_device_type.serialize_json(
                    input_["wireless_device_type"]
                ),
            )
        )
...
async def async_deregister_wireless_device(
