...
class CreateDeviceProfileRequest(TypedDict, closed=True):
...
def serialize_json(value: CreateDeviceProfileRequest) -> dict:
    ...
    if "sidewalk" in value:
        ...
        out["Sidewalk"] = (
            capo_iot_wireless.types.sidewalk_create_device_profile.serialize_json(
                value["sidewalk"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> CreateDeviceProfileRequest:
    ...
    if data.get("Sidewalk") is not None:
        ...
        out["sidewalk"] = (
            capo_iot_wireless.types.sidewalk_create_device_profile.deserialize_json(
                data["Sidewalk"]
            )
        )
    return out
