...
if TYPE_CHECKING:
    ...
    import capo_iot_wireless.types.f_nwk_s_int_key
    import capo_iot_wireless.types.nwk_s_enc_key
    import capo_iot_wireless.types.s_nwk_s_int_key


class SessionKeysAbpV1_1(TypedDict, closed=True):
    f_nwk_s_int_key: NotRequired["capo_iot_wireless.types.f_nwk_s_int_key.FNwkSIntKey"]
    """<p>The FNwkSIntKey value.</p>"""
    s_nwk_s_int_key: NotRequired["capo_iot_wireless.types.s_nwk_s_int_key.SNwkSIntKey"]
    """<p>The SNwkSIntKey value.</p>"""
    nwk_s_enc_key: NotRequired["capo_iot_wireless.types.nwk_s_enc_key.NwkSEncKey"]
...
def deserialize_json(data: dict) -> SessionKeysAbpV1_1:
    ...
    if data.get("FNwkSIntKey") is not None:
        out["f_nwk_s_int_key"] = data["FNwkSIntKey"]
    if data.get("SNwkSIntKey") is not None:
        out["s_nwk_s_int_key"] = data["SNwkSIntKey"]
    if data.get("NwkSEncKey") is not None:
