...
if TYPE_CHECKING:
    import capo_iot_wireless.types.lo_ra_wan_update_gateway_task_create
    import capo_iot_wireless.types.update_data_source


class UpdateWirelessGatewayTaskCreate(TypedDict, closed=True):
    update_data_source: NotRequired[
        "capo_iot_wireless.types.update_data_source.UpdateDataSource"
    ]
    ...
    lo_ra_wan: NotRequired[
        "capo_iot_wireless.types.lo_ra_wan_update_gateway_task_create.LoRaWANUpdateGatewayTaskCreate"
    ]
...
def serialize_json(value: UpdateWirelessGatewayTaskCreate) -> dict:
    ...
    if "lo_ra_wan" in value:
        ...
        out["LoRaWAN"] = (
            capo_iot_wireless.types.lo_ra_wan_update_gateway_task_create.serialize_json(
                value["lo_ra_wan"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> UpdateWirelessGatewayTaskCreate:
    ...
    if data.get("UpdateDataSource") is not None:
        out["update_data_source"] = data["UpdateDataSource"]
    if data.get("UpdateDataRole") is not None:
        out["update_data_role"] = data["UpdateDataRole"]
    if data.get("LoRaWAN") is not None:
        import capo_iot_wireless.types.lo_ra_wan_update_gateway_task_create

        out["lo_ra_wan"] = (
            capo_iot_wireless.types.lo_ra_wan_update_gateway_task_create.deserialize_json(
                data["LoRaWAN"]
            )
        )
    return out
