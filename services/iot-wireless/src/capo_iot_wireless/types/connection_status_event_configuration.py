...
if TYPE_CHECKING:
    ...
    import capo_iot_wireless.types.lo_ra_wan_connection_status_event_notification_configurations


class ConnectionStatusEventConfiguration(TypedDict, closed=True):
    lo_ra_wan: NotRequired[
        "capo_iot_wireless.types.lo_ra_wan_connection_status_event_notification_configurations.LoRaWANConnectionStatusEventNotificationConfigurations"
    ]
...
def serialize_json(value: ConnectionStatusEventConfiguration) -> dict:
    ...
    if "lo_ra_wan" in value:
        ...
        out["LoRaWAN"] = (
            capo_iot_wireless.types.lo_ra_wan_connection_status_event_notification_configurations.serialize_json(
                value["lo_ra_wan"]
            )
        )
...
def deserialize_json(data: dict) -> ConnectionStatusEventConfiguration:
    ...
    if data.get("LoRaWAN") is not None:
        ...
        out["lo_ra_wan"] = (
            capo_iot_wireless.types.lo_ra_wan_connection_status_event_notification_configurations.deserialize_json(
                data["LoRaWAN"]
            )
        )
    if data.get("WirelessGatewayIdEventTopic") is not None:
        import capo_iot_wireless.types.event_notification_topic_status
