"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DeleteAppVersionResourceResponse``."""
...
class DeleteAppVersionResourceResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: DeleteAppVersionResourceResponse) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> DeleteAppVersionResourceResponse:
    out: DeleteAppVersionResourceResponse = {}  # type: ignore[typeddict-item]
    if data.get("appArn") is not None:
        out["app_arn"] = data["appArn"]
    else:
        raise DeserializationError("DeleteAppVersionResourceResponse.app_arn required")
    if data.get("appVersion") is not None:
        out["app_version"] = data["appVersion"]
    else:
        raise DeserializationError(
            "DeleteAppVersionResourceResponse.app_version required"
        )
