...
if TYPE_CHECKING:
    import capo_resiliencehub.types.component_recommendation

ComponentRecommendationList: TypeAlias = list[
    "capo_resiliencehub.types.component_recommendation.ComponentRecommendation"
]
...
def serialize_json(value: ComponentRecommendationList) -> list:
    ...
    for item in value:
        out.append(
            capo_resiliencehub.types.component_recommendation.serialize_json(item)
        )
    return out
