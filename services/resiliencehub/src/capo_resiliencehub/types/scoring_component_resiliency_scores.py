...
ScoringComponentResiliencyScores: TypeAlias = dict[
    "capo_resiliencehub.types.resiliency_score_type.ResiliencyScoreType",
    "capo_resiliencehub.types.scoring_component_resiliency_score.ScoringComponentResiliencyScore",
]
...
def serialize_json(input_to_serialize: ScoringComponentResiliencyScores) -> dict:
    ...
    for key, value in input_to_serialize.items():
        ...
        out[capo_resiliencehub.types.resiliency_score_type.serialize_json(key)] = (
            capo_resiliencehub.types.scoring_component_resiliency_score.serialize_json(
                value
            )
        )
    return out
...
def deserialize_json(data: dict) -> ScoringComponentResiliencyScores:
    ...
    for key, value in data.items():
        ...
        out[capo_resiliencehub.types.resiliency_score_type.deserialize_json(key)] = (
            capo_resiliencehub.types.scoring_component_resiliency_score.deserialize_json(
                value
            )
        )
    return out
