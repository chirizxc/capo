...
if TYPE_CHECKING:
    import capo_resiliencehub.types.assessment_risk_recommendation_list
    import capo_resiliencehub.types.string500


class AssessmentSummary(TypedDict, closed=True):
    summary: NotRequired["capo_resiliencehub.types.string500.String500"]
    ...
    risk_recommendations: NotRequired[
        "capo_resiliencehub.types.assessment_risk_recommendation_list.AssessmentRiskRecommendationList"
    ]
...
def serialize_json(value: AssessmentSummary) -> dict:
    ...
    if "risk_recommendations" in value:
        ...
        out["riskRecommendations"] = (
            capo_resiliencehub.types.assessment_risk_recommendation_list.serialize_json(
...
def deserialize_json(data: dict) -> AssessmentSummary:
    ...
    if data.get("riskRecommendations") is not None:
        import capo_resiliencehub.types.assessment_risk_recommendation_list

        out["risk_recommendations"] = (
            capo_resiliencehub.types.assessment_risk_recommendation_list.deserialize_json(
