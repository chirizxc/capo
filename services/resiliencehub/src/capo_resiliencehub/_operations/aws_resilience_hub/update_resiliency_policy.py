"""Generated from Smithy shape ``com.amazonaws.resiliencehub#UpdateResiliencyPolicy``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.update_resiliency_policy_response.UpdateResiliencyPolicyResponse = capo_resiliencehub.types.update_resiliency_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/update-resiliency-policy"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.update_resiliency_policy_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_resiliency_policy(
