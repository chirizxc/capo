"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ResolveAppVersionResources``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.resolve_app_version_resources_response.ResolveAppVersionResourcesResponse = capo_resiliencehub.types.resolve_app_version_resources_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/resolve-app-version-resources"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.resolve_app_version_resources_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_resolve_app_version_resources(
