"""Generated from Smithy shape ``com.amazonaws.resiliencehub#CreateResiliencyPolicy``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.create_resiliency_policy_response.CreateResiliencyPolicyResponse = capo_resiliencehub.types.create_resiliency_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/create-resiliency-policy"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.create_resiliency_policy_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_resiliency_policy(
