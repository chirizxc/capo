"""Generated from Smithy shape ``com.amazonaws.resiliencehub#UpdateAppVersionResource``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.update_app_version_resource_response.UpdateAppVersionResourceResponse = capo_resiliencehub.types.update_app_version_resource_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/update-app-version-resource"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.update_app_version_resource_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_app_version_resource(
