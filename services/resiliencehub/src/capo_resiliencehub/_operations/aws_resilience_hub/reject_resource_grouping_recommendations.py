"""Generated from Smithy shape ``com.amazonaws.resiliencehub#RejectResourceGroupingRecommendations``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.reject_resource_grouping_recommendations_response.RejectResourceGroupingRecommendationsResponse = capo_resiliencehub.types.reject_resource_grouping_recommendations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/reject-resource-grouping-recommendations"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.reject_resource_grouping_recommendations_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_reject_resource_grouping_recommendations(
