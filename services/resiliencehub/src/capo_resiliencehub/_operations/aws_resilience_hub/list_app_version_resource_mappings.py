"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ListAppVersionResourceMappings``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.list_app_version_resource_mappings_response.ListAppVersionResourceMappingsResponse = capo_resiliencehub.types.list_app_version_resource_mappings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-app-version-resource-mappings"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.list_app_version_resource_mappings_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_app_version_resource_mappings(
