"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ListAppComponentRecommendations``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.list_app_component_recommendations_response.ListAppComponentRecommendationsResponse = capo_resiliencehub.types.list_app_component_recommendations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-app-component-recommendations"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.list_app_component_recommendations_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_app_component_recommendations(
