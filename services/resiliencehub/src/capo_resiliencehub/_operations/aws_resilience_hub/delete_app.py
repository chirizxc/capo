"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DeleteApp``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.delete_app_response.DeleteAppResponse = (
        capo_resiliencehub.types.delete_app_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-app"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.delete_app_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
def delete_app(
    ...
    capo_resiliencehub.types.delete_app_response.DeleteAppResponse, zapros.Response
]:
