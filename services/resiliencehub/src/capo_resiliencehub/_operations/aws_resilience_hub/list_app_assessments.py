...
def build_request(
    ...
    if "app_arn" in input_:
        params.append(("appArn", input_["app_arn"]))
    if "assessment_name" in input_:
        params.append(("assessmentName", input_["assessment_name"]))
    if "assessment_status" in input_:
        for item in input_["assessment_status"]:
            params.append(
                (
                    "assessmentStatus",
                    capo_resiliencehub.types.assessment_status.serialize_json(item),
                )
            )
    ...
    if "invoker" in input_:
        params.append(
            (
                "invoker",
                capo_resiliencehub.types.assessment_invoker.serialize_json(
                    input_["invoker"]
                ),
            )
        )
