"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DescribeDraftAppVersionResourcesImportStatus``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.describe_draft_app_version_resources_import_status_response.DescribeDraftAppVersionResourcesImportStatusResponse = capo_resiliencehub.types.describe_draft_app_version_resources_import_status_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/describe-draft-app-version-resources-import-status"
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.describe_draft_app_version_resources_import_status_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_describe_draft_app_version_resources_import_status(
