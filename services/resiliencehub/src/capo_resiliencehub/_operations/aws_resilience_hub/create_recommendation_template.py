"""Generated from Smithy shape ``com.amazonaws.resiliencehub#CreateRecommendationTemplate``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.create_recommendation_template_response.CreateRecommendationTemplateResponse = capo_resiliencehub.types.create_recommendation_template_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/create-recommendation-template"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.create_recommendation_template_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_recommendation_template(
