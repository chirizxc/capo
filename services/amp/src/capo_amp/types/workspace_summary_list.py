...
WorkspaceSummaryList: TypeAlias = list[
    "capo_amp.types.workspace_summary.WorkspaceSummary"
]
...
def serialize_json(value: WorkspaceSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_amp.types.workspace_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> WorkspaceSummaryList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_amp.types.workspace_summary.deserialize_json(item))
    return out
