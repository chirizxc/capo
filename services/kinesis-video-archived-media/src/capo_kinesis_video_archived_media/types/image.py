...
if TYPE_CHECKING:
    import capo_kinesis_video_archived_media.types.image_content
...
class Image(TypedDict, closed=True):
    ...
    error: NotRequired["capo_kinesis_video_archived_media.types.image_error.ImageError"]
    ...
    image_content: NotRequired[
        "capo_kinesis_video_archived_media.types.image_content.ImageContent"
    ]
...
def serialize_json(value: Image) -> dict:
    ...
    if "error" in value:
        ...
        out["Error"] = (
            capo_kinesis_video_archived_media.types.image_error.serialize_json(
                value["error"]
            )
        )
...
def deserialize_json(data: dict) -> Image:
    ...
    if data.get("Error") is not None:
        ...
        out["error"] = (
            capo_kinesis_video_archived_media.types.image_error.deserialize_json(
                data["Error"]
            )
        )
