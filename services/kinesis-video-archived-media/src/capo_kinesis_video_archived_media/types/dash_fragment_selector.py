...
class DASHFragmentSelector(TypedDict, closed=True):
    fragment_selector_type: NotRequired[
        "capo_kinesis_video_archived_media.types.dash_fragment_selector_type.DASHFragmentSelectorType"
    ]
    ...
    timestamp_range: NotRequired[
        "capo_kinesis_video_archived_media.types.dash_timestamp_range.DASHTimestampRange"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: DASHFragmentSelector) -> dict:
    out: dict = {}
    if "fragment_selector_type" in value:
        import capo_kinesis_video_archived_media.types.dash_fragment_selector_type

        out["FragmentSelectorType"] = (
            capo_kinesis_video_archived_media.types.dash_fragment_selector_type.serialize_json(
    ...
    if "timestamp_range" in value:
        ...
        out["TimestampRange"] = (
            capo_kinesis_video_archived_media.types.dash_timestamp_range.serialize_json(
...
def deserialize_json(data: dict) -> DASHFragmentSelector:
    ...
    if data.get("FragmentSelectorType") is not None:
        ...
        out["fragment_selector_type"] = (
            capo_kinesis_video_archived_media.types.dash_fragment_selector_type.deserialize_json(
    ...
    if data.get("TimestampRange") is not None:
        ...
        out["timestamp_range"] = (
            capo_kinesis_video_archived_media.types.dash_timestamp_range.deserialize_json(
                data["TimestampRange"]
            )
        )
    return out
